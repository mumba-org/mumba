...
class SimpleCatalog : public Catalog {
 public:
  ...
  // this catalog.
  void AddTable(const std::string& name, const Table* table) ABSL_LOCKS_EXCLUDED(mutex_);
  ...
  void AddOwnedTable(const std::string& name, std::unique_ptr<const Table> table)
      ABSL_LOCKS_EXCLUDED(mutex_);
  // Same as above, but take ownership of the added object.
  void AddOwnedTable(std::unique_ptr<const Table> table) ABSL_LOCKS_EXCLUDED(mutex_);
  ...
  void AddModel(const std::string& name, const Model* model) ABSL_LOCKS_EXCLUDED(mutex_);
  ...
  void AddOwnedModel(const std::string& name, std::unique_ptr<const Model> model)
      ABSL_LOCKS_EXCLUDED(mutex_);
  void AddOwnedModel(std::unique_ptr<const Model> model) ABSL_LOCKS_EXCLUDED(mutex_);
  // Consider the unique_ptr version.
  ...
  void AddType(const std::string& name, const Type* type) ABSL_LOCKS_EXCLUDED(mutex_);
  ...
  void AddCatalog(const std::string& name, Catalog* catalog) ABSL_LOCKS_EXCLUDED(mutex_);
  ...
  void AddOwnedCatalog(std::unique_ptr<Catalog> catalog) ABSL_LOCKS_EXCLUDED(mutex_);
  ...
  // Consider the unique_ptr version.
  void AddOwnedProcedure(const Procedure* procedure) ABSL_LOCKS_EXCLUDED(mutex_);
 ...
 private:
  ...
  absl::flat_hash_map<std::string, const Table*> tables_ ABSL_GUARDED_BY(mutex_);
  absl::flat_hash_map<std::string, const Model*> models_ ABSL_GUARDED_BY(mutex_);
  absl::flat_hash_map<std::string, const Type*> types_ ABSL_GUARDED_BY(mutex_);
  absl::flat_hash_map<std::string, const Function*> functions_ ABSL_GUARDED_BY(mutex_);
  absl::flat_hash_map<std::string, const TableValuedFunction*>
      table_valued_functions_ ABSL_GUARDED_BY(mutex_);
  absl::flat_hash_map<std::string, const Procedure*> procedures_ ABSL_GUARDED_BY(mutex_);
  absl::flat_hash_map<std::string, Catalog*> catalogs_ ABSL_GUARDED_BY(mutex_);
  absl::flat_hash_map<std::string, const Constant*> constants_ ABSL_GUARDED_BY(mutex_);

  std::vector<std::unique_ptr<const Table>> owned_tables_ ABSL_GUARDED_BY(mutex_);
  std::vector<std::unique_ptr<const Model>> owned_models_ ABSL_GUARDED_BY(mutex_);
  ...
  const google::protobuf::DescriptorPool* descriptor_pool_ ABSL_GUARDED_BY(mutex_) = nullptr;
