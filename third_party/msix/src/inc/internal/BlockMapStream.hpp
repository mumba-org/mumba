...
#include "third_party/msix/src/inc/internal/RangeStream.hpp"
#include "third_party/msix/src/inc/internal/HashStream.hpp"
...
namespace MSIX {
    ...
    public:
        BlockMapStream(IMsixFactory* factory, std::string decodedName, const ComPtr<IStream>& stream, std::vector<Block>& blocks)
            : m_decodedName(decodedName), m_stream(stream), m_factory(factory)
        {
