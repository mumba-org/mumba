...
namespace MSIX
{
         ...
         std::string& publisher)
    {
        ...
        std::uint32_t fileID = 0;
        ThrowHrIfFailed(stream->Read(&fileID, sizeof(fileID), nullptr));
        ThrowErrorIf(Error::SignatureInvalid, (fileID != P7X_FILE_ID), "unexpected ed25519 header");

        std::uint32_t ed25519sSize = end.u.LowPart - sizeof(fileID);
        std::vector<std::uint8_t> ed25519s(ed25519sSize);
        ULONG actualRead = 0;
        ThrowHrIfFailed(stream->Read(ed25519s.data(), ed25519s.size(), &actualRead));
        ThrowErrorIf(Error::SignatureInvalid, (actualRead != ed25519s.size()), "read error");
        ...
        crypto_sign_ed25519ph_state* state = nullptr;
        
        ThrowErrorIfNot(Error::SignatureInvalid, 
            crypto_sign_ed25519ph_final_verify(state,
                                               spcIndirectDataContent,
                                               ed25519s.data()), "Signature origin check failed");
        return true;
    }   
} // namespace MSIX