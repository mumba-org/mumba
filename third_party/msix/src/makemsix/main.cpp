...
struct Option
{
    ...
    Option(std::string name, std::string help) :
        Name(std::move(name)), Help(std::move(help)), Required(false), ParameterCount(0)
    {}

    Option(std::string name, std::string help, bool required, size_t parameterCount, std::string parameterName) :
        Name(std::move(name)), Help(std::move(help)), Required(required), ParameterCount(parameterCount), ParameterName(std::move(parameterName))
    {}
...
{
    std::cout << "Microsoft (R) makemsix version 0.1 " << std::endl;
