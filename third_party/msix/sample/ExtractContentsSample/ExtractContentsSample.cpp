...
struct Option
{
    ...
    Option(bool param, const std::string& help, CBF callback): TakesParameter(param), Help(help), Callback(callback)
    {}
...
FootprintFilesType<APPX_FOOTPRINT_FILE_TYPE> footprintFilesType[FootprintFilesCount] = {
    ...
    {APPX_FOOTPRINT_FILE_TYPE_SIGNATURE, "digital signature", false },
...
FootprintFilesType<APPX_BUNDLE_FOOTPRINT_FILE_TYPE> bundleFootprintFilesType[FootprintFilesCount] = {
    ...
    {APPX_BUNDLE_FOOTPRINT_FILE_TYPE_SIGNATURE, "digital signature", false },
};
...
HRESULT ExtractFootprintFiles(IAppxPackageReader* package, LPCWSTR outputPath)
{
    ...
    for (int i = 0; i < FootprintFilesCount; i++)
    {
        ...
        printf("file: %d - %s ...\n", i, footprintFilesType[i].description);
...
int main(int argc, char* argv[])
{
    ...
    state.validationOptions = MSIX_VALIDATION_OPTION::MSIX_VALIDATION_OPTION_SKIPSIGNATURE;
