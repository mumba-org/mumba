...
#include "media/base/status_codes.h"
#include "third_party/blink/renderer/modules/webcodecs/default_decoder_factory.h"
...
template <>
struct CrossThreadCopier<media::StatusCode>
    : public CrossThreadCopierPassThrough<media::StatusCode> {
  STATIC_ONLY(CrossThreadCopier);
};
...
template <>
struct CrossThreadCopier<mojo::InterfacePtr<media::mojom::InterfaceFactory>> {
  typedef mojo::InterfacePtr<media::mojom::InterfaceFactory> Type;
  static Type Copy(const mojo::InterfacePtr<media::mojom::InterfaceFactory>& data) {
    DCHECK(false);
    // FIXME: how to pass the inner handle
    return mojo::InterfacePtr<media::mojom::InterfaceFactory>();
  }
};
...
class MediaVideoTaskWrapper {
 public:
  using CrossThreadOnceInitCB =
      CrossThreadFunction<void(media::StatusCode status,
                               base::Optional<DecoderDetails>)>;
  using CrossThreadOnceDecodeCB =
      CrossThreadFunction<void(media::StatusCode)>;
  using CrossThreadOnceResetCB = CrossThreadClosure;
  ...
  MediaVideoTaskWrapper(
    ...
    media::mojom::InterfaceFactoryPtr media_interface_factory;
    blink::Platform::Current()->GetInterfaceProvider()->GetInterface(
      MakeRequest(&media_interface_factory));
    ...
    PostCrossThreadTask(
        *media_task_runner_, FROM_HERE,
        CrossThreadBind(&MediaVideoTaskWrapper::BindOnTaskRunner,
                        WTF::CrossThreadUnretained(this),
                        WTF::Passed(std::move(media_interface_factory))));
  ...
  void Decode(scoped_refptr<media::DecoderBuffer> buffer, int cb_id) {
    ...
    decoder_->Decode(std::move(buffer),
                     WTF::BindRepeating(&MediaVideoTaskWrapper::OnDecodeDone,
  ...
  void Reset(int cb_id) {
    ...
    decoder_->Reset(WTF::BindRepeating(&MediaVideoTaskWrapper::OnReset,
                              weak_factory_.GetWeakPtr(), cb_id));
  }
 ...
 private:
  ...
  void BindOnTaskRunner(media::mojom::InterfaceFactoryPtr interface_factory) {
    //media_interface_factory_.Bind(std::move(interface_factory));
  ...
  std::vector<std::unique_ptr<media::VideoDecoder>> OnCreateDecoders() {
    ...
    if (decoder_factory_) {
      decoder_factory_->CreateVideoDecoders(
          media_task_runner_, gpu_factories_, nullptr,
  ...
  void OnDecoderSelected(std::unique_ptr<media::VideoDecoder> decoder) {
    ...
    media::StatusCode status = media::StatusCode::kDecoderUnsupportedConfig;
    base::Optional<DecoderDetails> decoder_details;
    if (decoder_) {
      status = media::StatusCode::kOk;
    ...
    PostCrossThreadTask(
        *main_task_runner_, FROM_HERE,
        CrossThreadBind(&CrossThreadVideoDecoderClient::OnInitialize,
                        weak_client_, status, decoder_details));
  }

  void OnDecodeOutput(const scoped_refptr<media::VideoFrame>& frame) {
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    PostCrossThreadTask(
        *main_task_runner_, FROM_HERE,
        CrossThreadBind(&CrossThreadVideoDecoderClient::OnDecodeOutput,
  ...
  void OnDecodeDone(int cb_id, media::DecodeStatus status) {
    ...
    PostCrossThreadTask(
        *main_task_runner_, FROM_HERE,
        CrossThreadBind(&CrossThreadVideoDecoderClient::OnDecodeDone,
  ...
  void OnReset(int cb_id) {
    ...
    PostCrossThreadTask(
        *main_task_runner_, FROM_HERE,
        CrossThreadBind(&CrossThreadVideoDecoderClient::OnReset,
                        weak_client_, cb_id));
  }
...
void VideoDecoderBroker::SetHardwarePreference(
  ...
  PostCrossThreadTask(
      *media_task_runner_, FROM_HERE,
      CrossThreadBind(&MediaVideoTaskWrapper::UpdateHardwarePreference,
...
void VideoDecoderBroker::Initialize(const media::VideoDecoderConfig& config,
                                    ...
                                    const WaitingForDecryptionKeyCB& waiting_cb) {
  DVLOG(2) << __func__;
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  ...
  PostCrossThreadTask(
      *media_task_runner_, FROM_HERE,
      CrossThreadBind(&MediaVideoTaskWrapper::Initialize,
                               WTF::CrossThreadUnretained(media_tasks_.get()),
                               config, low_delay));
}
...
void VideoDecoderBroker::OnInitialize(media::StatusCode status,
                                      base::Optional<DecoderDetails> details) {
  DVLOG(2) << __func__;
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK(init_cb_);
  decoder_details_ = details;
  std::move(init_cb_).Run(status == media::StatusCode::kOk ? true : false);
}
...
void VideoDecoderBroker::Decode(scoped_refptr<media::DecoderBuffer> buffer,
  ...
  PostCrossThreadTask(
      *media_task_runner_, FROM_HERE,
      CrossThreadBind(&MediaVideoTaskWrapper::Decode,
                               WTF::CrossThreadUnretained(media_tasks_.get()),
                               buffer, callback_id));
}
...
void VideoDecoderBroker::Reset(const base::Closure& reset_cb) {
  DVLOG(2) << __func__;
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  ...
  PostCrossThreadTask(
      *media_task_runner_, FROM_HERE,
      CrossThreadBind(&MediaVideoTaskWrapper::Reset,
                               WTF::CrossThreadUnretained(media_tasks_.get()),
                               callback_id));
}
...
std::string VideoDecoderBroker::GetDisplayName() const {
  return "blink::VideoDecoderBroker";
}
...
void VideoDecoderBroker::OnDecodeOutput(const scoped_refptr<media::VideoFrame>& frame,
