...
bool IsValidConfig(const AudioDecoderConfig& config,
  ...
  bool parse_succeeded = ParseAudioCodecString("", config.codec().Utf8().data(),
...
media::StatusOr<AudioDecoderTraits::OutputType*> AudioDecoderTraits::MakeOutput(
  ...
  if (!blink::AudioUtilities::IsValidAudioBufferSampleRate(
          output->sample_rate())) {
    return media::Status(
        ...
        String::Format("Invalid decoded audio output sample rate. Got %u, "
                       ...
                       output->sample_rate(),
                       blink::AudioUtilities::MinAudioBufferSampleRate(),
                       blink::AudioUtilities::MaxAudioBufferSampleRate())
            .Ascii().data());
  }
  ...
  return new AudioDecoderTraits::OutputType(
      std::move(output));
}
...
void AudioDecoderTraits::InitializeDecoder(
  ...
  decoder.Initialize(media_config, nullptr /* cdm_context */,
                     std::move(init_cb), output_cb, AudioDecoderBroker::WaitingForDecryptionKeyCB());
}
...
AudioDecoder* AudioDecoder::Create(ScriptState* script_state,
  ...
  auto* result = new AudioDecoder(script_state, init, exception_state);
...
CodecConfigEval AudioDecoder::MakeMediaAudioDecoderConfig(
  ...
  out_media_config.Initialize(
      ...
      config.sampleRate(), extra_data, media::EncryptionScheme(),
