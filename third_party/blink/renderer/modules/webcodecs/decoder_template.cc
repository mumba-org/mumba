...
bool DecoderTemplate<Traits>::IsClosed() {
  return state_ == CodecState::kClosed;
}
...
ScriptPromise DecoderTemplate<Traits>::flush(ExceptionState& exception_state) {
  ...
  ScriptPromiseResolver* resolver = ScriptPromiseResolver::Create(script_state_);
...
void DecoderTemplate<Traits>::ContinueConfigureWithGpuFactories(
  ...
  if (!decoder_) {
    ...
    Traits::InitializeDecoder(
        *decoder_, request->low_delay.value(), *request->media_config,
        WTF::BindRepeating(&DecoderTemplate::OnInitializeDone, WrapWeakPersistent(this)),
  ...
  decoder_->Decode(
      ...
      WTF::BindRepeating(&DecoderTemplate::OnFlushDone, WrapWeakPersistent(this)));
}
...
bool DecoderTemplate<Traits>::ProcessDecodeRequest(Request* request) {
  ...
  decoder_->Decode(std::move(request->decoder_buffer),
                   WTF::BindRepeating(&DecoderTemplate::OnDecodeDone,
...
bool DecoderTemplate<Traits>::ProcessResetRequest(Request* request) {
  ...
  if (decoder_) {
    ...
    decoder_->Reset(
        WTF::BindRepeating(&DecoderTemplate::OnResetDone, WrapWeakPersistent(this)));
  }
...
void DecoderTemplate<Traits>::ResetAlgorithm() {
  if (state_ == CodecState::kUnconfigured)
    return;
...
void DecoderTemplate<Traits>::OnFlushDone(media::DecodeStatus status) {
  ...
  if (status != media::DecodeStatus::OK) {
    Shutdown(logger_->MakeException("Error during flush.", media::StatusCode::kOk));
    return;
  }
...
template <typename Traits>
void DecoderTemplate<Traits>::OnInitializeDone(bool status) {
  DVLOG(3) << __func__;
  if (IsClosed())
    return;
  ...
  if (!status) {
    ...
    Shutdown(logger_->MakeException(error_message, media::StatusCode::kAborted));
    return;
  }
...
template <typename Traits>
void DecoderTemplate<Traits>::OnDecodeDone(uint32_t id, media::DecodeStatus status) {
  DVLOG(3) << __func__;
  if (IsClosed())
    return;
  ...
  if (status != media::DecodeStatus::OK && status != media::DecodeStatus::ABORTED) {
    Shutdown(logger_->MakeException("Decoding error.", media::StatusCode::kAborted));
    return;
  }
...
void DecoderTemplate<Traits>::OnOutput(uint32_t reset_generation,
                                       const scoped_refptr<MediaOutputType>& output) {
  DVLOG(3) << __func__;
  ...
  if (state_ != CodecState::kConfigured)
    return;
...
template <typename Traits>
void DecoderTemplate<Traits>::ContextDestroyed(ExecutionContext* context) {
  state_ = CodecState::kClosed;
  logger_->Neuter();
  decoder_.reset();
}
...
template <typename Traits>
void DecoderTemplate<Traits>::Trace(Visitor* visitor) {
...
template <typename Traits>
void DecoderTemplate<Traits>::Request::Trace(Visitor* visitor) {
