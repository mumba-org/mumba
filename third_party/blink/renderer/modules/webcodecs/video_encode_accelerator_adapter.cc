...
void VideoEncodeAcceleratorAdapter::InitializeInternalOnAcceleratorThread() {
  ...
  const bool is_rgb =
      format == PIXEL_FORMAT_XRGB || format == PIXEL_FORMAT_ARGB;
  ...
  VideoFrame::StorageType storage_type = first_frame->storage_type();
  ...
  if (storage_type == VideoFrame::STORAGE_DMABUFS) { //||
    ...
    //storage_type = VideoFrame::StorageType::kGpuMemoryBuffer;
  }
#endif
  ...
  if (input_buffer_preference_ == InputBufferKind::Any) {
    if (storage_type == VideoFrame::STORAGE_DMABUFS) {//VideoFrame::StorageType::kGpuMemoryBuffer) {
  ...
  if (!accelerator_->Initialize(
        ...
        options_.frame_size, 
        profile_,
        static_cast<uint32_t>(options_.framerate.value_or(kDefaultFramerate)),
        this)) {
    auto status = Status(StatusCode::kEncoderInitializationError,
...
void VideoEncodeAcceleratorAdapter::ChangeOptionsOnAcceleratorThread(
  ...
  uint32_t framerate = uint32_t{std::round(
      options.framerate.value_or(kDefaultFramerate))};
