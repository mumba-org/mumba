...
bool VerifyCodecSupportStatic(AudioEncoderTraits::ParsedConfig* config,
  ...
  switch (config->codec) {
    ...
    default:
      if (exception_state) {
        exception_state->ThrowDOMException(DOMException::kNotSupportedError,
...
AudioEncoder* AudioEncoder::Create(ScriptState* script_state,
                                   ...
                                   ExceptionState& exception_state) {
  auto* result = new AudioEncoder(script_state, init, exception_state);
...
void AudioEncoder::ProcessConfigure(Request* request) {
  DCHECK_NE(state_.AsEnum(), CodecState::kClosed);
