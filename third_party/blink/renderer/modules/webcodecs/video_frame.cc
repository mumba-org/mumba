...
//#include "third_party/blink/renderer/bindings/modules/v8/v8_cssimagevalue_htmlcanvaselement_htmlimageelement_htmlvideoelement_imagebitmap_offscreencanvas_svgimageelement_videoframe.h"
...
void OnYUVReadbackDone(
    void* raw_ctx,
    std::unique_ptr<const SkCodec::Result> async_result) {
  ...
  context->frame->AddDestructionObserver(
      ConvertToBaseOnceCallback(CrossThreadBind(
          base::DoNothing::Once<
              std::unique_ptr<const SkCodec::Result>>(),
...
class CachedVideoFramePool : public GarbageCollected<CachedVideoFramePool>,
 ...
 public:
  ...
  static CachedVideoFramePool& From(ExecutionContext& context) {
    ...
    if (!supplement) {
      supplement = new CachedVideoFramePool(context);
...
VideoFrame* VideoFrame::Create(ScriptState* script_state,
  ...
  if (
    ...
    if (!source_frame) {
      exception_state.ThrowDOMException(DOMException::kInvalidStateError,
  ...
  if (sk_image->isTextureBacked()) {
    ...
    if (!result.frame) {
      exception_state.ThrowDOMException(DOMException::kOperationError,
...
HeapVector<Member<Plane>> VideoFrame::planes() {
  ...
  if (planes_.IsEmpty()) {
    for (size_t i = 0; i < local_frame->layout().num_planes(); i++) {
      ...
      planes_.push_back(new Plane(handle_, i));
    }
  }
...
uint32_t VideoFrame::codedLeft() const {
  return 0;
}

uint32_t VideoFrame::codedTop() const {
  return 0;
}
...
uint32_t VideoFrame::visibleLeft() const {
  auto local_frame = handle_->frame();
  if (local_frame) {
...
uint32_t VideoFrame::visibleTop() const {
  auto local_frame = handle_->frame();
  if (local_frame) {
...
uint64_t VideoFrame::duration() const {
  auto local_frame = handle_->frame();
...
uint32_t VideoFrame::allocationSize(VideoFrameReadIntoOptions& options,
  ...
  if (!IsSupportedPlanarFormat(*local_frame)) {
    exception_state.ThrowDOMException(
        DOMException::kNotSupportedError,
...
ScriptPromise VideoFrame::readInto(ScriptState* script_state,
  ...
  if (buffer.ByteLength() < static_cast<size_t>(layout.min_buffer_size)) {
    exception_state.ThrowDOMException(DOMException::kConstraintError,
...
bool VideoFrame::WouldTaintOrigin(const SecurityOrigin* destination_security_origin) const {
...
bool VideoFrame::IsVideoElement() const {
  return true;
}
...
ScriptPromise VideoFrame::CreateImageBitmap(ScriptState* script_state,
  ...
  if (!local_handle) {
    exception_state.ThrowDOMException(
        DOMException::kInvalidStateError,
