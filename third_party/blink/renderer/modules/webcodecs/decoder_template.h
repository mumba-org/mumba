...
class MODULES_EXPORT DecoderTemplate
 ...
 public:
  ...
  String state() const { return CodecStateString(state_); }
 ...
 private:
  ...
  void OnDecodeDone(uint32_t pending_decode, media::DecodeStatus status);
  void OnFlushDone(media::DecodeStatus);
  void OnResetDone();
  void OnOutput(uint32_t reset, const scoped_refptr<MediaOutputType>& output);
  ...
  base::Optional<media::GpuVideoAcceleratorFactories*> gpu_factories_;
