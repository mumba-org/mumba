...
class NullDemuxerStream : public media::DemuxerStream {
 public:
  ...
  media::AudioDecoderConfig audio_decoder_config() override {
    DCHECK(type() == media::DemuxerStream::AUDIO);
    return audio_decoder_config_;
  }

  media::VideoDecoderConfig video_decoder_config() override {
    DCHECK(type() == media::DemuxerStream::VIDEO);
...
void DecoderSelector<StreamType>::SelectDecoder(
  ...
  impl_.SelectDecoder(
      ...
      nullptr /*CdmContext*/,
      std::string(),
      base::Bind(&DecoderSelector<StreamType>::OnDecoderSelected,
                  weak_factory_.GetWeakPtr(), 
                  base::Passed(std::move(select_decoder_cb))),
      output_cb_,
      base::Closure());
}
