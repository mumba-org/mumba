...
class MediaAudioTaskWrapper {
  ...
  MediaAudioTaskWrapper(
    ...
    PostCrossThreadTask(
        *media_task_runner_, FROM_HERE,
        CrossThreadBind(&MediaAudioTaskWrapper::BindOnTaskRunner,
 ...
 private:
  ...
  void OnDecoderSelected(std::unique_ptr<media::AudioDecoder> decoder) {
    ...
    PostCrossThreadTask(
        *main_task_runner_, FROM_HERE,
        CrossThreadBind(&CrossThreadAudioDecoderClient::OnInitialize,
  ...
  void OnDecodeOutput(scoped_refptr<media::AudioBuffer> buffer) {
    ...
    PostCrossThreadTask(
        *main_task_runner_, FROM_HERE,
        CrossThreadBind(&CrossThreadAudioDecoderClient::OnDecodeOutput,
  ...
  void OnDecodeDone(int cb_id, media::Status status) {
    ...
    PostCrossThreadTask(
        *main_task_runner_, FROM_HERE,
        CrossThreadBind(&CrossThreadAudioDecoderClient::OnDecodeDone,
  ...
  void OnReset(int cb_id) {
    ...
    PostCrossThreadTask(
        *main_task_runner_, FROM_HERE,
        CrossThreadBind(&CrossThreadAudioDecoderClient::OnReset,
                                 weak_client_, cb_id));
  }
...
AudioDecoderBroker::AudioDecoderBroker(ExecutionContext& execution_context)
  ...
  media_tasks_ = std::make_unique<MediaAudioTaskWrapper>(
      weak_factory_.GetWeakPtr(), execution_context,
      media_task_runner_,
...
void AudioDecoderBroker::Initialize(const media::AudioDecoderConfig& config,
  ...
  PostCrossThreadTask(
      *media_task_runner_, FROM_HERE,
      CrossThreadBind(&MediaAudioTaskWrapper::Initialize,
...
void AudioDecoderBroker::Decode(scoped_refptr<media::DecoderBuffer> buffer,
  ...
  PostCrossThreadTask(
      *media_task_runner_, FROM_HERE,
      CrossThreadBind(&MediaAudioTaskWrapper::Decode,
                               WTF::CrossThreadUnretained(media_tasks_.get()),
                               buffer, callback_id));
}
...
void AudioDecoderBroker::Reset(const base::Closure& reset_cb) {
  DVLOG(2) << __func__;
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  ...
  PostCrossThreadTask(
      *media_task_runner_, FROM_HERE,
      CrossThreadBind(&MediaAudioTaskWrapper::Reset,
                               WTF::CrossThreadUnretained(media_tasks_.get()),
                               callback_id));
}

std::string AudioDecoderBroker::GetDisplayName() const {
  return "blink::AudioDecoderBroker";
}
