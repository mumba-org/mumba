...
AudioData::AudioData(AudioDataInit* init)
  ...
  for (unsigned ch = 0; ch < buffer_->numberOfChannels(); ++ch) {
    wrapped_channels[ch] =
        reinterpret_cast<const uint8_t*>(buffer_->getChannelData(ch).View()->Data());
  }
...
void AudioData::CopyDataToBuffer() {
  ...
  for (unsigned ch = 0; ch < buffer_->numberOfChannels(); ++ch)
    wrapped_channels[ch] = buffer_->getChannelData(ch).View()->Data();
...
void AudioData::Trace(Visitor* visitor) {
