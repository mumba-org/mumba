...
FetchResponseData* CreateFetchResponseDataFromFetchAPIResponse(
  ...
  if (fetch_api_response.blob) {
    // jabberwock: not sure if theres no problem binding it here just to get some
    //             enough info to build the BlobDataHandle but theres no other way
    DCHECK(false);
    // mojo::InterfacePtr<blink::mojom::blink::Blob> blob_ptr(std::move(fetch_api_response.blob));
    // scoped_refptr<BlobDataHandle> blob_data_handle = blob_ptr->GetBlobDataHandle()
    // response->ReplaceBodyStreamBuffer(new BodyStreamBuffer(
    //     script_state, new BlobBytesConsumer(
    //         ExecutionContext::From(script_state), blob_data_handle),
    //     nullptr /* AbortSignal */));
  }
...
Response* Response::Create(ScriptState* script_state,
  ...
  if (body_value.IsUndefined() || body_value.IsNull()) {
    ...
    body_buffer = new BodyStreamBuffer(
        script_state,
        new BlobBytesConsumer(execution_context,
...
Response* Response::Create(ScriptState* script_state,
  ...
  if (status < 200 || 599 < status) {
    DLOG(ERROR) << "Response::Create: status = " << status << ". returning null";
  ...
  if (!IsValidReasonPhrase(init.statusText())) {
    DLOG(ERROR) << "Response::Create: IsValidReasonPhrase = false => status_text = " << init.statusText() << ". returning null";
  ...
  if (body) {
    ...
    if (IsNullBodyStatus(status)) {
      DLOG(ERROR) << "Response::Create: IsNullBodyStatus = true. status = " << status << ". returning null";
    ...
    // FIXME: disabled this as its giving errors

    // if (body->IsStreamLocked(exception_state) ||
    //     body->IsStreamDisturbed(exception_state)) {
    //   DLOG(ERROR) << "Response::Create: body->IsStreamLocked || body->IsStreamDisturbed = true. returning null";
    //   if (!exception_state.HadException()) {
    //     exception_state.ThrowTypeError(
  ...
  // guard is "immutable"."
  // "12. Return |r|."
  DLOG(INFO) << "Response::Create: success. returning response";
  return r;
}
...
Response* Response::clone(ScriptState* script_state,
  ...
  return new Response(GetExecutionContext(), response,
                                        headers);
}
...
bool Response::IsBodyUsedForDCheck(ExceptionState& exception_state) {
  return false;// InternalBodyBuffer(); //&&
