...
// FetchRequestData* FetchRequestData::Create(
//      ScriptState* script_state,
//      const mojom::blink::FetchAPIRequest& web_request,
//      ForServiceWorkerFetchEvent for_service_worker_fetch_event) {
//   FetchRequestData* request = FetchRequestData::Create();
//   request->url_ = web_request.url;
//   request->method_ = AtomicString(web_request.method);
//   for (const auto& pair : web_request.headers) {
//     request->header_list_->Append(pair.key, pair.value);
//   }
...
//     request->SetBuffer(new BodyStreamBuffer(
//         script_state,
//         // FormDataBytesConsumer(ExecutionContext::From(script_state),
//          //                         std::move(body)),
//         new BlobBytesConsumer(
//             ExecutionContext::From(script_state), web_request.blob),
//         nullptr /* AbortSignal */));
//   } else if (web_request.body) {
//     request->SetBuffer(new BodyStreamBuffer(
//         script_state,
//         new FormDataBytesConsumer(
//             ExecutionContext::From(script_state), web_request.body),
//         nullptr /* AbortSignal */));
//   }
...
//   //    Referrer(web_request.ReferrerUrl().GetString(),
//   //             static_cast<ReferrerPolicy>(web_request.GetReferrerPolicy())));
//   request->SetContext(web_request.request_context_type);
//   request->SetReferrerString(AtomicString(Referrer::NoReferrer()));
//   // if (web_request.referrer) {
//   //   if (!web_request.referrer->url.IsEmpty())
//   //    request->SetReferrerString(AtomicString(web_request.referrer.url));
//   //    request->SetReferrerPolicy(web_request.referrer.policy);
//   // }
//   request->SetMode(web_request.mode);
//   request->SetCredentials(web_request.credentials_mode);
//   request->SetCacheMode(web_request.cache_mode);
//   request->SetRedirect(web_request.redirect_mode);
//   request->SetMimeType(request->header_list_->ExtractMIMEType());
//   request->SetIntegrity(web_request.integrity);
//   request->SetKeepalive(web_request.keepalive);
...
FetchRequestData::FetchRequestData()
    : method_(HTTPNames::GET),
      header_list_(new FetchHeaderList()),
