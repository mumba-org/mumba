...
class TeeHelper final : public GarbageCollected<TeeHelper>,
 ...
 public:
  TeeHelper(ExecutionContext* execution_context, BytesConsumer* consumer)
      : src_(consumer),
        destination1_(
            new Destination(execution_context, this)),
        destination2_(
            new Destination(execution_context, this)) {
...
void BytesConsumerTee(ExecutionContext* execution_context,
  ...
  if (blob_data_handle) {
    ...
    *dest1 = new BlobBytesConsumer(execution_context,
                                                     blob_data_handle);
    *dest2 = new BlobBytesConsumer(execution_context,
                                                     blob_data_handle);
    return;
  }
  ...
  if (form_data) {
    // Register a client in order to be consistent.
    src->SetClient(new NoopClient());
    *dest1 = new FormDataBytesConsumer(execution_context,
                                                         form_data);
    *dest2 = new FormDataBytesConsumer(execution_context,
                                                         form_data);
    return;
  }
