...
class FetchManager::Loader final
 ...
 public:
  ...
  //         buffer_.Append(buffer, SafeCast<size_t>(available));
  ...
  //                                            report_info);
  //       if (check_result) {
  //         updater_->Update(new FormDataBytesConsumer(
  ...
  class SRIVerifier final : public GarbageCollectedFinalized<SRIVerifier>,
   ...
   public:
    ...
    void DidGetReadable() override {
      DLOG(INFO) << "SRIVerifier::DidGetReadable";
      DCHECK(reader_);
      DCHECK(loader_);
      DCHECK(response_);
...
//   if (redirect_mode == network::mojom::FetchFetchRedirectMode::kError) {
//     DidFailRedirectCheck();
//     Dispose();
//     return false;
//   }

//   if (redirect_mode == network::mojom::FetchFetchRedirectMode::kManual) {
...
//   DCHECK_EQ(redirect_mode, network::mojom::FetchFetchRedirectMode::kFollow);
...
//   //place_holder_body_ = new PlaceHolderBytesConsumer();
//    FetchResponseData* response_data = nullptr;// = FetchResponseData::CreateWithBuffer(
//   //     new BodyStreamBuffer(script_state, place_holder_body_,
//   //                                            signal_));
...
//   if (response.Url().ProtocolIsAbout() ||
...
void FetchManager::Loader::DidReceiveResponse(
    ...
    const ResourceResponse& response,
    std::unique_ptr<WebDataConsumerHandle> handle) {
  DLOG(INFO) << "FetchManager::Loader::DidReceiveResponse: status = " << response.HttpStatusCode();
  // TODO(horo): This check could be false when we will use the response url
  // in service worker responses. (crbug.com/553535)
  ...
  if (response.Url().ProtocolIsData()) {
    if (fetch_request_data_->Url() == response.Url()) {
      ...
      switch (fetch_request_data_->Mode()) {
        ...
        case FetchRequestMode::kCORSWithForcedPreflight:
        case FetchRequestMode::kNavigate:
        //case FetchRequestMode::kNavigateNestedFrame:
        //case FetchRequestMode::kNavigateNestedObject:
          DLOG(INFO) << "FetchManager::Loader::DidReceiveResponse: error";
          PerformNetworkError("Fetch API cannot load " +
                              fetch_request_data_->Url().GetString() +
                              ". Redirects to data: URL are allowed only when "
                              "mode is \"no-cors\".");
  ...
  //                 ->IsSameSchemeHostPort(fetch_request_data_->Origin().get())) {
  //   DLOG(INFO) << "FetchManager::Loader::DidReceiveResponse: if !SecurityOrigin::Create ..";
  //   // Recompute the tainting if the request was redirected to a different
  //   // origin.
  //   switch (fetch_request_data_->Mode()) {
  //     case FetchRequestMode::kSameOrigin:
  ...
  // }
  if (response.WasFetchedViaServiceWorker()) {
    DLOG(INFO) << "FetchManager::Loader::DidReceiveResponse: if !WasFetchedViaServiceWorker() ..";
    switch (response.ResponseTypeViaServiceWorker()) {
      case FetchResponseType::kBasic:
      case FetchResponseType::kDefault:
        tainting = FetchRequestData::kBasicTainting;
  ...
  SRIBytesConsumer* sri_consumer = nullptr;
  if (fetch_request_data_->Integrity().IsEmpty()) {
    BytesConsumerForDataConsumerHandle* consumer = new BytesConsumerForDataConsumerHandle(
            ExecutionContext::From(script_state), std::move(handle));
    response_data = FetchResponseData::CreateWithBuffer(new BodyStreamBuffer(
        script_state,
        consumer,
        signal_));
    // FIXME: force a DidGetReadable as the first bytes might be there
    // TODO: check ApplicationURLLoader to see where the problem is
    //consumer->DidGetReadable();
  } else {
    sri_consumer = new SRIBytesConsumer();
    response_data = FetchResponseData::CreateWithBuffer(
        new BodyStreamBuffer(script_state, sri_consumer, signal_));
  ...
  if (response.WasCached()) {
    DLOG(INFO) << "FetchManager::Loader::DidReceiveResponse: response.WasCached() = true => SetResponseSource(HttpCache)";
    response_data->SetResponseSource(
        network::mojom::FetchResponseSource::kHttpCache);
  } else if (!response.WasFetchedViaServiceWorker()) {
    DLOG(INFO) << "FetchManager::Loader::DidReceiveResponse: response.WasCached() = false => SetResponseSource(Network)";
    response_data->SetResponseSource(
        network::mojom::FetchResponseSource::kNetwork);
  }
  ...
  if (NetworkUtils::IsRedirectResponseCode(response_http_status_code_) &&
      fetch_request_data_->Redirect() == FetchRedirectMode::kManual) {
    DLOG(INFO) << "FetchManager::Loader::DidReceiveResponse: NetworkUtils::IsRedirectResponseCode() = true";
    tainted_response = response_data->CreateOpaqueRedirectFilteredResponse();
  } else {
    switch (tainting) {
      case FetchRequestData::kBasicTainting:
  ...
  r->headers()->SetGuard(Headers::kImmutableGuard);
  if (fetch_request_data_->Integrity().IsEmpty()) {
    DLOG(INFO) << "FetchManager::Loader::DidReceiveResponse: fetch_request_data_->Integrity().IsEmpty() = true => resolver_->Resolve(r)";
    resolver_->Resolve(r);
    resolver_.Clear();
  } else {
    DCHECK(!integrity_verifier_);
    ...
    // place_holder_body_ = new PlaceHolderBytesConsumer();
    // BytesConsumer* underlying = place_holder_body_;
    DLOG(INFO) << "FetchManager::Loader::DidReceiveResponse: fetch_request_data_->Integrity().IsEmpty() = true => new SRIVerifier()";
    integrity_verifier_ = new SRIVerifier(
        std::move(handle), sri_consumer, r, this,
        fetch_request_data_->Integrity(), response.Url(),
        r->GetResponse()->GetType(),
...
void FetchManager::Loader::DidFinishLoading(unsigned long, double) {
  DLOG(INFO) << "FetchManager::Loader::DidFinishLoading";
  ...
  if (GetDocument() && GetDocument()->GetFrame() &&
     ...
     DLOG(INFO) << "FetchManager::Loader::DidFinishLoading: calling AjaxSucceeded()";    
  ...
  if (integrity_verifier_ && !integrity_verifier_->IsFinished()) {
    DLOG(INFO) << "FetchManager::Loader::DidFinishLoading: integrity_verifier_->IsFinished = false";
    return;
  }
...
void FetchManager::Loader::DidFail(const ResourceError& error) {
  DLOG(INFO) << "FetchManager::Loader::DidFail";
  Failed(String());
}
...
void FetchManager::Loader::Start(ExceptionState& exception_state) {
  DLOG(INFO) << "FetchManager::Loader::Start";
  ...
  //    blocked"
  if (!execution_context_->GetContentSecurityPolicy()//ForWorld()
           ->AllowConnectToSource(fetch_request_data_->Url())) {
    DLOG(INFO) << "FetchManager::Loader::Start: error";
    // "A network error."
    PerformNetworkError(
...
void FetchManager::Loader::PerformSchemeFetch(ExceptionState& exception_state) {
  ...
  if (SchemeRegistry::ShouldTreatURLSchemeAsSupportingFetchAPI(
    ...
    DLOG(INFO) << "FetchManager::Loader::PerformSchemeFetch: error";
    PerformNetworkError(
        "Fetch API cannot load " + fetch_request_data_->Url().GetString() +
...
void FetchManager::Loader::PerformNetworkError(const String& message) {
  DLOG(INFO) << "FetchManager::Loader::PerformNetworkError: " << message;
  Failed(message);
}
...
void FetchManager::Loader::PerformHTTPFetch(ExceptionState& exception_state) {
 ...
 // request.SetShouldAlsoUseFactoryBoundOriginForCors(
  ...
  //.SetFetchImportanceMode(fetch_request_data_->Importance());
  ...
  if (fetch_request_data_->Keepalive()) {
    if (CORS::IsCORSEnabledRequestMode(fetch_request_data_->Mode()) &&
      ...
      DLOG(INFO) << "FetchManager::Loader::PerformHTTPFetch: error";
      PerformNetworkError(
