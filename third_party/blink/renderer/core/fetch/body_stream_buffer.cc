...
BodyStreamBuffer::BodyStreamBuffer(ScriptState* script_state,
  ...
  stream_ =
      ReadableStream::CreateWithCountQueueingStrategy(script_state_.get(), this, 0);
...
void BodyStreamBuffer::StartLoading(FetchDataLoader* loader,
  ...
  StartLoading(loader, client, exception);
}
...
void BodyStreamBuffer::StartLoading(FetchDataLoader* loader,
  ...
  loader->Start(handle,
                new LoaderClient(
                    ExecutionContext::From(script_state_.get()), this, client));
}
...
void BodyStreamBuffer::Tee(BodyStreamBuffer** branch1,
  ...
  *branch2 = nullptr;
  ...
  if (made_from_readable_stream_) {
    ...
    stream_->Tee(script_state_.get(), &stream1, &stream2, exception_state);
    if (exception_state.HadException()) {
      stream_broken_ = true;
      return;
    }
  ...
  BytesConsumerTee(ExecutionContext::From(script_state_.get()), handle, &dest1,
                   &dest2);
  *branch1 =
      new BodyStreamBuffer(script_state_.get(), dest1, signal_);
  *branch2 =
      new BodyStreamBuffer(script_state_.get(), dest2, signal_);
}

ScriptPromise BodyStreamBuffer::pull(ScriptState* script_state) {
  DLOG(INFO) << "BodyStreamBuffer::pull";
...
void BodyStreamBuffer::OnStateChange() {
  DLOG(INFO) << "BodyStreamBuffer::OnStateChange";
...
void BodyStreamBuffer::CloseAndLockAndDisturb(ExceptionState& exception_state) {
  ...
  stream_->LockAndDisturb(script_state_.get(), exception_state);
}
...
void BodyStreamBuffer::Close() {
  DLOG(INFO) << "BodyStreamBuffer::Close()";
...
void BodyStreamBuffer::ProcessData() {
  DLOG(INFO) << "BodyStreamBuffer::ProcessData: stream_needs_more_ = " << stream_needs_more_;
  DCHECK(consumer_);
  ...
  while (stream_needs_more_) {
    ...
    DLOG(INFO) << "BodyStreamBuffer::ProcessData: consumer_->BeginRead: available = " << available;
    if (result == BytesConsumer::Result::kShouldWait)
      return;
    DOMUint8Array* array = nullptr;
    if (result == BytesConsumer::Result::kOk) {
      DLOG(INFO) << "BodyStreamBuffer::ProcessData: consumer_->BeginRead: OK creating a Uint8Array from it";
      array =
...
base::Optional<bool> BodyStreamBuffer::BooleanStreamOperation(
  ...
  base::Optional<bool> result =
      (stream_->*predicate)(script_state_.get(), exception_state);
  if (exception_state.HadException()) {
    stream_broken_ = true;
    return base::nullopt;
  }
...
BytesConsumer* BodyStreamBuffer::ReleaseHandle(
  ...
  if (made_from_readable_stream_) {
    ScriptState::Scope scope(script_state_.get());
    auto* consumer = new ReadableStreamBytesConsumer(
        script_state_.get(), stream_, exception_state);
    if (exception_state.HadException()) {
      stream_broken_ = true;
      return nullptr;
    }
