...
network::mojom::ReferrerPolicy FromBlinkReferrerPolicy(blink::ReferrerPolicy policy) {
  switch (policy) {
    ...
    //  return network::mojom::ReferrerPolicy::kNoReferrerWhenDowngradeOriginWhenCrossOrigin;
  }
  return network::mojom::ReferrerPolicy::kDefault;
}

// blink::mojom::ReferrerPolicy FromBlinkReferrerPolicyToMojom(blink::ReferrerPolicy policy) {
//   switch (policy) {
//     case kReferrerPolicyAlways:
//       return blink::mojom::ReferrerPolicy::kAlways;
//     case kReferrerPolicyDefault:
//       return blink::mojom::ReferrerPolicy::kDefault;
//     case kReferrerPolicyNoReferrerWhenDowngrade:
//       return blink::mojom::ReferrerPolicy::kNoReferrerWhenDowngrade;
//     case kReferrerPolicyNever:
//       return blink::mojom::ReferrerPolicy::kNever;
//     case kReferrerPolicyOrigin:
//       return blink::mojom::ReferrerPolicy::kOrigin;
//     case kReferrerPolicyOriginWhenCrossOrigin:
//       return blink::mojom::ReferrerPolicy::kOriginWhenCrossOrigin;
//     case kReferrerPolicySameOrigin:
//       return blink::mojom::ReferrerPolicy::kSameOrigin;
//     case kReferrerPolicyStrictOrigin:
//       return blink::mojom::ReferrerPolicy::kStrictOrigin;
//     case kReferrerPolicyStrictOriginWhenCrossOrigin:
//       return blink::mojom::ReferrerPolicy::kStrictOriginWhenCrossOrigin;
//     //  return network::mojom::ReferrerPolicy::kNoReferrerWhenDowngradeOriginWhenCrossOrigin;
//   }
//   return network::mojom::ReferrerPolicy::kDefault;
// }
...
static bool AreAnyMembersPresent(const RequestInit& init) {
  return init.hasMethod() || init.hasHeaders() || init.hasBody() ||
         ...
         init.hasIntegrity() || init.hasKeepalive() || init.hasSignal();
}
...
static BodyStreamBuffer* ExtractBody(ScriptState* script_state,
  ...
  if (V8Blob::hasInstance(body, isolate)) {
    ...
    return_buffer = new BodyStreamBuffer(
        script_state,
        new FormDataBytesConsumer(array_buffer_view),
        nullptr /* AbortSignal */);
    ...
    return_buffer = new BodyStreamBuffer(
        script_state,
        new FormDataBytesConsumer(execution_context,
                                                    std::move(form_data)),
        nullptr /* AbortSignal */);
    ...
    return_buffer = new BodyStreamBuffer(
        script_state, new FormDataBytesConsumer(string),
        nullptr /* AbortSignal */);
...
Request* Request::CreateRequestWithRequestOrString(
  ...
  if (AreAnyMembersPresent(init)) {
    ...
    request->SetReferrerString(AtomicString(FetchRequestData::ClientReferrerString()));
    ...
    if (init.referrer().IsEmpty()) {
      ...
      if ((parsed_referrer.ProtocolIsAbout() &&
        ...
        request->SetReferrerString(
            AtomicString(FetchRequestData::ClientReferrerString()));
      } else {
  ...
  if (init.hasReferrerPolicy()) {
    ...
    blink::ReferrerPolicy referrer_policy;
...
// static 
Request* Request::Create(ScriptState* script_state, const RequestInfo& input, ExceptionState& exception_state) {
  if (input.IsUSVString())
    return Create(script_state, input.GetAsUSVString(), RequestInit(), exception_state);
  return Create(script_state, input.GetAsRequest(), RequestInit(), exception_state);
}
...
Request* Request::Create(ScriptState* script_state,
  ...
  return Create(script_state, input.GetAsRequest(), request_init, exception_state);
}
...
Request* Request::Create(ScriptState* script_state,
                         ...
                         ExceptionState& exception_state) {
  // FIXME
  RequestInit request_init;//(script_state->GetIsolate(), init.V8Value(), exception_state);
  /* RequestInit params
    ByteString method;
    HeadersInit headers;
    any body;
...
String Request::referrer() const {
  ...
  DCHECK_EQ(FetchRequestData::ClientReferrerString(), "about:client");
...
String Request::mode() const {
  ...
  switch (request_->Mode()) {
    ...
    case network::mojom::FetchRequestMode::kNavigate:
      return "navigate";
    //case network::mojom::FetchRequestMode::kNavigateNestedFrame:
    //case network::mojom::FetchRequestMode::kNavigateNestedObject:
    //  return "navigate";
  }
  NOTREACHED();
  return "";
...
mojom::blink::FetchAPIRequestPtr Request::CreateFetchAPIRequest() const {
  ...
  if (!request_->ReferrerString().IsEmpty()) {
    fetch_api_request->referrer = Referrer(KURL(NullURL(), request_->ReferrerString()), request_->GetReferrerPolicy());
