...
class BodyFormDataConsumer final : public BodyConsumerBase {
 public:
  ...
  void DidFetchDataLoadedString(const String& string) override {
    auto* formData = FormData::Create(WTF::ASCIIEncoding());
...
ScriptPromise Body::arrayBuffer(ScriptState* script_state) {
  ...
  return arrayBuffer(script_state, exception);
}
...
ScriptPromise Body::blob(ScriptState* script_state) {
  NonThrowableExceptionState exception;
  return blob(script_state, exception);
}
...
ScriptPromise Body::formData(ScriptState* script_state) {
  NonThrowableExceptionState exception;
  return formData(script_state, exception);
}
...
ScriptPromise Body::json(ScriptState* script_state) {
  NonThrowableExceptionState exception;
  return json(script_state, exception);
}
...
ScriptPromise Body::text(ScriptState* script_state) {
  NonThrowableExceptionState exception;
  return text(script_state, exception);
}
...
ReadableStream* Body::body(ScriptState*) {
  return body();
}
...
Body::BodyUsed Body::IsBodyUsed() {
  NonThrowableExceptionState exception;
  return IsBodyUsed(exception);
}
...
bool Body::IsBodyLocked() {
  NonThrowableExceptionState exception;
  return IsBodyLocked(exception) == BodyLocked::kLocked ? true : false;
}
