...
DedicatedWorker* DedicatedWorker::Create(ExecutionContext* context,
                                         ...
                                         WorkerNativeClient* worker_native_client) {
  ...
  DedicatedWorker* worker = new DedicatedWorker(context, script_url, options, worker_native_client);
  worker->Start();
  return worker;
}
...
DedicatedWorker::DedicatedWorker(ExecutionContext* context,
    ...
    : AbstractWorker(context),
      ...
      worker_native_client_(worker_native_client) {
  DCHECK(context->IsContextThread());
...
void DedicatedWorker::postMessage(ScriptState* script_state,
  ...
  if (exception_state.HadException())
    return;
  if (options_.type() == "native") {
    context_proxy_->PostMessageToWorkerNative(std::move(message),
                                              std::move(channels));
  } else {
...
void DedicatedWorker::Start() {
  DLOG(INFO) << "DedicatedWorker::Start";
  DCHECK(GetExecutionContext()->IsContextThread());
  ...
  if (options_.type() == "classic") {
    DLOG(INFO) << "DedicatedWorker::Start: options_.type() == 'classic' => WorkerClassicScriptLoader->LoadAsynchronously(";
  ...
  if (options_.type() == "module") {
    DLOG(INFO) << "DedicatedWorker::Start: options_.type() == 'module' => context_proxy_->StartWorkerGlobalScope() .. modules will be fetched on worker thread";
  ...
  if (options_.type() == "native") {
    DCHECK(worker_native_client_);
    std::unique_ptr<GlobalScopeCreationParams> params = CreateGlobalScopeCreationParams();
    context_proxy_->StartWorkerGlobalScope(std::move(params), options_, script_url_, stack_id,
                                            String() /* source_code */);
    return;
  }
...
scoped_refptr<base::SingleThreadTaskRunner> DedicatedWorker::GetTaskRunner(TaskType type) {
  return context_proxy_->GetTaskRunner(type);
}
...
WorkerClients* DedicatedWorker::CreateWorkerClients() {
  DLOG(INFO) << "DedicatedWorker::CreateWorkerClients: ProvideLocalFileSystemToWorker(), ProvideIndexedDBClientToWorker()";
...
void DedicatedWorker::OnFinished(const v8_inspector::V8StackTraceId& stack_id) {
  DLOG(INFO) << "DedicatedWorker::OnFinished";
  DCHECK(GetExecutionContext()->IsContextThread());
  if (classic_script_loader_->Canceled()) {
    // Do nothing.
    DLOG(INFO) << "DedicatedWorker::OnFinished: classic_script_loader_->Canceled()";
  } else if (classic_script_loader_->Failed()) {
    DLOG(INFO) << "DedicatedWorker::OnFinished: classic_script_loader_->Failed()";
    DispatchEvent(Event::CreateCancelable(EventTypeNames::error));
  } else {
    DLOG(INFO) << "DedicatedWorker::OnFinished: calling context_proxy_->StartWorkerGlobalScope() with classic_script_loader_->SourceText()";
