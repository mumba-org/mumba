...
void DedicatedWorkerMessagingProxy::StartWorkerGlobalScope(
  ...
  DLOG(INFO) << "DedicatedWorkerMessagingProxy::StartWorkerGlobalScope";
  ...
  DLOG(INFO) << "DedicatedWorkerMessagingProxy::StartWorkerGlobalScope: InitializeWorkerThread()";
  InitializeWorkerThread(
      std::move(creation_params),
      CreateBackingThreadStartupData(ToIsolate(GetExecutionContext())));

  if (options.type() == "classic") {
    DLOG(INFO) << "DedicatedWorkerMessagingProxy::StartWorkerGlobalScope: options.type() == 'classic' => GetWorkerThread()->EvaluateClassicScript()";
    GetWorkerThread()->EvaluateClassicScript(
        script_url, source_code, nullptr /* cached_meta_data */, stack_id);
  } else if (options.type() == "module") {
    DLOG(INFO) << "DedicatedWorkerMessagingProxy::StartWorkerGlobalScope: options.type() == 'module' => GetWorkerThread()->ImportModuleScript()";
    ...
    DLOG(INFO) << "DedicatedWorkerMessagingProxy::StartWorkerGlobalScope: options.type() == 'native' => GetWorkerThread()->ImportModuleScript()";
    network::mojom::FetchCredentialsMode credentials_mode;
    bool result =
        Request::ParseCredentialsMode(options.credentials(), &credentials_mode);
    DCHECK(result);
    GetWorkerThread()->StartNative();
  } else {
    NOTREACHED();
  }
}
...
void DedicatedWorkerMessagingProxy::PostMessageToWorkerNative(scoped_refptr<SerializedScriptValue> message,
                                                              Vector<MessagePortChannel> channels) {
  if (AskedToTerminate())
    return;
  ...
  PostCrossThreadTask(
      *GetWorkerThread()->GetTaskRunner(TaskType::kPostedMessage), FROM_HERE,
      CrossThreadBind(
          &DedicatedWorkerObjectProxy::ProcessMessageFromWorkerObjectNative,
          CrossThreadUnretained(&WorkerObjectProxy()), std::move(message),
          WTF::Passed(std::move(channels)),
...
scoped_refptr<base::SingleThreadTaskRunner> DedicatedWorkerMessagingProxy::GetTaskRunner(TaskType type) {
  if (!GetWorkerThread()) {
    return scoped_refptr<base::SingleThreadTaskRunner>();
  }
  return GetWorkerThread()->GetTaskRunner(type);
}
