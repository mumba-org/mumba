...
ScriptValue ReadableStreamOperations::CreateCountQueuingStrategy(
  ...
  //if (!global->Get(context, V8AtomicString(isolate, "createBuiltInCountQueuingStrategy"))
  //         .ToLocal(&function_value)) {
  //  DLOG(INFO) << "'createBuiltInCountQueuingStrategy' not found on global object";
  //  return ScriptValue();
  //}   

  //v8::Local<v8::Function> function = function_value.As<v8::Function>();
  
  //DLOG(INFO) << "CreateCountQueuingStrategy: calling V8ScriptRunner::CallInternalFunction()";
  return ScriptValue(
      script_state,
      //V8ScriptRunner::CallInternalFunction(
      //  isolate, function, v8::Undefined(isolate), 1, args));    
      V8ScriptRunner::CallExtraOrCrash(
