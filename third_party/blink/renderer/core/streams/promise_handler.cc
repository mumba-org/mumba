...
v8::Local<v8::Promise> StreamThenPromise(v8::Local<v8::Context> context,
  ...
  DLOG(INFO) << "StreamThenPromise";
  v8::MaybeLocal<v8::Promise> result_maybe;
  if (!on_fulfilled) {
    DLOG(INFO) << "StreamThenPromise: on_fulfilled is null";
    DCHECK(on_rejected);
    ...
    result_maybe =
        promise->Then(context, on_rejected->BindToV8Function());
        //promise->Then(context, noop, on_rejected->BindToV8Function());
  } else if (on_rejected) {
    DLOG(INFO) << "StreamThenPromise: on_rejected is also here => promise->Then(on_fulfilled->BindToV8Function) ";
    //result_maybe = promise->Then(context, //on_fulfilled->BindToV8Function(),
    //                             on_rejected->BindToV8Function());
    result_maybe = promise->Then(context, on_fulfilled->BindToV8Function());
  } else {
    DLOG(INFO) << "StreamThenPromise: on_fulfilled => promise->Then(on_fulfilled->BindToV8Function) ";
    result_maybe = promise->Then(context, on_fulfilled->BindToV8Function());
  }
