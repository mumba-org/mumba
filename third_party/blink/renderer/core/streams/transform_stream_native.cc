...
class TransformStreamNative::ControllerInterface final
 ...
 public:
  ...
  void Enqueue(v8::Local<v8::Value> chunk,
    ...
    TransformStreamDefaultController::Enqueue(script_state_.get(), controller_, chunk,
                                              exception_state);
  }
...
void TransformStreamNative::InitFromJS(ScriptState* script_state,
  ...
  const auto* stream = new TransformStreamNative(
...
void TransformStreamNative::Init(ScriptState* script_state,
  ...
  auto* transform_algorithm =
      new TransformAlgorithm(transformer);
  auto* flush_algorithm = new FlushAlgorithm(transformer);
...
TransformStreamNative* TransformStreamNative::Create(
  ...
  //    prototype property).
  auto* stream = new TransformStreamNative();
  ...
  auto* controller = new TransformStreamDefaultController();
  ...
  v8::Local<v8::Promise> Run(ScriptState* script_state,
    ...
    if (stream_->had_backpressure_) {
      ...
      return StreamThenPromise(
          ...
          new ResponseFunction(script_state, stream_, chunk));
    }
 ...
 public:
  ...
  v8::Local<v8::Promise> Run(ScriptState* script_state,
    ...
    return StreamThenPromise(
        ...
        new ResolveFunction(script_state, readable),
        new RejectFunction(script_state, stream_));
  }
...
void TransformStreamNative::Initialize(
  ...
  auto* start_algorithm =
      new ReturnStartPromiseAlgorithm(start_promise);
  ...
  auto* write_algorithm =
      new DefaultSinkWriteAlgorithm(stream);
  ...
  auto* pull_algorithm =
      new DefaultSourcePullAlgorithm(stream);
  ...
  auto* cancel_algorithm =
      new DefaultSourceCancelAlgorithm(stream);
