...
v8::Local<v8::Value> TransformStreamDefaultController::SetUpFromTransformer(
  ...
  auto* controller = new TransformStreamDefaultController();//MakeGarbageCollected<TransformStreamDefaultController>();
  ...
  if (transform_method->IsUndefined()) {
    ...
    transform_algorithm = new DefaultTransformAlgorithm(controller);
...
v8::Local<v8::Promise> TransformStreamDefaultController::PerformTransform(
  ...
  return StreamThenPromise(
      ...
      new RejectFunction(script_state, controller->controlled_transform_stream_));
}
