...
StreamPromiseResolver* ReadableStreamDefaultController::PullSteps(
    ScriptState* script_state) {
  DLOG(INFO) << "ReadableStreamDefaultController::PullSteps";
  ...
  if (!queue_->IsEmpty()) {
    DLOG(INFO) << "ReadableStreamDefaultController::PullSteps: queue_->IsEmpty() = false";
    ...
    if (is_close_requested_ && queue_->IsEmpty()) {
      DLOG(INFO) << "ReadableStreamDefaultController::PullSteps: ClearAlgorithms()";
      ...
      DLOG(INFO) << "ReadableStreamDefaultController::PullSteps: CallPullIfNeeded";
      // c. Otherwise, perform !
...
void ReadableStreamDefaultController::CallPullIfNeeded(
    ScriptState* script_state,
    ReadableStreamDefaultController* controller) {
  DLOG(INFO) << "ReadableStreamDefaultController::CallPullIfNeeded";
  ...
  if (!should_pull) {
    DLOG(INFO) << "ReadableStreamDefaultController::CallPullIfNeeded: should_pull = false. canceling";
    return;
  }

  // 3. If controller.[[pulling]] is true,
  if (controller->is_pulling_) {
    DLOG(INFO) << "ReadableStreamDefaultController::CallPullIfNeeded: controller->is_pulling_ = true. returning";
  ...
  class ResolveFunction final : public PromiseHandler {
   public:
    ...
    void CallWithLocal(v8::Local<v8::Value>) override {
      DLOG(INFO) << "ReadableStreamDefaultController::CallPullIfNeeded::ResolveFunction";
  ...
  class RejectFunction final : public PromiseHandler {
   public:
    ...
    void CallWithLocal(v8::Local<v8::Value> e) override {
      DLOG(INFO) << "ReadableStreamDefaultController::CallPullIfNeeded::RejectFunction";
  ...
  StreamThenPromise(
      ...
      new ResolveFunction(script_state, controller),
      new RejectFunction(script_state, controller));
}
  ...
  if (!start_algorithm->Run(script_state, exception_state)
    ...
    if (!exception_state.HadException()) {
      ...
      exception_state.ThrowDOMException(
          static_cast<int>(kInvalidStateError),
