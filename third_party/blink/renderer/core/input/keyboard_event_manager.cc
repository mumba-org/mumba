...
void KeyboardEventManager::DefaultKeyboardEventHandler(
  ...
  DLOG(INFO) << "KeyboardEventManager::DefaultKeyboardEventHandler";
  if (event->type() == EventTypeNames::keydown) {
    DLOG(INFO) << "KeyboardEventManager::DefaultKeyboardEventHandler: event->type() == EventTypeNames::keydown";
    frame_->GetEditor().HandleKeyboardEvent(event);
    if (event->DefaultHandled()) {
      DLOG(INFO) << "KeyboardEventManager::DefaultKeyboardEventHandler: (keydown) event->DefaultHandled(). cancelling";
      return;
    }
    ...
    if (event->keyCode() == kVKeyProcessKey) {
      DLOG(INFO) << "KeyboardEventManager::DefaultKeyboardEventHandler: (keydown) event->keyCode() == kVKeyProcessKey. cancelling";
      return;
    }
    if (event->key() == "Tab") {
      DLOG(INFO) << "KeyboardEventManager::DefaultKeyboardEventHandler: (keydown) event->key() == \"Tab\" -> DefaultTabEventHandler(event)";
      DefaultTabEventHandler(event);
    } else if (event->key() == "Escape") {
      DLOG(INFO) << "KeyboardEventManager::DefaultKeyboardEventHandler: (keydown) event->key() == \"Escape\" -> DefaultEscapeEventHandler(event)";
      DefaultEscapeEventHandler(event);
    } else {
      DLOG(INFO) << "KeyboardEventManager::DefaultKeyboardEventHandler: (keydown) else {} -> DefaultArrowEventHandler(event)";
      DefaultArrowEventHandler(event, possible_focused_node);
    }
  }
  if (event->type() == EventTypeNames::keypress) {
    DLOG(INFO) << "KeyboardEventManager::DefaultKeyboardEventHandler: event->type() == EventTypeNames::keypress";
    frame_->GetEditor().HandleKeyboardEvent(event);
    if (event->DefaultHandled()) {
      DLOG(INFO) << "KeyboardEventManager::DefaultKeyboardEventHandler: (keypress) event->DefaultHandled(). cancelling";
      return;
    }
    if (event->charCode() == ' ') {
      DLOG(INFO) << "KeyboardEventManager::DefaultKeyboardEventHandler: (keypress) event->charCode() == ' '. DefaultSpaceEventHandler(event)";
