...
class NinjaBuildWriter {
 ...
 private:
  ...
  void WritePhonyRule(Target* target, const std::string& phony_name);

  const BuildSettings* build_settings_;

  const std::unordered_map<const Settings*, Toolchain*>& used_toolchains_;
  scoped_refptr<Toolchain> default_toolchain_;
  const std::vector<Target*>& default_toolchain_targets_;
