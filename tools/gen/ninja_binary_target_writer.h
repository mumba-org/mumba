...
class NinjaBinaryTargetWriter : public NinjaTargetWriter {
 public:
  ...
  NinjaBinaryTargetWriter(scoped_refptr<Target> target, std::ostream& out);
 ...
 private:
  ...
  // swift specific
  // TODO: this is not ninja specific, so why is it here?
  // cant we do this with a simple action?
  void WriteOutputFileMap();
  ...
  // the swift compiler is not a per-source compiler, but needs
  // to build all of the module sources at once 
  void WriteCompilerBuildLineForSwift(
  ...
  // |extra_object_files|.
  void ClassifyDependency(scoped_refptr<Target> dep,
