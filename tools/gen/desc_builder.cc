...
#include "gen/desc_builder.h"
#include "gen/input_file.h"
...
void RecursiveCollectChildDeps(Target* target,
                               std::set<Target*>* result);

void RecursiveCollectDeps(Target* target,
                          std::set<Target*>* result) {
...
void RecursiveCollectChildDeps(Target* target,
  ...
  for (const auto& pair : target->GetDeps(Target::DEPS_ALL))
    RecursiveCollectDeps(pair.ptr.get(), result);
}
...
class ConfigDescBuilder : public BaseDescBuilder {
 public:
  ConfigDescBuilder(Config* config, const std::set<std::string>& what)
...
#define CONFIG_VALUE_ARRAY_HANDLER(name, type)                        \
  if (what(#name)) {                                                  \
    ...
    CONFIG_VALUE_ARRAY_HANDLER(cflags_swift, std::string)
...
class TargetDescBuilder : public BaseDescBuilder {
 public:
  TargetDescBuilder(Target* target,
