...
#include "gen/ninja_action_target_writer.h"
#include "gen/ninja_binary_target_writer.h"
#include "gen/ninja_bundle_data_target_writer.h"
#include "gen/ninja_copy_target_writer.h"
#include "gen/ninja_create_bundle_target_writer.h"
...
NinjaTargetWriter::NinjaTargetWriter(scoped_refptr<Target> target,
                                     std::ostream& out)
...
// static
std::string NinjaTargetWriter::RunAndWriteFile(scoped_refptr<Target> target) {
...
std::vector<OutputFile> NinjaTargetWriter::WriteInputDepsStampAndGetDep(
    const std::vector<Target*>& extra_hard_deps,
  ...
  // files above.
  std::vector<Target*> input_deps_targets;
  ...
  const std::set<Target*>& hard_deps(target_->recursive_hard_deps());
  for (Target* target : hard_deps)
  ...
  for (Target* target : extra_hard_deps) {
  ...
  for (const auto& toolchain_dep : toolchain_deps) {
    ...
    input_deps_targets.push_back(toolchain_dep.ptr.get());
  }
