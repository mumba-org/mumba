...
typedef std::multimap<Target*, Target*> DepMap;
...
void FillDepMap(Setup* setup, DepMap* dep_map) {
  for (auto* target : setup->builder().GetAllResolvedTargets()) {
    for (const auto& dep_pair : target->GetDeps(Target::DEPS_ALL))
      dep_map->insert(std::make_pair(dep_pair.ptr.get(), target));
  }
}
...
bool TargetContainsFile(Target* target, const SourceFile& file) {
...
bool TargetReferencesConfig(Target* target, Config* config) {
...
size_t DoTreeOutput(const DepMap& dep_map,
                    const UniqueVector<Target*>& implicit_target_matches,
                    const UniqueVector<Target*>& explicit_target_matches,
                    bool all) {
  TargetSet seen_targets;
  size_t count = 0;

  // Implicit targets don't get printed themselves.
  for (Target* target : implicit_target_matches) {
    if (all)
...
size_t DoAllListOutput(
  ...
  for (Target* target : implicit_target_matches)
    RecursiveCollectChildRefs(dep_map, target, &results);
  for (Target* target : explicit_target_matches) {
...
int RunRefs(const std::vector<std::string>& args) {
  ...
  UniqueVector<Target*> explicit_target_matches;
