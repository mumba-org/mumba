...
#include "gen/ninja_build_writer.h"
...
#include "gen/input_file_manager.h"
...
Err GetDuplicateOutputError(const std::vector<Target*>& all_targets,
...
NinjaBuildWriter::NinjaBuildWriter(
    ...
    const std::unordered_map<const Settings*, Toolchain*>&
        used_toolchains,
    scoped_refptr<Toolchain> default_toolchain,
    const std::vector<Target*>& default_toolchain_targets,
    std::ostream& out,
    std::ostream& dep_out)
...
bool NinjaBuildWriter::RunAndWriteFile(
  ...
  std::vector<Target*> all_targets = builder.GetAllResolvedTargets();
  std::unordered_map<const Settings*, Toolchain*> used_toolchains;
  ...
  used_toolchains.emplace(std::make_pair(default_toolchain_settings, default_toolchain.get()));

  std::vector<Target*> default_toolchain_targets;
  ...
  for (Target* target : all_targets) {
    if (target->settings() == default_toolchain_settings) {
      ...
      // settings array.
               ...
               used_toolchains.end()) {
      scoped_refptr<Toolchain> toolchain = builder.GetToolchain(target->settings()->toolchain_label());
      used_toolchains.emplace(std::make_pair(target->settings(), toolchain.get()));
    }
  }
...
void NinjaBuildWriter::WriteAllPools() {
  ...
  std::unordered_set<Pool*> used_pools;
  for (const auto& pair : used_toolchains_) {
    for (int j = Toolchain::TYPE_NONE + 1; j < Toolchain::TYPE_NUMTYPES; j++) {
      ...
      Tool* tool = pair.second->GetTool(tool_type);
      if (tool && tool->pool().ptr)
        used_pools.insert(tool->pool().ptr.get());
    }
  }
  ...
  for (const Target* target : default_toolchain_targets_) {
    if (target->output_type() == Target::ACTION) {
      ...
      if (pool.ptr)
        used_pools.insert(pool.ptr.get());
    }
  }
...
bool NinjaBuildWriter::WriteSubninjas(Err* err) {
  ...
  std::vector<std::pair<const Settings*, Toolchain*>> sorted_settings(
      used_toolchains_.begin(), used_toolchains_.end());
  std::sort(sorted_settings.begin(), sorted_settings.end(),
            [this](const std::pair<const Settings*, Toolchain*>& a,
                   const std::pair<const Settings*, Toolchain*>& b) {
  ...
  Toolchain* previous_toolchain = nullptr;
...
bool NinjaBuildWriter::WritePhonyAndAllRules(Err* err) {
  ...
  scoped_refptr<Target> default_target = nullptr;

  // Targets in the root build file.
  std::vector<Target*> toplevel_targets;
  ...
  std::vector<Target*> toplevel_dir_targets;
  ...
  for (Target* target : default_toolchain_targets_) {
  ...
  for (Target* target : toplevel_targets) {
  ...
  for (Target* target : toplevel_dir_targets) {
    if (written_rules.insert(target->label().name()).second)
      WritePhonyRule(target, target->label().name());
  }
...
void NinjaBuildWriter::WritePhonyRule(Target* target,
