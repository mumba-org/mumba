...
std::set<Label> LabelsFor(const std::set<Target*>& targets) {
...
std::set<Target*> Intersect(const std::set<Target*>& l,
                                  const std::set<Target*>& r) {
...
Analyzer::Analyzer(const Builder& builder,
  ...
  for (auto* item : all_items_) {      
    ...
    scoped_refptr<Config> config = item->AsConfig();
    scoped_refptr<Toolchain> toolchain = item->AsToolchain();

    if (target) {
      for (const auto& dep_target_pair :
           target->GetDeps(Target::DEPS_ALL))
        dep_map_.insert(std::make_pair(dep_target_pair.ptr.get(), item));

      for (const auto& dep_config_pair : target->configs())
        dep_map_.insert(std::make_pair(dep_config_pair.ptr.get(), item));

      dep_map_.insert(std::make_pair(target->toolchain().get(), item));

      if (target->output_type() == Target::ACTION ||
          target->output_type() == Target::ACTION_FOREACH) {
        const LabelPtrPair<Pool>& pool =
            target->action_values().pool();
        if (pool.ptr)
          dep_map_.insert(std::make_pair(pool.ptr.get(), item));
      }
    } else if (config) {
      for (const auto& dep_config_pair : config->configs())
        dep_map_.insert(std::make_pair(dep_config_pair.ptr.get(), item));
    } else if (toolchain) {
      for (const auto& dep_pair : toolchain->deps())
        dep_map_.insert(std::make_pair(dep_pair.ptr.get(), item));
    } else {
      DCHECK(item->AsPool());
    }
  }
...
std::string Analyzer::Analyze(const std::string& input, Err* err) const {
  ...
  std::set<Item*> affected_items =
      GetAllAffectedItems(inputs.source_files);
  std::set<Target*> affected_targets;
  for (Item* affected_item : affected_items) {
    ...
    if (affected_item->AsTarget() &&
      ...
      affected_targets.insert(affected_item->AsTarget().get());
  }
  ...
  std::set<Target*> root_targets;
  for (auto* item : all_items_) {
    if (item->AsTarget() && dep_map_.find(item) == dep_map_.end())
      root_targets.insert(item->AsTarget().get());
  }

  std::set<Target*> compile_targets = TargetsFor(inputs.compile_labels);
  if (inputs.compile_included_all) {
    for (auto* root_target : root_targets)
      compile_targets.insert(root_target);
  }
  std::set<Target*> filtered_targets = Filter(compile_targets);
  outputs.compile_labels =
  ...
  std::set<Target*> test_targets = TargetsFor(inputs.test_labels);
...
std::set<Target*> Analyzer::TargetsFor(
  ...
  for (const auto& label : labels) {
    ...
    if (it != labels_to_items_.end()) {
      ...
      targets.insert(it->second->AsTarget().get());
    }
  }
  return targets;
}

std::set<Target*> Analyzer::Filter(
    const std::set<Target*>& targets) const {
  std::set<Target*> seen;
  std::set<Target*> filtered;
  for (auto* target : targets)
    FilterTarget(target, &seen, &filtered);
  return filtered;
}

void Analyzer::FilterTarget(Target* target,
                            std::set<Target*>* seen,
                            std::set<Target*>* filtered) const {
  if (seen->find(target) == seen->end()) {
    ...
    if (target->output_type() != Target::GROUP) {
      ...
      for (const auto& pair : target->GetDeps(Target::DEPS_ALL))
        FilterTarget(pair.ptr.get(), seen, filtered);
    }
  }
}

bool Analyzer::ItemRefersToFile(Item* item,
  ...
  for (ConfigValuesIterator iter(target.get()); !iter.done(); iter.Next()) {
  ...
  target->action_values().GetOutputsAsSourceFiles(target.get(), &outputs);
...
void Analyzer::AddItemsDirectlyReferringToFile(
    ...
    std::set<Item*>* directly_affected_items) const {
  for (auto* item : all_items_) {
...
void Analyzer::AddAllItemsReferringToItem(
    Item* item,
    std::set<Item*>* all_affected_items) const {
