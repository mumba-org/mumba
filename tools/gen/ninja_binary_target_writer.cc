...
#include "gen/ninja_binary_target_writer.h"
...
void GetPCHOutputFiles(scoped_refptr<Target> target,
...
// output vector.
void AddSourceSetObjectFiles(scoped_refptr<Target> source_set,
...
NinjaBinaryTargetWriter::NinjaBinaryTargetWriter(scoped_refptr<Target> target,
                                                 std::ostream& out)
...
//  bool is_swift_target = used_types.Get(SOURCE_SWIFT);
  ...
  std::vector<OutputFile> order_only_deps = WriteInputDepsStampAndGetDep(
      std::vector<Target*>(), num_stamp_uses);
...
void NinjaBinaryTargetWriter::WriteOutputFileMap() {
  //"output-file-map.json";

  OutputFile output_file_map_file =
      GetBuildDirForTargetAsOutputFile(target_, BuildDirType::OBJ);
  //output_file_map_file.value().append("output-file-map.json");

  std::string out_dir = target_->settings()->build_settings()->root_path_utf8() + settings_->toolchain_output_dir().value().substr(settings_->toolchain_output_dir().value().find('/')+1);
#if defined(OS_WIN)
  base::FilePath output_file_map_dir(base::ASCIIToUTF16(out_dir.append(output_file_map_file.value())));
#elif defined(OS_POSIX)
  base::FilePath output_file_map_dir(out_dir.append(output_file_map_file.value()));
#endif
  base::FilePath output_file_map_path = output_file_map_dir.AppendASCII("output-file-map.json");
  base::FilePath output_obj_dir = output_file_map_dir.AppendASCII(target_->label().name());
  ...
  for (const auto& source : target_->sources()) {
    ...
    const std::string& source_file_name_noext = source_file_name.substr(0, source_file_name.find('.'));
    ...
    output[target_->settings()->build_settings()->root_path_utf8() + source_file_full] = settings_body;
  }
...
void NinjaBinaryTargetWriter::WriteCompilerVars(
  ...
  // HERE
  // if (used_types.Get(SOURCE_SWIFT)) {
  //   std::string out_dir = target_->settings()->build_settings()->root_path_utf8() + settings_->toolchain_output_dir().value().substr(settings_->toolchain_output_dir().value().find('/')+1);
  ...
  //       if (pair.ptr->sources().size() > 0) {
  //         middle_path = pair.ptr->sources()[0].GetDir().value();
  ...
  if (used_types.Get(SOURCE_SWIFT)) {
    WriteOneFlag(SUBSTITUTION_CFLAGS_SWIFT, has_precompiled_headers,
                 Toolchain::TYPE_SWIFT, &ConfigValues::cflags_swift, opts);
  }
...
void NinjaBinaryTargetWriter::WriteGCCPCHCommand(
  ...
  } else if (tool_type == Toolchain::TYPE_SWIFT) {
    RecursiveTargetConfigStringsToStream(target_,
        &ConfigValues::cflags_swift, opts, out_);
  }
...
void NinjaBinaryTargetWriter::WriteSources(
  ...
  bool is_swift_target = false;
  std::vector<SourceFile> swift_files;
  ...
  for (const auto& source : target_->sources()) {
    ...
    // temp!
    if (tool_type == Toolchain::TYPE_SWIFT) {
      is_swift_target = true;
      swift_files.push_back(source);
      //continue;
    }
    ...
    if (tool_type != Toolchain::TYPE_SWIFT) {
      ...
      for (auto out : tool_outputs) {
        object_files->push_back(out);
      }
    }
  }
  ...
  if (is_swift_target) {
    ...
    // This is really nasty, but i dont know where
    // the paths i need really are over here

    // NOTE: this might break the gen given a case.. 
    // so we need to fix this
    //std::string last_dir = tool_outputs[0].value();
    //for (int i = 0; i < 2; i++) {
    //  auto offset = last_dir.find_last_of('/');
    //  if (offset != std::string::npos) {
    //    last_dir = last_dir.substr(0, offset);  
    //  }
    //}
    ...
    //std::string out_dir = target_->settings()->build_settings()->root_path_utf8() + settings_->toolchain_output_dir().value().substr(settings_->toolchain_output_dir().value().find('/')+1);
    //output_dir << out_dir << last_dir << "/" << target_->label().name() << ".swiftmodule";
    
   // OutputFile tool_output(output_dir.str());
    
    //printf("target %s. adding output: %s\n", target_->label().name().c_str(), output_dir.str().c_str());

    //object_files->push_back(tool_output);
    
    WriteCompilerBuildLineForSwift(
      swift_files, deps, order_only_deps, Toolchain::TYPE_SWIFT, *object_files);
...
void NinjaBinaryTargetWriter::WriteCompilerBuildLineForSwift(
  ...
  out_ << ": " << rule_prefix_ << Toolchain::ToolTypeToName(tool_type);
  out_ << " ";
  path_output_.WriteFilesFullPath(
    out_, 
...
void NinjaBinaryTargetWriter::WriteLinkerStuff(
  ...
  UniqueVector<Target*> linkable_deps;
  UniqueVector<Target*> non_linkable_deps;
...
void NinjaBinaryTargetWriter::GetDeps(
    UniqueVector<OutputFile>* extra_object_files,
    UniqueVector<Target*>* linkable_deps,
    UniqueVector<Target*>* non_linkable_deps) const {
  ...
  for (const auto& data_dep_pair : target_->data_deps())
    non_linkable_deps->push_back(data_dep_pair.ptr.get());
}

void NinjaBinaryTargetWriter::ClassifyDependency(
    scoped_refptr<Target> dep,
    UniqueVector<OutputFile>* extra_object_files,
    UniqueVector<Target*>* linkable_deps,
    UniqueVector<Target*>* non_linkable_deps) const {
...
void NinjaBinaryTargetWriter::WriteOrderOnlyDependencies(
    const UniqueVector<Target*>& non_linkable_deps) {
