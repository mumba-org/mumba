...
void Scheduler::AddUnknownGeneratedInput(scoped_refptr<Target> target,
                                         const SourceFile& file) {
  base::AutoLock lock(lock_);
  unknown_generated_inputs_.insert(std::make_pair(file, target.get()));
}

void Scheduler::AddWriteRuntimeDepsTarget(scoped_refptr<Target> target) {
  base::AutoLock lock(lock_);
  write_runtime_deps_targets_.push_back(target.get());
}

std::vector<Target*> Scheduler::GetWriteRuntimeDepsTargets() const {
  base::AutoLock lock(lock_);
...
std::multimap<SourceFile, Target*>
  ...
  std::multimap<SourceFile, Target*> filtered = unknown_generated_inputs_;
