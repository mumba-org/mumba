...
#include "gen/parse_tree.h"
#include "gen/path_output.h"
#include "gen/source_file_type.h"
#include "gen/standard_out.h"
#include "gen/target.h"
#include "gen/variables.h"
#include "gen/visual_studio_utils.h"
#include "gen/xml_element_writer.h"
...
std::string GetConfigurationType(scoped_refptr<Target> target, Err* err) {
...
void ParseCompilerOptions(scoped_refptr<Target> target, CompilerOptions* options) {
  for (ConfigValuesIterator iter(target); !iter.done(); iter.Next()) {
    ...
    ParseCompilerOptions(iter.cur().cflags_swift(), options);
  }
}
...
void ParseLinkerOptions(scoped_refptr<Target> target, LinkerOptions* options) {
  for (ConfigValuesIterator iter(target); !iter.done(); iter.Next()) {
...
bool FilterTargets(const BuildSettings* build_settings,
  ...
  while (!to_process.empty()) {
    Target* target = to_process.front();
    to_process.pop();
    for (const auto& pair : target->GetDeps(Target::DEPS_ALL)) {
      if (labels.find(pair.label) == labels.end()) {
        targets->push_back(pair.ptr.get());
        to_process.push(pair.ptr.get());
...
bool UnicodeTarget(scoped_refptr<Target> target) {
...
bool VisualStudioWriter::RunAndWriteFiles(const BuildSettings* build_settings,
  ...
  for (Target* target : targets) {
    ...
    if (!writer.WriteProjectFiles(scoped_refptr<Target>(target), ninja_extra_args, err))
      return false;
  }
...
bool VisualStudioWriter::WriteProjectFiles(scoped_refptr<Target> target,
                                           const std::string& ninja_extra_args,
                                           Err* err) {
...
std::string VisualStudioWriter::GetNinjaTarget(scoped_refptr<Target> target) {
