...
class Analyzer {
 ...
 private:
  ...
  std::set<Item*> GetAllAffectedItems(
  ...
  std::set<Target*> TargetsFor(const std::set<Label>& labels) const;
  ...
  std::set<Target*> Filter(const std::set<Target*>& targets) const;

  // Filter an individual target and adds the results to filtered
  // (see Filter(), above).
  void FilterTarget(Target*,
                    std::set<Target*>* seen,
                    std::set<Target*>* filtered) const;

  bool ItemRefersToFile(Item* item, const SourceFile* file) const;

  void AddItemsDirectlyReferringToFile(
      const SourceFile* file,
      std::set<Item*>* affected_items) const;

  void AddAllItemsReferringToItem(Item* item,
                                  std::set<Item*>* affected_items) const;
