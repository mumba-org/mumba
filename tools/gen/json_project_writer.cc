...
void AddTargetDependencies(Target* target,
                           std::set<Target*>* deps) {
  for (const auto& pair : target->GetDeps(Target::DEPS_LINKED)) {
    if (deps->find(pair.ptr.get()) == deps->end()) {
      deps->insert(pair.ptr.get());
      AddTargetDependencies(pair.ptr.get(), deps);
    }
  }
}
