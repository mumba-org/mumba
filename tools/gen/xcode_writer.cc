...
#include "gen/xcode_object.h"

namespace {

using TargetToFileList = std::unordered_map<Target*, Target::FileList>;
using TargetToTarget = std::unordered_map<Target*, Target*>;
using TargetToPBXTarget = std::unordered_map<Target*, PBXTarget*>;
...
bool IsApplicationTarget(scoped_refptr<Target> target) {
  return target->output_type() == Target::CREATE_BUNDLE &&
         target->bundle_data().product_type() ==
             "com.apple.product-type.application";
}

bool IsXCUITestRunnerTarget(scoped_refptr<Target> target) {
...
bool IsXCTestModuleTarget(scoped_refptr<Target> target) {
  return target->output_type() == Target::CREATE_BUNDLE &&
         target->bundle_data().product_type() ==
...
bool IsXCUITestModuleTarget(scoped_refptr<Target> target) {
  return target->output_type() == Target::CREATE_BUNDLE &&
         target->bundle_data().product_type() ==
...
void AddDependencyTargetForTestModuleTargets(
  ...
  for (Target* target : targets) {
    ...
    Target* test_application_target = FindApplicationTargetByName(
...
void SearchXCTestFilesForTarget(scoped_refptr<Target> target,
                                TargetToFileList* xctest_files_per_target) {
  // Early return if already visited and processed.
  if (xctest_files_per_target->find(target.get()) != xctest_files_per_target->end())
    return;
  ...
  for (const auto& t : target->public_deps()) {
    ...
    const Target::FileList& deps_xctest_files =
        (*xctest_files_per_target)[t.ptr.get()];
  ...
  xctest_files_per_target->insert(std::make_pair(target.get(), xctest_files));
}
...
bool XcodeWriter::FilterTargets(const BuildSettings* build_settings,
  ...
  for (Target* target : all_targets) {
    ...
    for (const auto& pair : target->GetDeps(Target::DEPS_LINKED)) {
      ...
      auto iter = std::lower_bound(targets->begin(), targets->end(), pair.ptr.get());
      if (iter != targets->end() && *iter == pair.ptr.get())
        targets->erase(iter);
    }
  }
...
void XcodeWriter::CreateProductsProject(
  ...
  std::vector<Target*> bundle_targets;
  ...
  for (Target* target : targets) {
    switch (target->output_type()) {
      ...
      case Target::CREATE_BUNDLE: {
        ...
        Target* target_with_xctest_files = nullptr;
