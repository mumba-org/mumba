...
class BuilderRecord {
 public:
  ...
  static bool IsItemOfType(scoped_refptr<Item> item, ItemType type);

  // Returns the type enum for the given item.
  static ItemType TypeOfItem(scoped_refptr<Item> item);

  scoped_refptr<Item> item() const { return item_; }
  //const Item* item() const { return item_.get(); }
  void set_item(scoped_refptr<Item> item) { item_ = std::move(item); }
