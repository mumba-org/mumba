...
using RuntimeDepsVector = std::vector<std::pair<OutputFile, Target*>>;
...
void RecursiveCollectRuntimeDeps(scoped_refptr<Target> target,
                                 bool is_target_data_dep,
                                 RuntimeDepsVector* deps,
                                 std::map<Target*, bool>* seen_targets,
                                 std::set<OutputFile>* found_files) {
  const auto& found_seen_target = seen_targets->find(target.get());
  ...
  seen_targets->emplace(std::make_pair(target.get(), is_target_data_dep));
...
bool CollectRuntimeDepsFromFlag(const Builder& builder,
  ...
  for (const auto& line :
    ...
    scoped_refptr<Item> item = builder.GetItem(label);
    scoped_refptr<Target> target = item ? item->AsTarget() : scoped_refptr<Target>();
    if (!target) {
    ...
    files_to_write->push_back(std::make_pair(output_file, target.get()));
  }
  return true;
}
...
RuntimeDepsVector ComputeRuntimeDeps(scoped_refptr<Target> target) {
...
bool WriteRuntimeDepsFilesIfNecessary(const Builder& builder, Err* err) {
  ...
  for (Target* target : g_scheduler->GetWriteRuntimeDepsTargets()) {
