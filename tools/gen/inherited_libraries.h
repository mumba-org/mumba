...
class InheritedLibraries {
 public:
  ...
  std::vector<Target*> GetOrdered() const;
  std::vector<std::pair<Target*, bool>> GetOrderedAndPublicFlag() const;

  // Adds a single dependency to the end of the list. See note on adding above.
  void Append(scoped_refptr<Target> target, bool is_public);
 ...
 private:
  ...
  typedef std::map<Target*, Node> LibraryMap;
  LibraryMap map_;
