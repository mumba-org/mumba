...
void Args::AddDefaultArgOverrides(const Scope::KeyValueMap& overrides) {
  base::AutoLock lock(lock_);
  for (const auto& cur_override : overrides) {
    DLOG(INFO) << "Args::AddDefaultArgOverrides: key = " << cur_override.first;
    if (cur_override.second.type() == Value::STRING) {
      DLOG(INFO) << "Args::AddDefaultArgOverrides: value = " << cur_override.second.string_value();
    }
    overrides_[cur_override.first] = cur_override.second;
  }
}
