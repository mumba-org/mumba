...
#include "gen/c_include_iterator.h"
...
// toolchain.
bool TargetLabelsMatchExceptToolchain(Target* a, Target* b) {
...
bool FriendMatches(Target* annotation_on,
                   Target* is_marked_friend) {
...
bool HeaderChecker::Run(const std::vector<Target*>& to_check,
                        bool force_check,
      ...
      if (vect_i.target->check_includes()) {
        ...
        base::PostTaskWithTraits(FROM_HERE, {base::MayBlock()},
                                 base::BindOnce(&HeaderChecker::DoWork, this,
                                                scoped_refptr<Target>(vect_i.target), file.first));
      }
    }
  }
...
void HeaderChecker::DoWork(scoped_refptr<Target> target, const SourceFile& file) {
  Err err;
...
// static
void HeaderChecker::AddTargetToFileMap(Target* target, FileMap* dest) {
...
bool HeaderChecker::CheckFile(scoped_refptr<Target> from_target,
  ...
  for (ConfigValuesIterator iter(from_target.get()); !iter.done(); iter.Next()) {
...
bool HeaderChecker::CheckInclude(scoped_refptr<Target> from_target,
                                 const InputFile& source_file,
  ...
  for (const auto& target : targets) {
    ...
    // target.
    Target* to_target = target.target;
    if (to_target == from_target)
      return true;

    bool is_permitted_chain = false;
    if (IsDependencyOf(to_target, from_target.get(), &chain, &is_permitted_chain)) {
      DCHECK(chain.size() >= 2);
      ...
      bool effectively_public =
          target.is_public || FriendMatches(to_target, from_target.get());
  ...
  if (!found_dependency) {
    ...
    *err = MakeUnreachableError(source_file, range, from_target.get(), targets);
    return false;
  }
...
bool HeaderChecker::IsDependencyOf(Target* search_for,
                                   Target* search_from,
                                   Chain* chain,
...
bool HeaderChecker::IsDependencyOf(Target* search_for,
  ...
  std::map<Target*, ChainLink> breadcrumbs;
  ...
  while (!work_queue.empty()) {
    ...
    scoped_refptr<Target> target = cur_link.target;
    work_queue.pop();

    if (target.get() == search_for) {
      // Found it! Reconstruct the chain.
      chain->clear();
      while (target.get() != search_from) {
        chain->push_back(cur_link);
        cur_link = breadcrumbs[target.get()];
        target = cur_link.target;
      }
      chain->push_back(ChainLink(scoped_refptr<Target>(search_from), true));
      return true;
    }

    // Always consider public dependencies as possibilities.
    for (const auto& dep : target->public_deps()) {
      if (breadcrumbs.insert(std::make_pair(dep.ptr.get(), cur_link)).second)
...
Err HeaderChecker::MakeUnreachableError(
  ...
  std::vector<Target*> targets_with_matching_toolchains;
  std::vector<Target*> targets_with_other_toolchains;
  ...
  for (int other_index = 0;
       ...
       other_index++) {
    for (Target* cur_matching : targets_with_matching_toolchains) {
