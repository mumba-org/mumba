...
std::vector<Target*> InheritedLibraries::GetOrdered() const {
...
std::vector<std::pair<Target*, bool>>
InheritedLibraries::GetOrderedAndPublicFlag() const {
  std::vector<std::pair<Target*, bool>> result;
  result.resize(map_.size());
...
void InheritedLibraries::Append(scoped_refptr<Target> target, bool is_public) {
  // Try to insert a new node.
  auto insert_result = map_.insert(
      std::make_pair(target.get(), Node(map_.size(), is_public)));
