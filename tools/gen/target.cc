...
void MergePublicConfigsFrom(scoped_refptr<Target> from_target,
...
void MergeAllDependentConfigsFrom(scoped_refptr<Target> from_target,
                                  UniqueVector<LabelConfigPair>* dest,
...
Err MakeTestOnlyError(scoped_refptr<Target> from, scoped_refptr<Target> to) {
...
bool EnsureFileIsGeneratedByDependency(scoped_refptr<Target> target,
                                       const OutputFile& file,
                                       bool check_private_deps,
                                       bool consider_object_files,
                                       bool check_data_deps,
                                       std::set<Target*>* seen_targets) {
  if (seen_targets->find(target.get()) != seen_targets->end())
    return false;  // Already checked this one and it's not found.
  seen_targets->insert(target.get());
...
Target::Target(const Settings* settings,
    ...
    : Item(settings, label, build_dependency_files),
      ...
      has_module_map_cached_(false),
      has_module_map_(false),
...
scoped_refptr<Target> Target::AsTarget() {
  return this;
}
...
std::string Target::GetComputedOutputName() {
  DCHECK(toolchain_)
  ...
  std::string result;
  const Tool* tool = toolchain_->GetToolForTargetFinalOutput(scoped_refptr<Target>(this));
  if (tool) {
...
bool Target::SetToolchain(scoped_refptr<Toolchain> toolchain, Err* err) {
  DCHECK(!toolchain_);
...
bool Target::GetOutputFilesForSource(const SourceFile& source,
  ...
  *computed_tool_type = toolchain_->GetToolTypeForSourceType(file_type);
  ...
  SubstitutionWriter::ApplyListToCompilerAsOutputFile(
      scoped_refptr<Target>(this), source, tool->outputs(), outputs);
...
void Target::PullDependentTargetLibsFrom(scoped_refptr<Target> dep, bool is_public) {
...
void Target::PullRecursiveHardDeps() {
  for (const auto& pair : GetDeps(DEPS_LINKED)) {
    ...
    if (pair.ptr->hard_dep())
      recursive_hard_deps_.insert(pair.ptr.get());
...
bool Target::ResolvePrecompiledHeaders(Err* err) {
  ...
  for (ConfigValuesIterator iter(scoped_refptr<Target>(this)); !iter.done(); iter.Next()) {
    if (!iter.GetCurrentConfig())
      continue;  // Skip the one on the target itself.

    scoped_refptr<Config> config = iter.GetCurrentConfig();
...
bool Target::CheckVisibility(Err* err) {
  for (const auto& pair : GetDeps(DEPS_ALL)) {
    if (!Visibility::CheckItemVisibility(scoped_refptr<Target>(this), pair.ptr, err))
      return false;
  }
  return true;
}

bool Target::CheckTestonly(Err* err) {
  ...
  for (const auto& pair : GetDeps(DEPS_ALL)) {
    if (pair.ptr->testonly()) {
      *err = MakeTestOnlyError(scoped_refptr<Target>(this), pair.ptr);
      return false;
    }
  }
...
bool Target::CheckAssertNoDeps(Err* err) {
  ...
  if (!RecursiveCheckAssertNoDeps(scoped_refptr<Target>(this), false, assert_no_deps_, &visited,
                                  &failure_path_str, &failure_pattern)) {
...
void Target::CheckSourcesGenerated() {
...
void Target::CheckSourceGenerated(const SourceFile& source) {
  ...
  if (!EnsureFileIsGeneratedByDependency(scoped_refptr<Target>(this), out_file, true,
    ...
    if (!EnsureFileIsGeneratedByDependency(scoped_refptr<Target>(this), out_file, true,
                                           consider_object_files,
                                           check_data_deps, &seen_targets))
      g_scheduler->AddUnknownGeneratedInput(scoped_refptr<Target>(this), source);
  }
}

bool Target::ComputeModuleMap() {
  // check deps
  for (const auto& dep : private_deps_ ) {
    if (dep.ptr->has_module_map()) {
      module_map_ = dep.ptr->module_map_;
      return true;
    }
  }

  for (const auto& source : sources_) {
    if (source.GetName() == "module.modulemap") {
      module_map_ = source;
      return true;
    }
  }
...
void Target::ConfigureSwiftTarget() {
  std::vector<SourceFile> module_maps;  

  // 1 - add extra includes
  // TODO: maybe if we use rebase
  
  std::string rel_out = settings()->toolchain_output_dir().value();
      
  std::string out_dir = 
    settings()->build_settings()->root_path_utf8() + rel_out.substr(rel_out.find('/') + 1);
  
  std::string gen_dir = GetBuildDirAsOutputFile(
   BuildDirContext(this),
  ...
  for (const auto& pair : GetDeps(Target::DEPS_ALL)) {
    ...
    if (dep_target->sources().size() > 0) {
      ...
      auto inout_offset = middle_path.find(rel_out);
      size_t init = 1;
      size_t endby = 2;

      if (inout_offset != std::string::npos) {
        init = 0;
        endby = 1;
        middle_path = middle_path.substr(rel_out.size()-endby);
      }

      auto off0 = middle_path.find("Sources");
      auto off1 = middle_path.find("Source");
      
      // try with 's' first
      if (off0 != std::string::npos) {
        middle_path = middle_path.substr(init, off0-endby);
      } else {
        // now without it
        if(off1 != std::string::npos) {
          middle_path = middle_path.substr(init, off1-endby);
        } else {
          middle_path = middle_path.substr(init, middle_path.size()-2);
        }
      }

      if (auto off3 = middle_path.find(gen_dir) != std::string::npos) {
        middle_path = middle_path.substr(gen_dir.size());
      }

    }
    std::string fullpath = out_dir + "obj" + middle_path;
    config_values().include_dirs().push_back(SourceDir(fullpath));
  ...
  for (const auto& source : module_maps) {
    ...
    out.append(settings()->build_settings()->root_path_utf8());
    ...
    config_values().cflags_swift().push_back("-Xcc");
    config_values().cflags_swift().push_back(out);
  }
}
