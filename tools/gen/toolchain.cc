...
const char* Toolchain::kToolSwift = "swiftc";
...
scoped_refptr<Toolchain> Toolchain::AsToolchain() {
  return this;
}
...
Toolchain::ToolType Toolchain::ToolNameToType(const base::StringPiece& str) {
  ...
  if (str == kToolSwift) return TYPE_SWIFT;
...
std::string Toolchain::ToolTypeToName(ToolType type) {
  switch (type) {
    ...
    case TYPE_SWIFT: return kToolSwift;
...
const Tool* Toolchain::GetToolForTargetFinalOutput(scoped_refptr<Target> target) const {
