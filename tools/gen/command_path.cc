...
void BreadthFirstSearch(const Target* from, const Target* to,
  ...
  while (!work_queue.empty()) {
    ...
    for (const auto& pair : current_target->public_deps()) {
      ...
      work_queue.back().push_back(TargetDep(pair.ptr.get(), DepType::PUBLIC));
    }
    ...
    if (private_deps == PrivateDeps::INCLUDE) {
      ...
      for (const auto& pair : current_target->private_deps()) {
        ...
        work_queue.back().push_back(
            TargetDep(pair.ptr.get(), DepType::PRIVATE));
      }
    }
    ...
    if (data_deps == DataDeps::INCLUDE) {
      ...
      for (const auto& pair : current_target->data_deps()) {
        work_queue.push_back(current_path);
        work_queue.back().push_back(TargetDep(pair.ptr.get(), DepType::DATA));
      }
    }
  }
}
...
int RunPath(const std::vector<std::string>& args) {
  ...
  scoped_refptr<Target> target1 = ResolveTargetFromCommandLineString(setup, args[1]);
  if (!target1)
    return 1;
  scoped_refptr<Target> target2 = ResolveTargetFromCommandLineString(setup, args[2]);
  if (!target2)
    return 1;
  ...
  Stats stats;
  DoSearch(target1.get(), target2.get(), options, &stats);
  if (stats.total_paths() == 0) {
    ...
    DoSearch(target2.get(), target1.get(), options, &stats);
  }
