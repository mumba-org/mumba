...
bool PrintTarget(Target* target,
...
#define HANDLER(property, handler_name) \
  ...
  HANDLER(variables::kCflagsSwift, DefaultHandler);
...
bool PrintConfig(Config* config,
                 const std::string& what,
                 bool single_config) {
  std::unique_ptr<base::DictionaryValue> dict =
...
int RunDesc(const std::vector<std::string>& args) {
  ...
  if (json) {
    ...
    if (!target_matches.empty()) {
      for (auto* target : target_matches) {
    ...
    for (Target* target : target_matches) {
      if (printed_output)
        OutputString("\n\n");
      printed_output = true;
    ...
    for (Config* config : config_matches) {
      if (printed_output)
        OutputString("\n\n");
      printed_output = true;
