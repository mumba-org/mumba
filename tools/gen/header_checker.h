...
class HeaderChecker : public base::RefCountedThreadSafe<HeaderChecker> {
 public:
  ...
  struct ChainLink {
    ...
    ChainLink(scoped_refptr<Target> t, bool p) : target(std::move(t)), is_public(p) {}
  ...
  bool Run(const std::vector<Target*>& to_check,
           bool force_check,
 ...
 private:
  ...
  struct TargetInfo {
    ...
    TargetInfo(Target* t, bool is_pub, bool is_gen)
        : target(t),
          is_public(is_pub),
          is_generated(is_gen) {
    }
  ...
  void DoWork(scoped_refptr<Target> target, const SourceFile& file);

  // Adds the sources and public files from the given target to the given map.
  static void AddTargetToFileMap(Target* target, FileMap* dest);
  ...
  // error messages.
  bool CheckFile(scoped_refptr<Target> from_target,
  ...
  // reporting.
  bool CheckInclude(scoped_refptr<Target> from_target,
                    const InputFile& source_file,
  ...
  // included.
  bool IsDependencyOf(Target* search_for,
                      Target* search_from,
                      Chain* chain,
