...
int main(int argc, char** argv) {
  ...
  const google::protobuf::Descriptor* output_descr = GetMessageDescriptorNamed(file_descr, "ListServiceResult");
  if (!output_descr) {
  ...
  if (cmd->HasSwitch("port")) {
    ...
    if (!base::StringToInt(port_string, &int_port)) {
      printf("service list: port parameter is not a integer\n");
      return 1;
    }
  }
  ...
  grpc_slice method = grpc_slice_from_static_string("/mumba.Shell/ListServices");
  ...
  grpc_event event = grpc_completion_queue_pluck(completion_queue, tag(1), grpc_timeout_milliseconds_to_deadline(1000), nullptr);
