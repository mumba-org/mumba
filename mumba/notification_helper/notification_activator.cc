...
#include "mumba/notification_helper/notification_activator.h"
...
//#include "core/common/switches.h"
...
HRESULT NotificationActivator::Activate(
  ...
  command_line.AppendSwitchNative("notification-launch-id",
                                  invoked_args);

  // Check to see if a user response (inline reply) is also supplied.
  for (ULONG i = 0; i < count; ++i) {
    if (lstrcmpW(kUserResponse, data[i].Key) == 0) {
      command_line.AppendSwitchNative("notification-inline-reply",
                                      data[i].Value);
      break;
    }
  }
