...
bool Bootstrapper::CompileExtraBuiltin(Isolate* isolate, int index) {
  ...
  bool r = Bootstrapper::CompileNative(isolate, name, source_code,
                                     arraysize(args), args, EXTENSION_CODE);
  printf("CompileExtraBuiltin: %d => '%s' ok? %d\n", index, name.start(), r);
  return r;
}
...
bool Genesis::InstallExtraNatives() {
  ...
  for (int i = 0;//ExtraNatives::GetDebuggerCount();
