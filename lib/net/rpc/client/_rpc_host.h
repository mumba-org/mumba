...
typedef std::unordered_map<std::string, RpcDescriptor*> RpcNodes;
...
public:
  ...
  RpcDescriptor* GetNode(const std::string& name) const;
  RpcDescriptor* GetNode(const base::UUID& uuid) const;
  void AddNode(RpcDescriptor* node);
  void AddNode(std::unique_ptr<RpcDescriptor> node);
  void AddNodes(std::vector<RpcDescriptor> descriptors);
  void RemoveNode(RpcDescriptor* node);
...
private:
  ...
  std::vector<RpcDescriptor> owned_nodes_;
