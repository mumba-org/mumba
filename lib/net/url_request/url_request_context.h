...
class NET_EXPORT URLRequestContext
 ...
 public:
  ...
  // hold call states(streams) that must outlive the transaction state
  RpcNetworkSession* rpc_network_session() const {
    return rpc_network_session_.get();
  }

  void set_rpc_network_session(
      std::unique_ptr<RpcNetworkSession> rpc_network_session);
 ...
 private:
  ...
  std::unique_ptr<RpcNetworkSession> rpc_network_session_;
#if BUILDFLAG(ENABLE_REPORTING)
