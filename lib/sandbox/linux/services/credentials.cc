...
__attribute__((no_sanitize_memory))
#endif
int ChrootToGivenDir(void* dir) {
  // This function can be run from a vforked child, so it should not write to
  // any memory other than the stack or errno. Reads from TLS may be different
  // from in the parent process.
  RAW_CHECK(sys_chroot((const char*)dir) == 0);
...
#error "Unsupported architecture"
  ...
  pid = clone(DontChrootCb, stack, clone_flags, nullptr, nullptr, tls,
              nullptr);
  PCHECK(pid != -1);
...
bool ChrootToDir(const base::FilePath& path) {
  // We need to chroot to a fdinfo that is unique to a process and have that
  // process die.
  // 1. We don't want to simply fork() because duplicating the page tables is
  // slow with a big address space.
...
#error "Unsupported architecture"
  ...
  tls = tls_buf;
#endif
  pid = clone(ChrootToGivenDir, stack, clone_flags, const_cast<char *>(path.value().c_str()), nullptr, tls,
              nullptr);
  PCHECK(pid != -1);
...
bool Credentials::DontDropFileSystemAccess(int proc_fd) {
  CHECK_LE(0, proc_fd);
...
bool Credentials::ChrootTo(int proc_fd, const base::FilePath& path) {
  CHECK_LE(0, proc_fd);

  CHECK(ChrootToDir(path));
  CHECK(!HasFileSystemAccess());
  CHECK(!ProcUtil::HasOpenDirectory(proc_fd));
  // We never let this function fail.
  return true;
