...
std::string ns_type_to_name(int type) {
...
class WriteUidGidMapChrootedDelegate : public base::LaunchOptions::PreExecDelegate {
 public:
  WriteUidGidMapChrootedDelegate(
    const base::FilePath& chroot_dir)
      : uid_(getuid()),
        gid_(getgid()),
        chroot_dir_(chroot_dir) {}

  ~WriteUidGidMapChrootedDelegate() override {}
  ...
  void RunAsyncSafe() override {
    ...
    RAW_CHECK(Credentials::SetCapabilitiesOnCurrentThread(caps));
  
    RAW_CHECK(NamespaceUtils::WriteToIdMapFile("/proc/self/uid_map", uid_));
    RAW_CHECK(NamespaceUtils::WriteToIdMapFile("/proc/self/gid_map", gid_));
    //RAW_CHECK(sys_chroot(chroot_dir_.value().c_str()) == 0);
    //RAW_CHECK(chdir("/") == 0);

    // Make sure we drop CAP_SYS_ADMIN.
    RAW_CHECK(Credentials::DropAllCapabilities());
    ...
    RAW_CHECK(sys_unshare(CLONE_NEWUSER) == 0);
  }

 private:
  const uid_t uid_;
  const gid_t gid_;
  base::FilePath chroot_dir_;

  DISALLOW_COPY_AND_ASSIGN(WriteUidGidMapChrootedDelegate);
};
...
base::Process NamespaceSandbox::LaunchProcess(
    ...
    base::FilePath chroot_path) {
  return LaunchProcessWithOptions(cmdline.argv(), launch_options, Options(), std::move(chroot_path));
}
...
base::Process NamespaceSandbox::LaunchProcessWithOptions(
  ...
  for (const int ns_type : kSupportedTypes) {
    ...
    if (NamespaceUtils::KernelSupportsUnprivilegedNamespace(ns_type)) {
      DLOG(INFO) << "kernel support unprivileged " << ns_type_to_name(ns_type) << ". turning on";
      clone_flags |= ns_type;
    } else if (ns_sandbox_options.fail_on_unsupported_ns_type) {
      LOG(ERROR) << "unsupport namespace type for unprivileged access: " << ns_type_to_name(ns_type)
        << "\nfailing and returning a null process";
      return base::Process();
    }
  }
  ...
  WriteUidGidMapChrootedDelegate write_uid_gid_map_delegate_for_chroot(chroot_path);

  base::LaunchOptions launch_options_copy = launch_options;
  //base::LaunchOptions::PreExecDelegate* delegate = &write_uid_gid_map_delegate;
  
  //if (!chroot_path.empty()) {
  base::LaunchOptions::PreExecDelegate*  delegate = &write_uid_gid_map_delegate_for_chroot;
  //}
  
  launch_options_copy.pre_exec_delegate = delegate;
...
pid_t NamespaceSandbox::ForkInNewPidNamespace(bool drop_capabilities_in_child) {
  ...
  DLOG(INFO) << "NamespaceSandbox::ForkInNewPidNamespace: pid = " << pid;
  return pid;
}
