...
class SANDBOX_EXPORT Credentials {
 public:
  ...
  static bool DontDropFileSystemAccess(int proc_fd) WARN_UNUSED_RESULT;
  
  // like the above, but we enter in a given chroot. USE ONE OR OTHER
  // this is a special case for containers, with their own root filesystem
  static bool ChrootTo(int proc_fd, const base::FilePath& path) WARN_UNUSED_RESULT;
