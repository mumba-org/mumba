...
bool GpuInit::InitializeAndStartSandbox(base::CommandLine* command_line,
  ...
  //(INFO) << "GpuInit: gl_disabled = " << gl_disabled;
  ...
  if (!gl_disabled && !use_swiftshader) {
    if (!CollectGraphicsInfo(&gpu_info_)) {
      DLOG(INFO) << "GpuInit: CollectGraphicsInfo() failed";
      return false;
    }
    gpu::SetKeysForCrashLogging(gpu_info_);
    gpu_feature_info_ = gpu::ComputeGpuFeatureInfo(
  ...
  if (use_swiftshader) {
    ...
    //DLOG(INFO) << "GpuInit: gl_disabled = true -> AdjustInfoToNoGpu()";  
    AdjustInfoToNoGpu();
  }
  ...
  if (kGpuFeatureStatusEnabled !=
    ...
    //DLOG(INFO) << "GpuInit: accelerated video decode disabled";  
  ...
  //DLOG(INFO) << "GpuInit: use_swiftshader? " << use_swiftshader << " SoftwareGLImplementation ? " << (gl::GetGLImplementation() == gl::GetSoftwareGLImplementation());  
    
  if (use_swiftshader ||
      gl::GetGLImplementation() == gl::GetSoftwareGLImplementation()) {
    DLOG(INFO) << "GpuInit: falling for software rendering";
