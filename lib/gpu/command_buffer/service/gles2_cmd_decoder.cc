...
void GLES2DecoderImpl::DoBindTexture(GLenum target, GLuint client_id) {
 // DLOG(INFO) << "GLES2DecoderImpl::DoBindTexture: client_id = " << client_id;
  TextureRef* texture_ref = nullptr;
  GLuint service_id = 0;
  if (client_id != 0) {
    texture_ref = GetTexture(client_id);
    if (!texture_ref) {
      //DLOG(INFO) << "GLES2DecoderImpl::DoBindTexture: GetTexture(client_id) = null";
      if (!group_->bind_generates_resource()) {
        LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION,
                           "glBindTexture",
                           "id not generated by glGenTextures");
        DLOG(INFO) << "GLES2DecoderImpl::DoBindTexture: OOPS";
        return;
      }
    ...
    //DLOG(INFO) << "GLES2DecoderImpl::DoBindTexture: texture_manager()->GetDefaultTextureInfo() = " << texture_ref;
  }
  ...
  if (texture_ref) {
    ...
    //DLOG(INFO) << "GLES2DecoderImpl::DoBindTexture: texture_ref = " << texture_ref << " => api()->glBindTextureFn(target, 0)";
