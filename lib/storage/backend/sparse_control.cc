...
// const int64_t kMaxMapSize = 8 * 1024;

// We can have up to 131k children.
const int64_t kMaxMapSize = 8 * 16384;

// The maximum number of bytes that a child can store.
const int64_t kMaxEntrySize = 0x100000;

// The size of each data block (tracked by the child allocation bitmap).
//const int64_t kBlockSize = 1024;
const int64_t kBlockSize = 16384;
...
class ChildrenDeleter
    : public base::RefCounted<ChildrenDeleter>,
      public storage::FileIOCallback {
 public:
  ChildrenDeleter(storage::StorageBackend* backend, const std::string& name)
  ...
  void Start(char* buffer, int64_t len);
  void ReadData(storage::Addr address, int64_t len);
 ...
 private:
  ...
  base::WeakPtr<storage::StorageBackend> backend_;
  std::string name_;
  storage::Bitmap children_map_;
  int64_t signature_;
...
void ChildrenDeleter::OnFileIOComplete(int64_t bytes_copied) {
  char* buffer = buffer_.release();
  Start(buffer, bytes_copied);
}

void ChildrenDeleter::Start(char* buffer, int64_t len) {
  buffer_.reset(buffer);
  if (len < static_cast<int>(sizeof(storage::SparseData)))
    return Release();

  // Just copy the information from |buffer|, delete |buffer| and start deleting
  // the child entries.
  storage::SparseData* data =
      reinterpret_cast<storage::SparseData*>(buffer);
  signature_ = data->header.signature;

  int64_t num_bits = (len - sizeof(storage::SparseHeader)) * 8;
...
void ChildrenDeleter::ReadData(storage::Addr address, int64_t len) {
  DCHECK(address.is_block_file());
  if (!backend_.get())
    return Release();

  storage::File* file(backend_->File(address));
  if (!file)
    return Release();
  
  size_t file_offset = address.start_block() * address.BlockSize() +
                       storage::kBlockHeaderSize;

  //D//LOG(INFO) << "start_block: " << address.start_block() << " block_size: " << address.BlockSize() << " = offset: " << file_offset << " requested len: " << len; 
...
net::NetLogEventType GetSparseEventType(
    storage::SparseControl::SparseOperation operation) {
  switch (operation) {
    case storage::SparseControl::kReadOperation:
      return net::NetLogEventType::SPARSE_READ;
    case storage::SparseControl::kWriteOperation:
      return net::NetLogEventType::SPARSE_WRITE;
    case storage::SparseControl::kGetRangeOperation:
...
SparseControl::SparseControl(StorageEntry* entry)
    : entry_(entry),
      child_(NULL),
...
int SparseControl::Init() {
  ...
  if (entry_->GetDataSize(kDATA_SPARSE_DATA))
    return net::ERR_CACHE_OPERATION_NOT_SUPPORTED;

  // Now see if there is something where we store our data.
  int rv = net::OK;
  int64_t data_len = entry_->GetDataSize(kDATA_SPARSE_INDEX);
  if (!data_len) {
...
bool SparseControl::CouldBeSparse() const {
  ...
  return (entry_->GetDataSize(kDATA_SPARSE_INDEX) != 0);
}
...
int SparseControl::StartIO(SparseOperation op,
  ...
  operation_ = op;
  offset_ = offset;
  user_buf_ = buf ? new net::DrainableIOBuffer(buf, static_cast<size_t>(buf_len)) : NULL;
  buf_len_ = buf_len;
...
int SparseControl::GetAvailableRange(int64_t offset, int64_t len, int64_t* start) {
  DCHECK(init_);
  // We don't support simultaneous IO for sparse data.
  if (operation_ != kNoOperation)
    return net::ERR_CACHE_OPERATION_NOT_SUPPORTED;
...
// Static
void SparseControl::DeleteChildren(StorageEntry* entry) {
  DCHECK(entry->GetEntryFlags() & PARENT_ENTRY);
  int data_len = entry->GetDataSize(kDATA_SPARSE_INDEX);
  if (data_len < static_cast<int>(sizeof(SparseData)) ||
      entry->GetDataSize(kDATA_SPARSE_DATA))
    return;
  ...
  char* buffer;
  Addr address;
  entry->GetData(kDATA_SPARSE_INDEX, &buffer, &address);
...
int SparseControl::CreateSparseEntry() {
  ...
  int rv = entry_->WriteData(kDATA_SPARSE_INDEX, 0, buf.get(), sizeof(sparse_header_),
...
int SparseControl::OpenSparseEntry(int64_t data_len) {
  if (data_len < static_cast<int64_t>(sizeof(SparseData)))
    return net::ERR_CACHE_OPERATION_NOT_SUPPORTED;
  ...
  int64_t map_len = data_len - sizeof(sparse_header_);
  if (map_len > kMaxMapSize || map_len % 4)
    return net::ERR_CACHE_OPERATION_NOT_SUPPORTED;
  ...
  // Read header.
  int rv = entry_->ReadData(kDATA_SPARSE_INDEX, 0, buf.get(), sizeof(sparse_header_),
                            CompletionCallback());
  ...
  buf = new net::IOBuffer(static_cast<size_t>(map_len));
  rv = entry_->ReadData(kDATA_SPARSE_INDEX, sizeof(sparse_header_), buf.get(),
...
bool SparseControl::OpenChild() {
  ...
  child_ = entry_->backend_->OpenStorageEntry(key);
  if (!child_)
    return ContinueWithoutChild(key);

  if (!(CHILD_ENTRY & child_->GetEntryFlags()) ||
      child_->GetDataSize(kDATA_SPARSE_INDEX) < static_cast<int>(sizeof(child_data_)))
  ...
  // Read signature.
  int rv = child_->ReadData(kDATA_SPARSE_INDEX, 0, buf.get(), sizeof(child_data_),
                            CompletionCallback());
...
void SparseControl::CloseChild() {
  ...
  int rv = child_->WriteData(kDATA_SPARSE_INDEX, 0, buf.get(), sizeof(child_data_),
                             CompletionCallback(), false);
  if (rv != sizeof(child_data_))
...
bool SparseControl::ContinueWithoutChild(const std::string& key) {
  ...
  child_ = entry_->backend_->CreateStorageEntry(key);
  if (!child_) {
    child_ = NULL;
    result_ = net::ERR_CACHE_READ_FAILURE;
    return false;
...
void SparseControl::WriteSparseData() {
  ...
  int64_t len = children_map_.ArraySize() * 4;
  int rv = entry_->WriteData(kDATA_SPARSE_INDEX, sizeof(sparse_header_), buf.get(),
...
bool SparseControl::VerifyRange() {
  ...
  if (child_map_.FindNextBit(&start, last_bit, false)) {
    ...
    int64_t partial_block_len = PartialBlockLength(start);
...
void SparseControl::UpdateRange(int result) {
  ...
  int64_t block_offset = child_offset_ & (kBlockSize - 1);
...
bool SparseControl::DoChildIO() {
  ...
  switch (operation_) {
    case kReadOperation:
      ...
      rv = child_->ReadDataImpl(kDATA_SPARSE_DATA, child_offset_, user_buf_.get(),
                                child_len_, callback);
      break;
    case kWriteOperation:
      ...
      rv = child_->WriteDataImpl(kDATA_SPARSE_DATA, child_offset_, user_buf_.get(),
...
void SparseControl::OnChildIOCompleted(int64_t result) {
