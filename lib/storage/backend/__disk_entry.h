...
class StorageEntry
 ...
 private:
  ...
  int InternalReadData(int index, int64_t offset, IOBuffer* buf,
                       int64_t buf_len, const CompletionCallback& callback);
  int InternalWriteData(int index, int64_t offset, IOBuffer* buf, int64_t buf_len,
                        const CompletionCallback& callback, bool truncate);
...
#endif  // STORAGE_STORAGE_BACKEND_STORAGE_ENTRY_H_
