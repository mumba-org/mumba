...
#ifndef STORAGE_STORAGE_BACKEND_BLOCKFILE_BLOCK_FILES_H_
#define STORAGE_STORAGE_BACKEND_BLOCKFILE_BLOCK_FILES_H_
...
class STORAGE_EXPORT_PRIVATE BlockHeader {
 public:
  BlockHeader();
  ...
  // apropriate counters.
  bool CreateMapBlock(int64_t size, int* index);

  // Deletes the block pointed by |index|.
  void DeleteMapBlock(int index, int64_t block_size);

  // Returns true if the specified block is used.
  bool UsedMapBlock(int index, int64_t size);
...
class STORAGE_EXPORT_PRIVATE BlockFiles {
 public:
 ...
 private:
  ...
  FRIEND_TEST_ALL_PREFIXES(StorageCacheTest, BlockFiles_ZeroSizeFile);
  FRIEND_TEST_ALL_PREFIXES(StorageCacheTest, BlockFiles_TruncatedFile);
  FRIEND_TEST_ALL_PREFIXES(StorageCacheTest, BlockFiles_InvalidFile);
  FRIEND_TEST_ALL_PREFIXES(StorageCacheTest, BlockFiles_Stats);
...
#endif  // STORAGE_STORAGE_BACKEND_BLOCKFILE_BLOCK_FILES_H_
