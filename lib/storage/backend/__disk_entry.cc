...
class StorageEntry::UserBuffer {
 public:
  ...
  void Write(int64_t offset, IOBuffer* buf, int64_t len);
  ...
  int64_t Read(int64_t offset, IOBuffer* buf, int64_t len);

  // Prepare this buffer for reuse.
  void Reset();

  char* Data() { return buffer_.size() ? &buffer_[0] : NULL; }
  int64_t Size() { return static_cast<int64_t>(buffer_.size()); }
  int64_t Start() { return offset_; }
  int64_t End() { return offset_ + Size(); }

 private:
  int64_t capacity() { return static_cast<int64_t>(buffer_.capacity()); }
...
bool StorageEntry::UserBuffer::PreWrite(int64_t offset, int64_t len) {
  ...
  int64_t required = offset - offset_ + len;
...
void StorageEntry::UserBuffer::Write(int64_t offset, IOBuffer* buf, int64_t len) {
  DCHECK_GE(offset, 0);
  DCHECK_GE(len, 0);
  DCHECK_GE(offset + len, 0);
  DCHECK_GE(offset, offset_);
...
int64_t StorageEntry::UserBuffer::Read(int64_t offset, IOBuffer* buf, int64_t len) {
  DCHECK_GE(offset, 0);
  DCHECK_GT(len, 0);
...
int StorageEntry::ReadDataImpl(int index, int offset, IOBuffer* buf, int buf_len,
...
int StorageEntry::WriteDataImpl(int index, int offset, IOBuffer* buf, int buf_len,
                             const CompletionCallback& callback,
                             bool truncate) {
  if (net_log_.IsCapturing()) {
    net_log_.BeginEvent(
...
int StorageEntry::GetAvailableRangeImpl(int64_t offset, int len, int64_t* start) {
  int result = InitSparseData();
  if (net::OK != result)
    return result;
...
void StorageEntry::BeginLogging(net::NetLog* net_log, bool created) {
  ...
  net_log_ = net::NetLogWithSource::Make(
      net_log, net::NetLogSourceType::STORAGE_CACHE_ENTRY);
  net_log_.BeginEvent(net::NetLogEventType::STORAGE_CACHE_ENTRY_IMPL,
...
int32_t StorageEntry::GetDataSize(int index) const {
...
int StorageEntry::ReadData(int index, int offset, IOBuffer* buf, int buf_len,
                        const CompletionCallback& callback) {
  if (callback.is_null())
...
int StorageEntry::WriteData(int index, int offset, IOBuffer* buf, int buf_len,
...
StorageEntry::~StorageEntry() {
  ...
  net_log_.EndEvent(net::NetLogEventType::STORAGE_CACHE_ENTRY_IMPL);
...
bool StorageEntry::CreateDataBlock(int index, int size) {
...
bool StorageEntry::CreateBlock(int size, Addr* address) {
  DCHECK(!address->is_initialized());
  if (!backend_.get())
    return false;
...
// that case.
bool StorageEntry::PrepareTarget(int index, int offset, int buf_len,
                              bool truncate) {
  if (truncate)
...
bool StorageEntry::HandleTruncation(int index, int offset, int buf_len) {
  Addr address(entry_.Data()->data_addr[index]);
...
bool StorageEntry::ImportSeparateFile(int index, int new_size) {
...
bool StorageEntry::PrepareBuffer(int index, int offset, int buf_len) {
  DCHECK(user_buffers_[index].get());
...
void StorageEntry::UpdateSize(int index, int old_size, int new_size) {
  if (entry_.Data()->data_size[index] == new_size)
    return;
