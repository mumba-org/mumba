...
class BackendIO : public BackgroundIO {
 public:
  ...
  void SyncStorageEntry(StorageEntry* entry);
 ...
 private:
  ...
  base::OnceCallback<void(int64_t)> callback_;
...
class InFlightBackendIO : public InFlightIO {
 public:
  ...
  // Proxied operations.
  void Init(base::OnceCallback<void(int64_t)> callback);
  void OpenEntry(const std::string& key, StorageEntry** entry,
  ...
  void EndEnumeration(std::unique_ptr<Rankings::Iterator> iterator);
  void OnExternalCacheHit(const std::string& key);
  void CloseStorageEntry(StorageEntry* entry, CompletionCallback callback);
  void SyncStorageEntry(StorageEntry* entry, const CompletionCallback& callback);
  void DoomStorageEntry(StorageEntry* entry);
