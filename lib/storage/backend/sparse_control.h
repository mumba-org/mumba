...
#ifndef STORAGE_STORAGE_BACKEND_BLOCKFILE_SPARSE_CONTROL_H_
#define STORAGE_STORAGE_BACKEND_BLOCKFILE_SPARSE_CONTROL_H_
...
// Basically, sparse IO is directed from StorageEntry to this class, and we split
...
class SparseControl {
 public:
  typedef CompletionCallback CompletionCallback;
  ...
  explicit SparseControl(StorageEntry* entry);
  ~SparseControl();
  ...
  int GetAvailableRange(int64_t offset, int64_t len, int64_t* start);
  ...
  static void DeleteChildren(StorageEntry* entry);
 ...
 private:
  ...
  int OpenSparseEntry(int64_t data_len);

  // Opens and closes a child entry. A child entry is a regular StorageEntry object
  ...
  void OnChildIOCompleted(int64_t result);

  // Reports to the user that we are done.
  void DoUserCallback();
  void DoAbortCallbacks();

  StorageEntry* entry_;  // The sparse entry.
  scoped_refptr<StorageEntry> child_;  // The current child entry.
  ...
  int64_t buf_len_;  // Bytes to read or write.
  int child_offset_;  // Offset to use for the current child.
  int64_t child_len_;  // Bytes to read or write for this child.
  int result_;
...
#endif  // STORAGE_STORAGE_BACKEND_BLOCKFILE_SPARSE_CONTROL_H_
