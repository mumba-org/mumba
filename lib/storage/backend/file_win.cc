...
struct MyOverlapped {
  MyOverlapped(storage::File* file, size_t offset,
               storage::FileIOCallback* callback);
  ~MyOverlapped() {}
  ...
  scoped_refptr<storage::File> file_;
  scoped_refptr<CompletionHandler> completion_handler_;
  storage::FileIOCallback* callback_;
};
...
MyOverlapped::MyOverlapped(storage::File* file, size_t offset,
                           storage::FileIOCallback* callback) {
