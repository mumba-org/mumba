...
#ifndef STORAGE_STORAGE_BACKEND_BLOCKFILE_ADDR_H_
#define STORAGE_STORAGE_BACKEND_BLOCKFILE_ADDR_H_
...
const int64_t kMaxNumBlocks = 4;
const int64_t kMaxBlockSize = 4096 * kMaxNumBlocks;
...
// the general case.
class STORAGE_EXPORT_PRIVATE Addr {
 public:
  Addr() : value_(0) {}
  explicit Addr(StorageAddr address) : value_(address) {}
  ...
  StorageAddr value() const { return value_; }
  void set_value(StorageAddr address) {
    value_ = address;
  }
...
#endif  // STORAGE_STORAGE_BACKEND_BLOCKFILE_ADDR_H_
