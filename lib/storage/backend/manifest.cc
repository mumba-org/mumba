...
bool Manifest::Init(InitParams params, void* data, int num_bytes, Addr address) {
  ...
  if (!num_bytes) {
    ...
    //D//LOG(INFO) << "loading manifest..";
    manifest = reinterpret_cast<OnStorageManifest*>(data);
    if (!VerifyManifest(manifest)) {
      //D//LOG(INFO) << "manifest verification failed.";
  ...
  //D//LOG(INFO) << "manifest initialization ok.";
  return true;
}
