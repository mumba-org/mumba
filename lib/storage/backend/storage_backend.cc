...
#include "storage/backend/errors.h"
#include "storage/backend/experiments.h"
...
// for most users.
const int64_t k200kEntriesStore = 1000 * 1000 * 1000;
const int64_t kBaseTableLen = 256 * 1024;

// Avoid trimming the cache for the first 5 minutes (10 timer ticks).
const int kTrimDelay = 10;

int64_t DesiredIndexTableLen(int64_t storage_size) {
  if (storage_size <= k200kEntriesStore)
    return kBaseTableLen;
  if (storage_size <= k200kEntriesStore * 2)
    return kBaseTableLen * 2;
  if (storage_size <= k200kEntriesStore * 4)
    return kBaseTableLen * 4;
  if (storage_size <= k200kEntriesStore * 8)
...
int64_t MaxStorageSizeForTable(int64_t table_len) {
  return table_len * (k200kEntriesStore / kBaseTableLen);
}

size_t GetIndexSize(int64_t table_len) {
  size_t table_size = sizeof(storage::StorageAddr) * table_len;
  return sizeof(storage::IndexHeader) + table_size;
}
...
// discarded.
bool InitExperiment(storage::IndexHeader* header, bool cache_created) {
  if (header->experiment == storage::EXPERIMENT_OLD_FILE1 ||
      header->experiment == storage::EXPERIMENT_OLD_FILE2) {
  ...
  if (base::FieldTrialList::FindFullName("SimpleCacheTrial") ==
          "ExperimentControl") {
    if (cache_created) {
      header->experiment = storage::EXPERIMENT_SIMPLE_CONTROL;
      return true;
    }
    return header->experiment == storage::EXPERIMENT_SIMPLE_CONTROL;
  }

  header->experiment = storage::NO_EXPERIMENT;
  return true;
}

// A callback to perform final cleanup on the background thread.
void FinalCleanupCallback(storage::StorageBackend* backend) {
...
int StorageBackend::Init(Manifest::InitParams params, base::OnceCallback<void(int64_t)> callback) {
  init_params_ = std::move(params);
  background_queue_.Init(std::move(callback));
  return net::ERR_IO_PENDING;
}

int StorageBackend::SyncInit() {
  ...
  if (!disabled_ && should_create_timer) {
    ...
    timer_->Start(FROM_HERE, TimeDelta::FromMilliseconds(timer_delay), this,
                  &StorageBackend::OnStatsTimer);
  }

  return disabled_ ? net::ERR_FAILED : net::OK;
}

void StorageBackend::CleanupCache() {
  DCHECK(background_queue_.BackgroundIsCurrentSequence());
...
int StorageBackend::SyncOpenEntry(const std::string& key,
                               scoped_refptr<StorageEntry>* entry) {
  DCHECK(entry);
  *entry = OpenStorageEntry(key);
  return (*entry) ? net::OK : net::ERR_FAILED;
}

int StorageBackend::SyncCreateEntry(const std::string& key,
                                 scoped_refptr<StorageEntry>* entry) {
  DCHECK(entry);
  *entry = CreateStorageEntry(key);
  return (*entry) ? net::OK : net::ERR_FAILED;
}

int StorageBackend::SyncDoomEntry(const std::string& key) {
  if (disabled_)
    return net::ERR_FAILED;

  scoped_refptr<StorageEntry> entry = OpenStorageEntry(key);
  if (!entry)
    return net::ERR_FAILED;
...
int StorageBackend::SyncDoomAllEntries() {
  if (disabled_)
    return net::ERR_FAILED;
  ...
  if (!num_refs_) {
    ...
    if (disabled_)
      return net::ERR_FAILED;
    //D//LOG(INFO) << "StorageBackend::SyncDoomAllEntries: CALLING TRIM";
    eviction_.TrimCache(true);
    return net::OK;
  }
}

int StorageBackend::SyncDoomEntriesBetween(const base::Time initial_time,
  ...
  scoped_refptr<StorageEntry> next = OpenNextStorageEntry(iterator.get());
  if (!next)
    return net::OK;

  while (next) {
    node = std::move(next);
    next = OpenNextStorageEntry(iterator.get());
...
int64_t StorageBackend::SyncCalculateSizeOfAllEntries() {
  //DCHECK_NE(net::APP_CACHE, cache_type_);
  if (disabled_)
    return net::ERR_FAILED;
...
// We use OpenNextStorageEntry to retrieve elements from the cache, until we get
// entries that are too old.
int StorageBackend::SyncDoomEntriesSince(const base::Time initial_time) {
  DCHECK_NE(net::APP_CACHE, cache_type_);
  if (disabled_)
    return net::ERR_FAILED;
  ...
  for (;;) {
    std::unique_ptr<Rankings::Iterator> iterator(new Rankings::Iterator());
    scoped_refptr<StorageEntry> entry = OpenNextStorageEntry(iterator.get());
    if (!entry)
      return net::OK;
...
int StorageBackend::SyncOpenNextEntry(Rankings::Iterator* iterator,
                                   scoped_refptr<StorageEntry>* next_entry) {
  *next_entry = OpenNextStorageEntry(iterator);
  return (*next_entry) ? net::OK : net::ERR_FAILED;
}

void StorageBackend::SyncEndEnumeration(
    std::unique_ptr<Rankings::Iterator> iterator) {
  iterator->Reset();
}

void StorageBackend::SyncOnExternalCacheHit(const std::string& key) {
  if (disabled_)
    return;

  uint32_t hash = base::Hash(key);
  bool error;
  scoped_refptr<StorageEntry> cache_entry =
...
scoped_refptr<StorageEntry> StorageBackend::OpenStorageEntry(const std::string& key) {
  if (disabled_)
    return NULL;
...
scoped_refptr<StorageEntry> StorageBackend::CreateStorageEntry(const std::string& key) {
  ...
  scoped_refptr<StorageEntry> parent;
  ...
  if (entry_address.is_initialized()) {
    ...
    // a hash conflict.
    bool error;
    scoped_refptr<StorageEntry> old_entry =
        MatchEntry(key, hash, false, Addr(), &error);
    if (old_entry)
  ...
  int num_blocks = StorageEntry::NumBlocksForEntry(key.size());
  ...
  scoped_refptr<StorageEntry> cache_entry(
      new StorageEntry(this, entry_address, false));
  IncreaseNumRefs();
...
scoped_refptr<StorageEntry> StorageBackend::OpenNextStorageEntry(
  ...
  scoped_refptr<StorageEntry> entries[kListsToSearch];
  if (!iterator->my_rankings) {
    ...
    for (int i = 0; i < kListsToSearch; i++) {
      if (iterator->list == i) {
        bool ret = OpenFollowingEntryFromList(iterator->list, &iterator->nodes[i],
                                   &entries[i]);
  ...
  scoped_refptr<StorageEntry> next_entry = entries[newest];
  iterator->list = static_cast<Rankings::List>(newest);
  return next_entry;
}

bool StorageBackend::SetMaxSize(int64_t max_bytes) {
  ...
  if (max_bytes >= std::numeric_limits<int64_t>::max() -
                       std::numeric_limits<int64_t>::max() / 10) {
    max_bytes = std::numeric_limits<int64_t>::max() -
                std::numeric_limits<int64_t>::max() / 10 - 1;
  }
...
void StorageBackend::SetType(net::CacheType type) {
  DCHECK_NE(net::MEMORY_CACHE, type);
  cache_type_ = type;
}

base::FilePath StorageBackend::GetFileName(Addr address) const {
...
MappedFile* StorageBackend::File(Addr address) {
  if (disabled_)
    return NULL;
  return block_files_.GetFile(address);
}

base::WeakPtr<InFlightBackendIO> StorageBackend::GetBackgroundQueue() {
  return background_queue_.GetWeakPtr();
}

bool StorageBackend::CreateExternalFile(Addr* address) {
...
bool StorageBackend::CreateBlock(FileType block_type, int block_count,
                             Addr* block_address) {
  return block_files_.CreateBlock(block_type, block_count, block_address);
}

void StorageBackend::DeleteBlock(Addr block_address, bool deep) {
  block_files_.DeleteBlock(block_address, deep);
}

LruData* StorageBackend::GetLruData() {
  return &data_->header.lru;
}

void StorageBackend::UpdateRank(StorageEntry* entry, bool modified) {
  if (read_only_ || (!modified))// && cache_type() == net::SHADER_CACHE))
    return;
  eviction_.UpdateRank(entry, modified);
}

void StorageBackend::RecoveredEntry(CacheRankingsBlock* rankings) {
...
void StorageBackend::InternalDoomEntry(StorageEntry* entry) {
  uint32_t hash = entry->GetHash();
  std::string key = entry->GetKey();
  Addr entry_addr = entry->entry()->address();
  bool error;
  scoped_refptr<StorageEntry> parent_entry =
      MatchEntry(key, hash, true, entry_addr, &error);
  StorageAddr child(entry->GetNextAddress());
...
StorageAddr StorageBackend::GetNextAddr(Addr address) {
  EntriesMap::iterator it = open_entries_.find(address.value());
  if (it != open_entries_.end()) {
    StorageEntry* this_entry = it->second;
...
void StorageBackend::NotLinked(StorageEntry* entry) {
  Addr entry_addr = entry->entry()->address();
...
void StorageBackend::RemoveEntry(StorageEntry* entry) {
#if defined(NET_BUILD_STRESS_CACHE)
  NotLinked(entry);
#endif
  if (!new_eviction_)
...
void StorageBackend::OnEntryDestroyBegin(Addr address) {
  EntriesMap::iterator it = open_entries_.find(address.value());
  if (it != open_entries_.end())
    open_entries_.erase(it);
}

void StorageBackend::OnEntryDestroyEnd() {
  DecreaseNumRefs();
  consider_evicting_at_op_end_ = true;
}

void StorageBackend::OnSyncBackendOpComplete() {
  if (consider_evicting_at_op_end_) {
    if (data_->header.num_bytes > max_size_ && !read_only_ &&
        (up_ticks_ > kTrimDelay || user_flags_ & kNoRandom)) {
      //D//LOG(INFO) << "StorageBackend::OnSyncBackendOpComplete: CALLING TRIM";
...
StorageEntry* StorageBackend::GetOpenEntry(CacheRankingsBlock* rankings) const {
...
int32_t StorageBackend::GetCurrentEntryId() const {
  return data_->header.this_id;
}

int64_t StorageBackend::MaxFileSize() const {
  //return cache_type() == net::PNACL_CACHE ? max_size_ : max_size_ / 8;
  return max_size_;
}

void StorageBackend::ModifyStorageSize(int64_t old_size, int64_t new_size) {
...
void StorageBackend::TooMuchStorageRequested(int64_t size) {
  stats_.ModifyStorageStats(0, size);
}

bool StorageBackend::IsAllocAllowed(int64_t current_size, int64_t new_size) {
...
void StorageBackend::BufferDeleted(int64_t size) {
  buffer_bytes_ -= size;
  DCHECK_GE(size, 0);
}

bool StorageBackend::IsLoaded() const {
...
std::string StorageBackend::HistogramName(const char* name, int experiment) const {
  if (!experiment)
    return base::StringPrintf("StorageCache.%d.%s", cache_type_, name);
  return base::StringPrintf("StorageCache.%d.%s_%d", cache_type_,
                            name, experiment);
}

base::WeakPtr<StorageBackend> StorageBackend::GetWeakPtr() {
  return ptr_factory_.GetWeakPtr();
}

// We want to remove biases from some histograms so we only send data once per
// week.
bool StorageBackend::ShouldReportAgain() {
  if (uma_report_)
...
void StorageBackend::FirstEviction() {
...
void StorageBackend::CriticalError(int error) {
  STRESS_NOTREACHED();
  ...
  if (!num_refs_)
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::Bind(&StorageBackend::RestartCache, GetWeakPtr(), true));
}

void StorageBackend::ReportError(int error) {
...
void StorageBackend::OnEvent(Stats::Counters an_event) {
  stats_.OnEvent(an_event);
}

void StorageBackend::OnRead(int64_t bytes) {
  DCHECK_GE(bytes, 0);
  byte_count_ += bytes;
  if (byte_count_ < 0)
...
void StorageBackend::OnWrite(int64_t bytes) {
  // We use the same implementation as OnRead... just log the number of bytes.
  OnRead(bytes);
}

void StorageBackend::OnStatsTimer() {
  if (disabled_)
    return;
...
void StorageBackend::IncrementIoCount() {
  num_pending_io_++;
}

void StorageBackend::DecrementIoCount() {
  num_pending_io_--;
}

void StorageBackend::SetUnitTestMode() {
  user_flags_ |= kUnitTestMode;
  unit_test_ = true;
}

void StorageBackend::SetUpgradeMode() {
  user_flags_ |= kUpgradeMode;
  read_only_ = true;
}

void StorageBackend::SetNewEviction() {
  user_flags_ |= kNewEviction;
  new_eviction_ = true;
}

void StorageBackend::SetFlags(uint32_t flags) {
  user_flags_ |= flags;
}

void StorageBackend::ClearRefCountForTest() {
  num_refs_ = 0;
}

int StorageBackend::FlushQueueForTest(const CompletionCallback& callback) {
  background_queue_.FlushQueue(callback);
  return net::ERR_IO_PENDING;
}

int StorageBackend::RunTaskForTest(const base::Closure& task,
                                const CompletionCallback& callback) {
  background_queue_.RunTask(task, callback);
  return net::ERR_IO_PENDING;
}

void StorageBackend::TrimForTest(bool empty) {
  eviction_.SetTestMode();
  //D//LOG(INFO) << "StorageBackend::TrimForTest: CALLING TRIM";
  eviction_.TrimCache(empty);
}

void StorageBackend::TrimDeletedListForTest(bool empty) {
  eviction_.SetTestMode();
  eviction_.TrimDeletedList(empty);
}

base::RepeatingTimer* StorageBackend::GetTimerForTest() {
  return timer_.get();
}

int StorageBackend::SelfCheck() {
  if (!init_) {
...
void StorageBackend::FlushIndex() {
  if (index_.get() && !disabled_)
    index_->Flush();
}

void StorageBackend::OnEntrySyncBegin(Addr address) {
  //OnEntryDestroyBegin(address);
}

void StorageBackend::OnEntrySyncEnd(Addr address) {
  //OnEntryDestroyEnd();
}

// ------------------------------------------------------------------------

net::CacheType StorageBackend::GetCacheType() const {
  return cache_type_;
}

int64_t StorageBackend::GetEntryCount() const {
  if (!index_.get() || disabled_)
    return 0;
...
int StorageBackend::OpenEntry(const std::string& key, StorageEntry** entry,
                           const CompletionCallback& callback) {
...
int StorageBackend::CreateEntry(const std::string& key, StorageEntry** entry,
                             const CompletionCallback& callback) {
  DCHECK(!callback.is_null());
...
int StorageBackend::DoomEntry(const std::string& key,
                           const CompletionCallback& callback) {
  DCHECK(!callback.is_null());
...
int StorageBackend::OpenEntry(const base::UUID& key,
                           StorageEntry** entry,
                           const CompletionCallback& callback) {
  return OpenEntry(key.to_string(), entry, callback);
}

int StorageBackend::CreateEntry(const base::UUID& key,
                             StorageEntry** entry,
                             const CompletionCallback& callback) {
  return CreateEntry(key.to_string(), entry, callback);
}

int StorageBackend::DoomEntry(const base::UUID& key,
                           const CompletionCallback& callback) {
  return DoomEntry(key.to_string(), callback);
}

int StorageBackend::DoomAllEntries(const CompletionCallback& callback) {
  DCHECK(!callback.is_null());
  background_queue_.DoomAllEntries(callback);
  return net::ERR_IO_PENDING;
}

int StorageBackend::DoomEntriesBetween(const base::Time initial_time,
                                    const base::Time end_time,
                                    const CompletionCallback& callback) {
  DCHECK(!callback.is_null());
...
int StorageBackend::DoomEntriesSince(const base::Time initial_time,
                                  const CompletionCallback& callback) {
  DCHECK(!callback.is_null());
...
int64_t StorageBackend::CalculateSizeOfAllEntries(const CompletionCallback& callback) {
  DCHECK(!callback.is_null());
  background_queue_.CalculateSizeOfAllEntries(callback);
  return net::ERR_IO_PENDING;
}

class StorageBackend::StorageIteratorImpl : public StorageIterator {
 public:
  explicit StorageIteratorImpl(base::WeakPtr<InFlightBackendIO> background_queue)
      : background_queue_(background_queue),
        iterator_(new Rankings::Iterator()) {
  }

  ~StorageIteratorImpl() override {
    if (background_queue_)
      background_queue_->EndEnumeration(std::move(iterator_));
  }

  int OpenNextEntry(StorageEntry** next_entry,
...
std::unique_ptr<StorageIterator> StorageBackend::CreateIterator() {
  return std::unique_ptr<StorageIterator>(
      new StorageIteratorImpl(GetBackgroundQueue()));
}

void StorageBackend::GetStats(StatsItems* stats) {
  if (disabled_)
    return;
...
void StorageBackend::OnExternalCacheHit(const std::string& key) {
  background_queue_.OnExternalCacheHit(key);
}

size_t StorageBackend::DumpMemoryStats(
...
bool StorageBackend::CreateBackingStore(class File* file) {
...
bool StorageBackend::InitBackingStore(bool* file_created) {
  ...
  *file_created = base_file.created();
  ...
  scoped_refptr<storage::File> file(
      new storage::File(std::move(base_file)));
  if (*file_created)
...
void StorageBackend::AdjustMaxCacheSize(int64_t table_len) {
  if (max_size_)
    return;
...
void StorageBackend::StoreStats() {
  int size = stats_.StorageSize();
  std::unique_ptr<char[]> data(new char[size]);
  Addr address;
...
void StorageBackend::RestartCache(bool failure) {
...
void StorageBackend::PrepareForRestart() {
...
int StorageBackend::NewEntry(Addr address, scoped_refptr<StorageEntry>* entry) {
  EntriesMap::iterator it = open_entries_.find(address.value());
  if (it != open_entries_.end()) {
  ...
  scoped_refptr<StorageEntry> cache_entry(
      new StorageEntry(this, address, read_only_));
  IncreaseNumRefs();
  *entry = NULL;
  ...
  if (!cache_entry->DataSanityCheck()) {
    LOG(WARNING) << "\n\nMessed up entry found. key: '" << cache_entry->GetKey() << "' addr: " << address.value() << "\n\n";
    cache_entry->SetDirtyFlag(0);
...
scoped_refptr<StorageEntry> StorageBackend::MatchEntry(const std::string& key,
                                                 uint32_t hash,
                                                 bool find_parent,
                                                 Addr entry_addr,
                                                 bool* match_error) {
  Addr address(data_->table[hash & mask_]);
  scoped_refptr<StorageEntry> cache_entry, parent_entry;
  bool found = false;
  std::set<StorageAddr> visited;
  *match_error = false;
  ...
  for (;;) {
    ...
    if (error) {
      ...
      Trace("NewEntry failed on MatchEntry 0x%x", address.value());
      LOG(ERROR) << "NewEntry failed on MatchEntry: " << address.value() << " err:" << error;
    ...
    if (cache_entry->dirty()) {
      //DLOG(WARNING) << "entry '" << cache_entry->GetKey() << "' was not closed";
      bool check = cache_entry->SanityCheck();
      bool data_check = cache_entry->DataSanityCheck();
      if (check && data_check) {
        cache_entry->dirty_ = false;
        //DLOG(ERROR) << "SanityCheck() and DataSanityCheck() passed. marking as non dirty";
      } else {
        DLOG(ERROR) << "BAD: entry '" << cache_entry->GetKey() << "'(dirty) didnt pass the sanity checks.\n" <<
          "sanity check: " << (check ? "ok" : "failed") << "\n" <<
          "data sanity check: " << (data_check ? "ok" : "failed") << "\n";
      }
...
bool StorageBackend::OpenFollowingEntryFromList(
    Rankings::List list,
...
scoped_refptr<StorageEntry> StorageBackend::GetEnumeratedEntry(
...
scoped_refptr<StorageEntry> StorageBackend::ResurrectEntry(
...
void StorageBackend::DestroyInvalidEntry(StorageEntry* entry) {
...
void StorageBackend::AddStorageSize(int64_t bytes) {
  data_->header.num_bytes += bytes;
  DCHECK_GE(data_->header.num_bytes, 0);
}

void StorageBackend::SubstractStorageSize(int64_t bytes) {
  data_->header.num_bytes -= bytes;
  DCHECK_GE(data_->header.num_bytes, 0);
}

void StorageBackend::IncreaseNumRefs() {
  num_refs_++;
  if (max_refs_ < num_refs_)
    max_refs_ = num_refs_;
}

void StorageBackend::DecreaseNumRefs() {
  DCHECK(num_refs_);
  num_refs_--;
...
void StorageBackend::IncreaseNumEntries() {
  data_->header.num_entries++;
  DCHECK_GT(data_->header.num_entries, 0);
}

void StorageBackend::DecreaseNumEntries() {
  data_->header.num_entries--;
...
void StorageBackend::ReportStats() {
...
void StorageBackend::UpgradeTo2_1() {
...
bool StorageBackend::CheckIndex() {
  DCHECK(data_);
  ...
  if (data_->header.num_bytes < 0 ||
    ...
    LOG(ERROR) << "Invalid cache (current) size. data_->header.num_bytes = " << data_->header.num_bytes;
    return false;
  }
#endif
...
int StorageBackend::CheckAllEntries() {
  int num_dirty = 0;
  int num_entries = 0;
...
bool StorageBackend::CheckEntry(StorageEntry* cache_entry) {
...
bool StorageBackend::InitManifest() {
  //D//LOG(INFO) << "StorageBackend::InitManifest";
  Addr address(data_->header.manifest);

  int size = manifest_.StorageSize();

  if (!address.is_initialized()) {
    //D//LOG(INFO) << "StorageBackend::InitManifest: address.is_initialized() = false. creating new manifest and persisting";
    FileType file_type = Addr::RequiredFileType(size);
    DCHECK_NE(file_type, EXTERNAL);
    int num_blocks = Addr::RequiredBlocks(size, file_type);
    ...
    data_->header.manifest = address.value();

    bool created = manifest_.Init(std::move(init_params_), NULL, 0, address);
    if (!created) {
      return false;
    }
    manifest_.set_dirty(true);
    // load as a file than flush it to the disk now to avoid 
    // the lost of information if there is a unclean shutdown
    StoreManifest();

    return created;
  }

  //D//LOG(INFO) << "StorageBackend::InitManifest: address.is_initialized() = true. loading Manifest from disk";
  ...
  if (!manifest_.Init(std::move(init_params_), data.get(), size, address))
    return false;
   
  return true;
}

void StorageBackend::StoreManifest() {
  int size = manifest_.StorageSize();
  std::unique_ptr<char[]> data(new char[size]);
  Addr address;
  size = manifest_.Serialize(data.get(), size, &address);
  DCHECK(size);
  if (!address.is_initialized())
    return;
  ...
  manifest_.set_dirty(false);
}

int64_t StorageBackend::MaxBuffersSize() {
  static int64_t total_memory = base::SysInfo::AmountOfPhysicalMemory();
  static bool done = false;

  if (!done) {
    const int64_t kMaxBuffersSize = 30 * 1024 * 1024;
  ...
  return static_cast<int64_t>(total_memory);
}

void StorageBackend::FlushForTesting() {
