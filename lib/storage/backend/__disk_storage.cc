...
int StorageBackend::SyncInit() {
  ...
  if (!(user_flags_ & kNoRandom)) {
    ...
    new_eviction_ = (cache_type_ == net::STORAGE_CACHE);
  }
  ...
  if (!(user_flags_ & kNoRandom) && cache_type_ == net::STORAGE_CACHE &&
...
int StorageBackend::SyncCalculateSizeOfAllEntries() {
  DCHECK_NE(net::APP_CACHE, cache_type_);
  if (disabled_)
    return net::ERR_FAILED;
...
bool StorageBackend::IsAllocAllowed(int64_t current_size, int64_t new_size) {
  ...
  int64_t to_add = new_size - current_size;
...
void StorageBackend::OnRead(int32_t bytes) {
  DCHECK_GE(bytes, 0);
  byte_count_ += bytes;
  if (byte_count_ < 0)
...
void StorageBackend::OnWrite(int32_t bytes) {
...
int32_t StorageBackend::GetEntryCount() const {
  if (!index_.get() || disabled_)
    return 0;
...
int StorageBackend::CalculateSizeOfAllEntries(const CompletionCallback& callback) {
  DCHECK(!callback.is_null());
...
bool StorageBackend::CreateBackingStore(storage::File* file) {
...
void StorageBackend::AdjustMaxCacheSize(int table_len) {
  if (max_size_)
    return;
  ...
  int64_t available = base::SysInfo::AmountOfFreeStorageSpace(path_);
  if (available < 0) {
...
bool StorageBackend::InitStats() {
  ...
  if (!stats_.Init(data.get(), size, address))
    return false;
  if (cache_type_ == net::STORAGE_CACHE && ShouldReportAgain())
...
void StorageBackend::AddStorageSize(int32_t bytes) {
  data_->header.num_bytes += bytes;
  DCHECK_GE(data_->header.num_bytes, 0);
}

void StorageBackend::SubstractStorageSize(int32_t bytes) {
  data_->header.num_bytes -= bytes;
  DCHECK_GE(data_->header.num_bytes, 0);
}
...
void StorageBackend::ReportStats() {
  ...
  if (cache_type_ == net::STORAGE_CACHE)
...
bool StorageBackend::InitManifest() {
  ...
  if (!address.is_initialized()) {
    ...
    return manifest_.Init(init_params_, NULL, 0, address);
  }
  ...
  if (!manifest_.Init(init_params_, data.get(), size, address))
    return false;
