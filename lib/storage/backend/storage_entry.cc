...
class SyncCallback: public storage::FileIOCallback {
 public:
  ...
  SyncCallback(scoped_refptr<storage::StorageEntry> entry,
               net::IOBuffer* buffer,
               const storage::CompletionCallback& callback,
  ...
  void OnFileIOComplete(int64_t bytes_copied) override;
  void Discard();

 private:
  scoped_refptr<storage::StorageEntry> entry_;
  storage::CompletionCallback callback_;
...
void SyncCallback::OnFileIOComplete(int64_t bytes_copied) {
  entry_->DecrementIoCount();
  if (!callback_.is_null()) {
    // if (entry_->net_log().IsCapturing()) {
    //   entry_->net_log().EndEvent(
    //       end_event_type_,
    //       storage::CreateNetLogReadWriteCompleteCallback(bytes_copied));
    // }
    entry_->ReportIOTime(storage::StorageEntry::kAsyncIO, start_);
...
const int64_t kMaxBufferSize = 1024 * 1024;  // 1 MB.
...
class StorageEntry::UserBuffer {
 public:
  explicit UserBuffer(StorageBackend* backend)
  ...
  bool PreRead(int64_t eof, int64_t offset, int64_t* len);

  // Read |len| bytes from |buf| at the given |offset|.
  int Read(int64_t offset, net::IOBuffer* buf, int64_t len);
 ...
 private:
  ...
  bool GrowBuffer(int64_t required, int64_t limit);

  base::WeakPtr<StorageBackend> backend_;
  int64_t offset_;
...
bool StorageEntry::UserBuffer::PreWrite(int64_t offset, int64_t len) {
  DCHECK_GE(offset, 0);
  DCHECK_GE(len, 0);
...
void StorageEntry::UserBuffer::Truncate(int64_t offset) {
  DCHECK_GE(offset, 0);
...
void StorageEntry::UserBuffer::Write(int64_t offset, net::IOBuffer* buf, int64_t len) {
  DCHECK_GE(offset, 0);
  DCHECK_GE(len, 0);
  DCHECK_GE(offset + len, 0);
  DCHECK_GE(offset, offset_);
  ...
  int64_t valid_len = Size() - offset;
  int64_t copy_len = std::min(valid_len, len);
  if (copy_len) {
...
bool StorageEntry::UserBuffer::PreRead(int64_t eof, int64_t offset, int64_t* len) {
  DCHECK_GE(offset, 0);
  DCHECK_GT(*len, 0);
...
int StorageEntry::UserBuffer::Read(int64_t offset, net::IOBuffer* buf, int64_t len) {
  DCHECK_GE(offset, 0);
  DCHECK_GT(len, 0);
  DCHECK(Size() || offset < offset_);

  int64_t clean_bytes = 0;
  if (offset < offset_) {
  ...
  int64_t start = offset - offset_;
  int64_t available = Size() - start;
  DCHECK_GE(start, 0);
...
void StorageEntry::UserBuffer::Reset() {
  if (!grow_allowed_) {
    if (backend_.get())
      backend_->BufferDeleted(capacity() - kMaxBlockSize);
    grow_allowed_ = true;
...
bool StorageEntry::UserBuffer::GrowBuffer(int64_t required, int64_t limit) {
  DCHECK_GE(required, 0);
  int64_t current_size = capacity();
  ...
  int64_t to_add = std::max(required - current_size, kMaxBlockSize * 4);
...
StorageEntry::StorageEntry(StorageBackend* backend, Addr address, bool read_only)
...
int StorageEntry::Sync(const CompletionCallback& callback) {
  if (callback.is_null())
    return SyncImpl(callback);

  if (background_queue_.get())
    background_queue_->SyncStorageEntry(this, callback);
  
  return net::ERR_IO_PENDING;
}

// void StorageEntry::SyncImpl() {
//   uint32_t hash = base::Hash(key_);
//   bool error;
//   scoped_refptr<StorageEntry> cache_entry =
//       backend_->MatchEntry(key_, hash, false, Addr(), &error);
//   if (cache_entry)// && ENTRY_NORMAL == cache_entry->entry()->Data()->state)
//     backend_->UpdateRank(cache_entry.get(), false);
// }

int StorageEntry::SyncImpl(const CompletionCallback& callback) {
  bool ret = true;

  backend_->OnEntrySyncBegin(entry_.address());

  EntryStore* stored = entry_.Data();

  for (int index = 0; index < kNumStreams; index++) {
     size_t data_size = stored->data_size[index]; 
     Addr data_address(stored->data_addr[index]);
     //D//LOG(INFO) << "flushing: stream: " << index << " size: " << data_size << " is_separate_file ? " << data_address.is_separate_file();
     if (user_buffers_[index].get() && data_address.is_separate_file()) {
       ret = Flush(index, 0);
       if (ret) {
 ...
 //   node_.Store();
  ...
  for (int index = 0; index < kNumStreams; index++) {
    Addr data_address(stored->data_addr[index]);
    size_t data_size = stored->data_size[index]; 
    //D//LOG(INFO) << "after store: stream: " << index << " size: " << data_size << " is_separate_file ? " << data_address.is_separate_file(); 
  }

  backend_->OnEntrySyncEnd(entry_.address());
  //if (!callback.is_null()) {
  //  callback.Run(net::OK);
  //}
  return net::OK;
...
int StorageEntry::ReadDataImpl(int index, int64_t offset, net::IOBuffer* buf, int64_t buf_len,
...
int StorageEntry::WriteDataImpl(int index, int64_t offset, net::IOBuffer* buf, int64_t buf_len,
                                const CompletionCallback& callback,
                                bool truncate) {
  //if (net_log_.IsCapturing()) {
  //  net_log_.BeginEvent(
...
int StorageEntry::ReadSparseDataImpl(int64_t offset,
                                     net::IOBuffer* buf,
                                     int64_t buf_len,
                                     const CompletionCallback& callback) {
...
int StorageEntry::WriteSparseDataImpl(int64_t offset,
                                   net::IOBuffer* buf,
                                   int64_t buf_len,
                                   const CompletionCallback& callback) {
  DCHECK(node_.Data()->dirty || read_only_);
...
int StorageEntry::GetAvailableRangeImpl(int64_t offset, int64_t len, int64_t* start) {
  int result = InitSparseData();
  if (net::OK != result)
    return result;
...
void StorageEntry::CancelSparseIOImpl() {
  if (!sparse_.get())
    return;
...
int StorageEntry::ReadyForSparseIOImpl(const CompletionCallback& callback) {
  DCHECK(sparse_.get());
  return sparse_->ReadyToUse(callback);
}

uint32_t StorageEntry::GetHash() {
  return entry_.Data()->hash;
}

bool StorageEntry::CreateEntry(Addr node_address,
  ...
  ////LOG(INFO) << "criando '" << key << "' creation_time: " << entry_store->creation_time;

  if (entry_store->key_len > kMaxInternalKeyLength) {
    ////D//LOG(INFO) << "key len (" << entry_store->key_len << ") > (" << kMaxInternalKeyLength << "): alocando em um bloco. (não é bom)";
    Addr address(0);
...
bool StorageEntry::IsSameEntry(const std::string& key, uint32_t hash) {
...
void StorageEntry::InternalDoom() {
...
void StorageEntry::DeleteEntryData(bool everything) {
...
StorageAddr StorageEntry::GetNextAddress() {
  return entry_.Data()->next;
}

void StorageEntry::SetNextAddress(Addr address) {
...
bool StorageEntry::LoadNodeAddress() {
...
void StorageEntry::SetDirtyFlag(int32_t current_id) {
  DCHECK(node_.HasData());
...
void StorageEntry::SetPointerForInvalidEntry(int32_t new_id) {
  node_.Data()->dirty = new_id;
  node_.Store();
}

bool StorageEntry::LeaveRankingsBehind() {
...
bool StorageEntry::DataSanityCheck() {
  EntryStore* stored = entry_.Data();
  Addr key_addr(stored->long_key);

  // The key must be NULL terminated.
  if (!key_addr.is_initialized() && stored->key[stored->key_len]) {
    DLOG(ERROR) << "The key must be NULL terminated";
    return false;
  }

  if (stored->hash != base::Hash(GetKey())) {
    DLOG(ERROR) << "hash of the key doesnt match with the stored hash";
    return false;
  }

  for (int i = 0; i < kNumStreams; i++) {
    Addr data_addr(stored->data_addr[i]);
    int64_t data_size = stored->data_size[i];
    
    if (data_size < 0) {
      DLOG(ERROR) << "error stream " << i << " data size < 0 = " << data_size;
      return false;
    }
    if (!data_size && data_addr.is_initialized()) {
      DLOG(ERROR) << "error stream " << i << " data size == " << data_size << " and is initialized";
      return false;
    }
    if (!data_addr.SanityCheck()) {
      DLOG(ERROR) << "error stream " << i << " failed on sanity check";
      return false;
    }
    if (!data_size)
      continue;
    if (data_size <= kMaxBlockSize && data_addr.is_separate_file()) {
      DLOG(ERROR) << "error stream " << i << " data size (" << data_size << ") < " << kMaxBlockSize << " && data_addr.is_separate_file()";
      return false;
    }
    if (data_size > kMaxBlockSize && data_addr.is_block_file()) {
      DLOG(ERROR) << "error stream " << i << " data size (" << data_size << ") > " << kMaxBlockSize << " && data_addr.is_block_file()";
      return false;
    }
  }
  return true;
...
void StorageEntry::FixForDelete() {
  EntryStore* stored = entry_.Data();
  Addr key_addr(stored->long_key);
...
void StorageEntry::IncrementIoCount() {
  backend_->IncrementIoCount();
}

void StorageEntry::DecrementIoCount() {
  if (backend_.get())
    backend_->DecrementIoCount();
}

void StorageEntry::OnEntryCreated(StorageBackend* backend) {
  // Just grab a reference to the backround queue.
  background_queue_ = backend->GetBackgroundQueue();
}

void StorageEntry::SetTimes(base::Time last_used, base::Time last_modified) {
  node_.Data()->last_used = last_used.ToInternalValue();
  node_.Data()->last_modified = last_modified.ToInternalValue();
  node_.set_modified();
}

void StorageEntry::ReportIOTime(Operation op, const base::TimeTicks& start) {
  if (!backend_.get())
    return;
...
void StorageEntry::BeginLogging(net::NetLog* net_log, bool created) {
...
const net::NetLogWithSource& StorageEntry::net_log() const {
  return net_log_;
}

// static
int StorageEntry::NumBlocksForEntry(int key_size) {
...
void StorageEntry::Doom() {
  if (background_queue_.get())
    background_queue_->DoomStorageEntry(this);
}

void StorageEntry::Close(CompletionCallback callback) {
  if (background_queue_.get())
    background_queue_->CloseStorageEntry(this, std::move(callback));
}

std::string StorageEntry::GetKey() const {
  ...
  File* key_file = const_cast<StorageEntry*>(this)->GetBackingFile(address,
                                                                kKeyFileIndex);
  if (!key_file)
    return std::string();
...
int StorageEntry::ReadData(int index, int64_t offset, net::IOBuffer* buf, int64_t buf_len,
                           const CompletionCallback& callback) {
  if (callback.is_null()) {
...
int StorageEntry::WriteData(int index, int64_t offset, net::IOBuffer* buf, int64_t buf_len,
...
int StorageEntry::ReadSparseData(int64_t offset,
                              net::IOBuffer* buf,
                              int64_t buf_len,
                              const CompletionCallback& callback) {
  if (callback.is_null())
...
int StorageEntry::WriteSparseData(int64_t offset,
                               net::IOBuffer* buf,
                               int64_t buf_len,
                               const CompletionCallback& callback) {
  if (callback.is_null())
...
int StorageEntry::GetAvailableRange(int64_t offset,
                                 int64_t len,
                                 int64_t* start,
                                 const CompletionCallback& callback) {
  if (!background_queue_.get())
...
bool StorageEntry::CouldBeSparse() const {
  if (sparse_.get())
    return true;

  std::unique_ptr<SparseControl> sparse;
  sparse.reset(new SparseControl(const_cast<StorageEntry*>(this)));
  return sparse->CouldBeSparse();
}

void StorageEntry::CancelSparseIO() {
  if (background_queue_.get())
    background_queue_->CancelSparseIO(this);
}

int StorageEntry::ReadyForSparseIO(const CompletionCallback& callback) {
  if (!sparse_.get())
    return net::OK;
...
void StorageEntry::SetLastUsedTimeForTest(base::Time time) {
  SetTimes(time, time);
}
...
StorageEntry::~StorageEntry() {
  ...
  Trace("~StorageEntry out 0x%p", reinterpret_cast<void*>(this));
  net_log_.EndEvent(net::NetLogEventType::DISK_CACHE_ENTRY_IMPL);
  backend_->OnEntryDestroyEnd();
  //D//LOG(INFO) << "~StorageEntry end";
}

// ------------------------------------------------------------------------

int StorageEntry::InternalReadData(int index, int64_t offset,
                                net::IOBuffer* buf, int64_t buf_len,
                                const CompletionCallback& callback) {
  //DCHECK(node_.Data()->dirty || read_only_);
  ...
  int64_t entry_size = entry_.Data()->data_size[index];
  if (offset >= entry_size || offset < 0 || !buf_len)
    return 0;
...
int StorageEntry::InternalWriteData(int index, int64_t offset,
                                    net::IOBuffer* buf, int64_t buf_len,
                                    const CompletionCallback& callback,
                                    bool truncate) {
  //DCHECK(node_.Data()->dirty || read_only_);
  ...
  int64_t max_file_size = backend_->MaxFileSize();

  // offset or buf_len could be negative numbers.
  if (offset > max_file_size || buf_len > max_file_size ||
      offset + buf_len > max_file_size) {
    int64_t size = offset + buf_len;
...
bool StorageEntry::CreateDataBlock(int index, int64_t size) {
...
bool StorageEntry::CreateBlock(int64_t size, Addr* address) {
  DCHECK(!address->is_initialized());
  if (!backend_.get())
    return false;
  ...
  if (EXTERNAL == file_type) {
    ...
    int64_t num_blocks = Addr::RequiredBlocks(size, file_type);
...
void StorageEntry::DeleteData(Addr address, int index) {
  DCHECK(backend_.get());
  if (!address.is_initialized())
    return;
  if (address.is_separate_file()) {
    int failure = !DeleteStorageFile(backend_->GetFileName(address));
...
void StorageEntry::UpdateRank(bool modified) {
  if (!backend_.get())
    return;
...
File* StorageEntry::GetBackingFile(Addr address, int index) {
  if (!backend_.get())
    return NULL;
...
File* StorageEntry::GetExternalFile(Addr address, int index) {
...
// that case.
bool StorageEntry::PrepareTarget(int index, int64_t offset, int64_t buf_len,
                                 bool truncate) {
  if (truncate)
...
bool StorageEntry::HandleTruncation(int index, int64_t offset, int64_t buf_len) {
  Addr address(entry_.Data()->data_addr[index]);

  int64_t current_size = entry_.Data()->data_size[index];
  int64_t new_size = offset + buf_len;
...
bool StorageEntry::CopyToLocalBuffer(int index) {
  Addr address(entry_.Data()->data_addr[index]);
  DCHECK(!user_buffers_[index].get());
  DCHECK(address.is_initialized());

  int64_t len = std::min(entry_.Data()->data_size[index], kMaxBlockSize);
  user_buffers_[index].reset(new UserBuffer(backend_.get()));
...
bool StorageEntry::MoveToLocalBuffer(int index) {
  if (!CopyToLocalBuffer(index))
    return false;
...
bool StorageEntry::ImportSeparateFile(int index, int64_t new_size) {
...
bool StorageEntry::PrepareBuffer(int index, int64_t offset, int64_t buf_len) {
  DCHECK(user_buffers_[index].get());
...
bool StorageEntry::Flush(int index, int64_t min_len) {
  Addr address(entry_.Data()->data_addr[index]);
  DCHECK(user_buffers_[index].get());
  DCHECK(!address.is_initialized() || address.is_separate_file());
  DVLOG(3) << "Flush";

  int64_t size = std::max(entry_.Data()->data_size[index], min_len);
  ...
  int64_t len = user_buffers_[index]->Size();
  int64_t offset = user_buffers_[index]->Start();
  if (!len && !offset)
    return true;
...
void StorageEntry::UpdateSize(int index, int64_t old_size, int64_t new_size) {
  if (entry_.Data()->data_size[index] == new_size)
    return;
...
int StorageEntry::InitSparseData() {
  if (sparse_.get())
    return net::OK;
...
void StorageEntry::SetEntryFlags(uint32_t flags) {
  entry_.Data()->flags |= flags;
  entry_.set_modified();
}

uint32_t StorageEntry::GetEntryFlags() {
  return entry_.Data()->flags;
}

void StorageEntry::GetData(int index, char** buffer, Addr* address) {
  DCHECK(backend_.get());
...
void StorageEntry::Log(const char* msg) {
  int dirty = 0;
  if (node_.HasData()) {
  ...
  Trace("  data: 0x%x 0x%x 0x%x", entry_.Data()->data_addr[0],
        //entry_.Data()->data_addr[1], entry_.Data()->data_addr[2]);
