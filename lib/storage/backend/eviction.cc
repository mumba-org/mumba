...
void Eviction::Init(StorageBackend* backend) {
...
void Eviction::UpdateRank(StorageEntry* entry, bool modified) {
  if (new_eviction_)
...
void Eviction::OnOpenEntry(StorageEntry* entry) {
  if (new_eviction_)
    return OnOpenEntryV2(entry);
}

void Eviction::OnCreateEntry(StorageEntry* entry) {
  if (new_eviction_)
...
void Eviction::OnDoomEntry(StorageEntry* entry) {
  if (new_eviction_)
...
void Eviction::OnDestroyEntry(StorageEntry* entry) {
  if (new_eviction_)
...
bool Eviction::ShouldTrim() {
  ...
  UMA_HISTOGRAM_COUNTS_1M("StorageCache.TrimDelays", trim_delays_);
  trim_delays_ = 0;
  return true;
}
...
void Eviction::ReportTrimTimes(StorageEntry* entry) {
  if (first_trim_) {
    first_trim_ = false;
...
Rankings::List Eviction::GetListForEntry(StorageEntry* entry) {
  return Rankings::NO_USE;
}

bool Eviction::EvictEntry(CacheRankingsBlock* node, bool empty,
                          Rankings::List list) {
  scoped_refptr<StorageEntry> entry = backend_->GetEnumeratedEntry(node, list);
  if (!entry) {
...
void Eviction::UpdateRankV2(StorageEntry* entry, bool modified) {
  rankings_->UpdateRank(entry->rankings(), modified, GetListForEntryV2(entry));
}

void Eviction::OnOpenEntryV2(StorageEntry* entry) {
  EntryStore* info = entry->entry()->Data();
  DCHECK_EQ(ENTRY_NORMAL, info->state);
...
void Eviction::OnCreateEntryV2(StorageEntry* entry) {
  EntryStore* info = entry->entry()->Data();
  switch (info->state) {
    case ENTRY_NORMAL: {
...
void Eviction::OnDoomEntryV2(StorageEntry* entry) {
  EntryStore* info = entry->entry()->Data();
...
void Eviction::OnDestroyEntryV2(StorageEntry* entry) {
  if (entry->LeaveRankingsBehind())
    return;
...
Rankings::List Eviction::GetListForEntryV2(StorageEntry* entry) {
  EntryStore* info = entry->entry()->Data();
  DCHECK_EQ(ENTRY_NORMAL, info->state);
