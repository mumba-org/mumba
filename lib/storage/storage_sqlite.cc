...
int StorageSyncBlob(csqlite_file* ref, int flags) {
  //D//LOG(INFO) << "StorageSyncBlob";
  disk_file_t* file = reinterpret_cast<disk_file_t*>(ref);
  scoped_refptr<storage::Torrent> torrent(reinterpret_cast<storage::Torrent*>(file->torrent));
  int r = torrent->Sync();
  //D//LOG(INFO) << "StorageSyncBlob: done";
  return r;
}
...
int StorageLockBlob(csqlite_file* ref, int) {
  disk_file_t* file = reinterpret_cast<disk_file_t*>(ref);
  scoped_refptr<storage::Torrent> torrent(reinterpret_cast<storage::Torrent*>(file->torrent));
  //D//LOG(INFO) << "StorageLockBlob " << torrent->id().to_string();
  torrent->Lock();
  return SQLITE_OK;
}

int StorageUnlockBlob(csqlite_file* ref, int) {
  disk_file_t* file = reinterpret_cast<disk_file_t*>(ref);
  scoped_refptr<storage::Torrent> torrent(reinterpret_cast<storage::Torrent*>(file->torrent));
  //D//LOG(INFO) << "StorageUnlockBlob " << torrent->id().to_string();
  torrent->Unlock();
  return SQLITE_OK;
}
...
int StorageOpenBlob(csqlite_vfs* vfs, const char *zName, csqlite_file* file, int flags, int *pOutFlags) {
  ...
  if (zName == nullptr) {
    DLOG(ERROR) << "StorageOpenBlob: nullptr passed as name of file to open";
    //PrintStackTrace();
    DCHECK(false);
    return SQLITE_ERROR;
  }
