...
int FileSet::Load(const base::FilePath& file_path, size_t size, bool readonly) {
  ...
  if (!base::PathExists(file_path) && size > 0) {
    ...
    if (!mmaped_file->Initialize(file_path, readonly ? base::MemoryMappedFile::READ_ONLY : base::MemoryMappedFile::READ_WRITE_EXTEND)) {
      // check if is a empty file first
  ...
  //}
  //DLOG(INFO) << "FileSet::Load: added '" << file_path << "' size: " << size;
  return num;
}
...
base::StringPiece FileSet::GetPath(int index) const {
  for (auto it = file_path_map_.begin(); it != file_path_map_.end(); ++it) {
    if (it->second == index) {
#if defined(OS_WIN)      
      return base::StringPiece(base::UTF16ToASCII(it->first));
#else
...
base::StringPiece FileSet::GetFirstBytes(int index) const {
  auto it = files_.find(index);
  if (it == files_.end()) {
    return base::StringPiece();
  }
  return GetFirstBytesInternal(it->second.get()); 
}

size_t FileSet::GetLength(int index) const {
  int64_t file_size = 0;
  base::StringPiece path = GetPath(index);
  if (path.empty()) {
    DLOG(INFO) << "FileSet::GetLength(" << index << "): no path for " << index << ". returning 0";
    return 0;
  }
  if (!base::GetFileSize(base::FilePath(path), &file_size)) {
    DLOG(INFO) << "FileSet::GetLength(" << index << "): GetFileSize for path " << path << " failed. returning 0";
    return 0;
  }
...
size_t FileSet::GetLength(const base::FilePath& file_path) const {
  ...
  if (!base::GetFileSize(file_path, &file_size)) {
    DLOG(INFO) << "FileSet::GetLength(" << file_path << "): GetFileSize for path " << file_path << " failed. returning 0";
    return 0;
  }
  return static_cast<size_t>(file_size);
}
...
base::StringPiece FileSet::GetFirstBytesInternal(base::MemoryMappedFile* file) const {
  size_t len = file->length() > 255 ? 255 : file->length();
  return base::StringPiece(reinterpret_cast<const char *>(file->data()), len);
}
