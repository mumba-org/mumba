...
int kHashSize = SHA_DIGEST_LENGTH;
...
int GetBlockCount(size_t len) {
  return (static_cast<int>(len) + kBlockSize - 1) / kBlockSize;
  //return static_cast<int>(len) / kBlockSize;
}

int GetMerkleParent(int tree_node) {
  // node 0 doesn't have a parent
  DCHECK(tree_node > 0);
  return (tree_node - 1) / 2;
}

int GetMerkleSibling(int tree_node) {
...
int GetMerkleNodeCount(int leafs) {
  DCHECK(leafs > 0);
  return (leafs << 1) - 1;
}

int GetMerkleLeafCount(int pieces) {
  DCHECK(pieces > 0);
...
MerkleTreeNode::MerkleTreeNode(): 
  valid_(false),
  initialized_(false),
  level_(std::numeric_limits<int>::max()),
  hash_(0) {

}

MerkleTreeNode::MerkleTreeNode(int level): 
  valid_(false),
  initialized_(false),
  level_(level),
  hash_(0) {
...
MerkleTreeNode::MerkleTreeNode(int level, void* digest_buffer):
  valid_(true),
  initialized_(true),
  level_(level), 
...
int MerkleTreeNode::sibling_offset() const {
  DCHECK(level_ != 0);
  return GetMerkleSibling(level_);
}

int MerkleTreeNode::parent_offset() const {
  DCHECK(level_ != 0);
...
// static 
int MerkleTree::GetTreeLength(size_t data_size) {
  const int num_blocks = GetBlockCount(data_size);
  const int num_leafs = GetMerkleLeafCount(num_blocks);
	const int node_count = GetMerkleNodeCount(num_leafs);
  DLOG(INFO) << "MerkleTree::GetTreeLength:\n " << 
   "data_size: " << data_size << "\n" <<
...
std::unique_ptr<MerkleTree> MerkleTree::Create(size_t data_len) {
  const int block_count = GetBlockCount(data_len);
  DLOG(INFO) << "block_count = " << block_count << " for size = " << data_len << " aprox =" << (data_len / kBlockSize);
...
std::unique_ptr<MerkleTree> MerkleTree::CreateAndBuild(const void* input_data, size_t input_size) {
  const int block_count = GetBlockCount(input_size);
  std::unique_ptr<MerkleTree> result = std::make_unique<MerkleTree>(block_count);
  result->Init();
...
std::unique_ptr<MerkleTree> MerkleTree::CreateAndBuild(FileSet* fileset) {
  //DLOG(INFO) << "MerkleTree::CreateAndBuild: fileset->GetTotalBlockCount: " << fileset->GetTotalBlockCount();
  std::unique_ptr<MerkleTree> result = std::make_unique<MerkleTree>(fileset->GetTotalBlockCount());
  result->Init();
...
std::unique_ptr<MerkleTree> MerkleTree::Load(const void* input_digests, size_t input_len) {
  const int block_count = GetBlockCount(input_len);
  std::unique_ptr<MerkleTree> result = std::make_unique<MerkleTree>(block_count);
  result->Init();
...
MerkleTree::MerkleTree(int block_count):
...
std::string MerkleTree::root_hash() const {
  ...
  if (root_->valid()) {
    ...
    //D//LOG(INFO) << "MerkleTree::root_hash: warning: root node is not valid";
...
void MerkleTree::Init() {
  // allocate the buf
  //DLOG(INFO) << "MerkleTree::Init: block_count_ = " << block_count_ << " leaf_count_ = " << leaf_count_ << " node_count_ = " << node_count_;
...
bool MerkleTree::AddLeaf(const void* input_data, size_t input_size) {
  int node_offset = first_leaf_ + block_offset_;
  return AddLeaf(node_offset, input_data, input_size);
}

bool MerkleTree::AddLeaf(int node_offset, const void* input_data, size_t input_size) {
  int current_pos = node_offset - first_leaf_;
  ...
  if (node_offset >= nodes_.size()) {
    int diff = node_offset - (nodes_.size() + 1);
    //DLOG(INFO) << "AddLeaf: node_offset >= nodes_.size(). node_offset = " << node_offset << " nodes_.size() = " << nodes_.size() << " diff = " << diff << " block_count_ = " << block_count_;
    for (int i = 0; i < diff; ++i) {
      nodes_.push_back(std::make_unique<MerkleTreeNode>());
      block_count_++;
    }
    //DLOG(INFO) << "AddLeaf: updated block_count_ = " << block_count_;
    leaf_count_ = GetMerkleLeafCount(block_count_);
    node_count_ = GetMerkleNodeCount(leaf_count_);
  }
  ...
  int hash_size = kBlockSize > input_size ? input_size : kBlockSize;
...
bool MerkleTree::UpdateLeaf(int offset, const void* input_data, size_t input_size) {
...
bool MerkleTree::Rebuild() {
  ...
  if (digest_buffer_realloc_) {
    ////D//LOG(INFO) << "oops theres nodes to add so we need to move the leaf_start to a new position";
    int nodes_to_add = node_count_ - nodes_.size();
    old_first_leaf_ = first_leaf_;
    first_leaf_ = node_count_ - leaf_count_;

    std::vector<std::unique_ptr<MerkleTreeNode>> leafs; 
    
    for (int i = old_first_leaf_; i < nodes_.size(); i++) {
      ////D//LOG(INFO) << "adding leaf: " << i << " : " << base::ToLowerASCII(base::HexEncode(nodes_[i]->hash(), kHashSize));
    ...
    int offset = first_leaf_;
    for (int i = 0; i < leafs.size(); i++) {
      ////D//LOG(INFO) << "moving leaf: " << old_first_leaf_ + i << " to " << offset;
      nodes_[offset] = std::move(leafs[i]);
      nodes_[offset]->level_ = offset;
      //D//LOG(INFO) << "[" << offset << "] : " << base::ToLowerASCII(base::HexEncode(nodes_[offset]->hash(), kHashSize));
      offset++;
    }
    
    for (int i = old_first_leaf_; i < first_leaf_; i++) {
      auto node = std::make_unique<MerkleTreeNode>();
    ...
    int rest_offset = first_leaf_ + leafs.size();
    for (int i = rest_offset; i < node_count_; ++i) {
  ...
  ////D//LOG(INFO) << "MerkleTree::Rebuild: level_start(first_leaf_): " << level_start << " level_size(leaf_count_): " << level_size << " node_count: " << node_count_ << " nodes.size: " << nodes_.size();
  while (level_start > 0) {
    int parent = GetMerkleParent(level_start);
    for (int i = level_start; i < level_start + level_size; i += 2, ++parent) {
      ////D//LOG(INFO) << "parent: " << parent << " at " << (intptr_t)nodes_[parent]->hash_ << " left " << i << ":" << (intptr_t)nodes_[i]->hash_ << " init: " << nodes_[i]->initialized_ << " right: " << i + 1 << ":" << (intptr_t)nodes_[i+1]->hash_ << " init: " << nodes_[i+1]->initialized_;
      nodes_[parent]->level_ = parent;
      //nodes_[parent]->Init(ptr);
      ////D//LOG(INFO) << "updating " << parent << " with the hash on " << i << " (left)";
      nodes_[parent]->Update(nodes_[i].get());
      ////D//LOG(INFO) << "updating " << parent << " with the hash on " << i + 1 << " (right)";
      nodes_[parent]->Update(nodes_[i + 1].get());
      ////D//LOG(INFO) << "nodes_[" << parent << "]->Finish()"; 
      nodes_[parent]->Finish();
      ////D//LOG(INFO) << "MerkleTree::Rebuild: parent: " << parent 
...
bool MerkleTree::Build(FileSet* fileset) {
  ...
  for (int i = 0; i < fileset->file_count(); ++i) {
    ...
    char* input_data = 0;
    int blk_cnt_pfile = fileset->GetBlockCount(i);
    int file_size = fileset->GetLength(i);
...
bool MerkleTree::LoadInternal(const void* input_digests, size_t data_len) {
  ...
  for (int i = 0; i < leaf_count_; ++i) {
    ////D//LOG(INFO) << "init " << first_leaf_ + i << " with " << (void*)output_digest;
    nodes_[first_leaf_ + i]->level_ = first_leaf_ + i;
    nodes_[first_leaf_ + i]->Init(output_digest, true);
    output_digest += kHashSize;
    block_offset_++;
  ...
  while (level_start > 0) {
    int parent = GetMerkleParent(level_start);
    for (int i = level_start; i < level_start + level_size; i += 2, ++parent) {
     // //D//LOG(INFO) << "init " << parent << " with " << (void*)output_digest;
      nodes_[parent]->level_ = parent;
      nodes_[parent]->Init(output_digest, true);
      output_digest += kHashSize;
    }
...
void MerkleTree::GrowDigestBuffer(size_t node_realloc_offset) {
 ...
 // //D//LOG(INFO) << "growing digest buffer: old: " << (void *) old_pos  << " new:" << (void *)digest_buffer_;
  DigestBufferChanged();
}
