...
extern int kHashSize;

class MerkleTreeNode {
public:
  MerkleTreeNode();
  MerkleTreeNode(int level);
  ~MerkleTreeNode();
...
private:
  ...
  MerkleTreeNode(int level, void* digest_buffer);
...
class MerkleTree {
public:
  static int GetTreeLength(size_t data_size);
  ...
  const MerkleTreeNode* node(int index) const {
  ...
  bool AddLeaf(int offset, const void* input_data, size_t input_size);
  bool UpdateLeaf(int offset, const void* input_data, size_t input_size);
