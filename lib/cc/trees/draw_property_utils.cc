...
static bool LayerNeedsUpdateInternal(LayerType* layer,
                                     bool layer_is_drawn,
                                     const PropertyTrees* property_trees) {
  //DLOG(INFO) << "LayerNeedsUpdate: processing layer " << layer->id();
  ...
  if (!layer_is_drawn) {
//    DLOG(INFO) << "LayerNeedsUpdate: layer " << layer->id() << ": layer_is_drawn = false. LayerNeedsUpdate -> false";
    return false;
  }

  if (!layer->DrawsContent() || layer->bounds().IsEmpty()) {
  //  DLOG(INFO) << "LayerNeedsUpdate:  " << layer->id() << ": !layer->DrawsContent() = " << layer->DrawsContent() << " || layer->bounds().IsEmpty() = " << layer->bounds().IsEmpty() << ". LayerNeedsUpdate -> false";
    return false;
  }
  ...
  if (layer->should_check_backface_visibility()) {
    //DLOG(INFO) << "LayerNeedsUpdate:  " << layer->id() << ": layer->should_check_backface_visibility()";
    ...
    if (TransformToScreenIsKnown(layer, backface_transform_id, tree) &&
      ...
      //DLOG(INFO) << "LayerNeedsUpdate: " << layer->id() << ": layer->should_check_backface_visibility(). returning false";
      return false;
    }
  }

  //DLOG(INFO) << "LayerNeedsUpdate: " << layer->id() << ": LayerNeedsUpdate -> true";
...
static inline bool LayerShouldBeSkippedInternal(
  ...
  //DLOG(INFO) << "LayerShouldBeSkippedInternal: layer " << layer->id() 
  // << "\neffect_node: " << effect_node << " id: " << effect_node->id
  // << "\neffect_node->has_render_surface: " << effect_node->has_render_surface 
  // << "\neffect_node->subtree_has_copy_request: " << effect_node->subtree_has_copy_request
  // << "\ntransform_node->node_and_ancestors_are_animated_or_invertible: " << transform_node->node_and_ancestors_are_animated_or_invertible
  // << "\neffect_node->hidden_by_backface_visibility: " << effect_node->hidden_by_backface_visibility 
  // << "\neffect_node->is_drawn: " << effect_node->is_drawn
  // << "\neffect_node->subtree_hidden: " << effect_node->subtree_hidden
  // << "\nlayer->effect_tree_index() = " << layer->effect_tree_index();
...
void FindLayersThatNeedUpdates(LayerTreeHost* layer_tree_host,
  ...
  for (auto* layer : *layer_tree_host) {
    //LOG(INFO) << "FindLayersThatNeedUpdates: iterando cc::layer " << layer->id() << " parent: " << ( layer->parent() ? layer->parent()->id() : -1);
    //LOG(INFO) << "FindLayersThatNeedUpdates: !IsRootLayer(layer) = " << IsRootLayer(layer) << " && LayerShouldBeSkippedForDrawPropertiesComputation() = " << LayerShouldBeSkippedForDrawPropertiesComputation(
    //                               layer, transform_tree, effect_tree) << " branch. layer = " << layer->id();
    ...
    if (LayerNeedsUpdate(layer, layer_is_drawn, property_trees)) {
      //LOG(INFO) << "FindLayersThatNeedUpdates: adding cc::layer " << layer->id() << " to update_layer_list";
