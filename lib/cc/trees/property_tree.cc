...
void EffectTree::UpdateIsDrawn(EffectNode* node, EffectNode* parent_node) {
  //DLOG(INFO) << "EffectTree::UpdateIsDrawn: EffectiveOpacity(node) = " << EffectiveOpacity(node) << " node->opacity = " << node->opacity << " node->subtree_hidden = " << node->subtree_hidden;
  ...
  if (node->has_copy_request || node->cache_render_surface) {
    //DLOG(INFO) << "EffectTree::UpdateIsDrawn: if (node->has_copy_request || node->cache_render_surface)\nnode->has_copy_request:" << node->has_copy_request
    //<< "\nnode->cache_render_surface: " << node->cache_render_surface << " -> node->is_drawn = true";
    node->is_drawn = true;
    ...
    //DLOG(INFO) << "EffectTree::UpdateIsDrawn: into \"\nEffectiveOpacity(node) == 0.f (="<<EffectiveOpacity(node)<<")&&"
    //       << "\n(!node->has_potential_opacity_animation (="<<node->has_potential_opacity_animation<<")||"
    //       << "\nproperty_trees()->is_active) (="<<property_trees()->is_active<<") &&"
    //       << "\nnode->background_filters.IsEmpty() (="<<node->background_filters.IsEmpty()<<")\" -> node->is_drawn = false";
    node->is_drawn = false;
  } else if (parent_node) {
    //DLOG(INFO) << "EffectTree::UpdateIsDrawn: else if (parent_node)" << "\nparent_node->is_drawn: " << parent_node->is_drawn;
    node->is_drawn = parent_node->is_drawn;
  } else {
    //DLOG(INFO) << "EffectTree::UpdateIsDrawn: else\nnode->is_drawn = true";
    node->is_drawn = true;
  }
}
