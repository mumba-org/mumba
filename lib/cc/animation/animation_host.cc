...
void AnimationHost::RegisterKeyframeEffectForElement(
  ...
  //DLOG(INFO) << "AnimationHost::RegisterKeyframeEffectForElement";
  DCHECK(element_id);
  ...
  scoped_refptr<ElementAnimations> element_animations =
      GetElementAnimationsForElementId(element_id);
  if (!element_animations) {
    //DLOG(INFO) << "AnimationHost::RegisterKeyframeEffectForElement: !element_animations.. creating new ones";
  ...
  if (element_animations->animation_host() != this) {
    //DLOG(INFO) << "AnimationHost::RegisterKeyframeEffectForElement: element_animations->animation_host() != this";
...
bool AnimationHost::UpdateAnimationState(bool start_ready_animations,
  ...
  //DLOG(INFO) << "AnimationHost::UpdateAnimationState"; 
  if (!NeedsTickAnimations()) {
    //DLOG(INFO) << "AnimationHost::UpdateAnimationState: !NeedsTickAnimations(). RETURNING FALSE"; 
    return false;
  }
  ...
  AnimationsList ticking_animations_copy = ticking_animations_;
  for (auto& it : ticking_animations_copy) {
    //DLOG(INFO) << "AnimationHost::UpdateAnimationState: for (...) anim->UpdateState()"; 
...
void AnimationHost::SetAnimationEvents(
  ...
  //DLOG(INFO) << "AnimationHost::SetAnimationEvents";
  auto events =
...
void AnimationHost::AddToTicking(scoped_refptr<Animation> animation) {
  //DLOG(INFO) << "AnimationHost::AddToTicking";
