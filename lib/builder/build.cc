  ...
  *end_time = (int)(now - start_time_millis_);
  ...
  if (!output.empty()) {
    ...
    if (!printer_.is_smart_terminal())
      final_output = util::StripAnsiEscapeCodes(output);
    else
      final_output = output;
...
string BuildStatus::FormatProgressStatus(
  ...
  for (const char* s = progress_status_format; *s != '\0'; ++s) {
    if (*s == '%') {
      ...
      default:
        util::Fatal("unknown placeholder '%%%c' in $NINJA_STATUS", *s);
        return "";
      }
    } else {
      out.push_back(*s);
...
bool RealCommandRunner::CanRunMore() {
  ...
  return (int)subproc_number < config_.parallelism
    && ((subprocs_.running_.empty() || config_.max_load_average <= 0.0f)
        || util::GetLoadAverage() < config_.max_load_average);
}
...
bool Builder::ExtractDeps(CommandRunner::Result* result,
  ...
  if (deps_type == "gcc") {
    ...
    for (vector<StringPiece>::iterator i = deps.ins_.begin();
         i != deps.ins_.end(); ++i) {
      uint64_t slash_bits;
      if (!util::CanonicalizePath(const_cast<char*>(i->str_), &i->len_, &slash_bits,
                            err))
        return false;
    ...
    util::Fatal("unknown deps type '%s'", deps_type.c_str());
  }
