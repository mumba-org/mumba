...
struct NinjaMain : public BuildLogUser {
  ...
  virtual bool IsPathDead(StringPiece s) const {
    ...
    if (mtime == -1)
      util::Error("%s", err.c_str());  // Log and ignore Stat() errors.
    return mtime == 0;
  }
};
...
int GuessParallelism() {
  switch (int processors = util::GetProcessorCount()) {
  case 0:
  case 1:
    return 2;
  case 2:
...
bool NinjaMain::RebuildManifest(const char* input_file, string* err) {
  ...
  if (!util::CanonicalizePath(&path, &slash_bits, err))
    return false;
  ...
  if (argc >= 1) {
    ...
    if (mode == "rule") {
      ...
      const char* suggestion =
          util::SpellcheckString(mode.c_str(), "rule", "depth", "all", NULL);
      if (suggestion) {
        util::Error("unknown target tool mode '%s', did you mean '%s'?",
              mode.c_str(), suggestion);
      } else {
        util::Error("unknown target tool mode '%s'", mode.c_str());
      }
      return 1;
    }
  }
...
int NinjaMain::ToolCompilationDatabase(const Options* options, int argc, char* argv[]) {
  ...
  if (errno != 0 && errno != ERANGE) {
    util::Error("cannot determine working directory: %s", strerror(errno));
    return 1;
  }
...
const Tool* ChooseTool(const string& tool_name) {
  ...
  const char* suggestion = util::SpellcheckStringV(tool_name, words);
  if (suggestion) {
    util::Fatal("unknown tool '%s', did you mean '%s'?",
          tool_name.c_str(), suggestion);
  } else {
    util::Fatal("unknown tool '%s'", tool_name.c_str());
  }
...
"multiple modes can be enabled via -d FOO -d BAR\n");
    ...
    const char* suggestion =
        util::SpellcheckString(name.c_str(),
                         "stats", "explain", "keepdepfile", "keeprsp",
                         "nostatcache", NULL);
    if (suggestion) {
      util::Error("unknown debug setting '%s', did you mean '%s'?",
            name.c_str(), suggestion);
    } else {
      util::Error("unknown debug setting '%s'", name.c_str());
    }
    return false;
  }
}
...
"  dupbuild={err,warn}  multiple build lines for one target\n");
    ...
    const char* suggestion =
        util::SpellcheckString(name.c_str(), "dupbuild=err", "dupbuild=warn", NULL);
    if (suggestion) {
      util::Error("unknown warning flag '%s', did you mean '%s'?",
            name.c_str(), suggestion);
    } else {
      util::Error("unknown warning flag '%s'", name.c_str());
    }
    return false;
  }
}
...
bool NinjaMain::OpenBuildLog(bool recompact_only) {
  ...
  if (!build_log_.Load(log_path, &err)) {
    util::Error("loading build log %s: %s", log_path.c_str(), err.c_str());
    return false;
  }
  if (!err.empty()) {
    // Hack: Load() can return a warning via err by returning true.
    util::Warning("%s", err.c_str());
    err.clear();
  }
  ...
  if (recompact_only) {
    ...
    if (!success)
      util::Error("failed recompaction: %s", err.c_str());
    return success;
  }

  if (!config_.dry_run) {
    if (!build_log_.OpenForWrite(log_path, *this, &err)) {
      util::Error("opening build log: %s", err.c_str());
      return false;
    }
  }
...
bool NinjaMain::OpenDepsLog(bool recompact_only) {
  ...
  if (!deps_log_.Load(path, &state_, &err)) {
    util::Error("loading deps log %s: %s", path.c_str(), err.c_str());
    return false;
  }
  if (!err.empty()) {
    // Hack: Load() can return a warning via err by returning true.
  ...
  if (!config_.dry_run) {
    if (!deps_log_.OpenForWrite(path, &err)) {
      util::Error("opening deps log: %s", err.c_str());
      return false;
    }
  }
...
bool NinjaMain::EnsureBuildDirExists() {
  ...
  if (!build_dir_.empty() && !config_.dry_run) {
    if (!disk_interface_.MakeDirs(build_dir_ + "/.") && errno != EEXIST) {
      util::Error("creating build directory %s: %s",
...
void TerminateHandler() {
  ...
  util::Fatal("terminate handler called");
}
...
int ExceptionFilter(unsigned int code, struct _EXCEPTION_POINTERS *ep) {
  util::Error("exception: 0x%X", code);  // e.g. EXCEPTION_ACCESS_VIOLATION
  fflush(stderr);
  ...
  while (!options->tool &&
    ...
    switch (opt) {
      ...
      case 'j': {
        ...
        if (*end != 0 || value <= 0)
          util::Fatal("invalid -j parameter");
      ...
      case 'k': {
        ...
        int value = strtol(optarg, &end, 10);
        if (*end != 0)
          util::Fatal("-k parameter not numeric; did you mean -k 0?");
      ...
      case 'l': {
        ...
        if (end == optarg)
          util::Fatal("-l parameter not numeric: did you mean -l 0.0?");
...
int real_main(int argc, char** argv) {
  ...
  if (options.working_dir) {
    ...
    if (chdir(options.working_dir) < 0) {
      util::Fatal("chdir to '%s' - %s", options.working_dir, strerror(errno));
    }
  }
  ...
  for (int cycle = 1; cycle <= kCycleLimit; ++cycle) {
    ...
    if (ninja.RebuildManifest(options.input_file, &err)) {
      ...
      util::Error("rebuilding '%s': %s", options.input_file, err.c_str());
      return 1;
    }
  ...
  util::Error("manifest '%s' still dirty after %d tries\n",
...
int main(int argc, char** argv) {
  ...
  __try {
    ...
    return builder::real_main(argc, argv);
  }
