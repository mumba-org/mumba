...
bool Subprocess::Start(SubprocessSet* set, const string& command) {
  ...
  if (pipe(output_pipe) < 0)
    util::Fatal("pipe: %s", strerror(errno));
  fd_ = output_pipe[0];
#if !defined(USE_PPOLL)
  // If available, we use ppoll in DoWork(); otherwise we use pselect
  // and so must avoid overly-large FDs.
  if (fd_ >= static_cast<int>(FD_SETSIZE))
    util::Fatal("pipe: %s", strerror(EMFILE));
#endif  // !USE_PPOLL
  util::SetCloseOnExec(fd_);

  posix_spawn_file_actions_t action;
  if (posix_spawn_file_actions_init(&action) != 0)
    util::Fatal("posix_spawn_file_actions_init: %s", strerror(errno));

  if (posix_spawn_file_actions_addclose(&action, output_pipe[0]) != 0)
    util::Fatal("posix_spawn_file_actions_addclose: %s", strerror(errno));

  posix_spawnattr_t attr;
  if (posix_spawnattr_init(&attr) != 0)
    util::Fatal("posix_spawnattr_init: %s", strerror(errno));

  short flags = 0;

  flags |= POSIX_SPAWN_SETSIGMASK;
  if (posix_spawnattr_setsigmask(&attr, &set->old_mask_) != 0)
    util::Fatal("posix_spawnattr_setsigmask: %s", strerror(errno));
  ...
  if (!use_console_) {
    ...
    if (posix_spawn_file_actions_addopen(&action, 0, "/dev/null", O_RDONLY,
                                         0) != 0) {
      util::Fatal("posix_spawn_file_actions_addopen: %s", strerror(errno));
    }

    if (posix_spawn_file_actions_adddup2(&action, output_pipe[1], 1) != 0)
      util::Fatal("posix_spawn_file_actions_adddup2: %s", strerror(errno));
  ...
  if (posix_spawnattr_setflags(&attr, flags) != 0)
    util::Fatal("posix_spawnattr_setflags: %s", strerror(errno));

  const char* spawned_args[] = { "/bin/sh", "-c", command.c_str(), NULL };
  if (posix_spawn(&pid_, "/bin/sh", &action, &attr,
                  const_cast<char**>(spawned_args), environ) != 0)
    util::Fatal("posix_spawn: %s", strerror(errno));

  if (posix_spawnattr_destroy(&attr) != 0)
    util::Fatal("posix_spawnattr_destroy: %s", strerror(errno));
  if (posix_spawn_file_actions_destroy(&action) != 0)
    util::Fatal("posix_spawn_file_actions_destroy: %s", strerror(errno));
...
void Subprocess::OnPipeReady() {
  ...
  if (len > 0) {
    ...
    if (len < 0)
      util::Fatal("read: %s", strerror(errno));
    close(fd_);
    fd_ = -1;
  }
}
...
ExitStatus Subprocess::Finish() {
  ...
  if (waitpid(pid_, &status, 0) < 0)
    util::Fatal("waitpid(%d): %s", pid_, strerror(errno));
  pid_ = -1;
...
SubprocessSet::SubprocessSet() {
  ...
  if (sigprocmask(SIG_BLOCK, &set, &old_mask_) < 0)
    util::Fatal("sigprocmask: %s", strerror(errno));

  struct sigaction act;
  memset(&act, 0, sizeof(act));
  act.sa_handler = SetInterruptedFlag;
  if (sigaction(SIGINT, &act, &old_int_act_) < 0)
    util::Fatal("sigaction: %s", strerror(errno));
