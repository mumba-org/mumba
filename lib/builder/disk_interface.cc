...
int DoMakeDir(const string& path) {
#ifdef _WIN32
...
bool RealDiskInterface::WriteFile(const string& path, const string& contents) {
  ...
  if (fp == NULL) {
    util::Error("WriteFile(%s): Unable to create file. %s",
          path.c_str(), strerror(errno));
    return false;
  }

  if (fwrite(contents.data(), 1, contents.length(), fp) < contents.length())  {
    util::Error("WriteFile(%s): Unable to write to the file. %s",
          path.c_str(), strerror(errno));
    fclose(fp);
    return false;
  }
  ...
  if (fclose(fp) == EOF) {
    util::Error("WriteFile(%s): Unable to close the file. %s",
          path.c_str(), strerror(errno));
    return false;
  }
...
bool RealDiskInterface::MakeDir(const string& path) {
  if (DoMakeDir(path) < 0) {
    ...
    util::Error("mkdir(%s): %s", path.c_str(), strerror(errno));
    return false;
  }
  return true;
}
...
FileReader::Status RealDiskInterface::ReadFile(const string& path,
                                               string* contents,
                                               string* err) {
  switch (util::ReadFile(path, contents, err)) {
  case 0:       return Okay;
  ...
  if (remove(path.c_str()) < 0) {
    switch (errno) {
      case ENOENT:
        return 1;
      default:
        util::Error("remove(%s): %s", path.c_str(), strerror(errno));
        return -1;
    }
  } else {
    return 0;
