...
VariationsCrashKeys::VariationsCrashKeys() {
  //DLOG(INFO) << "VariationsCrashKeys::VariationsCrashKeys";
  base::FieldTrial::ActiveGroups active_groups;
  base::FieldTrialList::GetActiveFieldTrialGroups(&active_groups);
  //DLOG(INFO) << "after base::FieldTrialList::GetActiveFieldTrialGroups";
  for (const auto& entry : active_groups) {
    AppendFieldTrial(entry.trial_name, entry.group_name);
  }
  UpdateCrashKeys();
  //DLOG(INFO) << "after UpdateCrashKeys()";

  ui_thread_task_runner_ = base::SequencedTaskRunnerHandle::Get();
  //DLOG(INFO) << "after base::SequencedTaskRunnerHandle::Get";
  base::FieldTrialList::SetSynchronousObserver(this);
  //DLOG(INFO) << "VariationsCrashKeys::VariationsCrashKeys end";
}
