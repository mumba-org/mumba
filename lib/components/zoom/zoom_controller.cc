...
#include "core/common/page_type.h"
...
double ZoomController::GetZoomLevelForApplicationContents(
    const host::ApplicationContents* app_contents) {
  if (!app_contents)
    return 0.0;

  auto* zoom_controller = FromApplicationContents(app_contents);
  if (zoom_controller)
    return zoom_controller->GetZoomLevel();

  return host::HostZoomMap::GetZoomLevel(app_contents);
}

ZoomController::ZoomController(host::ApplicationContents* app_contents)
    : host::ApplicationContentsObserver(app_contents),
      can_show_bubble_(true),
      zoom_mode_(ZOOM_MODE_DEFAULT),
      zoom_level_(1.0) {//,
      //browser_context_(app_contents->GetBrowserContext()) {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  host_zoom_map_ = host::HostZoomMap::GetForApplicationContents(app_contents);
...
bool ZoomController::IsAtDefaultZoom() const {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  return common::ZoomValuesEqual(GetZoomLevel(), GetDefaultZoomLevel());
}
...
ZoomController::RelativeZoom ZoomController::GetZoomRelativeToDefault() const {
  ...
  if (common::ZoomValuesEqual(current_level, default_level))
...
double ZoomController::GetZoomLevel() const {
  ...
  return zoom_mode_ == ZOOM_MODE_MANUAL
             ? zoom_level_
             : host::HostZoomMap::GetZoomLevel(application_contents());
}

int ZoomController::GetZoomPercent() const {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  double zoom_factor = common::ZoomLevelToZoomFactor(GetZoomLevel());
...
bool ZoomController::SetZoomLevelByClient(
  ...
  if (zoom_mode_ == ZOOM_MODE_DISABLED ||
      !application_contents()->GetApplicationWindowHost()->IsApplicationWindowLive())
    return false;
  ...
  if (zoom_mode_ == ZOOM_MODE_MANUAL) {
    ...
    if (common::ZoomValuesEqual(zoom_level_, zoom_level))
      return true;
    ...
    host::HostZoomMap::ZoomLevelChange change;
    change.mode = host::HostZoomMap::ZOOM_CHANGED_TEMPORARY_ZOOM;
    change.zoom_level = zoom_level;
    ZoomEventManager::Get()//ForBrowserContext(browser_context_)
    ...
    ZoomChangedEventData zoom_change_data(application_contents(), old_zoom_level,
  ...
  host::HostZoomMap* zoom_map =
      host::HostZoomMap::GetForApplicationContents(application_contents());
  DCHECK(zoom_map);
  DCHECK(!event_data_);
  event_data_.reset(new ZoomChangedEventData(application_contents(), GetZoomLevel(),
                                             zoom_level, zoom_mode_,
                                             false /* can_show_bubble */));
  int process_id = application_contents()->GetApplicationWindowHost()->GetProcess()->GetID();
  int view_id = application_contents()->GetApplicationWindowHost()->GetRoutingID();
  ...
  //   }
  //   std::string host =
  //       net::GetHostOrSpecFromURL(host::HostZoomMap::GetURLFromEntry(entry));
...
void ZoomController::SetZoomMode(ZoomMode new_mode) {
  ...
  event_data_.reset(new ZoomChangedEventData(
      application_contents(), original_zoom_level, original_zoom_level, new_mode,
  ...
  switch (new_mode) {
    case ZOOM_MODE_DEFAULT: {
      ...
      // if (entry) {
      //   GURL url = host::HostZoomMap::GetURLFromEntry(entry);
...
void ZoomController::ResetZoomModeOnNavigationIfNeeded(const GURL& url) {
  ...
  double old_zoom_level = zoom_map->GetZoomLevel(application_contents());
  double new_zoom_level = zoom_map->GetZoomLevelForHostAndScheme(
      url.scheme(), net::GetHostOrSpecFromURL(url));
  event_data_.reset(new ZoomChangedEventData(application_contents(), old_zoom_level,
...
//     host::NavigationHandle* navigation_handle) {
//   DCHECK_CURRENTLY_ON(HostThread::UI);
//   if (!navigation_handle->IsInMainFrame() || !navigation_handle->HasCommitted())
//     return;

//   if (navigation_handle->IsErrorPage())
//     host::HostZoomMap::SendErrorPageZoomLevelRefresh(application_contents());
...
void ZoomController::ApplicationContentsDestroyed() {
  DCHECK_CURRENTLY_ON(HostThread::UI);
...
void ZoomController::ApplicationWindowChanged(
    host::ApplicationWindowHost* old_host,
    host::ApplicationWindowHost* new_host) {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  // If our associated HostZoomMap changes, update our event subscription.
  host::HostZoomMap* new_host_zoom_map =
      host::HostZoomMap::GetForApplicationContents(application_contents());
...
void ZoomController::UpdateState(const std::string& host) {
  ...
  //if (!host.empty()) {
    // Use the navigation entry's URL instead of the ApplicationContents' so virtual
    // URLs work (e.g. chrome://settings). http://crbug.com/153950
    //host::NavigationEntry* entry =
    //    application_contents()->GetController().GetLastCommittedEntry();
    //if (!entry ||
        //host != net::GetHostOrSpecFromURL(
         //           host::HostZoomMap::GetURLFromEntry(entry))) {
      //return;
    //}
  //}
  ...
  if (event_data_) {
    ...
    ZoomChangedEventData zoom_change_data(application_contents(), zoom_level,
                                          zoom_level, zoom_mode_,
                                          false /* can_show_bubble */);
    for (auto& observer : observers_)
      observer.OnZoomChanged(zoom_change_data);
...
bool ZoomController::PageScaleFactorIsOne() const {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  return host::HostZoomMap::PageScaleFactorIsOne(application_contents());
}
