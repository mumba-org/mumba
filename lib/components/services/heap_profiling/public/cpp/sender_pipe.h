 ...
 public:
  ...
  class PipePair {
   public:
    ...
    PipePair();
    PipePair(PipePair&&);
    mojo::ScopedPlatformHandle PassSender() { return std::move(sender_); }
    mojo::ScopedPlatformHandle PassReceiver() {
      return std::move(receiver_);
    }

   private:
    mojo::ScopedPlatformHandle sender_;
    mojo::ScopedPlatformHandle receiver_;
