...
//   for (const common::Manifest::Icon& icon : manifest.icons) {
...
// static
void ContentFaviconDriver::CreateForApplicationContents(
  ...
  app_contents->SetUserData(
      UserDataKey(), base::WrapUnique(new ContentFaviconDriver(
                         app_contents, favicon_service)));//, history_service)));
}
...
GURL ContentFaviconDriver::GetActiveURL() {
  //content::NavigationEntry* entry =
  //    web_contents()->GetController().GetLastCommittedEntry();
  return GURL();//entry ? entry->GetURL() : GURL();
}

ContentFaviconDriver::ContentFaviconDriver(
    host::ApplicationContents* web_contents,
    FaviconService* favicon_service)//,
    //history::HistoryService* history_service)
    : ApplicationContentsObserver(web_contents),
      FaviconDriverImpl(favicon_service, nullptr),
...
int ContentFaviconDriver::DownloadImage(const GURL& url,
  ...
  //return application_contents()->DownloadImage(url, true, max_image_size, bypass_cache,
  //                                     std::move(callback));
  return -1;
}
...
void ContentFaviconDriver::OnFaviconUpdated(
  ...
  //if (notification_icon_type == FaviconDriverObserver::NON_TOUCH_16_DIP) {
    ...
    //application_contents()->NotifyNavigationStateChanged(content::INVALIDATE_TYPE_TAB);
  //}
...
void ContentFaviconDriver::DidUpdateFaviconURL(
    const std::vector<common::FaviconURL>& candidates) {
