...
class ContentFaviconDriver
    : public host::ApplicationContentsObserver,
      public host::ApplicationContentsUserData<ContentFaviconDriver>,
      public FaviconDriverImpl {
 public:
  ~ContentFaviconDriver() override;

  static void CreateForApplicationContents(host::ApplicationContents* web_contents,
  ...
  std::vector<common::FaviconURL> favicon_urls() const {
    return favicon_urls_.value_or(std::vector<common::FaviconURL>());
  }
 ...
 protected:
  ContentFaviconDriver(host::ApplicationContents* app_contents,
                       FaviconService* favicon_service);//,
                       //history::HistoryService* history_service);

 private:
  friend class host::ApplicationContentsUserData<ContentFaviconDriver>;
  ...
  void DidUpdateFaviconURL(
      const std::vector<common::FaviconURL>& candidates) override;
  ...
  base::Optional<std::vector<common::FaviconURL>> favicon_urls_;
