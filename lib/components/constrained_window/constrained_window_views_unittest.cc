...
class ConstrainedWindowViewsTest : public views::ViewsTestBase {
 public:
  ...
  void SetUp() override {
    ...
    dialog_host_.reset(new web_modal::TestApplicationContentsModalDialogHost(
  ...
  web_modal::TestApplicationContentsModalDialogHost* dialog_host() {
 ...
 private:
  ...
  std::unique_ptr<web_modal::TestApplicationContentsModalDialogHost> dialog_host_;
  Widget* dialog_;
...
TEST_F(ConstrainedWindowViewsTest, MaximumApplicationContentsDialogSize) {
  UpdateApplicationContentsModalDialogPosition(dialog(), dialog_host());
