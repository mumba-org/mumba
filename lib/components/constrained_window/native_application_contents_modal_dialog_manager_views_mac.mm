...
#include "components/constrained_window/native_application_contents_modal_dialog_manager_views_mac.h"
...
using web_modal::ApplicationContentsModalDialogManager;
...
NativeApplicationContentsModalDialogManagerViewsMac::
    ...
    : NativeApplicationContentsModalDialogManagerViews(dialog, native_delegate) {}

// NativeApplicationContentsModalDialogManagerViews:
void NativeApplicationContentsModalDialogManagerViewsMac::OnPositionRequiresUpdate() {
  NativeApplicationContentsModalDialogManagerViews::OnPositionRequiresUpdate();
  ...
  host::ApplicationContents* application_contents = native_delegate()->GetApplicationContents();
  // Note: Can't use ApplicationContents container bounds here because it doesn't
  // include the DevTool panel width.
  CGFloat window_width =
      NSWidth([application_contents->GetTopLevelNativeWindow() frame]);
  gfx::Rect tab_view_size = application_contents->GetContainerBounds();
  widget->SetBounds(gfx::Rect(tab_view_size.x(),
                              widget->GetWindowBoundsInScreen().y(),
                              window_width, tab_view_size.height()));
}

void NativeApplicationContentsModalDialogManagerViewsMac::ShowWidget(
    views::Widget* widget) {
  ...
  if (![dialog_window attachedSheet]) {
    NativeApplicationContentsModalDialogManagerViews::ShowWidget(widget);
...
void NativeApplicationContentsModalDialogManagerViewsMac::HideWidget(
    views::Widget* widget) {
  NSWindow* dialog_window = widget->GetNativeWindow();
