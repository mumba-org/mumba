...
#import "components/constrained_window/native_app_contents_modal_dialog_manager_views_mac.h"
#endif
...
class WidgetModalDialogHostObserverViews
 ...
 public:
  ...
  // ApplicationContentsModalDialogHostObserver overrides
...
host::ApplicationContents* GetTopLevelApplicationContents(
  ...
  return nullptr;//guest_view::GuestViewBase::GetTopLevelApplicationContents(
      //initiator_app_contents);
}
...
views::Widget* ShowWebModalDialogViews(
  ...
  // For embedded ApplicationContents, use the embedder's ApplicationContents for constrained
  // window.
  host::ApplicationContents* app_contents =
      GetTopLevelApplicationContents(initiator_app_contents);
  views::Widget* widget = CreateWebModalDialogViews(dialog, app_contents);
  ShowModalDialog(widget->GetNativeWindow(), app_contents);
  return widget;
}
...
views::Widget* ShowWebModalDialogWithOverlayViews(
  ...
  web_modal::ApplicationContentsModalDialogManager* manager =
      web_modal::ApplicationContentsModalDialogManager::FromApplicationContents(app_contents);
  std::unique_ptr<web_modal::SingleApplicationContentsDialogManager> dialog_manager(
      new NativeApplicationContentsModalDialogManagerViewsMac(widget->GetNativeWindow(),
                                                      manager));
