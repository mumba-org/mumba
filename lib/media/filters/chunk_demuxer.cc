...
void ChunkDemuxer::Initialize(DemuxerHost* host,
  ...
  DLOG(INFO) << "ChunkDemuxer::Initialize";
...
ChunkDemuxer::Status ChunkDemuxer::AddId(const std::string& id,
  ...
  DLOG(INFO) << "ChunkDemuxer::AddId: binding ChunkDemuxer::OnSourceInitDone on source_state->Init()";
  source_state->Init(
...
bool ChunkDemuxer::AppendData(const std::string& id,
  ...
  DLOG(INFO) << "ChunkDemuxer::AppendData: id = " << id << " len: " << length << " state_ = " << state_;
...

  {
    ...
    if (length == 0u) {
      DLOG(INFO) << "ChunkDemuxer::AppendData: len = 0. returning true";
      return true;
    }
...
void ChunkDemuxer::MarkEndOfStream(PipelineStatus status) {
  DLOG(INFO) << "ChunkDemuxer::MarkEndOfStream";
  ...
  if (state_ == INITIALIZING) {
    DLOG(INFO) << "ChunkDemuxer::MarkEndOfStream: state_ == INITIALIZING -> DEMUXER_ERROR_COULD_NOT_OPEN";
...
void ChunkDemuxer::OnSourceInitDone(
  ...
  DLOG(INFO) << "ChunkDemuxer::SourceInitDone";    
