  ...
  for (size_t i = 0; i < arraysize(kSupportedTypeInfo); ++i) {
    ...
    if (type == type_info.type) {
      if (codecs.empty()) {
        ...
        DLOG(INFO) << "A codecs parameter must be provided for " << type;
        return false;
      }
      ...
      for (size_t j = 0; j < codecs.size(); ++j) {
        ...
        if (!found_codec) {
          DLOG(INFO) << "Codec '" << codec_id << "' is not supported for " << type;
          MEDIA_LOG(DEBUG, media_log)
...
std::unique_ptr<StreamParser> StreamParserFactory::Create(
    ...
    const std::vector<std::string>& codecs,
    MediaLog* media_log) {
  DLOG(INFO) << "StreamParserFactory::Create: type = " << type;
  ...
  if (CheckTypeAndCodecs(type, codecs, media_log, &factory_function,
    ...
    for (size_t i = 0; i < audio_codecs.size(); ++i) {
      DLOG(INFO) << " type = " << type << " audio codec [" << i << "] = '" << audio_codecs[i] << "'";
  
      UMA_HISTOGRAM_ENUMERATION("Media.MSE.AudioCodec", audio_codecs[i],
                                CodecInfo::HISTOGRAM_MAX + 1);
    }
    for (size_t i = 0; i < video_codecs.size(); ++i) {
      DLOG(INFO) << " type = " << type << " video codec [" << i << "] = '" << video_codecs[i] << "'";
