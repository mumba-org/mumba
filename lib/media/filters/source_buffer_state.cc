...
void SourceBufferState::Init(
  ...
  DLOG(INFO) << "SourceBufferState::Init";
...
bool SourceBufferState::Append(const uint8_t* data,
  ...
  DLOG(INFO) << "SourceBufferState::Append";
  ...
  DLOG(INFO) << "SourceBufferState::Append: calling stream_parser_->Parse()";
...
bool SourceBufferState::OnNewConfigs(
  ...
  for (const auto& track : tracks->tracks()) {
    ...
    if (track->type() == MediaTrack::Audio) {
      ...
      DLOG(INFO) << "Video track_id=" << track_id
                 << " config: " << video_config.AsHumanReadableString();         
      DCHECK(video_config.IsValidConfig());
      // mumba: changed here 
      for(auto it = expected_vcodecs.begin(); it != expected_vcodecs.end(); ++it) {
        DLOG(INFO) << "codec: " << GetCodecName(*it);
      }
...
void SourceBufferState::OnSourceInitDone(
  ...
  DLOG(INFO) << "SourceBufferState::OnSourceInitDone";
