...
#include "rpc/surface/event_string.h"
...
static grpc_event cq_next(grpc_completion_queue* cq, gpr_timespec deadline,
  ...
  for (;;) {
    ...
    if (c != nullptr) {
      ...
      int queue_items = cq_event_queue_num_items(&cqd->queue);
    ...
    int pending = gpr_atm_acq_load(&cqd->pending_events);
    if (pending == 0) {
...
static void cq_shutdown_next(grpc_exec_ctx* exec_ctx,
  ...
  int pending = gpr_atm_full_fetch_add(&cqd->pending_events, -1);
  if (pending == 1) {
    cq_finish_shutdown_next(exec_ctx, cq);
  }
  gpr_mu_unlock(cq->mu);
