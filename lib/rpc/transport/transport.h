...
#include "rpc/transport/metadata_batch.h"
...
GRPCAPI extern grpc_core::DebugOnlyTraceFlag grpc_trace_stream_refcount;
...
#ifndef NDEBUG
GRPCAPI void grpc_stream_ref_init(grpc_stream_refcount* refcount, int initial_refs,
                          grpc_iomgr_cb_func cb, void* cb_arg,
                          const char* object_type);
GRPCAPI void grpc_stream_ref(grpc_stream_refcount* refcount, const char* reason);
GRPCAPI void grpc_stream_unref(grpc_exec_ctx* exec_ctx, grpc_stream_refcount* refcount,
                       const char* reason);
...
GRPCAPI void grpc_stream_ref_init(grpc_stream_refcount* refcount, int initial_refs,
                          grpc_iomgr_cb_func cb, void* cb_arg);
GRPCAPI void grpc_stream_ref(grpc_stream_refcount* refcount);
GRPCAPI void grpc_stream_unref(grpc_exec_ctx* exec_ctx, grpc_stream_refcount* refcount);
#define GRPC_STREAM_REF_INIT(rc, ir, cb, cb_arg, objtype) \
...
/* Wrap a buffer that is owned by some stream object into a slice that shares
   the same refcount */
GRPCAPI grpc_slice grpc_slice_from_stream_owned_buffer(grpc_stream_refcount* refcount,
...
GRPCAPI void grpc_transport_move_one_way_stats(grpc_transport_one_way_stats* from,
                                       grpc_transport_one_way_stats* to);

GRPCAPI void grpc_transport_move_stats(grpc_transport_stream_stats* from,
...
/* Returns the amount of memory required to store a grpc_stream for this
   transport */
GRPCAPI size_t grpc_transport_stream_size(grpc_transport* transport);
...
GRPCAPI int grpc_transport_init_stream(grpc_exec_ctx* exec_ctx,
                               grpc_transport* transport, grpc_stream* stream,
                               grpc_stream_refcount* refcount,
                               const void* server_data, gpr_arena* arena);

GRPCAPI void grpc_transport_set_pops(grpc_exec_ctx* exec_ctx, grpc_transport* transport,
...
GRPCAPI void grpc_transport_destroy_stream(grpc_exec_ctx* exec_ctx,
                                   grpc_transport* transport,
                                   grpc_stream* stream,
                                   grpc_closure* then_schedule_closure);

GRPCAPI void grpc_transport_stream_op_batch_finish_with_failure(
    grpc_exec_ctx* exec_ctx, grpc_transport_stream_op_batch* op,
    grpc_error* error, grpc_call_combiner* call_combiner);

GRPCAPI char* grpc_transport_stream_op_batch_string(grpc_transport_stream_op_batch* op);
GRPCAPI char* grpc_transport_op_string(grpc_transport_op* op);
...
/* Send a batch of operations on a transport
   ...
   */
GRPCAPI void grpc_transport_perform_stream_op(grpc_exec_ctx* exec_ctx,
                                      grpc_transport* transport,
                                      grpc_stream* stream,
                                      grpc_transport_stream_op_batch* op);

GRPCAPI void grpc_transport_perform_op(grpc_exec_ctx* exec_ctx,
                               grpc_transport* transport,
...
GRPCAPI void grpc_transport_ping(grpc_transport* transport, grpc_closure* cb);

/* Advise peer of pending connection termination. */
GRPCAPI void grpc_transport_goaway(grpc_transport* transport, grpc_status_code status,
                           grpc_slice debug_data);

/* Destroy the transport */
GRPCAPI void grpc_transport_destroy(grpc_exec_ctx* exec_ctx, grpc_transport* transport);

/* Get the endpoint used by \a transport */
GRPCAPI grpc_endpoint* grpc_transport_get_endpoint(grpc_exec_ctx* exec_ctx,
                                           grpc_transport* transport);

/* Allocate a grpc_transport_op, and preconfigure the on_consumed closure to
   \a on_consumed and then delete the returned transport op */
GRPCAPI grpc_transport_op* grpc_make_transport_op(grpc_closure* on_consumed);
...
GRPCAPI grpc_transport_stream_op_batch* grpc_make_transport_stream_op(
    grpc_closure* on_consumed);
