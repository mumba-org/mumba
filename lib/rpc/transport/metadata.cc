...
#include "rpc/support/murmur_hash.h"
...
void grpc_mdelem_unref(grpc_exec_ctx* exec_ctx, grpc_mdelem gmd DEBUG_ARGS) {
  switch (GRPC_MDELEM_STORAGE(gmd)) {
    ...
    case GRPC_MDELEM_STORAGE_STATIC:
      break;
    case GRPC_MDELEM_STORAGE_INTERNED: {
      // mumba: added this line here for debugging
      // sometimes theres an fatal error going on here
      if ((intptr_t)GRPC_MDELEM_DATA(gmd) == 0xcdcdcdcdcdcdcdcc) {
        //DLOG(INFO) << "grpc_mdelem_unref: md (grpc_mdelem) is not null but a dangling pointer. leaving";
        break;
      }
      interned_metadata* md = (interned_metadata*)GRPC_MDELEM_DATA(gmd);
#ifndef NDEBUG
