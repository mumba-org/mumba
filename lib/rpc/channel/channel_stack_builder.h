...
GRPCAPI grpc_channel_stack_builder* grpc_channel_stack_builder_create(void);

/// Assign a name to the channel stack: \a name must be statically allocated
GRPCAPI void grpc_channel_stack_builder_set_name(grpc_channel_stack_builder* builder,
                                         const char* name);

/// Set the target uri
GRPCAPI void grpc_channel_stack_builder_set_target(grpc_channel_stack_builder* b,
                                           const char* target);

GRPCAPI const char* grpc_channel_stack_builder_get_target(
    grpc_channel_stack_builder* b);

/// Attach \a transport to the builder (does not take ownership)
GRPCAPI void grpc_channel_stack_builder_set_transport(
    grpc_channel_stack_builder* builder, grpc_transport* transport);

/// Fetch attached transport
GRPCAPI grpc_transport* grpc_channel_stack_builder_get_transport(
    grpc_channel_stack_builder* builder);

/// Set channel arguments: copies args
GRPCAPI void grpc_channel_stack_builder_set_channel_arguments(
    grpc_exec_ctx* exec_ctx, grpc_channel_stack_builder* builder,
    const grpc_channel_args* args);

/// Return a borrowed pointer to the channel arguments
GRPCAPI const grpc_channel_args* grpc_channel_stack_builder_get_channel_arguments(
    grpc_channel_stack_builder* builder);

/// Begin iterating over already defined filters in the builder at the beginning
GRPCAPI grpc_channel_stack_builder_iterator*
...
GRPCAPI bool grpc_channel_stack_builder_iterator_is_first(
    grpc_channel_stack_builder_iterator* iterator);

/// Is an iterator at the end?
GRPCAPI bool grpc_channel_stack_builder_iterator_is_end(
    grpc_channel_stack_builder_iterator* iterator);

/// What is the name of the filter at this iterator position?
GRPCAPI const char* grpc_channel_stack_builder_iterator_filter_name(
    grpc_channel_stack_builder_iterator* iterator);

/// Move an iterator to the next item
GRPCAPI bool grpc_channel_stack_builder_move_next(
    grpc_channel_stack_builder_iterator* iterator);

/// Move an iterator to the previous item
GRPCAPI bool grpc_channel_stack_builder_move_prev(
    grpc_channel_stack_builder_iterator* iterator);

/// Return an iterator at \a filter_name, or at the end of the list if not
/// found.
GRPCAPI grpc_channel_stack_builder_iterator* grpc_channel_stack_builder_iterator_find(
...
/// created.
GRPCAPI bool grpc_channel_stack_builder_add_filter_after(
    grpc_channel_stack_builder_iterator* iterator,
...
/// Call \a post_init_func(..., \a user_data) once the channel stack is
/// created.
GRPCAPI bool grpc_channel_stack_builder_add_filter_before(
    grpc_channel_stack_builder_iterator* iterator,
    const grpc_channel_filter* filter,
    grpc_post_filter_create_init_func post_init_func,
    void* user_data) GRPC_MUST_USE_RESULT;
...
/// Call \a post_init_func(..., \a user_data) once the channel stack is
/// created.
GRPCAPI bool grpc_channel_stack_builder_prepend_filter(
...
/// Call \a post_init_func(..., \a user_data) once the channel stack is
/// created.
GRPCAPI bool grpc_channel_stack_builder_append_filter(
    grpc_channel_stack_builder* builder, const grpc_channel_filter* filter,
    grpc_post_filter_create_init_func post_init_func,
    void* user_data) GRPC_MUST_USE_RESULT;
...
GRPCAPI bool grpc_channel_stack_builder_remove_filter(
    grpc_channel_stack_builder* builder, const char* filter_name);

/// Terminate iteration and destroy \a iterator
GRPCAPI void grpc_channel_stack_builder_iterator_destroy(
    grpc_channel_stack_builder_iterator* iterator);
...
GRPCAPI grpc_error* grpc_channel_stack_builder_finish(
    grpc_exec_ctx* exec_ctx, grpc_channel_stack_builder* builder,
    size_t prefix_bytes, int initial_refs, grpc_iomgr_cb_func destroy,
    void* destroy_arg, void** result);

/// Destroy the builder without creating a channel stack
GRPCAPI void grpc_channel_stack_builder_destroy(grpc_exec_ctx* exec_ctx,
                                        grpc_channel_stack_builder* builder);
