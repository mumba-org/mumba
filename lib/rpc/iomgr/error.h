...
GRPCAPI const char* grpc_error_string(grpc_error* error);

/// Create an error - but use GRPC_ERROR_CREATE instead
GRPCAPI grpc_error* grpc_error_create(const char* file, int line, grpc_slice desc,
...
#ifndef NDEBUG
GRPCAPI grpc_error* grpc_error_ref(grpc_error* err, const char* file, int line);
GRPCAPI void grpc_error_unref(grpc_error* err, const char* file, int line);
...
#else
GRPCAPI grpc_error* grpc_error_ref(grpc_error* err);
GRPCAPI void grpc_error_unref(grpc_error* err);
#define GRPC_ERROR_REF(err) grpc_error_ref(err)
#define GRPC_ERROR_UNREF(err) grpc_error_unref(err)
#endif

GRPCAPI grpc_error* grpc_error_set_int(grpc_error* src, grpc_error_ints which,
                                       intptr_t value) GRPC_MUST_USE_RESULT;
GRPCAPI bool grpc_error_get_int(grpc_error* error, grpc_error_ints which, intptr_t* p);
GRPCAPI grpc_error* grpc_error_set_str(grpc_error* src, grpc_error_strs which,
                               grpc_slice str) GRPC_MUST_USE_RESULT;
/// Returns false if the specified string is not set.
/// Caller does NOT own the slice.
GRPCAPI bool grpc_error_get_str(grpc_error* error, grpc_error_strs which,
                                grpc_slice* s);

/// Add a child error: an error that is believed to have contributed to this
/// error occurring. Allows root causing high level errors from lower level
/// errors that contributed to them.
GRPCAPI grpc_error* grpc_error_add_child(grpc_error* src,
                                         grpc_error* child) GRPC_MUST_USE_RESULT;
GRPCAPI grpc_error* grpc_os_error(const char* file, int line, int err,
...
GRPCAPI grpc_error* grpc_wsa_error(const char* file, int line, int err,
                           const char* call_name) GRPC_MUST_USE_RESULT;
/// windows only: create an error associated with WSAGetLastError()!=0
#define GRPC_WSA_ERROR(err, call_name) \
  grpc_wsa_error(__FILE__, __LINE__, err, call_name)

GRPCAPI bool grpc_log_if_error(const char* what, grpc_error* error, const char* file,
                       int line);
