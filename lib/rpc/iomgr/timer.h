...
#ifdef GRPC_UV
#include "rpc/iomgr/timer_uv.h"
#else
#include "rpc/iomgr/timer_generic.h"
#endif /* GRPC_UV */
...
void GRPCAPI grpc_timer_init(grpc_exec_ctx* exec_ctx, grpc_timer* timer,
...
void GRPCAPI grpc_timer_init_unset(grpc_timer* timer);
...
void GRPCAPI grpc_timer_cancel(grpc_exec_ctx* exec_ctx, grpc_timer* timer);
...
/* Check for timers to be run, and run them.
   ...
   *next is never guaranteed to be updated on any given execution; however,
   ...
   at any time slice. */
grpc_timer_check_result GRPCAPI grpc_timer_check(grpc_exec_ctx* exec_ctx,
                                         grpc_millis* next);
void GRPCAPI grpc_timer_list_init(grpc_exec_ctx* exec_ctx);
void GRPCAPI grpc_timer_list_shutdown(grpc_exec_ctx* exec_ctx);

/* Consume a kick issued by grpc_kick_poller */
void GRPCAPI grpc_timer_consume_kick(void);

/* the following must be implemented by each iomgr implementation */

void GRPCAPI grpc_kick_poller(void);
