...
#include "rpc/iomgr/socket_factory_posix.h"
#include "rpc/iomgr/socket_mutator.h"

/* a wrapper for accept or accept4 */
int GRPCAPI grpc_accept4(int sockfd, grpc_resolved_address* resolved_addr, int nonblock,
                 int cloexec);

int GRPCAPI grpc_accept(int sockfd, grpc_resolved_address* resolved_addr, int nonblock, int cloexec);

/* set a socket to non blocking mode */
grpc_error* GRPCAPI grpc_set_socket_nonblocking(int fd, int non_blocking);

/* set a socket to close on exec */
grpc_error* GRPCAPI grpc_set_socket_cloexec(int fd, int close_on_exec);

/* set a socket to reuse old addresses */
grpc_error* GRPCAPI grpc_set_socket_reuse_addr(int fd, int reuse);

/* disable nagle */
grpc_error* GRPCAPI grpc_set_socket_low_latency(int fd, int low_latency);

/* set SO_REUSEPORT */
grpc_error* GRPCAPI grpc_set_socket_reuse_port(int fd, int reuse);
...
int GRPCAPI grpc_ipv6_loopback_available(void);

/* Tries to set SO_NOSIGPIPE if available on this platform.
   If SO_NO_SIGPIPE is not available, returns 1. */
grpc_error* GRPCAPI grpc_set_socket_no_sigpipe_if_possible(int fd);

/* Tries to set IP_PKTINFO if available on this platform.
   If IP_PKTINFO is not available, returns 1. */
grpc_error* GRPCAPI grpc_set_socket_ip_pktinfo_if_possible(int fd);

/* Tries to set IPV6_RECVPKTINFO if available on this platform.
   If IPV6_RECVPKTINFO is not available, returns 1. */
grpc_error* GRPCAPI grpc_set_socket_ipv6_recvpktinfo_if_possible(int fd);

/* Tries to set the socket's send buffer to given size. */
grpc_error* GRPCAPI grpc_set_socket_sndbuf(int fd, int buffer_size_bytes);

/* Tries to set the socket's receive buffer to given size. */
grpc_error* GRPCAPI grpc_set_socket_rcvbuf(int fd, int buffer_size_bytes);

/* Tries to set the socket using a grpc_socket_mutator */
grpc_error* GRPCAPI grpc_set_socket_with_mutator(int fd, grpc_socket_mutator* mutator);
...
grpc_error* GRPCAPI grpc_create_dualstack_socket(const grpc_resolved_address* addr,
