...
#include "rpc/ext/transport/chttp2/transport/http2_settings.h"
#include "rpc/support/manual_constructor.h"
#include "rpc/transport/bdp_estimator.h"
#include "rpc/transport/pid_controller.h"
...
inline FlowControlTrace::FlowControlTrace(const char* reason, TransportFlowControl* tfc,
                   StreamFlowControl* sfc) {
    if (enabled_) Init(reason, tfc, sfc);
  }

inline FlowControlTrace::~FlowControlTrace() {
    if (enabled_) Finish();
}
