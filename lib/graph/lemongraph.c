...
typedef struct txn_info * txn_info_t;
struct txn_info {
	txnID_t id;
	logID_t start;
	logID_t count;
...
static INLINE void _entry_unset(graph_txn_t txn, logID_t id, void *key, size_t klen){
	struct prop p = { .pid = id, .rectype = GRAPH_PROP };
...
entry_t graph_entry(graph_txn_t txn, const logID_t id){
	static const int recsizes[] = {
		[GRAPH_DELETION] = sizeof(struct entry),
		[GRAPH_NODE]     = sizeof(struct node),
		[GRAPH_EDGE]     = sizeof(struct edge),
		[GRAPH_PROP]     = sizeof(struct prop),
	};
...
logID_t graph_updateID(graph_txn_t txn, logID_t beforeID){
	static struct entry top = { .id = 0, .next = 0 };
...
prop_t graph_get(graph_txn_t txn, void *key, size_t klen, logID_t beforeID){
	static struct entry parent = { .id = 0 };
