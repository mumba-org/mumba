...
ui::EventDispatchDetails InputMethodAuraLinux::DispatchKeyEvent(
    ui::KeyEvent* event) {
  DLOG(INFO) << "InputMethodAuraLinux::DispatchKeyEvent";
  DCHECK(event->type() == ET_KEY_PRESSED || event->type() == ET_KEY_RELEASED);

  // If no text input client, do nothing.
  if (!GetTextInputClient()) {
    DLOG(INFO) << "  GetTextInputClient() = false. cancelling and calling DispatchKeyEventPostIME()";  
    return DispatchKeyEventPostIME(event);
  }

  if (!event->HasNativeEvent() && sending_key_event()) {
    DLOG(INFO) << "  event->HasNativeEvent() = false && sending_key_event()";  
    ...
    DLOG(INFO) << "  event->is_char() ? " << 
      event->is_char() << " event->GetDomKey().IsCharacter() ? " << 
      event->GetDomKey().IsCharacter() << " event->type() == ui::ET_KEY_PRESSED ? " << 
      (event->type() == ui::ET_KEY_PRESSED);  
    
    if ((event->is_char() || event->GetDomKey().IsCharacter()) &&
        event->type() == ui::ET_KEY_PRESSED) {
      DLOG(INFO) << "  GetTextInputClient()->InsertChar()";
  ...
  bool filtered = false;
  {
    ...
    if (text_input_type_ != TEXT_INPUT_TYPE_NONE &&
      ...
      DLOG(INFO) << " context_->DispatchKeyEvent(*event)";  
      filtered = context_->DispatchKeyEvent(*event);
    } else {
      DLOG(INFO) << " context_simple_->DispatchKeyEvent(*event)";  
  ...
  if (text_input_type_ != TEXT_INPUT_TYPE_PASSWORD &&
    ...
    DLOG(INFO) << " GetEngine()->ProcessKeyEvent()";  
...
ui::EventDispatchDetails InputMethodAuraLinux::ProcessKeyEventDone(
    ...
    bool filtered,
    bool is_handled) {
  DLOG(INFO) << "InputMethodAuraLinux::ProcessKeyEventDone: is_handled ? " << is_handled;  
  DCHECK(event);
  if (is_handled) {
    return ui::EventDispatchDetails();
  }
  ...
  DLOG(INFO) << "  event->type() == ui::ET_KEY_PRESSED ? " << (event->type() == ui::ET_KEY_PRESSED) << " filtered? " << filtered;
  if (event->type() == ui::ET_KEY_PRESSED && filtered) {
    if (NeedInsertChar()) {
      DLOG(INFO) << " NeedInsertChar() = true. DispatchKeyEventPostIME() ";
      details = DispatchKeyEventPostIME(event);
    } else if (HasInputMethodResult()) {
      DLOG(INFO) << " HasInputMethodResult() = true. SendFakeProcessKeyEvent() ";
      details = SendFakeProcessKeyEvent(event);
    }
    if (details.dispatcher_destroyed) {
      DLOG(INFO) << " details.dispatcher_destroyed = true. returning details";
      return details;
    }
    ...
    if (event->stopped_propagation() || details.target_destroyed) {
      DLOG(INFO) << " event->stopped_propagation() || details.dispatcher_destroyed = true. ResetContext() -> return details;";     
      ResetContext();
      return details;
    }
  ...
  if (client && !result_text_.empty()) {
    if (filtered && NeedInsertChar()) {
      DLOG(INFO) << " filtered && NeedInsertChar() = false. client->InsertChar() ";
      ...
      // keydown event once.
      DLOG(INFO) << " filtered && NeedInsertChar() = false. client->InsertText() ";
  ...
  if (client && composition_changed_ && !IsTextInputTypeNone()) {
    ...
    if (!composition_.text.empty()) {
      DLOG(INFO) << " composition_.text.empty() = false. client->SetCompositionText() ";
      client->SetCompositionText(composition_);
    } else if (result_text_.empty()) {
      DLOG(INFO) << " composition_.text.empty() = true && result_text_.empty() = true. client->ClearCompositionText() ";
  ...
  if (!filtered) {
    ...
    if (event->stopped_propagation() || details.target_destroyed) {
      ResetContext();
       ...
       DLOG(INFO) << " event->type() == ui::ET_KEY_PRESSED -> event->GetCharacter() && GetTextInputClient()->InsertChar(*event)";
