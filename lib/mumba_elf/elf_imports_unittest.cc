...
//
// This test makes sure that mumba_elf.dll has only certain types of imports.
...
TEST_F(ELFImportsTest, ChromeElfSanityCheck) {
  ...
  dll = dll.Append(L"mumba_elf.dll");
  ...
  ASSERT_LT(0u, elf_imports.size())
      << "Ensure the mumba_elf_import_unittests "
         "target was built, instead of mumba_elf_import_unittests.exe";
  ...
  for (const std::string& import : elf_imports) {
    ...
    ASSERT_TRUE(match) << "Illegal import in mumba_elf.dll: " << import;
  }
}

TEST_F(ELFImportsTest, ChromeElfLoadSanityTest) {
  // mumba_elf will try to launch crashpad_handler by reinvoking the current
  ...
  // doesn't get loaded by import table when mumba_elf.dll does.
...
TEST_F(ELFImportsTest, DISABLED_ChromeElfLoadSanityTestImpl) {
  ...
  // We don't expect user32 to be loaded in mumba_elf_import_unittests. If this
  // test case fails, then it means that a dependency on user32 has crept into
  // the mumba_elf_imports_unittests executable, which needs to be removed.
  ...
  HMODULE mumba_elf_module_handle = ::LoadLibrary(dll.value().c_str());
  EXPECT_TRUE(mumba_elf_module_handle != nullptr);
  // Loading mumba_elf.dll should not load user32.dll
  EXPECT_EQ(nullptr, ::GetModuleHandle(L"user32.dll"));
  EXPECT_TRUE(!!::FreeLibrary(mumba_elf_module_handle));
}
...
TEST_F(ELFImportsTest, ChromeExeSanityCheck) {
  ...
  EXPECT_EQ("mumba_elf.dll", exe_imports[0])
      << "Illegal import order in chrome.exe (ensure the "
         "mumba_elf_import_unittest "
         "target was built, instead of just mumba_elf_import_unittests.exe)";
}
...
int main(int argc, char** argv) {
  ...
  // CommandLineToArgvW API. The mumba_elf_imports_unittests test suite should
