...
void CreateChannel(PlatformHandle* local_endpoint,
  ...
  *local_endpoint = PlatformHandle(
  ...
  SECURITY_ATTRIBUTES security_attributes = {sizeof(SECURITY_ATTRIBUTES),
                                             nullptr, TRUE};
  *remote_endpoint = PlatformHandle(
...
void CreateChannel(PlatformHandle* local_endpoint,
  ...
  *local_endpoint = PlatformHandle(fds[0]);
  *remote_endpoint = PlatformHandle(fds[1]);
  DCHECK(local_endpoint->is_valid());
  DCHECK(remote_endpoint->is_valid());
}
#else
...
void PlatformChannel::PrepareToPassRemoteEndpoint(HandlePassingInfo* info,
...
#if defined(OS_WIN)
  //->push_back(remote_endpoint_.platform_handle().GetHandle().Get());
  //*value = base::NumberToString(
  //    HandleToLong(remote_endpoint_.platform_handle().GetHandle().Get()));
  info->push_back(remote_endpoint_.platform_handle().handle);
  *value = base::NumberToString(
      HandleToLong(remote_endpoint_.platform_handle().handle));
#elif defined(OS_FUCHSIA)
  ...
  *value = base::NumberToString(mapped_fd);
  ...
  //                   target_fd);
  info->emplace_back(remote_endpoint_.platform_handle().handle,
                     target_fd);
...
PlatformChannelEndpoint PlatformChannel::RecoverPassedEndpointFromString(
  ...
  if (value.empty() || !base::StringToInt(value, &handle_value)) {
    DLOG(ERROR) << "Invalid PlatformChannel endpointv string.";
    return PlatformChannelEndpoint();
  }
  return PlatformChannelEndpoint(
      PlatformHandle(LongToHandle(handle_value)));//base::win::ScopedHandle(LongToHandle(handle_value))));
#elif defined(OS_FUCHSIA)
  ...
  return PlatformChannelEndpoint(PlatformHandle(fd));
#endif
}
