...
PlatformHandle CreateUnixDomainSocket() {
  ...
  PlatformHandle handle(socket(AF_UNIX, SOCK_STREAM, 0));
  if (!handle.is_valid()) {
  ...
  if (!base::SetNonBlocking(handle.handle)) {//handle.GetFD().get())) {
    PLOG(ERROR) << "base::SetNonBlocking() failed " << handle.handle;//handle.GetFD().get();
    return PlatformHandle();
  }
  return handle;
}
...
PlatformChannelServerEndpoint NamedPlatformChannel::CreateServerEndpoint(
  ...
  LOG(INFO) << "NamedPlatformChannel::CreateServerEndpoint";
  ...
  if (bind(handle.handle, reinterpret_cast<const sockaddr*>(&unix_addr),
           unix_addr_len) < 0) {
  ...
  if (listen(handle.handle, SOMAXCONN) < 0) {
...
PlatformChannelEndpoint NamedPlatformChannel::CreateClientEndpoint(
  ...
  LOG(INFO) << "NamedPlatformChannel::CreateClientEndpoint";
  DCHECK(!server_name.empty());
  ...
  if (HANDLE_EINTR(connect(handle.handle,  
