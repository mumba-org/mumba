...
MojoHandle Core::ExtractMessagePipeFromInvitation(const std::string& name) {
  //DLOG(INFO) << "Core::ExtractMessagePipeFromInvitation";
  ...
  //DLOG(INFO) << "Core::ExtractMessagePipeFromInvitation END";
  return handle;
}
...
MojoResult Core::SendInvitation(
  ...
  // of the dispatcher.
  {
    ...
    if (result != MOJO_RESULT_OK) {
      ...
      connection_params.TakeEndpoint().TakePlatformHandle().CloseIfNecessary();
      connection_params.TakeServerEndpoint().TakePlatformHandle().CloseIfNecessary();
      return result;
    }
