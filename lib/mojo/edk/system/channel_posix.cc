...
class ChannelPosixLegacy : public Channel,
                     public base::MessageLoopCurrent::DestructionObserver,
                     public base::MessagePumpForIO::FdWatcher {
 public:
  ChannelPosixLegacy(Delegate* delegate,
  ...
  void Start() override {
    if (io_task_runner_->RunsTasksInCurrentSequence()) {
      ...
      io_task_runner_->PostTask(
          FROM_HERE, base::BindOnce(&ChannelPosixLegacy::StartOnIOThread, this));
    }
  }
  ...
  void ShutDownImpl() override {
    ...
    io_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&ChannelPosixLegacy::ShutDownOnIOThread, this));
  }
  ...
  void Write(MessagePtr message) override {
    ...
    if (write_error) {
      ...
      io_task_runner_->PostTask(
          FROM_HERE, base::BindOnce(&ChannelPosixLegacy::OnWriteError, this,
                                    Error::kDisconnected));
    }
  }
 ...
 private:
  ...
  void WaitForWriteOnIOThreadNoLock() {
    ...
    if (io_task_runner_->RunsTasksInCurrentSequence()) {
      ...
      io_task_runner_->PostTask(
          FROM_HERE,
          base::BindOnce(&ChannelPosixLegacy::WaitForWriteOnIOThread, this));
    }
  }
  ...
  DISALLOW_COPY_AND_ASSIGN(ChannelPosixLegacy);
};
...
class ChannelPosix : public Channel,
 ...
 public:
  ChannelPosix(Delegate* delegate,
    ...
    if (connection_params.server_endpoint().is_valid()) {
      ...
      socket_ = ScopedPlatformHandle(connection_params.TakeEndpoint().TakePlatformHandle());
  ...
  bool GetReadPlatformHandles(
    ...
    if (incoming_fds_.size() < num_handles)
     ...
     handles->resize(num_handles);
     for (size_t i = 0; i < num_handles; ++i) {
       handles->at(i) = std::move(incoming_fds_.front());
 ...
 private:
  ...
  void StartOnIOThread() {
    ...
    if (server_.is_valid()) {
      base::MessageLoopCurrentForIO::Get()->WatchFileDescriptor(
          ...
          server_.platform_handle().handle, false /* persistent */,
          base::MessagePumpForIO::WATCH_READ, read_watcher_.get(), this);
    } else {
      write_watcher_.reset(
          new base::MessagePumpForIO::FdWatchController(FROM_HERE));
      base::MessageLoopCurrentForIO::Get()->WatchFileDescriptor(
          ...
          socket_.get().handle, true /* persistent */,
          base::MessagePumpForIO::WATCH_READ, read_watcher_.get(), this);
      base::AutoLock lock(write_lock_);
      FlushOutgoingMessagesNoLock();
    }
  ...
  void WaitForWriteOnIOThreadNoLock() {
    ...
    if (io_task_runner_->RunsTasksInCurrentSequence()) {
      ...
      base::MessageLoopCurrentForIO::Get()->WatchFileDescriptor(
          ...
          socket_.get().handle, false /* persistent */,
          base::MessagePumpForIO::WATCH_WRITE, write_watcher_.get(), this);
    } else {
      io_task_runner_->PostTask(
          FROM_HERE,
  ...
  void ShutDownOnIOThread() {
    ...
    //}
    if (!leak_handle_) {
      server_.TakePlatformHandle().CloseIfNecessary();
      socket_.reset();
    } else {
      ignore_result(socket_.release());
    }
  ...
  void OnFileCanReadWithoutBlocking(int fd) override {
    if (server_.is_valid()) {
      ...
      CHECK_EQ(fd, server_.platform_handle().handle);
#if !defined(OS_NACL)
      read_watcher_.reset();
      base::MessageLoopCurrent::Get()->RemoveDestructionObserver(this);

      //AcceptSocketConnection(server_.platform_handle().GetFD().get(), &socket_);
      AcceptSocketConnection(server_.platform_handle().handle, &socket_);
      ignore_result(server_.TakePlatformHandle());
      if (!socket_.is_valid()) {
        OnError(Error::kConnectionFailed);
        return;
    ...
    CHECK_EQ(fd, socket_.get().handle);
    ...
    do {
      ...
      ssize_t read_result =
       ...
       PlatformChannelRecvmsg(socket_, buffer, buffer_capacity, &incoming_fds_);
      //for (auto& fd : incoming_fds) {
      //  incoming_fds_.emplace_back(std::move(fd));
        //incoming_fds_.push_back(std::move(fd));
      //}
  ...
  bool WriteNoLock(MessageView message_view) {
   ...
   //   std::vector<PlatformHandleInTransit> handles = message_view.TakeHandles();
      ...
      if (!handles.empty()) {
        ...
        std::vector<int> fds(handles.size());
        for (size_t i = 0; i < handles.size(); ++i)
          fds[i] = handles[i].release().handle;//TakeHandle().handle;//.TakeFD();
        // TODO: Handle lots of handles.
        //result = SendmsgWithHandles(socket_.get(), &iov, 1, fds);
        result = PlatformChannelSendmsgWithHandles(socket_.get(), &iov, 1, fds);
        if (result >= 0) {
#if defined(OS_MACOSX)
          // There is a bug on OSX which makes it dangerous to close
          // a file descriptor while it is in transit. So instead we
          ...
          for (size_t i = 0; i < fds.size(); ++i) {
            handles[i] = ScopedPlatformHandle(PlatformHandle(std::move(fds[i])));
        ...
        //                     message_view.data_num_bytes());
        result = PlatformChannelWrite(socket_.get(), message_view.data(),
                                      message_view.data_num_bytes());
      }
  ...
  base::circular_deque<ScopedPlatformHandle> incoming_fds_;
...
scoped_refptr<Channel> Channel::Create(
    ...
    ConnectionParams connection_params,
    scoped_refptr<base::TaskRunner> io_task_runner) {
  if (connection_params.type() == ConnectionParams::kOLD) {
    return new ChannelPosixLegacy(delegate, std::move(connection_params),
                                 io_task_runner);
  }
