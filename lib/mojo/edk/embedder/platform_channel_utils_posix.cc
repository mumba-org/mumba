...
ssize_t PlatformChannelWrite(const PlatformHandle& h,
                             const void* bytes,
                             size_t num_bytes) {
  DCHECK(h.is_valid());
  DCHECK(bytes);
...
bool AcceptSocketConnection(base::PlatformFile server_fd,
  ...
  //base::ScopedFD accepted_handle(HANDLE_EINTR(accept(server_fd, nullptr, 0)));
   ScopedPlatformHandle accept_handle(PlatformHandle(
      HANDLE_EINTR(accept(server_fd, NULL, 0))));
  ...
  if (check_peer_user && !IsPeerAuthorized(accept_handle.get()))
    return true;
  if (!base::SetNonBlocking(accept_handle.get().handle)) {
    PLOG(ERROR) << "base::SetNonBlocking() failed " << accept_handle.get().handle;
    return true;
  }

  *connection_fd = std::move(accept_handle);
  return true;
#endif  // defined(OS_NACL)
}
