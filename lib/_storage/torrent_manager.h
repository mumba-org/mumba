...
public:
  ...
  TorrentManager(Delegate* delegate, std::shared_ptr<libtorrent::io_context> io_context, scoped_refptr<base::SingleThreadTaskRunner> backend_io_runner);
  ...
  // TorrentCache
  scoped_refptr<Torrent> NewTorrent(IOHandler* io_handler, std::unique_ptr<storage_proto::Info> info) override;
...
private:
  ...
  void RunBackendIO();
  void ProcessFinalShutdown(base::WaitableEvent* shutdown_event);
  void ProcessShutdownEvents(base::WaitableEvent* shutdown_event);
  ...
  bool AddTorrentToSession(const scoped_refptr<Torrent>& torrent);
