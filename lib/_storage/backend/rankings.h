...
#ifndef STORAGE_STORAGE_BACKEND_BLOCKFILE_RANKINGS_H_
#define STORAGE_STORAGE_BACKEND_BLOCKFILE_RANKINGS_H_
...
class Rankings {
 public:
  ...
  bool Init(StorageBackend* backend, bool count_lists);
  ...
  void SetContents(CacheRankingsBlock* node, StorageAddr address);

 private:
  typedef std::pair<StorageAddr, CacheRankingsBlock*> IteratorPair;
  ...
  bool IsHead(StorageAddr addr, List* list) const;
  bool IsTail(StorageAddr addr, List* list) const;
...
#endif  // STORAGE_STORAGE_BACKEND_BLOCKFILE_RANKINGS_H_
