...
BackendIO::BackendIO(InFlightIO* controller, StorageBackend* backend,
...
void InFlightBackendIO::Init(const CompletionCallback& callback) {
...
void InFlightBackendIO::CloseStorageEntry(StorageEntry* entry) {
  scoped_refptr<BackendIO> operation(
      new BackendIO(this, backend_, CompletionCallback()));
