...
STORAGE_EXPORT_PRIVATE RankCrashes g_rankings_crash = NO_CRASH;
}
...
class Transaction {
 public:
  ...
  Transaction(volatile storage::LruData* data, storage::Addr addr,
              Operation op, int list);
  ~Transaction();
 private:
  volatile storage::LruData* data_;
  DISALLOW_COPY_AND_ASSIGN(Transaction);
};

Transaction::Transaction(volatile storage::LruData* data,
                         storage::Addr addr, Operation op, int list)
    : data_(data) {
...
#ifndef NDEBUG
  if (storage::NO_CRASH == storage::g_rankings_crash)
    return;
  switch (location) {
    case ON_INSERT_1:
      switch (storage::g_rankings_crash) {
        case storage::INSERT_ONE_1:
        case storage::INSERT_LOAD_1:
          TerminateSelf();
        default:
          break;
      }
      break;
    case ON_INSERT_2:
      if (storage::INSERT_EMPTY_1 == storage::g_rankings_crash)
        TerminateSelf();
      break;
    case ON_INSERT_3:
      switch (storage::g_rankings_crash) {
        case storage::INSERT_EMPTY_2:
        case storage::INSERT_ONE_2:
        case storage::INSERT_LOAD_2:
          TerminateSelf();
        default:
          break;
      }
    ...
    case ON_INSERT_4:
      switch (storage::g_rankings_crash) {
        case storage::INSERT_EMPTY_3:
        case storage::INSERT_ONE_3:
          TerminateSelf();
        default:
          break;
      }
    ...
    case ON_REMOVE_1:
      switch (storage::g_rankings_crash) {
        case storage::REMOVE_ONE_1:
        case storage::REMOVE_HEAD_1:
        case storage::REMOVE_TAIL_1:
        case storage::REMOVE_LOAD_1:
          TerminateSelf();
        default:
          break;
      }
      break;
    case ON_REMOVE_2:
      if (storage::REMOVE_ONE_2 == storage::g_rankings_crash)
        TerminateSelf();
      break;
    case ON_REMOVE_3:
      if (storage::REMOVE_ONE_3 == storage::g_rankings_crash)
        TerminateSelf();
      break;
    case ON_REMOVE_4:
      if (storage::REMOVE_HEAD_2 == storage::g_rankings_crash)
        TerminateSelf();
      break;
    case ON_REMOVE_5:
      if (storage::REMOVE_TAIL_2 == storage::g_rankings_crash)
        TerminateSelf();
      break;
    case ON_REMOVE_6:
      if (storage::REMOVE_TAIL_3 == storage::g_rankings_crash)
        TerminateSelf();
      break;
    case ON_REMOVE_7:
      switch (storage::g_rankings_crash) {
        case storage::REMOVE_ONE_4:
        case storage::REMOVE_LOAD_2:
        case storage::REMOVE_HEAD_3:
          TerminateSelf();
        default:
          break;
      }
    ...
    case ON_REMOVE_8:
      switch (storage::g_rankings_crash) {
        case storage::REMOVE_HEAD_4:
        case storage::REMOVE_LOAD_3:
          TerminateSelf();
        default:
          break;
      }
...
void UpdateTimes(storage::CacheRankingsBlock* node, bool modified) {
...
bool Rankings::Init(StorageBackend* backend, bool count_lists) {
  DCHECK(!init_);
  if (init_)
    return false;
...
void Rankings::Remove(CacheRankingsBlock* node, List list, bool strict) {
  ...
  StorageAddr node_value = node->address().value();
  Addr& my_head = heads_[list];
  Addr& my_tail = tails_[list];
...
void Rankings::SetContents(CacheRankingsBlock* node, StorageAddr address) {
...
bool Rankings::GetRanking(CacheRankingsBlock* rankings) {
  ...
  StorageEntry* entry = backend_->GetOpenEntry(rankings);
  if (!entry) {
...
void Rankings::CompleteTransaction() {
  Addr node_addr(static_cast<StorageAddr>(control_data_->transaction));
...
bool Rankings::CheckLinks(CacheRankingsBlock* node, CacheRankingsBlock* prev,
  ...
  StorageAddr node_addr = node->address().value();
...
int Rankings::CheckListSection(List list, Addr end1, Addr end2, bool forward,
  ...
  *num_items = 0;
  ...
  do {
    ...
    StorageAddr next = forward ? node->Data()->next : node->Data()->prev;
    StorageAddr prev = forward ? node->Data()->prev : node->Data()->next;
...
bool Rankings::IsHead(StorageAddr addr, List* list) const {
  for (int i = 0; i < LAST_ELEMENT; i++) {
...
bool Rankings::IsTail(StorageAddr addr, List* list) const {
  for (int i = 0; i < LAST_ELEMENT; i++) {
...
void Rankings::UpdateIterators(CacheRankingsBlock* node) {
  StorageAddr address = node->address().value();
