...
TEST(StorageCacheStatsTest, Init) {
...
TEST(StorageCacheStatsTest, InitWithEmptyBuffer) {
  disk_cache::Stats stats;
...
TEST(StorageCacheStatsTest, FailsInit) {
  disk_cache::Stats stats;
  int required_len = stats.StorageSize();
  std::unique_ptr<char[]> storage(new char[required_len]);
  memset(storage.get(), 0, required_len);
...
TEST(StorageCacheStatsTest, SaveRestore) {
