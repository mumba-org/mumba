...
int StorageBackend::Init(Manifest::InitParams params, const CompletionCallback& callback) {
...
int StorageBackend::SyncDoomAllEntries() {
  ...
  if (!num_refs_) {
    ...
    if (disabled_)
      return net::ERR_FAILED;
    DLOG(INFO) << "StorageBackend::SyncDoomAllEntries: CALLING TRIM";
...
void StorageBackend::OnSyncBackendOpComplete() {
  if (consider_evicting_at_op_end_) {
    if (data_->header.num_bytes > max_size_ && !read_only_ &&
      ...
      DLOG(INFO) << "StorageBackend::OnSyncBackendOpComplete: CALLING TRIM";
...
void StorageBackend::TrimForTest(bool empty) {
  ...
  DLOG(INFO) << "StorageBackend::TrimForTest: CALLING TRIM";
...
scoped_refptr<StorageEntry> StorageBackend::MatchEntry(const std::string& key,
  ...
  *match_error = false;
  ...
  for (;;) {
    ...
    if (cache_entry->dirty()) {
      DLOG(ERROR) << "\n\nentry '" << cache_entry->GetKey() << "' is marked DIRTY which mean it was not properly closed";
...
bool StorageBackend::InitManifest() {
  DLOG(INFO) << "StorageBackend::InitManifest";
  Addr address(data_->header.manifest);

  int size = manifest_.StorageSize();

  if (!address.is_initialized()) {
    DLOG(INFO) << "StorageBackend::InitManifest: address.is_initialized() = false. creating new manifest and persisting";
    FileType file_type = Addr::RequiredFileType(size);
    DCHECK_NE(file_type, EXTERNAL);
    int num_blocks = Addr::RequiredBlocks(size, file_type);
  ...
  DLOG(INFO) << "StorageBackend::InitManifest: address.is_initialized() = true. loading Manifest from disk";
