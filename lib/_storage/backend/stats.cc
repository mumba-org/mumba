...
const int32_t kStorageSignature = 0xF01427E0;

struct OnStorageStats {
  int32_t signature;
  int size;
  int data_sizes[storage::Stats::kDataSizesLength];
  int64_t counters[storage::Stats::MAX_COUNTER];
};
static_assert(sizeof(OnStorageStats) < 512, "needs more than 2 blocks");
...
static_assert(arraysize(kCounterNames) == storage::Stats::MAX_COUNTER,
              "update the names");
...
bool VerifyStats(OnStorageStats* stats) {
  if (stats->signature != kStorageSignature)
    return false;
  ...
  if (static_cast<unsigned int>(stats->size) > sizeof(*stats)) {
    ...
    stats->signature = kStorageSignature;
...
bool Stats::Init(void* data, int num_bytes, Addr address) {
  OnStorageStats local_stats;
  OnStorageStats* stats = &local_stats;
  if (!num_bytes) {
    memset(stats, 0, sizeof(local_stats));
    local_stats.signature = kStorageSignature;
    local_stats.size = sizeof(local_stats);
  } else if (num_bytes >= static_cast<int>(sizeof(*stats))) {
    stats = reinterpret_cast<OnStorageStats*>(data);
...
void Stats::InitSizeHistogram() {
  ...
  base::HistogramBase* stats_histogram = base::Histogram::FactoryGet(
      "StorageCache.SizeStats2", min, max, num_buckets,
...
int Stats::StorageSize() {
  // If we have more than 512 bytes of counters, change kStorageSignature so we
  // don't overwrite something else (LoadStats must fail).
  static_assert(sizeof(OnStorageStats) <= 256 * 2, "use more blocks");
  return 256 * 2;
}
...
int Stats::SerializeStats(void* data, int num_bytes, Addr* address) {
  OnStorageStats* stats = reinterpret_cast<OnStorageStats*>(data);
