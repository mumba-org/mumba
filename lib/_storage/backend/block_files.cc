...
#include "storage/backend/file_lock.h"
...
bool BlockHeader::CreateMapBlock(int64_t size, int* index) {
  DCHECK(size > 0 && size <= kMaxNumBlocks);
  int target = 0;
  for (int64_t i = size; i <= kMaxNumBlocks; i++) {
  ...
  for (int i = 0; i < header_->max_entries / 32; i++, current++) {
    ...
    for (int j = 0; j < 8; j++, map_block >>= 4) {
      ...
      *index = current * 32 + index_offset;
      ...
      LOCAL_HISTOGRAM_TIMES("StorageCache.CreateBlock", TimeTicks::Now() - start);
      return true;
    }
  }
...
void BlockHeader::DeleteMapBlock(int index, int64_t size) {
  ...
  LOCAL_HISTOGRAM_TIMES("StorageCache.DeleteBlock", TimeTicks::Now() - start);
}

// Note that this is a simplified version of DeleteMapBlock().
bool BlockHeader::UsedMapBlock(int index, int64_t size) {
  if (size < 0 || size > kMaxNumBlocks)
    return false;
...
int64_t BlockHeader::EmptyBlocks() const {
  int64_t empty_blocks = 0;
  for (int i = 0; i < kMaxNumBlocks; i++) {
    empty_blocks += header_->empty[i] * (i + 1);
...
int64_t BlockHeader::MinimumAllocations() const {
  return header_->empty[kMaxNumBlocks - 1];
}

int64_t BlockHeader::Capacity() const {
...
int64_t BlockHeader::Size() const {
  return static_cast<int64_t>(sizeof(*header_));
}
...
void BlockFiles::ReportStats() {
  ...
  UMA_HISTOGRAM_COUNTS_1M("StorageCache.Blocks_0", used_blocks[0]);
  UMA_HISTOGRAM_COUNTS_1M("StorageCache.Blocks_1", used_blocks[1]);
  UMA_HISTOGRAM_COUNTS_1M("StorageCache.Blocks_2", used_blocks[2]);
  UMA_HISTOGRAM_COUNTS_1M("StorageCache.Blocks_3", used_blocks[3]);

  UMA_HISTOGRAM_ENUMERATION("StorageCache.BlockLoad_0", load[0], 101);
  UMA_HISTOGRAM_ENUMERATION("StorageCache.BlockLoad_1", load[1], 101);
  UMA_HISTOGRAM_ENUMERATION("StorageCache.BlockLoad_2", load[2], 101);
  UMA_HISTOGRAM_ENUMERATION("StorageCache.BlockLoad_3", load[3], 101);
}
...
MappedFile* BlockFiles::FileForNewBlock(FileType block_type, int block_count) {
  ...
  LOCAL_HISTOGRAM_TIMES("StorageCache.GetFileForNewBlock",
                        TimeTicks::Now() - start);
  return file;
}
...
bool BlockFiles::RemoveEmptyFile(FileType block_type) {
  ...
  while (header->next_file) {
    ...
    if (!next_header->num_entries) {
      ...
      int failure = DeleteStorageFile(name) ? 0 : 1;
      UMA_HISTOGRAM_COUNTS_1M("StorageCache.DeleteFailed2", failure);
      if (failure)
