...
bool StorageEntry::CreateEntry(Addr node_address,
  ...
  if (entry_store->key_len > kMaxInternalKeyLength) {
    //DLOG(INFO) << "key len (" << entry_store->key_len << ") > (" << kMaxInternalKeyLength << "): alocando em um bloco. (não é bom)";
    Addr address(0);
...
bool StorageEntry::DataSanityCheck() {
  ...
  for (int i = 0; i < kNumStreams; i++) {
    ...
    if (data_size <= kMaxBlockSize && data_addr.is_separate_file()) {
      DLOG(ERROR) << "error stream " << i << " data size < " << kMaxBlockSize << " && data_addr.is_separate_file()";
      return false;
    }
    if (data_size > kMaxBlockSize && data_addr.is_block_file()) {
      DLOG(ERROR) << "error stream " << i << " data size > " << kMaxBlockSize << " && data_addr.is_block_file()";
      return false;
    }
  }
  return true;
...
void StorageEntry::Close() {
  if (background_queue_.get())
    background_queue_->CloseStorageEntry(this);
}
