...
void TorrentStorage::set_settings(lt::settings_pack const* pack) {
  DLOG(INFO) << "TorrentStorage::set_settings";
...
void TorrentStorage::async_read(
    ...
    lt::disk_job_flags_t) {
  DLOG(INFO) << "TorrentStorage::async_read";
...
bool TorrentStorage::async_write(lt::storage_index_t storage, 
    ...
    std::function<void(lt::storage_error const&)> handler,
     lt::disk_job_flags_t) {
  DLOG(INFO) << "TorrentStorage::async_write";
...
void TorrentStorage::async_hash(
  ...
  DLOG(INFO) << "TorrentStorage::async_hash";
  lt::storage_error error;
  ...
  DLOG(INFO) << "TorrentStorage::async_hash: hash for " << piece << " = " << base::HexEncode(hash_data, 20);
...
void TorrentStorage::async_move_storage(
  ...
  DLOG(INFO) << "TorrentStorage::async_move_storage";
}

void TorrentStorage::async_release_files(
    lt::storage_index_t storage, 
    std::function<void()> handler) {
  DLOG(INFO) << "TorrentStorage::async_release_files";
...
void TorrentStorage::async_delete_files(lt::storage_index_t, 
    lt::remove_flags_t, 
    std::function<void(lt::storage_error const&)> handler) {
  DLOG(INFO) << "TorrentStorage::async_delete_files";
}
...
void TorrentStorage::async_check_files(
  ...
  DLOG(INFO) << "TorrentStorage::async_check_files: opening entry '" << index << "' in sync mode..";
  if (!manager_->OpenEntry(index)) {
    if (!manager_->CreateEntry(index)) {
      DLOG(INFO) << "TorrentStorage::async_check_files: failed while creating entry";
      ret = lt::status_t::fatal_disk_error;
    } else {// else {
    //  ret = lt::status_t::file_exist;//no_error;
    //}
      DLOG(INFO) << "TorrentStorage::async_check_files: entry was created now. returning no_error";  
      ret = lt::status_t::no_error;
    }
  } else if (manager_->EntryChecked(index)) {
    DLOG(INFO) << "TorrentStorage::async_check_files: entry already checked. returning no_error";  
    ret = lt::status_t::no_error;//file_exist;
  } else {
    DLOG(INFO) << "TorrentStorage::async_check_files: entry exists, but not checked. returning need_full_check";  
    //std::vector<const char *> hashes = delegate_->GetEntryBlockHashes(index);
    //if (hashes.size() > 0) {
    //  DLOG(INFO) << "TorrentStorage::async_check_files: updating torrent info merkle tree piece values";
    //  delegate_->UpdateMerkleTree(index, hashes);
    //}
    ret = lt::status_t::need_full_check;
  }
  //for (auto file : links) {
  //  DLOG(INFO) << "file -- name: " << file;//.first << " index: " << file.second;
  //}
...
void TorrentStorage::async_rename_file(
  ...
  DLOG(INFO) << "TorrentStorage::async_rename_file";
}

void TorrentStorage::async_stop_torrent(lt::storage_index_t, std::function<void()> handler) {
  DLOG(INFO) << "TorrentStorage::async_stop_torrent";
  if (!handler) {
    return;
  }
  post(io_context_, std::move(handler));
...
void TorrentStorage::async_set_file_priority(
  ...
  DLOG(INFO) << "TorrentStorage::async_set_file_priority";
}
