...
void DataCatalog::Close(bool sync) {
  //DLOG(INFO) << "DataCatalog::Close";
  if (db_task_runner_->BelongsToCurrentThread()) {
    CloseImpl(nullptr);
  } else {
    //DLOG(INFO) << "DataCatalog::Close: posting to db thread.. sync ? " << sync;
    if (sync) {
      base::WaitableEvent do_sync{base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED};
      db_task_runner_->PostTask(
          FROM_HERE, 
          base::Bind(&DataCatalog::CloseImpl, 
            base::Unretained(this), 
            base::Unretained(&do_sync)));
      //DLOG(INFO) << "DataCatalog::Close: waiting on db thread... base::WaitableEvent = " << &do_sync;
      do_sync.Wait(); 
      //DLOG(INFO) << "DataCatalog::Close: end waiting on db thread";
    } else {
      db_task_runner_->PostTask(
          FROM_HERE,
          base::Bind(&DataCatalog::CloseImpl, 
...
std::unique_ptr<Block> DataCatalog::Scan(const zetasql::ResolvedQueryStmt* stmt) {
  ...
  for (auto const* scan : scan_nodes) {
    ////DLOG(INFO) << "iterating over scan node '" << scan->node_kind_string() << "'";
  ...
  if (!table_scan || !project_scan) {
    //DLOG(INFO) << "theres no table or project scan on query. aborting";
    return {};
  }
  ...
  while (cursor->IsValid()) { // for each row
    ...
    if (!table_message->ParseFromArray(kv.second.data(), kv.second.size())) {
      //DLOG(INFO) << "oops. problem parsing row. raw data (" << kv.second.size() << "):\n'" << kv.second.as_string() << "'";
    ...
    for (auto const& column : column_list) {
      ...
      if (!proto_field) { 
        //DLOG(INFO) << "oops. " << column.name() << " was not found in the descriptor";
        cursor->Next();
        continue;
      }
...
bool DataCatalog::LoadMetatables() {
  ...
  ////DLOG(INFO) << "getting 'catalog.proto' at table " << kMetaTableIndex;
...
void DataCatalog::CloseImpl(base::WaitableEvent* do_sync) {
  //DLOG(INFO) << "DataCatalog::CloseImpl: base::WaitableEvent = " << do_sync;
  if (torrent_->db_is_open()) {
    //DLOG(INFO) << "DataCatalog::CloseImpl: calling database_->Close()";
    torrent_->db().Close();
    opened_ = false;
    //DLOG(INFO) << "DataCatalog::CloseImpl: calling database_->Close() end";
  }
  if (do_sync) {
    //DLOG(INFO) << "DataCatalog::CloseImpl: sending signal";
    do_sync->Signal();
  }
}

void DataCatalog::OnInfoHeaderChanged(const storage_proto::Info& info) {
  //DLOG(INFO) << "DataCatalog::OnInfoHeaderChanged. see if we can update the info directly from torrent now..";
