...
void IOCompletionCallbackBaseInternal::DidSetResult() {
  DLOG(INFO) << "IOCompletionCallbackBaseInternal::DidSetResult";
  have_result_ = true;
  if (event_)//run_loop_)
    event_->Signal();
    //run_loop_->Quit();
  DLOG(INFO) << "IOCompletionCallbackBaseInternal::DidSetResult end";
}

void IOCompletionCallbackBaseInternal::WaitForResult() {
  //DCHECK(!run_loop_);
  DLOG(INFO) << "IOCompletionCallbackBaseInternal::WaitForResult";
  DCHECK(!event_);
  if (!have_result_) {
  ...
  DLOG(INFO) << "IOCompletionCallbackBaseInternal::WaitForResult end";
}
