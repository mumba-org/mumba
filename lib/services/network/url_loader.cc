...
void URLLoader::PauseReadingBodyFromNet() {
  DLOG(INFO) << "URLLoader::PauseReadingBodyFromNet()";
...
void URLLoader::ResumeReadingBodyFromNet() {
  DLOG(INFO) << "URLLoader::ResumeReadingBodyFromNet()";
...
void URLLoader::ResumeStart() {
  DLOG(INFO) << "URLLoader::ResumeStart";
  url_request_->LogUnblocked();
  url_request_->Start();
}

void URLLoader::OnResponseStarted(net::URLRequest* url_request, int net_error) {
  DLOG(INFO) << "URLLoader::OnResponseStarted";
  DCHECK(url_request == url_request_.get());

  if (net_error != net::OK) {
    DLOG(INFO) << "URLLoader::OnResponseStarted: calling NotifyCompleted()";
  ...
  if (!(options_ & mojom::kURLLoadOptionSniffMimeType) || 
    ...
    DLOG(INFO) << "URLLoader::OnResponseStarted: calling SendResponseToClient()";
...
void URLLoader::ReadMore() {
  ...
  if (!pending_write_.get()) {
    ...
    if (result != MOJO_RESULT_OK && result != MOJO_RESULT_SHOULD_WAIT) {
      ...
      DLOG(INFO) << "URLLoader::ReadMore: NotifyCompleted(net::ERR_FAILED)";
...
void URLLoader::DidRead(int num_bytes, bool completed_synchronously) {
  DLOG(INFO) << "URLLoader::DidRead: bytes = " << num_bytes;
  if (num_bytes > 0) {
  ...
  if (consumer_handle_.is_valid()) {
    ...
    if (made_final_decision) {
      DLOG(INFO) << "URLLoader::DidRead: net::SniffMimeType() = true => SendResponseToClient()";
      SendResponseToClient();
    } else {
      complete_read = false;
    }
  ...
  if (!url_request_->status().is_success() || num_bytes == 0) {
    DLOG(INFO) << "URLLoader::DidRead: CompletePendingWrite(). num_byte = " << num_bytes;
  ...
  if (complete_read) {
    DLOG(INFO) << "URLLoader::DidRead: complete_read = true calling CompletePendingWrite()";
    CompletePendingWrite();
  }
...
void URLLoader::OnReadCompleted(net::URLRequest* url_request, int bytes_read) {
  DLOG(INFO) << "URLLoader::OnReadCompleted: bytes = " << bytes_read;
  DCHECK(url_request == url_request_.get());
...
void URLLoader::NotifyCompleted(int error_code) {
  DLOG(INFO) << "URLLoader::NotifyCompleted: err_code = " << error_code;
  ...
  if (consumer_handle_.is_valid()) {
    DLOG(INFO) << "URLLoader::NotifyCompleted: consumer_handle_.is_valid() = true. calling SendResponseToClient()";
    SendResponseToClient();
  }
  ...
  DLOG(INFO) << "URLLoader::NotifyCompleted: url_loader_client_->OnComplete(status)";
  url_loader_client_->OnComplete(status);
  DeleteSelf();
}

void URLLoader::OnConnectionError() {
  DLOG(INFO) << "URLLoader::OnConnectionError";
  NotifyCompleted(net::ERR_FAILED);
}

void URLLoader::OnResponseBodyStreamConsumerClosed(MojoResult result) {
  DLOG(INFO) << "URLLoader::OnResponseBodyStreamConsumerClosed";
  NotifyCompleted(net::ERR_FAILED);
}

void URLLoader::OnResponseBodyStreamReady(MojoResult result) {
  DLOG(INFO) << "URLLoader::OnResponseBodyStreamReady";
...
void URLLoader::SendResponseToClient() {
  DLOG(INFO) << "URLLoader::SendResponseToClient";
  base::Optional<net::SSLInfo> ssl_info;
  mojom::DownloadedTempFilePtr downloaded_file_ptr;

  DLOG(INFO) << "URLLoader::SendResponseToClient: calling url_loader_client_->OnReceiveResponse()";
  ...
  DLOG(INFO) << "URLLoader::SendResponseToClient: url_loader_client_->OnStartLoadingResponseBody()";
  url_loader_client_->OnStartLoadingResponseBody(std::move(consumer_handle_));
  response_ = nullptr;
}

void URLLoader::CompletePendingWrite() {
  DLOG(INFO) << "URLLoader::CompletePendingWrite";
...
void URLLoader::SendUploadProgress(const net::UploadProgress& progress) {
  DLOG(INFO) << "URLLoader::SendUploadProgress";
