 ...
 private:
  ...
  void AdjustCommandLineArgumentsForTarget(
    ...
    if (main_delegate_->ShouldLaunchAsServiceProcess(target)) {
      command_line->AppendSwitch(switches::kProcessTypeService);
...
// handlers so we can get a stack trace when crashing.
  ...
  if (process_type == ProcessType::kDefault) {
    if (command_line.HasSwitch(switches::kProcessTypeServiceManager)) {
      mojo_config.is_broker_process = true;
      process_type = ProcessType::kServiceManager;
    } else if (command_line.HasSwitch(switches::kProcessTypeService)) {
