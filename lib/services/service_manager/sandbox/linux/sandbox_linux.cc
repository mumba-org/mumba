...
bool UpdateProcessTypeAndEnableSandbox(
  ...
  DLOG(INFO) << "UpdateProcessTypeAndEnableSandbox: we need to set this up..";
  base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
  std::string process_type;
  if (command_line->HasSwitch(switches::kGpuProcess)) {
    process_type = "gpu";
  } else if (command_line->HasSwitch(switches::kShellProcess)) {
    process_type = "shell";
  ...
  command_line->AppendSwitch(process_type.append("-broker"));
      //switches::kProcessType,
...
void SandboxLinux::EngageNamespaceSandbox(bool from_zygote) {
  DLOG(INFO) << "SandboxLinux::EngageNamespaceSandbox";
  CHECK(pre_initialized_);
  if (from_zygote) {
  ...
  //DLOG(INFO) << "DontDropFileSystemAccess";
  CHECK(sandbox::Credentials::DontDropFileSystemAccess(proc_fd_));
...
bool SandboxLinux::InitializeSandbox(SandboxType sandbox_type,
  ...
  if (!IsSingleThreaded()) {
    ...
    if (base::SysInfo::IsRunningOnChromeOS() &&
        command_line->HasSwitch(switches::kGpuProcess)) {
    ...
    bool sandbox_failure_fatal = !command_line->HasSwitch(switches::kGpuProcess);
...
bool SandboxLinux::LimitAddressSpace(const Options& options) {
...
void SandboxLinux::CheckForBrokenPromises(SandboxType sandbox_type) {
  if (sandbox_type != SANDBOX_TYPE_APPLICATION &&
      sandbox_type != SANDBOX_TYPE_SHELL) {
    return;
  }
