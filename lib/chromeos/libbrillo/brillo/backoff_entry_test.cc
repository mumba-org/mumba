...
TEST(BackoffEntryTest, CanDiscardNeverExpires) {
  ...
  never_expires.set_now(TimeTicks() + base::Days(100));
  EXPECT_FALSE(never_expires.CanDiscard());
}
...
TEST(BackoffEntryTest, CanDiscard) {
  ...
  entry.set_now(entry.GetReleaseTime() + base::Milliseconds(1));
  EXPECT_FALSE(entry.CanDiscard());
...
TEST(BackoffEntryTest, CanDiscardAlwaysDelay) {
  ...
  // Because lifetime is non-zero, we shouldn't be able to discard yet.
  entry.set_now(entry.GetReleaseTime() + base::Milliseconds(2000));
  EXPECT_TRUE(entry.CanDiscard());
  ...
  entry.set_now(entry.GetReleaseTime() + base::Milliseconds(1000));
  EXPECT_TRUE(entry.CanDiscard());
}
...
TEST(BackoffEntryTest, ReleaseTimeCalculation) {
  ...
  EXPECT_EQ(entry.ImplGetTimeNow() + base::Milliseconds(1000), result);
  EXPECT_EQ(base::Milliseconds(1000), entry.GetTimeUntilRelease());

  // 2 errors.
  entry.InformOfRequest(false);
  result = entry.GetReleaseTime();
  EXPECT_EQ(entry.ImplGetTimeNow() + base::Milliseconds(2000), result);
  EXPECT_EQ(base::Milliseconds(2000), entry.GetTimeUntilRelease());

  // 3 errors.
  entry.InformOfRequest(false);
  result = entry.GetReleaseTime();
  EXPECT_EQ(entry.ImplGetTimeNow() + base::Milliseconds(4000), result);
  ...
  entry.InformOfRequest(false);
  result = entry.GetReleaseTime();
  EXPECT_EQ(entry.ImplGetTimeNow() + base::Milliseconds(20000), result);
}
...
TEST(BackoffEntryTest, ReleaseTimeCalculationWithJitter) {
  for (int i = 0; i < 10; ++i) {
    ...
    entry.InformOfRequest(false);
    TimeTicks result = entry.GetReleaseTime();
    EXPECT_LE(entry.ImplGetTimeNow() + base::Milliseconds(3200), result);
    EXPECT_GE(entry.ImplGetTimeNow() + base::Milliseconds(4000), result);
  }
}
...
TEST(BackoffEntryTest, FailureThenSuccess) {
  ...
  entry.set_now(release_time - base::Milliseconds(200));
  entry.InformOfRequest(true);
...
TEST(BackoffEntryTest, RetainCustomHorizon) {
  ...
  custom.set_now(TimeTicks() + base::Days(2));
  custom.InformOfRequest(false);
  custom.InformOfRequest(true);
  ...
  custom.set_now(TimeTicks() + base::Days(3));
  custom.InformOfRequest(false);
