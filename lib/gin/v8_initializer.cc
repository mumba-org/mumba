...
//base::MemoryMappedFile* g_mapped_extras = nullptr;
...
//const char kExtrasFileName[] = "libraries_extras.bin";
...
void V8Initializer::Initialize(IsolateHolder::ScriptMode mode,
  ...
  //v8::StartupData extras;
  //GetMappedFileData(g_mapped_extras, &extras);
  //v8::V8::SetExtrasDataBlob(&extras);
...
void V8Initializer::GetV8ExternalSnapshotData(const char** natives_data_out,
  ...
  v8::StartupData extras;
  v8::StartupData snapshot;
  GetV8ExternalSnapshotData(&natives, &extras, &snapshot);
  ...
  *snapshot_size_out = snapshot.raw_size;
  //*extras_data_out = extras.data;
  //*extras_size_out = extras.raw_size;
}
...
void V8Initializer::LoadV8Natives() {
  ...
  // LoadV8FileResult extra_result = MapOpenedFile(GetOpenedFile(kExtrasFileName),
  //                                         &g_mapped_extras);
  // if (extra_result != V8_LOAD_SUCCESS) {
  //   LOG(FATAL) << "Couldn't mmap v8 extras data file, status code is "
  //              << static_cast<int>(extra_result);
  // }
}
