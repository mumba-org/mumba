...
//        input.copyBytes(to: pointer + position, from: aOffset..<aOffset + written)
    ...
    mutating func flushToData(data: inout Data) -> Int {
        var totalWritten:Int = 0
        

        if tail > position {
            
            let written: Int = buffer.withUnsafeBufferPointer {
                let len = buffer.count - tail
                data.append($0.baseAddress! + tail, count: len)
                return len
            }
            if written <= 0 {
                return totalWritten
