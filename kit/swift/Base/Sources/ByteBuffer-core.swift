...
@inlinable func _toCapacity(_ value: Int) -> ByteBuffer._apacity {
    return ByteBuffer._apacity(truncatingIfNeeded: value)
}
...
public struct ByteBuffer {
    ...
    public typealias _apacity = UInt32
    ...
    @usableFromInline final class _Storage {
        private(set) var capacity: _apacity
        @usableFromInline private(set) var bytes: UnsafeMutableRawPointer
        private let allocator: ByteBufferAllocator

        public init(bytesNoCopy: UnsafeMutableRawPointer, capacity: _apacity, allocator: ByteBufferAllocator) {
        ...
        private static func allocateAndPrepareRawMemory(bytes: _apacity, allocator: Allocator) -> UnsafeMutableRawPointer {
        ...
        private func allocateStorage(capacity: _apacity) -> _Storage {
        ...
        public func reallocSlice(_ slice: Range<ByteBuffer._Index>, capacity: _apacity) -> _Storage {
        ...
        public func reallocStorage(capacity minimumNeededCapacity: _apacity) {
        ...
        public static func reallocated(minimumCapacity: _apacity, allocator: Allocator) -> _Storage {
    ...
    private mutating func _copyStorageAndRebase(capacity: _apacity, resetIndices: Bool = false) {
    ...
    @usableFromInline mutating func _copyStorageAndRebase(extraCapacity: _apacity = 0, resetIndices: Bool = false) {
        self._copyStorageAndRebase(capacity: _toCapacity(self._slice.count) + extraCapacity, resetIndices: resetIndices)
    }

    @usableFromInline mutating func _ensureAvailableCapacity(_ capacity: _apacity, at index: _Index) {
    ...
    @inlinable
    mutating func _set<S: ContiguousCollection>(bytes: S, at index: _Index) -> _apacity where S.Element == UInt8 {
        ...
        self._ensureAvailableCapacity(_apacity(bytesCount), at: index)
    ...
    @inlinable
    mutating func _set<S: Sequence>(bytes: S, at index: _Index) -> _apacity where S.Element == UInt8 {
        ...
        func ensureCapacityAndReturnStorageBase(capacity: Int) -> UnsafeMutablePointer<UInt8> {
            self._ensureAvailableCapacity(_apacity(capacity), at: index)
