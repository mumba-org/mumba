...
public final class Lock {
    ...
    static var handlesMultipleThreadPriorities: Bool {
    ...
    // no debug version. TODO: implement the debug one
    public func assertAcquired() {}
}
...
public final class ConditionVariable {
  ...
  private let condition: UnsafeMutablePointer<pthread_cond_t> = UnsafeMutablePointer.allocate(capacity: 1)
  private let userMutex: UnsafeMutablePointer<pthread_mutex_t>

  public init(lock: Lock) {
    userMutex = lock.mutex

    let attrs: UnsafeMutablePointer<pthread_condattr_t> = UnsafeMutablePointer.allocate(capacity: 1)
    var err = pthread_condattr_init(attrs)
    precondition(err == 0)

    pthread_condattr_setclock(attrs, CLOCK_MONOTONIC)
    err = pthread_cond_init(self.condition, attrs)
  ...
  deinit {
    let err = pthread_cond_destroy(self.condition)
    precondition(err == 0)
    self.condition.deallocate()
  }

  public func wait() {
    let err = pthread_cond_wait(self.condition, self.userMutex)
    precondition(err == 0)
  }

  public func timedWait(_ time: TimeDelta) {
    let usecs: Int64 =  Int64(time.microseconds)

    let relativeTime = 
      timespec(tv_sec:  __time_t(usecs / Int64(Time.MicrosecondsPerSecond)),
               tv_nsec: Int((usecs % Int64(Time.MicrosecondsPerSecond)) * Int64(Time.NanosecondsPerMicrosecond)))

    var now = timespec()
    clock_gettime(CLOCK_MONOTONIC, &now)

    var absoluteTime = timespec(tv_sec: now.tv_sec, tv_nsec: now.tv_nsec)
    absoluteTime.tv_sec += relativeTime.tv_sec
    absoluteTime.tv_nsec += relativeTime.tv_nsec
    absoluteTime.tv_sec += absoluteTime.tv_nsec / Int(Time.NanosecondsPerSecond)
    absoluteTime.tv_nsec %= Int(Time.NanosecondsPerSecond)
    let err = pthread_cond_timedwait(self.condition, self.userMutex, &absoluteTime)
    precondition(err == 0 || err == ETIMEDOUT)
  }

  public func broadcast() {
    let err = pthread_cond_broadcast(self.condition)
    precondition(err == 0)
  }
  
  public func signal() {
    let err = pthread_cond_signal(self.condition)
    precondition(err == 0)
  }
