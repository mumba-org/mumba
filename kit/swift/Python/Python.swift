...
public typealias PyObjectPointer = UnsafeMutablePointer<PyObject> //UnsafeMutableRawPointer
...
let falseStructType = UnsafeMutableRawPointer(&_Py_FalseStruct)
let trueStructType = UnsafeMutableRawPointer(&_Py_TrueStruct)
let Py_False = falseStructType.bindMemory(to: PyObject.self, capacity: 1)
let Py_True = trueStructType.bindMemory(to: PyObject.self, capacity: 1)
...
public struct PythonEnvironment {
    public var libraryPath: String = String()
    public init() {}
}
...
public struct PythonInterface {
    ...
    public init(environment: PythonEnvironment = PythonEnvironment()) {
        var config = PyConfig()
        PyConfig_InitIsolatedConfig(&config)
        environment.libraryPath.utf8CString.withUnsafeBufferPointer {
          var len: Int = 0
          let unicodeObj = PyUnicode_DecodeUTF8($0.baseAddress, $0.count - 1, nil)!
          let widePath = PyUnicode_AsWideCharString(unicodeObj, &len)
          Py_SetPythonHome(widePath)
          Py_SetPath(widePath)
          Py_DecRef(unicodeObj)
        }
        Py_InitializeFromConfig(&config)   // Initialize Python
...
private func isType(_ object: PythonObject,
    ...
    let result = PyObject_IsInstance(object.ownedPyObject, typePyRef.ownedPyObject)
    ...
    return pyObject != Py_False//_Py_ZeroStruct
}

extension Bool : PythonConvertible, ConvertibleFromPython {
    public init?(_ pythonObject: PythonObject) {
      //(PyObject*)&PyGen_Type
        let boolType = UnsafeMutableRawPointer(&PyBool_Type)
        guard isType(pythonObject, type: boolType.bindMemory(to: PyObject.self, capacity: 1)) else { return nil }
        
        let pyObject = pythonObject.ownedPyObject
        defer { Py_DecRef(pyObject) }
        
        self = pyObject == Py_True
    }
...
extension UInt : PythonConvertible, ConvertibleFromPython {
    ...
    public var pythonObject: PythonObject {
        //_ = Python // Ensure Python is initialized.
        return PythonObject(consuming: PyLong_FromSize_t(Int(self)))
    }
}
...
extension Range : ConvertibleFromPython where Bound : ConvertibleFromPython {
    public init?(_ pythonObject: PythonObject) {
        let sliceType = UnsafeMutableRawPointer(&PySlice_Type)
        guard isType(pythonObject, type: sliceType.bindMemory(to: PyObject.self, capacity: 1)) else { return nil }
