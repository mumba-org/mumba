...
private enum EventPollLifecycleState {
    case open
    case closing
    case closed
}
...
private extension timespec {
    init(timeDelta delta: TimeDelta) {
        let ns = delta.nanoseconds
        let sec = ns / Time.NanosecondsPerSecond
        self = timespec(tv_sec: Int(sec), tv_nsec: Int(ns - sec * Time.NanosecondsPerSecond))
    }
}
...
/// Represents IO events NIO might be interested in. `PollerEventSet` is used for two purposes:
...
public struct EventPollEventSet: OptionSet, Equatable {
    ...
    static let _none = EventPollEventSet(rawValue: 0)

    /// Connection reset or other errors.
    static let reset = EventPollEventSet(rawValue: 1 << 0)

    /// EOF at the read/input end of a `Selectable`.
    static let readEOF = EventPollEventSet(rawValue: 1 << 1)

    /// Interest in/availability of data to be read
    static let read = EventPollEventSet(rawValue: 1 << 2)

    /// Interest in/availability of data to be written
    static let write = EventPollEventSet(rawValue: 1 << 3)

    public init(rawValue: EventPollEventSet.RawValue) {
...
extension KQueueEventFilterSet {
    /// Convert NIO's `PollerEventSet` set to a `KQueueEventFilterSet`
    init(selectorEventSet: EventPollEventSet) {
...
extension EpollFilterSet {
    /// Convert NIO's `PollerEventSet` set to a `EpollFilterSet`
    init(selectorEventSet: EventPollEventSet) {
...
extension EventPollEventSet {
    ...
    fileprivate init(epollEvent: Epoll.epoll_event) {
        var selectorEventSet: EventPollEventSet = ._none
...
public protocol Registration {
    /// The `PollerEventSet` in which the `Registration` is interested.
    var interested: EventPollEventSet { get set }
}

/// Internal representation of a `Registration` to an `Poller`.
///
/// Whenever a `Selectable` is registered to a `Poller` a `Registration` is created internally that is also provided within the
/// `PollerEvent` that is provided to the user when an event is ready to be consumed for a `Selectable`. As we need to have access to the `ServerSocketChannel`
/// and `SocketChannel` (to dispatch the events) we create our own `Registration` that holds a reference to these.
public enum PlatformRegistration: Registration {
    case serverSocket(PlatformWatchable, EventPollEventSet)
    case clientSocket(PlatformWatchable, EventPollEventSet)
    case datagramSocket(PlatformWatchable, EventPollEventSet)

    /// The `PollerEventSet` in which this `NPlatformRegistration` is interested in.
    public var interested: EventPollEventSet {
        set {
            switch self {
            case .serverSocket(let d, _):
                self = .serverSocket(d, newValue)
            case .clientSocket(let d, _):
                self = .clientSocket(d, newValue)
            case .datagramSocket(let d, _):
                self = .datagramSocket(d, newValue)
            }
        }
        get {
            switch self {
            case .serverSocket(_, let i):
                return i
            case .clientSocket(_, let i):
                return i
            case .datagramSocket(_, let i):
                return i
            }
        }
    }
...
///  A `Poller` allows a user to register different `Selectable` sources to an underlying OS selector, and for that selector to notify them once IO is ready for them to process.
///
...
public final class PlatformEventPoll<R: Registration> {
    private var lifecycleState: EventPollLifecycleState
    ...
    private func growEventArrayIfNeeded(ready: Int) {
        ...
        PlatformEventPoll.deallocateEventsArray(events: events, capacity: eventsCapacity)

        // double capacity
        eventsCapacity = ready << 1
        events = PlatformEventPoll.allocateEventsArray(capacity: eventsCapacity)
    }
    ...
    public init() throws {
        ...
        ev.events = EventPollEventSet.read.epollEventSet
        ev.data.fd = eventfd
        ...
        event.data = 0
        event.udata = nil
        event.flags = UInt16(EV_ADD | EV_ENABLE | EV_LEAR)
    ...
    deinit {
        assert(self.lifecycleState == .closed, "Poller \(self.lifecycleState) (expected .closed) on deinit")
        PlatformEventPoll.deallocateEventsArray(events: events, capacity: eventsCapacity)
...
#if os(Linux)
        try! Posix.close(self.eventfd)
#else
        try! Posix.close(self.fd)
#endif
    }


#if os(macOS) || os(iOS) || os(tvOS) || os(watchOS)
    private func toKQueueTimeSpec(strategy: EventPollStrategy) -> timespec? {
        switch strategy {
        case .block:
            return nil
        case .now:
        ...
        case .blockUntilTimeout(let nanoseconds):
            return timespec(timeDelta: nanoseconds)
        }
    }
    ...
    private func kqueueApplyEventChangeSet(keventBuffer: UnsafeMutableBufferPointer<kevent>) throws {
        ...
        } catch let err as SystemError.IOError {
    ...
    private func kqueueUpdateEventNotifications<W: PlatformWatchable>(watchable: W, interested: PollerEventSet, oldInterested: EventPollEventSet?) throws {
    ...
    /// Register `Selectable` on the `Poller`.
    ///
    /// - parameters:
    ///     - selectable: The `Selectable` to register.
    ///     - interested: The `PollerEventSet` in which we are interested and want to be notified about.
    ///     - makeRegistration: Creates the registration data for the given `PollerEventSet`.
    public func register<W: PlatformWatchable>(watchable: W, interested: EventPollEventSet, makeRegistration: (EventPollEventSet) -> R) throws {
        assert(interested.contains(.reset))
        guard self.lifecycleState == .open else {
            throw SystemError.IOError(code: EBADF, reason: "can't register on selector as it's \(self.lifecycleState).")
        }

        try watchable.withUnsafeFileDescriptor { fd in
            ...
            #else
                try kqueueUpdateEventNotifications(selectable: watchable, interested: interested, oldInterested: nil)
            #endif
    ...
    ///     - interested: The `PollerEventSet` in which we are interested and want to be notified about.
    public func reregister<W: PlatformWatchable>(watchable: W, interested: EventPollEventSet) throws {
        guard self.lifecycleState == .open else {
            throw SystemError.IOError(code: EBADF, reason: "can't re-register on selector as it's \(self.lifecycleState).")
        }
        ...
        try watchable.withUnsafeFileDescriptor { fd in
            ...
            #else
                try kqueueUpdateEventNotifications(selectable: watchable, interested: interested, oldInterested: reg.interested)
            #endif
    ...
    ///
    /// After the `Selectable is deregistered no `PollerEventSet` will be produced anymore for the `Selectable`.
    ///
    /// - parameters:
    ///     - selectable: The `Selectable` to deregister.
    public func deregister<W: PlatformWatchable>(watchable: W) throws {
        guard self.lifecycleState == .open else {
            throw SystemError.IOError(code: EBADF, reason: "can't deregister from selector as it's \(self.lifecycleState).")
        }
        ...
        try watchable.withUnsafeFileDescriptor { fd in
            ...
            #else
                try kqueueUpdateEventNotifications(selectable: watchable, interested: .reset, oldInterested: reg.interested)
            #endif
        }
    }

    /// Apply the given `PollerStrategy` and execute `fn` once it's complete (which may produce `PollerEvent`s to handle).
    ///
    /// - parameters:
    ///     - strategy: The `PollerStrategy` to apply
    ///     - body: The function to execute for each `PollerEvent` that was produced.
    public func whenReady(strategy: EventPollStrategy, _ body: (EventPollEvent<R>) throws -> Void) throws -> Void {
        guard self.lifecycleState == .open else {
            throw SystemError.IOError(code: EBADF, reason: "can't call whenReady for selector as it's \(self.lifecycleState).")
        }
        ...
        case .blockUntilTimeout(let timeDelta):
            ...
            // expensive.
            //let next = DispatchTime.now().uptimeNanoseconds + UInt64(timeDelta.nanoseconds)
            let next = UInt64(TimeTicks.now.microseconds * Time.NanosecondsPerMicrosecond) + UInt64(timeDelta.nanoseconds)
            if next < self.earliestTimer {
                self.earliestTimer = next

                var ts = itimerspec()
                ts.it_value = timespec(timeDelta: timeDelta)
        ...
        for i in 0..<ready where !self.deregistrationsHappened {
            ...
            default:
                ...
                if let registration = registrations[Int(ev.data.fd)] {
                    var selectorEvent = EventPollEventSet(epollEvent: ev)
                    ...
                    try body((EventPollEvent(io: selectorEvent, registration: registration)))
                }
            }
        }
        ...
        for i in 0..<ready where !self.deregistrationsHappened {
            ...
            guard Int32(ev.flags) & EV_ERROR == 0 else {
                throw SystemError.IOError(code: Int32(ev.data), reason: "kevent returned with EV_ERROR set: \(String(describing: ev))")
            }
            guard filter != EVFILT_USER, let registration = registrations[Int(ev.ident)] else {
                continue
            }
            var selectorEvent: PollerEventSet = ._none
            switch filter {
            case EVFILT_READ:
            ...
            try body((PollerEvent(io: selectorEvent, registration: registration)))
        }
    ...
    /// Close the `Poller`.
    ///
    /// After closing the `Poller` it's no longer possible to use it.
    public func close() throws {
        guard self.lifecycleState == .open else {
            throw SystemError.IOError(code: EBADF, reason: "can't close selector as it's \(self.lifecycleState).")
        }
        self.lifecycleState = .closed
...
extension PlatformEventPoll: CustomStringConvertible {
    public var description: String {
        return "Poller { descriptor = \(self.fd) }"
    }
}

/// An event that is triggered once the `Poller` was able to select something.
public struct EventPollEvent<R> {
    public let registration: R
    public let io: EventPollEventSet

    /// Create new instance
    ///
    /// - parameters:
    ///     - io: The `PollerEventSet` that triggered this event.
    ///     - registration: The registration that belongs to the event.
    init(io: EventPollEventSet, registration: R) {
        self.io = io
        self.registration = registration
    }
}

// internal extension Poller where R == NPlatformRegistration {
//     /// Gently close the `Poller` after all registered `Channel`s are closed.
//     func closeGently(eventLoop: EventLoop) -> EventLoopFuture<Void> {
//         guard self.lifecycleState == .open else {
//             return eventLoop.newFailedFuture(error: SystemError.IOError(code: EBADF, reason: "can't close selector gently as it's \(self.lifecycleState)."))
//         }

//         let futures: [EventLoopFuture<Void>] = self.registrations.map { (_, reg: NPlatformRegistration) -> EventLoopFuture<Void> in
...
/// The strategy used for the `Poller`.
public enum EventPollStrategy {
    /// Block until there is some IO ready to be processed or the `Poller` is explicitly woken up.
    case block

    /// Block until there is some IO ready to be processed, the `Poller` is explicitly woken up or the given `TimeAmount` elapsed.
    case blockUntilTimeout(TimeDelta)
