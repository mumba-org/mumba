...
public protocol ServiceClient: class {
  var channel: RpcChannel { get }

  /// This metadata will be sent with all requests.
  var metadata: RpcMetadata { get set }
...
open class ServiceClientBase: ServiceClient {
  public let channel: RpcChannel

  public var metadata: RpcMetadata
  ...
  /// Create a client.
  required public init(address: String, secure: Bool = true, arguments: [RpcChannel.Argument] = []) {
    //gRPC.initialize()
    channel = RpcChannel(address: address, secure: secure, arguments: arguments)
    metadata = RpcMetadata()
  }

  /// Create a client using a pre-defined channel.
  required public init(channel: RpcChannel) {
  ...
  required public init(googleAPI: String, arguments: [RpcChannel.Argument] = []) {
    //gRPC.initialize()
    ...
    channel = RpcChannel(googleAddress: address, arguments: arguments)
    metadata = RpcMetadata()
  }

  /// Create a client that makes secure connections with a custom certificate.
  required public init(address: String, certificates: String, clientCertificates: String? = nil, clientKey: String? = nil, arguments: [RpcChannel.Argument] = []) {
    //gRPC.initialize()
    channel = RpcChannel(address: address, certificates: certificates, clientCertificates: clientCertificates, clientKey: clientKey, arguments: arguments)
    metadata = RpcMetadata()
  }
}
...
open class ServiceClientTestStubBase: ServiceClient {
  open var channel: RpcChannel { fatalError("not implemented") }
  open var metadata = RpcMetadata()
  open var host = ""
