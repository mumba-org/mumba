...
public struct TimeLimit: Equatable, CustomStringConvertible {
  ...
  private enum Wrapped: Equatable {
    case none
    case timeout(TimeDelta)
    case deadline(TimeDelta)
  }
  ...
  public static func timeout(_ timeout: TimeDelta) -> TimeLimit {
  ...
  /// deadline will result in the RPC being cancelled.
  public static func deadline(_ deadline: TimeDelta) -> TimeLimit {
...
extension TimeLimit {
  ...
  @usableFromInline
  internal func makeDeadline() -> TimeDelta {
    switch self.wrapped {
    case .none:
      return .max
  ...
  public var description: String {
    ...
    case let .deadline(deadline) where deadline == .max:
      return "deadline=.distantFuture"

    case let .deadline(deadline):
      return "deadline=\(deadline.nanoseconds) uptimeNanoseconds"    
