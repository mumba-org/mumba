...
class RpcOperationGroup {
  ...
  let completion: ((RpcOperationGroup) -> Void)?

  /// Indicates that the RpcOperationGroup completed successfully
  var success = false
  
  fileprivate var cachedInitialMetadata: RpcMetadata?
  fileprivate var cachedTrailingMetadata: RpcMetadata?

  /// Creates the underlying observer needed to run an operation
  ///
  /// - Parameter: operation: the operation to observe
  /// - Returns: the observer
  private func underlyingObserverForOperation(operation: RpcOperation) throws -> UnsafeMutableRawPointer {
  ...
  /// Initializes an RpcOperationGroup representation
  ///
  /// - Parameter operations: an array of operations
  init(call: RpcCall,
       operations: [RpcOperation],
       completion: ((RpcOperationGroup) -> Void)? = nil) throws {
    self.call = call
    self.operations = operations
    self.completion = completion
    // set tag to a unique value (per execution)
    RpcOperationGroup.tagMutex.lock()
    tag = RpcOperationGroup.nextTag
    RpcOperationGroup.nextTag += 1
    RpcOperationGroup.tagMutex.unlock()
  ...
  ///
  /// - Returns: metadata
  func receivedInitialMetadata() -> RpcMetadata? {
    ...
    for (i, operation) in operations.enumerated() {
      switch operation {
      case .receiveInitialMetadata:
        cachedInitialMetadata = RpcMetadata(
  ...
  ///
  /// - Returns: metadata
  func receivedTrailingMetadata() -> RpcMetadata? {
    ...
    for (i, operation) in operations.enumerated() {
      switch operation {
      case .receiveStatusOnClient:
        cachedTrailingMetadata = RpcMetadata(
