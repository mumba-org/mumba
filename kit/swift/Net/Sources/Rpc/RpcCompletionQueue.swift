...
class CompletionQueue {
  ...
  /// RpcOperation groups that are awaiting completion, keyed by tag
  private var operationGroups: [Int: RpcOperationGroup] = [:]
  ...
  deinit {
    ...
    cgrpc_completion_queue_destroy(underlyingCompletionQueue)
  }
  ...
  func register(_ operationGroup: RpcOperationGroup, onSuccess: () throws -> Void) throws {
    try operationGroupsMutex.synchronize {
      guard !_hasBeenShutdown
        else { throw RpcCallError.completionQueueShutdown }
