...
internal struct LengthPrefixedMessageWriter {
  ...
  func write(buffer: ByteBuffer, allocator: ByteBufferAllocator,
    ...
    if compressed, let compressor = self.compressor {
      ...
      buffer.set(integer: Int8(0), at: buffer.readerIndex)
      buffer.set(integer: Int32(messageSize), at: buffer.readerIndex + 1)
      ...
      lengthPrefixed.write(integer: Int8(0), endianness: .big)

      // Write the message length.
      lengthPrefixed.write(integer: Int32(buffer.readableBytes), endianness: .big)

      // Write the message.
      var buffer = buffer
      lengthPrefixed.write(buffer: &buffer)
...
extension ByteBuffer {
  ...
  mutating func writePayloadLength(_ length: UInt32, at index: Int) -> Int {
    ...
    return self.write(integer: length)
  }
}
