...
public enum _GRPCClientResponsePart<Response> {
  /// RpcMetadata received as the server acknowledges the RPC.
  case initialMetadata(RpcMetadata)
  ...
  case trailingMetadata(RpcMetadata)
...
public struct _GRPCRequestHead {
  ...
  internal var customMetadata: RpcMetadata
  ...
  internal var deadline: TimeDelta {
    get {
...
extension _GRPCRequestHead {
  internal init(
    ...
    if let requestID = requestID, let requestIDHeader = options.requestIDHeader {
      let customMetadata = options.customMetadata
      try! customMetadata.add(key: requestIDHeader, value: requestID)
...
internal final class GRPCClientChannelHandler {
  ...
  ///   - logger: Logger.
  internal init(callType: GRPCCallType) {
    switch callType {
    case .unary:
...
extension GRPCClientChannelHandler: ChannelInboundHandler {
   internal typealias InboundIn = ByteBuffer//HTTP2Frame.FramePayload
   internal typealias InboundOut = _RawGRPCClientResponsePart

  internal func channelRead(context: ChannelHandlerContext, data: NIOAny) {
  print("GRPCClientChannelHandler.channelRead") 
...
//     }
  ...
  /// - Parameter context: RpcChannel handler context.
...
extension GRPCClientChannelHandler: ChannelOutboundHandler {
   ...
   internal typealias OutboundOut = ByteBuffer//HTTP2Frame.FramePayload
  ...
  internal func write(
    ...
    print("GRPCClientChannelHandler.write")
