...
struct GRPCClientStateMachine {
  ...
  /// The default user-agent string.
  private static let userAgent = "grpc-swift"
  ...
  ) -> Result<RpcMetadata, SendRequestHeadersError> {
...
extension GRPCClientStateMachine.State {
  ...
  mutating func sendRequestHeaders(
    ...
    let result: Result<RpcMetadata, SendRequestHeadersError>
  ...
  private func parseResponseHeaders(
    ...
    let statusHeader = headers[":status"]
    ...
    if let encodingHeader = headers[GRPCHeaderName.encoding] {
  ...
  private func parseTrailers(_ trailers: RpcMetadata) -> GRPCStatus {
  ...
  private func readStatusCode(from trailers: RpcMetadata) -> StatusCode? {
    return trailers[GRPCHeaderName.statusCode]
      .flatMap(Int.init)
      .flatMap(StatusCode.init)
  }

  private func readStatusMessage(from trailers: RpcMetadata) -> String? {
    return trailers[GRPCHeaderName.statusMessage]
  ...
  private func parseTrailersOnly(
    ...
    let statusHeader = trailers[":status"]
    ...
    // the server instead.
    if let contentTypeHeader = trailers["content-type"],
