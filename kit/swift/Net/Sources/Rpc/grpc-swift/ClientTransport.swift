...
/// This class is the glue between a `NIO.RpcChannel` and the `ClientInterceptorPipeline`. In fact
...
/// response types used by the interceptor pipeline and the `NIO.RpcChannel`. In addition, the
/// transport keeps track of the state of the call and the `RpcChannel`, taking appropriate action
/// when these change. This includes buffering request parts from the interceptor pipeline until
/// the `NIO.RpcChannel` becomes active.
///
/// ### Thread Safety
///
...
internal final class ClientTransport<Request, Response> {
  ...
  /// A promise for the underlying `RpcChannel`. We'll succeed this when we transition to `active`
  ...
  private var writeBuffer = MarkedCircularBuffer<RequestAndPromise>(initialRingCapacity: 4)
  ...
  private struct RequestAndPromise {
    ...
    var promise: GRPCPromiseWithStatus?
  }
  ...
  // Our `NIO.RpcChannel` will fire trailers and the `GRPCStatus` to us separately. It's more
  ...
  private var trailers: RpcMetadata?
  ...
  /// The `NIO.RpcChannel` used by the transport, if it is available.
  private var channel: RpcChannel?
  ...
  internal init(
   ...
   // self.logger = logger
    ...
    self._pipeline = ClientInterceptorPipeline(
      ...
      onError: onError,
      onCancel: self.cancelFromPipeline,
      onRequestPart: self.sendFromPipeline,
      onResponsePart: onResponsePart
    )
  }

  // MARK: - RpcCall Object API
  ...
  internal func configure(_ configurator: @escaping (ChannelHandler) -> GRPCPromiseWithStatus) {
  ...
  /// - Important: This *must* to be called from the `callEventLoop`.
  @inlinable
  internal func send(_ part: GRPCClientRequestPart<Request>, _ promise: GRPCPromiseWithStatus?) {
    print("ClientTransport.send")
    ////self.callEventLoop.assertInEventLoop()
    if let pipeline = self._pipeline {
      print("ClientTransport.send: pipeline.send()")
      pipeline.send(part, promise: promise)
    } else {
      print("ClientTransport.send: no pipeline => promise(GRPCError.AlreadyComplete())")
      promise?(GRPCError.AlreadyComplete().makeGRPCStatus())
    }
  }
  ...
  ///   been handled.
  internal func cancel(_ promise: GRPCPromiseWithStatus?) {
    print("ClientTransport.cancel")
    ////self.callEventLoop.assertInEventLoop()
    if let pipeline = self._pipeline {
  ...
  /// A request for the underlying `RpcChannel`.
  internal func getChannel() -> RpcChannel? {
    ////self.callEventLoop.assertInEventLoop()
    ...
    //   let promise = self.callEventLoop.makePromise(of: RpcChannel.self)
...
extension ClientTransport {
  ...
  private func sendFromPipeline(
    ...
    print("ClientTransport.sendFromPipeline")
    //self.callEventLoop.assertInEventLoop()
    switch self.state.send() {
    case .writeToBuffer:
      print("ClientTransport.sendFromPipeline: state.send() = .writeToBuffer => buffer(part, promise: promise)")
      self.buffer(part, promise: promise)

    case .writeToChannel:
      print("ClientTransport.sendFromPipeline: state.send() = .writeToChannel => writeToChannel()")
      // Banging the channel is okay here: we'll only be told to 'writeToChannel' if we're in the
      // correct state, the requirements of that state are having an active `RpcChannel`.
      self.writeToChannel(
        self.channel!,
        part: part,
    ...
    case .alreadyComplete:
      print("ClientTransport.sendFromPipeline: state.send() = .alreadyComplete => promise?(GRPCError.AlreadyComplete())")
      promise?(GRPCError.AlreadyComplete().makeGRPCStatus())
    }
  }
  ...
  /// - Important: This *must* to be called from the `callEventLoop`.
  private func cancelFromPipeline(_ promise: GRPCPromiseWithStatus?) {
    //self.callEventLoop.assertInEventLoop()
...
extension ClientTransport: ChannelInboundHandler {
  ...
  internal func channelActive(context: ChannelHandlerContext) {
    print("ClientTransport.channelActive")
...
extension ClientTransport {
  /// The `RpcChannel` became active. Send out any buffered requests.
  private func transportActivated(channel: RpcChannel) {
  ...
  private func _transportActivated(channel: RpcChannel) {
    print("ClientTransport._transportActivated")
    //self.callEventLoop.assertInEventLoop()

    if self.state.activate() {
      print("ClientTransport._transportActivated: state.activate() => self.channel = channel, unbuffer()")
      self.channel = channel
      self.unbuffer()
    } else {
      print("ClientTransport._transportActivated: state.activate() = false => channel.close()")
      channel.close(mode: .all, promise: nil)
    }
  }

  /// The `RpcChannel` became inactive. Fail any buffered writes and forward an error to the
  ...
  private func _transportDeactivated() {
    ...
    case .failChannelPromise:
      let status = GRPCStatus(code: .cancelled, message: "Transport failed")
      self.forwardErrorToInterceptors(status)
      //self.channelPromise?.fail(GRPCError.AlreadyComplete())
    }
  }

  /// Drops any references to the `RpcChannel` and interceptor pipeline.
  ...
  /// interceptor pipeline and any buffered writes will be failed. Any underlying `RpcChannel` will
  /// also be closed.
  ...
  /// Receive initial metadata from the `RpcChannel`.
  private func receiveFromChannel(initialMetadata headers: RpcMetadata) {
    self._receiveFromChannel(initialMetadata: headers)
  }

  /// On-loop implementation of `receiveFromChannel(initialMetadata:)`.
  private func _receiveFromChannel(initialMetadata headers: RpcMetadata) {
    print("ClientTransport._receiveFromChannel: initialMetadata")
    //self.callEventLoop.assertInEventLoop()
  ...
  /// Receive response message bytes from the `RpcChannel`.
  ...
  private func _receiveFromChannel(message buffer: ByteBuffer) {
    print("ClientTransport._receiveFromChannel: message")
    //self.callEventLoop.assertInEventLoop()
    do {
  ...
  /// Receive trailing metadata from the `RpcChannel`.
  private func receiveFromChannel(trailingMetadata trailers: RpcMetadata) {
    print("ClientTransport._receiveFromChannel: trailingMetadata")
    // The `RpcChannel` delivers trailers and `GRPCStatus` separately, we want to emit them together
    // in the interceptor pipeline.
    self.trailers = trailers
  }

  /// Receive the final status from the `RpcChannel`.
  private func receiveFromChannel(status: GRPCStatus) {
    print("ClientTransport._receiveFromChannel: status")
  ...
  private func _receiveFromChannel(status: GRPCStatus) {
    ...
    if self.state.channelRead(isEnd: true) {
      self.forwardToInterceptors(.end(status, self.trailers ?? RpcMetadata()))
      self.trailers = nil
    }
  }
}
...
private enum ClientTransportState {
  ...
  /// Closing. Either the RPC was cancelled or any `RpcChannel` associated with the transport hasn't
  /// become inactive yet.
  ///
  /// Valid transitions:
  /// - `closed` (the channel becomes inactive)
...
extension ClientTransportState {
  ...
  mutating func send() -> SendAction {
    ...
    // We have a `RpcChannel`, we can pipe the write straight through.
    case .active:
  ...
  mutating func unbuffered() -> UnbufferedAction {
    print("ClientTransportState.unbuffered()")
    switch self {
    ...
    // active.
    case .activatingTransport:
      print("ClientTransportState.unbuffered() .activatingTransport => self = .active")
      self = .active
  ...
  /// Cancel the RPC and associated `RpcChannel`, if possible. Returns a boolean indicated whether
  /// cancellation can go ahead (and also whether the channel should be torn down).
  mutating func cancel() -> Bool {
    switch self {
    case .idle:
      // No RPC has been started and we don't have a `RpcChannel`. We need to tell the interceptor
      // we're done, fail any writes, and then deal with the cancellation promise.
      self = .closed
      return true

    case .awaitingTransport:
      // An RPC has started and we're waiting for the `RpcChannel` to activate. We'll mark ourselves as
      // closing. We don't need to explicitly close the `RpcChannel`, this will happen as a result of
      // the `RpcChannel` becoming active (see `channelActive(context:)`).
      self = .closing
      return true

    case .activatingTransport:
      // The RPC has started, the `RpcChannel` is active and we're emptying our write buffer. We'll
  ...
  /// `channelActive` was invoked on the transport by the `RpcChannel`.
  ...
  enum ChannelInactiveAction {
    ...
    case tearDown
    /// Fail the 'RpcChannel' promise, if one exists; the RPC is already complete.
    case failChannelPromise
    /// Do nothing.
    case doNothing
  }

  /// `channelInactive` was invoked on the transport by the `RpcChannel`.
  mutating func deactivate() -> ChannelInactiveAction {
    switch self {
    case .idle:
      // We can't become inactive before we've requested a `RpcChannel`.
  ...
  /// `channelRead` was invoked on the transport by the `RpcChannel`. Returns a boolean value
  ...
  mutating func channelRead(isEnd: Bool) -> Bool {
    switch self {
    case .idle, .awaitingTransport:
      // If there's no `RpcChannel` or the `RpcChannel` isn't active, then we can't read anything.
      preconditionFailure("Can't receive response part on idle transport")

    case .activatingTransport, .active:
      // We have an active `RpcChannel`, we can forward the request part but we may need to start
  ...
  enum HandleErrorAction {
    ...
    case propagateError
    /// As above, but close the 'RpcChannel' as well.
    case propagateErrorAndClose
  ...
  mutating func handleError() -> HandleErrorAction {
    switch self {
    case .idle:
      // The `RpcChannel` can't error if it doesn't exist.
      preconditionFailure("Can't catch error on idle transport")

    case .awaitingTransport:
      // We're waiting for the `RpcChannel` to become active. We're toast now, so close, failing any
  ...
  enum GetChannelAction {
    /// No action is required.
    case doNothing
    /// Succeed the RpcChannel promise.
    case succeed
    /// Fail the 'RpcChannel' promise, the RPC is already complete.
    case fail
  }

  /// The caller has asked for the underlying `RpcChannel`.
...
extension ClientTransport {
  ...
  private func configure(using configurator: (ChannelHandler) -> GRPCPromiseWithStatus) {
    let _ = configurator(self)
  ...
  private func buffer(
    _ part: GRPCClientRequestPart<Request>,
    promise: GRPCPromiseWithStatus?
  ) {
    print("ClientTransport.buffer")
    //self.callEventLoop.assertInEventLoop()
  ...
  /// Writes any buffered request parts to the `RpcChannel`.
  private func unbuffer() {
    print("ClientTransport.unbuffer()")
    //self.callEventLoop.assertInEventLoop()
    ...
    while self.state.isUnbuffering, !self.writeBuffer.isEmpty {
      ...
      while let write = self.writeBuffer.popFirst() {
        ...
        self.writeToChannel(channel, part: write.request, flush: false, write.promise)
      }
    ...
    case .doNothing:
      ()
    case .succeedChannelPromise:
      print("ClientTransport.unbuffer() => .succeedChannelPromise => self.channel = channel")
      self.channel = channel
    }
  }
  ...
  private func failBufferedWrites(with error: Error) {
    ...
    while let write = self.writeBuffer.popFirst() {
      //write.promise?(error)
      write.promise?(GRPCStatus.unknown)
    }
  }

  /// Write a request part to the `RpcChannel`.
  /// - Parameters:
  ///   - channel: The `RpcChannel` to write `part` to.
  ///   - part: The request part to write.
  ///   - promise: A promise to complete once the write has been completed.
  ///   - flush: Whether to flush the `RpcChannel` after writing.
  private func writeToChannel(
    _ channel: RpcChannel,
    part: GRPCClientRequestPart<Request>,
    flush: Bool,
    _ promise: GRPCPromiseWithStatus?
  ) {
    print("ClientTransport.writeToChannel")
    switch part {
...
extension ClientTransport {
  /// Returns whether the `RpcChannel` should be flushed after writing the given part to it.
  ...
  private func makeRequestHead(with metadata: RpcMetadata) -> _GRPCRequestHead {
