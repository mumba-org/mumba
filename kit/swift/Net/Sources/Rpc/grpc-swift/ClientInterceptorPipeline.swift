...
/// and the transport used to send and receive messages from the server (a `NIO.RpcChannel`). It holds
...
internal final class ClientInterceptorPipeline<Request, Response> {
  ...
  @usableFromInline
  internal let _onCancel: (GRPCPromiseWithStatus?) -> Void

  @usableFromInline
  internal let _onRequestPart: (GRPCClientRequestPart<Request>, GRPCPromiseWithStatus?) -> Void
  ...
  internal init(
    ...
    onCancel: @escaping (GRPCPromiseWithStatus?) -> Void,
    onRequestPart: @escaping (GRPCClientRequestPart<Request>, GRPCPromiseWithStatus?) -> Void,
  ...
  internal func _errorCaught(_ error: Error) {
    ...
    // } else {
      ...
      self._errorDelegate?.didCatchErrorWithoutContext(error)
    //}
  ...
  /// - Important: This *must* to be called from the `eventLoop`.
  @inlinable
  internal func send(_ part: GRPCClientRequestPart<Request>, promise: GRPCPromiseWithStatus?) {
  ...
  internal func _invokeSend(
    ...
    print("ClientInterceptorPipeline._invokeSend")
    switch index {
    case self._headIndex:
      print("ClientInterceptorPipeline._invokeSend: index = self._headIndex => _onRequestPart(part, promise)")
      self._onRequestPart(part, promise)

    case self._tailIndex:
      print("ClientInterceptorPipeline._invokeSend: index = self._tailIndex => _invokeSend()")
      self._invokeSend(
        part,
        promise: promise,
    ...
    default:
      print("ClientInterceptorPipeline._invokeSend: default => invokeSend(part, promise: promise)")
  ...
  /// - Important: This *must* to be called from the `eventLoop`.
  @inlinable
  internal func cancel(promise: GRPCPromiseWithStatus?) {
  ...
  @inlinable
  internal func invokeCancel(promise: GRPCPromiseWithStatus?, fromContextAtIndex index: Int) {
...
extension ClientInterceptorPipeline {
  ...
  internal func _setupDeadline() {
    func setup() {
      ...
      print("FIXME: _setupDeadline => timeout not set")
...
extension ClientInterceptorContext {
  ...
  internal func invokeSend(
    ...
    print("ClientInterceptorContext.invokeSend: interceptor.send()")
    self.interceptor.send(part, promise: promise, context: self)
  }

  @inlinable
  internal func invokeCancel(promise: GRPCPromiseWithStatus?) {
