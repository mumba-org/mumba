...
/// `RpcChannel`. `ChannelPipeline` implements an advanced form of the Intercepting Filter pattern
/// to give a user full control over how an event is handled and how the `ChannelHandler`s in a pipeline
/// interact with each other.
///
/// # Creation of a pipeline
///
/// Each `RpcChannel` has its own `ChannelPipeline` and it is created automatically when a new `RpcChannel` is created.
///
...
/// `RpcChannel`. The I/O thread often performs the actual output operation such as `Socket.write`.
///
///
...
public final class ChannelPipeline: ChannelInvoker {
    ...
    /// The `EventLoop` that is used by the underlying `RpcChannel`.
    //public let eventLoop: EventLoop

    /// The `RpcChannel` that this `ChannelPipeline` belongs to.
    ///
    /// - note: This will be nil after the channel has closed
    private var _channel: Optional<RpcChannel>
    ...
    public func addHandler(_ handler: ChannelHandler,
        ...
        let _ = addHandlerSync(handler, name: name, position: position)
    }
    ...
    public func removeHandler(_ handler: RemovableChannelHandler, promise: GRPCPromiseWithStatus?) {
        func removeHandler0() {
    ...
    ///     - promise: An `EventLoopPromise` that will complete when the `ChannelHandler` is removed.
    public func removeHandler(name: String, promise: GRPCPromiseWithStatus?) {
        func removeHandler0() {
    ...
    ///     - promise: An `EventLoopPromise` that will complete when the `ChannelHandler` is removed.
    public func removeHandler(context: ChannelHandlerContext, promise: GRPCPromiseWithStatus?) {
        guard context.handler is RemovableChannelHandler else {
            promise?(ChannelError.unremovableHandler.makeGRPCStatus())
            return
        }
        func removeHandler0() {
    ...
    public func context(handler: ChannelHandler) -> ChannelHandlerContext {
        return try! contextSync(handler: handler).get()
    }
    ...
    fileprivate func contextSync(handler: ChannelHandler) -> Result<ChannelHandlerContext, Error> {
        return _contextSync({ $0.handler === handler })
    }
    ...
    ///     - name: the name that was used to add the `ChannelHandler` to the `ChannelPipeline` before.
    /// - returns: the `EventLoopFuture` which will be notified once the the operation completes.
    public func context(name: String) -> ChannelHandlerContext {
       return try! contextSync(name: name).get()
    }
    ...
    /// - returns: the `EventLoopFuture` which will be notified once the the operation completes.
    @inlinable
    public func context<Handler: ChannelHandler>(handlerType: Handler.Type) -> ChannelHandlerContext {
        try! _contextSync(handlerType: handlerType).get()
    }
    ...
    /// `RemovableChannelHandler` API. This must only be used to clear the pipeline on `RpcChannel` tear down and
    /// as a result of the `leavePipeline` call in the `RemovableChannelHandler` API.
    internal func removeHandlerFromPipeline(context: ChannelHandlerContext, promise: GRPCPromiseWithStatus?) {
        //self.eventLoop.assertInEventLoop()
    ...
    public func close(mode: CloseMode = .all, promise: GRPCPromiseWithStatus?) {
    ...
    public func write(_ data: NIOAny, promise: GRPCPromiseWithStatus?) {
        write0(data, promise: promise)
    }

    public func writeAndFlush(_ data: NIOAny, promise: GRPCPromiseWithStatus?) {
        writeAndFlush0(data, promise: promise)
    }

    public func bind(to address: IpEndPoint, promise: GRPCPromiseWithStatus?) {
        bind0(to: address, promise: promise)
    }

    public func connect(to address: IpEndPoint, promise: GRPCPromiseWithStatus?) {
        connect0(to: address, promise: promise)
    }

    public func register(promise: GRPCPromiseWithStatus?) {
        register0(promise: promise)
    }

    public func triggerUserOutboundEvent(_ event: Any, promise: GRPCPromiseWithStatus?) {
    ...
    func close0(mode: CloseMode, promise: GRPCPromiseWithStatus?) {
    ...
    func writeAndFlush0(_ data: NIOAny, promise: GRPCPromiseWithStatus?) {
        if let firstOutboundCtx = firstOutboundCtx {
    ...
    func bind0(to address: IpEndPoint, promise: GRPCPromiseWithStatus?) {
        if let firstOutboundCtx = firstOutboundCtx {
    ...
    func connect0(to address: IpEndPoint, promise: GRPCPromiseWithStatus?) {
        if let firstOutboundCtx = firstOutboundCtx {
    ...
    func fireChannelActive0() {
        print("ChannelPipeline.fireChannelActive0")
        if let firstInboundCtx = firstInboundCtx {
            print("ChannelPipeline.fireChannelActive0: firstInboundCtx.invokeChannelActive")
    ...
    /// Create `ChannelPipeline` for a given `RpcChannel`. This method should never be called by the end-user
    /// directly: it is only intended for use with custom `RpcChannel` implementations. Users should always use
    /// `channel.pipeline` to access the `ChannelPipeline` for a `RpcChannel`.
    ///
    /// - parameters:
    ///    - channel: The `RpcChannel` this `ChannelPipeline` is created for.
...
extension ChannelPipeline {
    ...
    public func addHandlers(_ handlers: [ChannelHandler],
        ...
        self.addHandlersSync(handlers, position: position)
    }
...
extension ChannelPipeline {
    ...
    public struct SynchronousOperations {
        ...
        /// The `EventLoop` of the `RpcChannel` this synchronous operations view corresponds to.
...
/// Special `ChannelHandler` that forwards all events to the `RpcChannel.Unsafe` implementation.
/* private but tests */ final class HeadChannelHandler: _ChannelOutboundHandler {
    ...
    func bind(context: ChannelHandlerContext, to address: IpEndPoint, promise: GRPCPromiseWithStatus?) {
        context.channel._channelCore.bind0(to: address, promise: promise)
    }

    func connect(context: ChannelHandlerContext, to address: IpEndPoint, promise: GRPCPromiseWithStatus?) {
...
/* private but tests */ final class TailChannelHandler: _ChannelInboundHandler {
    ...
    func channelActive(context: ChannelHandlerContext) {
        print("TailChannelHandler(_ChannelInboundHandler).channelActive: should not be calling here.. blackhole")
        // Discard
    }
...
public final class ChannelHandlerContext: ChannelInvoker {
    ...
    public var remoteAddress: IpEndPoint? {
        do {
            ...
            // RpcChannel was closed already but we may still have the address cached so try to access it via the RpcChannel
    ...
    ///     - promise: The promise fulfilled when the `RpcChannel` has been closed or failed if it the closing failed.
    public func close(mode: CloseMode = .all, promise: GRPCPromiseWithStatus?) {
        if let outboundNext = self.prev {
    ...
    fileprivate func invokeChannelActive() {
        print("ChannelHandlerContext.invokeChannelActive")
        //self.eventLoop.assertInEventLoop()

        if let inboundHandler = self.inboundHandler {
            print(" inboundHandler.channelActive(context: self)")
            inboundHandler.channelActive(context: self)
        } else {
            print(" no inboundHandler. self.next?.invokeChannelActive()")
            self.next?.invokeChannelActive()
        }
    }
    ...
    fileprivate func invokeRegister(promise: GRPCPromiseWithStatus?) {
        //self.eventLoop.assertInEventLoop()
   ...
   fileprivate func invokeBind(to address: IpEndPoint, promise: GRPCPromiseWithStatus?) {
        //self.eventLoop.assertInEventLoop()
    ...
    fileprivate func invokeConnect(to address: IpEndPoint, promise: GRPCPromiseWithStatus?) {
        //self.eventLoop.assertInEventLoop()
    ...
    fileprivate func invokeWrite(_ data: NIOAny, promise: GRPCPromiseWithStatus?) {
        //self.eventLoop.assertInEventLoop()
    ...
    fileprivate func invokeWriteAndFlush(_ data: NIOAny, promise: GRPCPromiseWithStatus?) {
        //self.eventLoop.assertInEventLoop()
    ...
    fileprivate func invokeClose(mode: CloseMode, promise: GRPCPromiseWithStatus?) {
        //self.eventLoop.assertInEventLoop()
    ...
    fileprivate func invokeTriggerUserOutboundEvent(_ event: Any, promise: GRPCPromiseWithStatus?) {
        //self.eventLoop.assertInEventLoop()
...
extension ChannelHandlerContext {
    ...
    public struct RemovalToken {
        internal let promise: GRPCPromiseWithStatus?
    }
    ...
    internal func startUserTriggeredRemoval(promise: GRPCPromiseWithStatus?) {
        //self.eventLoop.assertInEventLoop()
        guard !self.userTriggeredRemovalStarted else {
            promise?(NIOAttemptedToRemoveHandlerMultipleTimesError().makeGRPCStatus())
            return
        }
...
extension ChannelPipeline: CustomDebugStringConvertible {
    ...
    @inlinable
    public func handler<Handler: ChannelHandler>(type _: Handler.Type) -> Handler? {
        let context = self.context(handlerType: Handler.self)
