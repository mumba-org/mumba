...
///
/// Note: while this object is a `struct`, its implementation delegates to `RpcCall`. It therefore
/// has reference semantics.
public struct ClientStreamingCall<RequestPayload, ResponsePayload>: StreamingRequestClientCall,
  UnaryResponseClientCall {
  private let call: RpcCall<RequestPayload, ResponsePayload>
  ...
  /// The `RpcChannel` used to transport messages for this RPC.
  public var subchannel: RpcChannel {
  ...
  public func cancel(_ promise: GRPCPromiseWithStatus?) {
    self.call.cancel(promise)
  }

  // MARK: - Response Parts

  /// The initial metadata returned from the server.
  public var initialMetadata: RpcMetadata {
  ...
  public var trailingMetadata: RpcMetadata {
  ...
  internal init(call: RpcCall<RequestPayload, ResponsePayload>) {
    self.call = call
    self.responseParts = UnaryResponseParts()
  }
  ...
  public func sendMessage(
    ...
    self.call.send(.message(message, .init(compress: compress, flush: true)), promise)
  }
  ...
  public func sendMessages<S>(
    ...
    self.call.sendMessages(messages, compression: compression, promise)
  }
