...
internal struct ClientTransportFactory<Request, Response> {
  ...
  private var factory: HTTP2ClientTransportFactory<Request, Response>
  ...
  /// Create a transport factory for HTTP/2 based transport with `GeneratedMessageProtocol` messages.
  /// - Parameters:
  ...
  // internal static func http2<Request: GeneratedMessageProtocol, Response: GeneratedMessageProtocol>(
  //   authority: String,
  //   scheme: String,
  ...
  //   )
  //   return ClientTransportFactory(http2)
  // }
  ...
  internal static func http2<Request: GRPCPayload, Response: GRPCPayload>(
   // multiplexer: EventLoopFuture<HTTP2StreamMultiplexer>,
    ...
    return ClientTransportFactory<Request, Response>(http2)
  }
  ...
  // internal static func fake<Request: GeneratedMessageProtocol, Response: GeneratedMessageProtocol>(
  ...
  internal func makeConfiguredTransport(
    ...
    factory.configure(client, transport)
    return transport
 }
...
private struct HTTP2ClientTransportFactory<Request, Response> {
 ...
 // private var multiplexer: EventLoopFuture<HTTP2StreamMultiplexer>
  ...
  fileprivate func makeTransport(
    ...
    print("HTTP2ClientTransportFactory.makeTransport")
  ...
  fileprivate func configure<Request, Response>(_ client: ClientConnection, _ transport: ClientTransport<Request, Response>) {
    print("HTTP2ClientTransportFactory.configure")
    transport.configure { _ in
    //   self.multiplexer.flatMap { multiplexer in
    //     let streamPromise = self.multiplexer.eventLoop.makePromise(of: RpcChannel.self)
    ...
    //   }
    // }
    //  transport.configure { configurator in
    // //   //let channel = createChannel()
    //   client.startConnecting()
    // //   let channel = transport.getChannel()!
    // //   let syncOperations = channel.pipeline.syncOperations
    // //   let clientHandler = GRPCClientChannelHandler(callType: transport.callDetails.type)
    // //   try! syncOperations.addHandler(clientHandler)
    // //   try! syncOperations.addHandler(transport)
      return { _ in GRPCStatus.ok }
    }
  }
