...
public final class ConnectionManager {
  ...
  internal struct ConnectedState {
    ...
    init(from state: ConnectingState, candidate: RpcChannel){//, multiplexer: HTTP2StreamMultiplexer) {
      self.backoffIterator = state.backoffIterator
      self.reconnect = state.reconnect
      self.candidate = candidate
      //.readyChannelMuxPromise = state.readyChannelMuxPromise
  ...
  internal struct TransientFailureState {
    var backoffIterator: ConnectionBackoffIterator?
    //var readyChannelMuxPromise: EventLoopPromise<HTTP2StreamMultiplexer>
    var scheduled: GRPCPromise?
    var reason: Error?

    init(from state: ConnectingState, scheduled: @escaping GRPCPromise, reason: Error) {
      self.backoffIterator = state.backoffIterator
    ...
    init(from state: ConnectedState, scheduled: @escaping GRPCPromise) {
      self.backoffIterator = state.backoffIterator
      //self.readyChannelMuxPromise = state.readyChannelMuxPromise
      self.scheduled = scheduled
    ...
    init(
      ...
      scheduled: @escaping GRPCPromise,
  ...
  internal struct ShutdownState {
    var closeFuture: GRPCPromise?
    /// The reason we are shutdown. Any requests for a `RpcChannel` in this state will be failed with
    /// this error.
    var reason: Error

    init(closeFuture: GRPCPromise?, reason: Error) {
      self.closeFuture = closeFuture
      self.reason = reason
    }

    static func shutdownByUser(closeFuture: GRPCPromise?) -> ShutdownState {
  ...
  internal enum State {
    ...
    /// We've established a `RpcChannel`, it might not be suitable (TLS handshake may fail, etc.).
    ...
    /// We have an active `RpcChannel` which has seen the initial HTTP/2 SETTINGS frame. We can use
    ...
    /// A `RpcChannel` is desired, we'll attempt to create one in the future.
    ///
    /// Valid next states:
    /// - `connecting`
    /// - `shutdown`
    ...
    /// We never want another `RpcChannel`: this state is terminal.
  ...
  /// An `EventLoopFuture<RpcChannel>` provider.
  ...
  private func _shutdown() {
    ...
    case .idle:
      shutdown = .shutdownByUser(closeFuture: nil)
    ...
    case var .transientFailure(state):
      shutdown = .shutdownByUser(closeFuture: nil)
      self.state = .shutdown(shutdown)
  ...
  internal func channelError(_ error: Error) {
    ...
    case .idle:
      ...
      //])
      print("ignoring unexpected error in idle")
  ...
  internal func channelActive(channel: RpcChannel) {//, multiplexer: HTTP2StreamMultiplexer) {
    print("ConnectionManager.channelActive")
    switch self.state {
    case var .connecting(connecting):
    ...
    case let .active(active):
      ...
      case .none:
        let shutdownState = ShutdownState(
          closeFuture: nil,//self.eventLoop.makeSucceededFuture(()),
          reason: error
        )
...
extension ConnectionManager {
  ...
  private func connectionFailed(withError error: Error) {
    print("ConnectionManager.connectionFailed")
    switch self.state {
    case let .connecting(connecting):
      ...
      case .none:
        self.state = .shutdown(
          ShutdownState(closeFuture: nil, reason: error)
        )
      ...
      case let .after(_):
        ...
        //   self.startConnecting()
        // }
        // FIXME: given this is not really scheduled, it wont work as intended
        let scheduled = {
          self.startConnecting()
        }
...
extension ConnectionManager {
  ...
  private func startConnecting(
    ...
    print("ConnectionManager.startConnecting")
    ...
    let channel: RpcChannel = self.channelProvider.makeChannel(
      managedBy: self,
      connectTimeout: timeoutAndBackoff.map { TimeDelta(seconds: Int64($0.timeout)) }
    )!
