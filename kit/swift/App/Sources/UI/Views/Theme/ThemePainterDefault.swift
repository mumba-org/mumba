...
fileprivate let trackColor = Color.fromRGB(0xF1, 0xF1, 0xF1)

public class ThemePainterDefault : ThemePainter {
  ...
  public func paintArrowButton(
    ...
    var bgColor: Color = trackColor
    // Aura-win uses slightly different arrow colors.
    var arrowColor: Color = Graphics.placeholderColor
    switch state {
      case .Disabled:
        arrowColor = Color.Gray//getArrowColor(state)
      case .Hovered:
        bgColor = Color.fromRGB(0xd2, 0xd2, 0xd2)
        fallthrough
      case .Normal:
        arrowColor = Color.fromRGB(0x50, 0x50, 0x50)
      case .Pressed:
        bgColor = Color.fromRGB(0x78, 0x78, 0x78)
        arrowColor = Color.White
    }

    let flags = PaintFlags()
    flags.color = bgColor
    canvas.drawIRect(rect, flags: flags)

    paintArrow(canvas: canvas, rect: rect, direction: direction, color: arrowColor)
  }
