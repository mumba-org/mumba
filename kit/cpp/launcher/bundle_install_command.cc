...
std::unique_ptr<BundleInstallCommand> BundleInstallCommand::Create() {
  return std::make_unique<BundleInstallCommand>();
}

BundleInstallCommand::BundleInstallCommand() {
...
std::string BundleInstallCommand::GetCommandMethod() const {
  return "/mumba.Mumba/BundleInstall";
}

int BundleInstallCommand::Run(CommandExecutor* executor, const base::CommandLine::StringVector& args) {
  ...
  if(!executor->EncodeMessage("BundleInstallRequest", input_map, &encoded_input)) {
    printf("bundle install: failed while encoding request\n");
    return 1;
  }
