...
ChromeBlobStorageContext* ChromeBlobStorageContext::GetFor(
  ...
  if (!domain->GetBlobStorageContext()) {
    ...
    //         blob.get()));
    domain->SetBlobStorageContext(blob);

     // Resolve our storage directories.
    FilePath blob_storage_parent =
        domain->partition_path().Append(kBlobStorageParentDirectory);
  ...
  return domain->GetBlobStorageContext();
...
ChromeBlobStorageContext::URLLoaderFactoryForToken(
  ...
  HostThread::PostTask(
      ...
      base::BindOnce(
          ...
          base::WrapRefCounted(GetFor(domain)),
          MakeRequest(&blob_url_loader_factory_ptr), token.PassInterface()));
  return base::MakeRefCounted<common::WrapperSharedURLLoaderFactory>(
