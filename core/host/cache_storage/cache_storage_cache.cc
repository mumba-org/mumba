...
#include "core/host/cache_storage/cache_storage_cache.h"
...
#include "core/host/cache_storage/cache_storage.pb.h"
#include "core/host/cache_storage/cache_storage_blob_to_disk_cache.h"
#include "core/host/cache_storage/cache_storage_cache_handle.h"
#include "core/host/cache_storage/cache_storage_cache_observer.h"
#include "core/host/cache_storage/cache_storage_scheduler.h"
...
#include "storage/host/blob/blob_data_builder.h"
#include "storage/host/blob/blob_data_handle.h"
#include "storage/host/blob/blob_impl.h"
#include "storage/host/blob/blob_storage_context.h"
#include "storage/host/blob/blob_url_request_job_factory.h"
...
bool VaryMatches(const common::ServiceWorkerHeaderMap& request,
                 const common::ServiceWorkerHeaderMap& cached_request,
                 const common::ServiceWorkerHeaderMap& response) {
  common::ServiceWorkerHeaderMap::const_iterator vary_iter = response.find("vary");
  ...
  for (const std::string& trimmed :
    ...
    common::ServiceWorkerHeaderMap::const_iterator request_iter = request.find(trimmed);
    common::ServiceWorkerHeaderMap::const_iterator cached_request_iter =
...
std::unique_ptr<common::ServiceWorkerFetchRequest> CreateRequest(
    const proto::CacheMetadata& metadata,
    const GURL& request_url) {
  auto request = std::make_unique<common::ServiceWorkerFetchRequest>(
      request_url, metadata.request().method(), common::ServiceWorkerHeaderMap(),
      common::Referrer(), false);
...
std::unique_ptr<common::ServiceWorkerResponse> CreateResponse(
    const proto::CacheMetadata& metadata,
  ...
  std::unique_ptr<common::ServiceWorkerHeaderMap> headers =
      std::make_unique<common::ServiceWorkerHeaderMap>();
  ...
  return std::make_unique<common::ServiceWorkerResponse>(
      ...
      std::make_unique<common::ServiceWorkerHeaderList>(
...
bool ShouldPadResourceSize(const proto::CacheResponse* response) {
  return ShouldPadResponseType(
      ProtoResponseTypeToFetchResponseType(response->response_type()),
      response->url_list_size());
}

bool ShouldPadResourceSize(const common::ServiceWorkerResponse* response) {
...
int64_t CalculateResponsePaddingInternal(
    const proto::CacheResponse* response,
    const crypto::SymmetricKey* padding_key,
    int side_data_size) {
...
void ReadBlobData(const base::Optional<std::vector<unsigned char>>& data) {
  if (!data.has_value()) {
    DLOG(INFO) << "CacheStorageCache::PutWriteBlobToCache (host): blob have no data";
  } else {
    DLOG(INFO) << "CacheStorageCache::PutWriteBlobToCache (host): data = '" << 
...
struct CacheStorageCache::PutContext {
  PutContext(std::unique_ptr<common::ServiceWorkerFetchRequest> request,
  ...
  std::unique_ptr<common::ServiceWorkerFetchRequest> request;
  std::unique_ptr<common::ServiceWorkerResponse> response;
  blink::mojom::BlobPtr blob;
  blink::mojom::BlobPtr side_data_blob;
...
struct CacheStorageCache::QueryCacheContext {
  QueryCacheContext(std::unique_ptr<common::ServiceWorkerFetchRequest> request,
                    const common::CacheStorageCacheQueryParams& options,
  ...
  // Input to QueryCache
  std::unique_ptr<common::ServiceWorkerFetchRequest> request;
  common::CacheStorageCacheQueryParams options;
  QueryCacheCallback callback;
...
void CacheStorageCache::MatchAll(
  ...
  if (backend_state_ == BACKEND_CLOSED) {
    std::move(callback).Run(CacheStorageError::kErrorStorage,
                            std::vector<common::ServiceWorkerResponse>());
    return;
  }
...
void CacheStorageCache::BatchOperation(
    const std::vector<common::CacheStorageBatchOperation>& operations,
  ...
  for (const auto& operation : operations) {
    if (operation.operation_type == common::CACHE_STORAGE_CACHE_OPERATION_TYPE_PUT) {
  ...
  for (const auto& operation : operations) {
    switch (operation.operation_type) {
      case common::CACHE_STORAGE_CACHE_OPERATION_TYPE_PUT:
        if (skip_side_data) {
          common::CacheStorageBatchOperation new_operation = operation;
...
void CacheStorageCache::Keys(std::unique_ptr<common::ServiceWorkerFetchRequest> request,
                             const common::CacheStorageCacheQueryParams& options,
                             RequestsCallback callback) {
  DLOG(INFO) << "CacheStorageCache::Keys(host)";
  if (backend_state_ == BACKEND_CLOSED) {
    DLOG(INFO) << "CacheStorageCache::Keys(host): BACKEND_CLOSED";
    std::move(callback).Run(CacheStorageError::kErrorStorage, nullptr);
    return;
  }
...
void CacheStorageCache::QueryCache(
  ...
  if (!options.ignore_method && request && !request->method.empty() &&
      request->method != "GET") {
    DLOG(INFO) << "CacheStorageCache::QueryCache(host): request->method != GET => sending empty results";    
    std::move(callback).Run(CacheStorageError::kSuccess,
                            std::make_unique<QueryCacheResults>());
    return;
  }

  common::ServiceWorkerFetchRequest* request_ptr = request.get();
  ...
  if (query_cache_context->request &&
    ...
    // URL.
    DLOG(INFO) << "CacheStorageCache::QueryCache(host): opening entry";    
    ...
    if (rv != net::ERR_IO_PENDING) {
      DLOG(INFO) << "CacheStorageCache::QueryCache(host): rv = " << rv;    
...
void CacheStorageCache::QueryCacheOpenNextEntry(
  ...
  if (!query_cache_context->backend_iterator) {
    DLOG(INFO) << "CacheStorageCache::QueryCache(host): !query_cache_context->backend_iterator => Iteration is complete";    
...
void CacheStorageCache::QueryCacheFilterEntry(
    std::unique_ptr<QueryCacheContext> query_cache_context,
    int rv) {
  DLOG(INFO) << "CacheStorageCache::QueryCacheFilterEntry(host)";    
  if (rv == net::ERR_FAILED) {
    DLOG(INFO) << "CacheStorageCache::QueryCacheFilterEntry(host): rv == net::ERR_FAILED => Iteration is complete";    
  ...
  if (rv < 0) {
    DLOG(INFO) << "CacheStorageCache::QueryCacheFilterEntry(host): rv < 0 => ERROR";    
    std::move(query_cache_context->callback)
  ...
  if (backend_state_ == BACKEND_CLOSED) {
    DLOG(INFO) << "CacheStorageCache::QueryCacheFilterEntry(host): BACKEND_CLOSED";    
    std::move(query_cache_context->callback)
...
void CacheStorageCache::QueryCacheDidReadMetadata(
  ...
  DLOG(INFO) << "CacheStorageCache::QueryCacheDidReadMetadata (host)";
  if (!metadata) {
    DLOG(INFO) << "CacheStorageCache::QueryCacheDidReadMetadata (host): NO METADATA. deleting entry..";
    entry->Doom();
    QueryCacheOpenNextEntry(std::move(query_cache_context));
    return;
  }
  ...
  if (query_cache_context->request &&
    ...
    QueryCacheOpenNextEntry(std::move(query_cache_context));
    DLOG(INFO) << "CacheStorageCache::QueryCacheDidReadMetadata (host): cancelling..";
    return;
  }
  ...
  if (query_cache_context->query_types & QUERY_CACHE_REQUESTS) {
    ...
    if (query_cache_context->estimated_out_bytes > max_query_size_bytes_) {
      ...
      DLOG(INFO) << "CacheStorageCache::QueryCacheDidReadMetadata (host): estimated_out_bytes > max_query_size_bytes_. cancelling..";
      return;
    }
  } else {
    match->request.reset();
  ...
  if (query_cache_context->query_types & QUERY_CACHE_RESPONSES_WITH_BODIES) {
    ...
    if (entry->GetDataSize(INDEX_RESPONSE_BODY) == 0) {
      DLOG(INFO) << "CacheStorageCache::QueryCacheDidReadMetadata (host): data size = 0 => reading next entry";
      QueryCacheOpenNextEntry(std::move(query_cache_context));
      return;
    }

    if (!blob_storage_context_) {
      DLOG(INFO) << "CacheStorageCache::QueryCacheDidReadMetadata (host): !blob_storage_context_";
      std::move(query_cache_context->callback)
...
void CacheStorageCache::MatchDidMatchAll(
    ...
    std::vector<common::ServiceWorkerResponse> match_all_responses) {
  ...
  std::unique_ptr<common::ServiceWorkerResponse> response =
      std::make_unique<common::ServiceWorkerResponse>(match_all_responses[0]);
...
void CacheStorageCache::MatchAllDidQueryCache(
  ...
  if (error != CacheStorageError::kSuccess) {
    std::move(callback).Run(error, std::vector<common::ServiceWorkerResponse>());
    return;
  }

  std::vector<common::ServiceWorkerResponse> out_responses;
...
void CacheStorageCache::WriteSideDataDidReadMetaData(
  ...
  DLOG(INFO) << "CacheStorageCache::WriteSideDataDidReadMetaData (host)";
  if (!headers ||
      headers->response().response_time() !=
          expected_response_time.ToInternalValue()) {
    DLOG(INFO) << "CacheStorageCache::WriteSideDataDidReadMetaData (host): NO HEADERS or response time not the expected one";
    std::move(callback).Run(CacheStorageError::kErrorNotFound);
    return;
  }
  ...
  std::unique_ptr<proto::CacheResponse> response(
...
void CacheStorageCache::WriteSideDataDidWrite(
    ...
    int rv) {
  DLOG(INFO) << "CacheStorageCache::WriteSideDataDidWrite (host): rv = " << rv;
...
void CacheStorageCache::Put(const common::CacheStorageBatchOperation& operation,
                            ErrorCallback callback) {
  DLOG(INFO) << "CacheStorageCache::Put (host)";
  DCHECK(BACKEND_OPEN == backend_state_ || initializing_);
  DCHECK_EQ(common::CACHE_STORAGE_CACHE_OPERATION_TYPE_PUT, operation.operation_type);

  std::unique_ptr<common::ServiceWorkerFetchRequest> request(
      new common::ServiceWorkerFetchRequest(
  ...
  std::unique_ptr<common::ServiceWorkerResponse> response =
      std::make_unique<common::ServiceWorkerResponse>(operation.response);
  blink::mojom::BlobPtr blob;
  blink::mojom::BlobPtr side_data_blob;
...
void CacheStorageCache::PutImpl(std::unique_ptr<PutContext> put_context) {
  DLOG(INFO) << "CacheStorageCache::PutImpl (host)";
  DCHECK_NE(BACKEND_UNINITIALIZED, backend_state_);
  if (backend_state_ != BACKEND_OPEN) {
    DLOG(INFO) << "CacheStorageCache::PutImpl (host): backend_state_ != BACKEND_OPEN";
  ...
  auto delete_request = std::make_unique<common::ServiceWorkerFetchRequest>(
      put_context->request->url, "", common::ServiceWorkerHeaderMap(), common::Referrer(),
      false);

  common::CacheStorageCacheQueryParams query_options;
...
void CacheStorageCache::PutDidDeleteEntry(
  ...
  std::unique_ptr<disk_cache::Entry*> scoped_entry_ptr(
      new disk_cache::Entry*());
  disk_cache::Entry** entry_ptr = scoped_entry_ptr.get();
  common::ServiceWorkerFetchRequest* request_ptr = put_context->request.get();
...
void CacheStorageCache::PutDidCreateEntry(
    ...
    std::unique_ptr<PutContext> put_context,
    int rv) {
  DLOG(INFO) << "CacheStorageCache::PutDidCreateEntry (host): rv = " << rv;
  ...
  for (common::ServiceWorkerHeaderMap::const_iterator it =
...
void CacheStorageCache::PutDidWriteHeaders(
    ...
    int expected_bytes,
    int rv) {
  DLOG(INFO) << "CacheStorageCache::PutDidWriteHeaders (host): rv = " << rv << " expected_bytes = " << expected_bytes;
  if (rv != expected_bytes) {
    DLOG(INFO) << "CacheStorageCache::PutDidWriteHeaders (host): rv != expected_bytes cancelling.";
  ...
  if (put_context->response->blob_uuid.empty()) {
    DLOG(INFO) << "CacheStorageCache::PutDidWriteHeaders (host): put_context->response->blob_uuid.empty() = true => UpdateCacheSize() (but dont write)";
...
void CacheStorageCache::PutWriteBlobToCache(
  ...
  DLOG(INFO) << "CacheStorageCache::PutWriteBlobToCache";
  ...
  blob->ReadSideData(base::Bind(&ReadBlobData));
...
void CacheStorageCache::PutDidWriteBlobToCache(
  ...
  DLOG(INFO) << "CacheStorageCache::PutDidWriteBlobToCache (host): ok ? " << success;
...
void CacheStorageCache::UpdateCacheSize(base::OnceClosure callback) {
  DLOG(INFO) << "CacheStorageCache::UpdateCacheSize (host)";
  if (backend_state_ != BACKEND_OPEN)
    return;
...
void CacheStorageCache::Delete(const common::CacheStorageBatchOperation& operation,
                               ErrorCallback callback) {
  DCHECK(BACKEND_OPEN == backend_state_ || initializing_);
  DCHECK_EQ(common::CACHE_STORAGE_CACHE_OPERATION_TYPE_DELETE,
...
void CacheStorageCache::KeysImpl(
    ...
    const common::CacheStorageCacheQueryParams& options,
    RequestsCallback callback) {
  DLOG(INFO) << "CacheStorageCache::KeysImpl(host)";    
  DCHECK_NE(BACKEND_UNINITIALIZED, backend_state_);
  if (backend_state_ != BACKEND_OPEN) {
    DLOG(ERROR) << "CacheStorageCache::KeysImpl(host): BACKEND_OPEN = false!";    
    std::move(callback).Run(CacheStorageError::kErrorStorage, nullptr);
    return;
  }
...
void CacheStorageCache::PopulateResponseBody(disk_cache::ScopedEntryPtr entry,
                                             common::ServiceWorkerResponse* response) {
  ...
  DLOG(INFO) << "CacheStorageCache::PopulateResponseBody(host): size = " << response->blob_size;    
  auto blob_data =
