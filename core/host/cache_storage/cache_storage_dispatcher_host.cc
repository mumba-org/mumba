...
bool OriginCanAccessCacheStorage(const url::Origin& origin) {
  return true;//!origin.unique() && common::IsOriginSecure(origin.GetURL());
}
...
class CacheStorageDispatcherHost::CacheImpl
 ...
 public:
  ...
  void Match(const common::ServiceWorkerFetchRequest& request,
    ...
    host::CacheStorageCache* cache = cache_handle_.value();
    if (!cache) {
    ...
    auto scoped_request = std::make_unique<common::ServiceWorkerFetchRequest>(
  ...
  void MatchAll(const common::ServiceWorkerFetchRequest& request,
                const common::CacheStorageCacheQueryParams& match_params,
  ...
  void Keys(const common::ServiceWorkerFetchRequest& request,
            const common::CacheStorageCacheQueryParams& match_params,
            KeysCallback callback) override {
    DLOG(INFO) << "CacheStorageDispatcherHost::CacheImpl::Keys(host)";
    host::CacheStorageCache* cache = cache_handle_.value();
    if (!cache) { 
      DLOG(ERROR) << "CacheStorageDispatcherHost::CacheImpl::Keys: CACHE NOT FOUND";
      std::move(callback).Run(blink::mojom::CacheKeysResult::NewStatus(
          blink::mojom::CacheStorageError::kErrorNotFound));
      return;
    }

    auto request_ptr = std::make_unique<common::ServiceWorkerFetchRequest>(
        request.url, request.method, request.headers, request.referrer,
        request.is_reload);
  ...
  void OnCacheKeysCallback(
      ...
      CacheStorageCacheHandle cache_handle,
      blink::mojom::CacheStorageError error,
      std::unique_ptr<host::CacheStorageCache::Requests> requests) {
    if (error != blink::mojom::CacheStorageError::kSuccess) {
  ...
  void Batch(const std::vector<common::CacheStorageBatchOperation>& batch_operations,
...
void CacheStorageDispatcherHost::Init(CacheStorageContextImpl* context) {
  ...
  DLOG(INFO) << "CacheStorageDispatcherHost::Init(host)";
  if (HostThread::CurrentlyOn(HostThread::IO)) {
    CreateCacheListener(context);
  } else {
    HostThread::PostTask(
...
void CacheStorageDispatcherHost::Open(
  ...
  DLOG(INFO) << "CacheStorageDispatcherHost::Open(host)";
...
void CacheStorageDispatcherHost::Delete(
  ...
  DLOG(INFO) << "CacheStorageDispatcherHost::Delete(host)";
  url::Origin origin = bindings_.dispatch_context();
  if (!OriginCanAccessCacheStorage(origin)) {
    bindings_.ReportBadMessage("CSDH_INVALID_ORIGIN");
    return;
...
void CacheStorageDispatcherHost::Keys(
  ...
  DLOG(INFO) << "CacheStorageDispatcherHost::Keys(host)";
  url::Origin origin = bindings_.dispatch_context();
...
void CacheStorageDispatcherHost::Match(
  ...
  DLOG(INFO) << "CacheStorageDispatcherHost::Match(host)";
  url::Origin origin = bindings_.dispatch_context();
  if (!OriginCanAccessCacheStorage(origin)) {
    bindings_.ReportBadMessage("CSDH_INVALID_ORIGIN");
    return;
...
void CacheStorageDispatcherHost::OnKeysCallback(
  ...
  DLOG(INFO) << "CacheStorageDispatcherHost::OnKeysCallback(host)";
...
void CacheStorageDispatcherHost::AddBinding(
  ...
  DLOG(INFO) << "\nCacheStorageDispatcherHost::AddBinding (host) \n\n";
  DCHECK_CURRENTLY_ON(HostThread::IO);
  //bindings_.AddBinding(this, std::move(request), origin);
}

void CacheStorageDispatcherHost::AddAssociatedBinding(blink::mojom::CacheStorageAssociatedRequest request, const url::Origin& origin) {
  bindings_.AddBinding(this, std::move(request), origin);
}
