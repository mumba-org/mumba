...
#include "content/test/test_content_host_client.h"
#include "storage/host/fileapi/file_permission_policy.h"
#include "storage/host/fileapi/file_system_url.h"
...
class ChildProcessSecurityPolicyTestHostClient
    : public TestContentHostClient {
 public:
  ChildProcessSecurityPolicyTestHostClient() {}
...
class ChildProcessSecurityPolicyTest : public testing::Test {
 public:
  ChildProcessSecurityPolicyTest() : old_host_client_(nullptr) {}

  void SetUp() override {
    old_host_client_ = SetHostClientForTesting(&test_host_client_);
    ...
    test_host_client_.AddScheme(kChromeUIScheme);

    // Claim to always handle file:// URLs like the host would.
    // net::URLRequest::IsHandledURL() no longer claims support for default
    // protocols as this is the responsibility of the host (which is
    // responsible for adding the appropriate ProtocolHandler).
    test_host_client_.AddScheme(url::kFileScheme);
  }

  void TearDown() override {
    test_host_client_.ClearSchemes();
    SetHostClientForTesting(old_host_client_);
  }

 protected:
  void RegisterTestScheme(const std::string& scheme) {
    test_host_client_.AddScheme(scheme);
  }
 ...
 private:
  ChildProcessSecurityPolicyTestHostClient test_host_client_;
  ContentHostClient* old_host_client_;
};
...
TEST_F(ChildProcessSecurityPolicyTest, RegisterWebSafeSchemeTest) {
  ...
  // Currently, "asdf" is destined for HostExecute, so it is allowed to be
...
TEST_F(ChildProcessSecurityPolicyTest, AddIsolatedOrigins) {
  ...
  // warning and 2) won't CHECK or crash the host process, 3) will not add
