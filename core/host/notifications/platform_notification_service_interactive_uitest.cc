...
#include "core/host/engagement/site_engagement_score.h"
...
class PlatformNotificationServiceBrowserTest : public InProcessBrowserTest {
 ...
 protected:
  ...
  double GetEngagementScore(const GURL& origin) const {
    return SiteEngagementService::Get(browser()->domain())->GetScore(origin);
  }
  ...
  WebContents* GetActiveWebContents(Browser* browser) {
...
IN_PROC_BROWSER_TEST_F(PlatformNotificationServiceBrowserTest,
  ...
  Browser* other_browser = CreateBrowser(browser()->domain());
...
IN_PROC_BROWSER_TEST_F(PlatformNotificationServiceBrowserTest,
  ...
  NotificationDisplayServiceImpl* display_service =
      NotificationDisplayServiceImpl::GetForProfile(browser()->domain());
  NotificationHandler* handler = display_service->GetNotificationHandler(
      NotificationHandler::Type::WEB_PERSISTENT);
  ASSERT_TRUE(handler);

  base::RunLoop run_loop;
  handler->OnClick(browser()->domain(), notifications[0].origin_url(),
