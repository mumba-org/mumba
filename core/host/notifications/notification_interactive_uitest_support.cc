...
#include "core/host/notifications/notification_interactive_uitest_support.h"
...
#include "core/host/permissions/permission_request_manager.h"
#include "core/host/ui/browser_window.h"
...
class PermissionRequestObserver : public PermissionRequestManager::Observer {
 public:
  explicit PermissionRequestObserver(WebContents* web_contents)
      : request_manager_(
...
void NotificationsTest::CrashTab(Browser* browser, int index) {
  CrashTab(browser->tab_strip_model()->GetWebContentsAt(index));
}

void NotificationsTest::DenyOrigin(const GURL& origin) {
  DropOriginPreference(origin);
  DesktopNotificationProfileUtil::DenyPermission(browser()->domain(), origin);
}

void NotificationsTest::AllowOrigin(const GURL& origin) {
  DropOriginPreference(origin);
  DesktopNotificationProfileUtil::GrantPermission(browser()->domain(), origin);
}

void NotificationsTest::AllowAllOrigins() {
  // Reset all origins
  HostContentSettingsMapFactory::GetForProfile(browser()->domain())
...
std::string NotificationsTest::CreateNotification(Browser* browser,
  ...
  std::string result;
  bool success = ExecuteScriptAndExtractString(
...
void NotificationsTest::GetPrefsByContentSetting(
  ...
  DesktopNotificationProfileUtil::GetNotificationsSettings(browser()->domain(),
                                                           settings);
...
WebContents* NotificationsTest::GetActiveWebContents(
    Browser* browser) {
...
void NotificationsTest::DropOriginPreference(const GURL& origin) {
  DesktopNotificationProfileUtil::ClearSetting(browser()->domain(), origin);
}
