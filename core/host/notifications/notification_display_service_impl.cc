...
// #include "core/host/extensions/api/notifications/extension_notification_handler.h"
// #endif
...
//#if BUILDFLAG(ENABLE_NATIVE_NOTIFICATIONS)
  ...
  return Host::Instance()->notification_platform_bridge();
#elif defined(OS_WIN)
...
#else
  //if (base::FeatureList::IsEnabled(features::kNativeNotifications) &&
   //   Host::Instance()->notification_platform_bridge()) {
    return Host::Instance()->notification_platform_bridge();
  //}
#endif
...
std::unique_ptr<NotificationPlatformBridge> CreateMessageCenterBridge(
    Domain* domain) {
#if BUILDFLAG(ENABLE_MESSAGE_CENTER)
  return std::make_unique<NotificationPlatformBridgeMessageCenter>(domain);
#else
  return nullptr;
#endif
}
...
NotificationDisplayServiceImpl* NotificationDisplayServiceImpl::GetForProfile(
    Domain* domain) {
  return static_cast<NotificationDisplayServiceImpl*>(NotificationDisplayServiceFactory::GetForDomain(domain));
}

NotificationDisplayServiceImpl::NotificationDisplayServiceImpl(Domain* domain)
    : profile_(domain),
      message_center_bridge_(CreateMessageCenterBridge(domain)),
...
// Callback to run once the domain has been loaded in order to perform a
...
void NotificationDisplayServiceImpl::ProfileLoadedCallback(
  ...
  if (!domain) {
    ...
    LOG(WARNING) << "Domain not loaded correctly";
    return;
  }

  NotificationDisplayServiceImpl* display_service =
      NotificationDisplayServiceImpl::GetForProfile(domain);
