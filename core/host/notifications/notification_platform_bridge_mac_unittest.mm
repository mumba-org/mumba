...
#include "core/host/notifications/notification_platform_bridge_mac.h"
#include "core/host/notifications/notification_test_util.h"
#include "core/host/notifications/stub_alert_dispatcher_mac.h"
#include "core/host/notifications/stub_notification_center_mac.h"
#include "core/host/ui/cocoa/notifications/notification_builder_mac.h"
#include "core/host/ui/cocoa/notifications/notification_constants_mac.h"
#include "core/host/ui/cocoa/notifications/notification_response_builder_mac.h"
  ...
  NSUserNotification* BuildNotification() {
    ...
    [builder
        setProfileId:base::SysUTF8ToNSString(
                         NotificationPlatformBridge::GetProfileId(domain()))];
    [builder setIncognito:domain()->IsOffTheRecord()];
...
TEST_F(NotificationPlatformBridgeMacTest, TestGetDisplayed) {
  ...
  bridge->GetDisplayed(
      domain(), base::Bind(&StoreNotificationCount, &notification_count));
...
TEST_F(NotificationPlatformBridgeMacTest, TestDisplayAlert) {
  ...
  std::unique_ptr<NotificationPlatformBridgeMac> bridge(
      new NotificationPlatformBridgeMac(notification_center(),
                                        alert_dispatcher()));
  bridge->Display(NotificationHandler::Type::WEB_PERSISTENT, domain(), *alert,
                  nullptr);
  EXPECT_EQ(0u, [[notification_center() deliveredNotifications] count]);
