...
#include "core/host/notifications/notification_platform_bridge_linux.h"
...
class NotificationPlatformBridgeLinuxImpl
 ...
 public:
  explicit NotificationPlatformBridgeLinuxImpl(scoped_refptr<dbus::Bus> bus)
      : bus_(bus) {
    DCHECK_CURRENTLY_ON(HostThread::UI);
    task_runner_ = host::GetDBusTaskRunner();
    registrar_.Add(this, host::NOTIFICATION_APP_TERMINATING,
  ...
  void Display(
    ...
    task_runner_->PostTask(
        ...
        base::BindOnce(
            ...
            notification_type, GetProfileId(domain), false, //domain->IsOffTheRecord(),
  ...
  void Close(Domain* domain, const std::string& notification_id) override {
    ...
    task_runner_->PostTask(
        ...
        base::BindOnce(&NotificationPlatformBridgeLinuxImpl::CloseOnTaskRunner,
                       this, GetProfileId(domain), notification_id));
  }
  ...
  void GetDisplayed(Domain* domain,
    ...
    task_runner_->PostTask(
        ...
        base::BindOnce(
            ...
            this, GetProfileId(domain), false,//domain->IsOffTheRecord(),
            std::move(callback)));
  }
 ...
 private:
  ...
  void Observe(int type,
    ...
    DCHECK_CURRENTLY_ON(HostThread::UI);
    DCHECK_EQ(host::NOTIFICATION_APP_TERMINATING, type);
...
void NotificationPlatformBridgeLinux::Display(
  ...
  impl_->Display(notification_type, domain, notification, std::move(metadata));
}

void NotificationPlatformBridgeLinux::Close(
    Domain* domain,
    const std::string& notification_id) {
  impl_->Close(domain, notification_id);
}

void NotificationPlatformBridgeLinux::GetDisplayed(
    Domain* domain,
    GetDisplayedNotificationsCallback callback) const {
  impl_->GetDisplayed(domain, std::move(callback));
}
