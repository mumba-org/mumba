...
#include "core/host/notifications/notification_image_retainer.h"
#include "core/host/notifications/notification_launch_id.h"
#include "core/host/notifications/notification_platform_bridge_win_metrics.h"
#include "core/host/notifications/notification_platform_bridge_win_util.h"
#include "core/host/notifications/notification_template_builder.h"
...
class NotificationPlatformBridgeWinImpl
    : public base::RefCountedThreadSafe<NotificationPlatformBridgeWinImpl>,
      public BrowserThread::DeleteOnThread<
          BrowserThread::UI> {
 public:
  ...
  HRESULT GetToastNotification(
    ...
    *toast_notification = nullptr;
    ...
    // features providing a sufficiently distinct notification id, domain id,
...
void NotificationPlatformBridgeWin::Display(
  ...
  task_runner_->PostTask(
      ...
      base::BindOnce(&NotificationPlatformBridgeWinImpl::Display, impl_,
                     notification_type, GetProfileId(domain),
                     domain->IsOffTheRecord(), std::move(notification_copy),
                     std::move(metadata)));
}

void NotificationPlatformBridgeWin::Close(Domain* domain,
                                          const std::string& notification_id) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&NotificationPlatformBridgeWinImpl::Close,
                                impl_, GetProfileId(domain),
                                domain->IsOffTheRecord(), notification_id));
}
...
void NotificationPlatformBridgeWin::GetDisplayed(
  ...
  task_runner_->PostTask(
      ...
      base::BindOnce(&NotificationPlatformBridgeWinImpl::GetDisplayed, impl_,
                     GetProfileId(domain), domain->IsOffTheRecord(),
                     std::move(callback)));
}
