...
class MessageCenterNotificationManager
 ...
 public:
  ...
  // center, for this combination of delegate id and domain.
  std::string GetMessageCenterNotificationIdForTest(
      const std::string& delegate_id, Domain* domain);
 ...
 private:
  ...
  std::map<std::string, std::unique_ptr<WorkspaceNotification>>
      domain_notifications_;

  // Helpers that add/remove the notification from local map.
  // The local map takes ownership of domain_notification object.
  void AddWorkspaceNotification(
      std::unique_ptr<WorkspaceNotification> domain_notification);
  void RemoveWorkspaceNotification(const std::string& notification_id);

  // Returns the WorkspaceNotification for the |id|, or NULL if no such
  // notification is found.
  WorkspaceNotification* FindWorkspaceNotification(const std::string& id) const;
