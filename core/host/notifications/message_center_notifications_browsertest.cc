...
class MessageCenterNotificationsTest : public InProcessBrowserTest {
 public:
  ...
  Domain* domain() { return browser()->domain(); }
...
IN_PROC_BROWSER_TEST_F(MessageCenterNotificationsTest, BasicAddCancel) {
  // Someone may create system notifications like "you're in multi-domain
  ...
  manager()->CancelAll();
  manager()->Add(CreateTestNotification("hey"), domain());
  EXPECT_EQ(1u, message_center()->NotificationCount());
  manager()->CancelById("hey", NotificationUIManager::GetProfileID(domain()));
  EXPECT_EQ(0u, message_center()->NotificationCount());
}

IN_PROC_BROWSER_TEST_F(MessageCenterNotificationsTest, BasicDelegate) {
  TestDelegate* delegate;
  manager()->Add(CreateTestNotification("hey", &delegate), domain());
  manager()->CancelById("hey", NotificationUIManager::GetProfileID(domain()));
...
IN_PROC_BROWSER_TEST_F(MessageCenterNotificationsTest, ButtonClickedDelegate) {
  TestDelegate* delegate;
  manager()->Add(CreateTestNotification("n", &delegate), domain());
  const std::string notification_id =
      manager()->GetMessageCenterNotificationIdForTest("n", domain());
...
IN_PROC_BROWSER_TEST_F(MessageCenterNotificationsTest,
  ...
  manager()->Add(CreateRichTestNotification("n", &delegate2), domain());

  manager()->CancelById("n", NotificationUIManager::GetProfileID(domain()));
...
IN_PROC_BROWSER_TEST_F(MessageCenterNotificationsTest, QueueWhenCenterVisible) {
  ...
  manager()->Add(CreateTestNotification("n2", &delegate2), domain());
  const std::string id_n2 =
      manager()->GetMessageCenterNotificationIdForTest("n2", domain());
...
IN_PROC_BROWSER_TEST_F(MessageCenterNotificationsTest,
  ...
  manager()->Add(notification, domain());
  const std::string notification_id =
      manager()->GetMessageCenterNotificationIdForTest("n", domain());
  ...
  manager()->Update(notification, domain());
...
IN_PROC_BROWSER_TEST_F(MessageCenterNotificationsTest, VerifyKeepAlives) {
  ...
  TestDelegate* delegate;
  manager()->Add(CreateTestNotification("a", &delegate), domain());
  RunLoopUntilIdle();
  EXPECT_TRUE(KeepAliveRegistry::GetInstance()->IsOriginRegistered(
      KeepAliveOrigin::NOTIFICATION));

  TestDelegate* delegate2;
  manager()->Add(CreateRichTestNotification("b", &delegate2), domain());
  RunLoopUntilIdle();
  EXPECT_TRUE(KeepAliveRegistry::GetInstance()->IsOriginRegistered(
      KeepAliveOrigin::NOTIFICATION));

  manager()->CancelById("a", NotificationUIManager::GetProfileID(domain()));
  RunLoopUntilIdle();
  EXPECT_TRUE(KeepAliveRegistry::GetInstance()->IsOriginRegistered(
      KeepAliveOrigin::NOTIFICATION));

  manager()->CancelById("b", NotificationUIManager::GetProfileID(domain()));
  RunLoopUntilIdle();
  EXPECT_FALSE(KeepAliveRegistry::GetInstance()->IsOriginRegistered(
      KeepAliveOrigin::NOTIFICATION));
