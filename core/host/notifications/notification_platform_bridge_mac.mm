...
#import "core/host/ui/cocoa/notifications/notification_delivery.h"
#import "core/host/ui/cocoa/notifications/notification_response_builder_mac.h"
...
// Loads the domain and process the Notification response
...
void NotificationPlatformBridgeMac::Display(
  ...
  [builder setProfileId:base::SysUTF8ToNSString(GetProfileId(domain))];
...
void NotificationPlatformBridgeMac::Close(Domain* domain,
                                          const std::string& notification_id) {
  NSString* candidate_id = base::SysUTF8ToNSString(notification_id);
  NSString* current_profile_id = base::SysUTF8ToNSString(GetProfileId(domain));
...
void NotificationPlatformBridgeMac::GetDisplayed(
  ...
  [alert_dispatcher_ getDisplayedAlertsForProfileId:base::SysUTF8ToNSString(
                                                        GetProfileId(domain))
                                          incognito:domain->IsOffTheRecord()
