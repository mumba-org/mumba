...
#include "core/host/notifications/notification_database_data_conversions.h"
...
#include "core/host/notifications/notification_database_data.pb.h"
...
bool DeserializeNotificationDatabaseData(const std::string& input,
  ...
  common::PlatformNotificationData* notification_data = &output->notification_data;
  ...
  switch (payload.direction()) {
    ...
    case NotificationDatabaseDataProto::NotificationData::AUTO:
      notification_data->direction = common::PlatformNotificationData::DIRECTION_AUTO;
      break;
  }
  ...
  for (const auto& payload_action : payload.actions()) {
    common::PlatformNotificationAction action;

    switch (payload_action.type()) {
      case NotificationDatabaseDataProto::NotificationAction::BUTTON:
        action.type = common::PLATFORM_NOTIFICATION_ACTION_TYPE_BUTTON;
        break;
      case NotificationDatabaseDataProto::NotificationAction::TEXT:
        action.type = common::PLATFORM_NOTIFICATION_ACTION_TYPE_TEXT;
        break;
      default:
        NOTREACHED();
    }
...
bool SerializeNotificationDatabaseData(const NotificationDatabaseData& input,
  ...
  const common::PlatformNotificationData& notification_data = input.notification_data;
  ...
  for (const common::PlatformNotificationAction& action : notification_data.actions) {
