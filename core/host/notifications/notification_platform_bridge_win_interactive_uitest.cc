...
#include "core/host/notifications/mock_itoastnotification.h"
#include "core/host/notifications/mock_itoastnotifier.h"
#include "core/host/notifications/notification_display_service_tester.h"
#include "core/host/notifications/notification_launch_id.h"
#include "core/host/notifications/notification_platform_bridge_win.h"
...
Domain* CreateTestingProfile(const base::FilePath& path) {
  ...
  Domain* domain =
      Domain::CreateProfile(path, nullptr, Domain::CREATE_MODE_SYNCHRONOUS);
  profile_manager->RegisterTestingProfile(domain, true, false);
  EXPECT_EQ(starting_number_of_profiles + 1,
            profile_manager->GetNumberOfProfiles());
  return domain;
}

Domain* CreateTestingProfile(const std::string& profile_name) {
  base::FilePath path;
...
IN_PROC_BROWSER_TEST_F(NotificationPlatformBridgeWinUITest, GetDisplayed) {
  ...
  // Add four items (two in each domain, one for each being incognito and one
  // for each that is not).
  bool incognito = true;

  Domain* profile1 = CreateTestingProfile("P1");
  ...
  Domain* profile2 = CreateTestingProfile("P2");
  ...
  // Query for domain P1 in incognito (should return 1 item).
  {
    base::RunLoop run_loop;
    bridge->GetDisplayed(
  ...
  // Query for domain P1 not in incognito (should return 1 item).
  {
    base::RunLoop run_loop;
    bridge->GetDisplayed(
        profile1,
  ...
  // Query for domain P2 in incognito (should return 1 item).
  {
    base::RunLoop run_loop;
    bridge->GetDisplayed(
  ...
  // Query for domain P2 not in incognito (should return 1 item).
  {
    base::RunLoop run_loop;
    bridge->GetDisplayed(
        profile2,
...
IN_PROC_BROWSER_TEST_F(NotificationPlatformBridgeWinUITest, DisplayWithMockAC) {
  ...
  Domain* domain = CreateTestingProfile("P1");

  {
    ...
    bridge->Display(NotificationHandler::Type::WEB_PERSISTENT, domain,
