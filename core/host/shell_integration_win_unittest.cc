...
class DomainIntegrationWinMigrateShortcutTest : public testing::Test {
 protected:
  DomainIntegrationWinMigrateShortcutTest() {}
  ...
  void SetUp() override {
    ...
    chrome_app_id_ = DomainUtil::GetBrowserModelId(true);
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(DomainIntegrationWinMigrateShortcutTest);
};

}  // namespace

TEST_F(DomainIntegrationWinMigrateShortcutTest, ClearDualModeAndAdjustAppIds) {
...
TEST(DomainIntegrationWinTest, GetAppModelIdForProfileTest) {
