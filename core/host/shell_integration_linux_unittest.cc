...
TEST(DomainIntegrationTest, GetDataWriteLocation) {
  content::TestHostThreadBundle test_browser_thread_bundle;
...
TEST(DomainIntegrationTest, GetDataSearchLocations) {
  content::TestHostThreadBundle test_browser_thread_bundle;
...
TEST(DomainIntegrationTest, GetExistingShortcutLocations) {
...
TEST(DomainIntegrationTest, GetExistingShortcutContents) {
...
TEST(DomainIntegrationTest, GetExtensionShortcutFilename) {
...
TEST(DomainIntegrationTest, GetExistingProfileShortcutFilenames) {
  base::FilePath kProfilePath("a/b/c/Profile Name?");
...
TEST(DomainIntegrationTest, GetWebShortcutFilename) {
  const struct {
...
TEST(DomainIntegrationTest, GetDesktopFileContents) {
...
TEST(DomainIntegrationTest, GetDesktopFileContentsAppList) {
  const base::FilePath kChromeExePath("/opt/google/chrome/google-chrome");
...
TEST(DomainIntegrationTest, GetDirectoryFileContents) {
  const struct {
    const char* const title;
    const char* const icon_name;
    const char* const expected_output;
...
TEST(DomainIntegrationTest, WmClass) {
