...
#ifndef MUMBA_HOST_THREAD_H__
#define MUMBA_HOST_THREAD_H__
...
// Use DCHECK_CURRENTLY_ON(HostThread::ID) to assert that a function can only
// be called on the named HostThread.
#define DCHECK_CURRENTLY_ON(thread_identifier)                      \
  (DCHECK(::host::HostThread::CurrentlyOn(thread_identifier)) \
   << ::host::HostThread::GetDCheckCurrentlyOnErrorMessage(   \
          thread_identifier))
...
//
//   HostThread::PostTask(HostThread::IO, FROM_HERE, task);
//
...
class CONTENT_EXPORT HostThread : public base::Thread {
public:
 ...
 static const char* GetThreadName(HostThread::ID identifier);

 // Construct a HostThreadImpl with the supplied identifier.  It is an error
 // to construct a HostThreadImpl that already exists.
 explicit HostThread(ID identifier);
...
// static bool IsMessageLoopValid(ID identifier) WARN_UNUSED_RESULT;
 ...
 //
 // Ownership remains with the HostThread implementation, so you
 // must not delete the pointer.
 //static base::MessageLoop* UnsafeGetMessageLoopForThread(ID identifier);

 // Sets the delegate for the specified HostThread.
 //
 ...
 static void SetDelegate(ID identifier, HostThreadDelegate* delegate);
 ...
 //           Foo, HostThread::DeleteOnIOThread> {
 //
 // ...
 //  private:
 ...
 // std::unique_ptr<Foo, HostThread::DeleteOnIOThread> ptr;

 struct DeleteOnUIThread : public DeleteOnThread<UI> { };
 struct DeleteOnIOThread : public DeleteOnThread<IO> { };
 //struct DeleteOnFileThread : public DeleteOnThread<FILE> { };
 //struct DeleteOnDBThread : public DeleteOnThread<DB> { };
 //struct DeleteONRpcThread : public DeleteOnThread<Rpc> { };
 //struct DeleteOnDispatcherThread : public DeleteOnThread<DISPATCHER> { };
 //struct DeleteOnNotificationThread : public DeleteOnThread<NOTIFICATION> { };
 struct DeleteOnProcessLauncherThread : public DeleteOnThread<PROCESS_LAUNCHER>{};
 //struct DeleteOnWatchdogThread : public DeleteOnThread<WATCHDOG>{};
...
private:
 ...
 //void RpcThreadRun(base::RunLoop* run_loop);
 //void NotificationThreadRun(base::RunLoop* run_loop);
 //void DispatcherThreadRun(base::RunLoop* run_loop);
 void IOThreadRun(base::RunLoop* run_loop);
 void ProcessLauncherThreadRun(base::RunLoop* run_loop);
 //void WatchdogThreadRun(base::RunLoop* run_loop);
 ...
 DISALLOW_COPY_AND_ASSIGN(HostThread);
};
