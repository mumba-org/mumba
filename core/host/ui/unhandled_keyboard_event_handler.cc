...
void UnhandledKeyboardEventHandler::HandleKeyboardEvent(
  ...
  DLOG(INFO) << "UnhandledKeyboardEventHandler::HandleKeyboardEvent";
  if (!focus_manager) {
    NOTREACHED();
    return;
  }
  ...
  if (event.GetType() == blink::WebInputEvent::kChar &&
    ...
    DLOG(INFO) << "UnhandledKeyboardEventHandler::HandleKeyboardEvent: event.GetType() == blink::WebInputEvent::kChar -> ignore_next_char_event_ = false";
    ignore_next_char_event_ = false;
    return;
  }
  ...
  if (event.GetType() == blink::WebInputEvent::kRawKeyDown) {
    DLOG(INFO) << "UnhandledKeyboardEventHandler::HandleKeyboardEvent: event.GetType() == blink::WebInputEvent::kRawKeyDown ->  ui::GetAcceleratorFromNativeWebKeyboardEvent(event)";
    ...
    if (focus_manager->ProcessAccelerator(accelerator)) {
      DLOG(INFO) << "UnhandledKeyboardEventHandler::HandleKeyboardEvent: focus_manager->ProcessAccelerator(accelerator) = TRUE";
      return;
    }
  ...
  if (event.os_event && !event.skip_in_browser) {
    DLOG(INFO) << "UnhandledKeyboardEventHandler::HandleKeyboardEvent: HandleNativeKeyboardEvent(event.os_event, focus_manager)";
