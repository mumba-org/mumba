...
ContentsLayoutManager::~ContentsLayoutManager() {
  // just to silence the compiler, about not being used
...
void ContentsLayoutManager::Layout(views::View* contents_container) {
  ...
  //int devtools_height = height / 3;
  //int contents_height = height - devtools_height;

  // We dont have devtools, so it can fill all the container size
  gfx::Rect new_contents_bounds(0, 0, width, height);
  ...
  // layout here.
  //new_devtools_bounds = host_->GetMirroredRect(new_devtools_bounds);
  new_contents_bounds = host_->GetMirroredRect(new_contents_bounds);
  //new_devtools_bounds.set_height(devtools_height);
  //new_contents_bounds.set_height(contents_height);
  //DLOG(INFO) << "ContentsLayoutManager::Layout: dw: " << new_devtools_bounds.width() << " dh: " << new_devtools_bounds.height() << " cw: " << new_contents_bounds.width() << " ch: " << new_contents_bounds.height();
  //devtools_view_->SetBoundsRect(new_devtools_bounds);
  contents_view_->SetBoundsRect(new_contents_bounds);
  //gfx::Rect bounds_rect = host_->GetMirroredRect(new_contents_bounds);
  //contents_view_->SetBoundsRect(bounds_rect);
}
