...
class DockLayoutProvider : public views::LayoutProvider {
 public:
  DockLayoutProvider();
  ~DockLayoutProvider() override;

  static DockLayoutProvider* Get();
  ...
  virtual int GetCornerRadiusMetric(DockEmphasisMetric emphasis_metric,
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(DockLayoutProvider);
};
