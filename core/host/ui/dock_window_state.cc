...
//#include "core/host/profiles/profile.h"
//#include "core/host/sessions/session_service.h"
//#include "core/host/sessions/session_service_factory.h"
...
std::string GetWindowName(const Dock* dock) {
  if (dock->app_name().empty()) {
    return "dock.window";
  }
  return dock->app_name();
}
...
bool ShouldSaveWindowPlacement(const Dock* dock) {
  ...
  //return (dock->type() == Dock::TYPE_TABBED) ||
  //  ((dock->type() == Dock::TYPE_POPUP) && dock->is_trusted_source());
  return false;
}
...
bool SavedBoundsAreContentBounds(const Dock* dock) {
  ...
  return dock->is_type_popup();// && !dock->is_app() &&
         //!dock->is_trusted_source();
}
...
void SaveWindowPlacement(const Dock* dock,
  ...
  //if (session_service)
  //  session_service->SetWindowBounds(dock->session_id(), bounds, show_state);
}
...
void SaveWindowWorkspace(const Dock* dock, const std::string& workspace) {
  //SessionService* session_service =
      //SessionServiceFactory::GetForProfileIfExisting(dock->profile());
  //if (session_service)
    //session_service->SetWindowWorkspace(dock->session_id(), workspace);
}
...
void GetSavedWindowBoundsAndShowState(const Dock* dock,
  ...
  DCHECK(bounds);
  DCHECK(show_state);
  *bounds = dock->override_bounds();
  WindowSizer::GetDockWindowBoundsAndShowState(dock->app_name(),
                                                  *bounds,
                                                  dock,
                                                  bounds,
                                                  show_state);
