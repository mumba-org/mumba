// Copyright 2015 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "core/host/ui/dock_non_client_frame_view_mus.h"

#include <algorithm>

//#include "chrome/dock/profiles/profiles_state.h"
#include "core/host/themes/theme_properties.h"
#include "core/host/ui/dock.h"
#include "core/host/ui/layout_constants.h"
#include "core/host/ui/dock_frame.h"
//#include "core/host/ui/dock_frame_header_ash.h"
#include "core/host/ui/dock_frame_mus.h"
#include "core/host/ui/dock_window.h"
//#include "core/host/ui/immersive_mode_controller.h"
#include "core/host/ui/top_container_view.h"
//#include "chrome/dock/ui/views/profiles/profile_indicator_icon.h"
#include "core/host/ui/tab_icon_view.h"
#include "core/host/ui/tablist/tablist.h"
//#include "chrome/dock/web_applications/web_app.h"
#include "core/host/application/application_contents.h"
#include "ui/accessibility/ax_node_data.h"
#include "ui/aura/client/aura_constants.h"
#include "ui/aura/window.h"
#include "ui/base/hit_test.h"
#include "ui/base/layout.h"
#include "ui/base/material_design/material_design_controller.h"
#include "ui/base/theme_provider.h"
#include "ui/gfx/canvas.h"
#include "ui/gfx/geometry/rect_conversions.h"
#include "ui/gfx/image/image_skia.h"
#include "ui/views/controls/label.h"
#include "ui/views/mus/desktop_window_tree_host_mus.h"
#include "ui/views/mus/window_manager_frame_values.h"
#include "ui/views/widget/widget.h"
#include "ui/views/widget/widget_delegate.h"

#if defined(OS_CHROMEOS)
#include "ash/ash_layout_constants.h"
#endif

#if !defined(OS_CHROMEOS)
#define FRAME_AVATAR_BUTTON
#endif

namespace host {

namespace {

//#if defined(FRAME_AVATAR_BUTTON)
// Space between the new avatar button and the minimize button.
//const int kAvatarButtonOffset = 5;
//#endif
// Space between right edge of tablist and maximize button.
const int kTablistRightSpacing = 10;
// Height of the shadow in the tab image, used to ensure clicks in the shadow
// area still drag restored windows.  This keeps the clickable area large enough
// to hit easily.
const int kTabShadowHeight = 4;

// #if defined(FRAME_AVATAR_BUTTON)
// // Combines View::ConvertPointToTarget() and View::HitTest() for a given
// // |point|. Converts |point| from |src| to |dst| and hit tests it against |dst|.
// bool ConvertedHitTest(views::View* src,
//                       views::View* dst,
//                       const gfx::Point& point) {
//   DCHECK(src);
//   DCHECK(dst);
//   gfx::Point converted_point(point);
//   views::View::ConvertPointToTarget(src, dst, &converted_point);
//   return dst->HitTestPoint(converted_point);
// }
// #endif

const views::WindowManagerFrameValues& frame_values() {
  return views::WindowManagerFrameValues::instance();
}

}  // namespace

///////////////////////////////////////////////////////////////////////////////
// DockNonClientFrameViewMus, public:

// static
const char DockNonClientFrameViewMus::kViewClassName[] =
    "DockNonClientFrameViewMus";

DockNonClientFrameViewMus::DockNonClientFrameViewMus(
    DockFrame* frame,
    DockWindow* dock_window)
    : DockNonClientFrameView(frame, dock_window),
      window_icon_(nullptr),
      tablist_(nullptr) {
}

DockNonClientFrameViewMus::~DockNonClientFrameViewMus() {
  if (tablist_) {
    tablist_->RemoveObserver(this);
    tablist_ = nullptr;
  }
}

void DockNonClientFrameViewMus::Init() {
  // Initializing the TabIconView is expensive, so only do it if we need to.
  if (dock_window()->ShouldShowWindowIcon()) {
    window_icon_ = new TabIconView(this, nullptr);
    window_icon_->set_is_light(true);
    AddChildView(window_icon_);
    window_icon_->Update();
  }
}

///////////////////////////////////////////////////////////////////////////////
// DockNonClientFrameView:

void DockNonClientFrameViewMus::OnDockWindowInitViewsComplete() {
  DCHECK(dock_window()->tablist());
  DCHECK(!tablist_);
  tablist_ = dock_window()->tablist();
  tablist_->AddObserver(this);
}

gfx::Rect DockNonClientFrameViewMus::GetBoundsForTablist(
    views::View* tablist) const {
  if (!tablist || !dock_window()->top_container()->visible())
    return gfx::Rect();

  int left_inset = GetTablistLeftInset();
  int right_inset = GetTablistRightInset();
  const gfx::Rect bounds(left_inset, GetTopInset(false),
                         std::max(0, width() - left_inset - right_inset),
                         tablist->GetPreferredSize().height());
  return bounds;
}

int DockNonClientFrameViewMus::GetTopInset(bool restored) const {
  if (!ShouldPaint()) {
    // When immersive fullscreen unrevealed, tablist is offscreen with normal
    // tapstrip bounds, the top inset should reach this topmost edge.
    //const ImmersiveModeController* const immersive_controller =
    //    dock_window()->immersive_mode_controller();
    //if (immersive_controller->IsEnabled() &&
    //    !immersive_controller->IsRevealed()) {
    //  return (-1) * dock_window()->GetTablistHeight();
    //}
    return 0;
  }

  const int header_height = GetHeaderHeight();

  if (dock_window()->IsTablistVisible())
    return header_height - dock_window()->GetTablistHeight();

  return header_height;
}

int DockNonClientFrameViewMus::GetThemeBackgroundXInset() const {
  return 5;
}

void DockNonClientFrameViewMus::UpdateThrobber(bool running) {
  if (window_icon_)
    window_icon_->Update();
}

void DockNonClientFrameViewMus::UpdateClientArea() {
  std::vector<gfx::Rect> additional_client_area;
  //int top_container_offset = 0;
//  ImmersiveModeController* immersive_mode_controller =
      //dock_window()->immersive_mode_controller();
  // Frame decorations (the non-client area) are visible if not in immersive
  // mode, or in immersive mode *and* the reveal widget is showing.
  const bool show_frame_decorations = true;//!immersive_mode_controller->IsEnabled() ||
                                      //immersive_mode_controller->IsRevealed();
  if (dock_window()->IsTablistVisible() && show_frame_decorations) {
    gfx::Rect tablist_bounds(GetBoundsForTablist(tablist_));

    int tablist_max_x = tablist_->GetMaxX();
    if (!tablist_bounds.IsEmpty() && tablist_max_x) {
      tablist_bounds.set_width(tablist_max_x);
      //if (immersive_mode_controller->IsEnabled()) {
      //  top_container_offset =
      //      immersive_mode_controller->GetTopContainerVerticalOffset(
      //          dock_window()->top_container()->size());
      //  tablist_bounds.set_y(tablist_bounds.y() + top_container_offset);
      //  tablist_bounds.Intersect(gfx::Rect(size()));
      //}
      additional_client_area.push_back(tablist_bounds);
    }
  }
  aura::WindowTreeHostMus* window_tree_host_mus =
      static_cast<aura::WindowTreeHostMus*>(
          GetWidget()->GetNativeWindow()->GetHost());
  if (show_frame_decorations) {
    const int header_height = GetHeaderHeight();
    gfx::Insets client_area_insets =
        views::WindowManagerFrameValues::instance().normal_insets;
    client_area_insets.Set(header_height, client_area_insets.left(),
                           client_area_insets.bottom(),
                           client_area_insets.right());
    window_tree_host_mus->SetClientArea(client_area_insets,
                                        additional_client_area);
    //views::Widget* reveal_widget = immersive_mode_controller->GetRevealWidget();
    //if (reveal_widget) {
      // In immersive mode the reveal widget needs the same client area as
      // the Dock widget. This way mus targets the window manager (ash) for
      // clicks in the frame decoration.
//      static_cast<aura::WindowTreeHostMus*>(
          //reveal_widget->GetNativeWindow()->GetHost())
          //->SetClientArea(client_area_insets, additional_client_area);
    //}
  } else {
    window_tree_host_mus->SetClientArea(gfx::Insets(), additional_client_area);
  }
}

void DockNonClientFrameViewMus::UpdateMinimumSize() {
  gfx::Size min_size = GetMinimumSize();
  aura::Window* frame_window = frame()->GetNativeWindow();
  const gfx::Size* previous_min_size =
      frame_window->GetProperty(aura::client::kMinimumSize);
  if (!previous_min_size || *previous_min_size != min_size) {
    frame_window->SetProperty(aura::client::kMinimumSize,
                              new gfx::Size(min_size));
  }
}

int DockNonClientFrameViewMus::GetTablistLeftInset() const {
  return DockNonClientFrameView::GetTablistLeftInset() +
         frame_values().normal_insets.left();
}

///////////////////////////////////////////////////////////////////////////////
// views::NonClientFrameView:

gfx::Rect DockNonClientFrameViewMus::GetBoundsForClientView() const {
  // The ClientView must be flush with the top edge of the widget so that the
  // web contents can take up the entire screen in immersive fullscreen (with
  // or without the top-of-window views revealed). When in immersive fullscreen
  // and the top-of-window views are revealed, the TopContainerView paints the
  // window header by redirecting paints from its background to
  // DockNonClientFrameViewMus.
  return bounds();
}

gfx::Rect DockNonClientFrameViewMus::GetWindowBoundsForClientBounds(
    const gfx::Rect& client_bounds) const {
  return client_bounds;
}

int DockNonClientFrameViewMus::NonClientHitTest(const gfx::Point& point) {
  // TODO(sky): figure out how this interaction should work.
  int hit_test = HTCLIENT;

//#if defined(FRAME_AVATAR_BUTTON)
//  views::View* profile_switcher_view = GetProfileSwitcherButton();
//  if (hit_test == HTCAPTION && profile_switcher_view &&
      //ConvertedHitTest(this, profile_switcher_view, point)) {
    //return HTCLIENT;
  //}
//#endif

  // When the window is restored we want a large click target above the tabs
  // to drag the window, so redirect clicks in the tab's shadow to caption.
  if (hit_test == HTCLIENT &&
      !(frame()->IsMaximized() || frame()->IsFullscreen())) {
    // Convert point to client coordinates.
    gfx::Point client_point(point);
    View::ConvertPointToTarget(this, frame()->client_view(), &client_point);
    // Report hits in shadow at top of tablist as caption.
    gfx::Rect tablist_bounds(dock_window()->tablist()->bounds());
    if (client_point.y() < tablist_bounds.y() + kTabShadowHeight)
      hit_test = HTCAPTION;
  }
  return hit_test;
}

void DockNonClientFrameViewMus::GetWindowMask(const gfx::Size& size,
                                              gfx::Path* window_mask) {
  // Aura does not use window masks.
}

void DockNonClientFrameViewMus::ResetWindowControls() {}

void DockNonClientFrameViewMus::UpdateWindowIcon() {
  if (window_icon_)
    window_icon_->SchedulePaint();
}

void DockNonClientFrameViewMus::UpdateWindowTitle() {}

void DockNonClientFrameViewMus::SizeConstraintsChanged() {}

///////////////////////////////////////////////////////////////////////////////
// views::View:

void DockNonClientFrameViewMus::OnPaint(gfx::Canvas* canvas) {
  if (!ShouldPaint())
    return;

  //if (dock_window()->IsToolbarVisible())
  //  PaintToolbarBackground(canvas);
  else if (!UseContainerdAppHeaderStyle())
    PaintContentEdge(canvas);
}

void DockNonClientFrameViewMus::Layout() {
  //if (profile_indicator_icon())
  //  LayoutIncognitoButton();

//#if defined(FRAME_AVATAR_BUTTON)
//  if (GetProfileSwitcherButton())
//    LayoutProfileSwitcher();
//#endif

  DockNonClientFrameView::Layout();

  UpdateClientArea();
}

const char* DockNonClientFrameViewMus::GetClassName() const {
  return kViewClassName;
}

void DockNonClientFrameViewMus::GetAccessibleNodeData(
    ui::AXNodeData* node_data) {
  node_data->role = ax::mojom::Role::kTitleBar;
}

gfx::Size DockNonClientFrameViewMus::GetMinimumSize() const {
  gfx::Size min_client_view_size(frame()->client_view()->GetMinimumSize());
  const int min_frame_width = frame_values().max_title_bar_button_width +
                              frame_values().normal_insets.width();
  int min_width = std::max(min_frame_width, min_client_view_size.width());
  if (dock_window()->IsTablistVisible()) {
    // Ensure that the minimum width is enough to hold a minimum width tab strip
    // at its usual insets.
    int min_tablist_width =
        dock_window()->tablist()->GetMinimumSize().width();
    min_width =
        std::max(min_width, min_tablist_width + GetTablistLeftInset() +
                                GetTablistRightInset());
  }
  return gfx::Size(min_width, min_client_view_size.height());
}

///////////////////////////////////////////////////////////////////////////////
// TabIconViewModel:

bool DockNonClientFrameViewMus::ShouldTabIconViewAnimate() const {
  // This function is queried during the creation of the window as the
  // TabIconView we host is initialized, so we need to null check the selected
  // ApplicationContents because in this condition there is not yet a selected tab.
  ApplicationContents* current_tab = dock_window()->GetActiveApplicationContents();
  return current_tab ? current_tab->IsLoading() : false;
}

gfx::ImageSkia DockNonClientFrameViewMus::GetFaviconForTabIconView() {
  views::WidgetDelegate* delegate = frame()->widget_delegate();
  if (!delegate)
    return gfx::ImageSkia();
  return delegate->GetWindowIcon();
}
///////////////////////////////////////////////////////////////////////////////
// DockNonClientFrameViewMus, protected:

// DockNonClientFrameView:
// AvatarButtonStyle DockNonClientFrameViewMus::GetAvatarButtonStyle() const {
// #if defined(FRAME_AVATAR_BUTTON)
//   return AvatarButtonStyle::NATIVE;
// #else
//   return AvatarButtonStyle::NONE;
// #endif
// }

///////////////////////////////////////////////////////////////////////////////
// DockNonClientFrameViewMus, private:

void DockNonClientFrameViewMus::TablistMaxXChanged(Tablist* tablist) {
  UpdateClientArea();
}

void DockNonClientFrameViewMus::TablistDeleted(Tablist* tablist) {
  tablist_->RemoveObserver(this);
  tablist_ = nullptr;
}

int DockNonClientFrameViewMus::GetTablistRightInset() const {
  const int frame_right_insets = frame_values().normal_insets.right() +
                                 frame_values().max_title_bar_button_width;
  int right_inset = kTablistRightSpacing + frame_right_insets;

//#if defined(FRAME_AVATAR_BUTTON)
//  views::View* profile_switcher_view = GetProfileSwitcherButton();
//  if (profile_switcher_view) {
    //right_inset +=
        //kAvatarButtonOffset + profile_switcher_view->GetPreferredSize().width();
  //}
//#endif

  return right_inset;
}

bool DockNonClientFrameViewMus::UseContainerdAppHeaderStyle() const {
  // Use for non tabbed trusted source windows, e.g. Settings, as well as apps.
  const Dock* const dock = dock_window()->dock();
  //return (!dock->is_type_tabbed() && dock->is_trusted_source()) ||
  //       dock->is_app();
  return !dock->is_type_tabbed();
}

void DockNonClientFrameViewMus::LayoutProfileSwitcher() {
//#if defined(FRAME_AVATAR_BUTTON)
//  views::View* profile_switcher_view = GetProfileSwitcherButton();
//  gfx::Size button_size = profile_switcher_view->GetPreferredSize();
//  int button_x = width() - GetTablistRightInset() + kAvatarButtonOffset;
//  profile_switcher_view->SetBounds(button_x, 0, button_size.width(),
//                                   button_size.height());
//#endif
}

bool DockNonClientFrameViewMus::ShouldPaint() const {
  if (!frame()->IsFullscreen())
    return true;

  // We need to paint when the top-of-window views are revealed in immersive
  // fullscreen.
  //ImmersiveModeController* immersive_mode_controller =
  //    dock_window()->immersive_mode_controller();
  //return immersive_mode_controller->IsEnabled() &&
  //       immersive_mode_controller->IsRevealed();
  return false;
}

void DockNonClientFrameViewMus::PaintContentEdge(gfx::Canvas* canvas) {
  DCHECK(!UseContainerdAppHeaderStyle());
  const int bottom = frame_values().normal_insets.bottom();
  canvas->FillRect(
      gfx::Rect(0, bottom, width(), kClientEdgeThickness),
      GetThemeProvider()->GetColor(
          ThemeProperties::COLOR_TOOLBAR_BOTTOM_SEPARATOR));
}

int DockNonClientFrameViewMus::GetHeaderHeight() const {
#if defined(OS_CHROMEOS)
  // TODO: move ash_layout_constants to ash/public/cpp.
  const bool restored = !frame()->IsMaximized() && !frame()->IsFullscreen();
  return GetAshLayoutSize(restored ? AshLayoutSize::kDockCaptionRestored
                                   : AshLayoutSize::kDockCaptionMaximized)
      .height();
#else
  return views::WindowManagerFrameValues::instance().normal_insets.top();
#endif
}

}