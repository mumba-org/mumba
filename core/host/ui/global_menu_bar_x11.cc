...
//#include "chrome/dock/history/top_sites_factory.h"
//#include "chrome/dock/workspaces/workspace.h"
//#include "chrome/dock/workspaces/workspace_manager.h"
//#include "chrome/dock/workspaces/workspace_window.h"
...
#include "core/host/ui/global_menu_bar_registrar_x11.h"
#include "mumba/grit/generated_resources.h"
//#include "components/bookmarks/common/bookmark_pref_names.h"
//#include "components/history/core/dock/top_sites.h"
...
//const char kProfileId[] = "workspace-id";
...
// GlobalMenuBarCommand workspaces_menu[] = {
//   { MENU_SEPARATOR, MENU_SEPARATOR },
//   { MENU_END, MENU_END }
// };
...
//  base::string16 title;
  ...
  // This ID is unique for a dock session and can be passed to the
...
GlobalMenuBarX11::GlobalMenuBarX11(DockWindow* dock_window,
                                   DockDesktopWindowTreeHostX11* host)
    : dock_(dock_window->dock()),
      workspace_(dock_->workspace()),
      dock_window_(dock_window),
      host_(host),
      server_(nullptr),
      root_item_(nullptr),
      //history_menu_(nullptr),
      //workspaces_menu_(nullptr),
      //top_sites_(nullptr),
...
void GlobalMenuBarX11::InitServer(unsigned long xid) {
 ...
 //     root_item_, IDS_HISTORY_MENU_LINUX, history_menu);
  ...
  //workspaces_menu_ = BuildStaticMenu(
  //    root_item_, IDS_PROFILES_OPTIONS_GROUP_NAME, workspaces_menu);
  ...
  for (CommandIDMenuItemMap::const_iterator it = id_to_menu_item_.begin();
    ...
    menuitem_property_set_bool(it->second, kPropertyEnabled,
                               host::IsCommandEnabled(dock_, it->first));

    ui::Accelerator accelerator;
    if (dock_window_->GetAccelerator(it->first, &accelerator))
      RegisterAccelerator(it->second, accelerator);

    host::AddCommandObserver(dock_, it->first, this);
  }

  //pref_change_registrar_.Init(dock_->workspace()->GetPrefs());
  ...
  //top_sites_ = TopSitesFactory::GetForProfile(workspace_);
  //if (top_sites_) {
    //GetTopSitesData();
  ...
  //ProfileManager* workspace_manager = g_dock_process->workspace_manager();
  //DCHECK(workspace_manager);
  //avatar_menu_.reset(new AvatarMenu(
      //&workspace_manager->GetProfileAttributesStorage(), this, nullptr));
...
void GlobalMenuBarX11::Disable() {
  for (CommandIDMenuItemMap::const_iterator it = id_to_menu_item_.begin();
       it != id_to_menu_item_.end(); ++it) {
    host::RemoveCommandObserver(dock_, it->first, this);
  }
...
//     PrefService* prefs = dock_->workspace()->GetPrefs();
...
//   ClearMenuSection(workspaces_menu_, TAG_PROFILES);
...
//   // active workspace and closing all its dock windows.
//   int active_workspace_index = -1;
...
//     //menuitem_child_add_position(workspaces_menu_, menu_item, i);
//     //g_object_unref(menu_item);
//   //}

//   // There is a separator between the list of workspaces and the possible actions.
//   int index = 1;//avatar_menu_->GetNumberOfItems() + 1;

//   DbusmenuMenuitem* edit_workspace_item = BuildMenuItem(
//       l10n_util::GetStringUTF8(IDS_PROFILES_MANAGE_BUTTON_LABEL), TAG_PROFILES);
//   DbusmenuMenuitem* create_workspace_item = BuildMenuItem(
//       l10n_util::GetStringUTF8(IDS_PROFILES_CREATE_BUTTON_LABEL),
//       TAG_PROFILES);

//   // There is no active workspace in Guest mode, in which case the action buttons
//   // should be disabled.
//   if (active_workspace_index >= 0) {
//     g_object_set_data(G_OBJECT(edit_workspace_item), kProfileId,
//                       GINT_TO_POINTER(active_workspace_index));
//     g_signal_connect(edit_workspace_item, "item-activated",
//                      G_CALLBACK(OnEditProfileItemActivatedThunk), this);
//     g_signal_connect(create_workspace_item, "item-activated",
//                      G_CALLBACK(OnCreateProfileItemActivatedThunk), this);
//   } else {
//     menuitem_property_set_bool(edit_workspace_item, kPropertyEnabled, false);
//     menuitem_property_set_bool(create_workspace_item, kPropertyEnabled, false);
//   }

//   menuitem_child_add_position(workspaces_menu_, edit_workspace_item, index++);
//   menuitem_child_add_position(workspaces_menu_, create_workspace_item, index);
//   g_object_unref(edit_workspace_item);
//   g_object_unref(create_workspace_item);
// }
...
void GlobalMenuBarX11::OnDockSetLastActive(Dock* dock) {
  // Notify the avatar menu of the change and rebuild the menu. Note: The
  // ActiveDockChanged() call needs to happen first to update the state.
  //avatar_menu_->ActiveDockChanged(dock);
  //avatar_menu_->RebuildMenu();
//  RebuildProfilesMenu();
}
...
//   // just load the URL.
//   sessions::TabRestoreService* service =
//       TabRestoreServiceFactory::GetForProfile(workspace_);
//   if (item->session_id.is_valid() && service) {
//     service->RestoreEntryById(dock_->live_tab_context(), item->session_id,
...
//   if (!tab_restore_service_) {
//     tab_restore_service_ = TabRestoreServiceFactory::GetForProfile(workspace_);
//     if (tab_restore_service_) {
...
//                                                     unsigned int timestamp) {
//   workspaces::CreateAndSwitchToNewProfile(ProfileManager::CreateCallback(),
