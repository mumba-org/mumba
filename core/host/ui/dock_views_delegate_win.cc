...
//#include "core/host/ui/native_widget.h"
//#include "core/host/win/app_icon.h"
...
//     taskbar_data.hWnd = ::FindWindow(L"Host_TrayWnd", NULL);
...
HICON DockViewsDelegate::GetDefaultWindowIcon() const {
  //return GetAppIcon();
  return nullptr;
}

HICON DockViewsDelegate::GetSmallWindowIcon() const {
...
views::NativeWidget* DockViewsDelegate::CreateNativeWidget(
  ...
  //    native_widget_type = views::NativeWidgetType::DESKTOP_NATIVE_WIDGET_AURA;
  //  }
  //}
  return ::host::CreateNativeWidget(native_widget_type, params, delegate);
}

/* int DockViewsDelegate::GetAppbarAutohideEdges(HMONITOR monitor,
  ...
  if (monitor && !in_autohide_edges_callback_) {
    ...
    base::PostTaskWithTraitsAndReplyWithResult(
        ...
        base::Bind(&DockViewsDelegate::OnGotAppbarAutohideEdges,
...
void DockViewsDelegate::OnGotAppbarAutohideEdges(
    const base::Closure& callback,
    HMONITOR monitor,
    int returned_edges,
    int edges) {
