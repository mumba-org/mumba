...
TEST_F(WindowSizerAshTest, PlaceNewWindows) {
  ...
  std::unique_ptr<Browser> browser2 = (CreateTestBrowser(
      CreateTestWindowInDomainWithId(0), gfx::Rect(16, 32, 640, 320), &params2));
  ...
  std::unique_ptr<Browser> browser_popup(
      CreateTestBrowser(CreateTestWindowInDomainWithId(1),
  ...
  std::unique_ptr<Browser> browser_panel(
      CreateTestBrowser(CreateTestWindowInDomainWithId(2),
...
TEST_F(WindowSizerAshTest, PlaceNewWindowsOnMultipleDisplays) {
  ...
  ASSERT_EQ(ash::Domain::GetPrimaryRootWindow(),
            ash::Domain::GetRootWindowForNewWindows());
  ...
  // window. It should be opened then on the secondary display.
  {
    ...
    EXPECT_NE(ash::Domain::GetPrimaryRootWindow(),
              ash::Domain::GetRootWindowForNewWindows());
    gfx::Rect window_bounds;
  ...
  // It should be created in the primary display.
  {
    ...
    EXPECT_EQ(ash::Domain::GetPrimaryRootWindow(),
              ash::Domain::GetRootWindowForNewWindows());
...
TEST_F(WindowSizerAshTest, DefaultBoundsInTargetDisplay) {
  ...
  aura::Window* first_root = ash::Domain::GetAllRootWindows()[0];
  EXPECT_EQ(first_root, ash::Domain::GetRootWindowForNewWindows());
  gfx::Rect bounds;
...

  {
    ...
    aura::Window* second_root = ash::Domain::GetAllRootWindows()[1];
