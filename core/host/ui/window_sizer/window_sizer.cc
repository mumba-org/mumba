...
// and persistent state from the dock window and the user's profile.
class DefaultStateProvider : public WindowSizer::StateProvider {
 public:
  DefaultStateProvider(const std::string& app_name, const Dock* dock)
      : app_name_(app_name) {//, dock_(dock) {
  }
  ...
  bool GetPersistentState(gfx::Rect* bounds,
    ...
    // if (!dock_ || !dock_->profile()->GetPrefs())
    //   return false;

    // const base::DictionaryValue* wp_pref =
    //     chrome::GetWindowPlacementDictionaryReadOnly(
    //         chrome::GetWindowName(dock_), dock_->profile()->GetPrefs());
...
//     // If a reference dock is set, use its window. Otherwise find last
//     // active. Panels are never used as reference browsers as panels are
//     // specially positioned.
//     DockWindow* window = NULL;
//     // Window may be null if dock is just starting up.
//     if (dock_ && dock_->window()) {
//       window = dock_->window();
//     } else {
//       const BrowserList* dock_list = BrowserList::GetInstance();
//       for (BrowserList::const_reverse_iterator it =
//                dock_list->begin_last_active();
//            it != dock_list->end_last_active(); ++it) {
//         Dock* last_active = *it;
...
//     }
 ...
 private:
  ...
  // If set, is used as the reference dock for GetLastActiveWindowState.
  //const Dock* dock_;
...
display::Display WindowSizer::DefaultTargetDisplayProvider::GetTargetDisplay(
  ...
  if (ash_util::ShouldOpenAshOnStartup()) {
    aura::Window* target = ash::Domain::GetRootWindowForNewWindows();
...
// static
void WindowSizer::GetDockWindowBoundsAndShowState(
...
ui::WindowShowState WindowSizer::GetWindowDefaultShowState() const {
  ...
  //bool use_command_line = dock_->is_type_tabbed() || dock_->is_devtools();

//#if defined(USE_AURA)
  // We use the apps save state as well on aura.
//  use_command_line = use_command_line || dock_->is_app();
//#endif
...
//  }
  ...
  return dock_->initial_show_state();
}
