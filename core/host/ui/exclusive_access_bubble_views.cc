...
#include "core/host/ui/subtle_notification_view.h"
...
ExclusiveAccessBubbleViews::ExclusiveAccessBubbleViews(
  ...
  dock_fullscreen_exit_accelerator_ = accelerator.GetShortcutText();
...
void ExclusiveAccessBubbleViews::UpdateViewContent(
  ...
  if (bubble_type ==
    ...
    accelerator = dock_fullscreen_exit_accelerator_;
  } else {
...
views::View* ExclusiveAccessBubbleViews::GetDockRootView() const {
...
bool ExclusiveAccessBubbleViews::WindowContainsPoint(gfx::Point pos) {
  return GetDockRootView()->HitTestPoint(pos);
}
