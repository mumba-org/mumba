...
void PrintDialogGtk2::PrintDocument(const printing::MetafilePlayer& metafile,
  ...
  // NOTE: this is broken given we cannot depend back on 'host'
  //       giving its a circular dependency
  //       with shared libraries, this give us a linking issue
  
  //  One solution would be to pass the TaskRunner in the method, and just
  //  call/post into whatever the 'host' give us
  DCHECK(false); 
  //HostThread::PostTask(
