...
bool X11InputMethodContextImplGtk2::DispatchKeyEvent(
  ...
  DLOG(INFO) << "X11InputMethodContextImplGtk2::DispatchKeyEvent";
  if (!key_event.HasNativeEvent() || !gtk_context_) {
    DLOG(INFO) << "  key_event.HasNativeEvent() = false ? " << key_event.HasNativeEvent() << " || gtk_context_ = nullptr ? "  << (gtk_context_ == nullptr) << ""; 
    return false;
  }
