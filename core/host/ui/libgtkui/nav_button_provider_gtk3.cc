...
#include "core/host/ui/libgtkui/nav_button_provider_gtk3.h"

#include <gtk/gtk.h>

#include "core/host/ui/libgtkui/gtk3_background_painter.h"
#include "core/host/ui/libgtkui/gtk_util.h"
...
const char* ButtonStyleClassFromButtonType(
    host::FrameButtonDisplayType type) {
  switch (type) {
    case host::FrameButtonDisplayType::kMinimize:
      return "minimize";
    case host::FrameButtonDisplayType::kMaximize:
    case host::FrameButtonDisplayType::kRestore:
      return "maximize";
    case host::FrameButtonDisplayType::kClose:
      return "close";
    default:
      NOTREACHED();
      return "";
...
const char* IconNameFromButtonType(host::FrameButtonDisplayType type) {
  switch (type) {
    case host::FrameButtonDisplayType::kMinimize:
...
ScopedGObject<GdkPixbuf> LoadNavButtonIcon(host::FrameButtonDisplayType type,
...
void CalculateUnscaledButtonSize(host::FrameButtonDisplayType type,
                                 bool maximized,
...
class NavButtonImageSource : public gfx::ImageSkiaSource {
 public:
  NavButtonImageSource(host::FrameButtonDisplayType type,
 ...
 private:
  host::FrameButtonDisplayType type_;
...
void NavButtonProviderGtk3::RedrawImages(int top_area_height,
  ...
  double scale = 1.0f;
  std::map<host::FrameButtonDisplayType, gfx::Size> button_sizes;
  std::map<host::FrameButtonDisplayType, gfx::Insets> button_margins;
  std::vector<host::FrameButtonDisplayType> display_types{
      host::FrameButtonDisplayType::kMinimize,
      maximized ? host::FrameButtonDisplayType::kRestore
                : host::FrameButtonDisplayType::kMaximize,
      host::FrameButtonDisplayType::kClose,
  };
...
gfx::Insets NavButtonProviderGtk3::GetNavButtonMargin(
    host::FrameButtonDisplayType type) const {
