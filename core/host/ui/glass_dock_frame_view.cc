...
//#include "mumba/app/chrome_dll_resource.h"
...
//#include "core/host/ui/hosted_app_button_container.h"
#include "core/host/ui/tablist/new_tab_button.h"
#include "core/host/ui/tablist/tab.h"
#include "core/host/ui/tablist/tablist.h"
//#include "core/host/ui/toolbar/toolbar_view.h"
//#include "core/host/ui/web_applications/app_browser_controller.h"
#include "core/host/win/titlebar_config.h"
...
HICON GlassDockFrameView::throbber_icons_[
    GlassDockFrameView::kThrobberIconCount];
...
int GetFrameThickness(HWND hwnd) {//HMONITOR monitor) {
  ...
  const int resize_frame_thickness =
      display::win::ScreenWin::GetSystemMetricsForHwnd(hwnd,//GetSystemMetricsForMonitor(monitor,
                                                       SM_CXSIZEFRAME);
...
///////////////////////////////////////////////////////////////////////////////
// GlassDockFrameView, public:

constexpr char GlassDockFrameView::kClassName[];

SkColor GlassDockFrameView::GetReadableFeatureColor(
...
GlassDockFrameView::GlassDockFrameView(DockFrame* frame,
  ...
  drag_handle_padding_ = kTopResizeFrameArea;
      //browser_view->dock()->is_focus_mode() ? 0 : kTopResizeFrameArea;
}

GlassDockFrameView::~GlassDockFrameView() {
}

///////////////////////////////////////////////////////////////////////////////
// GlassDockFrameView, DockNonClientFrameView implementation:

bool GlassDockFrameView::CaptionButtonsOnLeadingEdge() const {
...
gfx::Rect GlassDockFrameView::GetBoundsForTablist(views::View* tablist) const {
  ...
  return gfx::Rect(x, TopAreaHeight(false), std::max(0, end_x - x),
                   tablist->GetPreferredSize().height());
}

int GlassDockFrameView::GetTopInset(bool restored) const {
...
int GlassDockFrameView::GetThemeBackgroundXInset() const {
  return 0;
}

SkColor GlassDockFrameView::GetCaptionColor() const {
   const SkAlpha title_alpha = SK_AlphaOPAQUE;//ShouldPaintAsActive(active_state)
                                  // ? SK_AlphaOPAQUE
                                  // : kInactiveTitlebarFeatureAlpha;
   return SkColorSetA(GetReadableFeatureColor(GetFrameColor()),
                      title_alpha);
}


// bool GlassDockFrameView::HasVisibleBackgroundTabShapes(
...
//   return DockNonClientFrameView::HasVisibleBackgroundTabShapes(active_state);
// }

// bool GlassDockFrameView::CanDrawStrokes() const {
...
//   return DockNonClientFrameView::CanDrawStrokes();
// }

// SkColor GlassDockFrameView::GetCaptionColor(ActiveState active_state) const {
...
void GlassDockFrameView::UpdateThrobber(bool running) {
  if (ShowCustomIcon())
    window_icon_->Update();
...
gfx::Size GlassDockFrameView::GetMinimumSize() const {
  gfx::Size min_size(dock_window()->GetMinimumSize());
...
///////////////////////////////////////////////////////////////////////////////
// GlassDockFrameView, views::NonClientFrameView implementation:

gfx::Rect GlassDockFrameView::GetBoundsForClientView() const {
  return client_view_bounds_;
}

gfx::Rect GlassDockFrameView::GetWindowBoundsForClientBounds(
    const gfx::Rect& client_bounds) const {
  HWND hwnd = views::HWNDForWidget(frame());
  if (!dock_window()->IsTablistVisible() && hwnd) {
...
int GlassDockFrameView::NonClientHitTest(const gfx::Point& point) {
  int super_component = DockNonClientFrameView::NonClientHitTest(point);
  ...
  if (dock_window()->ShouldShowWindowIcon() && frame_component != HTCLIENT) {
...
void GlassDockFrameView::UpdateWindowIcon() {
  if (ShowCustomIcon() && !frame()->IsFullscreen())
    window_icon_->SchedulePaint();
}

void GlassDockFrameView::UpdateWindowTitle() {
...
void GlassDockFrameView::ResetWindowControls() {
  DockNonClientFrameView::ResetWindowControls();
...
void GlassDockFrameView::ButtonPressed(views::Button* sender,
...
bool GlassDockFrameView::ShouldTabIconViewAnimate() const {
...
gfx::ImageSkia GlassDockFrameView::GetFaviconForTabIconView() {
  DCHECK(ShowCustomIcon());
  return frame()->widget_delegate()->GetWindowIcon();
}

bool GlassDockFrameView::IsMaximized() const {
  return frame()->IsMaximized();
}

///////////////////////////////////////////////////////////////////////////////
// GlassDockFrameView, views::View overrides:

const char* GlassDockFrameView::GetClassName() const {
  return kClassName;
}

void GlassDockFrameView::OnPaint(gfx::Canvas* canvas) {
  TRACE_EVENT0("views.frame", "GlassDockFrameView::OnPaint");
  if (ShouldCustomDrawSystemTitlebar())
    PaintTitlebar(canvas);
}

void GlassDockFrameView::Layout() {
  TRACE_EVENT0("views.frame", "GlassDockFrameView::Layout");
  if (ShouldCustomDrawSystemTitlebar())
...
///////////////////////////////////////////////////////////////////////////////
// GlassDockFrameView, private:

int GlassDockFrameView::FrameBorderThickness() const {
  return (IsMaximized() || frame()->IsFullscreen())
             ? 0
             : display::win::ScreenWin::GetSystemMetricsInDIP(SM_CXSIZEFRAME);
}

int GlassDockFrameView::FrameTopBorderThickness(bool restored) const {
...
int GlassDockFrameView::FrameTopBorderThicknessPx(bool restored) const {
  ...
  return GetFrameThickness(HWNDForView(this));
      //MonitorFromWindow(HWNDForView(this), MONITOR_DEFAULTTONEAREST));
}

int GlassDockFrameView::TopAreaHeight(bool restored) const {
  if (frame()->IsFullscreen() && !restored)
    return 0;
  ...
  //  thickness =
  //      std::max(thickness, DockNonClientFrameView::kMinimumDragHeight);
  //}
  return top + thickness;
}

int GlassDockFrameView::TitlebarMaximizedVisualHeight() const {
...
int GlassDockFrameView::TitlebarHeight(bool restored) const {
  if (frame()->IsFullscreen() && !restored)
    return 0;
...
int GlassDockFrameView::WindowTopY() const {
...
int GlassDockFrameView::MinimizeButtonX() const {
...
bool GlassDockFrameView::IsToolbarVisible() const {
  return false;//dock_window()->IsToolbarVisible() &&
       //!dock_window()->toolbar()->GetPreferredSize().IsEmpty();
}

bool GlassDockFrameView::ShowCustomIcon() const {
  // Hosted app windows don't include the window icon as per UI mocks.
  return false;//!hosted_app_button_container() && ShouldCustomDrawSystemTitlebar() &&
         //dock_window()->ShouldShowWindowIcon();
}

bool GlassDockFrameView::ShowCustomTitle() const {
  return ShouldCustomDrawSystemTitlebar() &&
         dock_window()->ShouldShowWindowTitle();
}

bool GlassDockFrameView::ShowSystemIcon() const {
  return !ShouldCustomDrawSystemTitlebar() &&
         dock_window()->ShouldShowWindowIcon();
}

SkColor GlassDockFrameView::GetTitlebarColor() const {
  return GetFrameColor();
}

Windows10CaptionButton* GlassDockFrameView::CreateCaptionButton(
    ViewID button_type,
...
void GlassDockFrameView::PaintTitlebar(gfx::Canvas* canvas) const {
  DLOG(INFO) << "GlassDockFrameView::PaintTitlebar";
  TRACE_EVENT0("views.frame", "GlassDockFrameView::PaintTitlebar");
  ...
  if (ShowCustomTitle())
    window_title_->SetEnabledColor(GetCaptionColor());
}

void GlassDockFrameView::LayoutTitleBar() {
  TRACE_EVENT0("views.frame", "GlassDockFrameView::LayoutTitleBar");
  ...
  if (ShowCustomTitle()) {
    ...
    window_title_->SetText(dock_window()->GetWindowTitle());
...
void GlassDockFrameView::LayoutCaptionButton(Windows10CaptionButton* button,
                                                int previous_button_x) {
  TRACE_EVENT0("views.frame", "GlassDockFrameView::LayoutCaptionButton");
  gfx::Size button_size = button->GetPreferredSize();
  button->SetBounds(previous_button_x - button_size.width(), WindowTopY(),
                    button_size.width(), button_size.height());
}

void GlassDockFrameView::LayoutCaptionButtons() {
  TRACE_EVENT0("views.frame", "GlassDockFrameView::LayoutCaptionButtons");
...
void GlassDockFrameView::StopThrobber() {
  DCHECK(ShowSystemIcon());
  if (throbber_running_) {
    throbber_running_ = false;
    ...
    gfx::ImageSkia icon = dock_window()->GetWindowIcon();
    if (!icon.isNull()) {
...
void GlassDockFrameView::DisplayNextThrobberFrame() {
...
// static
void GlassDockFrameView::InitThrobberIcons() {
