...
#ifndef MUMBA_HOST_UI_WINDOW_HOST_VIEW_H_
#define MUMBA_HOST_UI_WINDOW_HOST_VIEW_H_
...
#include "core/host/ui/exclusive_access_bubble_views_context.h"
...
class DockWindowLayout;
class Tablist;
class Workspace;
class ThemeService;
class ExclusiveAccessContext;
class TopContainerView;
class ApplicationContentsCloseHandler;
...
class DockWindowLayoutDelegateImpl;
#if defined(OS_WIN)
class LoadCompleteListener;
#endif

// BrowserView + BrowserWindow
class DockWindow : public TablistModelObserver,
...
public:
  ...
  static DockWindow* CreateDockWindow(Dock* dock, bool user_gesture, bool tabs_hidden);
  ...
  void set_frame(DockFrame* frame) { frame_ = frame; }
  ...
  static DockWindow* GetDockWindowForNativeWindow(gfx::NativeWindow window);

  // Returns the BrowserView used for the specified Browser.
  static DockWindow* GetDockWindowForDock(const Dock* dock);//const Browser* browser);
  ...
  // After calling RevealTablistIfNeeded(), there is normally a delay before
  ...
  const Dock* dock() const { return dock_.get(); }
  ...
  // Returns the preferred height of the Tablist. Used to position the
  // incognito avatar icon.
  int GetTablistHeight() const;
  ...
  gfx::Point OffsetPointForToolbarBackgroundImage(const gfx::Point& point) const;

  // Container for the tabstrip, toolbar, etc.
  TopContainerView* top_container() { return top_container_; }

  // Accessor for the Tablist.
  ...
  // Accessor for the contents ApplicationView.
  ApplicationView* application_view() { return contents_application_view_; }

  // Returns true if various window components are visible.
  bool IsTablistVisible() const;
  ...
  // TabIconView::WindowContentsProvider implementations.
  ...
  // bool IsSelectedWindowLoading() const; or something like that. We could even
...
//  }
  ...
  //void SetTranslateIconToggled(bool is_lit);
  void OnActiveTabChanged(ApplicationContents* old_contents,
  ...
  void Minimize();
  void Restore();
  void EnterFullscreen(const GURL& url, ExclusiveAccessBubbleType bubble_type);
  void ExitFullscreen();
  ExclusiveAccessContext* GetExclusiveAccessContext();
  void UpdateExclusiveAccessExitBubbleContent(
      const GURL& url,
      ExclusiveAccessBubbleType bubble_type,
      ExclusiveAccessBubbleHideCallback bubble_first_hide_callback);
  void OnExclusiveAccessUserInput();
  bool ShouldHideUIForFullscreen() const;
  bool IsFullscreen() const;
  bool IsFullscreenBubbleVisible() const;
  //LocationBar* GetLocationBar() const;
  //void SetFocusToLocationBar(bool select_all);
  void UpdateReloadStopState(bool is_loading, bool force);
  //void UpdateToolbar(ApplicationContents* contents);
  //void ResetToolbarWindowState(ApplicationContents* contents);
  //void FocusToolbar();
  ...
  bool IsTablistEditable() const;
  //bool IsToolbarVisible() const;
  //bool IsToolbarShowing() const;
  //void ShowUpdateChromeDialog();
//#if defined(OS_CHROMEOS)
  //void ShowIntentPickerBubble(
  //    std::vector<IntentPickerBubbleView::AppInfo> app_info,
  //    IntentPickerResponse callback);
  //void SetIntentPickerViewVisibility(bool visible);
...
//#endif
  ...
  //bool IsDownloadShelfVisible() const;
  ...
  int GetRenderViewHeightInsetWithDetachedBookmarkBar();
  ...
  //        callback);
  std::string GetDesktopWorkspace() const;
  bool IsVisibleOnAllDesktopWorkspaces() const;

  scoped_refptr<Workspace> GetWorkspace() const;

  //BookmarkBarView* GetBookmarkBarView() const;
  //LocationBarView* GetLocationBarView() const;

  ThemeService* GetThemeServiceForActiveTab();
  ...
  scoped_refptr<Workspace> GetWorkspace();
  ApplicationContents* GetActiveApplicationContents();
  ...
  // "Page title - Window state".
  base::string16 GetAccessibleWindowLabel(bool include_app_name, int index) const;
 ...
 private:
  ...
  friend class DockWindowLayoutDelegateImpl;
  ...
  DockWindowLayout* GetDockWindowLayout() const;
  ...
  void UpdateDevToolsForContents(ApplicationContents* web_contents,
  ...
  // removed.
  void UpdateUIForContents(ApplicationContents* contents);
  ...
  bool DoCutCopyPasteForApplicationContents(
      ApplicationContents* contents,
  ...
  base::string16 GetAccessibleWindowTitleImpl() const;

  void OnDevToolsContentsAvailable(ApplicationContents* devtools, DevToolsContentsResizingStrategy strategy, bool update_devtools_app_contents);

  // The BrowserFrame that hosts this view.
  DockFrame* frame_ = nullptr;
  ...
  ApplicationViewImpl* contents_application_view_ = nullptr;

  // The view that contains devtools window for the selected WebContents.
  //views::WebView* devtools_web_view_ = nullptr;
  ApplicationViewImpl* devtools_web_view_ = nullptr;
  ...
  // The Toolbar containing the menus, applist and the app/address bar, at the bottom of the window
  //ToolbarView* toolbar_ = nullptr;
  ...
  std::unique_ptr<ApplicationContentsCloseHandler> app_contents_close_handler_;
  ...
  mutable base::WeakPtrFactory<DockWindow> activate_modal_dialog_factory_{
      this};
      
  DISALLOW_COPY_AND_ASSIGN(DockWindow);
};
