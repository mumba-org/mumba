...
///////////////////////////////////////////////////////////////////////////////
// DockFrameViewLayout, public:

// statics

// The content edge images have a shadow built into them.
const int DockFrameViewLayout::kContentEdgeShadowThickness = 2;

// Besides the frame border, there's empty space atop the window in restored
// mode, to use to drag the window around.
const int DockFrameViewLayout::kNonClientRestoredExtraThickness = 0;//11;

// The frame border is only visible in restored mode and is hardcoded to 4 px on
// each side regardless of the system window border size.
const int DockFrameViewLayout::kFrameBorderThickness = 4;

// The titlebar has a 2 px 3D edge along the top.
const int DockFrameViewLayout::kTitlebarTopEdgeThickness = 2;

// The icon is inset 1 px from the left frame border.
const int DockFrameViewLayout::kIconLeftSpacing = 1;

// There is a 4 px gap between the icon and the title text.
const int DockFrameViewLayout::kIconTitleSpacing = 4;

// The horizontal spacing to use in most cases when laying out things near the
// caption button area.
const int DockFrameViewLayout::kCaptionSpacing = 5;

// The minimum vertical padding between the bottom of the caption buttons and
// the top of the content shadow.
const int DockFrameViewLayout::kCaptionButtonBottomPadding = 3;
...
const int DockFrameViewLayout::kNewTabCaptionCondensedSpacing = 16;

DockFrameViewLayout::DockFrameViewLayout()
...
void DockFrameViewLayout::SetButtonOrdering(
...
gfx::Rect DockFrameViewLayout::GetBoundsForTablist(
    const gfx::Size& tabstrip_preferred_size,
    int available_width) const {
  const int x = GetTablistLeftInset();
  available_width -= x + NewTabCaptionSpacing() + trailing_button_start_;
  return gfx::Rect(x, GetTablistInsetsTop(false), std::max(0, available_width),
                   tabstrip_preferred_size.height());
}

gfx::Size DockFrameViewLayout::GetMinimumSize(
    int available_width) const {
  gfx::Size min_size = delegate_->GetDockWindowMinimumSize();
  ...
  if (delegate_->IsTablistVisible()) {
    gfx::Size preferred_size = delegate_->GetTablistPreferredSize();
...
int DockFrameViewLayout::GetTablistLeftInset() const {
  return 0;//leading_button_start_; //+ DockFrameView::GetAvatarIconPadding();
}

gfx::Rect DockFrameViewLayout::GetWindowBoundsForClientBounds(
...
int DockFrameViewLayout::FrameBorderThickness(bool restored) const {
  return (!restored && (IsTitleBarCondensed() || delegate_->IsFullscreen())) ?
      0 : kFrameBorderThickness;
}

int DockFrameViewLayout::NonClientBorderThickness() const {
...
int DockFrameViewLayout::NonClientTopHeight(bool restored) const {
  ...
  if (!restored && delegate_->IsTablistVisible() && IsTitleBarCondensed())
    thickness -= GetLayoutInsets(TAB).top() - 1;
  return thickness;
}

int DockFrameViewLayout::GetTablistInsetsTop(bool restored) const {
  int top = NonClientTopHeight(restored);
  if (!restored && (IsTitleBarCondensed() || delegate_->IsFullscreen())) {
    return top;
  }
...
int DockFrameViewLayout::TitlebarTopThickness(bool restored) const {
  if (!delegate_->UseCustomFrame()) {
    return 0;
  }
  int result = (restored || !IsTitleBarCondensed()) ?
      kTitlebarTopEdgeThickness : FrameBorderThickness(false);
  return result;
}

int DockFrameViewLayout::DefaultCaptionButtonY(bool restored) const {
...
int DockFrameViewLayout::CaptionButtonY(
    FrameButtonDisplayType button_id,
    bool restored) const {
  return DefaultCaptionButtonY(restored);
}

int DockFrameViewLayout::TopAreaPadding() const {
  int padding = FrameBorderThickness(false);
  return padding;
}

gfx::Rect DockFrameViewLayout::IconBounds() const {
  return window_icon_bounds_;
}

gfx::Rect DockFrameViewLayout::CalculateClientAreaBounds(
    int width,
    int height) const {
  int top_height = NonClientTopHeight(false);
  int border_thickness = NonClientBorderThickness();
  gfx::Rect bounds(border_thickness, top_height,
         std::max(0, width - (2 * border_thickness)),
         std::max(0, height - top_height - border_thickness));
  return bounds;
}

FrameButtonDisplayType
DockFrameViewLayout::GetButtonDisplayType(
    views::FrameButton button_id) const {
  switch (button_id) {
    case views::FRAME_BUTTON_MINIMIZE:
      return FrameButtonDisplayType::kMinimize;
    case views::FRAME_BUTTON_MAXIMIZE:
      return delegate_->IsMaximized()
                 ? FrameButtonDisplayType::kRestore
                 : FrameButtonDisplayType::kMaximize;
    case views::FRAME_BUTTON_CLOSE:
      return FrameButtonDisplayType::kClose;
    default:
      NOTREACHED();
      return FrameButtonDisplayType::kClose;
  }
...
int DockFrameViewLayout::GetWindowCaptionSpacing(
...
bool DockFrameViewLayout::IsTitleBarCondensed() const {
...
///////////////////////////////////////////////////////////////////////////////
// DockFrameViewLayout, protected:

void DockFrameViewLayout::LayoutNewStyleAvatar(views::View* host) {
...
bool DockFrameViewLayout::ShouldDrawImageMirrored(
...
///////////////////////////////////////////////////////////////////////////////
// DockFrameViewLayout, private:

bool DockFrameViewLayout::ShouldIncognitoIconBeOnRight() const {
...
int DockFrameViewLayout::NewTabCaptionSpacing() const {
  return (has_trailing_buttons_ && IsTitleBarCondensed()) ?
      kNewTabCaptionCondensedSpacing : kCaptionSpacing;
}

void DockFrameViewLayout::LayoutWindowControls(views::View* host) {
...
void DockFrameViewLayout::LayoutTitleBar(views::View* host) {
...
void DockFrameViewLayout::LayoutIncognitoIcon(views::View* host) {
...
void DockFrameViewLayout::ConfigureButton(views::View* host,
                                                   views::FrameButton button_id,
...
void DockFrameViewLayout::HideButton(views::FrameButton button_id) {
  switch (button_id) {
    case views::FRAME_BUTTON_MINIMIZE:
...
void DockFrameViewLayout::SetBoundsForButton(
    views::FrameButton button_id,
    views::View* host,
    views::ImageButton* button,
    ButtonAlignment alignment) {
...
void DockFrameViewLayout::SetView(int id, views::View* view) {
  // Why do things this way instead of having an Init() method, where we're
  // passed the views we'll handle? Because DockFrameView doesn't own
  // all the views which are part of it. The avatar stuff, for example, will be
  // added and removed by the base class of DockFrameView.
  switch (id) {
...
///////////////////////////////////////////////////////////////////////////////
// DockFrameViewLayout, views::LayoutManager:

void DockFrameViewLayout::Layout(views::View* host) {
  TRACE_EVENT0("views.frame", "DockFrameViewLayout::Layout");
...
gfx::Size DockFrameViewLayout::GetPreferredSize(
...
void DockFrameViewLayout::ViewAdded(views::View* host,
                                             views::View* view) {
  SetView(view->id(), view);
}

void DockFrameViewLayout::ViewRemoved(views::View* host,
                                               views::View* view) {
