...
#ifndef MUMBA_HOST_UI_APPLICATION_CONTENTS_CLOSE_HANDLER_H_
#define MUMBA_HOST_UI_APPLICATION_CONTENTS_CLOSE_HANDLER_H_
...
class ApplicationContentsCloseHandler {
 public:
  explicit ApplicationContentsCloseHandler(ApplicationContentsCloseHandlerDelegate* delegate);
  ~ApplicationContentsCloseHandler();
 ...
 private:
  //friend class ApplicationContentsCloseHandlerTest;

  // Invoked from the |timer_|. If hit it means enough time has expired after a
  // close was canceled.
  void OnStillHaventClosed();

  ApplicationContentsCloseHandlerDelegate* delegate_;
  ...
  DISALLOW_COPY_AND_ASSIGN(ApplicationContentsCloseHandler);
};
