...
void ShowItem(scoped_refptr<host::Workspace> workspace,
              const base::FilePath& full_path,
              bool use_nautilus_file_browser) {
  if (use_nautilus_file_browser) {
    OpenItem(workspace, full_path, SHOW_ITEM_IN_FOLDER, OpenOperationCallback());
  } else {
    ...
    OpenItem(workspace, full_path.DirName(), OPEN_FOLDER,
...
void ShowItemInFolder(scoped_refptr<host::Workspace> workspace, const base::FilePath& full_path) {
  ...
  base::PostTaskWithTraitsAndReplyWithResult(
      ...
      base::BindOnce(&ShowItem, workspace, full_path));
}

void OpenExternal(scoped_refptr<host::Workspace> workspace, const GURL& url) {
  DCHECK_CURRENTLY_ON(HostThread::UI);
