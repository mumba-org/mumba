...
class MouseLockController : public ExclusiveAccessControllerBase {
 public:
  ...
  void RequestToLockMouse(ApplicationContents* web_contents,
                          bool user_gesture,
  ...
  // If true, does not call into the ApplicationContents to lock the mouse. Just assumes
  // that it works. This may be necessary when calling
  // Browser::RequestToLockMouse in tests, because the proper signal will not
  // have been passed to the ApplicationWindowHost.
  ...
  void set_web_contents_granted_silent_mouse_lock_permission_for_test(
    ...
    application_contents_granted_silent_mouse_lock_permission_ = app_contents;
  }
 ...
 private:
  ...
  void OnBubbleHidden(ApplicationContents*, ExclusiveAccessBubbleHideReason);

  MouseLockState mouse_lock_state_;

  // Optionally a ApplicationContents instance that is granted permission to silently
  // lock the mouse. This is granted only if the ApplicationContents instance has
  // previously locked and displayed the permission bubble until the bubble
  // time out has expired. https://crbug.com/725370
  ApplicationContents* application_contents_granted_silent_mouse_lock_permission_ =
      nullptr;
