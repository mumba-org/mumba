...
class KeyboardLockController : public ExclusiveAccessControllerBase {
 public:
  ...
  void RequestKeyboardLock(ApplicationContents* web_contents,
                           bool esc_key_locked);

  // Cancels an existing request for keyboard lock for |web_contents|.
  void CancelKeyboardLockRequest(ApplicationContents* web_contents);
  ...
  bool HandleKeyEvent(const NativeWebKeyboardEvent& event);
 ...
 private:
  ...
  void LockKeyboard(ApplicationContents* app_contents, bool esc_key_locked);
