...
class FullscreenController : public ExclusiveAccessControllerBase {
 public:
  ...
  void ToggleDockFullscreenModeWithExtension(const GURL& extension_url);
  ...
  // DockWindow::IsFullscreen() may still return false.
  ...
  // origin will be used.
  void EnterFullscreenModeForTab(ApplicationContents* app_contents);//,
                                 //const GURL& origin);

  // Leave a tab-initiated fullscreen mode.
  // |web_contents| represents the tab that requests to no longer be fullscreen.
  void ExitFullscreenModeForTab(ApplicationContents* app_contents);

  // Platform Fullscreen ///////////////////////////////////////////////////////

  // Overrde from ExclusiveAccessControllerBase.
  void OnTabDetachedFromView(ApplicationContents* web_contents) override;
  void OnTabClosing(ApplicationContents* web_contents) override;
  ...
  // Called by Dock::WindowFullscreenStateWillChange.
  void WindowFullscreenStateWillChange();

  // Called by Dock::WindowFullscreenStateChanged.
 ...
 private:
  ...
  void SetFullscreenedTab(ApplicationContents* tab, const GURL& origin);
  ...
  bool MaybeToggleFullscreenWithinTab(ApplicationContents* app_contents,
                                      bool enter_fullscreen);
  // Returns true if |web_contents| is in fullscreen mode as a screen-captured
  // tab. See 'FullscreenWithinTab Note'.
  bool IsFullscreenWithinTab(const ApplicationContents* app_contents) const;
