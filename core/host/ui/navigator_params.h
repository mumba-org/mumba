...
struct NavigateParams {
  ...
  explicit NavigateParams(ApplicationContents* a_target_contents);
#else
  NavigateParams(Dock* dock,
                 const GURL& a_url,
                 ui::PageTransition a_transition);
  NavigateParams(Dock* dock,
                 ApplicationContents* a_target_contents);
#endif
  NavigateParams(scoped_refptr<Workspace> workspace,
                 const GURL& a_url,
                 ui::PageTransition a_transition);
  ...
  // The dock-global ID of the frame to navigate, or
  ...
  // [in]  A ApplicationContents to be navigated or inserted into the target
  //       Dock's tabstrip. If NULL, |url| or the homepage will be used
  ...
  //       (i.e. |url| is ignored).
  //       Default is NULL.
  // [out] The ApplicationContents in which the navigation occurred or that was
  //       inserted. Guaranteed non-NULL except for note below:
  // Note: If this field is set to NULL by the caller and Navigate() creates
  //       a new ApplicationContents, this field will remain NULL and the
  //       ApplicationContents deleted if the ApplicationContents it created is
  //       not added to a TabStripModel before Navigate() returns.
  ApplicationContents* target_contents = nullptr;

  // [in]  The ApplicationContents that initiated the Navigate() request if such
  //       context is necessary. Default is NULL, i.e. no context.
  // [out] If NULL, this value will be set to the selected ApplicationContents in
  //       the originating dock prior to the operation performed by
  //       Navigate(). However, if the originating page is from a different
  //       workspace (e.g. an OFF_THE_RECORD page originating from a non-OTR
  //       window), then |source_contents| is reset to NULL.
  ApplicationContents* source_contents = nullptr;
  ...
  //
  // [in]:                Condition:                        [out]:
  // NEW_BACKGROUND_TAB   target dock tabstrip is empty  NEW_FOREGROUND_TAB
  // CURRENT_TAB          "     "     "                     NEW_FOREGROUND_TAB
  // NEW_BACKGROUND_TAB   target dock is an app dock  NEW_FOREGROUND_TAB
  // OFF_THE_RECORD       target dock workspace is incog.  NEW_FOREGROUND_TAB
  //
  ...
  // Allows setting the opener for the case when new ApplicationContents are created
  // (i.e. when |disposition| asks for a new tab or window).
  ApplicationWindowHost* opener = nullptr;

  // Sets dock->is_trusted_source.
  ...
  int tablist_add_types = TablistModel::ADD_ACTIVE;
  ...
  // TODO(beng): Figure out if this can be used to create Dock windows
  ...
  enum WindowAction {
    // Do not show or activate the dock window after navigating.
    NO_ACTION,
    // Show and activate the dock window after navigating.
    SHOW_WINDOW,
    // Show the dock window after navigating but do not activate.
...
#if !defined(OS_ANDROID)
  // [in]  Specifies a Dock object where the navigation could occur or the
  //       tab could be added. Navigate() is not obliged to use this Dock if
  //       it is not compatible with the operation being performed. This can be
  //       NULL, in which case |initiating_workspace| must be provided.
  // [out] Specifies the Dock object where the navigation occurred or the
  ...
  // Note: If |show_window| is set to false and a new Dock is created by
  //       Navigate(), the caller is responsible for showing it so that its
  //       window can assume responsibility for the Dock's lifetime (Dock
  //       objects are deleted when the user closes a visible dock window).
  Dock* dock = nullptr;
#endif

  // The workspace that is initiating the navigation. If there is a non-NULL
  // dock passed in via |dock|, it's workspace will be used instead.
  scoped_refptr<Workspace> initiating_workspace;
  ...
  // ApplicationContentsDelegate::OpenURLFromTab implementation and is used to determine
  ...
  TabStyle tab_style = TabStyle::kAPP;
