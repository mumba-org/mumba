...
class DockWindowLayout : public views::LayoutManager {
 public:
  DockWindowLayout();
  ~DockWindowLayout() override;

  // Sets all the views to be managed. Takes ownership of |delegate|.
  // |browser_view| may be null in tests.
  void Init(DockWindowLayoutDelegate* delegate,
            Dock* dock,
  ...
  void set_tablist(Tablist* tablist) {
    tablist_ = tablist;
  }
 ...
 private:
  ...
  int LayoutTablistRegion(int top);
  ...
  std::unique_ptr<DockWindowLayoutDelegate> delegate_;

  // The browser from the owning BrowserView.
  Dock* dock_;

  // The owning browser view.
  views::ClientView* dock_window_;
  ...
  DISALLOW_COPY_AND_ASSIGN(DockWindowLayout);
};
