...
#include "core/host/ui/tablist/new_tab_promo_bubble_view.h"
...
NewTabButton::NewTabButton(Tablist* tablist, views::ButtonListener* listener)
    : views::ImageButton(listener),
      tablist_(tablist),
      is_incognito_(tablist->IsIncognito()) {
...
void NewTabButton::ShowPromoForLastActiveBrowser() {
  DockWindow* dock = static_cast<DockWindow*>(
      DockList::GetInstance()->GetLastActive()->window());

  dock->tablist()->new_tab_button()->ShowPromo();
}

// static
void NewTabButton::CloseBubbleForLastActiveBrowser() {
  DockWindow* dock =  DockList::GetInstance()->GetLastActive()->window();
  dock->tablist()->new_tab_button()->CloseBubble();
}
  ...
  const SkColor stroke_color =
      new_tab_promo_observer_.IsObservingSources()
          ? color_utils::AlphaBlend(
                ...
                0x70)
          : tablist_->GetToolbarTopSeparatorColor();
...
bool NewTabButton::GetHitTestMask(gfx::Path* mask) const {
  ...
  GetBorderPath(GetTopOffset() * scale, scale,
                tablist_->SizeTabButtonToTopOfTablist(), &border);
...
int NewTabButton::GetCornerRadius() const {
  return DockLayoutProvider::Get()->GetCornerRadiusMetric(
...
void NewTabButton::PaintFill(bool pressed,
  ...
  if (is_touch_ui || !pressed) {
    ...
    bool custom_image;
    const int bg_id = tablist_->GetBackgroundResourceId(&custom_image);
    ...
    const SkColor stroke_color = tablist_->GetToolbarTopSeparatorColor();
    const SkAlpha alpha =
  ...
  // Tablist::PaintChildren(), but the pressed state is excluded there and
