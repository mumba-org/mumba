...
struct LastMuteMetadata
    : public host::ApplicationContentsUserData<LastMuteMetadata> {
  host::TabMutedReason reason = host::TabMutedReason::NONE;
  std::string extension_id;

 private:
  explicit LastMuteMetadata(host::ApplicationContents* contents) {}
  friend class host::ApplicationContentsUserData<LastMuteMetadata>;
};
...
TabAlertState GetTabAlertStateForContents(ApplicationContents* contents) {
  if (!contents)
  ...
  //UsbTabHelper* usb_tab_helper = UsbTabHelper::FromApplicationContents(contents);
...
// bool CanToggleAudioMute(ApplicationContents* contents) {
...
// TabMutedReason GetTabAudioMutedReason(ApplicationContents* contents) {
...
// const std::string& GetExtensionIdForMutedTab(ApplicationContents* contents) {
...
// TabMutedResult SetTabAudioMuted(ApplicationContents* contents,
//                                 bool mute,
...
// void UnmuteIfMutedByExtension(ApplicationContents* contents,
//                               const std::string& extension_id) {
//   LastMuteMetadata::CreateForWebContents(contents);  // Ensures metadata exists.
//   LastMuteMetadata* const metadata =
//       LastMuteMetadata::FromWebContents(contents);
...
// bool AreAllTabsMuted(const TablistModel& tab_strip,
...
// void SetSitesMuted(const TablistModel& tab_strip,
//                    const std::vector<int>& indices,
//                    const bool mute) {
//   for (int tab_index : indices) {
//     ApplicationContents* web_contents = tab_strip.GetWebContentsAt(tab_index);
...
// bool IsSiteMuted(const TablistModel& tab_strip, const int index) {
//   ApplicationContents* web_contents = tab_strip.GetWebContentsAt(index);
...
// bool AreAllSitesMuted(const TablistModel& tab_strip,
//                       const std::vector<int>& indices) {
//   for (int tab_index : indices) {
