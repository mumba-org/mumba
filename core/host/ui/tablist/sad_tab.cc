...
#include "core/host/memory/oom_memory_details.h"
#endif
...
bool AreOtherTabsOpen() {
  ...
  for (auto* browser : *DockList::GetInstance())
    tab_count += browser->tablist_model()->count();
  return (tab_count > 1U);
}
...
std::vector<int> SadTab::GetSubMessages() {
  ...
  switch (kind_) {
    ...
    case SAD_TAB_KIND_KILLED:
      ...
      //if (!app_contents_->GetBrowserContext()->IsOffTheRecord())
...
void SadTab::PerformAction(SadTab::Action action) {
  ...
  switch (action) {
    case Action::BUTTON:
      ...
      // if (show_feedback_button_) {
      //   ShowFeedbackPage(
      //       chrome::FindBrowserWithWebContents(app_contents_),
      ...
      // } else {
      //   app_contents_->GetController().Reload(content::ReloadType::NORMAL,
      //                                         true);
      // }
      break;
    ...
    //              ui_metrics::SadTabEvent::HELP_LINK_CLICKED);
      ...
      //app_contents_->OpenURL(params);
      break;
  }
}

SadTab::SadTab(ApplicationContents* app_contents, SadTabKind kind)
  ...
  switch (kind) {
    ...
    case SAD_TAB_KIND_KILLED_BY_OOM:
      ...
      {
        const std::string spec = app_contents->GetURL().GetOrigin().spec();
    ...
    case SAD_TAB_KIND_KILLED:
      ...
      LOG(WARNING) << "Tab Killed: "
                   << app_contents->GetURL().GetOrigin().spec();
      break;
  }
}
