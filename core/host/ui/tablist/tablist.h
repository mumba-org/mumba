...
#include "core/host/ui/tablist/tab_controller.h"
...
namespace host {
class NewTabButton;
class StackedTablistLayout;
class Tab;
class TabDragController;
class TablistController;
class TablistObserver;

// A View that represents the TablistModel. The Tablist has the
// following responsibilities:
//
//  - It implements the TablistModelObserver interface, and acts as a
...
class Tablist : public views::View,
 ...
 public:
  ...
  explicit Tablist(std::unique_ptr<TablistController> controller);
  ~Tablist() override;

  // Returns whether or not strokes should be drawn around and under the tabs.
  static bool ShouldDrawStrokes();

  // Add and remove observers to changes within this Tablist.
  void AddObserver(TablistObserver* observer);
  void RemoveObserver(TablistObserver* observer);
  ...
  // Returns true if the specified rect (in Tablist coordinates) intersects
  // the window caption area of the browser window.
  bool IsRectInWindowCaption(const gfx::Rect& rect);

  // Returns true if the specified point (in Tablist coordinates) is in the
  ...
  void SetTabIsCrashed(int tab_index);
  ...
  // strip.
  bool SizeTabButtonToTopOfTablist();
  ...
  void AddTabAt(int model_index, TabRendererData data, bool is_active, TabStyle style);
  ...
  void RemoveTabAt(ApplicationContents* contents, int model_index);
  ...
  // Cover method for TablistController::GetCount.
  int GetModelCount() const;

  // Cover method for TablistController::IsValidIndex.
  bool IsValidModelIndex(int model_index) const;

  TablistController* controller() const { return controller_.get(); }
  ...
  // Returns true if Tabs in this Tablist are currently changing size or
  // position.
  ...
  // to allow for a drop region that extends outside the bounds of the Tablist.
 ...
 private:
  ...
  friend class TablistTest;
  FRIEND_TEST_ALL_PREFIXES(TabDragControllerTest, GestureEndShouldEndDragTest);
  FRIEND_TEST_ALL_PREFIXES(TablistTest, TabForEventWhenStacked);
  FRIEND_TEST_ALL_PREFIXES(TablistTest, TabCloseButtonVisibilityWhenStacked);
  FRIEND_TEST_ALL_PREFIXES(TablistTest,
                           TabCloseButtonVisibilityWhenNotStacked);
  FRIEND_TEST_ALL_PREFIXES(TablistTest, ActiveTabWidthWhenTabsAreTiny);
  ...
  // Cleans up the Tab from the Tablist. This is called from the tab animation
  ...
  // Perform an animated resize-relayout of the Tablist immediately.
  ...
  // Starts various types of Tablist animations.
  void StartResizeLayoutAnimation();
  void StartPinnedTabAnimation();
  void StartMouseInitiatedRemoveTabAnimation(int model_index);

  // Returns true if the specified point in Tablist coords is within the
  ...
  base::ObserverList<TablistObserver> observers_;

  // There is a one-to-one mapping between each of the tabs in the
  // TablistController (TablistModel) and |tabs_|. Because we animate tab
  ...
  std::unique_ptr<TablistController> controller_;
  ...
  std::unique_ptr<StackedTablistLayout> touch_layout_;
  ...
  DISALLOW_COPY_AND_ASSIGN(Tablist);
};
