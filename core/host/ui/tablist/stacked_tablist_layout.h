...
namespace host {
class Tab;
// StackedTablistLayout is used by Tablist in touch
// mode. StackedTablistLayout is responsible for managing the bounds of the
// tabs. StackedTablistLayout differs from the normal layout in that it stacks
...
class StackedTablistLayout {
 public:
  ...
  StackedTablistLayout(const gfx::Size& size,
                        int overlap,
                        int stacked_padding,
                        int max_stacked_count,
                        views::ViewModelBase* view_model);
  ~StackedTablistLayout();
  ...
  // StackedTablistLayout forces the active index to be in the normal tabs.
 ...
 private:
  friend class StackedTablistLayoutTest;
  ...
  // Where bounds are placed. This is owned by Tablist.
  ...
  DISALLOW_COPY_AND_ASSIGN(StackedTablistLayout);
};
