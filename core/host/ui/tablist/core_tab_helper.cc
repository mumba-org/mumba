...
CoreTabHelper::CoreTabHelper(ApplicationContents* application_contents)
...
CoreTabHelper::~CoreTabHelper() {
  //DLOG(INFO) << "~CoreTabHelper: " << this;
}

base::string16 CoreTabHelper::GetDefaultTitle() {
  // just using something
  return l10n_util::GetStringUTF16(IDS_TAB_CXMENU_NEWTAB);//IDS_DEFAULT_TAB_TITLE);
}

base::string16 CoreTabHelper::GetStatusText() const {
  base::string16 status_text;
  GetStatusTextForApplicationContents(&status_text, application_contents());
  return status_text;
}
...
// #if !defined(OS_ANDROID)
//   Dock* dock = chrome::FindBrowserWithApplicationContents(application_contents());
//   if (!dock)
//     return;

//   dock->command_controller()->ContentRestrictionsChanged();
// #endif
}
...
// static
bool CoreTabHelper::GetStatusTextForApplicationContents(
    base::string16* status_text, ApplicationContents* source) {
  if (!source->IsLoading() ||
...
void CoreTabHelper::OnVisibilityChanged(Visibility visibility) {
...
//        application_contents()->GetMainFrame()->GetProcess()->GetID());
//  }
}

void CoreTabHelper::ApplicationContentsDestroyed() {
  // OnCloseStarted isn't called in unit tests.
  if (!close_start_time_.is_null()) {
    bool fast_tab_close_enabled = false;
...
// Update back/forward buttons for application_contents that are active.
//void CoreTabHelper::NavigationEntriesDeleted() {
//#if !defined(OS_ANDROID)
//  for (Dock* dock : *DockList::GetInstance()) {
//    if (application_contents() == dock->tablist_model()->GetActiveApplicationContents())
//      dock->command_controller()->TabStateChanged();
//  }
//#endif
//}
...
//         content_type.c_str());
//   }
//   application_contents()->OpenURL(open_url_params);
// }
