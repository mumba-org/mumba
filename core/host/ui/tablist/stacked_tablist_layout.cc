...
StackedTablistLayout::StackedTablistLayout(const gfx::Size& size,
                                           int overlap,
                                           int stacked_padding,
...
StackedTablistLayout::~StackedTablistLayout() {
}

void StackedTablistLayout::SetXAndPinnedCount(int x, int pinned_tab_count) {
  first_tab_x_ = x;
  x_ = x;
...
void StackedTablistLayout::SetWidth(int width) {
  if (width_ == width)
    return;
...
void StackedTablistLayout::SetActiveIndex(int index) {
...
void StackedTablistLayout::DragActiveTab(int delta) {
...
void StackedTablistLayout::SizeToFit() {
  if (!tab_count())
    return;
...
void StackedTablistLayout::AddTab(int index, int add_types, int start_x) {
...
void StackedTablistLayout::RemoveTab(int index, int start_x, int old_x) {
...
void StackedTablistLayout::MoveTab(int from,
                                    int to,
...
bool StackedTablistLayout::IsStacked(int index) const {
...
void StackedTablistLayout::SetActiveTabLocation(int x) {
  if (!requires_stacking())
    return;
...
#if !defined(NDEBUG)
std::string StackedTablistLayout::BoundsString() const {
  std::string result;
...
void StackedTablistLayout::Reset(int x,
                                  int width,
                                  int pinned_tab_count,
                                  int active_index) {
  x_ = x;
...
void StackedTablistLayout::ResetToIdealState() {
...
void StackedTablistLayout::MakeVisible(int index) {
...
int StackedTablistLayout::ConstrainActiveX(int x) const {
  return std::min(GetMaxX(active_index()),
                  std::max(GetMinX(active_index()), x));
}

void StackedTablistLayout::SetActiveBoundsAndLayoutFromActiveTab() {
...
void StackedTablistLayout::LayoutByTabOffsetAfter(int index) {
...
void StackedTablistLayout::LayoutByTabOffsetBefore(int index) {
...
void StackedTablistLayout::LayoutUsingCurrentAfter(int index) {
  for (int i = index + 1; i < tab_count(); ++i) {
...
void StackedTablistLayout::LayoutUsingCurrentBefore(int index) {
  for (int i = index - 1; i >= pinned_tab_count_; --i) {
...
void StackedTablistLayout::PushTabsAfter(int index, int delta) {
  for (int i = index + 1; i < tab_count(); ++i)
    SetIdealBoundsAt(i, std::min(ideal_x(i) + delta, GetMaxDragX(i)));
}

void StackedTablistLayout::PushTabsBefore(int index, int delta) {
  for (int i = index - 1; i > pinned_tab_count_; --i)
    SetIdealBoundsAt(i, std::max(ideal_x(i) - delta, GetMinDragX(i)));
}

void StackedTablistLayout::LayoutForDragAfter(int index) {
  for (int i = index + 1; i < tab_count(); ++i) {
...
void StackedTablistLayout::LayoutForDragBefore(int index) {
  for (int i = index - 1; i >= pinned_tab_count_; --i) {
...
void StackedTablistLayout::ExpandTabsBefore(int index, int delta) {
...
void StackedTablistLayout::ExpandTabsAfter(int index, int delta) {
...
void StackedTablistLayout::AdjustStackedTabs() {
...
void StackedTablistLayout::AdjustLeadingStackedTabs() {
...
void StackedTablistLayout::AdjustTrailingStackedTabs() {
...
void StackedTablistLayout::SetIdealBoundsAt(int index, int x) {
  view_model_->set_ideal_bounds(index, gfx::Rect(gfx::Point(x, 0), size_));
}

int StackedTablistLayout::GetMinX(int index) const {
...
int StackedTablistLayout::GetMaxX(int index) const {
  int leading_count = index - pinned_tab_count_;
