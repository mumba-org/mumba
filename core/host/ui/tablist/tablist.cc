...
#include "core/host/ui/tablist/new_tab_button.h"
...
#include "core/host/ui/tablist/tablist_layout.h"
#include "core/host/ui/tablist/tablist_observer.h"
//#include "core/host/ui/touch_uma/touch_uma.h"
...
const int kTablistAnimationVSlop = 40;
...
enum NewTabButtonPosition {
  LEADING,     // Pinned to the leading edge of the tablist region.
  AFTER_TABS,  // After the last tab.
  TRAILING,    // Pinned to the trailing edge of the tablist region.
};

// Animation delegate used for any automatic tab movement.  Hides the tab if it
// is not fully visible within the tablist area, to prevent overflow clipping.
class TabAnimationDelegate : public gfx::AnimationDelegate {
 public:
  TabAnimationDelegate(Tablist* tablist, Tab* tab);
  ~TabAnimationDelegate() override;

  void AnimationProgressed(const gfx::Animation* animation) override;

 protected:
  Tablist* tablist() { return tablist_; }
  Tab* tab() { return tab_; }

 private:
  Tablist* const tablist_;
  Tab* const tab_;

  DISALLOW_COPY_AND_ASSIGN(TabAnimationDelegate);
};

TabAnimationDelegate::TabAnimationDelegate(Tablist* tablist, Tab* tab)
    : tablist_(tablist), tab_(tab) {}

TabAnimationDelegate::~TabAnimationDelegate() {}

void TabAnimationDelegate::AnimationProgressed(
    const gfx::Animation* animation) {
  tab_->SetVisible(tablist_->ShouldTabBeVisible(tab_));
}
...
class ResetDraggingStateDelegate : public TabAnimationDelegate {
 public:
  ResetDraggingStateDelegate(Tablist* tablist, Tab* tab);
...
ResetDraggingStateDelegate::ResetDraggingStateDelegate(Tablist* tablist,
                                                       Tab* tab)
    : TabAnimationDelegate(tablist, tab) {}
...
///////////////////////////////////////////////////////////////////////////////
// Tablist::RemoveTabDelegate
//
// AnimationDelegate used when removing a tab. Does the necessary cleanup when
// done.
class Tablist::RemoveTabDelegate : public TabAnimationDelegate {
 public:
  RemoveTabDelegate(Tablist* tablist, Tab* tab);
...
Tablist::RemoveTabDelegate::RemoveTabDelegate(Tablist* tablist, Tab* tab)
    : TabAnimationDelegate(tablist, tab) {}

void Tablist::RemoveTabDelegate::AnimationEnded(
    const gfx::Animation* animation) {
  DCHECK(tab()->closing());
  tablist()->RemoveAndDeleteTab(tab());
  ...
  // drag-detaches.
  if (!tablist()->IsDragSessionActive() &&
      tablist()->ShouldHighlightCloseButtonAfterRemove()) {
    // The widget can apparently be null during shutdown.
    views::Widget* widget = tablist()->GetWidget();
    if (widget)
      widget->SynthesizeMouseMoveEvent();
  }
}

void Tablist::RemoveTabDelegate::AnimationCanceled(
    const gfx::Animation* animation) {
  AnimationEnded(animation);
}
...
Tablist::Tablist(std::unique_ptr<TablistController> controller)
...
Tablist::~Tablist() {
  for (TablistObserver& observer : observers_)
    observer.TablistDeleted(this);
...
// static
bool Tablist::ShouldDrawStrokes() {
  return false;//MD::GetMode() != MD::MATERIAL_REFRESH;
}

void Tablist::AddObserver(TablistObserver* observer) {
  observers_.AddObserver(observer);
}

void Tablist::RemoveObserver(TablistObserver* observer) {
  observers_.RemoveObserver(observer);
}

Tablist::NewTabButtonPosition Tablist::GetNewTabButtonPosition() const {
  //if (MD::GetMode() != MD::MATERIAL_REFRESH)
  return AFTER_TABS;

  //const auto* frame_view = static_cast<const DockNonClientFrameView*>(
  //    GetWidget()->non_client_view()->frame_view());
  //return LEADING;//frame_view->CaptionButtonsOnLeadingEdge() ? TRAILING : LEADING;
}

int Tablist::GetMaxX() const {
  // This function should not currently be called for TRAILING mode; if it is,
  // the API will need changing, since callers assume the tablist uses
...
void Tablist::SetBackgroundOffset(const gfx::Point& offset) {
  for (int i = 0; i < tab_count(); ++i)
    tab_at(i)->set_background_offset(offset);
  new_tab_button_->set_background_offset(offset);
}

bool Tablist::IsRectInWindowCaption(const gfx::Rect& rect) {
...
bool Tablist::IsPositionInWindowCaption(const gfx::Point& point) {
  return IsRectInWindowCaption(gfx::Rect(point, gfx::Size(1, 1)));
}

bool Tablist::IsTablistCloseable() const {
  return !IsDragSessionActive();
}

bool Tablist::IsTablistEditable() const {
  return !IsDragSessionActive() && !IsActiveDropTarget();
}

bool Tablist::IsTabCrashed(int tab_index) const {
  return tab_at(tab_index)->data().IsCrashed();
}

void Tablist::SetTabIsCrashed(int tab_index) {
  tab_at(tab_index)->SetIsCrashed();
}

bool Tablist::TabHasNetworkError(int tab_index) const {
  return tab_at(tab_index)->data().network_state == TabNetworkState::kError;
}

TabAlertState Tablist::GetTabAlertState(int tab_index) const {
  return tab_at(tab_index)->data().alert_state;
}

void Tablist::UpdateLoadingAnimations() {
  for (int i = 0; i < tab_count(); i++)
    tab_at(i)->StepLoadingAnimation();
}

void Tablist::SetStackedLayout(bool stacked_layout) {
...
gfx::Rect Tablist::GetNewTabButtonBounds() {
  return new_tab_button_->bounds();
}

bool Tablist::SizeTabButtonToTopOfTablist() {
  // Extend the button to the screen edge in maximized and immersive fullscreen.
  views::Widget* widget = GetWidget();
  return //browser_defaults::kSizeTabButtonToTopOfTablist ||
         (widget && (widget->IsMaximized() || widget->IsFullscreen()));
}

void Tablist::StartHighlight(int model_index) {
  tab_at(model_index)->StartPulse();
}

void Tablist::StopAllHighlighting() {
  for (int i = 0; i < tab_count(); ++i)
    tab_at(i)->StopPulse();
}

void Tablist::AddTabAt(int model_index, TabRendererData data, bool is_active, TabStyle style) {
  Tab* tab = new Tab(this, animation_container_.get(), style);
  AddChildView(tab);
  ...
  if (touch_layout_) {
    ...
    int add_types = 0;
    if (pinned)
      add_types |= StackedTablistLayout::kAddTypePinned;
    if (is_active)
      add_types |= StackedTablistLayout::kAddTypeActive;
  ...
  for (TablistObserver& observer : observers_)
    observer.TablistAddedTabAt(this, model_index);
...
void Tablist::MoveTab(int from_model_index,
                       int to_model_index,
  ...
  for (TablistObserver& observer : observers_)
    observer.TablistMovedTab(this, from_model_index, to_model_index);
}

void Tablist::RemoveTabAt(ApplicationContents* contents, int model_index) {
  if (touch_layout_) {
  ...
  for (TablistObserver& observer : observers_)
    observer.TablistRemovedTabAt(this, model_index);
  ...
  // the model and tablist are in sync.
...
void Tablist::SetTabData(int model_index, TabRendererData data) {
  Tab* tab = tab_at(model_index);
...
bool Tablist::ShouldTabBeVisible(const Tab* tab) const {
  ...
  const int tablist_right =
      trailing_new_tab_button ? GetTabAreaWidth() : width();
  if (right_edge > tablist_right)
    return false;
...
void Tablist::PrepareForCloseAt(int model_index, CloseTabSource source) {
...
void Tablist::SetSelection(const ui::ListSelectionModel& old_selection,
  ...
  if (touch_layout_) {
    ...
    if (current_inactive_width_ == current_active_width_) {
      ...
      // closure--we won't expand the tablist back to the full window
...
void Tablist::TabTitleChangedNotLoading(int model_index) {
  tab_at(model_index)->TabTitleChangedNotLoading();
}

void Tablist::SetTabNeedsAttention(int model_index, bool attention) {
  tab_at(model_index)->SetTabNeedsAttention(attention);
}

int Tablist::GetModelIndexOfTab(const Tab* tab) const {
  return tabs_.GetIndexOfView(tab);
}

int Tablist::GetModelCount() const {
  return controller_->GetCount();
}

bool Tablist::IsValidModelIndex(int model_index) const {
  return controller_->IsValidIndex(model_index);
}

bool Tablist::IsDragSessionActive() const {
  return drag_controller_.get() != NULL;
}

bool Tablist::IsActiveDropTarget() const {
  for (int i = 0; i < tab_count(); ++i) {
    Tab* tab = tab_at(i);
    if (tab->dragging())
      return true;
...
SkAlpha Tablist::GetInactiveAlpha(bool for_new_tab_button) const {
#if defined(OS_CHROMEOS)
...
bool Tablist::IsAnimating() const {
  return bounds_animator_.IsAnimating();
}

void Tablist::StopAnimating(bool layout) {
  if (!IsAnimating())
    return;
...
void Tablist::ToggleTabAudioMute(Tab* tab) {
  int model_index = GetModelIndexOfTab(tab);
  if (IsValidModelIndex(model_index))
    controller_->ToggleTabAudioMute(model_index);
}

void Tablist::ShowContextMenuForTab(Tab* tab,
                                     const gfx::Point& p,
                                     ui::MenuSourceType source_type) {
  controller_->ShowContextMenuForTab(tab, p, source_type);
}

bool Tablist::IsActiveTab(const Tab* tab) const {
  int model_index = GetModelIndexOfTab(tab);
  return IsValidModelIndex(model_index) &&
         controller_->IsActiveTab(model_index);
}

bool Tablist::IsTabSelected(const Tab* tab) const {
  int model_index = GetModelIndexOfTab(tab);
  return IsValidModelIndex(model_index) &&
         controller_->IsTabSelected(model_index);
}

bool Tablist::IsTabPinned(const Tab* tab) const {
  if (tab->closing())
    return false;
...
void Tablist::MaybeStartDrag(
  ...
  // creating the DragController remembers the ApplicationContents delegates and we need
...
void Tablist::ContinueDrag(views::View* view, const ui::LocatedEvent& event) {
...
bool Tablist::EndDrag(EndDragReason reason) {
  if (!drag_controller_.get())
    return false;
...
Tab* Tablist::GetTabAt(Tab* tab, const gfx::Point& tab_in_tab_coordinates) {
  ...
  // Walk up the view hierarchy until we find a tab, or the Tablist.
...
void Tablist::OnMouseEventInTab(views::View* source,
                                 const ui::MouseEvent& event) {
  UpdateStackedLayoutFromMouseEvent(source, event);
}

bool Tablist::ShouldPaintTab(
    const Tab* tab,
...
bool Tablist::CanPaintThrobberToLayer() const {
...
SkColor Tablist::GetToolbarTopSeparatorColor() const {
  return controller_->GetToolbarTopSeparatorColor();
}

// Returns the accessible tab name for the tab.
base::string16 Tablist::GetAccessibleTabName(const Tab* tab) const {
  int model_index = GetModelIndexOfTab(tab);
  if (IsValidModelIndex(model_index))
...
int Tablist::GetBackgroundResourceId(bool* custom_image) const {
...
void Tablist::MouseMovedOutOfHost() {
  ResizeLayoutTabs();
...
void Tablist::PaintChildren(const views::PaintInfo& paint_info) {
...
const char* Tablist::GetClassName() const {
  static const char kViewClassName[] = "Tablist";
  return kViewClassName;
}

gfx::Size Tablist::CalculatePreferredSize() const {
  ...
  if (touch_layout_ || adjust_layout_) {
    ...
    // Don't let the tablist shrink smaller than is necessary to show one tab,
...
void Tablist::OnDragEntered(const DropTargetEvent& event) {
...
int Tablist::OnDragUpdated(const DropTargetEvent& event) {
...
void Tablist::OnDragExited() {
  SetDropIndex(-1, false);
}

int Tablist::OnPerformDrop(const DropTargetEvent& event) {
  if (!drop_info_.get())
    return ui::DragDropTypes::DRAG_NONE;
...
void Tablist::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  node_data->role = ax::mojom::Role::kTabList;
}

views::View* Tablist::GetTooltipHandlerForPoint(const gfx::Point& point) {
  if (!HitTestPoint(point))
    return NULL;

  if (!touch_layout_) {
    // Return any view that isn't a Tab or this Tablist immediately. We don't
    // want to interfere.
...
void Tablist::StartInsertTabAnimation(int model_index) {
  PrepareForAnimation();

  // The Tablist can now use its entire width to lay out Tabs.
  in_tab_close_ = false;
  available_width_for_tabs_ = -1;

  GenerateIdealBounds();

  // Set the current bounds to be the correct portal but 0 width.
  Tab* tab = tab_at(model_index);
  if (model_index == 0) {
...
void Tablist::StartMoveTabAnimation() {
  PrepareForAnimation();
  GenerateIdealBounds();
  AnimateToIdealBounds();
}

void Tablist::StartRemoveTabAnimation(int model_index) {
  PrepareForAnimation();
...
void Tablist::ScheduleRemoveTabAnimation(Tab* tab) {
...
void Tablist::AnimateToIdealBounds() {
  for (int i = 0; i < tab_count(); ++i) {
    Tab* tab = tab_at(i);
    if (!tab->dragging()) {
...
bool Tablist::ShouldHighlightCloseButtonAfterRemove() {
  return in_tab_close_;
}

bool Tablist::MayHideNewTabButtonWhileDragging() const {
  return GetNewTabButtonPosition() == AFTER_TABS;
}

bool Tablist::TitlebarBackgroundIsTransparent() const {
#if defined(OS_WIN)
...
void Tablist::SetTabVisibility() {
  // We could probably be more efficient here by making use of the fact that the
  // tablist will always have any visible tabs, and then any invisible tabs, so
...
void Tablist::DragActiveTab(const std::vector<int>& initial_positions,
                             int delta) {
...
void Tablist::SetIdealBoundsFromPositions(const std::vector<int>& positions) {
...
void Tablist::StackDraggedTabs(int delta) {
...
bool Tablist::IsStackingDraggedTabs() const {
...
void Tablist::LayoutDraggedTabsAt(const Tabs& tabs,
                                   Tab* active_tab,
...
void Tablist::CalculateBoundsForDraggedTabs(const Tabs& tabs,
                                             std::vector<gfx::Rect>* bounds) {
  const int overlap = Tab::GetOverlap();
  int x = 0;
  for (size_t i = 0; i < tabs.size(); ++i) {
...
int Tablist::NewTabButtonX() const {
  ...
  // tablist. Constrain the x-coordinate of the new tab button so that it is
  // always visible.
...
int Tablist::GetSizeNeededForTabs(const Tabs& tabs) {
  int width = 0;
  for (size_t i = 0; i < tabs.size(); ++i) {
    Tab* tab = tabs[i];
    width += tab->width();
...
int Tablist::GetPinnedTabCount() const {
  int pinned_count = 0;
...
const Tab* Tablist::GetLastVisibleTab() const {
  for (int i = tab_count() - 1; i >= 0; --i) {
    const Tab* tab = tab_at(i);
    if (tab->visible())
      return tab;
  }
  // While in normal use the tablist should always be wide enough to have at
  // least one visible tab, it can be zero-width in tests, meaning we get here.
  return NULL;
}

void Tablist::RemoveTabFromViewModel(int index) {
...
void Tablist::RemoveAndDeleteTab(Tab* tab) {
...
void Tablist::UpdateTabsClosingMap(int index, int delta) {
...
void Tablist::StartedDraggingTabs(const Tabs& tabs) {
...
void Tablist::DraggedTabsDetached() {
  // Let the controller know that the user is not dragging this tablist's tabs
  // anymore.
  controller_->OnStoppedDraggingTabs();
  new_tab_button_->SetVisible(true);
}

void Tablist::StoppedDraggingTabs(const Tabs& tabs,
...
void Tablist::StoppedDraggingTab(Tab* tab, bool* is_first_tab) {
...
void Tablist::OwnDragController(TabDragController* controller) {
  ...
  // potential source tablist in MaybeStartDrag(). If a drag actually starts,
  // we then call Attach() on the source tablist, but since the source tablist
...
void Tablist::DestroyDragController() {
  new_tab_button_->SetVisible(true);
  drag_controller_.reset();
}

TabDragController* Tablist::ReleaseDragController() {
  return drag_controller_.release();
}

Tablist::FindClosingTabResult Tablist::FindClosingTab(const Tab* tab) {
  DCHECK(tab->closing());
...
void Tablist::PaintClosingTabs(int index, const views::PaintInfo& paint_info) {
...
void Tablist::UpdateStackedLayoutFromMouseEvent(views::View* source,
                                                 const ui::MouseEvent& event) {
  if (!adjust_layout_)
    return;

  // The following code attempts to switch to shrink (not stacked) layout when
  // the mouse exits the tablist (or the mouse is pressed on a stacked tab) and
  ...
  switch (event.type()) {
    case ui::ET_MOUSE_PRESSED:
      ...
      if (reset_to_shrink_on_exit_ && touch_layout_) {
        gfx::Point tablist_point(event.location());
        views::View::ConvertPointToTarget(source, this, &tablist_point);
        Tab* tab = FindTabForEvent(tablist_point);
...
void Tablist::ResizeLayoutTabs() {
  // We've been called back after the Tablist has been emptied out (probably
...
void Tablist::ResizeLayoutTabsFromTouch() {
  // Don't resize if the user is interacting with the tablist.
  if (!drag_controller_.get())
    ResizeLayoutTabs();
  else
    StartResizeLayoutTabsFromTouchTimer();
...
void Tablist::StartResizeLayoutTabsFromTouchTimer() {
  resize_layout_timer_.Stop();
  resize_layout_timer_.Start(
      FROM_HERE, base::TimeDelta::FromMilliseconds(kTouchResizeLayoutTimeMS),
      this, &Tablist::ResizeLayoutTabsFromTouch);
}

void Tablist::SetTabBoundsForDrag(const std::vector<gfx::Rect>& tab_bounds) {
  StopAnimating(false);
...
void Tablist::AddMessageLoopObserver() {
  if (!mouse_watcher_.get()) {
    mouse_watcher_ = std::make_unique<views::MouseWatcher>(
        std::make_unique<views::MouseWatcherViewHost>(
            this, gfx::Insets(0, 0, kTablistAnimationVSlop, 0)),
        this);
  }
  mouse_watcher_->Start();
}

void Tablist::RemoveMessageLoopObserver() {
  mouse_watcher_.reset(NULL);
}

gfx::Rect Tablist::GetDropBounds(int drop_index,
                                  bool drop_before,
                                  bool* is_beneath) {
...
void Tablist::UpdateDropIndex(const DropTargetEvent& event) {
...
void Tablist::SetDropIndex(int tab_data_index, bool drop_before) {
...
int Tablist::GetDropEffect(const ui::DropTargetEvent& event) {
...
// static
gfx::ImageSkia* Tablist::GetDropArrowImage(bool is_down) {
...
Tablist::DropInfo::DropInfo(int drop_index,
                             bool drop_before,
                             bool point_down,
...
void Tablist::PrepareForAnimation() {
...
void Tablist::GenerateIdealBounds() {
  if (tab_count() == 0)
  ...
  if (GetMaxX() != old_max_x) {
    for (TablistObserver& observer : observers_)
      observer.TablistMaxXChanged(this);
  }
}

int Tablist::GenerateIdealBoundsForPinnedTabs(int* first_non_pinned_index) {
...
int Tablist::GetTabAreaWidth() const {
  return width() - GetNewTabButtonWidth(IsIncognito());
}

void Tablist::StartResizeLayoutAnimation() {
  PrepareForAnimation();
  GenerateIdealBounds();
  AnimateToIdealBounds();
}

void Tablist::StartPinnedTabAnimation() {
  in_tab_close_ = false;
  available_width_for_tabs_ = -1;
...
void Tablist::StartMouseInitiatedRemoveTabAnimation(int model_index) {
  PrepareForAnimation();
...
bool Tablist::IsPointInTab(Tab* tab,
                            const gfx::Point& point_in_tablist_coords) {
  gfx::Point point_in_tab_coords(point_in_tablist_coords);
  View::ConvertPointToTarget(this, tab, &point_in_tab_coords);
  return tab->HitTestPoint(point_in_tab_coords);
}

int Tablist::GetStartXForNormalTabs() const {
  const int start_x = TabStartX();
  const int pinned_tab_count = GetPinnedTabCount();
...
// static
void Tablist::ResetTabSizeInfoForTesting() {
  if (g_tab_size_info) {
...
Tab* Tablist::FindTabForEvent(const gfx::Point& point) {
  DCHECK(touch_layout_);
...
Tab* Tablist::FindTabForEventFrom(const gfx::Point& point,
                                   int start,
                                   int delta) {
...
Tab* Tablist::FindTabHitByPoint(const gfx::Point& point) {
...
std::vector<int> Tablist::GetTabXCoordinates() {
...
void Tablist::SwapLayoutIfNecessary() {
...
bool Tablist::NeedsTouchLayout() const {
  if (!stacked_layout_)
    return false;
...
void Tablist::SetResetToShrinkOnExit(bool value) {
  ...
  // Add an observer so we know when the mouse moves out of the tablist.
  if (reset_to_shrink_on_exit_)
    AddMessageLoopObserver();
  else
    RemoveMessageLoopObserver();
...
void Tablist::ButtonPressed(views::Button* sender, const ui::Event& event) {
  if (sender == new_tab_button_) {
    base::RecordAction(UserMetricsAction("NewTab_Button"));
    UMA_HISTOGRAM_ENUMERATION("Tab.NewTab", TablistModel::NEW_TAB_BUTTON,
...
const views::View* Tablist::GetViewByID(int view_id) const {
  if (tab_count() > 0) {
...
bool Tablist::OnMousePressed(const ui::MouseEvent& event) {
...
bool Tablist::OnMouseDragged(const ui::MouseEvent& event) {
  ContinueDrag(this, event);
  return true;
}

void Tablist::OnMouseReleased(const ui::MouseEvent& event) {
  EndDrag(END_DRAG_COMPLETE);
  UpdateStackedLayoutFromMouseEvent(this, event);
}

void Tablist::OnMouseCaptureLost() {
  EndDrag(END_DRAG_CAPTURE_LOST);
}

void Tablist::OnMouseMoved(const ui::MouseEvent& event) {
  UpdateStackedLayoutFromMouseEvent(this, event);
}

void Tablist::OnMouseEntered(const ui::MouseEvent& event) {
  SetResetToShrinkOnExit(true);
}

void Tablist::OnGestureEvent(ui::GestureEvent* event) {
  SetResetToShrinkOnExit(false);
  switch (event->type()) {
...
views::View* Tablist::TargetForRect(views::View* root, const gfx::Rect& rect) {
  CHECK_EQ(root, this);
