...
//
// A TablistModel has one delegate that it relies on to perform certain tasks
// like creating new TablistModels (probably hosted in Browser windows) when
// required. See TabStripDelegate above for more information.
//
// A TablistModel also has N observers (see TablistModelObserver above),
...
//
////////////////////////////////////////////////////////////////////////////////
class TablistModel : public ApplicationContentsCloseDelegate {
 public:
  ...
  // Construct a TablistModel with a delegate to help it do certain things
  // (see the TablistModelDelegate documentation). |delegate| cannot be NULL.
  explicit TablistModel(TablistModelDelegate* delegate, scoped_refptr<Workspace> workspace);
  ~TablistModel() override;

  // Retrieves the TablistModelDelegate associated with this TablistModel.
  TablistModelDelegate* delegate() const { return delegate_; }

  // Add and remove observers to changes within this TablistModel.
  void AddObserver(TablistModelObserver* observer);
  void RemoveObserver(TablistModelObserver* observer);

  // Retrieve the number of WebContentses/emptiness of the TablistModel.
  int count() const { return static_cast<int>(contents_data_.size()); }
  bool empty() const { return contents_data_.empty(); }

  SkColor tablist_color() const;

  SkColor tab_color(int index) const;

  CustomThemeService* GetThemeServiceForTab(int index) const;
  CustomThemeService* GetThemeServiceForActiveTab() const;

  // Retrieve the Profile associated with this TablistModel.
  ...
  // Determines if the specified index is contained within the TablistModel.
  ...
  void AppendApplicationContents(std::unique_ptr<ApplicationContents> contents,
                                 bool foreground);
  ...
  void InsertApplicationContentsAt(int index,
  ...
  bool CloseApplicationContentsAt(int index, uint32_t close_types);
  ...
  // to the caller.
  std::unique_ptr<ApplicationContents> ReplaceApplicationContentsAt(
      int index,
  ...
  std::unique_ptr<ApplicationContents> DetachApplicationContentsAt(int index);
  ...
  // one slot.
  void MoveApplicationContentsAt(int index, int to_position, bool select_after_move);
  ...
  void SetTablistThemeColor(SkColor color, int tab_index);

  // Returns the currently active WebContents, or NULL if there is none.
  ApplicationContents* GetActiveApplicationContents() const;

  // Returns the WebContents at the specified index, or NULL if there is
  // none.
  ApplicationContents* GetApplicationContentsAt(int index) const;

  // Returns the index of the specified WebContents, or TablistModel::kNoTab
  // if the WebContents is not in this TablistModel.
  int GetIndexOfApplicationContents(const ApplicationContents* contents) const;
  ...
  void UpdateApplicationContentsStateAt(int index, TabChangeType change_type);
  ...
  ApplicationContents* GetOpenerOfApplicationContentsAt(int index);

  // Changes the |opener| of the WebContents at |index|.
  // Note: |opener| must be in this tab strip.
  void SetOpenerOfApplicationContentsAt(int index, ApplicationContents* opener);

  // Returns the index of the last WebContents in the model opened by the
  // specified opener, starting at |start_index|.
  int GetIndexOfLastApplicationContentsOpenedBy(const ApplicationContents* opener,
  ...
  // navigation, the TablistModel may adjust its selection and grouping
  // behavior.
  void TabNavigating(ApplicationContents* contents,
  ...
  // Adds a WebContents at the best position in the TablistModel given
  ...
  void AddApplicationContents(std::unique_ptr<ApplicationContents> contents,
                      int index,
                      ui::PageTransition transition,
                      int add_types);
  ...
  // TablistModel index |context_index|.  If |context_index| is selected the
  ...
  static bool ContextMenuCommandToDockCommand(int cmd_id, int* dock_cmd);

  // Access the order controller. Exposed only for unit tests.
  TablistModelOrderController* order_controller() const {
  ...
  // the opener may not.
  int GetIndexOfNextApplicationContentsOpenedBy(const ApplicationContents* opener,
                                                int start_index,
  ...
  void ForgetGroup(ApplicationContents* contents);
  ...
  bool ShouldResetGroupOnSelect(ApplicationContents* contents) const;

 private:
  class ApplicationContentsData;
  ...
  // ApplicationContentsCloseDelegate:
  bool ContainsApplicationContents(ApplicationContents* contents) override;
  void OnWillDeleteApplicationContents(ApplicationContents* contents,
  ...
  // WebContents.
  std::vector<ApplicationContents*> GetApplicationContentsFromIndices(
  ...
  bool IsNewTabAtEndOfTablist(ApplicationContents* contents) const;
  ...
  bool InternalCloseTabs(base::span<ApplicationContents* const> items,
                            uint32_t close_types);

  // Gets the WebContents at an index. Does no bounds checking.
  ApplicationContents* GetApplicationContentsAtImpl(int index) const;

  // Returns the WebContentses at the specified indices. This does no checking
  // of the indices, it is assumed they are valid.
  std::vector<ApplicationContents*> GetApplicationContentsesByIndices(
      const std::vector<int>& indices);

  // Notifies the observers if the active tab is being deactivated.
  void NotifyIfTabDeactivated(ApplicationContents* contents);

  // Notifies the observers if the active tab has changed.
  void NotifyIfActiveTabChanged(ApplicationContents* old_contents,
                                   Notify notify_types);
  ...
  void NotifyTablistColorChanged(SkColor color, int tab_index);
  ...
  void MoveApplicationContentsAtImpl(int index,
                              int to_position,
  ...
  static bool OpenerMatches(const std::unique_ptr<ApplicationContentsData>& data,
  ...
  // The WebContents data currently hosted within this TablistModel.
  std::vector<std::unique_ptr<ApplicationContentsData>> contents_data_;

  TablistModelDelegate* delegate_;
  base::ObserverList<TablistModelObserver> observers_;

  // A profile associated with this TablistModel.
  ...
  std::unique_ptr<TablistModelOrderController> order_controller_;
  ...
  base::WeakPtrFactory<TablistModel> weak_factory_;

  DISALLOW_IMPLICIT_CONSTRUCTORS(TablistModel);
};
