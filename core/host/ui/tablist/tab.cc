...
#include "core/host/ui/tablist/alert_indicator_button.h"
#include "core/host/ui/tablist/dock_tablist_controller.h"
#include "core/host/ui/tablist/tab_close_button.h"
#include "core/host/ui/tablist/tab_controller.h"
#include "core/host/ui/tablist/tab_icon.h"
...
gfx::Path GetInteriorPath(float scale,
  ...
  gfx::Path right_path;
  gfx::Path left_path;
  if (style == TabStyle::kMATERIAL) {
    const float radius = (endcap_width / 2) * scale;

    const float stroke_thickness = Tablist::ShouldDrawStrokes() ? 1 : 0;
    ...
    left_path.close();
  } else if (style == TabStyle::kAPP) {
    const float radius = 0.0;//(endcap_width / 2) * scale;
   // const float top_radius = (endcap_width / 2) * scale;
...
gfx::Path GetBorderPath(TabStyle style,
                        float scale,
                        bool unscale_at_end,
                        bool extend_to_top,
                        float endcap_width,
  ...
  if (style == TabStyle::kMATERIAL) {
    ...
    const float radius = 0.0f;//(endcap_width / 2) * scale;
    const float bottom_radius = 0.0f;//radius - stroke_thickness;
    const float top_radius = (endcap_width / 2) * scale;
...
Tab::Tab(TabController* controller, gfx::AnimationContainer* container, TabStyle style)
...
void Tab::SetData(TabRendererData data) {
  ...
  if (title.empty()) {
    ...
    Dock::FormatTitleForDisplay(&title);
  }
...
void Tab::SetIsCrashed() {
  data_.crashed_status = base::TERMINATION_STATUS_PROCESS_CRASHED;
  icon_->SetIsCrashed(true);
  Layout();
  SchedulePaint();
}
...
gfx::Size Tab::GetMinimumActiveSize() {
  ...
  minimum_size.Enlarge(TabIcon::FaviconSize(), 0);
  return minimum_size;
}
...
void Tab::PaintChildren(const views::PaintInfo& info) {
  ...
  clip_recorder.ClipPathWithAntiAliasing(GetInteriorPath(
      paint_recording_scale, size(), GetTabEndcapWidth(), 1 /* padding */, tab_style_));
  View::PaintChildren(info);
}

void Tab::Layout() {
  const gfx::Rect lb = GetContentsBounds();
  const int favicon_size = TabIcon::FaviconSize();
  ...
  if (showing_icon_) {
    ...
    favicon_bounds.set_y(lb.y() + (lb.height() - favicon_size + 1) / 2);
    ...
    if (center_favicon_) {
      ...
      MaybeAdjustLeftForPinnedTab(&favicon_bounds, favicon_size);
    }
  }
  ...
  if (showing_close_button_) {
    ...
    // for touch events, and the close-image should show up in the right portal.
...
bool Tab::GetTooltipText(const gfx::Point& p, base::string16* tooltip) const {
  ...
  *tooltip = AssembleTabTooltipText(data_.title, data_.alert_state);
...
void Tab::PaintTabBackground(gfx::Canvas* canvas,
  ...
  // FIXME
  const ui::ThemeProvider* tp = static_cast<DockTablistController*>(static_cast<Tablist*>(controller_)->controller())->model()->GetThemeServiceForActiveTab()->GetThemeProvider();
  ...
  if (fill_id || paint_hover_effect) {
    gfx::Path fill_path =
        GetInteriorPath(canvas->image_scale(), size(), endcap_width, 0, tab_style_);
    ...
    if (Tablist::ShouldDrawStrokes()) {
      gfx::Path stroke_path = GetBorderPath(tab_style_, 
                                            canvas->image_scale(), 
                                            false, 
                                            false,
                                            endcap_width, size());
  ...
  if (!cache.CacheKeyMatches(canvas->image_scale(), size(), active_color,
    ...
    gfx::Path fill_path =
        GetInteriorPath(canvas->image_scale(), size(), endcap_width, 0, tab_style_);
    gfx::Path stroke_path = GetBorderPath(tab_style_, canvas->image_scale(), false, false,
                                          endcap_width, size());
...
void Tab::UpdateIconVisibility() {
  ...
  const bool is_touch_optimized = MD::IsTouchOptimizedUiEnabled();
  const int favicon_width = TabIcon::FaviconSize();
