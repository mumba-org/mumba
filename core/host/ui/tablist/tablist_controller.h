...
class TablistController {
 public:
  ...
  // Returns true if the associated Tablist's delegate supports tab moving or
  ...
  virtual bool IsCompatibleWith(Tablist* other) const = 0;
  ...
  // Tablist::FileSupported to report the result.
  ...
  // Returns the profile associated with the Tablist.
  virtual scoped_refptr<Workspace> GetWorkspace() const = 0;
};
