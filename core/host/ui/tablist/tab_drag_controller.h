...
#include "core/host/ui/tablist/tablist_types.h"
...
// that the tabs should be moved out of the tab strip a new Dock is created
...
class TabDragController : public views::WidgetObserver,
 ...
 public:
  // What should happen as the mouse is dragged within the tablist.
  enum MoveBehavior {
  ...
  // Amount above or below the tablist the user has to drag before detaching.
  ...
  // |source_tablist|. |source_tab| is the tab that initiated the drag and is
  ...
  void Init(Tablist* source_tablist,
            Tab* source_tab,
  ...
  // finishing the drag.
  static bool IsAttachedTo(const Tablist* tablist);
 ...
 private:
  ...
  // Indicates what should happen after invoking DragDockToNewTablist().
  enum DragDockResultType {
  ...
  struct TabDragData {
    ...
    // The ApplicationContents being dragged.
    ApplicationContents* contents;

    // This is the index of the tab in |source_tablist_| when the drag
    // began. This is used to restore the previous state if the drag is aborted.
    int source_model_index;

    // If attached this is the tab in |attached_tablist_|.
    Tab* attached_tab;
  ...
  // notifications and resets the delegate of the ApplicationContents.
  ...
  // Overriden from TablistModelObserver:
  ...
  // Invoked once a drag has started to determine the appropriate tablist to
  // drag to (which may be the currently attached one).
  Liveness ContinueDragging(const gfx::Point& point_in_screen)
      WARN_UNUSED_RESULT;

  // Transitions dragging from |attached_tablist_| to |target_tablist|.
  // |target_tablist| is NULL if the mouse is not over a valid tab strip.  See
  // DragDockResultType for details of the return type.
  DragDockResultType DragDockToNewTablist(
      Tablist* target_tablist,
      const gfx::Point& point_in_screen);

  // Handles dragging for a touch tablist when the tabs are stacked. Doesn't
  ...
  // Returns the Tablist for the specified window, or NULL if one doesn't exist
  // or isn't compatible.
  Tablist* GetTablistForWindow(gfx::NativeWindow window);

  // Returns the compatible Tablist to drag to at the specified point (screen
  // coordinates), or NULL if there is none.
  Liveness GetTargetTablistForPoint(const gfx::Point& point_in_screen,
                                    Tablist** tablist);

  // Returns true if |tablist| contains the specified point in screen
  // coordinates.
  bool DoesTablistContain(Tablist* tablist,
  ...
  // Attach the dragged Tab to the specified Tablist.
  void Attach(Tablist* attached_tablist, const gfx::Point& point_in_screen);

  // Detach the dragged Tab from the current Tablist.
  void Detach(ReleaseCapture release_capture);

  // Detaches the tabs being dragged, creates a new Dock to contain them and
  // runs a nested move loop.
  void DetachIntoNewDockAndRunMoveLoop(const gfx::Point& point_in_screen);

  // Runs a nested run loop that handles moving the current
  // Dock. |drag_offset| is the offset from the window origin and is used in
  ...
  // Returns the index where the dragged ApplicationContents should be inserted into
  // |attached_tablist_| given the DraggedTabView's bounds |dragged_bounds| in
  // coordinates relative to |attached_tablist_| and has had the mirroring
  ...
  // Used by GetInsertionIndexForDraggedBounds() when the tablist is stacked.
  ...
  // Tablist. |tablist_point| is in the attached Tablist's coordinate
  // system.
  gfx::Rect GetDraggedViewTablistBounds(const gfx::Point& tablist_point);
  ...
  // Finds the Tabs within the specified Tablist that corresponds to the
  // ApplicationContents of the dragged tabs. Returns an empty vector if not attached.
  std::vector<Tab*> GetTabsMatchingDraggedContents(Tablist* tablist);
  ...
  // any dragged contents (as happens when a ApplicationContents is deleted out from
  // under us).
  void ResetSelection(TablistModel* model);

  // Restores |initial_selection_model_| to the |source_tablist_|.
  ...
  gfx::Rect GetViewScreenBounds(views::View* tablist) const;

  // Hides the frame for the window that contains the Tablist the current
  ...
  ApplicationContents* source_dragged_contents() {
    return source_tab_drag_data()->contents;
  }

  // Returns the Widget of the currently attached Tablist's DockView.
  views::Widget* GetAttachedDockWidget();
  ...
  // source browser.
  gfx::Rect CalculateDraggedDockBounds(Tablist* source,
                                       const gfx::Point& point_in_screen,
                                        std::vector<gfx::Rect>* drag_bounds);

  // Calculates scaled |drag_bounds| for dragged tabs and sets the tabs bounds.
  // Layout of the tablist is performed and a new tablist width calculated.
  // When |last_tablist_width| is larger than the new tablist width the tabs
  // in attached tablist are scaled and the attached browser is positioned such
  // that the tab that was dragged remains under the |point_in_screen|.
  void AdjustDockAndTabBoundsForDrag(int last_tablist_width,
                                        const gfx::Point& point_in_screen,
                                        std::vector<gfx::Rect>* drag_bounds);

  // Creates and returns a new Dock to handle the drag.
  Dock* CreateDockForDrag(Tablist* source,
                          const gfx::Point& point_in_screen,
                          gfx::Vector2d* drag_offset,
                          std::vector<gfx::Rect>* drag_bounds);

  // Returns the TablistModel for the specified tablist.
  TablistModel* GetModel(Tablist* tablist) const;
  ...
  // The Tablist the drag originated from.
  Tablist* source_tablist_;

  // The Tablist the dragged Tab is currently attached to, or NULL if the
  ...
  // attached Tablist from the top left of the window.
  gfx::Point window_create_point_;

  // Location of the first tab in the source tablist in screen coordinates.
  ...
  // The selection model of |attached_tablist_| before the tabs were attached.
  ...
  // True if |attached_tablist_| is in a browser specifically created for
  // the drag.
  bool is_dragging_new_dock_;

  // True if |source_tablist_| was maximized before the drag.
  bool was_source_maximized_;

  // True if |source_tablist_| was in immersive fullscreen before the drag.
  ...
  // The Tablist to attach to after the move loop completes.
  Tablist* tablist_to_attach_to_after_exit_;
  ...
  // (in terms of the tablist) and the insertion index at the time tabs are
