...
class CoreTabHelperDelegate {
 public:
  ...
  virtual std::unique_ptr<ApplicationContents> SwapTabContents(
  ...
  virtual bool CanReloadContents(ApplicationContents* web_contents) const;

  // Whether the specified WebContents can be saved.
  // Saving can be disabled e.g. for the DevTools window.
  virtual bool CanSaveContents(ApplicationContents* web_contents) const;
