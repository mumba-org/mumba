...
//#include "core/common/webui_url_constants.h"
...
bool ShouldThemifyFaviconForUrl(const GURL& url) {
  return false;//url.SchemeIs(content::kChromeUIScheme) &&
         ...
         //url.host_piece() != chrome::kChromeUIBrowserHost &&
...
TabIcon::TabIcon() {
  ...
  SetPreferredSize(gfx::Size(kFaviconSize + kAttentionIndicatorRadius,
                             kFaviconSize + kAttentionIndicatorRadius));
}
...
void TabIcon::OnPaint(gfx::Canvas* canvas) {
  ...
  gfx::Rect icon_bounds(
      GetMirroredXWithWidthInView(0, kFaviconSize),
      static_cast<int>(contents_bounds.height() * hiding_fraction_),
      std::min(kFaviconSize, contents_bounds.width()),
      std::min(kFaviconSize, contents_bounds.height()));
...
void TabIcon::PaintAttentionIndicatorAndIcon(gfx::Canvas* canvas,
  ...
  gfx::Point circle_center(
      bounds.x() + (base::i18n::IsRTL() ? 0 : kFaviconSize),
      bounds.y() + kFaviconSize);
