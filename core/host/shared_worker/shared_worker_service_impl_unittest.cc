...
#include "core/host/shared_worker/mock_shared_worker.h"
...
class SharedWorkerServiceImplTest : public RenderViewHostImplTestHarness {
 public:
  common::mojom::SharedWorkerConnectorPtr MakeSharedWorkerConnector(
      RenderProcessHost* process_host,
      int frame_id) {
    common::mojom::SharedWorkerConnectorPtr connector;
  ...
  static void BindSharedWorkerFactory(mojo::ScopedMessagePipeHandle handle) {
    s_factory_request_received_.push(
        common::mojom::SharedWorkerFactoryRequest(std::move(handle)));
  }
 ...
 protected:
  ...
  static std::queue<common::mojom::SharedWorkerFactoryRequest>
...
// static
std::queue<common::mojom::SharedWorkerFactoryRequest>
    SharedWorkerServiceImplTest::s_factory_request_received_;

namespace {

void ConnectToSharedWorker(common::mojom::SharedWorkerConnectorPtr connector,
                           const GURL& url,
  ...
  *local_port = MessagePortChannel(std::move(message_pipe.handle0));
  ...
  common::mojom::SharedWorkerClientPtr client_proxy;
...
TEST_F(SharedWorkerServiceImplTest, BasicTest) {
  ...
  renderer_host->OverrideBinderForTesting(
      common::mojom::SharedWorkerFactory::Name_,
  ...
  common::mojom::SharedWorkerFactoryRequest factory_request;
  EXPECT_TRUE(CheckReceivedFactoryRequest(&factory_request));
  MockSharedWorkerFactory factory(std::move(factory_request));
  base::RunLoop().RunUntilIdle();

  common::mojom::SharedWorkerHostPtr worker_host;
  common::mojom::SharedWorkerRequest worker_request;
...
TEST_F(SharedWorkerServiceImplTest, CreateWorkerTest_NormalCase_URLMismatch) {
  ...
  common::mojom::SharedWorkerFactoryRequest factory_request0;
  EXPECT_TRUE(CheckReceivedFactoryRequest(&factory_request0));
  MockSharedWorkerFactory factory0(std::move(factory_request0));
  base::RunLoop().RunUntilIdle();

  common::mojom::SharedWorkerHostPtr worker_host0;
  common::mojom::SharedWorkerRequest worker_request0;
  ...
  common::mojom::SharedWorkerFactoryRequest factory_request1;
  EXPECT_TRUE(CheckReceivedFactoryRequest(&factory_request1));
  MockSharedWorkerFactory factory1(std::move(factory_request1));
  base::RunLoop().RunUntilIdle();

  common::mojom::SharedWorkerHostPtr worker_host1;
  common::mojom::SharedWorkerRequest worker_request1;
