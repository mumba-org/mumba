...
#include "core/shared/common/shared_worker/shared_worker_client.mojom.h"
...
bool IsShuttingDown(ApplicationProcessHost* host) {
...
void CreateScriptLoaderOnIO(
    ...
    int process_id,
    base::OnceCallback<void(common::mojom::ServiceWorkerProviderInfoForSharedWorkerPtr,
                            network::mojom::URLLoaderFactoryAssociatedPtrInfo)>
        callback) {
  DCHECK_CURRENTLY_ON(HostThread::IO);

  // Set up for service worker.
  auto provider_info = common::mojom::ServiceWorkerProviderInfoForSharedWorker::New();
  base::WeakPtr<ServiceWorkerProviderHost> host =
      context->PreCreateHostForSharedWorker(process_type, process_id, &provider_info);
...
void SharedWorkerServiceImpl::ConnectToWorker(
  ...
  if (process_type == kPROCESS_TYPE_APPLICATION) {
    ApplicationWindowHost* render_frame_host =
      ApplicationWindowHost::FromID(process_id, frame_id);
    ...
    DomainProcessHost* service_host =
      DomainProcessHost::FromID(process_id);
    if (!service_host) {
    // TODO(nhiroki): Support the case where the requester is a worker (i.e.,
    // nested worker) (https://crbug.com/31666).
      client->OnScriptLoadFailed();
    ...
    GURL url(service_host->domain()->name() + "://example.com/");
  ...
  if (host) {
    ...
    if (!IsShuttingDown(ApplicationProcessHost::FromID(host->process_id()))) {
      host->AddClient(std::move(client), process_type, process_id, frame_id, message_port);
      return;
    }
  ...
  CreateWorker(std::move(instance), std::move(client), process_type, process_id, frame_id,
               message_port);
}
...
void SharedWorkerServiceImpl::DestroyHost(SharedWorkerHost* host) {
  ...
  //     ApplicationProcessHost::FromID(host->process_id());
  ...
  //       ApplicationProcessHost::KeepAliveClientType::kSharedWorker);
}
...
void SharedWorkerServiceImpl::CreateWorker(
  ...
  auto host =
      std::make_unique<SharedWorkerHost>(this, std::move(instance), process_type, process_id);
  ...
  if (common::ServiceWorkerUtils::IsServicificationEnabled()) {
    HostThread::PostTask(
        ...
        base::BindOnce(
            ...
            service_worker_context_->domain()->url_loader_factory_getter(),
            service_worker_context_, process_type, process_id,
            base::BindOnce(&SharedWorkerServiceImpl::StartWorker,
                           weak_factory_.GetWeakPtr(), std::move(instance),
                           weak_host, std::move(client), process_type, process_id, frame_id,
                           message_port)));
    return;
  }

  StartWorker(std::move(instance), weak_host, std::move(client), process_type, process_id,
...
void SharedWorkerServiceImpl::StartWorker(
  ...
  if (process_type == kPROCESS_TYPE_APPLICATION)  {
    ApplicationProcessHost* process_host = ApplicationProcessHost::FromID(process_id);
    ...
    common::BindInterface(process_host, &factory);
  } else {
    DomainProcessHost* process_host = DomainProcessHost::FromID(process_id);
    // If the target process is shutting down, then just drop this request and
    // tell the host to destruct. This also means clients that were still waiting
    // for the shared worker to start will fail.
    if (!process_host) {
