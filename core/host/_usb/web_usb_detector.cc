...
// The WebUSB notification should be displayed for all workspaces.
...
GURL GetActiveTabURL() {
  Browser* browser = chrome::FindLastActiveWithWorkspace(
      WorkspaceManager::GetLastUsedWorkspaceAllowedByPolicy());
  if (!browser)
    return GURL();
  ...
  return application_contents->GetURL();
}
...
class WebUsbNotificationDelegate : public TabStripModelObserver,
 ...
 public:
  ...
  void Click(const base::Optional<int>& button_index,
    ...
    ApplicationContents* tab_to_activate = nullptr;
    ...
    if (tab_to_activate) {
      ...
      tab_strip_model->ActivateTabAt(
          tab_strip_model->GetIndexOfApplicationContents(tab_to_activate), false);
