...
#else
#include "core/host/posix_file_descriptor_info.h"
#endif
...
class ChildProcessLauncherHelper :
 ...
 public:
...
#if BUILDFLAG(USE_ZYGOTE_HANDLE)
    common::ZygoteHandle zygote = nullptr;
  ...
  void StartLaunchOnClientThread(bool named_pipe);
  ...
  // Platform specific.
  mojo::ScopedPlatformHandle PrepareMojoPipeHandlesOnClientThread();
  mojo::ScopedPlatformHandle CreateServerPlatformHandle(bool named_pipe);
 ...
 private:
  ...
  void LaunchOnLauncherThread(bool named_pipe);

  const mojo::PlatformHandle& mojo_client_handle() const {
  ...
  const HostThread::ID client_thread_id_;
  base::TimeTicks begin_launch_time_;
  std::unique_ptr<base::CommandLine> command_line_;
  std::unique_ptr<common::SandboxedProcessLauncherDelegate> delegate_;
  base::WeakPtr<ChildProcessLauncher> child_process_launcher_;
  mojo::ScopedPlatformHandle mojo_client_handle_;
  mojo::ScopedPlatformHandle mojo_server_handle_;
