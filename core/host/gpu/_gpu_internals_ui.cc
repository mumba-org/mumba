...
#include "core/host/gpu/gpu_internals_ui.h"
...
#include "core/host/web_ui_message_handler.h"
...
class GpuMessageHandler
 ...
 public:
  ...
  // Messages
  void OnHostBridgeInitialized(const base::ListValue* list);
...
/* HostBridge.callAsync prepends a requestID to these messages. */
void GpuMessageHandler::RegisterMessages() {
  DCHECK_CURRENTLY_ON(HostThread::UI);

  web_ui()->RegisterMessageCallback(
      "hostBridgeInitialized",
      base::BindRepeating(&GpuMessageHandler::OnHostBridgeInitialized,
...
void GpuMessageHandler::OnCallAsync(const base::ListValue* args) {
  ...
  // call HostBridge.onCallAsyncReply with result
  if (ret) {
    web_ui()->CallJavascriptFunctionUnsafe("hostBridge.onCallAsyncReply",
                                           *requestId, *ret);
  } else {
    web_ui()->CallJavascriptFunctionUnsafe("hostBridge.onCallAsyncReply",
                                           *requestId);
...
void GpuMessageHandler::OnHostBridgeInitialized(
...
void GpuMessageHandler::OnGpuInfoUpdate() {
  ...
  web_ui()->CallJavascriptFunctionUnsafe("hostBridge.onGpuInfoUpdate",
                                         *(gpu_info_val.get()));
}
...
GpuInternalsUI::GpuInternalsUI(WebUI* web_ui)
  ...
  HostContext* host_context =
      web_ui->GetWebContents()->GetHostContext();
  WebUIDataSource::Add(host_context, CreateGpuHTMLSource());
}
