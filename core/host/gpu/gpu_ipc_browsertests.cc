...
#include "core/common/gpu_stream_constants.h"
...
#include "content/test/gpu_hosttest_helpers.h"
...
class ContextTestBase : public content::ContentHostTest {
 public:
  void SetUpOnMainThread() override {
    ...
    scoped_refptr<gpu::GpuChannelHost> gpu_channel_host =
        content::GpuHosttestEstablishGpuChannelSyncRunLoop();
    CHECK(gpu_channel_host);

    provider_ =
        content::GpuHosttestCreateContext(std::move(gpu_channel_host));
    auto result = provider_->BindToCurrentThread();
    CHECK_EQ(result, gpu::ContextResult::kSuccess);
    gl_ = provider_->ContextGL();
    context_support_ = provider_->ContextSupport();

    ContentHostTest::SetUpOnMainThread();
  }

  void TearDownOnMainThread() override {
    // Must delete the context first.
    provider_ = nullptr;
    ContentHostTest::TearDownOnMainThread();
  }
...
class HostGpuChannelHostFactoryTest : public ContentHostTest {
 public:
  void SetUpOnMainThread() override {
 ...
 protected:
  gpu::GpuChannelEstablishFactory* GetFactory() {
    return HostMainLoop::GetInstance()->gpu_channel_establish_factory();
  }
  ...
  void EstablishAndWait() {
    gpu_channel_host_ = content::GpuHosttestEstablishGpuChannelSyncRunLoop();
  }
...
#endif
IN_PROC_BROWSER_TEST_F(HostGpuChannelHostFactoryTest, MAYBE_Basic) {
...
#endif
IN_PROC_BROWSER_TEST_F(HostGpuChannelHostFactoryTest,
  ...
  GetFactory()->EstablishGpuChannel(
      base::BindOnce(&HostGpuChannelHostFactoryTest::Signal,
...
IN_PROC_BROWSER_TEST_F(HostGpuChannelHostFactoryTest,
  ...
  base::RunLoop run_loop;
  GetFactory()->EstablishGpuChannel(
      base::BindOnce(&HostGpuChannelHostFactoryTest::SignalAndQuitLoop,
...
IN_PROC_BROWSER_TEST_F(HostGpuChannelHostFactoryTest,
  ...
  scoped_refptr<ui::ContextProviderCommandBuffer> provider =
      content::GpuHosttestCreateContext(GetGpuChannel());
...
using GpuProcessHostHostTest = HostGpuChannelHostFactoryTest;

IN_PROC_BROWSER_TEST_F(GpuProcessHostHostTest, Shutdown) {
  DCHECK(!IsChannelEstablished());
  EstablishAndWait();
  base::RunLoop run_loop;
...
#if defined(OS_LINUX) && !defined(OS_CHROMEOS)
IN_PROC_BROWSER_TEST_F(HostGpuChannelHostFactoryTest, CreateTransferBuffer) {
  DCHECK(!IsChannelEstablished());
  EstablishAndWait();
