...
#include "core/host/field_trial_recorder.h"
...
class GpuSandboxedProcessLauncherDelegate
 ...
 public:
  ...
  enum GPUAppDomainEnableState{
      AC_ENABLED = 0, AC_DISABLED_GL = 1, AC_DISABLED_FORCE = 2,
      MAX_ENABLE_STATE = 3,
  };

  bool GetAppContainerId(std::string* appdomain_id) override {
    if (UseOpenGLRenderer()) {
      base::UmaHistogramEnumeration("GPU.AppDomain.EnableState",
                                    AC_DISABLED_GL, MAX_ENABLE_STATE);
      return false;
    }

    if (!enable_appdomain_) {
      base::UmaHistogramEnumeration("GPU.AppDomain.EnableState",
                                    AC_DISABLED_FORCE, MAX_ENABLE_STATE);
      return false;
    }

    *appdomain_id = base::WideToUTF8(cmd_line_.GetProgram().value());
    base::UmaHistogramEnumeration("GPU.AppDomain.EnableState", AC_ENABLED,
                                  MAX_ENABLE_STATE);
    return true;
  }
  ...
  bool PreSpawnTarget(sandbox::TargetPolicy* policy) override {
    if (UseOpenGLRenderer()) {
      ...
      // turn blocks on the host UI thread. So, instead we forgo a window
    ...
    // Block this DLL even if it is not loaded by the host process.
  ...
  // TODO: Remove this once AppDomain sandbox is enabled by default.
  void DisableAppDomain() { enable_appdomain_ = false; }
#endif  // OS_WIN
...
#if defined(OS_WIN)
void RecordAppDomainStatus(int error_code, bool crashed_before) {
  base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
  if (!crashed_before &&
      service_manager::SandboxWin::IsAppContainerEnabledForSandbox(
          *command_line, service_manager::SANDBOX_TYPE_GPU)) {
    base::UmaHistogramSparse("GPU.AppDomain.Status", error_code);
  }
}
#endif  // defined(OS_WIN)
...
void BindDiscardableMemoryRequestOnUI(
  ...
  HostThread::PostTask(
      ...
      base::BindOnce(
          ...
          HostMainLoop::GetInstance()->discardable_shared_memory_manager()));
}

}  // anonymous namespace

class GpuProcessHost::ConnectionFilterImpl : public common::ConnectionFilter {
 public:
  ConnectionFilterImpl() {
    auto task_runner = HostThread::GetTaskRunnerForThread(HostThread::UI);
 ...
 private:
  ...
  void OnBindInterface(const service_manager::BindSourceInfo& source_info,
    ...
    if (!registry_.TryBindInterface(interface_name, interface_pipe)) {
      // FROM void ChromeContentBrowserClient::BindInterfaceRequest 
      ...
      //}
      //common::GetClient()->host()->BindInterfaceRequest(
      //   source_info, interface_name, interface_pipe);
       
       //NOTREACHED();
      // DLOG(ERROR) << "GpuProcessHost: trying to bind service interface '" << interface_name << "' on " <<
      //                "non-binded service.\n" <<
...
GpuProcessHost* GpuProcessHost::Get(GpuProcessKind kind, bool force_create) {
  ...
  // Do not create a new process if host is shutting down.
  if (HostMainRunner::ExitedMainMessageLoop()) {
    DLOG(ERROR) << "HostMainRunner::ExitedMainMessageLoop()";
    return nullptr;
  }
...
GpuProcessHost::GpuProcessHost(int host_id, GpuProcessKind kind)
  ...
  process_.reset(new HostChildProcessHostImpl(
      common::PROCESS_TYPE_GPU, this, common::mojom::kGpuServiceName));
}
...
void GpuProcessHost::InitOzone() {
  ...
  if (host_command_line->HasSwitch(switches::kEnableDrmMojo)) {
    ...
    auto io_callback = base::BindOnce(
        [](int host_id,
          ...
          platform->GetGpuPlatformSupportHost()->OnGpuProcessLaunched(
              host_id, HostThread::GetTaskRunnerForThread(HostThread::UI),
              HostThread::GetTaskRunnerForThread(HostThread::IO),
              send_callback);
        },
...
bool GpuProcessHost::Init() {
  ...
  if (common::ServiceManagerConnection::GetForProcess()) {
    common::ServiceManagerConnection::GetForProcess()->AddConnectionFilter(
  ...
  if (in_process_) {
    ...
    in_process_gpu_thread_.reset(GetGpuMainThreadFactory()(
        common::InProcessChildThreadParams(
...
void GpuProcessHost::OnProcessLaunched() {
  ...
  if (kind_ == GPU_PROCESS_KIND_SANDBOXED)
    RecordAppDomainStatus(sandbox::SBOX_ALL_OK, crashed_before_);
#endif  // defined(OS_WIN)
}
...
#if defined(OS_WIN)
  if (kind_ == GPU_PROCESS_KIND_SANDBOXED)
    RecordAppDomainStatus(error_code, crashed_before_);
#endif  // defined(OS_WIN)
  RecordProcessCrash();
}
...
void GpuProcessHost::DidInitialize(
  ...
  // Set GPU driver bug workaround flags that are checked on the host side.
  ...
  // listeners.
  //DLOG(INFO) << "GpuProcessHost::DidInitialize: NOTE: forcing GPU acceleration";
  //gpu::GpuFeatureInfo gpu_feature_info_copy = gpu_feature_info;

  //gpu_feature_info_copy.status_values[gpu::GPU_FEATURE_TYPE_GPU_RASTERIZATION] = gpu::kGpuFeatureStatusEnabled;
  //gpu_feature_info_copy.status_values[gpu::GPU_FEATURE_TYPE_ACCELERATED_WEBGL] = gpu::kGpuFeatureStatusEnabled;
  //gpu_feature_info_copy.status_values[gpu::GPU_FEATURE_TYPE_ACCELERATED_WEBGL2] = gpu::kGpuFeatureStatusEnabled;
  //gpu_feature_info_copy.status_values[gpu::GPU_FEATURE_TYPE_ACCELERATED_2D_CANVAS] = gpu::kGpuFeatureStatusEnabled;
  //gpu_feature_info_copy.status_values[gpu::GPU_FEATURE_TYPE_FLASH3D] = gpu::kGpuFeatureStatusEnabled;
  //gpu_feature_info_copy.status_values[gpu::GPU_FEATURE_TYPE_FLASH_STAGE3D] = gpu::kGpuFeatureStatusEnabled;
  //gpu_feature_info_copy.status_values[gpu::GPU_FEATURE_TYPE_FLASH_STAGE3D_BASELINE] = gpu::kGpuFeatureStatusEnabled;
  //gpu_feature_info_copy.status_values[gpu::GPU_FEATURE_TYPE_ACCELERATED_VIDEO_DECODE] = gpu::kGpuFeatureStatusEnabled;
  //gpu_feature_info_copy.status_values[gpu::GPU_FEATURE_TYPE_GPU_COMPOSITING] = gpu::kGpuFeatureStatusEnabled;

  //gpu::GpuFeatureInfo gpu_feature_info_for_hardware_gpu_copy = gpu_feature_info_copy;

  //gpu_data_manager->UpdateGpuFeatureInfo(gpu_feature_info_copy,
  //                                       gpu_feature_info_for_hardware_gpu_copy);
...
bool GpuProcessHost::LaunchGpuProcess() {
  ...
  base::CommandLine::StringType gpu_launcher =
      host_command_line.GetSwitchValueNative(switches::kGpuLauncher);
...
#else
#if defined(OS_LINUX)
  int child_flags = gpu_launcher.empty() ? common::ChildProcessHost::CHILD_ALLOW_SELF :
                                           common::ChildProcessHost::CHILD_NORMAL;
#else
  ...
  cmd_line->AppendSwitch(switches::kGpuProcess);
...
//#endif  // defined(OS_WIN)
  ...
  // If you want a host command-line switch passed to the GPU process
  ...
  cmd_line->CopySwitchesFrom(
      host_command_line, switches::kGLSwitchesCopiedFromGpuProcessHost,
      switches::kGLSwitchesCopiedFromGpuProcessHostNumSwitches);

  std::vector<const char*> gpu_workarounds;
  gpu::GpuDriverBugList::AppendAllWorkarounds(&gpu_workarounds);
  cmd_line->CopySwitchesFrom(host_command_line, gpu_workarounds.data(),
                             gpu_workarounds.size());

  // Mumba: this is forced, see in which cases we dont want this..
  // eg: software rendering fallback, in-process, android, etc..
  cmd_line->AppendSwitch(switches::kEnableOOPRasterization);

  //common::GetClient()->host()->AppendExtraCommandLineSwitches(
...
std::string GpuProcessHost::GetShaderPrefixKey() {
  if (shader_prefix_key_.empty()) {
    ...
    shader_prefix_key_ = common::GetClient()->GetProduct() + "-" +
