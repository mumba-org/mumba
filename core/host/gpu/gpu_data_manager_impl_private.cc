...
#include "core/host/gpu/gpu_data_manager_impl_private.h"
...
void GpuDataManagerImplPrivate::RequestGpuSupportedRuntimeVersion() {
  ...
  HostThread::PostDelayedTask(HostThread::IO, FROM_HERE, std::move(task),
...
void GpuDataManagerImplPrivate::UpdateGpuInfo(
  ...
  common::GetClient()->SetGpuInfo(gpu_info_);
  NotifyGpuInfoUpdate();
}
...
void GpuDataManagerImplPrivate::AppendGpuCommandLine(
  ...
  if (card_disabled_ && SwiftShaderAllowed()) {
    ...
    use_gl = host_command_line->GetSwitchValueASCII(switches::kUseGL);
  }
  if (!use_gl.empty()) {
  ...
  if (host_command_line->HasSwitch(
          switches::kOverrideUseSoftwareGLForTests) ||
      host_command_line->HasSwitch(switches::kHeadless)) {
...
void GpuDataManagerImplPrivate::HandleGpuSwitch() {
  GpuDataManagerImpl::UnlockedSession session(owner_);
  // Notify observers in the host process.
...
bool GpuDataManagerImplPrivate::Are3DAPIsBlocked(const GURL& top_origin_url,
                                                 ...
                                                 int render_frame_id,
                                                 common::ThreeDAPIType requester) {
