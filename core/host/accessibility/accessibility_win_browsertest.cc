...
#include "core/host/accessibility/accessibility_event_recorder.h"
#include "core/host/accessibility/accessibility_tree_formatter.h"
#include "core/host/accessibility/accessibility_tree_formatter_utils_win.h"
...
#include "core/test/content_browser_test_utils_internal.h"
...
IAccessible* AccessibilityWinBrowserTest::GetRendererAccessible() {
  content::ApplicationContents* application_contents = shell()->application_contents();
  return application_contents->GetRenderWidgetHostView()->GetNativeViewAccessible();
}

void AccessibilityWinBrowserTest::ExecuteScript(const std::wstring& script) {
  shell()->application_contents()->GetMainFrame()->ExecuteJavaScriptForTests(script);
}
...
void AccessibilityWinBrowserTest::SetUpInputField(
  ...
  AccessibilityNotificationWaiter waiter(
      shell()->application_contents(), ui::kAXModeComplete,
...
IN_PROC_BROWSER_TEST_F(AccessibilityWinBrowserTest,
  ...
  std::unique_ptr<AccessibilityNotificationWaiter> waiter(
      new AccessibilityNotificationWaiter(shell()->application_contents(),
                                          ui::kAXModeComplete,
...
IN_PROC_BROWSER_TEST_F(AccessibilityWinBrowserTest,
  ...
  waiter.reset(new AccessibilityNotificationWaiter(
      shell()->application_contents(), ui::kAXModeComplete, ax::mojom::Event::kBlur));
...
IN_PROC_BROWSER_TEST_F(AccessibilityWinBrowserTest, HasHWNDAfterNavigation) {
  ...
  WebContentsView* application_contents_view = application_contents->GetView();
  WebContentsViewAura* application_contents_view_aura =
      static_cast<WebContentsViewAura*>(application_contents_view);

  // Set a flag that will cause WebContentsViewAura to initialize a
  // RenderWidgetHostViewAura with a null parent view.
  application_contents_view_aura->set_init_rwhv_with_null_parent_for_testing(true);
  ...
  application_contents_view->GetNativeView()->AddChild(
      application_contents->GetRenderWidgetHostView()->GetNativeView());
...
IN_PROC_BROWSER_TEST_F(AccessibilityWinBrowserTest,
  ...
  auto* main_frame = shell()->application_contents()->GetMainFrame();
  main_frame->ExecuteJavaScriptWithUserGestureForTests(L"");

  // Trigger a reload here, which will get cancelled.
  shell()->application_contents()->GetController().Reload(ReloadType::NORMAL, false);
