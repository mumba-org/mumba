...
#include "core/host/site_per_process_browsertest.h"
...
class MAYBE_SitePerProcessAccessibilityBrowserTest
 ...
 public:
  ...
  void LoadCrossSitePageIntoFrame(FrameTreeNode* frame_tree_node,
    ...
    EXPECT_NE(shell()->application_contents()->GetSiteInstance(), site_instance);
...
IN_PROC_BROWSER_TEST_F(MAYBE_SitePerProcessAccessibilityBrowserTest,
  ...
  // Enable full accessibility for all current and future ApplicationContents.
  ...
  RenderFrameHostImpl* main_frame = static_cast<RenderFrameHostImpl*>(
      shell()->application_contents()->GetMainFrame());
...
IN_PROC_BROWSER_TEST_F(MAYBE_SitePerProcessAccessibilityBrowserTest,
  ...
  NavigateToURL(shell(), main_url);
  WaitForAccessibilityTreeToContainNodeWithName(shell()->application_contents(),
                                                "This page has no title.");

  // Open a new tab for b.com and wait for it to load.  This will create a
  // proxy in b.com for the original (opener) tab.
  GURL b_url(embedded_test_server()->GetURL("b.com", "/title2.html"));
  Domain* new_shell = OpenPopup(shell()->application_contents(), b_url, "popup");
  WaitForAccessibilityTreeToContainNodeWithName(new_shell->application_contents(),
                                                "Title Of Awesomeness");
