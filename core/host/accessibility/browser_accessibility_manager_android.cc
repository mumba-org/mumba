...
BrowserAccessibilityManagerAndroid::BrowserAccessibilityManagerAndroid(
    const ui::AXTreeUpdate& initial_tree,
    WebContentsAccessibilityAndroid* application_contents_accessibility,
    BrowserAccessibilityDelegate* delegate,
    BrowserAccessibilityFactory* factory)
    : BrowserAccessibilityManager(delegate, factory),
      application_contents_accessibility_(application_contents_accessibility),
      prune_tree_for_screen_reader_(true) {
  if (application_contents_accessibility)
    application_contents_accessibility->set_root_manager(this);
  Initialize(initial_tree);
}

BrowserAccessibilityManagerAndroid::~BrowserAccessibilityManagerAndroid() {
  if (application_contents_accessibility_)
    application_contents_accessibility_->set_root_manager(nullptr);
}
...
BrowserAccessibilityManagerAndroid::GetWebContentsAXFromRootManager() {
  ...
  if (!parent_node)
    return application_contents_accessibility_;
