...
void BrowserAccessibilityStateImpl::ResetAccessibilityMode() {
  ...
  std::vector<ApplicationContents*> application_contents_vector =
      ApplicationContents::GetAllWebContents();
  for (size_t i = 0; i < application_contents_vector.size(); ++i)
    application_contents_vector[i]->SetAccessibilityMode(accessibility_mode());
}
...
void BrowserAccessibilityStateImpl::AddAccessibilityModeFlags(ui::AXMode mode) {
  ...
  std::vector<ApplicationContents*> application_contents_vector =
      ApplicationContents::GetAllWebContents();
  for (size_t i = 0; i < application_contents_vector.size(); ++i)
    application_contents_vector[i]->AddAccessibilityMode(accessibility_mode_);
}
