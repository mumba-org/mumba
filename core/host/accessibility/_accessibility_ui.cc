...
#include "core/host/accessibility/accessibility_ui.h"
...
#include "core/host/accessibility/accessibility_tree_formatter_blink.h"
...
#include "core/host/webui/web_ui_data_source_impl.h"
#include "core/common/view_message_enums.h"
#include "core/grit/content_resources.h"
#include "core/host/host_thread.h"
#include "core/host/favicon_status.h"
...
std::unique_ptr<base::DictionaryValue> BuildTargetDescriptor(
  ...
  ApplicationContents* application_contents = static_cast<ApplicationContents*>(
      ApplicationContents::FromRenderViewHost(rvh));
  ui::AXMode accessibility_mode;
  ...
  if (application_contents) {
    ...
    url = application_contents->GetURL();
    title = base::UTF16ToUTF8(application_contents->GetTitle());
    NavigationController& controller = application_contents->GetController();
    NavigationEntry* entry = controller.GetVisibleEntry();
    if (entry != nullptr && entry->GetURL().is_valid())
      favicon_url = entry->GetFavicon().url;
    accessibility_mode = application_contents->GetAccessibilityMode();
  }
...
void AccessibilityUIMessageHandler::RegisterMessages() {
  DCHECK_CURRENTLY_ON(content::HostThread::UI);
...
void AccessibilityUIMessageHandler::ToggleAccessibility(
  ...
  auto* application_contents =
      static_cast<ApplicationContents*>(ApplicationContents::FromRenderViewHost(rvh));
  ui::AXMode current_mode = application_contents->GetAccessibilityMode();
  ...
  application_contents->SetAccessibilityMode(current_mode);
}
...
void AccessibilityUIMessageHandler::RequestWebContentsTree(
  ...
  auto* application_contents =
      static_cast<ApplicationContents*>(ApplicationContents::FromRenderViewHost(rvh));
  // No matter the state of the current application_contents, we want to force the mode
  // because we are about to show the accessibility tree
  application_contents->SetAccessibilityMode(
  ...
  auto* ax_mgr = application_contents->GetOrCreateRootBrowserAccessibilityManager();
  DCHECK(ax_mgr);
...
void AccessibilityUIMessageHandler::RequestNativeUITree(
  ...
  ApplicationContents* application_contents =
      static_cast<ApplicationContents*>(web_ui()->GetWebContents());
  gfx::NativeWindow native_window =
      application_contents->GetView()->GetTopLevelNativeWindow();
