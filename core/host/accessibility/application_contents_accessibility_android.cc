...
#include "core/host/accessibility/application_contents_accessibility_android.h"
...
#include "core/host/accessibility/browser_accessibility_android.h"
#include "core/host/accessibility/browser_accessibility_manager_android.h"
...
#include "core/host/android/application_window_host_connector.h"
...
class WebContentsAccessibilityAndroid::Connector
 ...
 public:
  Connector(ApplicationContents* application_contents,
...
WebContentsAccessibilityAndroid::Connector::Connector(
    ...
    WebContentsAccessibilityAndroid* accessibility)
    : RenderWidgetHostConnector(application_contents), accessibility_(accessibility) {
  Initialize();
}
...
WebContentsAccessibilityAndroid::Connector::~Connector() {
  ...
  auto* manager = static_cast<BrowserAccessibilityManagerAndroid*>(
      accessibility_->application_contents_->GetRootBrowserAccessibilityManager());
  if (manager)
    manager->set_application_contents_accessibility(nullptr);
}
...
void WebContentsAccessibilityAndroid::Connector::UpdateRenderProcessConnection(
  ...
  if (old_rwhva)
    old_rwhva->set_application_contents_accessibility(nullptr);
  if (new_rwhva)
    new_rwhva->set_application_contents_accessibility(accessibility_.get());
}
...
WebContentsAccessibilityAndroid::WebContentsAccessibilityAndroid(
    ...
    ApplicationContents* application_contents)
    : java_ref_(env, obj),
      application_contents_(static_cast<ApplicationContents*>(application_contents)),
      frame_info_initialized_(false),
      root_manager_(nullptr),
      connector_(new Connector(application_contents, this)) {
  CollectStats();
}
...
void WebContentsAccessibilityAndroid::Enable(JNIEnv* env,
  ...
  auto* manager = static_cast<BrowserAccessibilityManagerAndroid*>(
      application_contents_->GetRootBrowserAccessibilityManager());
...
jlong JNI_WebContentsAccessibilityImpl_Init(
  ...
  ApplicationContents* application_contents = ApplicationContents::FromJavaWebContents(japplication_contents);
  DCHECK(application_contents);

  return reinterpret_cast<intptr_t>(
      new WebContentsAccessibilityAndroid(env, obj, application_contents));
}
