...
#include "core/host/accessibility/dump_accessibility_browsertest_base.h"
...
#include "core/common/content_paths.h"
...
DumpAccessibilityTestBase::DumpUnfilteredAccessibilityTreeAsString() {
  ...
  formatter->FormatAccessibilityTree(
      application_contents->GetRootBrowserAccessibilityManager()->GetRoot(),
      &ax_tree_dump);
  return ax_tree_dump;
}
...
void DumpAccessibilityTestBase::RunTestForPlatform(
  ...
  if (enable_accessibility_after_navigating_ &&
      application_contents->GetAccessibilityMode().is_mode_off()) {
    // Load the url, then enable accessibility.
    NavigateToURL(shell(), url);
    AccessibilityNotificationWaiter accessibility_waiter(
        application_contents, ui::kAXModeComplete, ax::mojom::Event::kNone);
    accessibility_waiter.WaitForNotification();
  } else {
    // Enable accessibility, then load the test html and wait for the
    // "load complete" AX event.
    AccessibilityNotificationWaiter accessibility_waiter(
        application_contents, ui::kAXModeComplete, ax::mojom::Event::kLoadComplete);
    NavigateToURL(shell(), url);
    accessibility_waiter.WaitForNotification();
  }
  ...
  for (;;) {
    ...
    RenderFrameHostImpl* main_frame = static_cast<RenderFrameHostImpl*>(
        application_contents->GetMainFrame());
