...
#include "core/host/sandbox_parameters_mac.h"
...
#include "core/host/plugin_service.h"
#include "core/common/content_client.h"
#include "core/common/content_switches.h"
#include "core/common/pepper_plugin_info.h"
...
void SetupCommonSandboxParameters(sandbox::SeatbeltExecClient* client) {
  ...
  NSBundle* disk = base::mac::OuterBundle();
  std::string bundle_id = base::SysNSStringToUTF8([disk bundleIdentifier]);
  CHECK(client->SetParameter(
      service_manager::SandboxMac::kSandboxChromeBundleId, bundle_id));

  CHECK(client->SetParameter(service_manager::SandboxMac::kSandboxHostPID,
                             std::to_string(getpid())));

  std::string logging_path =
      GetContentClient()->host()->GetLoggingFileName(*command_line).value();
  CHECK(client->SetParameter(
...
void SetupPPAPISandboxParameters(sandbox::SeatbeltExecClient* client) {
  ...
  for (const auto& plugin : plugins) {
    // Only add plugins which are external to Chrome's disk to the profile.
