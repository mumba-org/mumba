...
#include "mumba/install_static/install_details.h"
...
HICON GetAppIcon() {
  ...
  return LoadIcon(GetModuleHandle(constants::kBrowserResourcesDll),
...
HICON GetSmallAppIcon() {
  ...
  return static_cast<HICON>(LoadImage(
      GetModuleHandle(constants::kBrowserResourcesDll), MAKEINTRESOURCE(icon_id),
...
std::unique_ptr<gfx::ImageFamily> GetAppIconImageFamily() {
  ...
  HMODULE module = GetModuleHandle(constants::kBrowserResourcesDll);
  if (!module)
