...
class ExtendableMessageEventTestHelper : public EmbeddedWorkerTestHelper {
 public:
  ...
  const std::vector<common::mojom::ExtendableMessageEventPtr>& events() {
    return events_;
  }

 private:
  std::vector<common::mojom::ExtendableMessageEventPtr> events_;
};
...
class ServiceWorkerHandleTest : public testing::Test {
 public:
  ...
  GetRegistrationFromRemote(common::mojom::ServiceWorkerContainerHost* container_host,
                            const GURL& scope) {
...
TEST_F(ServiceWorkerHandleTest, DispatchExtendableMessageEvent_FromClient) {
  ...
  // ApplicationContents/RenderFrameHost must be created too because it's needed for
  // DispatchExtendableMessageEvent to populate ExtendableMessageEvent#source.
  RenderViewHostTestEnabler rvh_test_enabler;
  std::unique_ptr<ApplicationContents> web_contents(
      ApplicationContentsTester::CreateTestApplicationContents(helper_->browser_context(),
                                               nullptr));
