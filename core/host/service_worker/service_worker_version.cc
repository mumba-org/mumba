...
#include "core/host/service_worker/embedded_worker_registry.h"
...
class WhateverTrialPolicy : public blink::TrialPolicy {
 public:
  WhateverTrialPolicy() {}
  ~WhateverTrialPolicy() override {}
...
void RunCallbackAfterStartWorker(base::WeakPtr<ServiceWorkerVersion> version,
  ...
  if (status == common::SERVICE_WORKER_OK &&
    ...
    std::move(callback).Run(common::SERVICE_WORKER_ERROR_START_WORKER_FAILED);
    return;
  }
...
void OnEventDispatcherConnectionError(
  ...
  // DLOG(INFO) << "OnEventDispatcherConnectionError";
  if (!embedded_worker)
    return;
...
void OnOpenWindowFinished(
  ...
  DCHECK_CURRENTLY_ON(HostThread::IO);
  const bool success = (status == common::SERVICE_WORKER_OK);
...
void DidShowPaymentHandlerWindow(
  ...
  if (success) {
    service_worker_client_utils::DidNavigate(
        context, GetOrigin(url),
        base::BindOnce(&OnOpenWindowFinished, std::move(callback)),
        render_process_id, render_frame_id);
  } else {
    OnOpenWindowFinished(std::move(callback), common::SERVICE_WORKER_ERROR_FAILED,
                         nullptr /* client_info */);
  }
}
...
ServiceWorkerVersion::ServiceWorkerVersion(
    ...
    : version_id_(version_id),
      ...
      script_origin_(CreateUrlOrigin(script_url_)),
      ...
      validator_(std::make_unique<blink::TrialTokenValidator>(std::make_unique<WhateverTrialPolicy>())),
      weak_factory_(this) {
...
ServiceWorkerVersion::~ServiceWorkerVersion() {
  ...
  if (!start_callbacks_.empty()) {
    ...
    std::move(record_start_worker_result).Run(common::SERVICE_WORKER_ERROR_ABORT);
  }
  ...
  for (const auto& controllee : controllee_map_) {
    ...
    info.clients.insert(std::make_pair(
        ...
        ServiceWorkerVersionInfo::ClientInfo(
            host->process_id(), host->route_id(), host->application_contents_getter(),
            host->provider_type())));
  }
...
void ServiceWorkerVersion::StartWorker(ServiceWorkerMetrics::EventType purpose,
  ...
  // DLOG(INFO) << "ServiceWorkerVersion::StartWorker";
  ...
  DCHECK_CURRENTLY_ON(HostThread::IO);
  const bool is_browser_startup_complete = true;
  ...
  if (!context_) {
    ...
    RunSoon(base::BindOnce(std::move(callback), common::SERVICE_WORKER_ERROR_ABORT));
    return;
  }
  if (is_redundant()) {
    RecordStartWorkerResult(purpose, status_, kInvalidTraceId,
                            is_browser_startup_complete,
                            common::SERVICE_WORKER_ERROR_REDUNDANT);
    RunSoon(
        base::BindOnce(std::move(callback), common::SERVICE_WORKER_ERROR_REDUNDANT));
    return;
  }
  if (!IsStartWorkerAllowed()) {
    RecordStartWorkerResult(purpose, status_, kInvalidTraceId,
                            is_browser_startup_complete,
                            common::SERVICE_WORKER_ERROR_DISALLOWED);
    RunSoon(
        base::BindOnce(std::move(callback), common::SERVICE_WORKER_ERROR_DISALLOWED));
    return;
  }
  ...
  context_->storage()->FindRegistrationForId(
      registration_id_, scope_.GetOrigin(), process_type_, process_id_,
      base::BindOnce(
...
void ServiceWorkerVersion::RunAfterStartWorker(
    ...
    StatusCallback callback) {
  // DLOG(INFO) << "host::ServiceWorkerVersion::RunAfterStartWorker";    
  if (running_status() == EmbeddedWorkerStatus::RUNNING) {
    // DLOG(INFO) << "host::ServiceWorkerVersion::RunAfterStartWorker: already running. just running callback";
    DCHECK(start_callbacks_.empty());
    std::move(callback).Run(common::SERVICE_WORKER_OK);
    return;
  }
  // DLOG(INFO) << "host::ServiceWorkerVersion::RunAfterStartWorker: not running. starting first";
  StartWorker(purpose,
...
void ServiceWorkerVersion::ReportError(common::ServiceWorkerStatusCode status,
...
void ServiceWorkerVersion::SetValidOriginTrialTokens(
  ...
  origin_trial_tokens_ = validator_->GetValidTokens(
      CreateOrigin(scope()), tokens, clock_->Now());
}
...
void ServiceWorkerVersion::SetMainScriptHttpResponseInfo(
  ...
  if (!origin_trial_tokens_) {
    origin_trial_tokens_ = validator_->GetValidTokensFromHeaders(
        CreateOrigin(scope()), http_info.headers.get(), clock_->Now());
  }
...
void ServiceWorkerVersion::OnStarted() {
  // DLOG(INFO) << "ServiceWorkerVersion::OnStarted";
  DCHECK_EQ(EmbeddedWorkerStatus::RUNNING, running_status());
  
  // in case of native service worker, the disk caches is not initialized yet
  // if the first time.. so we just force it here
  if (script_type_ == blink::mojom::ScriptType::kNative) {
    context_->storage()->EnsureDiskCachedInitialized();
    // TODO: see if theres not problem signaling a resource that will never be filled
    //       with anything
    //       in theory this will be reused to retrieve the script back from cache
    //       but given this is native.. its never really needed
    //       the shenanigans here are just to get the cache into an acceptable
    //       state for further steps
    int64_t cache_resource_id = context_->storage()->NewResourceId();
    script_cache_map_.NotifyStartedCaching(script_url_, cache_resource_id);
    // FIXME: maybe low the spectations of waiting for resource data
    std::unique_ptr<ServiceWorkerResponseWriter> writer = context_->storage()->CreateResponseWriter(cache_resource_id);
    scoped_refptr<net::StringIOBuffer> buffer = new net::StringIOBuffer("__native__");
    writer->WriteData(buffer.get(), buffer->size(), base::BindOnce(&IgnoreMe));
    script_cache_map_.NotifyFinishedCaching(
      script_url_, buffer->size(), net::OK, std::string());
  }
  ...
  FinishStartWorker(common::SERVICE_WORKER_OK);
  for (auto& observer : listeners_)
    observer.OnRunningStateChanged(this);
}
...
void ServiceWorkerVersion::OnStopped(EmbeddedWorkerStatus old_status) {
  ...
  // DLOG(INFO) << "ServiceWorkerVersion::OnStopped => OnStoppedInternal()";
...
void ServiceWorkerVersion::OnDetached(EmbeddedWorkerStatus old_status) {
  ...
  // DLOG(INFO) << "ServiceWorkerVersion::OnDetached => OnStoppedInternal()";
  OnStoppedInternal(old_status);
}
...
void ServiceWorkerVersion::OnStartSentAndScriptEvaluated(
    common::ServiceWorkerStatusCode status) {
  // DLOG(INFO) << "ServiceWorkerVersion::OnStartSentAndScriptEvaluated";
  if (status != common::SERVICE_WORKER_OK) {
    scoped_refptr<ServiceWorkerVersion> protect(this);
...
void ServiceWorkerVersion::GetClient(const std::string& client_uuid,
  ...
  if (!provider_host ||
      GetOrigin(provider_host->document_url()) != GetOrigin(script_url_)) {
    // The promise will be resolved to 'undefined'.
...
void ServiceWorkerVersion::PostMessageToClient(
  ...
  // DLOG(INFO) << "host::ServiceWorkerVersion::PostMessageToClient";    
  if (!context_)
    return;
  ServiceWorkerProviderHost* provider_host =
      context_->GetProviderHostByClientID(client_uuid);
  ...
  if (GetOrigin(provider_host->document_url()) != GetOrigin(script_url_)) {
...
void ServiceWorkerVersion::OnSimpleEventFinished(
  ...
  std::move(callback).Run(mojo::ConvertTo<common::ServiceWorkerStatusCode>(status));
}
...
void ServiceWorkerVersion::DidEnsureLiveRegistrationForStartWorker(
  ...
  // DLOG(INFO) << "ServiceWorkerVersion::DidEnsureLiveRegistrationForStartWorker: status = " << status;   
  scoped_refptr<ServiceWorkerRegistration> protect = registration;
  if (status == common::SERVICE_WORKER_ERROR_NOT_FOUND) {
    // DLOG(INFO) << "ServiceWorkerVersion::DidEnsureLiveRegistrationForStartWorker: common::SERVICE_WORKER_ERROR_NOT_FOUND";
    ...
    if (protect) {
      ...
      status = common::SERVICE_WORKER_OK;
    }
  }
  if (status != common::SERVICE_WORKER_OK) {
    // DLOG(INFO) << "ServiceWorkerVersion::DidEnsureLiveRegistrationForStartWorker: error sending SERVICE_WORKER_ERROR_START_WORKER_FAILED";   
    ...
    RunSoon(base::BindOnce(std::move(callback),
                           common::SERVICE_WORKER_ERROR_START_WORKER_FAILED));
    return;
  }
  if (is_redundant()) {
    // DLOG(INFO) << "ServiceWorkerVersion::DidEnsureLiveRegistrationForStartWorker: is_redundant() = TRUE. error sending common::SERVICE_WORKER_ERROR_REDUNDANT";
    RecordStartWorkerResult(purpose, prestart_status, kInvalidTraceId,
                            is_browser_startup_complete,
                            common::SERVICE_WORKER_ERROR_REDUNDANT);
    RunSoon(
  ...
  switch (running_status()) {
    case EmbeddedWorkerStatus::RUNNING: {
      // DLOG(INFO) << "ServiceWorkerVersion::DidEnsureLiveRegistrationForStartWorker: running_status() = RUNNING. calling callback";
      RunSoon(base::BindOnce(std::move(callback), common::SERVICE_WORKER_OK));
      return;
    }
    case EmbeddedWorkerStatus::STARTING: {
      DCHECK(!start_callbacks_.empty());
    ...
    case EmbeddedWorkerStatus::STOPPING:
    case EmbeddedWorkerStatus::STOPPED:
      // DLOG(INFO) << "ServiceWorkerVersion::DidEnsureLiveRegistrationForStartWorker: running_status() = STOPPED.";
      if (start_callbacks_.empty()) {
  ...
  start_callbacks_.push_back(base::BindOnce(
      [](StatusCallback callback,
         ...
         common::ServiceWorkerStatusCode status) { std::move(callback).Run(status); },
...
void ServiceWorkerVersion::StartWorkerInternal() {
  // DLOG(INFO) << "ServiceWorkerVersion::StartWorkerInternal";
  ...
  auto params = common::mojom::EmbeddedWorkerStartParams::New();
  params->service_worker_version_id = version_id_;
  params->scope = scope_;
  params->script_url = script_url_;
  params->script_type = static_cast<common::mojom::EmbeddedWorkerScriptType>(script_type_);
...
void ServiceWorkerVersion::OnTimeoutTimer() {
  ...
  if (GetTickDuration(stop_time_) > kStopWorkerTimeout) {
    ...
    ReportError(common::SERVICE_WORKER_ERROR_TIMEOUT, "DETACH_STALLED_IN_STOPPING");
    ...
    // DLOG(INFO) << "ServiceWorkerVersion::OnTimeoutTimer => OnStoppedInternal()";
  ...
  if (GetTickDuration(start_time_) > start_limit) {
    DCHECK(running_status() == EmbeddedWorkerStatus::STARTING ||
        ...
        << static_cast<int>(running_status());
    scoped_refptr<ServiceWorkerVersion> protect(this);
    FinishStartWorker(common::SERVICE_WORKER_ERROR_TIMEOUT);
    if (running_status() == EmbeddedWorkerStatus::STARTING)
      embedded_worker_->Stop();
    return;
  }
...
void ServiceWorkerVersion::OnPingTimeout() {
  DCHECK(running_status() == EmbeddedWorkerStatus::STARTING ||
         running_status() == EmbeddedWorkerStatus::RUNNING);
  // TODO(falken): Change the error code to common::SERVICE_WORKER_ERROR_TIMEOUT.
...
void ServiceWorkerVersion::RecordStartWorkerResult(
  ...
  if (status == common::SERVICE_WORKER_OK && !start_time.is_null() &&
  ...
  if (status != common::SERVICE_WORKER_ERROR_TIMEOUT)
    return;
...
bool ServiceWorkerVersion::MaybeTimeoutRequest(
  ...
  // DLOG(INFO) << "ServiceWorkerVersion::MaybeTimeoutRequest";
  ...
  std::move(request->error_callback).Run(common::SERVICE_WORKER_ERROR_TIMEOUT);
...
common::ServiceWorkerStatusCode ServiceWorkerVersion::DeduceStartWorkerFailureReason(
    common::ServiceWorkerStatusCode default_code) {
  if (ping_controller_->IsTimedOut())
    return common::SERVICE_WORKER_ERROR_TIMEOUT;

  if (start_worker_status_ != common::SERVICE_WORKER_OK)
  ...
  if (main_script_status.status() != net::URLRequestStatus::SUCCESS) {
    if (net::IsCertificateError(main_script_status.error()))
      return common::SERVICE_WORKER_ERROR_SECURITY;
    switch (main_script_status.error()) {
      ...
      default:
        return common::SERVICE_WORKER_ERROR_NETWORK;
    }
  }
...
void ServiceWorkerVersion::MarkIfStale() {
  ...
  if (time_since_last_check > common::kServiceWorkerScriptMaxCacheAge)
...
void ServiceWorkerVersion::FoundRegistrationForUpdate(
  ...
  if (status != common::SERVICE_WORKER_OK || registration->active_version() != this)
    return;
...
void ServiceWorkerVersion::OnStoppedInternal(EmbeddedWorkerStatus old_status) {
  // DLOG(INFO) << "ServiceWorkerVersion::OnStoppedInternal";
  DCHECK_EQ(EmbeddedWorkerStatus::STOPPED, running_status());
  ...
  if (!should_restart) {
    // DLOG(INFO) << "ServiceWorkerVersion::OnStoppedInternal: OnStopped && should restart = false. sending SERVICE_WORKER_ERROR_START_WORKER_FAILED";   
  ...
  while (!iter.IsAtEnd()) {
    ...
    std::move(iter.GetCurrentValue()->error_callback)
        .Run(common::SERVICE_WORKER_ERROR_FAILED);
    iter.Advance();
  }
...
void ServiceWorkerVersion::FinishStartWorker(common::ServiceWorkerStatusCode status) {
  // DLOG(INFO) << "ServiceWorkerVersion::FinishStartWorker";
...
bool ServiceWorkerVersion::IsStartWorkerAllowed() const {
  ...
  if (!common::ServiceWorkerUtils::AllOriginsMatchAndCanAccessServiceWorkers(
          {script_url_})) {
    return false;
  }
  ...
  // tab, pass a null callback as ApplicationContents getter.
  ...
  //          base::Callback<ApplicationContents*(void)>()))) {
  //   return false;
  // }
