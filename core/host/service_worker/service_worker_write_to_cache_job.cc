...
bool ShouldIgnoreSSLError(net::URLRequest* request) {
  ...
  if (session_params && session_params->ignore_certificate_errors)
    return true;
  bool allow_localhost = true;//base::CommandLine::ForCurrentProcess()->HasSwitch(
...
ServiceWorkerWriteToCacheJob::ServiceWorkerWriteToCacheJob(
    ...
    : net::URLRequestJob(request, network_delegate),
      ...
      weak_factory_(this) {
  DCHECK(version_);
  DCHECK(resource_type_ == common::RESOURCE_TYPE_SCRIPT ||
         (resource_type_ == common::RESOURCE_TYPE_SERVICE_WORKER &&
...
int ServiceWorkerWriteToCacheJob::ReadRawData(net::IOBuffer* buf,
  ...
  if (rv < 0) {
    ...
    error = NotifyFinishedCaching(error, common::kServiceWorkerFetchScriptError);
    DCHECK_EQ(rv, error);
    return error;
  }
...
void ServiceWorkerWriteToCacheJob::InitNetRequest(
  ...
  net_request_->SetUserData(common::URLRequestServiceWorkerData::kUserDataKey,
                            std::make_unique<common::URLRequestServiceWorkerData>());
  if (extra_load_flags)
...
void ServiceWorkerWriteToCacheJob::OnReceivedRedirect(
  ...
  NotifyStartErrorHelper(net::ERR_UNSAFE_REDIRECT, common::kServiceWorkerRedirectError);
}
...
void ServiceWorkerWriteToCacheJob::OnResponseStarted(net::URLRequest* request,
  ...
  if (net_error != net::OK) {
    ...
    NotifyStartErrorHelper(error, common::kServiceWorkerFetchScriptError);
    return;
  }
  if (request->GetResponseCode() / 100 != 2) {
    std::string error_message = base::StringPrintf(
        common::kServiceWorkerBadHTTPResponseError, request->GetResponseCode());
  ...
  if (resource_type_ == common::RESOURCE_TYPE_SERVICE_WORKER) {
    ...
    if (!blink::IsSupportedJavascriptMimeType(mime_type)) {
      std::string error_message =
          mime_type.empty() ? common::kServiceWorkerNoMIMEError
...
void ServiceWorkerWriteToCacheJob::OnReadCompleted(net::URLRequest* request,
  ...
  if (bytes_read < 0) {
    ...
    result = NotifyFinishedCaching(error, common::kServiceWorkerFetchScriptError);
  } else {
...
bool ServiceWorkerWriteToCacheJob::CheckPathRestriction(
  ...
  bool has_header = headers->EnumerateHeader(nullptr, common::kServiceWorkerAllowed,
...
int ServiceWorkerWriteToCacheJob::HandleNetData(int bytes_read) {
  // DLOG(INFO) << "\n\nServiceWorkerWriteToCacheJob::HandleNetData: bytes_read = " << bytes_read << "\n";
...
net::Error ServiceWorkerWriteToCacheJob::NotifyFinishedCaching(
  ...
  if (net_error != net::OK) {
    ...
    version_->embedded_worker()->AddMessageToConsole(
        ...
        status_message.empty() ? common::kServiceWorkerFetchScriptError
                               : status_message);
  } else {
    size = cache_writer_->bytes_written();
    // DLOG(INFO) << "\n\nServiceWorkerWriteToCacheJob::NotifyFinishedCaching: bytes_written = " << size << "\n";
  }
...
bool ServiceWorkerWriteToCacheJob::ShouldByteForByteCheck() const {
  return incumbent_resource_id_ != common::kInvalidServiceWorkerResourceId &&
