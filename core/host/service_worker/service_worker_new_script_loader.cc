...
#include "core/host/service_worker/service_worker_new_script_loader.h"

#include <memory>
#include "base/numerics/safe_conversions.h"
#include "components/network_session_configurator/common/network_switches.h"
#include "core/host/appcache/appcache_response.h"
#include "core/host/service_worker/service_worker_cache_writer.h"
...
ServiceWorkerNewScriptLoader::ServiceWorkerNewScriptLoader(
    ...
    : request_url_(original_request.url),
      resource_type_(static_cast<common::ResourceType>(original_request.resource_type)),
      ...
      weak_factory_(this) {
  // DLOG(INFO) << "ServiceWorkerNewScriptLoader::ServiceWorkerNewScriptLoader";
  ...
  DCHECK((resource_type_ == common::RESOURCE_TYPE_SERVICE_WORKER &&
          version->status() == ServiceWorkerVersion::NEW) ||
         (resource_type_ == common::RESOURCE_TYPE_SCRIPT &&
  ...
  int64_t incumbent_cache_resource_id = common::kInvalidServiceWorkerResourceId;
  ...
  DCHECK(registration);
  const bool is_main_script = resource_type_ == common::RESOURCE_TYPE_SERVICE_WORKER;
  if (is_main_script) {
  ...
  if (common::ServiceWorkerUtils::ShouldBypassCacheDueToUpdateViaCache(
  ...
  if (incumbent_cache_resource_id != common::kInvalidServiceWorkerResourceId) {
  ...
  // DLOG(INFO) << "ServiceWorkerNewScriptLoader::ServiceWorkerNewScriptLoader: version_->script_cache_map()->NotifyStartedCaching()";
  ...
  //   HostThread::IO, 
  //   FROM_HERE, 
  //   base::BindOnce(&ServiceWorkerNewScriptLoader::PerfomRequestOnIO, base::Unretained(this), loader_factory_getter, routing_id, request_id, options, traffic_annotation));
}

ServiceWorkerNewScriptLoader::~ServiceWorkerNewScriptLoader() = default;

void ServiceWorkerNewScriptLoader::PerfomRequestOnIO(
  scoped_refptr<URLLoaderFactoryGetter> loader_factory_getter,
  ...
  if (non_network_loader_factory_) {
    // DLOG(INFO) << "ServiceWorkerNewScriptLoader::PerfomRequestOnIO: non_network_loader_factory_->CreateLoaderAndStart()";
    non_network_loader_factory_->CreateLoaderAndStart(
        mojo::MakeRequest(&network_loader_), routing_id, request_id, options,
        *resource_request_.get(), std::move(network_client),
        traffic_annotation);
  } else {
    // DLOG(INFO) << "ServiceWorkerNewScriptLoader::PerfomRequestOnIO: loader_factory_getter->GetNetworkFactory()->CreateLoaderAndStart(";
    loader_factory_getter->GetNetworkFactory()->CreateLoaderAndStart(
        mojo::MakeRequest(&network_loader_), routing_id, request_id, options,
        *resource_request_.get(), std::move(network_client),
        traffic_annotation);
...
void ServiceWorkerNewScriptLoader::OnReceiveResponse(
   ...
   // DLOG(INFO) << "ServiceWorkerNewScriptLoader::OnReceiveResponse: code = " << response_head.headers->response_code();
  if (!version_->context() || version_->is_redundant()) {
    CommitCompleted(network::URLLoaderCompletionStatus(net::ERR_FAILED),
                    common::kServiceWorkerFetchScriptError);
    return;
  }
  ...
  if (response_head.headers->response_code() / 100 != 2) {
    // DLOG(INFO) << "ServiceWorkerNewScriptLoader::OnReceiveResponse: code = " << response_head.headers->response_code() << " not 200. failing";
    // Non-2XX HTTP status code is handled as an error.
    std::string error_message =
        base::StringPrintf(common::kServiceWorkerBadHTTPResponseError,
  ...
  if (net::IsCertStatusError(response_head.cert_status) &&
    ...
    // DLOG(INFO) << "ServiceWorkerNewScriptLoader::OnReceiveResponse: response_head.cert_status = " << response_head.cert_status << " net::IsCertStatusError(response_head.cert_status) = true. failing";
    CommitCompleted(
        network::URLLoaderCompletionStatus(
            net::MapCertStatusToNetError(response_head.cert_status)),
        common::kServiceWorkerSSLError);
    return;
  }

  if (resource_type_ == common::RESOURCE_TYPE_SERVICE_WORKER) {
    if (!blink::IsSupportedJavascriptMimeType(response_head.mime_type)) {
      // DLOG(INFO) << "ServiceWorkerNewScriptLoader::OnReceiveResponse: response_head.mime_type = " << response_head.mime_type << " blink::IsSupportedJavascriptMimeType() = false. failing";
      std::string error_message =
          response_head.mime_type.empty()
              ? common::kServiceWorkerNoMIMEError
              : base::StringPrintf(common::kServiceWorkerBadMIMEError,
    ...
    bool has_header = response_head.headers->EnumerateHeader(
        nullptr, common::kServiceWorkerAllowed, &service_worker_allowed);
    std::string error_message;
    if (!common::ServiceWorkerUtils::IsPathRestrictionSatisfied(
            version_->scope(), request_url_,
            has_header ? &service_worker_allowed : nullptr, &error_message)) {
      // DLOG(INFO) << "ServiceWorkerNewScriptLoader::OnReceiveResponse: ServiceWorkerUtils::IsPathRestrictionSatisfied() = false. failing";
      CommitCompleted(
          network::URLLoaderCompletionStatus(net::ERR_INSECURE_RESPONSE),
          error_message);
      return;
...
void ServiceWorkerNewScriptLoader::OnReceiveRedirect(
  ...
  CommitCompleted(network::URLLoaderCompletionStatus(net::ERR_UNSAFE_REDIRECT),
                  common::kServiceWorkerRedirectError);
}
...
void ServiceWorkerNewScriptLoader::OnComplete(
  ...
  if (status.error_code != net::OK) {
    CommitCompleted(status, common::kServiceWorkerFetchScriptError);
    return;
  }
...
void ServiceWorkerNewScriptLoader::CommitCompleted(
  ...
  if (error_code == net::OK) {
    ...
    if (!cache_writer_->did_replace()) {
      version_->SetStartWorkerStatusCode(common::SERVICE_WORKER_ERROR_EXISTS);
