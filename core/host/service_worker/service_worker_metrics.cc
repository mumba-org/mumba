...
void ServiceWorkerMetrics::RecordStartWorkerStatus(
  ...
  if (!is_installed) {
    UMA_HISTOGRAM_ENUMERATION("ServiceWorker.StartNewWorker.Status", status,
                              common::SERVICE_WORKER_ERROR_MAX_VALUE);
    return;
  }
  ...
  RecordHistogramEnum(std::string("ServiceWorker.StartWorker.StatusByPurpose") +
                      ...
                      status, common::SERVICE_WORKER_ERROR_MAX_VALUE);
...
void ServiceWorkerMetrics::RecordNavigationPreloadResponse(
  ...
  DCHECK(resource_type == common::RESOURCE_TYPE_MAIN_FRAME ||
         resource_type == common::RESOURCE_TYPE_SUB_FRAME);
  const bool is_main_frame = (resource_type == common::RESOURCE_TYPE_MAIN_FRAME);
