...
//#include "core/host/devtools/service_worker_devtools_manager.h"
...
using SetupProcessCallback = base::OnceCallback<void(
    ...
    common::mojom::EmbeddedWorkerStartParamsPtr,
...
void SetupOnIOThread(
  ...
  std::unique_ptr<ServiceWorkerProcessManager::AllocatedProcessInfo> process_info,
  ...
  if (handle.type == kPROCESS_TYPE_APPLICATION) {
    handle.application->GetNetworkContext()->CreateRpcURLLoaderFactory(
      handle.application->loader_task_runner(),
      awh,
      registry,
      std::move(encoder),
      mojo::MakeRequest(&non_network_loader_factory));
  } else {
    handle.service->CreateURLLoaderFactory(mojo::MakeRequest(&non_network_loader_factory));
  }
...
void SetupOnUIThread(base::WeakPtr<ServiceWorkerProcessManager> process_manager,
                     ...
                     SetupProcessCallback callback) {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  ServiceWorkerProcessType process_type = type;
  bool will_create_non_network_loader_factory = false;
  auto process_info =
  ...
  if (type == kPROCESS_TYPE_APPLICATION) {
    // we are forcing the service worker here to always use the service process
    // as the other peer
    // revisit when/if a application might want a service worker on its own process
    // but this is unlikely as theres worker and shared worker already covering other uses  
    //aph = ApplicationProcessHost::FromID(app_process_id);
    //DCHECK(aph);
    //process_type = kPROCESS_TYPE_SERVICE;
    aph = ApplicationProcessHost::FromID(app_process_id);
    DCHECK(aph);
    //sph = aph->domain()->process();
    //DCHECK(sph);
  } 

  // Get a process.
  common::ServiceWorkerStatusCode status = process_manager->AllocateWorkerProcess(
      process_type,
  ...
  if (process_type == kPROCESS_TYPE_APPLICATION) {
    routing_id = aph->GetNextRoutingID();
    handle.application = aph;
  } else {
    routing_id = sph->GetNextRoutingID();
    handle.service = sph;
  }
  ...
  if (request.is_pending()) {
    if (process_type == kPROCESS_TYPE_APPLICATION) {
      common::BindInterface(aph, std::move(request));
    } else {
      common::BindInterface(sph, std::move(request));
    }
  }
  ...
  if (common::ServiceWorkerUtils::IsServicificationEnabled() &&
     ...
     will_create_non_network_loader_factory = true;
  ...
  params->devtools_worker_token = base::UnguessableToken::Create();
  ...
  if (!will_create_non_network_loader_factory) {
    ...
    ApplicationWindowHost* awh = nullptr;
    ApplicationContents* contents = nullptr;
    RouteController* controller = nullptr;
    RouteRegistry* registry = nullptr;
    RouteEntry* url_entry = nullptr;
    HostRpcService* service = nullptr;
    std::unique_ptr<net::RpcMessageEncoder> encoder;
    
    if (handle.type == kPROCESS_TYPE_APPLICATION) {
      ApplicationProcessHost* aph = handle.application;
      awh = aph->application_window_host();
      contents = awh->delegate()->GetAsApplicationContents();
      controller = contents->GetRouteController();
      registry = controller->registry();
      url_entry = controller->GetCurrent();
      service = url_entry->service();
      encoder = service->BuildEncoder();
    }

    HostThread::PostTask(
        HostThread::IO, 
        FROM_HERE,
        base::BindOnce(&SetupOnIOThread, 
          base::Passed(std::move(handle)), 
          base::Unretained(awh), 
          base::Unretained(registry), 
          base::Passed(std::move(encoder)),
          base::Passed(std::move(params)), 
          base::Passed(std::move(process_info)), 
...
class EmbeddedWorkerInstance::WorkerProcessHandle {
 public:
  WorkerProcessHandle(const base::WeakPtr<ServiceWorkerContextCore>& context,
    ...
    DCHECK_NE(common::ChildProcessHost::kInvalidUniqueID, process_id_);
  }
...
class EmbeddedWorkerInstance::StartTask {
 public:
  ...
  void Start(ServiceWorkerProcessType type,
             int process_id,
             common::mojom::EmbeddedWorkerStartParamsPtr params,
    ...
    if (type == kPROCESS_TYPE_SERVICE) {
      // FIXME: DomainProcessHost is allocated on IO while ApplicationProcessHost on UI
      //        this in turn affect the global id map allocated on them with thread affinity
    ...
    HostThread::PostTask(
      ...
      base::BindOnce(&SetupOnUIThread,
                  ...
                  type,
                  process_id,
                  can_use_existing_process, base::Passed(std::move(params)),
                  base::Passed(std::move(request_)), 
                  base::Unretained(instance_->context_.get()),
                  instance_->context_,
 ...
 private:
  ...
  common::mojom::EmbeddedWorkerInstanceClientRequest request_;
...
void EmbeddedWorkerInstance::Start(ServiceWorkerProcessType type,
                                   int process_id,
                                   common::mojom::EmbeddedWorkerStartParamsPtr params,
  ...
  //params->v8_cache_options = common::GetV8CacheOptions();
  ...
  inflight_start_task_->Start(type, process_id, std::move(params), std::move(callback));
}
...
void EmbeddedWorkerInstance::ResumeAfterDownload() {
  if (process_id() == common::ChildProcessHost::kInvalidUniqueID ||
      status_ != EmbeddedWorkerStatus::STARTING) {
    return;
  }
...
EmbeddedWorkerInstance::EmbeddedWorkerInstance(
    ...
    : context_(context),
      ...
      restart_count_(0),
      thread_id_(common::kInvalidEmbeddedWorkerThreadId),
...
common::ServiceWorkerStatusCode EmbeddedWorkerInstance::SendStartWorker(
    common::mojom::EmbeddedWorkerStartParamsPtr params,
    network::mojom::URLLoaderFactoryPtr non_network_loader_factory,
    ServiceWorkerProcessHandle handle) {
  if (!context_)
  ...
  if (!context_->GetDispatcherHost(process_id())) {
    ...
    return common::SERVICE_WORKER_ERROR_IPC_FAILED;
  }
  ...
  params->provider_info =
      std::move(provider_info_getter_)
          .Run(process_id(), std::move(non_network_loader_factory), std::move(handle));
...
void EmbeddedWorkerInstance::OnScriptEvaluated(bool success) {
  ...
  StartTask::RunStartCallback(
      ...
      success ? common::SERVICE_WORKER_OK
              : common::SERVICE_WORKER_ERROR_SCRIPT_EVALUATE_FAILED);
  // |this| may be destroyed by the callback.
}

void EmbeddedWorkerInstance::OnStarted(
    common::mojom::EmbeddedWorkerStartTimingPtr start_timing) {
...
int EmbeddedWorkerInstance::process_id() const {
  ...
  return common::ChildProcessHost::kInvalidUniqueID;
}
