...
using SetExtendableMessageEventSourceCallback =
    base::OnceCallback<bool(common::mojom::ExtendableMessageEventPtr*)>;
...
void DispatchExtendableMessageEventAfterStartWorker(
  ...
  common::mojom::ExtendableMessageEventPtr event = common::mojom::ExtendableMessageEvent::New();
  event->message = std::move(message);
  event->source_origin = CreateOrigin(source_origin);
...
void StartWorkerToDispatchExtendableMessageEvent(
    ...
    StatusCallback callback,
    SetExtendableMessageEventSourceCallback set_source_callback) {
  // DLOG(INFO) << "host::StartWorkerToDispatchExtendableMessageEvent";    
  ...
  if (timeout && *timeout < base::TimeDelta::FromMilliseconds(100)) {
    // DLOG(INFO) << "host::StartWorkerToDispatchExtendableMessageEvent: error SERVICE_WORKER_ERROR_TIMEOUT";
    std::move(callback).Run(common::SERVICE_WORKER_ERROR_TIMEOUT);
    return;
  }
...
bool SetSourceClientInfo(
  ...
  // DLOG(INFO) << "host::SetSourceClientInfo";    
...
bool SetSourceServiceWorkerInfo(scoped_refptr<ServiceWorkerVersion> worker,
                                ...
                                common::mojom::ExtendableMessageEventPtr* event) {
  // DLOG(INFO) << "host::SetSourceServiceWorkerInfo";                                
...
void DispatchExtendableMessageEventFromClient(
    ...
    StatusCallback callback,
    blink::mojom::ServiceWorkerClientInfoPtr source_client_info) {
  // DLOG(INFO) << "host::DispatchExtendableMessageEventFromClient";    
  // |source_client_info| may be null if a client sent the message but its
  // info could not be retrieved.
  if (!source_client_info) {
    // DLOG(INFO) << "host::DispatchExtendableMessageEventFromClient: failed";
    std::move(callback).Run(common::SERVICE_WORKER_ERROR_FAILED);
    return;
  }
...
void DispatchExtendableMessageEventFromServiceWorker(
    ...
    base::WeakPtr<ServiceWorkerProviderHost>
        source_service_worker_provider_host) {
  // DLOG(INFO) << "host::DispatchExtendableMessageEventFromServiceWorker";        
  if (!source_service_worker_provider_host) {
    // DLOG(INFO) << "host::DispatchExtendableMessageEventFromServiceWorker: failed";    
    std::move(callback).Run(common::SERVICE_WORKER_ERROR_FAILED);
    return;
  }
...
void ServiceWorkerHandle::PostMessageToServiceWorker(
    ::blink::TransferableMessage message) {
  // DLOG(INFO) << "host::ServiceWorkerHandle::PostMessageToServiceWorker";    
...
void ServiceWorkerHandle::DispatchExtendableMessageEvent(
    ::blink::TransferableMessage message,
    StatusCallback callback) {
  // DLOG(INFO) << "host::ServiceWorkerHandle::DispatchExtendableMessageEvent";    
  if (!context_) {
