...
void ReceiveResult(HostThread::ID run_quit_thread,
                   const base::Closure& quit,
                   Arg* out, Arg actual) {
  *out = actual;
  if (!quit.is_null())
    HostThread::PostTask(run_quit_thread, FROM_HERE, quit);
}

template <typename Arg> base::Callback<void(Arg)>
CreateReceiver(HostThread::ID run_quit_thread,
...
base::Callback<void(Arg)> CreateReceiverOnCurrentThread(
  ...
  HostThread::ID id;
  bool ret = HostThread::GetCurrentThreadIdentifier(&id);
  DCHECK(ret);
...
class ServiceWorkerRemoteProviderEndpoint {
 public:
  ...
  common::mojom::ServiceWorkerContainerHostAssociatedPtr* host_ptr() {
    return &host_ptr_;
  }
 ...
 private:
  ...
  common::mojom::ServiceWorkerContainerHostAssociatedPtr host_ptr_;
  // This is the other end of ServiceWorkerContainerAssociatedPtr owned by
  // content::ServiceWorkerProviderHost.
  common::mojom::ServiceWorkerContainerAssociatedRequest client_request_;
