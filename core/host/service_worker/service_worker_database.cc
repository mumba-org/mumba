...
#include "core/host/service_worker/service_worker_database.h"
...
#include "core/host/service_worker/service_worker_database.pb.h"
...
ServiceWorkerDatabase::Status ServiceWorkerDatabase::ReadRegistrationOrigin(
    int64_t registration_id,
    GURL* origin) {
  // DLOG(INFO) << "ServiceWorkerDatabase::ReadRegistrationOrigin: LazyOpen(true)";
  DCHECK(sequence_checker_.CalledOnValidSequence());
  DCHECK(origin);
...
ServiceWorkerDatabase::Status ServiceWorkerDatabase::WriteRegistration(
  ...
  // DLOG(INFO) << "ServiceWorkerDatabase::WriteRegistration: LazyOpen(true)";
  DCHECK(sequence_checker_.CalledOnValidSequence());
  ...
  PutUniqueOriginToBatch(GetOrigin(registration.scope), &batch);
  ...
  batch.Put(CreateRegistrationIdToOriginKey(registration.registration_id),
            GetOrigin(registration.scope).spec());
...
ServiceWorkerDatabase::Status ServiceWorkerDatabase::UpdateVersionToActive(
    int64_t registration_id,
    const GURL& origin) {
  // DLOG(INFO) << "ServiceWorkerDatabase::UpdateVersionToActive";
  DCHECK(sequence_checker_.CalledOnValidSequence());
  Status status = LazyOpen(true);
  if (IsNewOrNonexistentDatabase(status)) {
    // DLOG(INFO) << "ServiceWorkerDatabase::UpdateVersionToActive: IsNewOrNonexistentDatabase(status) = TRUE => STATUS_ERROR_NOT_FOUND";
    return STATUS_ERROR_NOT_FOUND;
  }
  if (status != STATUS_OK) {
    // DLOG(INFO) << "ServiceWorkerDatabase::UpdateVersionToActive: LazyOpen() != STATUS_OK";
    return status;
  }
  // if (!origin.is_valid()) {
  //   // DLOG(INFO) << "ServiceWorkerDatabase::UpdateVersionToActive: origin.is_valid = false => STATUS_ERROR_FAILED";
  //   return STATUS_ERROR_FAILED;
  // }
  ...
  if (status != STATUS_OK) {
    // DLOG(INFO) << "ServiceWorkerDatabase::UpdateVersionToActive: ReadRegistrationData() => status != STATUS_OK";
    return status;
  }
...
ServiceWorkerDatabase::WriteUncommittedResourceIds(
  ...
  // DLOG(INFO) << "ServiceWorkerDatabase::WriteUncommittedResourceIds";
  leveldb::WriteBatch batch;
...
ServiceWorkerDatabase::PurgeUncommittedResourceIds(
    const std::set<int64_t>& ids) {
  // DLOG(INFO) << "ServiceWorkerDatabase::PurgeUncommittedResourceIds";
  leveldb::WriteBatch batch;
  Status status = DeleteResourceIdsInBatch(
      service_worker_internals::kUncommittedResIdKeyPrefix, ids, &batch);
  if (status != STATUS_OK)
...
ServiceWorkerDatabase::Status ServiceWorkerDatabase::LazyOpen(
  ...
  if (state_ == DISABLED) {
    // DLOG(INFO) << "ServiceWorkerDatabase::LazyOpen: state_ == DISABLED => return STATUS_ERROR_FAILED";
    return STATUS_ERROR_FAILED;
  }
  if (IsOpen()) {
    // DLOG(INFO) << "ServiceWorkerDatabase::LazyOpen: already open => return STATUS_OK";
    return STATUS_OK;
  }
  ...
  if (!create_if_missing) {
    ...
    if (IsDatabaseInMemory() || !base::PathExists(path_) ||
      ...
      // DLOG(INFO) << "ServiceWorkerDatabase::LazyOpen: IsDatabaseInMemory() ? " << IsDatabaseInMemory() << " base::PathExists(path_) ? " << base::PathExists(path_) << " base::IsDirectoryEmpty(path_) ? " << base::IsDirectoryEmpty(path_) << " => return STATUS_ERROR_NOT_FOUND";
      return STATUS_ERROR_NOT_FOUND;
    }
  }

  // DLOG(INFO) << "ServiceWorkerDatabase::LazyOpen: opening db ...";
  ...
  if (status != STATUS_OK) {
    // DLOG(INFO) << "ServiceWorkerDatabase::LazyOpen: leveldb_env::OpenDB() failed";
  ...
  if (status != STATUS_OK) {
    // DLOG(INFO) << "ServiceWorkerDatabase::LazyOpen: ReadDatabaseVersion() failed";
    return status;
  }
  ...
  switch (db_version) {
    ...
    case 1:
      ...
      // should recreate it.
      // DLOG(INFO) << "ServiceWorkerDatabase::LazyOpen: ReadDatabaseVersion() == 1 => STATUS_ERROR_FAILED";
...
bool ServiceWorkerDatabase::IsNewOrNonexistentDatabase(
  ...
  if (status == STATUS_ERROR_NOT_FOUND) {
    // DLOG(INFO) << "ServiceWorkerDatabase::IsNewOrNonexistentDatabase: status = STATUS_ERROR_NOT_FOUND";
    return true;
  }
  if (status == STATUS_OK && state_ == UNINITIALIZED) {
    // DLOG(INFO) << "ServiceWorkerDatabase::IsNewOrNonexistentDatabase: status = STATUS_OK but state_ == UNINITIALIZED";
    return true;
  }
  return false;
}
...
ServiceWorkerDatabase::Status ServiceWorkerDatabase::ParseRegistrationData(
  ...
  if (!scope_url.is_valid() || !script_url.is_valid() ||
      GetOrigin(scope_url) != GetOrigin(script_url)) {
    return ServiceWorkerDatabase::STATUS_ERROR_CORRUPTED;
  }
  ...
  out->process_type = data.process_type();
  ...
  out->script_type = static_cast<blink::mojom::ScriptType>(data.script_type());
...
void ServiceWorkerDatabase::WriteRegistrationDataInBatch(
  ...
  data.set_process_id(registration.process_id);
  data.set_process_type(registration.process_type);
  ...
  data.set_script_type(static_cast<
    ServiceWorkerRegistrationData_ServiceWorkerScriptType>(
  ...
  DCHECK(success);
  GURL origin = GetOrigin(registration.scope);
...
ServiceWorkerDatabase::Status ServiceWorkerDatabase::WriteResourceIdsInBatch(
    ...
    const std::set<int64_t>& ids,
    leveldb::WriteBatch* batch) {
  // DLOG(INFO) << "ServiceWorkerDatabase::WriteResourceIdsInBatch: LazyOpen(true)";
  DCHECK(sequence_checker_.CalledOnValidSequence());
  DCHECK(id_key_prefix);
