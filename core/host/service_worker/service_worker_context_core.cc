...
void SuccessCollectorCallback(base::OnceClosure done_closure,
  ...
  if (status != common::ServiceWorkerStatusCode::SERVICE_WORKER_OK) {
...
void SuccessReportingCallback(
    const bool* success,
    base::OnceCallback<void(common::ServiceWorkerStatusCode)> callback) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  bool result = *success;
  std::move(callback).Run(
      result ? common::ServiceWorkerStatusCode::SERVICE_WORKER_OK
             : common::ServiceWorkerStatusCode::SERVICE_WORKER_ERROR_FAILED);
}

bool IsSameOriginClientProviderHost(const GURL& origin,
                                    ServiceWorkerProviderHost* host) {
  return host->IsProviderForClient() &&
         GetOrigin(host->document_url()) == origin &&
...
bool FrameListContainsMainFrameOnUI(
  ...
  for (const auto& frame : *frames) {
    ...
    ApplicationWindowHost* render_frame_host = ApplicationWindowHost::FromID(frame.first, frame.second);
...
class ClearAllServiceWorkersHelper
 ...
 public:
  ...
  void OnResult(common::ServiceWorkerStatusCode) {
    DCHECK_CURRENTLY_ON(HostThread::IO);
  ...
  void DidGetAllRegistrations(
    ...
    if (!context || status != common::SERVICE_WORKER_OK)
      return;
 ...
 private:
  ...
  ~ClearAllServiceWorkersHelper() {
    DCHECK_CURRENTLY_ON(HostThread::IO);
    HostThread::PostTask(HostThread::UI, FROM_HERE, std::move(callback_));
  }
...
void ServiceWorkerContextCore::CheckHasServiceWorker(
  ...
  storage()->FindRegistrationForDocument(
      url, kPROCESS_TYPE_APPLICATION, -1, base::BindOnce(&ServiceWorkerContextCore::
...
void ServiceWorkerContextCore::UpdateVersionFailureCount(
  ...
  if (status == common::SERVICE_WORKER_ERROR_DISALLOWED)
    return;
...
void ServiceWorkerContextCore::OnControlleeAdded(
  ...
  observer_list_->Notify(
      ...
      provider_host->application_contents_getter(), provider_host->provider_type());
}
...
void ServiceWorkerContextCore::DidFindRegistrationForCheckHasServiceWorker(
  ...
  if (!common::ServiceWorkerUtils::ScopeMatches(registration->pattern(), other_url)) {
    std::move(callback).Run(ServiceWorkerCapability::NO_SERVICE_WORKER);
    return;
  }
