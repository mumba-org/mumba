...
#include "core/host/service_worker/service_worker_context_core_observer.h"
...
#include "storage/host/blob/blob_reader.h"
...
void ReceiveFindRegistrationStatus(
  ...
  *out_status = status;
  if (!quit.is_null())
    HostThread::PostTask(run_quit_thread, FROM_HERE, std::move(quit));
}
...
class WorkerActivatedObserver
 ...
 public:
  ...
  void OnVersionStateChanged(int64_t version_id,
    ...
    ASSERT_TRUE(HostThread::CurrentlyOn(HostThread::IO));
...
class ServiceWorkerVersionBrowserTest : public ServiceWorkerBrowserTest {
 public:
  ...
  void StartWorker(ServiceWorkerStatusCode expected_status) {
    ASSERT_TRUE(HostThread::CurrentlyOn(HostThread::UI));
    ServiceWorkerStatusCode status = SERVICE_WORKER_ERROR_FAILED;
  ...
  void FindRegistrationForIdOnIOThread(base::OnceClosure done,
    ...
    wrapper()->context()->storage()->FindRegistrationForId(
        id, origin,
        CreateFindRegistrationReceiver(HostThread::UI, std::move(done),
                                       result));
  }
  ...
  void StartOnIOThread(const base::Closure& done,
    ...
    version_->StartWorker(ServiceWorkerMetrics::EventType::UNKNOWN,
                          CreateReceiver(HostThread::UI, done, result));
  }
  ...
  void ReceiveInstallEventOnIOThread(
    ...
    *out_result = mojo::ConvertTo<ServiceWorkerStatusCode>(status);
    if (!done.is_null())
      HostThread::PostTask(HostThread::UI, FROM_HERE, done);
  }
  ...
  void ReceiveFetchResultOnIOThread(
    ...
    if (!quit.is_null())
      HostThread::PostTask(HostThread::UI, FROM_HERE, quit);
  }
...
class WaitForLoaded : public EmbeddedWorkerInstance::Listener {
 public:
  ...
  void OnThreadStarted() override {
    DCHECK_CURRENTLY_ON(HostThread::IO);
    HostThread::PostTask(HostThread::UI, FROM_HERE, quit_);
  }
 ...
 public:
  ...
  void OnRunningStateChanged(ServiceWorkerVersion* version) override {
    if (version->running_status() == EmbeddedWorkerStatus::STOPPED)
      HostThread::PostTask(HostThread::UI, FROM_HERE, quit_closure_);
  }
