...
void ServiceWorkerRegistration::NotifyVersionAttributesChanged(
    common::ChangedVersionAttributesMask mask) {
  for (auto& observer : listeners_)
...
ServiceWorkerRegistrationInfo ServiceWorkerRegistration::GetInfo() {
  ...
  return ServiceWorkerRegistrationInfo(
      pattern(), update_via_cache(), script_type(), registration_id_,
...
void ServiceWorkerRegistration::SetActiveVersion(
  ...
  // DLOG(INFO) << "\nServiceWorkerRegistration::SetActiveVersion\n";
  ...
  mask.add(common::ChangedVersionAttributesMask::ACTIVE_VERSION);
...
void ServiceWorkerRegistration::SetWaitingVersion(
  ...
  mask.add(common::ChangedVersionAttributesMask::WAITING_VERSION);
...
void ServiceWorkerRegistration::SetInstallingVersion(
  ...
  mask.add(common::ChangedVersionAttributesMask::INSTALLING_VERSION);
...
void ServiceWorkerRegistration::ActivateWaitingVersionWhenReady() {
  // DLOG(INFO) << "ServiceWorkerRegistration::ActivateWaitingVersionWhenReady";
  DCHECK(waiting_version());
  should_activate_when_ready_ = true;
  if (IsReadyToActivate()) {
    // DLOG(INFO) << "ServiceWorkerRegistration::ActivateWaitingVersionWhenReady: IsReadyToActivate() = TRUE => ActivateWaitingVersion(false)";
    ActivateWaitingVersion(false /* delay */);
    return;
  } else {
    // DLOG(INFO) << "ServiceWorkerRegistration::ActivateWaitingVersionWhenReady: IsReadyToActivate() = false";
  }
...
void ServiceWorkerRegistration::ClaimClients() {
  // DLOG(INFO) << "ServiceWorkerRegistration::ClaimClients";
  DCHECK(context_);
  DCHECK(active_version());

  for (std::unique_ptr<ServiceWorkerContextCore::ProviderHostIterator> it =
           context_->GetClientProviderHostIterator(GetOrigin(pattern_));
       !it->IsAtEnd(); it->Advance()) {
    ServiceWorkerProviderHost* host = it->GetProviderHost();
    if (host->controller() == active_version()) {
      // DLOG(INFO) << "ServiceWorkerRegistration::ClaimClients: host->controller() == active_version() => continue";
      continue;
    }
    if (!host->IsContextSecureForServiceWorker()) {
      // DLOG(INFO) << "ServiceWorkerRegistration::ClaimClients: host->IsContextSecureForServiceWorker() = false => continue";
      continue;
    }
    if (host->MatchRegistration() == this) {
      // DLOG(INFO) << "ServiceWorkerRegistration::ClaimClients: host->MatchRegistration() == this => host->ClaimedByRegistration(this)";
...
void ServiceWorkerRegistration::AbortPendingClear(StatusCallback callback) {
  // DLOG(INFO) << "ServiceWorkerRegistration::AbortPendingClear";
  DCHECK(context_);
  if (!is_uninstalling()) {
    // DLOG(INFO) << "ServiceWorkerRegistration::AbortPendingClear: is_uninstalling() = false => calling callback(OK)";
    std::move(callback).Run(common::SERVICE_WORKER_OK);
    return;
  }
  // DLOG(INFO) << "ServiceWorkerRegistration::AbortPendingClear: is_uninstalling() = true => calling StoreRegistrtion()";
  is_uninstalling_ = false;
...
void ServiceWorkerRegistration::ActivateWaitingVersion(bool delay) {
  // DLOG(INFO) << "\n\nServiceWorkerRegistration::ActivateWaitingVersion\n\n";
  DCHECK_CURRENTLY_ON(HostThread::IO);
  DCHECK(context_);
...
void ServiceWorkerRegistration::ContinueActivation(
  ...
  // DLOG(INFO) << "ServiceWorkerRegistration::ContinueActivation";
  if (!context_)
    return;
...
void ServiceWorkerRegistration::DeleteVersion(
    const scoped_refptr<ServiceWorkerVersion>& version) {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::DeleteVersion";
...
void ServiceWorkerRegistration::DispatchActivateEvent(
  ...
  // DLOG(INFO) << "ServiceWorkerRegistration::DispatchActivateEvent";
  ...
  if (activating_version != active_version()) {
    OnActivateEventFinished(activating_version, common::SERVICE_WORKER_ERROR_FAILED);
    return;
  }
...
void ServiceWorkerRegistration::OnActivateEventFinished(
  ...
  // browser starts up.
  if (is_shutdown && status != common::SERVICE_WORKER_OK)
    return;
...
void ServiceWorkerRegistration::Clear() {
  // DLOG(INFO) << "ServiceWorkerRegistration::Clear";
  is_uninstalling_ = false;
  is_uninstalled_ = true;
  should_activate_when_ready_ = false;
  if (context_)
...
void ServiceWorkerRegistration::OnRestoreFinished(
    ...
    common::ServiceWorkerStatusCode status) {
  // DLOG(INFO) << "ServiceWorkerRegistration::OnRestoreFinished";    
  if (!context_) {
    // DLOG(INFO) << "ServiceWorkerRegistration::OnRestoreFinished: context_ is null => calling callback(ABORT)";
    std::move(callback).Run(common::SERVICE_WORKER_ERROR_ABORT);
    return;
  }
  context_->storage()->NotifyDoneInstallingRegistration(
      this, version.get(), status);
  // DLOG(INFO) << "ServiceWorkerRegistration::OnRestoreFinished: calling callback(status: " << status << " )";
