...
#include "core/host/service_worker/service_worker_register_job.h"
...
#include "core/host/service_worker/service_worker_metrics.h"
...
#include "core/host/service_worker/service_worker_write_to_cache_job.h"
...
ServiceWorkerRegisterJob::ServiceWorkerRegisterJob(
    ...
    : context_(context),
      ...
      promise_resolved_status_(common::SERVICE_WORKER_OK),
      weak_factory_(this) {
    // DLOG(INFO) << "ServiceWorkerRegister(): process_id = " << process_id_ << " type: " << type_;
}
...
ServiceWorkerRegisterJob::ServiceWorkerRegisterJob(
    ...
    : context_(context),
      ...
      script_type_(registration->script_type()),
...
void ServiceWorkerRegisterJob::Start() {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::Start: application_process_id = " << process_id_;
  HostThread::PostAfterStartupTask(
...
void ServiceWorkerRegisterJob::StartImpl() {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::StartImpl";
  SetPhase(START);
  ...
  if (registration.get())
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE,
        base::BindOnce(std::move(next_step), common::SERVICE_WORKER_OK, registration));
  else
...
void ServiceWorkerRegisterJob::Abort() {
  SetPhase(ABORT);
  CompleteInternal(common::SERVICE_WORKER_ERROR_ABORT, std::string());
...
void ServiceWorkerRegisterJob::DoomInstallingWorker() {
  ...
  if (phase_ == INSTALL)
    Complete(common::SERVICE_WORKER_ERROR_INSTALL_WORKER_FAILED, std::string());
}
...
void ServiceWorkerRegisterJob::ContinueWithRegistration(
  ...
  DCHECK_EQ(REGISTRATION_JOB, job_type_);
  if (status != common::SERVICE_WORKER_ERROR_NOT_FOUND && status != common::SERVICE_WORKER_OK) {
    // DLOG(INFO) << "ServiceWorkerRegisterJob::ContinueWithRegistration: status is bad. not calling UpdateAndContinue()";
    Complete(status);
    return;
  }

  if (!existing_registration.get() || existing_registration->is_uninstalled()) {
    // DLOG(INFO) << "ServiceWorkerRegisterJob::ContinueWithRegistration: calling RegisterAndContinue()";
    RegisterAndContinue();
    return;
  }
  ...
  if (existing_registration->GetNewestVersion()->script_url() == script_url_ &&
      ...
      // NOTE: change this here, as another app might retry the same script 
      //       another one already
      // TODO: maybe store some process uuid so we can compare if is the same instance here
    if (type_ == kPROCESS_TYPE_APPLICATION) { // || process_id_ != existing_registration->process_id()) {
      ...
      // DLOG(INFO) << "ServiceWorkerRegisterJob::ContinueWithRegistration: not calling UpdateAndContinue()";
      return;
    }
...
void ServiceWorkerRegisterJob::ContinueWithUpdate(
  ...
  if (existing_registration.get() != registration()) {
    // DLOG(INFO) << "ServiceWorkerRegisterJob::ContinueWithUpdate: error";
    Complete(common::SERVICE_WORKER_ERROR_NOT_FOUND);
    return;
  }
...
void ServiceWorkerRegisterJob::RegisterAndContinue() {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::RegisterAndContinue";
  SetPhase(REGISTER);
  ...
  if (registration_id == blink::mojom::kInvalidServiceWorkerRegistrationId) {
    // DLOG(INFO) << "ServiceWorkerRegisterJob::RegisterAndContinue: registration_id == blink::mojom::kInvalidServiceWorkerRegistrationId => aborting";
    Complete(common::SERVICE_WORKER_ERROR_ABORT);
    return;
  }
  ...
  blink::mojom::ServiceWorkerRegistrationOptions options(pattern_,
                                                         ...
                                                         update_via_cache_);
  set_registration(
      new ServiceWorkerRegistration(options, type_, process_id_, registration_id, context_));
...
void ServiceWorkerRegisterJob::ContinueWithRegistrationForSameScriptUrl(
    scoped_refptr<ServiceWorkerRegistration> existing_registration,
    common::ServiceWorkerStatusCode status) {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::ContinueWithRegistrationForSameScriptUrl";
  if (status != common::SERVICE_WORKER_OK) {
    // DLOG(INFO) << "ServiceWorkerRegisterJob::ContinueWithRegistrationForSameScriptUrl: status != common::SERVICE_WORKER_OK => not calling UpdateAndContinue()";
    Complete(status);
    return;
  }
  set_registration(existing_registration);
  ...
  if (existing_registration->active_version()) {
    // DLOG(INFO) << "ServiceWorkerRegisterJob::ContinueWithRegistrationForSameScriptUrl: existing_registration->active_version()";
    ResolvePromise(status, std::string(), existing_registration.get());
    Complete(common::SERVICE_WORKER_OK);
    return;
  }
...
void ServiceWorkerRegisterJob::UpdateAndContinue() {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::UpdateAndContinue: id: " << process_id_ << " type: " << type_;
  SetPhase(UPDATE);
  ...
  // the worker.
  set_new_version(new ServiceWorkerVersion(registration(), script_url_, script_type_,
                                           version_id, type_, process_id_, context_));
...
void ServiceWorkerRegisterJob::OnStartWorkerFinished(
    common::ServiceWorkerStatusCode status) {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::OnStartWorkerFinished: status = " << status;
  ...
  if (main_script_status.status() != net::URLRequestStatus::SUCCESS) {
    ...
    if (message.empty())
      message = common::kServiceWorkerFetchScriptError;
  }
...
void ServiceWorkerRegisterJob::InstallAndContinue() {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::InstallAndContinue";
  SetPhase(INSTALL);
  ...
  ResolvePromise(common::SERVICE_WORKER_OK, std::string(), registration());
  ...
  if (doom_installing_worker_)
    Complete(common::SERVICE_WORKER_ERROR_INSTALL_WORKER_FAILED);
}

void ServiceWorkerRegisterJob::DispatchInstallEvent(
    common::ServiceWorkerStatusCode start_worker_status) {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::DispatchInstallEvent";
...
void ServiceWorkerRegisterJob::OnInstallFinished(
  ...
  // DLOG(INFO) << "ServiceWorkerRegisterJob::OnInstallFinished: succeeded ? " << succeeded;
  new_version()->FinishRequest(request_id, succeeded, dispatch_event_time);

  if (!succeeded) {
    OnInstallFailed(mojo::ConvertTo<common::ServiceWorkerStatusCode>(event_status));
    return;
  }

  ServiceWorkerMetrics::RecordInstallEventStatus(common::SERVICE_WORKER_OK);
...
void ServiceWorkerRegisterJob::OnInstallFailed(common::ServiceWorkerStatusCode status) {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::OnInstallFailed: status = " << (int)status;
...
void ServiceWorkerRegisterJob::OnStoreRegistrationComplete(
    common::ServiceWorkerStatusCode status) {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::OnStoreRegistrationComplete";
  if (status != common::SERVICE_WORKER_OK && (type_ != kPROCESS_TYPE_SERVICE)) {
    // DLOG(INFO) << "ServiceWorkerRegisterJob::OnStoreRegistrationComplete: error";
    Complete(status);
    return;
  }
  ...
  if (registration()->waiting_version()) {
    // DLOG(INFO) << "ServiceWorkerRegisterJob::OnStoreRegistrationComplete: registration()->waiting_version() = true";
  ...
  // DLOG(INFO) << "ServiceWorkerRegisterJob::OnStoreRegistrationComplete: registration()->SetWaitingVersion(new_version())";
  ...
  // DLOG(INFO) << "ServiceWorkerRegisterJob::OnStoreRegistrationComplete: new_version()->SetStatus(ServiceWorkerVersion::INSTALLED)";
  ...
  // DLOG(INFO) << "ServiceWorkerRegisterJob::OnStoreRegistrationComplete: registration()->ActivateWaitingVersionWhenReady()";
  registration()->ActivateWaitingVersionWhenReady();

  Complete(common::SERVICE_WORKER_OK);
}

void ServiceWorkerRegisterJob::Complete(common::ServiceWorkerStatusCode status) {
...
void ServiceWorkerRegisterJob::CompleteInternal(
    common::ServiceWorkerStatusCode status,
    const std::string& status_message) {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::CompleteInternal: status = " << (int)status << " message: " << status_message;
  SetPhase(COMPLETE);
  ...
  if (status != common::SERVICE_WORKER_OK) {
    // DLOG(INFO) << "ServiceWorkerRegisterJob::CompleteInternal: error branch";
    if (registration()) {
      if (should_uninstall_on_failure_)
        registration()->ClearWhenReady();
      if (new_version()) {
        if (status == common::SERVICE_WORKER_ERROR_EXISTS)
          new_version()->SetStartWorkerStatusCode(common::SERVICE_WORKER_ERROR_EXISTS);
        else
      ...
      if (!registration()->waiting_version() &&
        ...
        context_->storage()->DeleteRegistration(
            registration()->id(), GetOrigin(registration()->pattern()),
            base::DoNothing());
      }
    }
    if (!is_promise_resolved_)
...
void ServiceWorkerRegisterJob::ResolvePromise(
    ...
    const std::string& status_message,
    ServiceWorkerRegistration* registration) {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::ResolvePromise";
  ...
  for (RegistrationCallback& callback : callbacks_) {
    // DLOG(INFO) << "ServiceWorkerRegisterJob::ResolvePromise: calling registration callback with status: " << status;
...
void ServiceWorkerRegisterJob::AddRegistrationToMatchingProviderHosts(
    ServiceWorkerRegistration* registration) {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::AddRegistrationToMatchingProviderHosts";
  DCHECK(registration);
  for (std::unique_ptr<ServiceWorkerContextCore::ProviderHostIterator> it =
    ...
    if (!common::ServiceWorkerUtils::ScopeMatches(registration->pattern(),
                                          host->document_url())) {
      continue;
    }
...
void ServiceWorkerRegisterJob::OnScriptLoaded() {
  // DLOG(INFO) << "ServiceWorkerRegisterJob::OnScriptLoaded";
  DCHECK(new_version()->pause_after_download());
  new_version()->set_pause_after_download(false);
  net::URLRequestStatus status =
      new_version()->script_cache_map()->main_script_status();
  if (!status.is_success() && type_ != kPROCESS_TYPE_SERVICE) {
     // DLOG(INFO) << "ServiceWorkerRegisterJob::OnScriptLoaded: status.is_success() = false";
    ...
    BumpLastUpdateCheckTimeIfNeeded();
    ResolvePromise(common::SERVICE_WORKER_OK, std::string(), registration());
    Complete(common::SERVICE_WORKER_ERROR_EXISTS,
