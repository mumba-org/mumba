...
#include "core/host/service_worker/service_worker_fetch_dispatcher.h"
...
//#include "core/host/devtools/service_worker_devtools_agent_host.h"
...
//#include "core/host/loader/url_loader_factory_impl.h"
...
EventType ResourceTypeToEventType(common::ResourceType resource_type) {
...
class ServiceWorkerFetchDispatcher::ResponseCallback
    : public common::mojom::ServiceWorkerFetchResponseCallback {
 public:
  ResponseCallback(common::mojom::ServiceWorkerFetchResponseCallbackRequest request,
  ...
  // Implements common::mojom::ServiceWorkerFetchResponseCallback.
  void OnResponse(const common::ServiceWorkerResponse& response,
  ...
  void OnResponseBlob(const common::ServiceWorkerResponse& response,
  ...
  void OnResponseLegacyBlob(const common::ServiceWorkerResponse& response,
  ...
  void OnFallback(base::Time dispatch_event_time) override {
    HandleResponse(fetch_dispatcher_, version_, fetch_event_id_,
                   common::ServiceWorkerResponse(), nullptr /* body_as_stream */,
 ...
 private:
  ...
  mojo::Binding<common::mojom::ServiceWorkerFetchResponseCallback> binding_;
...
ServiceWorkerFetchDispatcher::ServiceWorkerFetchDispatcher(
    ...
    : request_(std::move(request)),
      ...
      resource_type_(static_cast<common::ResourceType>(request_->resource_type)),
      net_log_(net_log),
...
void ServiceWorkerFetchDispatcher::StartWorker() {
  ...
  if (version_->status() != ServiceWorkerVersion::ACTIVATED) {
    ...
    DidFail(common::SERVICE_WORKER_ERROR_ACTIVATE_WORKER_FAILED);
    return;
  }
...
void ServiceWorkerFetchDispatcher::DispatchFetchEvent() {
  ...
  common::mojom::ServiceWorkerFetchResponseCallbackPtr response_callback_ptr;
  ...
  auto params = common::mojom::DispatchFetchEventParams::New();
...
void ServiceWorkerFetchDispatcher::DidFail(common::ServiceWorkerStatusCode status) {
  DCHECK_NE(common::SERVICE_WORKER_OK, status);
  Complete(status, FetchEventResult::kShouldFallback, common::ServiceWorkerResponse(),
           nullptr /* body_as_stream */, nullptr /* body_as_blob */);
}
...
void ServiceWorkerFetchDispatcher::DidFinish(
  ...
  Complete(common::SERVICE_WORKER_OK, fetch_result, response, std::move(body_as_stream),
           std::move(body_as_blob));
}
...
bool ServiceWorkerFetchDispatcher::MaybeStartNavigationPreload(
    ...
    base::OnceClosure on_response) {
  if (resource_type_ != common::RESOURCE_TYPE_MAIN_FRAME &&
      resource_type_ != common::RESOURCE_TYPE_SUB_FRAME) {
    return false;
  }
  ...
  request.request_initiator =
      original_request->initiator().has_value()
          ...
          : CreateOrigin(original_request->url());
  request.referrer = GURL(original_request->referrer());
  request.referrer_policy =
      common::Referrer::ReferrerPolicyForUrlRequest(original_info->GetReferrerPolicy());
  ...
  request.resource_type = common::RESOURCE_TYPE_SUB_RESOURCE;
  ...
  request.render_frame_id = original_info->GetApplicationWindowID();
  ...
  const int request_id = ResourceDispatcherHost::Get()->MakeRequestID();
  DCHECK_LT(request_id, -1);

  preload_handle_ = common::mojom::FetchEventPreloadHandle::New();
...
bool ServiceWorkerFetchDispatcher::MaybeStartNavigationPreloadWithURLLoader(
    ...
    base::OnceClosure on_response) {
  // DLOG(INFO) << "ServiceWorkerFetchDispatcher::MaybeStartNavigationPreloadWithURLLoader";
  if (resource_type_ != common::RESOURCE_TYPE_MAIN_FRAME &&
      resource_type_ != common::RESOURCE_TYPE_SUB_FRAME) {
    return false;
  }
  ...
  // Set to SUB_RESOURCE because we shouldn't trigger NavigationResourceThrottle
  // for the service worker navigation preload request.
  resource_request.resource_type = common::RESOURCE_TYPE_SUB_RESOURCE;
