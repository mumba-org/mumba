...
#include "core/host/service_worker/service_worker_info.h"
...
common::ServiceWorkerStatusCode DatabaseStatusToStatusCode(
  ...
  switch (status) {
    ...
    case ServiceWorkerDatabase::STATUS_ERROR_NOT_FOUND:
      return common::SERVICE_WORKER_ERROR_NOT_FOUND;
...
ServiceWorkerStorage::InitialData::InitialData()
    : next_registration_id(blink::mojom::kInvalidServiceWorkerRegistrationId),
      ...
      next_resource_id(common::kInvalidServiceWorkerResourceId) {}
...
void ServiceWorkerStorage::FindRegistrationForDocument(
  ...
  // DLOG(INFO) << "ServiceWorkerStorage::FindRegistrationForDocument";
  DCHECK(!document_url.has_ref());
  switch (state_) {
    case DISABLED:
      CompleteFindNow(scoped_refptr<ServiceWorkerRegistration>(),
                      common::SERVICE_WORKER_ERROR_ABORT, std::move(callback));
      return;
    case INITIALIZING:  // Fall-through.
    case UNINITIALIZED:
      LazyInitialize(base::BindOnce(
          &ServiceWorkerStorage::FindRegistrationForDocument,
          weak_factory_.GetWeakPtr(), document_url, process_type, process_id, std::move(callback)));
  ...
  if (!base::ContainsKey(registered_origins_, GetOrigin(document_url))) {
    // Look for something currently being installed.
    scoped_refptr<ServiceWorkerRegistration> installing_registration =
        FindInstallingRegistrationForDocument(document_url);
    common::ServiceWorkerStatusCode status = installing_registration
                                         ? common::SERVICE_WORKER_OK
  ...
  database_task_runner_->PostTask(
      ...
      base::BindOnce(
          ...
          base::ThreadTaskRunnerHandle::Get(), document_url, process_type, process_id,
...
void ServiceWorkerStorage::FindRegistrationForPattern(
    ...
    int process_id,
    FindRegistrationCallback callback) {
  // DLOG(INFO) << "ServiceWorkerStorage::FindRegistrationForPattern: process_id = " << process_id;
  switch (state_) {
    case DISABLED:
    ...
    case UNINITIALIZED:
      LazyInitialize(base::BindOnce(
          ...
          weak_factory_.GetWeakPtr(), scope, process_type, process_id, std::move(callback)));
      return;
    case INITIALIZED:
      break;
  }
  ...
  // See if there are any stored registrations for the origin.
  if (!base::ContainsKey(registered_origins_, GetOrigin(scope))) {
    // Look for something currently being installed.
    scoped_refptr<ServiceWorkerRegistration> installing_registration =
        FindInstallingRegistrationForPattern(scope);
    common::ServiceWorkerStatusCode installing_status =
        installing_registration ? common::SERVICE_WORKER_OK
                                : common::SERVICE_WORKER_ERROR_NOT_FOUND;
  ...
  database_task_runner_->PostTask(
      ...
      base::BindOnce(
          ...
          base::ThreadTaskRunnerHandle::Get(), scope, process_type, process_id,
...
void ServiceWorkerStorage::FindRegistrationForId(
    ...
    case UNINITIALIZED:
      LazyInitialize(
          base::BindOnce(&ServiceWorkerStorage::FindRegistrationForId,
                         weak_factory_.GetWeakPtr(), registration_id, origin, process_type, process_id,
                         std::move(callback)));
      return;
    case INITIALIZED:
      break;
  ...
  if (registration) {
    CompleteFindNow(std::move(registration), common::SERVICE_WORKER_OK,
                    std::move(callback));
    return;
  }
  ...
  database_task_runner_->PostTask(
      ...
      base::BindOnce(
          ...
          registration_id, origin, process_type, process_id,
...
void ServiceWorkerStorage::FindRegistrationForIdOnly(
  ...
  switch (state_) {
    case DISABLED:
      CompleteFindNow(nullptr, common::SERVICE_WORKER_ERROR_ABORT, std::move(callback));
      return;
    case INITIALIZING:  // Fall-through.
    case UNINITIALIZED:
      LazyInitialize(base::BindOnce(
          &ServiceWorkerStorage::FindRegistrationForIdOnly,
          weak_factory_.GetWeakPtr(), registration_id, process_type, process_id, std::move(callback)));
      return;
    case INITIALIZED:
      break;
  }
  ...
  if (registration) {
    ...
    FindRegistrationForId(registration_id, GetOrigin(registration->pattern()), process_type, process_id,
                          std::move(callback));
    return;
  }
  ...
  database_task_runner_->PostTask(
      ...
      base::BindOnce(
          ...
          base::ThreadTaskRunnerHandle::Get(), registration_id, process_type, process_id,
          base::BindOnce(&ServiceWorkerStorage::DidFindRegistrationForId,
                         weak_factory_.GetWeakPtr(), std::move(callback))));
}
...
void ServiceWorkerStorage::GetRegistrationsForOrigin(
  ...
  switch (state_) {
    case DISABLED:
      RunSoon(FROM_HERE,
              base::BindOnce(
                  std::move(callback), common::SERVICE_WORKER_ERROR_ABORT,
    ...
    case UNINITIALIZED:
      LazyInitialize(base::BindOnce(
          ...
          weak_factory_.GetWeakPtr(), origin, process_type, process_id, std::move(callback)));
      return;
    case INITIALIZED:
      break;
  }
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&ServiceWorkerStorage::DidGetRegistrationsForOrigin,
                     ...
                     origin, process_type, process_id));
}
...
void ServiceWorkerStorage::StoreRegistration(
  ...
  data.process_type = registration->process_type();
  data.process_id = registration->process_id();
  ...
  if (resources.empty()) {
    RunSoon(FROM_HERE,
            base::BindOnce(std::move(callback), common::SERVICE_WORKER_ERROR_FAILED));
    return;
  }
...
void ServiceWorkerStorage::UpdateToActiveState(
    ServiceWorkerRegistration* registration,
    StatusCallback callback) {
  // DLOG(INFO) << "ServiceWorkerStorage::UpdateToActiveState";
  DCHECK(registration);

  DCHECK(state_ == INITIALIZED || state_ == DISABLED) << state_;
  if (IsDisabled()) {
    // DLOG(INFO) << "ServiceWorkerStorage::UpdateToActiveState: IsDisabled() = true => common::SERVICE_WORKER_ERROR_ABORT";
    RunSoon(FROM_HERE,
            base::BindOnce(std::move(callback), common::SERVICE_WORKER_ERROR_ABORT));
    return;
  }
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&ServiceWorkerDatabase::UpdateVersionToActive,
                     ...
                     GetOrigin(registration->pattern())),
...
void ServiceWorkerStorage::StoreUncommittedResourceId(int64_t resource_id) {
  // DLOG(INFO) << "ServiceWorkerStorage::StoreUncommittedResourceId";
  DCHECK_NE(common::kInvalidServiceWorkerResourceId, resource_id);
...
void ServiceWorkerStorage::DoomUncommittedResources(
  ...
  // DLOG(INFO) << "ServiceWorkerStorage::DoomUncommittedResources";
  DCHECK(INITIALIZED == state_ || DISABLED == state_) << state_;
  if (IsDisabled())
    return;
...
void ServiceWorkerStorage::GetUserData(int64_t registration_id,
  ...
  if (registration_id == blink::mojom::kInvalidServiceWorkerRegistrationId ||
    ...
    RunSoon(FROM_HERE,
            base::BindOnce(std::move(callback), std::vector<std::string>(),
                           common::SERVICE_WORKER_ERROR_FAILED));
    return;
  }
...
void ServiceWorkerStorage::DeleteAndStartOver(StatusCallback callback) {
  // DLOG(INFO) << "ServiceWorkerStorage::DeleteAndStartOver";
  Disable();
...
void ServiceWorkerStorage::DiskCacheImplDoneWithDisk() {
  // DLOG(INFO) << "ServiceWorkerStorage::DiskCacheImplDoneWithDisk";
...
int64_t ServiceWorkerStorage::NewResourceId() {
  if (state_ == DISABLED)
    return common::kInvalidServiceWorkerResourceId;
  DCHECK_EQ(INITIALIZED, state_);
...
void ServiceWorkerStorage::NotifyInstallingRegistration(
      ServiceWorkerRegistration* registration) {
  // DLOG(INFO) << "\n\nServiceWorkerStorage::NotifyInstallingRegistration\n\n";
...
void ServiceWorkerStorage::NotifyDoneInstallingRegistration(
  ...
  if (status != common::SERVICE_WORKER_OK && version) {
    ResourceList resources;
    version->script_cache_map()->GetResources(&resources);
...
void ServiceWorkerStorage::Disable() {
  // DLOG(INFO) << "ServiceWorkerStorage::Disable";
  state_ = DISABLED;
  if (disk_cache_)
    disk_cache_->Disable();
}
...
ServiceWorkerStorage::ServiceWorkerStorage(
    ...
    : next_registration_id_(blink::mojom::kInvalidServiceWorkerRegistrationId),
      ...
      next_resource_id_(common::kInvalidServiceWorkerResourceId),
...
void ServiceWorkerStorage::DidFindRegistrationForDocument(
    ...
    ServiceWorkerDatabase::Status status) {
  // DLOG(INFO) << "ServiceWorkerStorage::DidFindRegistrationForDocument";   
  if (status == ServiceWorkerDatabase::STATUS_OK) {
  ...
  if (status == ServiceWorkerDatabase::STATUS_ERROR_NOT_FOUND) {
    ...
    TRACE_EVENT_ASYNC_END2(
        ...
        "Status", ServiceWorkerDatabase::StatusToString(status),
        "Info",
        (installing_status == common::SERVICE_WORKER_OK) ?
...
void ServiceWorkerStorage::DidFindRegistrationForPattern(
    ...
    const ResourceList& resources,
    ServiceWorkerDatabase::Status status) {
  // DLOG(INFO) << "ServiceWorkerStorage::DidFindRegistrationForPattern";
  if (status == ServiceWorkerDatabase::STATUS_OK) {
    ReturnFoundRegistration(std::move(callback), data, resources);
    return;
  }
...
void ServiceWorkerStorage::DidFindRegistrationForId(
    ...
    const ResourceList& resources,
    ServiceWorkerDatabase::Status status) {
  // DLOG(INFO) << "ServiceWorkerStorage::DidFindRegistrationForId";
  if (status == ServiceWorkerDatabase::STATUS_OK) {
    ReturnFoundRegistration(std::move(callback), data, resources);
    return;
  }
...
void ServiceWorkerStorage::ReturnFoundRegistration(
    ...
    const ServiceWorkerDatabase::RegistrationData& data,
    const ResourceList& resources) {
  // DLOG(INFO) << "ServiceWorkerStorage::ReturnFoundRegistration: process_id = " << data.process_id;
  DCHECK(!resources.empty());
  scoped_refptr<ServiceWorkerRegistration> registration =
      GetOrCreateRegistration(static_cast<ServiceWorkerProcessType>(data.process_type), data.process_id, data, resources);
  CompleteFindNow(std::move(registration), common::SERVICE_WORKER_OK,
                  std::move(callback));
}
...
void ServiceWorkerStorage::DidGetRegistrationsForOrigin(
    ...
    int process_id,
    ServiceWorkerDatabase::Status status) {
  // DLOG(INFO) << "ServiceWorkerStorage::DidGetRegistrationsForOrigin: process_id = " << process_id;
...
void ServiceWorkerStorage::DidGetAllRegistrationsInfos(
  ...
  std::move(callback).Run(common::SERVICE_WORKER_OK, infos);
}
...
void ServiceWorkerStorage::DidStoreRegistration(
  ...
  if (quota_manager_proxy_) {
    ...
    quota_manager_proxy_->NotifyStorageModified(
        storage::QuotaClient::kServiceWorker, CreateOrigin(origin),
...
void ServiceWorkerStorage::DidUpdateToActiveState(
  ...
  if (status != ServiceWorkerDatabase::STATUS_OK &&
      status != ServiceWorkerDatabase::STATUS_ERROR_NOT_FOUND) {
    // DLOG(INFO) << "ServiceWorkerStorage::DidUpdateToActiveState: ERROR status => " << (int)status;
    ScheduleDeleteAndStartOver();
  }
  std::move(callback).Run(DatabaseStatusToStatusCode(status));
}
...
void ServiceWorkerStorage::DidDeleteRegistration(
  ...
  if (quota_manager_proxy_) {
    ...
    quota_manager_proxy_->NotifyStorageModified(
        ...
        CreateOrigin(params->origin),
        blink::mojom::StorageType::kTemporary,
        -deleted_version.resources_total_size_bytes);
  }
  if (origin_state == OriginState::kDelete)
    registered_origins_.erase(params->origin);
  std::move(params->callback).Run(common::SERVICE_WORKER_OK);
...
ServiceWorkerStorage::GetOrCreateRegistration(
    ...
    const ServiceWorkerDatabase::RegistrationData& data,
    const ResourceList& resources) {
  // DLOG(INFO) << "\nServiceWorkerStorage::GetOrCreateRegistration: " << process_id << " type: " << process_type;
  scoped_refptr<ServiceWorkerRegistration> registration =
  ...
  registration =
      new ServiceWorkerRegistration(options, process_type, process_id, data.registration_id, context_);
  ...
  if (!version) {
    version = new ServiceWorkerVersion(
        registration.get(), data.script, data.script_type, data.version_id, process_type, process_id, context_);
  ...
  if (version->status() == ServiceWorkerVersion::ACTIVATED) {
    // DLOG(INFO) << "\nServiceWorkerStorage::GetOrCreateRegistration: version->status() == ServiceWorkerVersion::ACTIVATED => registration->SetActiveVersion(version)\n";
    registration->SetActiveVersion(version);
  } else if (version->status() == ServiceWorkerVersion::INSTALLED) {
    // DLOG(INFO) << "\nServiceWorkerStorage::GetOrCreateRegistration: version->status() == ServiceWorkerVersion::INSTALLED => registration->SetWaitingVersion(version)\n";
...
ServiceWorkerStorage::FindInstallingRegistrationForDocument(
    const GURL& document_url) {
  // DLOG(INFO) << "ServiceWorkerStorage::FindRegistrationForDocument: " << document_url.spec();
  DCHECK(!document_url.has_ref());

  common::LongestScopeMatcher matcher(document_url);
...
ServiceWorkerStorage::FindInstallingRegistrationForPattern(const GURL& scope) {
  // DLOG(INFO) << "ServiceWorkerStorage::FindInstallingRegistrationForPattern";
  for (const auto& registration : installing_registrations_)
    if (registration.second->pattern() == scope)
      return registration.second.get();
  return nullptr;
...
void ServiceWorkerStorage::EnsureDiskCachedInitialized() {
  base::IgnoreResult(disk_cache());
}
...
void ServiceWorkerStorage::InitializeDiskCache() {
  // DLOG(INFO) << "ServiceWorkerStorage::InitializeDiskCache: disk_cache_->InitWithDiskBackend(..) => path = " << GetDiskCachePath();
...
void ServiceWorkerStorage::OnDiskCacheInitialized(int rv) {
  // DLOG(INFO) << "ServiceWorkerStorage::OnDiskCacheInitialized: rv = " << rv;
  if (rv != net::OK) {
...
void ServiceWorkerStorage::WriteRegistrationInDB(
  ...
  original_task_runner->PostTask(
      FROM_HERE,
      base::BindOnce(std::move(callback), GetOrigin(data.script),
...
void ServiceWorkerStorage::FindForDocumentInDB(
  ...
  GURL origin = GetOrigin(document_url);
  RegistrationList registration_data_list;
...
void ServiceWorkerStorage::FindForPatternInDB(
    ...
    int process_id,
    FindInDBCallback callback) {
  // DLOG(INFO) << "ServiceWorkerStorage::FindForPatternInDB: process_id = " << process_id;
  GURL origin = GetOrigin(scope);
  RegistrationList registration_data_list;
  ServiceWorkerDatabase::Status status = database->GetRegistrationsForOrigin(
      origin, &registration_data_list, nullptr);
  if (status != ServiceWorkerDatabase::STATUS_OK) {
    // DLOG(INFO) << "ServiceWorkerStorage::FindForPatternInDB: NOT OK";
    original_task_runner->PostTask(
        FROM_HERE, base::BindOnce(std::move(callback),
                                  ServiceWorkerDatabase::RegistrationData(),
                                  ResourceList(), status));
...
void ServiceWorkerStorage::FindForIdOnlyInDB(
  ...
  FindForIdInDB(database, original_task_runner, registration_id, origin, process_type, process_id,
                std::move(callback));
}
...
void ServiceWorkerStorage::ScheduleDeleteAndStartOver() {
  // DLOG(INFO) << "ServiceWorkerStorage::ScheduleDeleteAndStartOver";
...
void ServiceWorkerStorage::DidDeleteDatabase(
    StatusCallback callback,
    ServiceWorkerDatabase::Status status) {
  // DLOG(INFO) << "ServiceWorkerStorage::DidDeleteDatabase";
...
void ServiceWorkerStorage::DidDeleteDiskCache(StatusCallback callback,
                                              bool result) {
  // DLOG(INFO) << "ServiceWorkerStorage::DidDeleteDiskCache";
  DCHECK_EQ(DISABLED, state_);
  if (!result) {
    // Give up the corruption recovery until the browser restarts.
