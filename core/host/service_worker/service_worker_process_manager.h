...
class CONTENT_EXPORT ServiceWorkerProcessManager {
 public:
  ...
  explicit ServiceWorkerProcessManager(Domain* domain);//BrowserContext* browser_context);
  ...
  //
  // Called on the UI thread.
  common::ServiceWorkerStatusCode AllocateWorkerProcess(ServiceWorkerProcessType type,
 ...
 private:
  ...
  //std::map<int, scoped_refptr<Application>> worker_process_map_;
  std::map<int, ServiceWorkerProcessHandle> worker_process_map_;
...
template <>
struct CONTENT_EXPORT default_delete<host::ServiceWorkerProcessManager> {
  void operator()(host::ServiceWorkerProcessManager* ptr) const;
};
}  // namespace std
