...
//#include "core/host/net/view_blob_internals_job_factory.h"
...
// URLRequestHostJob is a net::URLRequestJob that manages running
...
class URLRequestHostJob : public net::URLRequestJob {
 public:
  URLRequestHostJob(net::URLRequest* request,
  ...
  void DataAvailable(base::RefCountedMemory* bytes, bool should_complete);
 ...
 private:
  ...
  static void DelayStartForDevTools(
      const base::WeakPtr<URLRequestHostJob>& job);
  ...
  base::WeakPtrFactory<URLRequestHostJob> weak_factory_;

  DISALLOW_COPY_AND_ASSIGN(URLRequestHostJob);
};

URLRequestHostJob::URLRequestHostJob(net::URLRequest* request,
                                         net::NetworkDelegate* network_delegate,
                                         URLDataManagerBackend* backend)
...
URLRequestHostJob::~URLRequestHostJob() {
  CHECK(!backend_->HasPendingJob(this));
}

void URLRequestHostJob::Start() {
  ...
  //      base::BindOnce(&URLRequestHostJob::DelayStartForDevTools,
  ...
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE, base::BindOnce(&URLRequestHostJob::StartAsync,
                                weak_factory_.GetWeakPtr()));
...
void URLRequestHostJob::Kill() {
  weak_factory_.InvalidateWeakPtrs();
  backend_->RemoveRequest(this);
  URLRequestJob::Kill();
}

bool URLRequestHostJob::GetMimeType(std::string* mime_type) const {
  *mime_type = mime_type_;
  return !mime_type_.empty();
}

void URLRequestHostJob::GetResponseInfo(net::HttpResponseInfo* info) {
  DLOG(INFO) << "URLRequestHostJob::GetResponseInfo";
  DCHECK(!info->headers.get());
  URLDataSource* source = backend_->GetDataSourceFromURL(request()->url());
  std::string path;
  URLDataManagerBackend::URLToRequestPath(request()->url(), &path);
  info->headers = source->GetHeaders(request()->url().scheme(), path, GetOriginHeaderValue(request()));
  if (is_gzipped_) {
    info->headers->AddHeader("Content-Encoding: gzip");
  }
}

std::unique_ptr<net::SourceStream> URLRequestHostJob::SetUpSourceStream() {
...
void URLRequestHostJob::MimeTypeAvailable(const std::string& mime_type) {
  mime_type_ = mime_type;
  NotifyHeadersComplete();
}

void URLRequestHostJob::DataAvailable(base::RefCountedMemory* bytes, bool should_complete) {
  DLOG(INFO) << "URLRequestHostJob::DataAvailable";
...
int URLRequestHostJob::ReadRawData(net::IOBuffer* buf, int buf_size) {
  DLOG(INFO) << "URLRequestHostJob::ReadRawData";
...
int URLRequestHostJob::PostReadTask(scoped_refptr<net::IOBuffer> buf,
                                      int buf_size) {
  DLOG(INFO) << "URLRequestHostJob::PostReadTask";
  DCHECK(buf);
  DCHECK(data_);
  CHECK(buf->data());
  ...
  if (buf_size == 0) {
    DLOG(INFO) << "URLRequestHostJob::PostReadTask: buf_size == 0. returning 0";
    return 0;
  }
  ...
  base::PostTaskWithTraitsAndReply(
      ...
      base::BindOnce(&CopyData, base::RetainedRef(buf), buf_size, data_,
                     data_offset_),
      base::BindOnce(&URLRequestHostJob::ReadRawDataComplete,
...
void URLRequestHostJob::DelayStartForDevTools(
    const base::WeakPtr<URLRequestHostJob>& job) {
  HostThread::PostTask(
      HostThread::IO, FROM_HERE,
      base::BindOnce(&URLRequestHostJob::StartAsync, job));
}

void URLRequestHostJob::StartAsync() {
  DLOG(INFO) << "URLRequestHostJob::StartAsync";
...
class ChromeProtocolHandler
 ...
 public:
  ...
  net::URLRequestJob* MaybeCreateJob(
    ...
    // Check for app://network-error/, which uses its own job type.
    if (request->url().SchemeIs(kAppScheme) &&
...
URLDataManagerBackend::URLDataManagerBackend(Domain* domain)
    : domain_(domain), 
      route_registry_(domain_->workspace()->route_registry()), 
      next_request_id_(0) {
}

URLDataManagerBackend::~URLDataManagerBackend() {
  for (const auto& i : data_sources_)
    i.second->SetBackend(nullptr);
  data_sources_.clear();
}
...
void URLDataManagerBackend::AddDataSource(
  ...
  DataSourceMap::iterator i = data_sources_.find(source->GetSource());
  if (i != data_sources_.end()) {
    if (!source->ShouldReplaceExistingSource())
      return;
    i->second->SetBackend(nullptr);
  }
  data_sources_[source->GetSource()] = source;
  source->SetBackend(this);
  source->SetRouteRegistry(route_registry_);
}

void URLDataManagerBackend::UpdateAppDataSource(
    const std::string& source_name,
    const base::DictionaryValue& update) {
  //DataSourceMap::iterator it = data_sources_.find(source_name);
  //if (it == data_sources_.end() || !it->second->IsAppDataSourceImpl()) {
  //  NOTREACHED();
  //  return;
  //}
  //static_cast<RpcDataSource*>(it->second.get())
  //    ->AddLocalizedStrings(update);
}

bool URLDataManagerBackend::HasPendingJob(
    URLRequestHostJob* job) const {
...
bool URLDataManagerBackend::StartRequest(const net::URLRequest* request,
  ...
  URLDataSource* source = GetDataSourceFromURL(request->url());
  if (!source)
    return false;

  const ResourceRequestInfo* info = ResourceRequestInfo::ForRequest(request);
  if (!source->ShouldServiceRequest(
  ...
  job->set_is_gzipped(source->IsGzipped(scheme, path));

  // TODO(dschuyler): improve filtering of which resource to run template
  // replacements upon.
  std::string mime_type = source->GetMimeType(scheme, path);
  ...
  if (info)
    wc_getter = info->GetApplicationContentsGetterForRequest();

  // Forward along the request to the data source.
  scoped_refptr<base::SingleThreadTaskRunner> target_runner =
      source->TaskRunnerForRequestPath(scheme, path);
  if (!target_runner) {
    ...
    source->StartDataRequest(
        ...
        path, 
        std::move(wc_getter),
        base::Bind(&URLDataSource::SendResponse, source, request_id));
  } else {
    // The DataSource wants StartDataRequest to be called on a specific thread,
    // usually the UI thread, for this path.
    target_runner->PostTask(
        FROM_HERE, base::BindOnce(&URLDataManagerBackend::CallStartRequest,
                                  base::RetainedRef(source), request->url(), path,
                                  std::move(wc_getter), request_id));
  }
  return true;
}

URLDataSource* URLDataManagerBackend::GetDataSourceFromURL(
    const GURL& url) {
  std::string data;
  ...
  DLOG(INFO) << "URLDataManagerBackend::GetDataSourceFromURL: " << 
   "\n data sources available: count = " << data_sources_.size() << " url: " << url << " host: '" << url.scheme() << "' path: '" << url.path() << "' \n" << data;
  ...
  std::string host = url.scheme(); 
  //if (!host.empty()) {
  //  auto offset = host.find_last_of("/"); 
  //  if (offset != std::string::npos) {
  //    host = host.substr(2, offset - 2);
  //  }
    //auto last = host.find("/");
    //if (last != std::string::npos) {
    //  host = host.substr(0, last);
    //}
  //}
  DLOG(INFO) << "URLDataManagerBackend::GetDataSourceFromURL:  looking up for '" << host << "'";
  
  DataSourceMap::iterator i = data_sources_.find(host);// + "://");
  if (i != data_sources_.end())
    return i->second.get();
...
bool URLDataManagerBackend::HaveDataSourceForURL(const GURL& url) const {
  DataSourceMap::const_iterator i = data_sources_.find(url.scheme());
  if (i != data_sources_.end()) {
    return true;  
  }
  return false;
...
void URLDataManagerBackend::RemoveRequest(URLRequestHostJob* job) {
...
void URLDataManagerBackend::DataAvailable(RequestID request_id,
  ...
  DLOG(INFO) << "URLDataManagerBackend::DataAvailable";
  // Forward this data on to the pending net::URLRequest, if it exists.
  PendingRequestMap::iterator i = pending_requests_.find(request_id);
  if (i != pending_requests_.end()) {
    URLRequestHostJob* job = i->second;
    pending_requests_.erase(i);
    job->DataAvailable(contents.get(), should_complete);
  }
}

bool URLDataManagerBackend::CheckURLIsValid(const GURL& url) {
  std::vector<std::string> additional_schemes;
  //DCHECK(url.SchemeIs(kAppScheme) ||
...
void URLDataManagerBackend::URLToRequestPath(const GURL& url,
  ...
  //int end_offset = (offset + parsed.path.len) == spec.size() ? parsed.path.len : parsed.path.len -1;

  //if (offset < static_cast<int>(spec.size()))
  //  path->assign(spec.substr(offset, end_offset));
  path->assign(url.path().substr(1));
}
...
std::vector<std::string> URLDataManagerBackend::GetAppSchemes() {
  std::vector<std::string> schemes;
  schemes.push_back(kAppScheme);
