...
//#include "core/host/accessibility/one_shot_accessibility_tree_search.h"
#include "core/host/application/direct_manipulation_helper_win.h"
...
// static
LegacyApplicationWindowHostHWND* LegacyApplicationWindowHostHWND::Create(
    HWND parent) {
  // content_unittests passes in the desktop window as the parent. We allow
  // the LegacyApplicationWindowHostHWND instance to be created in this case for
  // these tests to pass.
  ...
  LegacyApplicationWindowHostHWND* legacy_window_instance =
      new LegacyApplicationWindowHostHWND(parent);
...
void LegacyApplicationWindowHostHWND::Destroy() {
...
void LegacyApplicationWindowHostHWND::UpdateParent(HWND parent) {
...
HWND LegacyApplicationWindowHostHWND::GetParent() {
  return ::GetParent(hwnd());
}

void LegacyApplicationWindowHostHWND::Show() {
  ::ShowWindow(hwnd(), SW_SHOW);
}

void LegacyApplicationWindowHostHWND::Hide() {
  ::ShowWindow(hwnd(), SW_HIDE);
}

void LegacyApplicationWindowHostHWND::SetBounds(const gfx::Rect& bounds) {
...
void LegacyApplicationWindowHostHWND::OnFinalMessage(HWND hwnd) {
  if (host_) {
...
LegacyApplicationWindowHostHWND::LegacyApplicationWindowHostHWND(HWND parent)
...
LegacyApplicationWindowHostHWND::~LegacyApplicationWindowHostHWND() {
  DCHECK(!::IsWindow(hwnd()));
}

bool LegacyApplicationWindowHostHWND::Init() {
...
// static
ui::WindowEventTarget* LegacyApplicationWindowHostHWND::GetWindowEventTarget(
    HWND parent) {
  return reinterpret_cast<ui::WindowEventTarget*>(ui::ViewProp::GetValue(
      parent, ui::WindowEventTarget::kWin32InputEventTarget));
}

LRESULT LegacyApplicationWindowHostHWND::OnEraseBkGnd(UINT message,
                                                 WPARAM w_param,
                                                 LPARAM l_param) {
  return 1;
}

LRESULT LegacyApplicationWindowHostHWND::OnGetObject(UINT message,
                                                WPARAM w_param,
                                                LPARAM l_param) {
...
LRESULT LegacyApplicationWindowHostHWND::OnKeyboardRange(UINT message,
                                                    WPARAM w_param,
                                                    LPARAM l_param,
                                                    BOOL& handled) {
  LRESULT ret = 0;
...
LRESULT LegacyApplicationWindowHostHWND::OnMouseRange(UINT message,
                                                 WPARAM w_param,
                                                 LPARAM l_param,
                                                 BOOL& handled) {
...
LRESULT LegacyApplicationWindowHostHWND::OnMouseLeave(UINT message,
                                                 WPARAM w_param,
                                                 LPARAM l_param) {
...
LRESULT LegacyApplicationWindowHostHWND::OnMouseActivate(UINT message,
                                                    WPARAM w_param,
                                                    LPARAM l_param) {
...
LRESULT LegacyApplicationWindowHostHWND::OnPointer(UINT message,
                                              WPARAM w_param,
                                              LPARAM l_param) {
  LRESULT ret = 0;
  if (GetWindowEventTarget(GetParent())) {
...
LRESULT LegacyApplicationWindowHostHWND::OnTouch(UINT message,
                                            WPARAM w_param,
                                            LPARAM l_param) {
  LRESULT ret = 0;
  if (GetWindowEventTarget(GetParent())) {
...
///LRESULT LegacyApplicationWindowHostHWND::OnInput(UINT message,
//                                            WPARAM w_param,
//                                            LPARAM l_param) {
//  LRESULT ret = 0;
//  if (GetWindowEventTarget(GetParent())) {
...
LRESULT LegacyApplicationWindowHostHWND::OnWindowPosChanged(UINT message,
                                                       WPARAM w_param,
                                                       LPARAM l_param) {
  DLOG(INFO) << "LegacyApplicationWindowHostHWND::OnWindowPosChanged: DISABLED as Activate() and Deactivate() not working";
...
LRESULT LegacyApplicationWindowHostHWND::OnScroll(UINT message,
                                             WPARAM w_param,
                                             LPARAM l_param) {
  LRESULT ret = 0;
  if (GetWindowEventTarget(GetParent())) {
...
LRESULT LegacyApplicationWindowHostHWND::OnNCHitTest(UINT message,
                                                WPARAM w_param,
                                                LPARAM l_param) {
  if (GetWindowEventTarget(GetParent())) {
    bool msg_handled = false;
...
LRESULT LegacyApplicationWindowHostHWND::OnNCPaint(UINT message,
                                              WPARAM w_param,
                                              LPARAM l_param) {
  return 0;
}

LRESULT LegacyApplicationWindowHostHWND::OnPaint(UINT message,
                                            WPARAM w_param,
                                            LPARAM l_param) {
  PAINTSTRUCT ps = {0};
  ::BeginPaint(hwnd(), &ps);
...
LRESULT LegacyApplicationWindowHostHWND::OnSetCursor(UINT message,
                                                WPARAM w_param,
                                                LPARAM l_param) {
  return 0;
}

LRESULT LegacyApplicationWindowHostHWND::OnNCCalcSize(UINT message,
                                                 WPARAM w_param,
                                                 LPARAM l_param) {
...
LRESULT LegacyApplicationWindowHostHWND::OnSize(UINT message,
                                           WPARAM w_param,
                                           LPARAM l_param) {
...
LRESULT LegacyApplicationWindowHostHWND::OnDestroy(UINT message,
                                              WPARAM w_param,
                                              LPARAM l_param) {
  //if (::switches::IsExperimentalAccessibilityPlatformUIAEnabled()) {
...
LRESULT LegacyApplicationWindowHostHWND::OnPointerHitTest(UINT message,
                                                     WPARAM w_param,
                                                     LPARAM l_param) {
  if (!direct_manipulation_helper_)
    return 0;
...
// LegacyApplicationWindowHostHWND::GetChildOfAXFragmentRoot() {
//   return GetOrCreateBrowserAccessibilityRoot();
// }

// gfx::NativeViewAccessible
// LegacyApplicationWindowHostHWND::GetParentOfAXFragmentRoot() {
//   if (host_)
//     return host_->GetParentNativeViewAccessible();
//   return nullptr;
// }

// bool LegacyApplicationWindowHostHWND::IsAXFragmentRootAControlElement() {
//   // Treat LegacyApplicationWindowHostHWND as a non-control element so that clients
//   // don't read out "Chrome Legacy Window" for it.
//   return false;
// }

// gfx::NativeViewAccessible
// LegacyApplicationWindowHostHWND::GetOrCreateWindowRootAccessible() {
//   if (::switches::IsExperimentalAccessibilityPlatformUIAEnabled()) {
...
// gfx::NativeViewAccessible
// LegacyApplicationWindowHostHWND::GetOrCreateBrowserAccessibilityRoot() {
//   //if (!host_)
//     return nullptr;
