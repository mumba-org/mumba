...
#include "core/host/render_frame_metadata_provider.h"
#include "core/host/application_window_host_iterator.h"
#include "core/host/application_window_host_observer.h"
...
#include "storage/host/fileapi/isolated_context.h"
...
// Implements the ApplicationWindowHostIterator interface. It keeps a list of
// ApplicationWindowHosts, and makes sure it returns a live ApplicationWindowHost at each
...
ApplicationWindowHost::ApplicationWindowHost(ApplicationWindowHostDelegate* delegate,
  ...
  if (!command_line->HasSwitch(switches::kDisableHangMonitor)) {
    hang_monitor_timeout_.reset(new TimeoutMonitor(
        base::Bind(&ApplicationWindowHost::RendererIsUnresponsive,
...
// static
ApplicationWindowHost* ApplicationWindowHost::FromID(
    int32_t process_id,
    int32_t routing_id) {
  return ApplicationWindowHost::FromID(process_id, routing_id);
}
...
// static
std::unique_ptr<ApplicationWindowHostIterator>
...
// static
std::unique_ptr<ApplicationWindowHostIterator>
ApplicationWindowHost::GetAllApplicationWindowHosts() {
  std::unique_ptr<ApplicationWindowHostIteratorImpl> hosts(
      new ApplicationWindowHostIteratorImpl());
  for (auto& it : g_routing_id_widget_map.Get())
    hosts->Add(it.second);
...
// static
ApplicationWindowHost* ApplicationWindowHost::From(ApplicationWindowHost* rwh) {
  return static_cast<ApplicationWindowHost*>(rwh);
}
...
RenderProcessHost* ApplicationWindowHost::GetProcess() const {
  return process_;
}
...
void ApplicationWindowHost::InitForFrame() {
  DCHECK(process_->HasConnection());
  renderer_initialized_ = true;
...
void ApplicationWindowHost::ShutdownAndDestroyWidget(bool also_delete) {
  CancelKeyboardLock();
...
bool ApplicationWindowHost::OnMessageReceived(const IPC::Message &msg) {
  ...
  bool handled = true;
  IPC_BEGIN_MESSAGE_MAP(ApplicationWindowHost, msg)
...
bool ApplicationWindowHost::Send(IPC::Message* msg) {
...
void ApplicationWindowHost::WasHidden() {
  ...
  TRACE_EVENT0("renderer_host", "ApplicationWindowHost::WasHidden");
  is_hidden_ = true;
...
void ApplicationWindowHost::WasShown(const ui::LatencyInfo& latency_info) {
  ...
  TRACE_EVENT0("renderer_host", "ApplicationWindowHost::WasShown");
  is_hidden_ = false;
...
#if defined(OS_ANDROID)
void ApplicationWindowHost::SetImportance(ChildProcessImportance importance) {
...
void ApplicationWindowHost::SetInitialRenderSizeParams(
...
#if defined(OS_MACOSX)
void ApplicationWindowHost::PauseForPendingResizeOrRepaints() {
  TRACE_EVENT0("browser",
      "ApplicationWindowHost::PauseForPendingResizeOrRepaints");
  ...
  TRACE_EVENT0("renderer_host", "ApplicationWindowHost::WaitForSurface");
...
bool ApplicationWindowHost::ScheduleComposite() {
...
void ApplicationWindowHost::DidNavigate(uint32_t next_source_id) {
...
void ApplicationWindowHost::ForwardMouseEvent(const WebMouseEvent& mouse_event) {
  ...
  Forwardcommon::MouseEventWithLatencyInfo(mouse_event,
                                   ui::LatencyInfo(ui::SourceEventType::MOUSE));
  if (owner_delegate_)
    owner_delegate_->RenderWidgetDidForwardMouseEvent(mouse_event);
}

void ApplicationWindowHost::Forwardcommon::MouseEventWithLatencyInfo(
    const blink::WebMouseEvent& mouse_event,
    const ui::LatencyInfo& latency) {
  TRACE_EVENT2("input", "ApplicationWindowHost::ForwardMouseEvent", "x",
...
void ApplicationWindowHost::ForwardWheelEvent(
...
void ApplicationWindowHost::ForwardWheelEventWithLatencyInfo(
    ...
    const ui::LatencyInfo& latency) {
  TRACE_EVENT2("input", "ApplicationWindowHost::ForwardWheelEvent", "dx",
...
void ApplicationWindowHost::ForwardGestureEvent(
    const blink::WebGestureEvent& gesture_event) {
  Forwardcommon::GestureEventWithLatencyInfo(
      gesture_event,
      ui::WebInputEventTraits::CreateLatencyInfoForWebGestureEvent(
          gesture_event));
}

void ApplicationWindowHost::Forwardcommon::GestureEventWithLatencyInfo(
    const blink::WebGestureEvent& gesture_event,
    const ui::LatencyInfo& latency) {
...
void ApplicationWindowHost::Forwardcommon::TouchEventWithLatencyInfo(
    const blink::WebTouchEvent& touch_event,
    const ui::LatencyInfo& latency) {
...
void ApplicationWindowHost::ForwardKeyboardEvent(
...
void ApplicationWindowHost::QueueSyntheticGesture(
...
void ApplicationWindowHost::SetCursor(const WebCursor& cursor) {
  if (!view_)
    return;
...
// static
void ApplicationWindowHost::DisableResizeAckCheckForTesting() {
  g_check_for_pending_resize_ack = false;
}

void ApplicationWindowHost::AddKeyPressEventCallback(
    const KeyPressEventCallback& callback) {
  key_press_event_callbacks_.push_back(callback);
}

void ApplicationWindowHost::RemoveKeyPressEventCallback(
    const KeyPressEventCallback& callback) {
...
void ApplicationWindowHost::AddMouseEventCallback(
    const MouseEventCallback& callback) {
  mouse_event_callbacks_.push_back(callback);
}

void ApplicationWindowHost::RemoveMouseEventCallback(
    const MouseEventCallback& callback) {
  for (size_t i = 0; i < mouse_event_callbacks_.size(); ++i) {
...
void ApplicationWindowHost::AddInputEventObserver(
    ApplicationWindowHost::InputEventObserver* observer) {
  if (!input_event_observers_.HasObserver(observer))
    input_event_observers_.AddObserver(observer);
}

void ApplicationWindowHost::RemoveInputEventObserver(
    ApplicationWindowHost::InputEventObserver* observer) {
...
void ApplicationWindowHost::GetScreenInfo(ScreenInfo* result) {
  TRACE_EVENT0("renderer_host", "ApplicationWindowHost::GetScreenInfo");
  if (view_)
...
void ApplicationWindowHost::DragTargetDrop(const DropData& drop_data,
                                          const gfx::PointF& client_pt,
                                          const gfx::PointF& screen_pt,
                                          int key_modifiers) {
...
void ApplicationWindowHost::FilterDropData(DropData* drop_data) {
#if DCHECK_IS_ON()
...
void ApplicationWindowHost::SetCursor(const CursorInfo& cursor_info) {
  WebCursor cursor;
  cursor.InitFromCursorInfo(cursor_info);
  SetCursor(cursor);
}

RenderProcessHost::Priority ApplicationWindowHost::GetPriority() {
...
mojom::WidgetInputHandler* ApplicationWindowHost::GetWidgetInputHandler() {
...
void ApplicationWindowHost::GetSnapshotFromBrowser(
...
void ApplicationWindowHost::SelectionChanged(const base::string16& text,
                                            uint32_t offset,
                                            const gfx::Range& range) {
  if (view_)
...
void ApplicationWindowHost::OnSelectionBoundsChanged(
    const ViewHostMsg_SelectionBounds_Params& params) {
  if (view_)
    view_->SelectionBoundsChanged(params);
}

void ApplicationWindowHost::OnFocusedNodeTouched(bool editable) {
  if (delegate_)
    delegate_->FocusedNodeTouched(editable);
}

void ApplicationWindowHost::OnStartDragging(
    const DropData& drop_data,
...
void ApplicationWindowHost::OnUpdateDragCursor(WebDragOperation current_op) {
...
void ApplicationWindowHost::RendererExited(base::TerminationStatus status,
                                          int exit_code) {
...
void ApplicationWindowHost::UpdateTextDirection(WebTextDirection direction) {
...
void ApplicationWindowHost::RendererIsUnresponsive() {
  NotificationService::current()->Notify(
  ...
  // ApplicationWindowHost in RendererUnresponsive.
}

void ApplicationWindowHost::RendererIsResponsive() {
  if (is_unresponsive_) {
    is_unresponsive_ = false;
    if (delegate_)
...
void ApplicationWindowHost::OnRenderProcessGone(int status, int exit_code) {
  // RenderFrameHost owns a ApplicationWindowHost when it needs one, in which case
...
void ApplicationWindowHost::OnHittestData(
    const FrameHostMsg_HittestData_Params& params) {
  if (delegate_)
    delegate_->GetInputEventRouter()->OnHittestData(params);
}

void ApplicationWindowHost::OnClose() {
  ShutdownAndDestroyWidget(true);
}

void ApplicationWindowHost::OnSetTooltipText(
...
void ApplicationWindowHost::OnUpdateScreenRectsAck() {
  waiting_for_screen_rects_ack_ = false;
  if (!view_)
    return;
...
void ApplicationWindowHost::OnRequestMove(const gfx::Rect& pos) {
  if (view_) {
    view_->SetBounds(pos);
...
void ApplicationWindowHost::OnResizeOrRepaintACK(
    const ViewHostMsg_ResizeOrRepaint_ACK_Params& params) {
  TRACE_EVENT0("renderer_host", "ApplicationWindowHost::OnResizeOrRepaintACK");
...
void ApplicationWindowHost::OnSetCursor(const WebCursor& cursor) {
  SetCursor(cursor);
}

void ApplicationWindowHost::OnAutoscrollStart(const gfx::PointF& position) {
...
void ApplicationWindowHost::OnAutoscrollFling(const gfx::Vector2dF& velocity) {
...
void ApplicationWindowHost::OnAutoscrollEnd() {
...
TouchEmulator* ApplicationWindowHost::GetTouchEmulator() {
  if (!touch_emulator_) {
    touch_emulator_.reset(
...
void ApplicationWindowHost::OnTextInputStateChanged(
...
void ApplicationWindowHost::OnLockMouse(bool user_gesture,
                                       bool privileged) {
  if (pending_mouse_lock_request_) {
    Send(new ViewMsg_LockMouse_ACK(routing_id_, false));
    return;
...
void ApplicationWindowHost::OnUnlockMouse() {
...
void ApplicationWindowHost::RequestKeyboardLock(
...
void ApplicationWindowHost::OnShowDisambiguationPopup(
...
void ApplicationWindowHost::DecrementInFlightEventCount(
...
void ApplicationWindowHost::DidOverscroll(
...
void ApplicationWindowHost::OnKeyboardEventAck(
  ...
  if (delegate_ && !processed && !is_hidden() && !event.event.skip_in_browser) {
    ...
    // WARNING: This ApplicationWindowHost can be deallocated at this point
    // (i.e.  in the case of Ctrl+W, where the call to
    // HandleKeyboardEvent destroys this ApplicationWindowHost).
  }
}

void ApplicationWindowHost::OnMouseEventAck(
...
void ApplicationWindowHost::OnWheelEventAck(
...
void ApplicationWindowHost::OnGestureEventAck(
...
void ApplicationWindowHost::OnTouchEventAck(
...
void ApplicationWindowHost::DidReceiveRendererFrame() {
...
void ApplicationWindowHost::OnGpuSwapBuffersCompleted(
  ...
  for (size_t i = 0; i < latency_info.size(); i++) {
    std::set<ApplicationWindowHost*> rwhi_set;
...
BrowserAccessibilityManager*
    ApplicationWindowHost::GetRootAccessibilityManager() {
  return delegate_ ? delegate_->GetRootAccessibilityManager() : nullptr;
}

BrowserAccessibilityManager*
    ApplicationWindowHost::GetOrCreateRootAccessibilityManager() {
  return delegate_ ? delegate_->GetOrCreateRootAccessibilityManager()
                   : nullptr;
}

void ApplicationWindowHost::GrantFileAccessFromDropData(DropData* drop_data) {
...
bool ApplicationWindowHost::HasGestureStopped() {
...
#if defined(OS_MACOSX)
device::mojom::WakeLock* ApplicationWindowHost::GetWakeLock() {
...
void ApplicationWindowHost::SetForceEnableZoom(bool enabled) {
  force_enable_zoom_ = enabled;
  input_router_->SetForceEnableZoom(enabled);
}

void ApplicationWindowHost::SetWidgetInputHandler(
...
void ApplicationWindowHost::SetWidget(mojom::WidgetPtr widget) {
  if (widget) {
...
void ApplicationWindowHost::ForceFirstFrameAfterNavigationTimeout() {
  DLOG(INFO) << "\n\n** ApplicationWindowHost::ForceFirstFrameAfterNavigationTimeout **";
  if (did_receive_first_frame_after_navigation_ ||
      !new_content_rendering_timeout_) { 
    DLOG(INFO) << "ApplicationWindowHost::ForceFirstFrameAfterNavigationTimeout: did_receive_first_frame_after_navigation_ ||
      !new_content_rendering_timeout_ -> cancelling";
    return;
  }
  new_content_rendering_timeout_->Stop();
...
void ApplicationWindowHost::SetScreenOrientationForTesting(
    uint16_t angle,
