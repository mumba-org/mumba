...
#ifndef MUMBA_HOST_APPLICATION_RENDER_WIDGET_HOST_VIEW_COCOA_H_
#define MUMBA_HOST_APPLICATION_RENDER_WIDGET_HOST_VIEW_COCOA_H_
...
namespace host {
class ApplicationWindowHostNSViewClient;
class ApplicationWindowHostViewMac;
class ApplicationWindowHostViewMacEditCommandHelper;
}
...
@protocol ApplicationWindowHostNSViewClientOwner
- (content::ApplicationWindowHostNSViewClient*)renderWidgetHostNSViewClient;
@end

// This is the view that lives in the Cocoa view hierarchy. In Windows-land,
// ApplicationWindowHostViewWin is both the view and the delegate. We split the roles
// but that means that the view needs to own the delegate and will dispose of it
// when it's removed from the view system.
// TODO(ccameron): Hide this interface behind ApplicationWindowHostNSViewBridge.
@interface ApplicationWindowHostViewCocoa
    : ToolTipBaseView<CommandDispatcherTarget,
                      ApplicationWindowHostNSViewClientOwner,
                      NSTextInputClient> {
 @private
  // The communications channel to the ApplicationWindowHostViewMac. This pointer is
  ...
  // a nullptr check.
  content::ApplicationWindowHostNSViewClient* client_;

  // Dummy client that is always valid (see above).
  std::unique_ptr<content::ApplicationWindowHostNSViewClient> noopClient_;

  // This ivar is the cocoa delegate of the NSResponder.
  base::scoped_nsobject<NSObject<ApplicationWindowHostViewMacDelegate>>
      responderDelegate_;
  BOOL canBeKeyView_;
  BOOL closeOnDeactivate_;
  std::unique_ptr<content::ApplicationWindowHostViewMacEditCommandHelper>
      editCommandHelper_;
  ...
  // The text selection, cached from the ApplicationWindowHostView. This is only ever
  // updated from the renderer.
  base::string16 textSelectionText_;
  size_t textSelectionOffset_;
  gfx::Range textSelectionRange_;

  // The composition range, cached from the ApplicationWindowHostView. This is only
...
// it to the ApplicationWindowHostNSViewClient (only if the screen is non-nil).
...
- (id)initWithClient:(content::ApplicationWindowHostNSViewClient*)client;
...
#endif  // MUMBA_HOST_APPLICATION_RENDER_WIDGET_HOST_VIEW_COCOA_H_
