...
class CONTENT_EXPORT LegacyApplicationWindowHostHWND
    : public ATL::CWindowImpl<LegacyApplicationWindowHostHWND,
                              ATL::CWindow,
                              ATL::CWinTraits<WS_CHILD>> {//,
      //public ui::AXFragmentRootDelegateWin {
 public:
  DECLARE_WND_CLASS_EX(L"Mumba_ApplicationWindowHostHWND", CS_DBLCLKS, 0)

  typedef ATL::CWindowImpl<LegacyApplicationWindowHostHWND,
                           ATL::CWindow,
                           ATL::CWinTraits<WS_CHILD>>
      Base;

  // Creates and returns an instance of the LegacyApplicationWindowHostHWND class on
  // successful creation of a child window parented to the parent window passed
  // in.
  static LegacyApplicationWindowHostHWND* Create(HWND parent);

  // Destroys the HWND managed by this class.
  void Destroy();

  BEGIN_MSG_MAP_EX(LegacyApplicationWindowHostHWND)
  ...
  // The pointer to the containing ApplicationWindowHostViewAura instance is passed
  // here.
  void set_host(ApplicationWindowHostViewAura* host) {
    host_ = host;
  }
 ...
 private:
  ...
  explicit LegacyApplicationWindowHostHWND(HWND parent);
  ~LegacyApplicationWindowHostHWND() override;
  ...
  ApplicationWindowHostViewAura* host_;
  ...
  DISALLOW_COPY_AND_ASSIGN(LegacyApplicationWindowHostHWND);
};
