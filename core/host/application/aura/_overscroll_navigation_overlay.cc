...
#include "core/host/application/aura/overscroll_window_delegate.h"
...
OverscrollNavigationOverlay::OverscrollNavigationOverlay(
    ...
    aura::Window* app_contents_window)
    : direction_(NavigationDirection::NONE),
      ...
      app_contents_window_(app_contents_window) {
}
...
std::unique_ptr<aura::Window> OverscrollNavigationOverlay::CreateOverlayWindow(
  ...
  app_contents_window_->AddChild(window.get());
  aura::Window* event_window = GetMainWindow();
  if (direction_ == NavigationDirection::FORWARD)
    app_contents_window_->StackChildAbove(window.get(), event_window);
  else
    app_contents_window_->StackChildBelow(window.get(), event_window);
...
const gfx::Image OverscrollNavigationOverlay::GetImageForDirection(
  ...
  const NavigationControllerImpl& controller = app_contents_->GetController();
...
std::unique_ptr<aura::Window> OverscrollNavigationOverlay::CreateFrontWindow(
    const gfx::Rect& bounds) {
  if (!app_contents_->GetController().CanGoForward())
    return nullptr;
...
std::unique_ptr<aura::Window> OverscrollNavigationOverlay::CreateBackWindow(
    const gfx::Rect& bounds) {
  if (!app_contents_->GetController().CanGoBack())
    return nullptr;
...
aura::Window* OverscrollNavigationOverlay::GetMainWindow() const {
  if (window_)
    return window_.get();
  return app_contents_->IsBeingDestroyed()
             ? nullptr
             : app_contents_->GetContentNativeView();
}
...
void OverscrollNavigationOverlay::OnOverscrollCompleted(
  ...
  window_->SetBounds(gfx::Rect(app_contents_window_->bounds().size()));
  window_->SetTransform(gfx::Transform());
  // Make sure the overlay window is on top.
  app_contents_window_->StackChildAtTop(window_.get());
  ...
  if (direction_ == NavigationDirection::FORWARD &&
      app_contents_->GetController().CanGoForward()) {
    app_contents_->GetController().GoForward();
    navigated = true;
  } else if (direction_ == NavigationDirection::BACK &&
      app_contents_->GetController().CanGoBack()) {
    app_contents_->GetController().GoBack();
    navigated = true;
  } else {
...
void OverscrollNavigationOverlay::DidFirstVisuallyNonEmptyPaint() {
  NavigationEntry* visible_entry =
      app_contents_->GetController().GetVisibleEntry();
