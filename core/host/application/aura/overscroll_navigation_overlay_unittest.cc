...
#include "core/host/web_contents/aura/overscroll_navigation_overlay.h"
...
#include "core/host/web_contents/aura/types.h"
#include "core/host/web_contents/web_contents_view.h"
...
class OverscrollTestWebContents : public TestWebContents {
 public:
  ...
  static OverscrollTestWebContents* Create(
    ...
    OverscrollTestWebContents* web_contents = new OverscrollTestWebContents(
        application_contents, std::move(fake_native_view),
        std::move(fake_contents_window));
    web_contents->Init(
        WebContents::CreateParams(application_contents, std::move(instance)));
    return web_contents;
  }
 ...
 protected:
  explicit OverscrollTestWebContents(
      ...
      std::unique_ptr<aura::Window> fake_native_view,
      std::unique_ptr<aura::Window> fake_contents_window)
      : TestWebContents(application_contents),
 ...
 protected:
  ...
  void SetUp() override {
    ...
    SetContents(OverscrollTestWebContents::Create(
        application_contents(), SiteInstance::Create(application_contents()),
    ...
    // correctly in ApplicationWindowHost.
