...
bool ShouldUpdateTextInputState(const common::TextInputState& old_state,
                                 const common::TextInputState& new_state) {
#if defined(USE_AURA)
...
TextInputManager::~TextInputManager() {
  ...
  std::vector<ApplicationWindowHostView*> views;
...
ApplicationWindowHost* TextInputManager::GetActiveWidget() const {
  return !!active_view_ ? active_view_->GetApplicationWindowHost()
                        : nullptr;
}

const common::TextInputState* TextInputManager::GetTextInputState() const {
...
const TextInputManager::SelectionRegion* TextInputManager::GetSelectionRegion(
    ApplicationWindowHostView* view) const {
...
void TextInputManager::UpdateTextInputState(
  ...
  if (text_input_state.type != ui::TEXT_INPUT_TYPE_NONE &&
    ...
    if (active_view_) {
      ...
      ApplicationWindowHostView* active_view = active_view_;
...
void TextInputManager::ImeCancelComposition(ApplicationWindowHostView* view) {
  DCHECK(IsRegistered(view));
...
void TextInputManager::SelectionBoundsChanged(
  ...
  if (params->anchor_rect == params->focus_rect) {
    ...
    bool anchor_LTR = params->anchor_dir == base::i18n::LEFT_TO_RIGHT;//blink::kWebTextDirectionLeftToRight;
    // Whether text is LTR at the focus handle.
    bool focus_LTR = params->focus_dir == base::i18n::LEFT_TO_RIGHT;//blink::kWebTextDirectionLeftToRight;
...
void TextInputManager::SelectionChanged(ApplicationWindowHostView* view,
...
void TextInputManager::Register(ApplicationWindowHostView* view) {
  DCHECK(!IsRegistered(view));

  text_input_state_map_[view] = common::TextInputState();
...
void TextInputManager::Unregister(ApplicationWindowHostView* view) {
  DCHECK(IsRegistered(view));
...
bool TextInputManager::IsRegistered(ApplicationWindowHostView* view) const {
