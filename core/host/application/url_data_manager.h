...
class CONTENT_EXPORT URLDataManager : public base::SupportsUserData::Data {
 public:
  explicit URLDataManager(Domain* domain);
  ...
  // Convenience wrapper function to add |source| to |application_contents|'s
  // |URLDataManager|. Creates a URLDataSourceImpl to wrap the given
  // source.
  static void AddDataSource(Domain* domain,
                            URLDataSource* source);

  // Adds a WebUI data source to |application_contents|'s |URLDataManager|.
  static void AddAppDataSource(Domain* domain,
                               RpcDataSource* source);

  static bool HaveDataSourceForURL(Domain* domain, const GURL& url);

  // Updates an existing WebUI data source.
  static void UpdateAppDataSource(
    Domain* domain,
