...
void ApplicationWindowHostViewChildFrame::SetFrameSinkId(
  ...
  DLOG(INFO) << "ApplicationWindowHostViewChildFrame::SetFrameSinkId";
  if (base::FeatureList::IsEnabled(features::kMash))
    frame_sink_id_ = frame_sink_id;
}
#endif  // defined(USE_AURA)

void ApplicationWindowHostViewChildFrame::UpdateBackgroundColor() {
...
//void ApplicationWindowHostViewChildFrame::UpdateBackgroundColorFromRenderer(
//    SkColor color) {
...
gfx::Rect ApplicationWindowHostViewChildFrame::GetViewBounds() const {
  ...
  if (frame_connector_) {
    ...
    // The parent_view can be null in tests when using a TestApplicationContents.
    if (parent_view) {
    ...
    // the size of the renderer content and where to url context menus and so
    // on. We want the location of the frame in screen coordinates to url
...
gfx::Size ApplicationWindowHostViewChildFrame::GetVisibleViewportSize() const {
  ...
  bool is_guest = false;//BrowserPluginGuest::IsGuest(RenderViewHostImpl::From(host()));
  if (frame_connector_ && !is_guest) {
    ...
    ApplicationWindowHostView* parent_view =
        frame_connector_->GetParentApplicationWindowHostView();
    // The parent_view can be null in unit tests when using a TestApplicationContents.
    if (parent_view)
...
void ApplicationWindowHostViewChildFrame::UpdateCursor(const common::WebCursor& cursor) {
  if (frame_connector_)
    frame_connector_->UpdateCursor(cursor);
}

void ApplicationWindowHostViewChildFrame::SetIsLoading(bool is_loading) {
  // It is valid for an inner ApplicationContents's SetIsLoading() to end up here.
  // This is because an inner ApplicationContents's main frame's ApplicationWindowHostView
  // is a ApplicationWindowHostViewChildFrame. In contrast, when there is no
  // inner/outer ApplicationContents, only subframe's ApplicationWindowHostView can be a
  // ApplicationWindowHostViewChildFrame which do not get a SetIsLoading() call.
  //if (GuestMode::IsCrossProcessFrameGuest(
  //        ApplicationContents::FromRenderViewHost(RenderViewHost::From(host()))))
  //  return;

  NOTREACHED();
}

void ApplicationWindowHostViewChildFrame::ApplicationProcessGone(
    base::TerminationStatus status,
    int error_code) {
  if (frame_connector_)
    frame_connector_->ApplicationProcessGone();
  Destroy();
}
...
void ApplicationWindowHostViewChildFrame::UpdateViewportIntersection(
  ...
  if (host()) {
    host()->UpdateViewportIntersection(viewport_intersection, compositor_visible_rect);
  }
}

void ApplicationWindowHostViewChildFrame::SetIsInert() {
  if (host() && frame_connector_) {
    host()->SetIsInert(frame_connector_->IsInert());
  }
}

void ApplicationWindowHostViewChildFrame::UpdateInheritedEffectiveTouchAction() {
  //if (host_ && frame_connector_) {
  //  HostThread::PostTask(
  //    HostThread::IO, 
 //     FROM_HERE,
  //    base::BindOnce(&common::mojom::ApplicationWindow::SetInheritedEffectiveTouchAction,
  //                   base::Unretained(host()->GetApplicationWindowInterface()), 
                     //host()->GetRoutingID(), 
...
void ApplicationWindowHostViewChildFrame::UpdateRenderThrottlingStatus() {
  if (host() && frame_connector_) {
    host()->UpdateRenderThrottlingStatus(
...
void ApplicationWindowHostViewChildFrame::GestureEventAck(
  ...
  bool should_bubble =
      ack_result == common::INPUT_EVENT_ACK_STATE_NOT_CONSUMED ||
      ack_result == common::INPUT_EVENT_ACK_STATE_NO_CONSUMER_EXISTS ||
      ack_result == common::INPUT_EVENT_ACK_STATE_CONSUMED_SHOULD_BUBBLE;
...
void ApplicationWindowHostViewChildFrame::DidCreateNewApplicationCompositorFrameSink(
...
gfx::Rect ApplicationWindowHostViewChildFrame::GetBoundsInRootWindow() {
  ...
  if (frame_connector_) {
    ...
    // The root_view can be null in tests when using a TestApplicationContents.
    if (root_view)
...
viz::FrameSinkId ApplicationWindowHostViewChildFrame::GetRootFrameSinkId() {
  if (frame_connector_) {
    ApplicationWindowHostView* root_view =
        frame_connector_->GetRootApplicationWindowHostView();
...
// ApplicationWindowHostViewChildFrame::CreateBrowserAccessibilityManager(
...
void ApplicationWindowHostViewChildFrame::GetScreenInfo(
    common::ScreenInfo* screen_info) const {
  if (frame_connector_)
...
ApplicationWindowHostViewChildFrame::DidUpdateVisualProperties(
    const cc::RenderFrameMetadata& metadata) {
  base::OnceCallback<void()> allocation_task = base::BindOnce(
      base::IgnoreResult(
          &ApplicationWindowHostViewChildFrame::OnDidUpdateVisualPropertiesComplete),
...
void ApplicationWindowHostViewChildFrame::OnDidUpdateVisualPropertiesComplete(
    const cc::RenderFrameMetadata& metadata) {
  if (frame_connector_)
