...
#ifndef MUMBA_HOST_APPLICATION_APPLICATION_WINDOW_HOST_VIEW_EVENT_HANDLER_H_
#define MUMBA_HOST_APPLICATION_APPLICATION_WINDOW_HOST_VIEW_EVENT_HANDLER_H_
...
// ApplicationWindowHostView. A delegate is required in order to provide platform
...
// ApplicationWindowHostView classes do.
// ApplicationWindowHostViewEventHandler::Delegate implementations may have
// overlapping functionality with the ui::TextInputClient.
class CONTENT_EXPORT ApplicationWindowHostViewEventHandler : public ui::EventHandler {
 public:
  ...
  class Delegate {
   public:
    ...
    virtual void ShowContextMenu(const common::ContextMenuParams& params) = 0;
  ...
  ApplicationWindowHostViewEventHandler(ApplicationWindowHost* host,
                                        ApplicationWindowHostView* host_view,
                                        Delegate* delegate);
  ~ApplicationWindowHostViewEventHandler() override;

  // Set child popup's host view, and event handler, in order to redirect input.
  void SetPopupChild(ApplicationWindowHostView* popup_child_host_view,
                     ui::EventHandler* popup_child_event_handler);
  // Begin tracking a host window, such as when ApplicationWindowHostView is
  // fullscreen.
  ...
  void SetContextMenuParams(const common::ContextMenuParams& params);
 ...
 private:
  ...
  FRIEND_TEST_ALL_PREFIXES(
      ApplicationWindowHostViewAuraTest,
  ...
  bool CanApplicationHandleEvent(const ui::MouseEvent* event,
                              bool mouse_locked,
  ...
  // Whether return characters should be passed on to the ApplicationWindowHost.
  ...
  ApplicationWindowHost* const host_;
  // Should create |this| and own it.
  ApplicationWindowHostView* const host_view_;
  // Optional, used to redirect events to a popup and associated handler.
  ApplicationWindowHostView* popup_child_host_view_;
  ui::EventHandler* popup_child_event_handler_;
  Delegate* const delegate_;
  aura::Window* window_;
  MouseWheelPhaseHandler mouse_wheel_phase_handler_;

  DISALLOW_COPY_AND_ASSIGN(ApplicationWindowHostViewEventHandler);
};

}  // namespace host

#endif  // MUMBA_HOST_APPLICATION_RENDER_WIDGET_HOST_VIEW_EVENT_HANDLER_H_
