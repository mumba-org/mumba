...
#import "core/host/accessibility/browser_accessibility_cocoa.h"
#import "core/host/accessibility/browser_accessibility_mac.h"
#include "core/host/accessibility/browser_accessibility_manager_mac.h"
#import "core/host/cocoa/system_hotkey_helper_mac.h"
#import "core/host/cocoa/system_hotkey_map.h"
...
#import "core/host/renderer_host/application_window_host_view_mac_editcommand_helper.h"
#import "core/host/application_window_host_view_mac_delegate.h"
...
using content::ApplicationWindowHostNSViewClient;
using content::ApplicationWindowHostViewMacEditCommandHelper;
...
// the client_ member of ApplicationWindowHostViewCocoa).
class NoopClient : public ApplicationWindowHostNSViewClient {
 public:
  NoopClient() {}
  ~NoopClient() override{};
...
// ApplicationWindowHostViewCocoa ---------------------------------------------------

// Private methods:
@interface ApplicationWindowHostViewCocoa () {
...
@implementation ApplicationWindowHostViewCocoa
@synthesize markedRange = markedRange_;

- (id)initWithClient:(ApplicationWindowHostNSViewClient*)client {
  self = [super initWithFrame:NSZeroRect];
  if (self) {
    self.acceptsTouchEvents = YES;
    editCommandHelper_.reset(new ApplicationWindowHostViewMacEditCommandHelper);
...
- (void)sendViewBoundsInWindowToClient {
  TRACE_EVENT0("browser",
               "ApplicationWindowHostViewCocoa::sendViewBoundsInWindowToClient");
  if (inSetFrame_)
    return;
...
- (void)sendWindowFrameInScreenToClient {
  TRACE_EVENT0("browser",
               "ApplicationWindowHostViewCocoa::sendWindowFrameInScreenToClient");
  NSWindow* enclosingWindow = [self window];
  if (!enclosingWindow)
    return;
...
- (void)setClientDisconnected {
  ...
  // |responderDelegate_| may attempt to access the ApplicationWindowHostViewMac
...
- (void)mouseEvent:(NSEvent*)theEvent {
  TRACE_EVENT0("browser", "ApplicationWindowHostViewCocoa::mouseEvent");
  if (responderDelegate_ &&
...
- (void)keyEvent:(NSEvent*)theEvent wasKeyEquivalent:(BOOL)equiv {
  TRACE_EVENT0("browser", "ApplicationWindowHostViewCocoa::keyEvent");
  ...
  // all event and Ime messages target the same ApplicationWindowHost throughout this
  // function call.
  ...
  base::scoped_nsobject<ApplicationWindowHostViewCocoa> keepSelfAlive([self retain]);
...
- (ApplicationWindowHostNSViewClient*)renderWidgetHostNSViewClient {
  return client_;
}
...
- (id)accessibilityAttributeValue:(NSString*)attribute {
  BrowserAccessibilityManager* manager =
      client_->GetRootAccessibilityManager();

  // Contents specifies document view of ApplicationWindowHostViewCocoa provided by
...
// ApplicationWindowHostViewMac is running in a browser process. On the other
...
//
// [ApplicationWindowHostViewMac keyEvent] (browser) ->
//     |Sync IPC (KeyDown)| (*1) ->
//     EventHandler::keyEvent() (renderer) ->
//     ...
//     EditorClientImpl::handleKeyboardEvent() (renderer) ->
//     |Sync IPC| (*2) ->
//     [ApplicationWindowHostViewMac _interceptEditingKeyEvent] (browser) ->
//     [self interpretKeyEvents] ->
//     [ApplicationWindowHostViewMac insertText] (browser) ->
//     |Async IPC| ->
...
// Each ApplicationWindowHostViewCocoa has its own input context, but we return
...
- (void)doCommandBySelector:(SEL)selector {
  ...
  std::string command(base::SysNSStringToUTF8(
      ApplicationWindowHostViewMacEditCommandHelper::CommandNameForSelector(
          selector)));
...
//
@implementation ApplicationWindowHostViewCocoa (NSServicesRequests)
