...
//#include "core/host/frame_host/interstitial_page_navigator_impl.h"
...
class InterstitialPageImpl::InterstitialPageRVHDelegateView
 ...
 public:
  ...
  // ApplicationWindowHostDelegateView implementation:
//#if BUILDFLAG(USE_EXTERNAL_POPUP_MENU)
  void ShowPopupMenu(ApplicationWindowHost* render_frame_host,
...
//#endif
  void StartDragging(const common::DropData& drop_data,
                     ...
                     ApplicationWindowHost* source_awh) override;
  void UpdateDragCursor(WebDragOperation operation) override;
  void GotFocus(ApplicationWindowHost* app_window_host) override;
  void LostFocus(ApplicationWindowHost* app_window_host) override;
...
typedef std::map<ApplicationContents*, InterstitialPageImpl*> InterstitialPageMap;
static InterstitialPageMap* g_app_contents_to_interstitial_page;

// Initializes g_app_contents_to_interstitial_page in a thread-safe manner.
// Should be called before accessing g_app_contents_to_interstitial_page.
static void InitInterstitialPageMap() {
  if (!g_app_contents_to_interstitial_page)
    g_app_contents_to_interstitial_page = new InterstitialPageMap;
}

InterstitialPage* InterstitialPage::Create(ApplicationContents* app_contents,
                                           bool new_navigation,
                                           const GURL& url,
                                           InterstitialPageDelegate* delegate) {
  return new InterstitialPageImpl(
      app_contents,
      static_cast<ApplicationWindowHostDelegate*>(
          static_cast<ApplicationContents*>(app_contents)),
...
InterstitialPage* InterstitialPage::GetInterstitialPage(
  ...
  InterstitialPageMap::const_iterator iter =
      g_app_contents_to_interstitial_page->find(app_contents);
  if (iter == g_app_contents_to_interstitial_page->end())
    return nullptr;

  return iter->second;
}

InterstitialPage* InterstitialPage::FromApplicationWindowHost(ApplicationWindowHost* rfh) {
  //if (!rfh)
    return nullptr;
  //return rfh->delegate()->GetAsInterstitialPage();
}

InterstitialPageImpl::InterstitialPageImpl(
    ApplicationContents* app_contents,
    ApplicationWindowHostDelegate* application_window_host_delegate,
    bool new_navigation,
    const GURL& url,
    InterstitialPageDelegate* delegate)
    : underlying_content_observer_(app_contents, this),
      app_contents_(app_contents),
      //controller_(static_cast<NavigationControllerImpl*>(
      //    &app_contents->GetController())),
      application_window_host_delegate_(application_window_host_delegate),
      url_(url),
      ...
      // the ApplicationContents one, so we can enforce no navigation policy here.
      ...
      //    this,
       //   this,
       //   this,
      //    static_cast<ApplicationContentsImpl*>(app_contents))),
      original_child_id_(
          app_contents->GetApplicationWindowHost()->GetProcess()->GetID()),
      original_rvh_id_(app_contents->GetApplicationWindowHost()->GetRoutingID()),
      //should_revert_app_contents_title_(false),
...
InterstitialPageImpl::~InterstitialPageImpl() {
  // ApplicationWindowHost::RenderWidgetLostFocus() will be eventually executed in
...
void InterstitialPageImpl::Show() {
  ...
  InterstitialPageMap::const_iterator iter =
      g_app_contents_to_interstitial_page->find(app_contents_);
  if (iter != g_app_contents_to_interstitial_page->end()) {
  ...
  // We need to be notified when the ApplicationWindowHost is destroyed so we can
  // cancel the blocked requests.  We cannot do that on
  // NOTIFY_WEB_CONTENTS_DESTROYED as at that point the ApplicationWindowHost has
  // already been destroyed.
  //widget_observer_.Add(
  //    controller_->delegate()->GetApplicationWindowHost());
  widget_observer_.Add(
    app_contents_->GetApplicationWindowHost());

  // Update the g_app_contents_to_interstitial_page map.
  iter = g_app_contents_to_interstitial_page->find(app_contents_);
  DCHECK(iter == g_app_contents_to_interstitial_page->end());
  (*g_app_contents_to_interstitial_page)[app_contents_] = this;
  ...
  if (new_navigation_) {
    ...
    //static_cast<ApplicationContentsImpl*>(app_contents_)
        app_contents_->DidChangeVisibleSecurityState();
  }

  DCHECK(!application_window_host_);
  application_window_host_ = CreateApplicationWindowHost();
  CreateApplicationContentsView();
...
void InterstitialPageImpl::Hide() {
  ...
  //ApplicationWindowHostView* old_view =
  //    controller_->delegate()->GetApplicationWindowHost()->GetWindow();
  ApplicationWindowHostView* old_view =
      app_contents_->GetApplicationWindowHost()->GetView();
          
  if (app_contents_->GetInterstitialPage() == this &&
      old_view &&
      !old_view->IsShowing() &&
      !app_contents_->IsHidden()) {
  ...
  bool has_focus = application_window_host_ &&
                   application_window_host_->GetView()->HasFocus();
  application_window_host_ = nullptr;
  //frame_tree_->root()->ResetForNewProcess();
  //controller_->delegate()->DetachInterstitialPage(has_focus);
  app_contents_->DetachInterstitialPage(has_focus);
  // Let's revert to the original title if necessary.
  //NavigationEntry* entry = controller_->GetVisibleEntry();
  //if (entry && !new_navigation_ && should_revert_app_contents_title_)
  //  app_contents_->UpdateTitleForEntry(entry, original_app_contents_title_);
  ...
  DCHECK(iter != g_app_contents_to_interstitial_page->end());
  if (iter != g_app_contents_to_interstitial_page->end())
    g_app_contents_to_interstitial_page->erase(iter);

  // Clear the ApplicationContents pointer, because it may now be deleted.
  // This signifies that we are in the process of shutting down.
  app_contents_ = nullptr;
}

void InterstitialPageImpl::ApplicationWindowHostDestroyed(
    ApplicationWindowHost* widget_host) {
  widget_observer_.Remove(widget_host);
  if (action_taken_ == NO_ACTION) {
    // The ApplicationWindowHost is being destroyed (as part of the tab being
    // closed); make sure we clear the blocked requests.
    DCHECK(widget_host->GetProcess()->GetID() == original_child_id_ &&
           widget_host->GetRoutingID() == original_rvh_id_);
    TakeActionOnResourceDispatcher(CANCEL);
  }
}

ApplicationWindowHost* InterstitialPageImpl::GetWindow() const {
...
void InterstitialPageImpl::Observe(
  ...
  switch (type) {
    case NOTIFICATION_NAV_ENTRY_PENDING:
      ...
      // the ApplicationWindowHost has not yet navigated so we'll unblock the
      // ApplicationWindowHost before the resource request for the new page we are
      // navigating arrives in the ResourceDispatcherHost.  This ensures that
      // request won't be blocked if the same ApplicationWindowHost was used for the
      // new navigation.
      Disable();
      TakeActionOnResourceDispatcher(CANCEL);
      break;
...
//    const IPC::Message& message) {
  //if (render_frame_host->GetApplicationWindowHost() != application_window_host_) {
  //  DCHECK(!application_window_host_)
...
void InterstitialPageImpl::ApplicationWindowCreated(
...
void InterstitialPageImpl::UpdateTitle(
  ...
  //ApplicationWindowHost* application_window_host = render_frame_host->GetApplicationWindowHost();
  //DCHECK(app_window_host == application_window_host_);
  //NavigationEntry* entry = controller_->GetVisibleEntry();
  //if (!entry) {
  ...
  //if (!new_navigation_ && !should_revert_app_contents_title_) {
  //  original_app_contents_title_ = entry->GetTitle();
  //  should_revert_app_contents_title_ = true;
  //}
  // TODO(evan): make use of title_direction.
  // http://code.google.com/p/chromium/issues/detail?id=27094
  //app_contents_->UpdateTitleForEntry(entry, title);
}
...
ui::AXMode InterstitialPageImpl::GetAccessibilityMode() const {
  //if (app_contents_)
  //  return app_contents_->GetAccessibilityMode();
  //else
    return ui::AXMode();
}
...
ApplicationWindowHostDelegateView* InterstitialPageImpl::GetDelegateView() {
  return rvh_delegate_view_.get();
}

ApplicationContents* InterstitialPageImpl::GetApplicationContents() const {
  return app_contents();
}
...
void InterstitialPageImpl::ApplicationWindowTerminated(
    ApplicationWindowHost* application_window_host,
...
//  pause_throbber_ = true;
  ...
  // The ApplicationWindowHost has loaded its contents, we can show it now.
  //if (!controller_->delegate()->IsHidden())
//    application_window_host_->GetView()->Show();
  //controller_->delegate()->AttachInterstitialPage(this);
//   application_contents_->AttachInterstitialPage(this);

//  ApplicationWindowHostView* rwh_view = application_window_host_->GetView();
      //controller_->delegate()->GetApplicationWindowHost()->GetWidget()->GetView();

  // The ApplicationWindowHost may already have crashed before we even get here.
//  if (rwh_view) {
...
ApplicationContents* InterstitialPageImpl::OpenURL(const GURL& url) {
  NOTREACHED();
  return nullptr;
}
...
common::RendererPreferences InterstitialPageImpl::GetRendererPrefs() const {
    //ApplicationContents* application_contents) const {
  delegate_->OverrideRendererPrefs(&renderer_preferences_);
  return renderer_preferences_;
}

void InterstitialPageImpl::ApplicationWindowDeleted(
    ApplicationWindowHost* application_window_host) {
...
KeyboardEventProcessingResult InterstitialPageImpl::PreHandleKeyboardEvent(
  ...
  return application_window_host_delegate_->PreHandleKeyboardEvent(event);
}

void InterstitialPageImpl::HandleKeyboardEvent(
      const NativeWebKeyboardEvent& event) {
  if (enabled())
    application_window_host_delegate_->HandleKeyboardEvent(event);
}

ApplicationContents* InterstitialPageImpl::app_contents() const {
  return app_contents_;
}
...
ApplicationWindowHost* InterstitialPageImpl::CreateApplicationWindowHost() {
  if (!enabled())
    return nullptr;

  // Interstitial pages don't want to share the session storage so we mint a
  // new one.
  //ApplicationContents* application_contents = app_contents()->GetApplicationContents();
  //scoped_refptr<SiteInstance> site_instance =
  //    SiteInstance::Create(application_contents);
  //DOMStorageContextWrapper* dom_storage_context =
  //    static_cast<DOMStorageContextWrapper*>(
  //        ApplicationContents::GetStoragePartition(
  //            application_contents, site_instance.get())->GetDOMStorageContext());
  //session_storage_namespace_ =
  //    SessionStorageNamespaceImpl::Create(dom_storage_context);

  // Use the ApplicationWindowHost from our FrameTree.
  // TODO(avi): The view routing ID can be restored to MSG_ROUTING_NONE once
  // ApplicationWindowHost has-a ApplicationWindowHost. https://crbug.com/545684
  ...
  return nullptr;//frame_tree_->root()->current_frame_host()->application_window_host();
}

ApplicationContentsView* InterstitialPageImpl::CreateApplicationContentsView() {
  if (!enabled() || !create_view_)
    return nullptr;
  ApplicationContentsView* wcv = app_contents()->GetView();
  ApplicationWindowHostView* view =
      wcv->CreateViewForWindow(application_window_host_);
  application_window_host_->SetView(view);
  //application_window_host_->GetMainFrame()->AllowBindings(
  //    BINDINGS_POLICY_DOM_AUTOMATION);

  application_window_host_->CreateApplicationWindow();//MSG_ROUTING_NONE, MSG_ROUTING_NONE,
                                      //base::UnguessableToken::Create(), false);
  //controller_->delegate()->ApplicationWindowForInterstitialPageCreated(
  //    frame_tree_->root()->current_frame_host());
  view->SetSize(app_contents()->GetContainerBounds().size());
...
void InterstitialPageImpl::SetSize(const gfx::Size& size) {
  ...
  if (application_window_host_->GetView())
    application_window_host_->GetView()->SetSize(size);
#else
...
void InterstitialPageImpl::Focus() {
  ...
  if (!enabled())
    return;
  application_window_host_->GetView()->Focus();
}

void InterstitialPageImpl::FocusThroughTabTraversal(bool reverse) {
  if (!enabled())
    return;
  application_window_host_->SetInitialFocus(reverse);
}

ApplicationWindowHostView* InterstitialPageImpl::GetView() {
  return application_window_host_->GetView();
}

//RenderFrameHost* InterstitialPageImpl::GetMainFrame() const {
//  return application_window_host_->GetMainFrame();
//}
...
//  if (app_contents_) {
//    static_cast<ApplicationContentsImpl*>(app_contents_)
//        ->SetAsFocusedApplicationContentsIfNecessary();
//  }
//}

void InterstitialPageImpl::CreateNewWindow(int32_t render_process_id,
                                           int32_t route_id,
...
void InterstitialPageImpl::CreateNewFullscreenWindow(int32_t render_process_id,
                                                     int32_t route_id) {//,
                                                    // mojom::WidgetPtr widget) {
  NOTREACHED()
...
void InterstitialPageImpl::ShowCreatedFullscreenWindow(int process_id,
                                                       int route_id) {
  NOTREACHED()
      << "InterstitialPage does not support showing full screen popups.";
}
...
void InterstitialPageImpl::TakeActionOnResourceDispatcher(
 ...
 //     return;
  ...
  // The tab might not have a application_window_host if it was closed (in which case,
  // we have taken care of the blocked requests when processing
  // NOTIFY_RENDER_WIDGET_HOST_DESTROYED.
  //ApplicationWindowHost* rvh = ApplicationWindowHost::FromID(original_child_id_,
  //                                                     original_rvh_id_);
  //if (!rvh)
  //  return;

  //ApplicationWindowHost* rfh =
  //    static_cast<ApplicationWindowHost*>(rvh->GetMainFrame());
  // Note, the ApplicationWindowHost can lose its main frame if a new RenderFrameHost
  // commits with a new ApplicationWindowHost. Additionally, ApplicationWindowHosts for OOPIF
...
void InterstitialPageImpl::OnDomOperationResponse(
  ...
  NotificationService::current()->Notify(NOTIFICATION_DOM_OPERATION_RESPONSE,
                                         Source<ApplicationContents>(app_contents()),
...
void InterstitialPageImpl::InterstitialPageRVHDelegateView::StartDragging(
  ...
  interstitial_page_->application_window_host_->
...
void InterstitialPageImpl::InterstitialPageRVHDelegateView::GotFocus(
    ApplicationWindowHost* application_window_host) {
  ApplicationContents* app_contents = interstitial_page_->app_contents();
  if (app_contents) {
    app_contents->NotifyApplicationContentsFocused(application_window_host);
  }
}
...
void InterstitialPageImpl::InterstitialPageRVHDelegateView::LostFocus(
  ...
  ApplicationContents* app_contents = interstitial_page_->app_contents();
  if (app_contents) {
    app_contents->NotifyApplicationContentsLostFocus(application_window_host);
  }
}

void InterstitialPageImpl::InterstitialPageRVHDelegateView::TakeFocus(
    bool reverse) {
  if (!interstitial_page_->app_contents())
    return;
  ApplicationContents* app_contents = interstitial_page_->app_contents();
  if (!app_contents->GetDelegateView())
    return;

  app_contents->GetDelegateView()->TakeFocus(reverse);
}
...
InterstitialPageImpl::UnderlyingContentObserver::UnderlyingContentObserver(
    ...
    : ApplicationContentsObserver(app_contents) {//, interstitial_(interstitial) {
  DLOG(INFO) << "InterstitialPageImpl::UnderlyingContentObserver: " << this;
}
...
void InterstitialPageImpl::UnderlyingContentObserver::ApplicationContentsDestroyed() {
  //interstitial_->OnNavigatingAwayOrTabClosing();
}

TextInputManager* InterstitialPageImpl::GetTextInputManager() {
  return !app_contents_ ? nullptr : app_contents_->GetTextInputManager();
}

ApplicationWindowHostInputEventRouter* InterstitialPageImpl::GetInputEventRouter() {
  return app_contents_->GetInputEventRouter();
}

//BrowserAccessibilityManager*
//InterstitialPageImpl::GetRootBrowserAccessibilityManager() {
//  return nullptr;
  //return app_contents_->GetRootBrowserAccessibilityManager();
//}

//BrowserAccessibilityManager*
//InterstitialPageImpl::GetOrCreateRootBrowserAccessibilityManager() {
//  return nullptr;
  //return app_contents_->GetOrCreateRootBrowserAccessibilityManager();
//}
