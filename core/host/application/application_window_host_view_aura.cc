...
//#include "core/common/input_messages.h"
...
//#include "core/host/accessibility/browser_accessibility_manager_win.h"
//#include "core/host/accessibility/browser_accessibility_win.h"
#include "core/host/application/legacy_application_window_host_win.h"
...
class WinScreenKeyboardObserver : public ui::OnScreenKeyboardObserver {
 public:
  WinScreenKeyboardObserver(ApplicationWindowHostViewAura* host_view)
 ...
 private:
  ApplicationWindowHostViewAura* host_view_;
...
class ApplicationWindowHostViewAura::EventFilterForPopupExit
    : public ui::EventHandler {
 public:
  explicit EventFilterForPopupExit(ApplicationWindowHostViewAura* rwhva)
      : rwhva_(rwhva) {
    DCHECK(rwhva_);
 ...
 private:
  ApplicationWindowHostViewAura* rwhva_;

  DISALLOW_COPY_AND_ASSIGN(EventFilterForPopupExit);
};

void ApplicationWindowHostViewAura::ApplyEventFilterForPopupExit(
  ...
  if (target != window_ &&
    ...
    // popup children per view. See: ApplicationWindowHostViewAura::InitAsPopup().
    Shutdown();
  }
}
...
// ApplicationWindowHostViewAura.
class ApplicationWindowHostViewAura::WindowObserver : public aura::WindowObserver {
 public:
  explicit WindowObserver(ApplicationWindowHostViewAura* view)
      : view_(view), old_root_(nullptr) {
  ...
  void OnWindowAddedToRootWindow(aura::Window* window) override {
    ...
    old_root_ = window->GetRootWindow();
  }
 ...
 private:
  ApplicationWindowHostViewAura* view_;
  aura::Window* old_root_;
...
class ApplicationWindowHostViewAura::WindowAncestorObserver
    : public aura::WindowObserver {
 public:
  explicit WindowAncestorObserver(ApplicationWindowHostViewAura* view)
      : view_(view) {
...
////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, public:

ApplicationWindowHostViewAura::ApplicationWindowHostViewAura(
    ApplicationWindowHost* application_window_host)
    : ApplicationWindowHostView(application_window_host),
      window_(nullptr),
      in_shutdown_(false),
...
#if defined(OS_WIN)
      legacy_application_window_host_HWND_(nullptr),
      legacy_window_destroyed_(false),
      virtual_keyboard_requested_(false),
#endif
      has_snapped_to_boundary_(false),
      device_scale_factor_(0.0f),
      event_handler_(new ApplicationWindowHostViewEventHandler(host(), this, this)),
      frame_sink_id_(host()->GetFrameSinkId()),//base::FeatureList::IsEnabled(features::kMash)
                     //    ? viz::FrameSinkId()
                     //    : host()->GetFrameSinkId()),
  ...
  ignore_result(host()->GetWebkitPreferences());
}

////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, ApplicationWindowHostView implementation:

void ApplicationWindowHostViewAura::InitAsChild(gfx::NativeView parent_view) {
...
void ApplicationWindowHostViewAura::InitAsFullscreen(ApplicationWindowHostView* reference_host_view) {
  ...
  if (reference_host_view) {
    aura::Window* reference_window =
        static_cast<ApplicationWindowHostViewAura*>(reference_host_view)->window_;
...
void ApplicationWindowHostViewAura::Show() {
  window_->Show();
  WasUnOccluded();
}

void ApplicationWindowHostViewAura::Hide() {
  window_->Hide();
  WasOccluded();
}

void ApplicationWindowHostViewAura::SetSize(const gfx::Size& size) {
...
void ApplicationWindowHostViewAura::SetBounds(const gfx::Rect& rect) {
  gfx::Point relative_origin(rect.origin());

  // ApplicationWindowHostViewAura::SetBounds() takes screen coordinates, but
...
gfx::NativeView ApplicationWindowHostViewAura::GetNativeView() const {
  return window_;
}

#if defined(OS_WIN)
HWND ApplicationWindowHostViewAura::GetHostWindowHWND() const {
  aura::WindowTreeHost* host = window_->GetHost();
  return host ? host->GetAcceleratedWidget() : nullptr;
}
#endif

gfx::NativeViewAccessible ApplicationWindowHostViewAura::GetNativeViewAccessible() {
// #if defined(OS_WIN)
//   aura::WindowTreeHost* window_host = window_->GetHost();
//   if (!window_host)
//     return static_cast<gfx::NativeViewAccessible>(NULL);
//   BrowserAccessibilityManager* manager =
//       host()->GetOrCreateRootAccessibilityManager();
//   if (manager)
...
ui::TextInputClient* ApplicationWindowHostViewAura::GetTextInputClient() {
  return this;
}

void ApplicationWindowHostViewAura::SetNeedsBeginFrames(bool needs_begin_frames) {
  needs_begin_frames_ = needs_begin_frames;
  UpdateNeedsBeginFramesInternal();
}

void ApplicationWindowHostViewAura::SetWantsAnimateOnlyBeginFrames() {
  if (delegated_frame_host_)
    delegated_frame_host_->SetWantsAnimateOnlyBeginFrames();
}

void ApplicationWindowHostViewAura::OnBeginFrame(base::TimeTicks frame_time) {
  host()->ProgressFling(frame_time);
  UpdateNeedsBeginFramesInternal();
}

// RenderFrameHostImpl* ApplicationWindowHostViewAura::GetFocusedFrame() const {
...
void ApplicationWindowHostViewAura::HandleParentBoundsChanged() {
  SnapToPhysicalPixelBoundary();
#if defined(OS_WIN)
  if (legacy_application_window_host_HWND_) {
    legacy_application_window_host_HWND_->SetBounds(
        window_->GetBoundsInRootWindow());
  }
#endif
  if (!in_shutdown_) {
    // Send screen rects through the delegate if there is one. Not every
    // ApplicationWindowHost has a delegate (for example, drop-down widgets).
    if (host_->delegate())
...
void ApplicationWindowHostViewAura::ParentHierarchyChanged() {
...
void ApplicationWindowHostViewAura::Focus() {
...
bool ApplicationWindowHostViewAura::HasFocus() const {
  return window_->HasFocus();
}

bool ApplicationWindowHostViewAura::IsSurfaceAvailableForCopy() const {
  if (!delegated_frame_host_)
    return false;
  return delegated_frame_host_->CanCopyFromCompositingSurface();
}

void ApplicationWindowHostViewAura::EnsureSurfaceSynchronizedForLayoutTest() {
  ++latest_capture_sequence_number_;
  SynchronizeVisualProperties(cc::DeadlinePolicy::UseInfiniteDeadline(),
                              base::nullopt);
}

bool ApplicationWindowHostViewAura::IsShowing() {
  return window_->IsVisible();
}

void ApplicationWindowHostViewAura::WasUnOccluded() {
  if (!host_->is_hidden()) {
    return;
  }
...
void ApplicationWindowHostViewAura::WasOccluded() {
  if (!host()->is_hidden()) {
    host()->WasHidden();
    if (delegated_frame_host_)
    ...
    aura::WindowTreeHost* host = window_->GetHost();
    if (host) {
      // We reparent the legacy Chrome_ApplicationWindowHostHWND window to the global
      // hidden window on the same lines as Windowed plugin windows.
      if (legacy_application_window_host_HWND_)
        legacy_application_window_host_HWND_->UpdateParent(ui::GetHiddenWindow());
    }
#endif
  }
...
#if defined(OS_WIN)
  if (legacy_application_window_host_HWND_)
    legacy_application_window_host_HWND_->Hide();
#endif
}

gfx::Rect ApplicationWindowHostViewAura::GetViewBounds() const {
  return window_->GetBoundsInScreen();
}

//void ApplicationWindowHostViewAura::SetBackgroundColor(SkColor color) {
//  //DLOG(INFO) << "ApplicationWindowHostViewAura::SetBackgroundColor: a: " << SkColorGetA(color) << " r: " << SkColorGetR(color) << " g: "<< SkColorGetG(color) << " b: " << SkColorGetB(color);
...
//SkColor ApplicationWindowHostViewAura::background_color() const {
//  return background_color_;
//}

void ApplicationWindowHostViewAura::UpdateBackgroundColor() {
  //DLOG(INFO) << "ApplicationWindowHostViewAura::UpdateBackgroundColor";
...
void ApplicationWindowHostViewAura::WindowTitleChanged() {
  if (delegated_frame_host_) {
...
bool ApplicationWindowHostViewAura::IsMouseLocked() {
  return event_handler_->mouse_locked();
}

gfx::Size ApplicationWindowHostViewAura::GetVisibleViewportSize() const {
  gfx::Rect requested_rect(GetRequestedApplicationSize());
  requested_rect.Inset(insets_);
  return requested_rect.size();
}

void ApplicationWindowHostViewAura::SetInsets(const gfx::Insets& insets) {
  if (insets != insets_) {
    insets_ = insets;
...
void ApplicationWindowHostViewAura::FocusedNodeTouched(
    bool editable) {
#if defined(OS_WIN)
...
void ApplicationWindowHostViewAura::UpdateCursor(const common::WebCursor& cursor) {
  GetCursorManager()->UpdateCursor(this, cursor);
}

void ApplicationWindowHostViewAura::DisplayCursor(const common::WebCursor& cursor) {
...
CursorManager* ApplicationWindowHostViewAura::GetCursorManager() {
  return cursor_manager_.get();
}

void ApplicationWindowHostViewAura::SetIsLoading(bool is_loading) {
  is_loading_ = is_loading;
  UpdateCursorIfOverSelf();
}

void ApplicationWindowHostViewAura::ApplicationProcessGone(base::TerminationStatus status,
                                                           int error_code) {
  UpdateCursorIfOverSelf();
  Destroy();
}

void ApplicationWindowHostViewAura::Destroy() {
  ...
  // ~ApplicationWindowHostViewAura when |window_| is destroyed. Otherwise, The
...
void ApplicationWindowHostViewAura::SetTooltipText(
    const base::string16& tooltip_text) {
  GetCursorManager()->SetTooltipTextForView(this, tooltip_text);
}

void ApplicationWindowHostViewAura::DisplayTooltipText(
    const base::string16& tooltip_text) {
...
gfx::Size ApplicationWindowHostViewAura::GetRequestedApplicationSize() const {
  return delegated_frame_host_
             ? delegated_frame_host_->GetRequestedRendererSize()
             : ApplicationWindowHostView::GetRequestedApplicationSize();
}

uint32_t ApplicationWindowHostViewAura::GetCaptureSequenceNumber() const {
  return latest_capture_sequence_number_;
}

void ApplicationWindowHostViewAura::CopyFromSurface(
...
#if defined(OS_WIN)
bool ApplicationWindowHostViewAura::UsesNativeWindowFrame() const {
  return (legacy_application_window_host_HWND_ != NULL);
}

void ApplicationWindowHostViewAura::UpdateMouseLockRegion() {
  RECT window_rect =
...
void ApplicationWindowHostViewAura::OnLegacyWindowDestroyed() {
  legacy_application_window_host_HWND_ = nullptr;
  legacy_window_destroyed_ = true;
}
#endif

void ApplicationWindowHostViewAura::DidCreateNewApplicationCompositorFrameSink(
...
void ApplicationWindowHostViewAura::SubmitCompositorFrame(
    const viz::LocalSurfaceId& local_surface_id,
    viz::CompositorFrame frame,
    viz::mojom::HitTestRegionListPtr hit_test_region_list) {
  DCHECK(delegated_frame_host_);
  TRACE_EVENT0("content", "ApplicationWindowHostViewAura::OnSwapCompositorFrame");
  ...
  ////DLOG(INFO) << "ApplicationWindowHostViewAura::SubmitCompositorFrame: UpdateBackgroundColorFromRenderer -> " << 
  //  " a: " << SkColorGetA(frame.metadata.root_background_color) << 
  //  " r: " << SkColorGetR(frame.metadata.root_background_color) << 
  //  " g: " << SkColorGetG(frame.metadata.root_background_color) << 
  //  " b: " << SkColorGetB(frame.metadata.root_background_color);
  //UpdateBackgroundColorFromRenderer(SK_ColorBLUE);//frame.metadata.root_background_color);
    
  delegated_frame_host_->SubmitCompositorFrame(
      local_surface_id, std::move(frame), std::move(hit_test_region_list));
}

void ApplicationWindowHostViewAura::OnDidNotProduceFrame(
    const viz::BeginFrameAck& ack) {
  if (delegated_frame_host_)
    delegated_frame_host_->DidNotProduceFrame(ack);
}

void ApplicationWindowHostViewAura::ClearCompositorFrame() {
  if (delegated_frame_host_)
    delegated_frame_host_->ClearDelegatedFrame();
}

void ApplicationWindowHostViewAura::DidStopFlinging() {
  selection_controller_client_->OnScrollCompleted();
}

gfx::Vector2d ApplicationWindowHostViewAura::GetOffsetFromRootSurface() {
  return window_->GetBoundsInRootWindow().OffsetFromOrigin();
}

gfx::Rect ApplicationWindowHostViewAura::GetBoundsInRootWindow() {
...
void ApplicationWindowHostViewAura::WheelEventAck(
    const blink::WebMouseWheelEvent& event,
    common::InputEventAckState ack_result) {
  if (overscroll_controller_) {
    overscroll_controller_->ReceivedEventACK(
        event, (common::INPUT_EVENT_ACK_STATE_CONSUMED == ack_result));
  }
}

void ApplicationWindowHostViewAura::DidOverscroll(
    const ui::DidOverscrollParams& params) {
  if (overscroll_controller_)
    overscroll_controller_->OnDidOverscroll(params);
}

void ApplicationWindowHostViewAura::GestureEventAck(
...
void ApplicationWindowHostViewAura::ProcessAckedTouchEvent(
  ...
  ui::EventResult result = (ack_result == common::INPUT_EVENT_ACK_STATE_CONSUMED)
                               ? ui::ER_HANDLED
                               : ui::ER_UNHANDLED;
...
ApplicationWindowHostViewAura::CreateSyntheticGestureTarget() {
  return std::unique_ptr<SyntheticGestureTarget>(
      new SyntheticGestureTargetAura(host()));
}

common::InputEventAckState ApplicationWindowHostViewAura::FilterInputEvent(
  ...
  if (consumed &&
    ...
    // ApplicationWindowHostViewChildFrame::FilterInputEvent()
    return common::INPUT_EVENT_ACK_STATE_NO_CONSUMER_EXISTS;
  }

  return consumed ? common::INPUT_EVENT_ACK_STATE_CONSUMED
                  : common::INPUT_EVENT_ACK_STATE_NOT_CONSUMED;
}

common::InputEventAckState ApplicationWindowHostViewAura::FilterChildGestureEvent(
    const blink::WebGestureEvent& gesture_event) {
  if (overscroll_controller_ &&
      overscroll_controller_->WillHandleEvent(gesture_event))
    return common::INPUT_EVENT_ACK_STATE_CONSUMED;
  return common::INPUT_EVENT_ACK_STATE_NOT_CONSUMED;
}

//BrowserAccessibilityManager*
//ApplicationWindowHostViewAura::CreateAccessibilityManager(
    //BrowserAccessibilityDelegate* delegate, bool for_root_frame) {
//  BrowserAccessibilityManager* manager = nullptr;
//#if defined(OS_WIN)
//  manager = new AccessibilityManagerWin(
//      AccessibilityManagerWin::GetEmptyDocument(), delegate);
//#else
...
gfx::AcceleratedWidget ApplicationWindowHostViewAura::AccessibilityGetAcceleratedWidget() {
#if defined(OS_WIN)
  if (legacy_application_window_host_HWND_)
    return legacy_application_window_host_HWND_->hwnd();
#endif
  return gfx::kNullAcceleratedWidget;
}

gfx::NativeViewAccessible ApplicationWindowHostViewAura::AccessibilityGetNativeViewAccessible() {
#if defined(OS_WIN)
  if (legacy_application_window_host_HWND_)
    return legacy_application_window_host_HWND_->window_accessible();
#endif
  return nullptr;
}

void ApplicationWindowHostViewAura::SetMainFrameAXTreeID(
    ui::AXTreeIDRegistry::AXTreeID id) {
  window_->SetProperty(ui::kChildAXTreeID, id);
}

bool ApplicationWindowHostViewAura::LockMouse() {
  return event_handler_->LockMouse();
}

void ApplicationWindowHostViewAura::UnlockMouse() {
  event_handler_->UnlockMouse();
}

bool ApplicationWindowHostViewAura::LockKeyboard(
    base::Optional<base::flat_set<int>> keys) {
  return event_handler_->LockKeyboard(std::move(keys));
}

void ApplicationWindowHostViewAura::UnlockKeyboard() {
  event_handler_->UnlockKeyboard();
}

bool ApplicationWindowHostViewAura::IsKeyboardLocked() {
  return event_handler_->IsKeyboardLocked();
}

////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, ui::TextInputClient implementation:
void ApplicationWindowHostViewAura::SetCompositionText(
...
void ApplicationWindowHostViewAura::ConfirmCompositionText() {
  if (text_input_manager_ && text_input_manager_->GetActiveWidget() &&
...
void ApplicationWindowHostViewAura::ClearCompositionText() {
  if (text_input_manager_ && text_input_manager_->GetActiveWidget() &&
      has_composition_text_)
...
void ApplicationWindowHostViewAura::InsertText(const base::string16& text) {
...
void ApplicationWindowHostViewAura::InsertChar(const ui::KeyEvent& event) {
  ui::LatencyInfo local_latency = *event.latency();
  local_latency.set_trace_id(++tab_show_sequence_);
  ...
  if (event_handler_->accept_return_character() ||
    ...
    ForwardKeyboardEventWithLatencyInfo(
        NativeWebKeyboardEvent(event, event.GetCharacter()), local_latency,
        nullptr);
  }
}

ui::TextInputType ApplicationWindowHostViewAura::GetTextInputType() const {
  if (text_input_manager_ && text_input_manager_->GetTextInputState())
    return text_input_manager_->GetTextInputState()->type;
  return ui::TEXT_INPUT_TYPE_NONE;
}

ui::TextInputMode ApplicationWindowHostViewAura::GetTextInputMode() const {
  if (text_input_manager_ && text_input_manager_->GetTextInputState())
    return text_input_manager_->GetTextInputState()->mode;
  return ui::TEXT_INPUT_MODE_DEFAULT;
}

base::i18n::TextDirection ApplicationWindowHostViewAura::GetTextDirection() const {
  NOTIMPLEMENTED();
  return base::i18n::UNKNOWN_DIRECTION;
}

int ApplicationWindowHostViewAura::GetTextInputFlags() const {
  if (text_input_manager_ && text_input_manager_->GetTextInputState())
    return text_input_manager_->GetTextInputState()->flags;
  return 0;
}

bool ApplicationWindowHostViewAura::CanComposeInline() const {
  if (text_input_manager_ && text_input_manager_->GetTextInputState())
    return text_input_manager_->GetTextInputState()->can_compose_inline;
  return true;
}

gfx::Rect ApplicationWindowHostViewAura::ConvertRectToScreen(
...
gfx::Rect ApplicationWindowHostViewAura::ConvertRectFromScreen(
    const gfx::Rect& rect) const {
...
gfx::Rect ApplicationWindowHostViewAura::GetCaretBounds() const {
  if (!text_input_manager_ || !text_input_manager_->GetActiveWidget())
    return gfx::Rect();
...
bool ApplicationWindowHostViewAura::GetCompositionCharacterBounds(
    uint32_t index,
    gfx::Rect* rect) const {
  DCHECK(rect);
...
bool ApplicationWindowHostViewAura::HasCompositionText() const {
  return has_composition_text_;
}

bool ApplicationWindowHostViewAura::GetTextRange(gfx::Range* range) const {
...
bool ApplicationWindowHostViewAura::GetCompositionTextRange(
...
bool ApplicationWindowHostViewAura::GetSelectionRange(gfx::Range* range) const {
  if (!text_input_manager_ || !GetFocusedWidget())
    return false;
...
bool ApplicationWindowHostViewAura::SetSelectionRange(const gfx::Range& range) {
  // TODO(suzhe): implement this method when fixing http://crbug.com/55130.
  NOTIMPLEMENTED();
  return false;
}

bool ApplicationWindowHostViewAura::DeleteRange(const gfx::Range& range) {
  // TODO(suzhe): implement this method when fixing http://crbug.com/55130.
  NOTIMPLEMENTED();
  return false;
}

bool ApplicationWindowHostViewAura::GetTextFromRange(
...
void ApplicationWindowHostViewAura::OnInputMethodChanged() {
  // TODO(suzhe): implement the newly added "locale" property of HTML DOM
  // TextEvent.
}

bool ApplicationWindowHostViewAura::ChangeTextDirectionAndLayoutAlignment(
...
void ApplicationWindowHostViewAura::ExtendSelectionAndDelete(
    size_t before, size_t after) {
  //ApplicationWindowHost* rfh = GetFocusedWidget();
  //if (rfh)
  //  rfh->GetWindowInputHandler()->ExtendSelectionAndDelete(before, after);

  // TODO: implement
  DCHECK(false);
}

void ApplicationWindowHostViewAura::EnsureCaretNotInRect(
...
bool ApplicationWindowHostViewAura::IsTextEditCommandEnabled(
    ui::TextEditCommand command) const {
  return false;
}

void ApplicationWindowHostViewAura::SetTextEditCommandForNextKeyEvent(
    ui::TextEditCommand command) {}

const std::string& ApplicationWindowHostViewAura::GetClientSourceInfo() const {
  return GURL().spec();//GetFocusedWidget()->GetLastCommittedURL().spec();
}

////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, display::DisplayObserver implementation:

void ApplicationWindowHostViewAura::OnDisplayAdded(
    const display::Display& new_display) {}

void ApplicationWindowHostViewAura::OnDisplayRemoved(
    const display::Display& old_display) {}

void ApplicationWindowHostViewAura::OnDisplayMetricsChanged(
    const display::Display& display,
    uint32_t metrics) {
...
////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, aura::WindowDelegate implementation:

gfx::Size ApplicationWindowHostViewAura::GetMinimumSize() const {
  return gfx::Size();
}

gfx::Size ApplicationWindowHostViewAura::GetMaximumSize() const {
  return gfx::Size();
}

void ApplicationWindowHostViewAura::OnBoundsChanged(const gfx::Rect& old_bounds,
                                               const gfx::Rect& new_bounds) {
  base::AutoReset<bool> in_bounds_changed(&in_bounds_changed_, true);
  // We care about this whenever ApplicationWindowHostViewAura is not owned by a
...
gfx::NativeCursor ApplicationWindowHostViewAura::GetCursor(const gfx::Point& point) {
  if (mouse_locked_)
    return ui::CursorType::kNone;
  return current_cursor_.GetNativeCursor();
}

int ApplicationWindowHostViewAura::GetNonClientComponent(
    const gfx::Point& point) const {
  return HTCLIENT;
}

bool ApplicationWindowHostViewAura::ShouldDescendIntoChildForEventHandling(
    aura::Window* child,
    const gfx::Point& location) {
  return true;
}

bool ApplicationWindowHostViewAura::CanFocus() {
  return popup_type_ == blink::kWebPopupTypeNone;
}

void ApplicationWindowHostViewAura::OnCaptureLost() {
  host()->LostCapture();
}

void ApplicationWindowHostViewAura::OnPaint(const ui::PaintContext& context) {
  NOTREACHED();
}

void ApplicationWindowHostViewAura::OnDeviceScaleFactorChanged(
...
void ApplicationWindowHostViewAura::OnWindowDestroying(aura::Window* window) {
#if defined(OS_WIN)
  // The LegacyApplicationWindowHostHWND instance is destroyed when its window is
  ...
  // ApplicationWindowHostViewAura instance goes away etc. To avoid that we
  // destroy the LegacyApplicationWindowHostHWND instance here.
  if (legacy_application_window_host_HWND_) {
    legacy_application_window_host_HWND_->Destroy();
    // The Destroy call above will delete the LegacyApplicationWindowHostHWND
    // instance.
    legacy_application_window_host_HWND_ = NULL;
  }
#endif
...
void ApplicationWindowHostViewAura::OnWindowDestroyed(aura::Window* window) {
...
void ApplicationWindowHostViewAura::OnWindowTargetVisibilityChanged(bool visible) {
}

bool ApplicationWindowHostViewAura::HasHitTestMask() const {
  return false;
}

void ApplicationWindowHostViewAura::GetHitTestMask(gfx::Path* mask) const {
}

////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, ui::EventHandler implementation:

void ApplicationWindowHostViewAura::OnKeyEvent(ui::KeyEvent* event) {
  event_handler_->OnKeyEvent(event);
}

void ApplicationWindowHostViewAura::OnMouseEvent(ui::MouseEvent* event) {
#if defined(OS_WIN)
...
#endif
  ////DLOG(INFO) << "ApplicationWindowHostViewAura::OnMouseEvent: FIXME: forcing trace id on mouse event latency";
  event->latency()->set_trace_id(++tab_show_sequence_);
  event_handler_->OnMouseEvent(event);
}

bool ApplicationWindowHostViewAura::TransformPointToLocalCoordSpace(
...
bool ApplicationWindowHostViewAura::TransformPointToCoordSpaceForView(
    const gfx::PointF& point,
...
viz::FrameSinkId ApplicationWindowHostViewAura::GetRootFrameSinkId() {
  if (window_->GetHost()->compositor())
    return window_->GetHost()->compositor()->frame_sink_id();
  return viz::FrameSinkId();
}

viz::SurfaceId ApplicationWindowHostViewAura::GetCurrentSurfaceId() const {
  return delegated_frame_host_ ? delegated_frame_host_->GetCurrentSurfaceId()
                               : viz::SurfaceId();
}

void ApplicationWindowHostViewAura::FocusedNodeChanged(
    bool editable,
...
void ApplicationWindowHostViewAura::ScheduleEmbed(
...
void ApplicationWindowHostViewAura::OnScrollEvent(ui::ScrollEvent* event) {
  event_handler_->OnScrollEvent(event);
}

void ApplicationWindowHostViewAura::OnTouchEvent(ui::TouchEvent* event) {
  event_handler_->OnTouchEvent(event);
}

void ApplicationWindowHostViewAura::OnGestureEvent(ui::GestureEvent* event) {
  event_handler_->OnGestureEvent(event);
}

////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, wm::ActivationDelegate implementation:

bool ApplicationWindowHostViewAura::ShouldActivate() const {
  aura::WindowTreeHost* host = window_->GetHost();
  if (!host)
    return true;
...
////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, aura::client::CursorClientObserver implementation:

void ApplicationWindowHostViewAura::OnCursorVisibilityChanged(bool is_visible) {
  NotifyRendererOfCursorVisibilityState(is_visible);
}

////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, aura::client::FocusChangeObserver implementation:

void ApplicationWindowHostViewAura::OnWindowFocused(aura::Window* gained_focus,
  ...
  //  BrowserAccessibilityManager* manager =
  //      host()->GetRootAccessibilityManager();
  //  if (manager)
...
////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, aura::WindowTreeHostObserver implementation:

void ApplicationWindowHostViewAura::OnHostMovedInPixels(
    aura::WindowTreeHost* host,
    const gfx::Point& new_origin_in_pixels) {
  TRACE_EVENT1("ui", "ApplicationWindowHostViewAura::OnHostMovedInPixels",
...
////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, RenderFrameMetadataProvider::Observer
// implementation:
void ApplicationWindowHostViewAura::OnRenderFrameMetadataChanged() {
  //DLOG(INFO) << "ApplicationWindowHostViewAura::OnRenderFrameMetadataChanged";
...
////////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewAura, private:
...
#if defined(OS_WIN)
  // The LegacyApplicationWindowHostHWND window should have been destroyed in
  // ApplicationWindowHostViewAura::OnWindowDestroying and the pointer should
  // be set to NULL.
  DCHECK(!legacy_application_window_host_HWND_);
#endif
...
void ApplicationWindowHostViewAura::CreateAuraWindow(aura::client::WindowType type) {
  DCHECK(!window_);
  window_ = new aura::Window(this);
  window_->SetName("ApplicationWindowHostViewAura");
...
void ApplicationWindowHostViewAura::CreateDelegatedFrameHostClient() {
  if (!frame_sink_id_.is_valid())
    return;
...
void ApplicationWindowHostViewAura::UpdateCursorIfOverSelf() {
  ...
  if (!window_at_screen_point && legacy_application_window_host_HWND_) {
    HWND hwnd_at_point = ::WindowFromPoint(cursor_screen_point.ToPOINT());

    if (hwnd_at_point == legacy_application_window_host_HWND_->hwnd())
      hwnd_at_point = legacy_application_window_host_HWND_->GetParent();
...
void ApplicationWindowHostViewAura::SynchronizeVisualProperties(
...
ui::InputMethod* ApplicationWindowHostViewAura::GetInputMethod() const {
  if (!window_)
    return nullptr;
  aura::Window* root_window = window_->GetRootWindow();
  if (!root_window)
...
ApplicationWindowHostView* ApplicationWindowHostViewAura::GetFocusedViewForTextSelection() {
...
void ApplicationWindowHostViewAura::Shutdown() {
  if (!in_shutdown_) {
    in_shutdown_ = true;
    host()->ShutdownAndDestroyWindow(true);
  }
}

TouchSelectionControllerClientManager*
ApplicationWindowHostViewAura::GetTouchSelectionControllerClientManager() {
  return selection_controller_client_.get();
}

bool ApplicationWindowHostViewAura::NeedsInputGrab() {
  return popup_type_ == blink::kWebPopupTypePage;
}

bool ApplicationWindowHostViewAura::NeedsMouseCapture() {
#if defined(OS_LINUX) && !defined(OS_CHROMEOS)
...
void ApplicationWindowHostViewAura::SetTooltipsEnabled(bool enable) {
  if (enable) {
...
void ApplicationWindowHostViewAura::ShowContextMenu(
  ...
  ApplicationWindowHostDelegate* delegate = host()->delegate();
  if (!delegate)
    return;

  ApplicationWindowHostDelegateView* delegate_view = delegate->GetDelegateView();
  if (!delegate_view)
    return;
  delegate_view->ShowContextMenu(GetFocusedWidget(), params);
}

void ApplicationWindowHostViewAura::NotifyRendererOfCursorVisibilityState(
    bool is_visible) {
  if (host()->is_hidden() ||
...
void ApplicationWindowHostViewAura::SetOverscrollControllerEnabled(bool enabled) {
  if (!enabled)
    overscroll_controller_.reset();
...
void ApplicationWindowHostViewAura::SetOverscrollControllerForTesting(
    std::unique_ptr<OverscrollController> controller) {
  overscroll_controller_ = std::move(controller);
}

void ApplicationWindowHostViewAura::SnapToPhysicalPixelBoundary() {
...
void ApplicationWindowHostViewAura::SetSelectionControllerClientForTest(
    std::unique_ptr<TouchSelectionControllerClientAura> client) {
  selection_controller_client_.swap(client);
  CreateSelectionController();
}

void ApplicationWindowHostViewAura::InternalSetBounds(const gfx::Rect& rect) {
  SnapToPhysicalPixelBoundary();
...
void ApplicationWindowHostViewAura::SyncSurfaceProperties(
    const cc::DeadlinePolicy& deadline_policy) {
...
#if defined(OS_WIN)
void ApplicationWindowHostViewAura::UpdateLegacyWin() {
  if (legacy_window_destroyed_ || !GetHostWindowHWND())
    return;

  if (!legacy_application_window_host_HWND_) {
    legacy_application_window_host_HWND_ =
        LegacyApplicationWindowHostHWND::Create(GetHostWindowHWND());
  }

  if (legacy_application_window_host_HWND_) {
    legacy_application_window_host_HWND_->set_host(this);
    legacy_application_window_host_HWND_->UpdateParent(GetHostWindowHWND());
    legacy_application_window_host_HWND_->SetBounds(
        window_->GetBoundsInRootWindow());
    // There are cases where the parent window is created, made visible and
    // the associated ApplicationWindow is also visible before the
    // LegacyApplicationWindowHostHWND instace is created. Ensure that it is shown
    // here.
    if (!host()->is_hidden())
      legacy_application_window_host_HWND_->Show();
  }
}
#endif

void ApplicationWindowHostViewAura::SchedulePaintIfNotInClip(
    const gfx::Rect& rect,
    const gfx::Rect& clip) {
  if (!clip.IsEmpty()) {
...
void ApplicationWindowHostViewAura::AddedToRootWindow() {
  ...
  // NOTE: commented here.. we 'force' the add of the input event
  // on window creation. See how this can affect badly other scenarios
  // or if this is what we really want

  //if (HasFocus()) {
    ui::InputMethod* input_method = GetInputMethod();
    if (input_method) {
      ////DLOG(INFO) << "ApplicationWindowHostViewAura::OnWindowFocused: " << this << " input_method = " << input_method << ". adding..";
      input_method->SetFocusedTextInputClient(this);
    }
  //} else {
  //  //DLOG(INFO) << "ApplicationWindowHostViewAura::AddedToRootWindow: " << this << "HasFocus = false -> not adding input method";
  //}
...
void ApplicationWindowHostViewAura::RemovingFromRootWindow() {
  aura::client::CursorClient* cursor_client =
      aura::client::GetCursorClient(window_->GetRootWindow());
  if (cursor_client)
...
void ApplicationWindowHostViewAura::DetachFromInputMethod() {
  ui::InputMethod* input_method = GetInputMethod();
  if (input_method) {
...
void ApplicationWindowHostViewAura::ForwardKeyboardEventWithLatencyInfo(
    const NativeWebKeyboardEvent& event,
    const ui::LatencyInfo& latency,
    bool* update_event) {
  ApplicationWindowHost* target_host = host();

  // If there are multiple widgets on the page (such as when there are
  // out-of-process iframes), pick the one that should process this event.
  if (host()->delegate())
    target_host = host()->delegate()->GetFocusedApplicationWindowHost(host());
  if (!target_host) {
    return;
  }
  ...
  if (!event.skip_in_browser &&
    ...
    common::EditCommands edit_commands;
    for (std::vector<ui::TextEditCommandAuraLinux>::const_iterator it =
             commands.begin(); it != commands.end(); ++it) {
      edit_commands.push_back(common::EditCommand(it->GetCommandString(),
                                          it->argument()));
    }
...
void ApplicationWindowHostViewAura::CreateSelectionController() {
...
void ApplicationWindowHostViewAura::OnDidNavigateMainFrameToNewPage() {
  ui::GestureRecognizer::Get()->CancelActiveTouches(window_);
}

viz::FrameSinkId ApplicationWindowHostViewAura::GetFrameSinkId() {
  return frame_sink_id_;
}

viz::LocalSurfaceId ApplicationWindowHostViewAura::GetLocalSurfaceId() const {
  return window_->GetLocalSurfaceId();
}

void ApplicationWindowHostViewAura::OnUpdateTextInputStateCalled(
  ...
  const common::TextInputState* state = text_input_manager_->GetTextInputState();
  ...
  if (auto* application_window_host = updated_view->host()) {
    // Monitor the composition information if there is a focused editable node.
    application_window_host->RequestCompositionUpdates(
...
void ApplicationWindowHostViewAura::OnImeCancelComposition(
    TextInputManager* text_input_manager,
    ApplicationWindowHostView* view) {
  // |view| is not necessarily the one corresponding to
  // TextInputManager::GetActiveWidget() as ApplicationWindowHostViewAura can call
...
void ApplicationWindowHostViewAura::OnSelectionBoundsChanged(
    TextInputManager* text_input_manager,
    ApplicationWindowHostView* updated_view) {
  if (GetInputMethod())
    GetInputMethod()->OnCaretBoundsChanged(this);
  ...
  if (!GetApplicationWindowHost() || !legacy_application_window_host_HWND_)
    return;
  ...
  gfx::Rect dip_caret_rect = display::win::ScreenWin::DIPToScreenRect(
      legacy_application_window_host_HWND_->hwnd(), caret_rect);
  dip_caret_rect.set_width(1);  // Collapse any selection.
  legacy_application_window_host_HWND_->MoveCaretTo(dip_caret_rect);
#endif  // defined(OS_WIN)
}

void ApplicationWindowHostViewAura::OnTextSelectionChanged(
    TextInputManager* text_input_manager,
    ApplicationWindowHostView* updated_view) {
  if (!GetTextInputManager())
    return;
  ...
  // this case we should use TextSelection for |this| since RWHV for guest
  // forwards text selection information to its platform view.
  ApplicationWindowHostView* focused_view = GetFocusedWidget() ? 
    GetFocusedWidget()->GetView() : nullptr;
...
void ApplicationWindowHostViewAura::SetPopupChild(
    ApplicationWindowHostViewAura* popup_child_host_view) {
...
void ApplicationWindowHostViewAura::UpdateNeedsBeginFramesInternal() {
  if (!delegated_frame_host_)
    return;
  delegated_frame_host_->SetNeedsBeginFrames(needs_begin_frames_);
}

void ApplicationWindowHostViewAura::ScrollFocusedEditableNodeIntoRect(
...
void ApplicationWindowHostViewAura::OnSynchronizedDisplayPropertiesChanged() {
  SynchronizeVisualProperties(cc::DeadlinePolicy::UseDefaultDeadline(),
                              base::nullopt);
}

viz::ScopedSurfaceIdAllocator ApplicationWindowHostViewAura::ResizeDueToAutoResize(
    const gfx::Size& new_size,
    const viz::LocalSurfaceId& child_local_surface_id) {
  base::OnceCallback<void()> allocation_task = base::BindOnce(
      &ApplicationWindowHostViewAura::SynchronizeVisualProperties,
...
bool ApplicationWindowHostViewAura::IsLocalSurfaceIdAllocationSuppressed() const {
  DCHECK(window_);
  return window_->IsLocalSurfaceIdAllocationSuppressed();
}

void ApplicationWindowHostViewAura::DidNavigate() {
...
// static
viz::FrameSinkId
ApplicationWindowHostViewAura::AllocateFrameSinkIdForGuestViewHack() {
  return ImageTransportFactory::GetInstance()
      ->GetContextFactoryPrivate()
      ->AllocateFrameSinkId();
}

void ApplicationWindowHostViewAura::TakeFallbackContentFrom(
    ApplicationWindowHostView* view) {
  //DCHECK(!static_cast<ApplicationWindowHostView*>(view)
  //            ->IsApplicationWindowHostViewChildFrame());
  //DCHECK(!static_cast<ApplicationWindowHostView*>(view)
  //            ->IsApplicationWindowHostViewGuest());
  ApplicationWindowHostViewAura* view_aura =
      static_cast<ApplicationWindowHostViewAura*>(view);
