...
#ifndef MUMBA_HOST_APPLICATION_TEXT_INPUT_MANAGER_H__
#define MUMBA_HOST_APPLICATION_TEXT_INPUT_MANAGER_H__
...
class CONTENT_EXPORT TextInputManager {
 public:
  ...
  // |active_view_|.
  ApplicationWindowHost* GetActiveWidget() const;

  // ---------------------------------------------------------------------------
  // The following methods can be used to obtain information about IME-related
  // state for the active ApplicationWindowHost. If the active widget is nullptr, the
  ...
  const common::TextInputState* GetTextInputState() const;
  ...
  const SelectionRegion* GetSelectionRegion(
      ApplicationWindowHostView* view = nullptr) const;
  ...
  void UpdateTextInputState(ApplicationWindowHostView* view,
                            const common::TextInputState& state);

  // The current IME composition has been cancelled on the renderer side for
  // the widget corresponding to |view|.
  void ImeCancelComposition(ApplicationWindowHostView* view);
  ...
  void SelectionBoundsChanged(ApplicationWindowHostView* view,
  ...
  void SelectionChanged(ApplicationWindowHostView* view,
  ...
  void Register(ApplicationWindowHostView* view);
  ...
  void Unregister(ApplicationWindowHostView* view);

  // Returns true if |view| is already registered.
  bool IsRegistered(ApplicationWindowHostView* view) const;
  ...
  ApplicationWindowHostView* active_view_for_testing() { return active_view_; }
 ...
 private:
  ...
  class ViewMap : public std::unordered_map<ApplicationWindowHostView*, Value> {
  };

  void NotifyObserversAboutInputStateUpdate(ApplicationWindowHostView* view,
                                            bool did_update_state);

  // The view with active text input state, i.e., a focused <input> element.
  // It will be nullptr if no such view exists. Note that the active view
  // cannot have a |TextInputState.type| of ui::TEXT_INPUT_TYPE_NONE.
  ApplicationWindowHostView* active_view_;
  ...
  ViewMap<common::TextInputState> text_input_state_map_;
...
#endif  // MUMBA_HOST_APPLICATION_TEXT_INPUT_MANAGER_H__
