...
const common::MediaStreamDevice* AudioInputDeviceManager::GetOpenedDeviceById(
    int session_id) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  common::MediaStreamDevices::iterator device = GetDevice(session_id);
...
int AudioInputDeviceManager::Open(const common::MediaStreamDevice& device) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
...
void AudioInputDeviceManager::Close(int session_id) {
  ...
  if (device == devices_.end())
    return;
  const common::MediaStreamType stream_type = device->type;
...
void AudioInputDeviceManager::OpenedOnIOThread(
  ...
  common::MediaStreamDevice media_stream_device(device.type, device.id, device.name);
...
void AudioInputDeviceManager::ClosedOnIOThread(common::MediaStreamType stream_type,
                                               int session_id) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  for (auto& listener : listeners_)
...
common::MediaStreamDevices::iterator AudioInputDeviceManager::GetDevice(
    int session_id) {
  for (common::MediaStreamDevices::iterator it = devices_.begin(); it != devices_.end();
       ++it) {
