...
#include "core/host/application/media/mock_video_capture_provider.h"
#include "core/host/application/media/video_capture_manager.h"
#include "core/host/media_device_id.h"
...
class MediaStreamDispatcherHostTest : public testing::Test {
 public:
  MediaStreamDispatcherHostTest()
      : thread_bundle_(TestHostThreadBundle::IO_MAINLOOP),
        origin_(url::Origin::Create(GURL("https://test.com"))) {
    audio_manager_ = std::make_unique<media::MockAudioManager>(
        std::make_unique<media::TestAudioThread>());
    audio_system_ =
        std::make_unique<media::AudioSystemImpl>(audio_manager_.get());
    application_contents_ = std::make_unique<TestApplicationContents>();
  ...
  std::pair<std::string, url::Origin> GetSaltAndOrigin(int /* process_id */,
                                                       int /* frame_id */) {
    return std::make_pair(application_contents_->GetMediaDeviceIDSalt(), origin_);
  }
 ...
 protected:
  ...
  bool DoesEveryDeviceMapToRawId(const MediaStreamDevices& devices,
    ...
    for (size_t i = 0; i < devices.size(); ++i) {
      ...
      for (; audio_it != audio_device_descriptions_.end(); ++audio_it) {
        if (DoesMediaDeviceIDMatchHMAC(application_contents_->GetMediaDeviceIDSalt(),
                                       origin, devices[i].id,
                                       audio_it->unique_id)) {
  ...
  TestHostThreadBundle thread_bundle_;
  std::unique_ptr<media::AudioManager> audio_manager_;
  std::unique_ptr<media::AudioSystem> audio_system_;
  std::unique_ptr<TestApplicationContents> application_contents_;
...
TEST_F(MediaStreamDispatcherHostTest, GenerateStreamWithDepthVideo) {
  ...
  std::string source_id = GetHMACForMediaDeviceID(
      application_contents_->GetMediaDeviceIDSalt(), origin_, kDepthVideoDeviceId);
...
TEST_F(MediaStreamDispatcherHostTest, GenerateStreamsWithSourceId) {
  ...
  for (; audio_it != audio_device_descriptions_.end(); ++audio_it) {
    std::string source_id = GetHMACForMediaDeviceID(
        application_contents_->GetMediaDeviceIDSalt(), origin_, audio_it->unique_id);
  ...
  for (const std::string& device_id : stub_video_device_ids_) {
    std::string source_id = GetHMACForMediaDeviceID(
        application_contents_->GetMediaDeviceIDSalt(), origin_, device_id);
    ASSERT_FALSE(source_id.empty());
    StreamControls controls(true, true);
    controls.video.device_id = source_id;
