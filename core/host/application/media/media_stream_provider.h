...
#ifndef MUMBA_HOST_APPLICATION_MEDIA_MEDIA_STREAM_PROVIDER_H_
#define MUMBA_HOST_APPLICATION_MEDIA_MEDIA_STREAM_PROVIDER_H_
...
class CONTENT_EXPORT MediaStreamProviderListener {
 public:
  ...
  virtual void Opened(common::MediaStreamType stream_type, int capture_session_id) = 0;

  // Called by a MediaStreamProvider when a stream has been closed.
  virtual void Closed(common::MediaStreamType stream_type, int capture_session_id) = 0;

  // Called by a MediaStreamProvider when the device has been aborted due to
  // device error.
  virtual void Aborted(common::MediaStreamType stream_type, int capture_session_id) = 0;
...
class CONTENT_EXPORT MediaStreamProvider
 ...
 public:
  ...
  virtual int Open(const common::MediaStreamDevice& device) = 0;
...
#endif  // MUMBA_HOST_APPLICATION_MEDIA_MEDIA_STREAM_PROVIDER_H_
