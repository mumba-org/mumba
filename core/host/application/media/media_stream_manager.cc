...
#include "core/host/application/media/in_process_video_capture_provider.h"
#include "core/host/application/media/media_capture_devices_impl.h"
#include "core/host/application/media/media_devices_manager.h"
//#include "core/host/application/media/media_stream_ui_proxy.h"
#include "core/host/application/media/service_video_capture_provider.h"
#include "core/host/application/media/video_capture_manager.h"
#include "core/host/application/media/video_capture_provider_switcher.h"
...
void ParseStreamType(const common::StreamControls& controls,
                     common::MediaStreamType* audio_type,
                     common::MediaStreamType* video_type) {
  *audio_type = common::MEDIA_NO_SERVICE;
  *video_type = common::MEDIA_NO_SERVICE;
  const bool audio_support_flag_for_desktop_share = true;
  ...
  if (controls.audio.requested) {
    if (!controls.audio.stream_source.empty()) {
       ...
       if (controls.audio.stream_source == common::kMediaStreamSourceTab) {
         *audio_type = common::MEDIA_TAB_AUDIO_CAPTURE;
       } else if (controls.audio.stream_source == common::kMediaStreamSourceSystem) {
         *audio_type = common::MEDIA_DESKTOP_AUDIO_CAPTURE;
       } else if (audio_support_flag_for_desktop_share &&
                  controls.audio.stream_source == common::kMediaStreamSourceDesktop) {
         *audio_type = common::MEDIA_DESKTOP_AUDIO_CAPTURE;
       }
     } else {
       // This is normal audio device capture.
       *audio_type = common::MEDIA_DEVICE_AUDIO_CAPTURE;
     }
  }
  if (controls.video.requested) {
    if (!controls.video.stream_source.empty()) {
      // This is tab or screen capture.
      if (controls.video.stream_source == common::kMediaStreamSourceTab) {
        *video_type = common::MEDIA_TAB_VIDEO_CAPTURE;
      } else if (controls.video.stream_source == common::kMediaStreamSourceScreen) {
        *video_type = common::MEDIA_DESKTOP_VIDEO_CAPTURE;
      } else if (controls.video.stream_source == common::kMediaStreamSourceDesktop) {
        *video_type = common::MEDIA_DESKTOP_VIDEO_CAPTURE;
      }
    } else {
      // This is normal video device capture.
      *video_type = common::MEDIA_DEVICE_VIDEO_CAPTURE;
    }
  }
}
...
void FilterAudioEffects(const common::StreamControls& controls, int* effects) {
  DCHECK(effects);
...
void EnableHotwordEffect(const common::StreamControls& controls, int* effects) {
  DCHECK(effects);
...
bool CalledOnIOThread() {
  ...
  return HostThread::CurrentlyOn(HostThread::IO) ||
         !HostThread::IsThreadInitialized(HostThread::IO);
}

bool GetDeviceIDFromHMAC(const std::string& salt,
                         const url::Origin& security_origin,
                         const std::string& hmac_device_id,
                         const common::MediaDeviceInfoArray& devices,
...
common::MediaStreamType ConvertToMediaStreamType(common::MediaDeviceType type) {
  switch (type) {
    case common::MEDIA_DEVICE_TYPE_AUDIO_INPUT:
      return common::MEDIA_DEVICE_AUDIO_CAPTURE;
    case common::MEDIA_DEVICE_TYPE_VIDEO_INPUT:
      return common::MEDIA_DEVICE_VIDEO_CAPTURE;
    default:
      NOTREACHED();
  }

  return common::MEDIA_NO_SERVICE;
}

common::MediaDeviceType ConvertToMediaDeviceType(common::MediaStreamType stream_type) {
  switch (stream_type) {
    case common::MEDIA_DEVICE_AUDIO_CAPTURE:
      return common::MEDIA_DEVICE_TYPE_AUDIO_INPUT;
    case common::MEDIA_DEVICE_VIDEO_CAPTURE:
      return common::MEDIA_DEVICE_TYPE_VIDEO_INPUT;
    default:
      NOTREACHED();
  }

  return common::NUM_MEDIA_DEVICE_TYPES;
}
...
class MediaStreamManager::DeviceRequest {
 public:
  DeviceRequest(
      ...
      const url::Origin& security_origin,
      bool user_gesture,
      common::MediaStreamRequestType request_type,
      ...
      : requesting_process_id(requesting_process_id),
        ...
        state_(common::NUM_MEDIA_TYPES, MEDIA_REQUEST_STATE_NOT_REQUESTED),
        audio_type_(common::MEDIA_NO_SERVICE),
        video_type_(common::MEDIA_NO_SERVICE),
        target_process_id_(-1),
        target_frame_id_(-1) {}

  ~DeviceRequest() { RunMojoCallbacks(); }

  void SetAudioType(common::MediaStreamType audio_type) {
    DCHECK(IsAudioInputMediaType(audio_type) ||
           audio_type == common::MEDIA_NO_SERVICE);
    audio_type_ = audio_type;
  }

  common::MediaStreamType audio_type() const { return audio_type_; }

  void SetVideoType(common::MediaStreamType video_type) {
    DCHECK(IsVideoMediaType(video_type) || video_type == common::MEDIA_NO_SERVICE);
    video_type_ = video_type;
  }

  common::MediaStreamType video_type() const { return video_type_; }
  ...
  void CreateUIRequest(const std::string& requested_audio_device_id,
    ...
    ui_request_.reset(new common::MediaStreamRequest(
  ...
  std::unique_ptr<common::MediaStreamRequest> DetachUIRequest() {
    return std::move(ui_request_);
  }

  // Update the request state and notify observers.
  void SetState(common::MediaStreamType stream_type, MediaRequestState new_state) {
    if (stream_type == common::NUM_MEDIA_TYPES) {
      for (int i = common::MEDIA_NO_SERVICE + 1; i < common::NUM_MEDIA_TYPES; ++i) {
        state_[static_cast<common::MediaStreamType>(i)] = new_state;
      }
    } else {
    ...
    if (stream_type == common::NUM_MEDIA_TYPES) {
      for (int i = common::MEDIA_NO_SERVICE + 1; i < common::NUM_MEDIA_TYPES; ++i) {
        media_observer->OnMediaRequestStateChanged(
            ...
            security_origin.GetURL(), static_cast<common::MediaStreamType>(i),
            new_state);
      }
    } else {
      media_observer->OnMediaRequestStateChanged(
  ...
  MediaRequestState state(common::MediaStreamType stream_type) const {
  ...
  void RunMojoCallbacks() {
    if (generate_stream_cb) {
      std::move(generate_stream_cb)
          .Run(common::MEDIA_DEVICE_FAILED_DUE_TO_SHUTDOWN, std::string(),
               common::MediaStreamDevices(), common::MediaStreamDevices());
    }

    if (open_device_cb) {
      std::move(open_device_cb)
          .Run(false /* success */, std::string(), common::MediaStreamDevice());
    }
  }
  ...
  const common::MediaStreamRequestType request_type;
...
//  std::unique_ptr<MediaStreamUIProxy> ui_proxy;
 ...
 private:
  ...
  std::unique_ptr<common::MediaStreamRequest> ui_request_;
  common::MediaStreamType audio_type_;
  common::MediaStreamType video_type_;
...
std::string MediaStreamManager::MakeMediaAccessRequest(
  ...
  DeviceRequest* request = new DeviceRequest(
      ...
      false,  // user gesture
      common::MEDIA_DEVICE_ACCESS, controls, std::string());
...
void MediaStreamManager::GenerateStream(
  ...
  DeviceRequest* request =
      new DeviceRequest(render_process_id, render_frame_id, page_request_id,
                        security_origin, user_gesture, common::MEDIA_GENERATE_STREAM,
  ...
  if (generate_stream_test_callback_) {
    ...
    if (generate_stream_test_callback_.Run(controls)) {
      ...
      FinalizeRequestFailed(label, request, common::MEDIA_DEVICE_INVALID_STATE);
    }
    return;
  }
...
void MediaStreamManager::CancelRequest(const std::string& label) {
  ...
  for (const common::MediaStreamDevice& device : request->devices) {
  ...
  request->SetState(common::NUM_MEDIA_TYPES, MEDIA_REQUEST_STATE_CLOSING);
  DeleteRequest(label);
}
...
void MediaStreamManager::StopStreamDevice(int render_process_id,
  ...
  for (const LabeledDeviceRequest& device_request : requests_) {
    ...
    if (request->requesting_process_id != render_process_id ||
        request->requesting_frame_id != render_frame_id ||
        request->request_type != common::MEDIA_GENERATE_STREAM) {
      continue;
    }
...
int MediaStreamManager::VideoDeviceIdToSessionId(
  ...
  for (const LabeledDeviceRequest& device_request : requests_) {
    for (const common::MediaStreamDevice& device : device_request.second->devices) {
      if (device.id == device_id && device.type == common::MEDIA_DEVICE_VIDEO_CAPTURE) {
        return device.session_id;
      }
    }
  }
  return common::MediaStreamDevice::kNoId;
}

void MediaStreamManager::StopDevice(common::MediaStreamType type, int session_id) {
  DVLOG(1) << "StopDevice"
           << "{type = " << type << "}"
  ...
  while (request_it != requests_.end()) {
    ...
    common::MediaStreamDevices* devices = &request->devices;
    if (devices->empty()) {
      // There is no device in use yet by this request.
      ++request_it;
      continue;
    }
    common::MediaStreamDevices::iterator device_it = devices->begin();
...
void MediaStreamManager::CloseDevice(common::MediaStreamType type, int session_id) {
  DVLOG(1) << "CloseDevice("
           << "{type = " << type <<  "} "
           << "{session_id = " << session_id << "})";
...
void MediaStreamManager::OpenDevice(int render_process_id,
                                    ...
                                    DeviceStoppedCallback device_stopped_cb) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  DCHECK(type == common::MEDIA_DEVICE_AUDIO_CAPTURE ||
         type == common::MEDIA_DEVICE_VIDEO_CAPTURE);
  DVLOG(1) << "OpenDevice ({page_request_id = " << page_request_id <<  "})";
  common::StreamControls controls;
  ...
  DeviceRequest* request = new DeviceRequest(
      render_process_id, render_frame_id, page_request_id, security_origin,
      false,  // user gesture
      common::MEDIA_OPEN_DEVICE_PEPPER_ONLY, controls, salt,
...
bool MediaStreamManager::TranslateSourceIdToDeviceId(
  ...
  DCHECK(stream_type == common::MEDIA_DEVICE_AUDIO_CAPTURE ||
         stream_type == common::MEDIA_DEVICE_VIDEO_CAPTURE);
  // The source_id can be empty if the constraint is set but empty.
  if (source_id.empty())
    return false;
  ...
  common::MediaDeviceInfoArray cached_devices =
...
void MediaStreamManager::StopRemovedDevice(
    ...
    const common::MediaDeviceInfo& media_device_info) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  DCHECK(type == common::MEDIA_DEVICE_TYPE_AUDIO_INPUT ||
         type == common::MEDIA_DEVICE_TYPE_VIDEO_INPUT);

  common::MediaStreamType stream_type = ConvertToMediaStreamType(type);
  ...
  AddLogMessageOnIOThread(
      base::StringPrintf(
          ...
          (stream_type == common::MEDIA_DEVICE_AUDIO_CAPTURE ? "audio" : "video"),
...
void MediaStreamManager::TranslateDeviceIdToSourceId(
    DeviceRequest* request,
    common::MediaStreamDevice* device) {
  if (request->audio_type() == common::MEDIA_DEVICE_AUDIO_CAPTURE ||
      request->video_type() == common::MEDIA_DEVICE_VIDEO_CAPTURE) {
...
void MediaStreamManager::StartEnumeration(DeviceRequest* request,
  ...
  bool request_audio_input = request->audio_type() != common::MEDIA_NO_SERVICE;
  if (request_audio_input)
    request->SetState(request->audio_type(), MEDIA_REQUEST_STATE_REQUESTED);

  bool request_video_input = request->video_type() != common::MEDIA_NO_SERVICE;
  ...
  devices_to_enumerate[common::MEDIA_DEVICE_TYPE_AUDIO_INPUT] = request_audio_input;
  devices_to_enumerate[common::MEDIA_DEVICE_TYPE_VIDEO_INPUT] = request_video_input;
...
void MediaStreamManager::PostRequestToUI(
  ...
  DLOG(ERROR) << "MediaStreamManager::PostRequestToUI: We disabled it.. check";
  ...
  const common::MediaStreamType audio_type = request->audio_type();
  const common::MediaStreamType video_type = request->video_type();
  ...
  // if (fake_ui_factory_ &&
  //     request->video_type() != common::MEDIA_DESKTOP_VIDEO_CAPTURE) {
  //   common::MediaStreamDevices devices = ConvertToMediaStreamDevices(
  //       request->audio_type(), enumeration[common::MEDIA_DEVICE_TYPE_AUDIO_INPUT]);
  //   common::MediaStreamDevices video_devices = ConvertToMediaStreamDevices(
  //       request->video_type(), enumeration[common::MEDIA_DEVICE_TYPE_VIDEO_INPUT]);
...
void MediaStreamManager::SetupRequest(const std::string& label) {
  ...
  common::MediaStreamType audio_type = common::MEDIA_NO_SERVICE;
  common::MediaStreamType video_type = common::MEDIA_NO_SERVICE;
  ParseStreamType(request->controls, &audio_type, &video_type);
  request->SetAudioType(audio_type);
  request->SetVideoType(video_type);

  const bool is_web_contents_capture = audio_type == common::MEDIA_TAB_AUDIO_CAPTURE ||
                                       video_type == common::MEDIA_TAB_VIDEO_CAPTURE;
  if (is_web_contents_capture && !SetupTabCaptureRequest(request)) {
    FinalizeRequestFailed(label,
                          request,
                          common::MEDIA_DEVICE_TAB_CAPTURE_FAILURE);
    return;
  }

  const bool is_screen_capture = video_type == common::MEDIA_DESKTOP_VIDEO_CAPTURE;
  if (is_screen_capture && !SetupScreenCaptureRequest(request)) {
    FinalizeRequestFailed(label,
                          request,
                          common::MEDIA_DEVICE_SCREEN_CAPTURE_FAILURE);
    return;
  }

  if (!is_web_contents_capture && !is_screen_capture) {
    if (audio_type == common::MEDIA_DEVICE_AUDIO_CAPTURE ||
        video_type == common::MEDIA_DEVICE_VIDEO_CAPTURE) {
    ...
    if (!SetupDeviceCaptureRequest(request, MediaDeviceEnumeration())) {
      FinalizeRequestFailed(label, request, common::MEDIA_DEVICE_NO_HARDWARE);
      return;
    }
  }
...
bool MediaStreamManager::SetupDeviceCaptureRequest(
    DeviceRequest* request,
    const MediaDeviceEnumeration& enumeration) {
  DCHECK((request->audio_type() == common::MEDIA_DEVICE_AUDIO_CAPTURE ||
          request->audio_type() == common::MEDIA_NO_SERVICE) &&
         (request->video_type() == common::MEDIA_DEVICE_VIDEO_CAPTURE ||
          request->video_type() == common::MEDIA_NO_SERVICE));
  std::string audio_device_id;
  if (request->controls.audio.requested &&
      !GetRequestedDeviceCaptureId(request, request->audio_type(),
                                   enumeration[common::MEDIA_DEVICE_TYPE_AUDIO_INPUT],
                                   &audio_device_id)) {
    return false;
  }
  ...
  if (request->controls.video.requested &&
      !GetRequestedDeviceCaptureId(request, request->video_type(),
                                   enumeration[common::MEDIA_DEVICE_TYPE_VIDEO_INPUT],
                                   &video_device_id)) {
    return false;
  }
...
bool MediaStreamManager::SetupTabCaptureRequest(DeviceRequest* request) {
  DCHECK(request->audio_type() == common::MEDIA_TAB_AUDIO_CAPTURE ||
         request->video_type() == common::MEDIA_TAB_VIDEO_CAPTURE);
  ...
  ApplicationContentsMediaCaptureId web_id;
  bool has_valid_device_id =
      ApplicationContentsMediaCaptureId::Parse(capture_device_id, &web_id);
  if (!has_valid_device_id ||
      (request->audio_type() != common::MEDIA_TAB_AUDIO_CAPTURE &&
       request->audio_type() != common::MEDIA_NO_SERVICE) ||
      (request->video_type() != common::MEDIA_TAB_VIDEO_CAPTURE &&
       request->video_type() != common::MEDIA_NO_SERVICE)) {
    return false;
...
bool MediaStreamManager::SetupScreenCaptureRequest(DeviceRequest* request) {
  DCHECK(request->audio_type() == common::MEDIA_DESKTOP_AUDIO_CAPTURE ||
         request->video_type() == common::MEDIA_DESKTOP_VIDEO_CAPTURE);
  ...
  if (request->video_type() == common::MEDIA_DESKTOP_VIDEO_CAPTURE) {
    ...
    if (video_stream_source == common::kMediaStreamSourceDesktop &&
  ...
  const std::string audio_device_id =
      request->audio_type() == common::MEDIA_DESKTOP_AUDIO_CAPTURE ? video_device_id
                                                           : "";
...
common::MediaStreamDevices MediaStreamManager::GetDevicesOpenedByRequest(
...
bool MediaStreamManager::FindExistingRequestedDevice(
    ...
    const common::MediaStreamDevice& new_device,
    common::MediaStreamDevice* existing_device,
...
void MediaStreamManager::FinalizeGenerateStream(const std::string& label,
  ...
  common::MediaStreamDevices audio_devices, video_devices;
  for (const common::MediaStreamDevice& device : request->devices) {
  ...
  std::move(request->generate_stream_cb)
      .Run(common::MEDIA_DEVICE_OK, label, audio_devices, video_devices);
}
...
void MediaStreamManager::FinalizeRequestFailed(
  ...
  switch (request->request_type) {
    case common::MEDIA_GENERATE_STREAM: {
      DCHECK(request->generate_stream_cb);
      std::move(request->generate_stream_cb)
          .Run(result, std::string(), common::MediaStreamDevices(),
               common::MediaStreamDevices());
      break;
    }
    ...
    case common::MEDIA_DEVICE_ACCESS: {
      DLOG(ERROR) << "MediaStreamManager::FinalizeRequestFailed: MEDIA_DEVICE_ACCESS => Not implemented";
      DCHECK(request->media_access_request_cb);
      //std::move(request->media_access_request_cb)
      //    .Run(common::MediaStreamDevices(), std::move(request->ui_proxy));
      break;
    }
    default:
      NOTREACHED();
...
void MediaStreamManager::FinalizeMediaAccessRequest(
  ...
  DLOG(ERROR) << "MediaStreamManager::FinalizeMediaAccessRequest: fix/reimplement";
  DCHECK(request->media_access_request_cb);
//  std::move(request->media_access_request_cb)
...
void MediaStreamManager::Opened(common::MediaStreamType stream_type,
  ...
  for (const LabeledDeviceRequest& labeled_request : requests_) {
    ...
    for (common::MediaStreamDevice& device : request->devices) {
      if (device.type == stream_type &&
        ...
        if (IsAudioInputMediaType(device.type)) {
          ...
          if (device.type != common::MEDIA_TAB_AUDIO_CAPTURE) {
            const common::MediaStreamDevice* opened_device =
...
void MediaStreamManager::Closed(common::MediaStreamType stream_type,
                                int capture_session_id) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
}
...
void MediaStreamManager::DevicesEnumerated(
  ...
  common::MediaStreamType stream_types[] = {common::MEDIA_DEVICE_AUDIO_CAPTURE,
...
void MediaStreamManager::Aborted(common::MediaStreamType stream_type,
                                 int capture_session_id) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
...
void MediaStreamManager::HandleAccessRequestResponse(
  ...
  for (const common::MediaStreamDevice& media_stream_device : devices) {
    common::MediaStreamDevice device = media_stream_device;

    if (device.type == common::MEDIA_TAB_VIDEO_CAPTURE ||
        device.type == common::MEDIA_TAB_AUDIO_CAPTURE) {
    ...
    // initialized.
    if (device.type == common::MEDIA_TAB_AUDIO_CAPTURE ||
        device.type == common::MEDIA_DESKTOP_AUDIO_CAPTURE) {
    ...
    if (request->request_type == common::MEDIA_GENERATE_STREAM) {
...
void MediaStreamManager::NotifyDevicesChanged(
    ...
    const common::MediaDeviceInfoArray& devices) {
  DLOG(INFO) << "MediaStreamManager::NotifyDevicesChanged";
  DCHECK_CURRENTLY_ON(HostThread::IO);
  MediaObserver* media_observer =
      common::GetClient()->host()->GetMediaObserver();

  common::MediaStreamType stream_type = ConvertToMediaStreamType(device_type);
  common::MediaStreamDevices new_devices =
...
void MediaStreamManager::OnMediaStreamUIWindowId(
  ...
  if (video_type != common::MEDIA_DESKTOP_VIDEO_CAPTURE)
    return;

  // Pass along for desktop screen and window capturing when
  // DesktopCaptureDevice is used.
  for (const common::MediaStreamDevice& device : devices) {
    if (device.type != common::MEDIA_DESKTOP_VIDEO_CAPTURE)
      continue;
...
common::MediaStreamDevices MediaStreamManager::ConvertToMediaStreamDevices(
    common::MediaStreamType stream_type,
  ...
  if (stream_type != common::MEDIA_DEVICE_VIDEO_CAPTURE)
    return devices;
...
void MediaStreamManager::OnStreamStarted(const std::string& label) {
  DLOG(ERROR) << "MediaStreamManager::OnStreamStarted: we disable it. fix";
