...
#ifndef MUMBA_HOST_APPLICATION_MEDIA_MEDIA_STREAM_MANAGER_H_
#define MUMBA_HOST_APPLICATION_MEDIA_MEDIA_STREAM_MANAGER_H_
...
#include "core/host/application/media/media_stream_provider.h"
...
class CONTENT_EXPORT MediaStreamManager
 ...
 public:
  ...
  using MediaAccessRequestCallback =
      base::OnceCallback<void(const common::MediaStreamDevices& devices,
                              std::unique_ptr<MediaStreamUIProxy> ui)>;

  using GenerateStreamCallback =
      base::OnceCallback<void(common::MediaStreamRequestResult result,
  ...
  using GenerateStreamTestCallback =
      base::Callback<bool(const common::StreamControls&)>;
  ...
  // The methods must be called on HostThread::IO threads. The callbacks of
  // media::VideoCaptureObserver also arrive on HostThread::IO threads.
  ...
  bool TranslateSourceIdToDeviceId(common::MediaStreamType stream_type,
                                   const std::string& salt,
                                   const url::Origin& security_origin,
  ...
  void Opened(common::MediaStreamType stream_type, int capture_session_id) override;
  void Closed(common::MediaStreamType stream_type, int capture_session_id) override;
  void Aborted(common::MediaStreamType stream_type, int capture_session_id) override;
  ...
  common::MediaStreamDevices GetDevicesOpenedByRequest(const std::string& label) const;
  ...
  // using TestHostThreadBundle because the notification happens too late.
  ...
  void NotifyDevicesChanged(common::MediaDeviceType stream_type,
  ...
  void StopRemovedDevice(common::MediaDeviceType type,
 ...
 private:
  ...
  void StopDevice(common::MediaStreamType type, int session_id);
  // Calls the correct capture manager and close the device with |session_id|.
  // All requests that uses the device are updated.
  void CloseDevice(common::MediaStreamType type, int session_id);
  ...
  MediaStreamProvider* GetDeviceManager(common::MediaStreamType stream_type);
  ...
  void OnMediaStreamUIWindowId(common::MediaStreamType video_type,
                               const common::MediaStreamDevices& devices,
  ...
  common::MediaStreamDevices ConvertToMediaStreamDevices(
...
#endif  // MUMBA_HOST_APPLICATION_MEDIA_MEDIA_STREAM_MANAGER_H_
