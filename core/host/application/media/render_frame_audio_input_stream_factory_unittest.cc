...
#include "core/common/media/renderer_audio_input_stream_factory.mojom.h"
...
class RenderFrameAudioInputStreamFactoryTest : public testing::Test {
 public:
  ...
  ~RenderFrameAudioInputStreamFactoryTest() override {
    ...
    // Shutdown HostThread::IO before tearing down members.
    thread_bundle_.reset();
  }

  // |thread_bundle_| needs to be up before the members below (as they use
  // HostThreads for their initialization) but then needs to be torn down
  // before them as some verify they're town down in a single-threaded
  // environment (while
  // !HostThread::IsThreadInitiaslized(HostThread::IO)).
  base::Optional<TestHostThreadBundle> thread_bundle_;
