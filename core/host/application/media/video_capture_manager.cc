...
int VideoCaptureManager::Open(const common::MediaStreamDevice& device) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
...
void VideoCaptureManager::ProcessDeviceStartRequestQueue() {
  ...
  if (controller->stream_type() == common::MEDIA_DEVICE_VIDEO_CAPTURE) {
    const media::VideoCaptureDeviceInfo* device_info =
        GetDeviceInfoById(controller->device_id());
    if (!device_info) {
...
void VideoCaptureManager::OnDeviceLaunched(VideoCaptureController* controller) {
  ...
  if (controller->stream_type() == common::MEDIA_DESKTOP_VIDEO_CAPTURE) {
...
void VideoCaptureManager::DisconnectClient(
  ...
  if (!aborted_due_to_error) {
    ...
    } else if (controller->stream_type() == common::MEDIA_DEVICE_VIDEO_CAPTURE) {
...
VideoCaptureManager::GetDeviceFormatInUse(common::MediaStreamType stream_type,
                                          const std::string& device_id) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
...
void VideoCaptureManager::MaybePostDesktopCaptureWindowId(
  ...
  DCHECK_EQ(common::MEDIA_DESKTOP_VIDEO_CAPTURE, existing_device->stream_type());
...
VideoCaptureController* VideoCaptureManager::GetOrCreateController(
  ...
  if (session_it == sessions_.end())
    return nullptr;
  const common::MediaStreamDevice& device_info = session_it->second;
...
base::Optional<common::CameraCalibration> VideoCaptureManager::GetCameraCalibration(
    const std::string& device_id) {
  media::VideoCaptureDeviceInfo* info = GetDeviceInfoById(device_id);
  if (!info)
    return base::Optional<common::CameraCalibration>();
