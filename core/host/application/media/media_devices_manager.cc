...
std::string GetLogMessageString(common::MediaDeviceType device_type,
...
common::MediaDeviceInfoArray GetFakeAudioDevices(bool is_input) {
...
std::string GuessVideoGroupID(const common::MediaDeviceInfoArray& audio_infos,
                              const common::MediaDeviceInfo& video_info) {
  ...
  std::function<bool(const common::MediaDeviceInfo&)>
      video_label_is_included_in_audio_label =
          [&video_label](const common::MediaDeviceInfo& audio_info) {
  ...
  std::string video_usb_model = video_has_usb_model
                                    ...
                                    : std::string();
  std::function<bool(const common::MediaDeviceInfo&)> usb_model_matches =
      [video_has_usb_model,
       &video_usb_model](const common::MediaDeviceInfo& audio_info) {
  ...
  for (auto* lambda :
    ...
    // find unique matches.
    auto real_device_matches = [lambda](const common::MediaDeviceInfo& audio_info) {
...
MediaDevicesManager::MediaDevicesManager(
    ...
    : use_fake_devices_(base::CommandLine::ForCurrentProcess()->HasSwitch(
      ...
      cache_infos_(common::NUM_MEDIA_DEVICE_TYPES),
...
void MediaDevicesManager::EnumerateDevices(
  ...
  for (size_t i = 0; i < common::NUM_MEDIA_DEVICE_TYPES; ++i) {
    if (requested_types[i] && cache_policies_[i] == CachePolicy::NO_CACHE) {
      all_results_cached = false;
      DoEnumerateDevices(static_cast<common::MediaDeviceType>(i));
    }
  }
...
uint32_t MediaDevicesManager::SubscribeDeviceChangeNotifications(
  ...
  media_devices_listener.set_connection_error_handler(
      ...
      // base::BindOnce(&MediaDevicesManager::UnsubscribeDeviceChangeNotifications,
      //                base::Unretained(this), subscription_id));
...
void MediaDevicesManager::SetCachePolicy(common::MediaDeviceType type,
                                         CachePolicy policy) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
...
void MediaDevicesManager::StartMonitoring() {
  ...
  if (base::FeatureList::IsEnabled(features::kMediaDevicesSystemMonitorCache)) {
    for (size_t i = 0; i < common::NUM_MEDIA_DEVICE_TYPES; ++i) {
      ...
      SetCachePolicy(static_cast<common::MediaDeviceType>(i),
  ...
  SetCachePolicy(common::MEDIA_DEVICE_TYPE_VIDEO_INPUT, CachePolicy::NO_CACHE);
  HostThread::PostTask(
...
void MediaDevicesManager::StartMonitoringOnUIThread() {
  ...
  if (!host_main_loop)
    return;
  host_main_loop->device_monitor_mac()->StartMonitoring();
}
#endif
...
void MediaDevicesManager::StopMonitoring() {
  ...
  monitoring_started_ = false;
  for (size_t i = 0; i < common::NUM_MEDIA_DEVICE_TYPES; ++i)
    SetCachePolicy(static_cast<common::MediaDeviceType>(i), CachePolicy::NO_CACHE);
}
...
void MediaDevicesManager::OnDevicesChanged(
  ...
  switch (device_type) {
    case base::SystemMonitor::DEVTYPE_AUDIO:
      HandleDevicesChanged(common::MEDIA_DEVICE_TYPE_AUDIO_INPUT);
      HandleDevicesChanged(common::MEDIA_DEVICE_TYPE_AUDIO_OUTPUT);
      break;
    case base::SystemMonitor::DEVTYPE_VIDEO_CAPTURE:
      HandleDevicesChanged(common::MEDIA_DEVICE_TYPE_VIDEO_INPUT);
      break;
    default:
...
media::VideoCaptureFormats MediaDevicesManager::GetVideoInputFormats(
  ...
  if (try_in_use_first) {
    base::Optional<media::VideoCaptureFormat> format =
        video_capture_manager_->GetDeviceFormatInUse(common::MEDIA_DEVICE_VIDEO_CAPTURE,
                                                     device_id);
    if (format.has_value()) {
...
common::MediaDeviceInfoArray MediaDevicesManager::GetCachedDeviceInfo(
...
void MediaDevicesManager::CheckPermissionsForEnumerateDevices(
  ...
  permission_checker_->CheckPermissions(
      ...
      // base::BindOnce(&MediaDevicesManager::OnPermissionsCheckDone,
      //                base::Unretained(this), group_id_salt_base,
      //                requested_types, request_video_input_capabilities,
      //                std::move(callback), salt_and_origin.first,
      //                salt_and_origin.second));
}
...
void MediaDevicesManager::OnDevicesEnumerated(
  ...
  const bool video_input_capabilities_requested =
      has_permissions[common::MEDIA_DEVICE_TYPE_VIDEO_INPUT] &&
      request_video_input_capabilities;

  common::MediaDeviceInfoArray video_device_infos =
  ...
  for (auto& video_device_info : video_device_infos) {
    video_device_info.group_id = GuessVideoGroupID(
        enumeration[common::MEDIA_DEVICE_TYPE_AUDIO_INPUT], video_device_info);
  }

  std::vector<common::MediaDeviceInfoArray> result(common::NUM_MEDIA_DEVICE_TYPES);
  for (size_t i = 0; i < common::NUM_MEDIA_DEVICE_TYPES; ++i) {
    if (!requested_types[i])
      continue;

    if (i == common::MEDIA_DEVICE_TYPE_VIDEO_INPUT) {
      for (const auto& device_info : video_device_infos) {
        common::MediaDeviceInfo translated_device_info = TranslateMediaDeviceInfo(
  ...
  std::move(callback).Run(
      ...
      video_input_capabilities_requested
          ? ComputeVideoInputCapabilities(result[common::MEDIA_DEVICE_TYPE_VIDEO_INPUT])
...
void MediaDevicesManager::DoEnumerateDevices(common::MediaDeviceType type) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  DCHECK(IsValidMediaDeviceType(type));
  ...
  switch (type) {
    ...
    case common::MEDIA_DEVICE_TYPE_VIDEO_INPUT:
      video_capture_manager_->EnumerateDevices(
          ...
          //           base::Unretained(this)));
      break;
    case common::MEDIA_DEVICE_TYPE_AUDIO_OUTPUT:
...
void MediaDevicesManager::EnumerateAudioDevices(bool is_input) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  common::MediaDeviceType type =
      is_input ? common::MEDIA_DEVICE_TYPE_AUDIO_INPUT : common::MEDIA_DEVICE_TYPE_AUDIO_OUTPUT;
...
void MediaDevicesManager::VideoInputDevicesEnumerated(
  ...
  DCHECK_CURRENTLY_ON(HostThread::IO);
  common::MediaDeviceInfoArray snapshot;
  for (const auto& descriptor : descriptors) {
    snapshot.emplace_back(descriptor);
  }
  DevicesEnumerated(common::MEDIA_DEVICE_TYPE_VIDEO_INPUT, snapshot);
}
...
void MediaDevicesManager::DevicesEnumerated(
    common::MediaDeviceType type,
    const common::MediaDeviceInfoArray& snapshot) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  DCHECK(IsValidMediaDeviceType(type));
...
void MediaDevicesManager::HandleDevicesChanged(common::MediaDeviceType type) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  DCHECK(IsValidMediaDeviceType(type));
  cache_infos_[type].InvalidateCache();
  DoEnumerateDevices(type);
...
void MediaDevicesManager::NotifyMediaStreamManager(
  ...
  for (const auto& old_device_info : current_snapshot_[type]) {
    auto it = std::find_if(new_snapshot.begin(), new_snapshot.end(),
                           [&old_device_info](const common::MediaDeviceInfo& info) {
...
void MediaDevicesManager::NotifyDeviceChangeSubscribers(
  ...
  for (auto& subscription : subscriptions_) {
    ...
    if (request.subscribe_types[type]) {
      base::PostTaskAndReplyWithResult(
          HostThread::GetTaskRunnerForThread(HostThread::UI).get(),
          FROM_HERE,
          ...
          base::BindOnce(&MediaDevicesManager::CheckPermissionForDeviceChange,
                         ...
                         //base::Unretained(this), subscription.first,
...
void MediaDevicesManager::CheckPermissionForDeviceChange(
  ...
  permission_checker_->CheckPermission(
      ...
      base::BindOnce(&MediaDevicesManager::NotifyDeviceChange,
                     ...
                     //base::Unretained(this), subscription_id, type,
