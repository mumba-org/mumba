...
void NotifyProcessHostStreamAdded(int render_process_id) {
  ...
  auto* process_host = ApplicationProcessHost::FromID(render_process_id);
  if (process_host)
...
//     DLOG(INFO) << "\n\nAudioInputDelegateImpl::ControllerEventHandler::OnCreated\n\n";
//     HostThread::PostTask(
...
//     DLOG(INFO) << "\n\nAudioInputDelegateImpl::ControllerEventHandler::OnError\n\n";
...
//     DLOG(INFO) << "\n\nAudioInputDelegateImpl::ControllerEventHandler::OnMuted\n\n";
...
class AudioInputDelegateImpl::ControllerEventHandler
 ...
 public:
  ControllerEventHandler(int stream_id,
                         AudioInputDelegateImpl* delegate)
      : stream_id_(stream_id), delegate_(delegate) {}
  ...
  void OnCreated(bool initially_muted) override {
    ...
    HostThread::PostTask(
        HostThread::IO, FROM_HERE,
        base::BindOnce(&AudioInputDelegateImpl::SendCreatedNotification,
                       base::Unretained(delegate_), initially_muted));
  }
  ...
  void OnError(media::AudioInputController::ErrorCode error_code) override {
    ...
    HostThread::PostTask(
        HostThread::IO, FROM_HERE,
        base::BindOnce(&AudioInputDelegateImpl::OnError, base::Unretained(delegate_)));
  }
  ...
  void OnMuted(bool is_muted) override {
    ...
    HostThread::PostTask(HostThread::IO, FROM_HERE,
                            base::BindOnce(&AudioInputDelegateImpl::OnMuted,
                                           base::Unretained(delegate_), is_muted));
  }
...
std::unique_ptr<media::AudioInputDelegate> AudioInputDelegateImpl::Create(
  ...
  DLOG(INFO) << "AudioInputDelegateImpl::Create\n";
  DCHECK_CURRENTLY_ON(HostThread::UI);
...
AudioInputDelegateImpl::AudioInputDelegateImpl(
    ...
    : subscriber_(subscriber),
      ...
      weak_factory_(this) {
  DLOG(INFO) << "AudioInputDelegateImpl: contructor\n";
  DCHECK_CURRENTLY_ON(HostThread::UI);
  ...
  controller_event_handler_ = std::make_unique<ControllerEventHandler>(
      stream_id_, this);//weak_factory_.GetWeakPtr());

  const std::string& device_id = device->id;

  if (ApplicationContentsMediaCaptureId::Parse(device_id, nullptr)) {
    ...
    controller_ = media::AudioInputController::CreateForStream(
        ...
        ApplicationContentsAudioInputStream::Create(
    ...
    if (device->type == common::MEDIA_DESKTOP_AUDIO_CAPTURE)
...
AudioInputDelegateImpl::~AudioInputDelegateImpl() {
  DLOG(INFO) << "~AudioInputDelegateImpl\n";
  DCHECK_CURRENTLY_ON(HostThread::UI);
  ...
  DLOG(INFO) << "~AudioInputDelegateImpl: end";
}
...
void AudioInputDelegateImpl::SendCreatedNotification(bool initially_muted) {
  DLOG(INFO) << "\n\nAudioInputDelegateImpl::SendCreatedNotification\n\n";
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
