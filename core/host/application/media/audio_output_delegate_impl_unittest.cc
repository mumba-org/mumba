...
class AudioOutputDelegateTest : public testing::Test {
 public:
  AudioOutputDelegateTest() {
    ...
    thread_bundle_ = std::make_unique<TestHostThreadBundle>(
        TestHostThreadBundle::Options::REAL_IO_THREAD);
  ...
  void CreateTest(base::Closure done) {
    ...
    SyncWithAllThreads();
    HostThread::PostTask(HostThread::UI, FROM_HERE, std::move(done));
  }
 ...
 private:
  void SyncWithAllThreads() {
    ...
    for (int i = 0; i < 20; ++i) {
      ...
      SyncWith(HostThread::GetTaskRunnerForThread(HostThread::UI));
