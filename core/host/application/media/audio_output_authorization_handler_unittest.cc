...
// TestApplicationContents has a URLRequestContextGetter which uses a NullTaskRunner.
// This causes it to be destroyed on the wrong thread. This ApplicationContents
// instead uses the IO thread task runner for the URLRequestContextGetter.
class TestApplicationContentsWithRealURLRequestContextGetter
    : public TestApplicationContents {
 public:
  TestApplicationContentsWithRealURLRequestContextGetter() {
    request_context_ =
        base::MakeRefCounted<net::TrivialURLRequestContextGetter>(
            &context_,
            HostThread::GetTaskRunnerForThread(HostThread::IO));
    salt_ = TestApplicationContents::GetMediaDeviceIDSalt();
  }

  ~TestApplicationContentsWithRealURLRequestContextGetter() override {}
 ...
 public:
  AudioOutputAuthorizationHandlerTest()
      : RenderViewHostTestHarness(
            content::TestHostThreadBundle::REAL_IO_THREAD) {
  ...
  ApplicationContents* CreateApplicationContents() override {
    // Caller takes ownership.
    return new TestApplicationContentsWithRealURLRequestContextGetter();
  }
 ...
 protected:
  ...
  void SyncWithAllThreads() {
    ...
    for (int i = 0; i < 20; ++i) {
      ...
      SyncWith(HostThread::GetTaskRunnerForThread(HostThread::IO));
...
TEST_F(AudioOutputAuthorizationHandlerTest, AuthorizeDefaultDevice_Ok) {
  ...
  SyncWithAllThreads();
  HostThread::DeleteSoon(HostThread::IO, FROM_HERE, handler.release());
  SyncWithAllThreads();
}
...
TEST_F(AudioOutputAuthorizationHandlerTest,
  ...
  std::string hashed_id = MediaStreamManager::GetHMACForMediaDeviceID(
      application_contents()->GetMediaDeviceIDSalt(), SecurityOrigin(),
      raw_nondefault_id);
...
TEST_F(AudioOutputAuthorizationHandlerTest,
  ...
  std::string raw_nondefault_id = GetRawNondefaultId();
  std::string hashed_id = MediaStreamManager::GetHMACForMediaDeviceID(
      application_contents()->GetMediaDeviceIDSalt(), origin, raw_nondefault_id);
  MockAuthorizationCallback listener;
  std::unique_ptr<AudioOutputAuthorizationHandler> handler =
      std::make_unique<AudioOutputAuthorizationHandler>(
          GetAudioSystem(), GetMediaStreamManager(), process()->GetID());
...
TEST_F(AudioOutputAuthorizationHandlerTest,
  ...
  auto* context =
      static_cast<TestApplicationContentsWithRealURLRequestContextGetter*>(
