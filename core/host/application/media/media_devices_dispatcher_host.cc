...
void MediaDevicesDispatcherHost::Create(
  ...
  mojo::MakeStrongBinding(
      std::make_unique<MediaDevicesDispatcherHost>(
          app_process_id, app_window_id, media_stream_manager),
      std::move(request));
}
...
MediaDevicesDispatcherHost::MediaDevicesDispatcherHost(
    ...
    : app_process_id_(app_process_id),
      app_window_id_(app_window_id),
      group_id_salt_base_(0),//ApplicationContents::CreateRandomMediaDeviceIDSalt()),
...
void MediaDevicesDispatcherHost::EnumerateDevices(
  ...
  if (!request_audio_input && !request_video_input && !request_audio_output) {
    bad_message::ReceivedBadMessage(
        app_process_id_, bad_message::MDDH_INVALID_DEVICE_TYPE_REQUEST);
    return;
  }
  ...
  devices_to_enumerate[common::MEDIA_DEVICE_TYPE_AUDIO_OUTPUT] = request_audio_output;

  media_stream_manager_->media_devices_manager()->EnumerateDevices(
      app_process_id_, app_window_id_, group_id_salt_base_,
...
void MediaDevicesDispatcherHost::AddMediaDevicesListener(
  ...
  devices_to_subscribe[common::MEDIA_DEVICE_TYPE_AUDIO_INPUT] = subscribe_audio_input;
  devices_to_subscribe[common::MEDIA_DEVICE_TYPE_VIDEO_INPUT] = subscribe_video_input;
  devices_to_subscribe[common::MEDIA_DEVICE_TYPE_AUDIO_OUTPUT] = subscribe_audio_output;
...
void MediaDevicesDispatcherHost::GetDefaultVideoInputDeviceID(
  ...
  GetDefaultMediaDeviceID(
      common::MEDIA_DEVICE_TYPE_VIDEO_INPUT, app_process_id_, app_window_id_,
      ...
      //base::Bind(&MediaDevicesDispatcherHost::GotDefaultVideoInputDeviceID,
      //           base::Unretained(this), base::Passed(&client_callback),
      //           salt_and_origin.first, salt_and_origin.second));
}
...
void MediaDevicesDispatcherHost::GetVideoInputDeviceFormats(
  ...
  base::PostTaskAndReplyWithResult(
      ...
      //base::BindOnce(
      //    &MediaDevicesDispatcherHost::EnumerateVideoDevicesForFormats,
      //    base::Unretained(this), std::move(client_callback), device_id,
      //    try_in_use_first));
}
...
void MediaDevicesDispatcherHost::EnumerateVideoDevicesForFormats(
  ...
  //media_stream_manager_->video_capture_manager()->EnumerateDevices(base::Bind(
  //    &MediaDevicesDispatcherHost::FinalizeGetVideoInputDeviceFormats,
  //    base::Unretained(this), base::Passed(&client_callback), device_id,
  //    try_in_use_first, salt_and_origin.first, salt_and_origin.second));
}
...
void MediaDevicesDispatcherHost::GetDefaultAudioInputDeviceID(
  ...
  GetDefaultMediaDeviceID(
      common::MEDIA_DEVICE_TYPE_AUDIO_INPUT, app_process_id_, app_window_id_,
...
void MediaDevicesDispatcherHost::GotAudioInputEnumeration(
  ...
  for (const auto& device_info : enumeration[common::MEDIA_DEVICE_TYPE_AUDIO_INPUT]) {
