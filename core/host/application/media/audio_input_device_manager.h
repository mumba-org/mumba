...
#ifndef MUMBA_HOST_APPLICATION_MEDIA_AUDIO_INPUT_DEVICE_MANAGER_H_
#define MUMBA_HOST_APPLICATION_MEDIA_AUDIO_INPUT_DEVICE_MANAGER_H_
...
class CONTENT_EXPORT AudioInputDeviceManager : public MediaStreamProvider {
 public:
  ...
  const common::MediaStreamDevice* GetOpenedDeviceById(int session_id);
 ...
 private:
  ...
  // device.
  void ClosedOnIOThread(common::MediaStreamType type, int session_id);

  // Helper to return iterator to the device referenced by |session_id|. If no
  // device is found, it will return devices_.end().
  common::MediaStreamDevices::iterator GetDevice(int session_id);
...
#endif  // MUMBA_HOST_APPLICATION_MEDIA_AUDIO_INPUT_DEVICE_MANAGER_H_
