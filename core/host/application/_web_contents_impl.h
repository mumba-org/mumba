...
#include "core/host/web_contents_observer.h"
#include "core/common/page_importance_signals.h"
#include "core/common/renderer_preferences.h"
#include "core/common/resource_type.h"
#include "core/common/three_d_api_types.h"
...
WebContentsView* CreateWebContentsView(
    ...
    RenderViewHostDelegateView** application_window_host_delegate_view);
...
class CONTENT_EXPORT WebContentsImpl : public WebContents,
 ...
 public:
  ...
  // Returns the SaveContainer which manages the page saving job. May be NULL.
  SaveContainer* save_container() const { return save_container_.get(); }
  ...
  void NotifyWebContentsFocused(ApplicationWindowHost* application_window_host);

  // Notify observers that the web contents has lost focus.
  void NotifyWebContentsLostFocus(ApplicationWindowHost* application_window_host);
  ...
  ApplicationWindowHostView* GetApplicationWindowHostView() const override;
  ApplicationWindowHostView* GetTopLevelApplicationWindowHostView() override;
  void ClosePage() override;
  ApplicationWindowHostView* GetFullscreenApplicationWindowHostView() const override;
  ...
  void Close() override;
  void SystemDragEnded(ApplicationWindowHost* source_rwh) override;
  ...
  bool OnMessageReceived(RenderViewHostImpl* application_window_host,
                         const IPC::Message& message) override;
  // RenderFrameHostDelegate has the same method, so list it there because this
  // interface is going away.
  // WebContents* GetAsWebContents() override;
  void RenderViewCreated(RenderViewHost* application_window_host) override;
  void RenderViewReady(RenderViewHost* application_window_host) override;
  void RenderViewTerminated(RenderViewHost* application_window_host,
                            base::TerminationStatus status,
                            int error_code) override;
  void RenderViewDeleted(RenderViewHost* application_window_host) override;
  void UpdateTargetURL(RenderViewHost* application_window_host,
                       const GURL& url) override;
  void Close(RenderViewHost* application_window_host) override;
  void RequestMove(const gfx::Rect& new_bounds) override;
  void DocumentAvailableInMainFrame(RenderViewHost* application_window_host) override;
  ...
  RendererPreferences GetRendererPrefs(
      ApplicationContents* application_contents) const override;
  ...
  void RenderWidgetCreated(ApplicationWindowHost* application_window_host) override;
  void RenderWidgetDeleted(ApplicationWindowHost* application_window_host) override;
  void RenderWidgetGotFocus(ApplicationWindowHost* application_window_host) override;
  void RenderWidgetLostFocus(ApplicationWindowHost* application_window_host) override;
  void RenderWidgetWasResized(ApplicationWindowHost* application_window_host,
  ...
  BrowserAccessibilityManager* GetOrCreateRootAccessibilityManager()
      override;
  ...
  void RendererUnresponsive(ApplicationWindowHost* application_window_host) override;
  void RendererResponsive(ApplicationWindowHost* application_window_host) override;
  ...
  void CreateApplicationWindowHostViewForRenderManager(
      RenderViewHost* application_window_host) override;
  ...
  // search this WebContentsImpl for a focused ApplicationWindowHost. The previously
  // focused WebContentsImpl, if any, will have its ApplicationWindowHosts
  // deactivated.
 ...
 private:
  ...
  explicit WebContentsImpl(ApplicationContents* application_contents);
  ...
  ApplicationWindowHostView* GetCreatedWidget(int process_id, int route_id);
  ...
  void SetHistoryOffsetAndLengthForView(RenderViewHost* application_window_host,
                                        int history_offset,
                                        int history_length);
  ...
  // provide this.
  RenderViewHostDelegateView* application_window_host_delegate_view_;
  ...
  // All live ApplicationWindowHosts that are created by this object and may
  // outlive it.
  std::set<ApplicationWindowHost*> created_widgets_;
  ...
  std::unique_ptr<ApplicationWindowHostInputEventRouter> rwh_input_event_router_;
  ...
  // ApplicationWindowHostViews on this WebContents. Only exists on the outermost
  ...
  // IME-related state for ApplicationWindowHosts on the inner WebContents is tracked
