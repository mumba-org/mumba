...
#include "core/host/application/input/one_shot_timeout_monitor.h"
...
RenderWidgetTargetResult::RenderWidgetTargetResult(
    ApplicationWindowHostView* in_view,
...
void RenderWidgetTargeter::FindTargetAndDispatch(
  ...
  ApplicationWindowHostView* target = result.view;
  ...
  if (result.should_query_view && !target->IsApplicationWindowHostViewGuest()) {
...
void RenderWidgetTargeter::ViewWillBeDestroyed(ApplicationWindowHostView* view) {
...
void RenderWidgetTargeter::FoundTarget(
  ...
  // ApplicationWindowHostViewMac can be deleted asynchronously, in which case the
  // View will be valid but there will no longer be a ApplicationWindowHost.
  if (!root_view || !root_view->GetApplicationWindowHost()) {
    return;
  }
