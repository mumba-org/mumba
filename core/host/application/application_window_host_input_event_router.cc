...
//#include "core/host/frame_host/application_window_host_view_guest.h"
...
gfx::PointF ComputePointInRootInPixels(
    ...
    host::ApplicationWindowHostView* root_view,
...
void ApplicationWindowHostInputEventRouter::OnApplicationWindowHostViewDestroyed(
    ApplicationWindowHostView* view) {
  // ApplicationWindowHostView::RemoveObserver() should only ever be called
  ...
  if (view == last_mouse_move_target_) {
    ...
    //      static_cast<ApplicationWindowHostViewChildFrame*>(last_mouse_move_target_)
    //          ->GetParentView();
    //else
...
void ApplicationWindowHostInputEventRouter::ClearAllObserverRegistrations() {
  // Since we're shutting down, it's safe to call ApplicationWindowHostView::
...
ApplicationWindowHostInputEventRouter::HittestDelegate::HittestDelegate(
    const std::unordered_map<viz::SurfaceId, HittestData, viz::SurfaceIdHash>&
        hittest_data)
    : hittest_data_(hittest_data) {}

bool ApplicationWindowHostInputEventRouter::HittestDelegate::RejectHitTarget(
...
bool ApplicationWindowHostInputEventRouter::HittestDelegate::AcceptHitTarget(
    const viz::SurfaceDrawQuad* surface_quad,
    const gfx::Point& point_in_quad_space) {
  auto it = hittest_data_.find(surface_quad->primary_surface_id);
  if (it != hittest_data_.end() && !it->second.ignored_for_hittest)
...
ApplicationWindowHostInputEventRouter::ApplicationWindowHostInputEventRouter()
...
RenderWidgetTargetResult ApplicationWindowHostInputEventRouter::FindMouseEventTarget(
    ApplicationWindowHostView* root_view,
    const blink::WebMouseEvent& event) const {
  ApplicationWindowHostView* target = nullptr;
...
ApplicationWindowHostInputEventRouter::FindMouseWheelEventTarget(
    ApplicationWindowHostView* root_view,
...
RenderWidgetTargetResult ApplicationWindowHostInputEventRouter::FindViewAtLocation(
    ApplicationWindowHostView* root_view,
    const gfx::PointF& point,
    const gfx::PointF& point_in_screen,
    viz::EventSource source,
    gfx::PointF* transformed_point) const {
  // Short circuit if owner_map has only one ApplicationWindowHostView, no need for
  // hit testing.
  ...
  if (use_viz_hit_test_) {
    ...
    // within a single process with only one ApplicationWindowHost.
  ...
  // anymore.
  if (!view) { //|| (ApplicationWindowHostViewGuest::GetRootView(view) != root_view)) {
    view = root_view;
    *transformed_point = point;
  }
...
void ApplicationWindowHostInputEventRouter::RouteMouseEvent(
    ApplicationWindowHostView* root_view,
...
void ApplicationWindowHostInputEventRouter::DispatchMouseEvent(
    ApplicationWindowHostView* root_view,
...
void ApplicationWindowHostInputEventRouter::RouteMouseWheelEvent(
    ApplicationWindowHostView* root_view,
...
void ApplicationWindowHostInputEventRouter::DispatchMouseWheelEvent(
    ApplicationWindowHostView* root_view,
    ApplicationWindowHostView* target,
  ...
  if (!target) {
    root_view->WheelEventAck(mouse_wheel_event,
                             common::INPUT_EVENT_ACK_STATE_NO_CONSUMER_EXISTS);
    return;
  }
...
void ApplicationWindowHostInputEventRouter::RouteGestureEvent(
    ApplicationWindowHostView* root_view,
...
void ApplicationWindowHostInputEventRouter::OnHandledTouchStartOrFirstTouchMove(
...
RenderWidgetTargetResult ApplicationWindowHostInputEventRouter::FindTouchEventTarget(
    ApplicationWindowHostView* root_view,
...
void ApplicationWindowHostInputEventRouter::DispatchTouchEvent(
    ApplicationWindowHostView* root_view,
    ApplicationWindowHostView* target,
  ...
  // reset to 0 in OnApplicationWindowHostViewDestroyed, and this can
...
void ApplicationWindowHostInputEventRouter::RouteTouchEvent(
    ApplicationWindowHostView* root_view,
...
void ApplicationWindowHostInputEventRouter::SendMouseEnterOrLeaveEvents(
    const blink::WebMouseEvent& event,
    ApplicationWindowHostView* target,
    ApplicationWindowHostView* root_view) {
  // This method treats ApplicationWindowHostViews as a tree, where the mouse
  ...
  // boundaries), all affected ApplicationWindowHostViews are ancestors of either
  ...
  std::vector<ApplicationWindowHostView*> entered_views;
  std::vector<ApplicationWindowHostView*> exited_views;
  ApplicationWindowHostView* cur_view = target;
  entered_views.push_back(cur_view);
  //while (cur_view->IsApplicationWindowHostViewChildFrame()) {
  //  cur_view =
  //      static_cast<ApplicationWindowHostViewChildFrame*>(cur_view)->GetParentView();
  ...
  //  entered_views.push_back(cur_view);
  //}
  // Non-root RWHVs are guaranteed to be ApplicationWindowHostViewChildFrames,
  ...
  if (cur_view) {
    ...
    //while (cur_view->IsApplicationWindowHostViewChildFrame()) {
    //  cur_view = static_cast<ApplicationWindowHostViewChildFrame*>(cur_view)
   //                  ->GetParentView();
   //   if (!cur_view) {
   //     last_mouse_move_target_ = target;
   //     last_mouse_move_root_view_ = root_view;
  ...
  ApplicationWindowHostView* common_ancestor = nullptr;
...
void ApplicationWindowHostInputEventRouter::ReportBubblingScrollToSameView(
...
void ApplicationWindowHostInputEventRouter::BubbleScrollEvent(
    ApplicationWindowHostView* target_view,
    const blink::WebGestureEvent& event,
...
void ApplicationWindowHostInputEventRouter::SendGestureScrollBegin(
    ApplicationWindowHostView* view,
    const blink::WebGestureEvent& event) {
...
void ApplicationWindowHostInputEventRouter::SendGestureScrollEnd(
    ApplicationWindowHostView* view,
    const blink::WebGestureEvent& event) {
...
void ApplicationWindowHostInputEventRouter::CancelScrollBubbling(
    ApplicationWindowHostView* target_view) {
  DCHECK(target_view);
  if (target_view == first_bubbling_scroll_target_.target) {
    first_bubbling_scroll_target_.target = nullptr;
...
void ApplicationWindowHostInputEventRouter::AddFrameSinkIdOwner(
...
void ApplicationWindowHostInputEventRouter::RemoveFrameSinkIdOwner(
    const viz::FrameSinkId& id) {
  ...
  if (it_to_remove != owner_map_.end()) {
    ...
    OnApplicationWindowHostViewDestroyed(it_to_remove->second);
  }
...
void ApplicationWindowHostInputEventRouter::OnHittestData(
    const viz::SurfaceId& surface_id, bool ignored_for_hittest) {
  if (owner_map_.find(surface_id.frame_sink_id()) == owner_map_.end()) {
    return;
  }
  HittestData data;
  data.ignored_for_hittest = ignored_for_hittest;
  hittest_data_[surface_id] = data;
}

ApplicationWindowHost*
ApplicationWindowHostInputEventRouter::GetApplicationWindowHostAtPoint(
    ApplicationWindowHostView* root_view,
    const gfx::PointF& point,
    gfx::PointF* transformed_point) {
  if (!root_view)
  ...
  return //ApplicationWindowHost::From(
      FindViewAtLocation(root_view, point, point_in_screen,
                         viz::EventSource::MOUSE, transformed_point)
          .view->GetApplicationWindowHost();//);
}

RenderWidgetTargetResult
ApplicationWindowHostInputEventRouter::FindTouchscreenGestureEventTarget(
    ApplicationWindowHostView* root_view,
...
bool ApplicationWindowHostInputEventRouter::IsViewInMap(
    const ApplicationWindowHostView* view) const {
  for (auto entry : owner_map_) {
    if (entry.second == view)
      return true;
  }
...
void ApplicationWindowHostInputEventRouter::DispatchTouchscreenGestureEvent(
    ApplicationWindowHostView* root_view,
    ApplicationWindowHostView* target,
    const blink::WebGestureEvent& gesture_event,
    const ui::LatencyInfo& latency,
  ...
  if (gesture_event.GetType() == blink::WebInputEvent::kGesturePinchBegin) {
    ...
    auto* rwhi =
        static_cast<ApplicationWindowHost*>(root_view->GetApplicationWindowHost());
    ...
    if (root_view != touchscreen_gesture_target_.target &&
      ...
      if (touchscreen_gesture_target_.target) {
        target_allowed_touch_action =
            (static_cast<ApplicationWindowHost*>(
                 touchscreen_gesture_target_.target->GetApplicationWindowHost()))
                ->input_router()
                ->AllowedTouchAction();
      }
...
void ApplicationWindowHostInputEventRouter::RouteTouchscreenGestureEvent(
    ApplicationWindowHostView* root_view,
    blink::WebGestureEvent* event,
    const ui::LatencyInfo& latency) {
...
RenderWidgetTargetResult
ApplicationWindowHostInputEventRouter::FindTouchpadGestureEventTarget(
    ApplicationWindowHostView* root_view,
...
void ApplicationWindowHostInputEventRouter::RouteTouchpadGestureEvent(
    ApplicationWindowHostView* root_view,
    blink::WebGestureEvent* event,
    const ui::LatencyInfo& latency) {
...
void ApplicationWindowHostInputEventRouter::DispatchTouchpadGestureEvent(
    ApplicationWindowHostView* root_view,
    ApplicationWindowHostView* target,
...
std::vector<ApplicationWindowHostView*>
ApplicationWindowHostInputEventRouter::GetApplicationWindowHostViewsForTests() const {
  std::vector<ApplicationWindowHostView*> hosts;
  for (auto entry : owner_map_)
...
ApplicationWindowHostInputEventRouter::GetRenderWidgetTargeterForTests() {
  return event_targeter_.get();
}

bool ApplicationWindowHostInputEventRouter::TransformPointToTargetCoordSpace(
    ApplicationWindowHostView* root_view,
    ApplicationWindowHostView* target,
    const gfx::PointF& point,
    gfx::PointF* transformed_point,
  ...
  //ApplicationWindowHostView* cur_view = target;
  //while (cur_view->IsApplicationWindowHostViewChildFrame()) {
  //  if (cur_view->IsApplicationWindowHostViewGuest()) {
  //    cur_view = static_cast<ApplicationWindowHostViewGuest*>(cur_view)
  //                   ->GetOwnerApplicationWindowHostView();
  //  } else {
  //    cur_view = static_cast<ApplicationWindowHostViewChildFrame*>(cur_view)
  //                   ->GetParentView();
  //  }
...
RenderWidgetTargetResult
ApplicationWindowHostInputEventRouter::FindTargetSynchronously(
    ApplicationWindowHostView* root_view,
...
void ApplicationWindowHostInputEventRouter::DispatchEventToTarget(
    ApplicationWindowHostView* root_view,
    ApplicationWindowHostView* target,
    const blink::WebInputEvent& event,
    const ui::LatencyInfo& latency,
...
ApplicationWindowHostInputEventRouter::FindViewFromFrameSinkId(
  ...
  // it likely means the ApplicationWindowHostView has been destroyed but its
