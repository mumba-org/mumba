...
#ifndef MUMBA_HOST_APPLICATION_RENDER_WIDGET_HOST_IMPL_H_
#define MUMBA_HOST_APPLICATION_RENDER_WIDGET_HOST_IMPL_H_
...
#include "core/host/application/render_frame_metadata_provider_impl.h"
...
#include "core/common/render_frame_metadata.mojom.h"
#include "core/common/render_widget_surface_properties.h"
#include "core/common/view_message_enums.h"
...
class ApplicationWindowHostOwnerDelegate;
...
// This implements the ApplicationWindowHost interface that is exposed to
// embedders of content, and adds things only visible to content.
class CONTENT_EXPORT ApplicationWindowHost
 ...
 public:
  ...
  // Similar to ApplicationWindowHost::FromID, but returning the Impl object.
  static ApplicationWindowHost* FromID(int32_t process_id, int32_t routing_id);

  // Returns all ApplicationWindowHosts including swapped out ones for
  // internal use. The public interface
  // ApplicationWindowHost::GetApplicationWindowHosts only returns active ones.
  static std::unique_ptr<ApplicationWindowHostIterator> GetAllApplicationWindowHosts();

  // Use ApplicationWindowHost::From(rwh) to downcast a ApplicationWindowHost to a
  // ApplicationWindowHost.
  static ApplicationWindowHost* From(ApplicationWindowHost* rwh);
  ...
  void set_owner_delegate(ApplicationWindowHostOwnerDelegate* owner_delegate) {
    owner_delegate_ = owner_delegate;
  }

  ApplicationWindowHostOwnerDelegate* owner_delegate() { return owner_delegate_; }
  ...
  ApplicationWindowHostView* GetView() const override;
  ...
  void AddInputEventObserver(
      ApplicationWindowHost::InputEventObserver* observer) override;
  void RemoveInputEventObserver(
      ApplicationWindowHost::InputEventObserver* observer) override;
  void AddObserver(ApplicationWindowHostObserver* observer) override;
  void RemoveObserver(ApplicationWindowHostObserver* observer) override;
  ...
  // Sets the View of this ApplicationWindowHost.
  ...
  // Initializes a ApplicationWindowHost that is attached to a RenderFrameHost.
  void InitForFrame();

  // Signal whether this ApplicationWindowHost is owned by a RenderFrameHost, in
  ...
  void Forwardcommon::GestureEventWithLatencyInfo(
      const blink::WebGestureEvent& gesture_event,
      const ui::LatencyInfo& latency) override;
  virtual void Forwardcommon::TouchEventWithLatencyInfo(
      const blink::WebTouchEvent& touch_event,
      const ui::LatencyInfo& latency);  // Virtual for testing.
  void Forwardcommon::MouseEventWithLatencyInfo(const blink::WebMouseEvent& mouse_event,
                                        const ui::LatencyInfo& latency);
  ...
  BrowserAccessibilityManager* GetRootAccessibilityManager();
  ...
  // The ApplicationWindowHost will keep showing the old page (for a while) after
 ...
 protected:
  ...
  // The View associated with the ApplicationWindowHost. The lifetime of this object
  // is associated with the lifetime of the Render process. If the Renderer
  // crashes, its View is destroyed and this pointer becomes NULL, even though
  // application_window_host_ lives on to load another URL (creating a new View while
  // doing so).
  base::WeakPtr<ApplicationWindowHostView> view_;

 private:
  FRIEND_TEST_ALL_PREFIXES(ApplicationWindowHostTest,
                           DontPostponeHangMonitorTimeout);
  FRIEND_TEST_ALL_PREFIXES(ApplicationWindowHostTest, RendererExitedNoDrag);
  FRIEND_TEST_ALL_PREFIXES(ApplicationWindowHostTest,
                           StopAndStartHangMonitorTimeout);
  FRIEND_TEST_ALL_PREFIXES(ApplicationWindowHostTest,
                           ShorterDelayHangMonitorTimeout);
  FRIEND_TEST_ALL_PREFIXES(ApplicationWindowHostViewAuraTest, AutoResizeWithScale);
  ...
  FRIEND_TEST_ALL_PREFIXES(ApplicationWindowHostViewMacTest,
                           ChildAllocationAcceptedInParent);
  FRIEND_TEST_ALL_PREFIXES(ApplicationWindowHostViewMacTest,
                           ConflictingAllocationsResolve);
...
#endif  // MUMBA_HOST_APPLICATION_RENDER_WIDGET_HOST_IMPL_H_
