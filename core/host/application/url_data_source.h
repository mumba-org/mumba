...
// notify.
class CONTENT_EXPORT URLDataSource : public base::RefCountedThreadSafe<
    URLDataSource, DeleteURLDataSource> {
 public:
  // Adds a URL data source to |application_contents|.
  static void Add(Domain* domain, URLDataSource* source);
  ...
  virtual void SetBackend(URLDataManagerBackend* backend) = 0;

  // Used by StartDataRequest so that the child class can return the data when
  // it's available.
  typedef base::Callback<void(int, scoped_refptr<base::RefCountedMemory>, bool /* should_complete*/)>
      GotDataCallback;

  virtual void SendResponse(int request_id,
                            int call_id,
                            scoped_refptr<base::RefCountedMemory> contents,
                            bool should_complete) = 0;    
  ...
  virtual void OnDataSent(int call_id, size_t bytes, GotDataCallback callback) = 0;

  virtual bool ShouldCompleteRequest(int call_id) = 0;

  // The following methods are all called on the IO thread.

  // Return the mimetype that should be sent with this response, or empty
  // string to specify no mime type.
  virtual std::string GetMimeType(const std::string& scheme, const std::string& path) const = 0;

  virtual scoped_refptr<net::HttpResponseHeaders> GetHeaders(
  ...
  // implementation returns HostThread::UI. If the delegate does not care
  ...
  virtual RouteRegistry* GetRouteRegistry() const = 0;
  virtual void SetRouteRegistry(RouteRegistry* route_registry) = 0;
  ...
  virtual bool IsGzipped(const std::string& scheme, const std::string& path) const;
