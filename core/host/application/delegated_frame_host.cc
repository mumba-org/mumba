...
DelegatedFrameHost::DelegatedFrameHost(const viz::FrameSinkId& frame_sink_id,
  ...
  //DLOG(INFO) << "DelegatedFrameHost::DelegatedFrameHost: frame_sink_id = " << frame_sink_id;
...
void DelegatedFrameHost::WasShown(
  ...
  SynchronizeVisualProperties(new_pending_local_surface_id,
                              ...
                              //cc::DeadlinePolicy::UseSpecifiedDeadline(10u));
}
...
bool DelegatedFrameHost::ShouldSkipFrame(const gfx::Size& size_in_dip) {
  ...
  bool skip = size_in_dip != resize_lock_->expected_size();
  return skip;
}
...
void DelegatedFrameHost::DidCreateNewRendererCompositorFrameSink(
  ...
  //DLOG(INFO) << "DelegatedFrameHost::DidCreateNewRendererCompositorFrameSink";
  ResetCompositorFrameSinkSupport();
...
void DelegatedFrameHost::OnFirstSurfaceActivation(
  ...
  if (enable_surface_synchronization_) {
    ...
    //} //else {
      //DLOG(INFO) << "DelegatedFrameHost::OnFirstSurfaceActivation: calling host_frame_sink_manager->DropTemporaryReference()";
      //ImageTransportFactory* factory = ImageTransportFactory::GetInstance();
      //viz::HostFrameSinkManager* host_frame_sink_manager =
      //    factory->GetContextFactoryPrivate()->GetHostFrameSinkManager();
...
void DelegatedFrameHost::SetCompositor(ui::Compositor* compositor) {
  ...
  //temp_layer_ = std::make_unique<ui::Layer>(ui::LAYER_SOLID_COLOR);
  //temp_layer_->SetColor(SK_ColorBLACK);
  //temp_layer_->SetBounds(
  //      gfx::Rect(0, 0, 300, 120));
  //client_->DelegatedFrameHostGetLayer()->Add(temp_layer_.get());
}
