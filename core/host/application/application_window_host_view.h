...
#ifndef MUMBA_HOST_APPLICATION_APPLICATION_WINDOW_HOST_VIEW_H_
#define MUMBA_HOST_APPLICATION_APPLICATION_WINDOW_HOST_VIEW_H_
...
class ApplicationWindowHostViewObserver;
class SyntheticGestureTarget;
class TextInputManager;
class TouchSelectionControllerClientManager;
class WebContentsAccessibility;

// Basic implementation shared by concrete ApplicationWindowHostView subclasses.
class CONTENT_EXPORT ApplicationWindowHostView
    : public IPC::Listener,
 ...
 public:
  ...
  ~ApplicationWindowHostView() override;

  float current_device_scale_factor() const {
    return current_device_scale_factor_;
  }

  // Returns the focused ApplicationWindowHost inside this |view|'s RWH.
  ApplicationWindowHost* GetFocusedWidget() const;

  virtual void InitAsChild(gfx::NativeView parent_view) = 0;
  virtual ApplicationWindowHost* GetApplicationWindowHost() const;
  virtual void SetSize(const gfx::Size& size) = 0;
  virtual void EnableAutoResize(const gfx::Size& min_size,
                        const gfx::Size& max_size);
  virtual void DisableAutoResize(const gfx::Size& new_size);
  virtual void SetBounds(const gfx::Rect& rect) = 0;
  virtual bool IsScrollOffsetAtTop() const;
  virtual void SetIsInVR(bool is_in_vr);
  ...
  virtual void WasUnOccluded() {}
  virtual void WasOccluded() {}
  virtual gfx::Rect GetViewBounds() const = 0;
  virtual base::string16 GetSelectedText();
  virtual TouchSelectionControllerClientManager* GetTouchSelectionControllerClientManager();
  virtual void SetBackgroundColor(SkColor color);
  virtual base::Optional<SkColor> GetBackgroundColor() const;
  //virtual void SetBackgroundColorToDefault();
  virtual bool LockMouse() = 0;
  virtual void UnlockMouse() = 0;
  virtual bool IsMouseLocked();
  virtual bool LockKeyboard(base::Optional<base::flat_set<int>> keys);
  virtual void UnlockKeyboard();
  virtual bool IsKeyboardLocked();
  virtual gfx::Size GetVisibleViewportSize() const;
  ...
  virtual viz::mojom::FrameSinkVideoCapturerPtr CreateVideoCapturer();
  virtual void FocusedNodeTouched(bool editable);
  virtual void SetNeedsBeginFrames(bool needs_begin_frames) = 0;
  virtual void SetWantsAnimateOnlyBeginFrames() = 0;
  virtual void GetScreenInfo(common::ScreenInfo* screen_info) const;
  ...
  virtual void TakeFallbackContentFrom(ApplicationWindowHostView* view) = 0;
  /*
   * End of ApplicationWindowHost
   */
  ...
  // to the view.
  uint32_t ApplicationFrameNumber();

  // Called each time the ApplicationWindowHost receives a new frame for display from
  // the renderer.
  void DidReceiveApplicationFrame();
  ...
  base::WeakPtr<ApplicationWindowHostView> GetWeakPtr();
  ...
  virtual gfx::Size GetRequestedApplicationSize() const;
  ...
  // crbug.com/751782
  virtual common::InputEventAckState FilterChildGestureEvent(
  ...
  // window container.
  //virtual BrowserAccessibilityManager* CreateAccessibilityManager(
  ...
  // This method is called by ApplicationWindowHost when a new
  ...
  virtual void DidCreateNewApplicationCompositorFrameSink(
      viz::mojom::CompositorFrameSinkClient*
          renderer_compositor_frame_sink) = 0;

  // This is called by the ApplicationWindowHost to provide a new compositor
  ...
  virtual void ProcessAckedTouchEvent(const common::TouchEventWithLatencyInfo& touch,
                                      common::InputEventAckState ack_result) {}
  ...
  // When there are multiple ApplicationWindowHostViews for a single page, input
  // events need to be targeted to the correct one for handling. The following
  // methods are invoked on the ApplicationWindowHostView that should be able to
  ...
  // embedded within the ApplicationWindowHostView's Surface to the
  ...
  // ApplicationWindow and needs to be translated to viewport coordinates for the
  ...
  // ApplicationWindowHostView to the coordinate space of the target_view.
  ...
  // ApplicationWindowHostViewGuests that is needed for special treatment during
  ...
  virtual bool IsApplicationWindowHostViewGuest();

  // Subclass identifier for ApplicationWindowHostViewChildFrames. This is useful
  ...
  virtual bool IsApplicationWindowHostViewChildFrame();

  // Notify the View that a screen rect update is being sent to the
  // ApplicationWindow. Related platform-specific updates can be sent from here.
  ...
  virtual void TextInputStateChanged(const common::TextInputState& text_input_state);
  ...
  // ApplicationWindow's window's origin. Focus and anchor bound are represented as
  // gfx::Rect.
  ...
  virtual void InitAsPopup(ApplicationWindowHostView* parent_host_view,
  ...
  virtual void InitAsFullscreen(ApplicationWindowHostView* reference_host_view) = 0;

  // Sets the cursor for this view to the one associated with the specified
  // cursor_type.
  virtual void UpdateCursor(const common::WebCursor& cursor) = 0;
  ...
  virtual void DisplayCursor(const common::WebCursor& cursor);
  ...
  // Called by the ApplicationWindowHost when an ambiguous gesture is detected to
  ...
  // Returns the associated ApplicationWindowHost.
  ApplicationWindowHost* host() const { return host_; }

  // Process swap messages sent before |frame_token| in ApplicationWindowHost.
  ...
  void AddObserver(ApplicationWindowHostViewObserver* observer);
  void RemoveObserver(ApplicationWindowHostViewObserver* observer);

  // Returns a reference to the current instance of TextInputManager. The
  // reference is obtained from ApplicationWindowHostDelegate. The first time a non-
  ...
  // Called when the ApplicationWindowHost has be initialized.
  virtual void OnApplicationWindowInit() {}

 protected:
  explicit ApplicationWindowHostView(ApplicationWindowHost* host);
  ...
  // ApplicationWindowHostViewGuest is removed.
  ApplicationWindowHost* host_;
 ...
 private:
  ...
  // ApplicationWindowHost in order to allow the view to allocate a new
  // LocalSurfaceId.
  ...
  base::ObserverList<ApplicationWindowHostViewObserver> observers_;

#if defined(USE_AURA)
  common::mojom::ApplicationWindowTreeClientPtr application_window_tree_client_;
  ...
  base::WeakPtrFactory<ApplicationWindowHostView> weak_factory_;

  DISALLOW_COPY_AND_ASSIGN(ApplicationWindowHostView);
};

}  // namespace host

#endif  // MUMBA_HOST_APPLICATION_RENDER_WIDGET_HOST_VIEW_BASE_H_
