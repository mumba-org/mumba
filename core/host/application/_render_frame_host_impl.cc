...
#include "core/host/bluetooth/web_bluetooth_service_impl.h"
#include "core/host/browser_main_loop.h"
...
#include "core/host/frame_host/debug_urls.h"
#include "core/host/frame_host/frame_tree.h"
#include "core/host/frame_host/frame_tree_node.h"
#include "core/host/frame_host/input/input_injector_impl.h"
#include "core/host/frame_host/keep_alive_handle_factory.h"
...
#include "core/host/frame_host/navigator.h"
...
#include "core/host/generic_sensor/sensor_provider_proxy_impl.h"
#include "core/host/geolocation/geolocation_service_impl.h"
#include "core/host/image_capture/image_capture_impl.h"
#include "core/host/installedapp/installed_app_provider_impl_default.h"
#include "core/host/interface_provider_filtering.h"
#include "core/host/keyboard_lock/keyboard_lock_service_impl.h"
#include "core/host/loader/prefetch_url_loader_service.h"
#include "core/host/loader/resource_dispatcher_host_impl.h"
#include "core/host/loader/resource_scheduler_filter.h"
...
#include "core/host/payments/payment_app_context_impl.h"
#include "core/host/permissions/permission_service_context.h"
#include "core/host/permissions/permission_service_impl.h"
#include "core/host/presentation/presentation_service_impl.h"
...
#include "core/host/scoped_active_url.h"
...
#include "core/host/webauth/authenticator_impl.h"
#include "core/host/webauth/scoped_virtual_authenticator_environment.h"
...
#include "core/host/webui/web_ui_url_loader_factory_internal.h"
#include "core/common/accessibility_messages.h"
...
#include "core/common/content_security_policy/content_security_policy.h"
#include "core/common/frame_messages.h"
#include "core/common/frame_owner_properties.h"
#include "core/common/input/input_handler.mojom.h"
#include "core/common/inter_process_time_ticks_converter.h"
#include "core/common/navigation_params.h"
#include "core/common/navigation_subresource_loader_params.h"
#include "core/common/render_message_filter.mojom.h"
#include "core/common/renderer.mojom.h"
#include "core/common/swapped_out_messages.h"
#include "core/common/url_loader_factory_bundle.mojom.h"
#include "core/shared/common/application.mojom.h"
#include "core/common/wrapper_shared_url_loader_factory.h"
#include "core/host/ax_event_notification_details.h"
#include "core/host/browser_accessibility_state.h"
...
#include "core/host/permission_manager.h"
#include "core/host/permission_type.h"
...
#include "core/host/stream_handle.h"
#include "core/host/webvr_service_provider.h"
...
#include "core/common/file_chooser_file_info.h"
#include "core/common/file_chooser_params.h"
#include "core/common/isolated_world_ids.h"
...
#include "storage/host/blob/blob_url_loader_factory.h"
...
#include "core/host/frame_host/render_frame_host_android.h"
#include "core/host/android/java_interfaces.h"
#endif

#if defined(OS_MACOSX)
#include "core/host/frame_host/popup_menu_helper_mac.h"
#endif
...
void RenderFrameHostImpl::SetNetworkFactoryForTesting(
  ...
  DCHECK(!HostThread::IsThreadInitialized(HostThread::UI) ||
         HostThread::CurrentlyOn(HostThread::UI));
...
RenderFrameHostImpl::RenderFrameHostImpl(SiteInstance* site_instance,
                                         RenderViewHostImpl* application_window_host,
                                         RenderFrameHostDelegate* delegate,
                                         ApplicationWindowHostDelegate* rwh_delegate,
  ...
  frame_tree_->AddRenderViewHostRef(application_window_host_);
  ...
  if (widget_routing_id != MSG_ROUTING_NONE) {
    ...
    // TODO(avi): Once RenderViewHostImpl has-a ApplicationWindowHost, the main
    // render frame should probably start owning the ApplicationWindowHost,
    // so this logic checking for an already existing RWHI should be removed.
    // https://crbug.com/545684
    application_window_host_ =
        ApplicationWindowHost::FromID(GetProcess()->GetID(), widget_routing_id);
    ...
    if (!application_window_host_) {
      ...
      application_window_host_ = ApplicationWindowHostFactory::Create(
          rwh_delegate, GetProcess(), widget_routing_id, std::move(widget),
          hidden);
      application_window_host_->set_owned_by_render_frame_host(true);
    } else {
      DCHECK(!application_window_host_->owned_by_render_frame_host());
      application_window_host_->SetWidget(std::move(widget));
    }
    application_window_host_->SetFrameDepth(frame_tree_node_->depth());
    application_window_host_->SetWidgetInputHandler(std::move(widget_handler),
...
RenderFrameHostImpl::~RenderFrameHostImpl() {
  ...
  bool will_render_view_clean_up_render_frame =
      frame_tree_node_->IsMainFrame() && application_window_host_->ref_count() == 1;
  ...
  if (application_window_host_ &&
    ...
    application_window_host_->ShutdownAndDestroyWidget(true);
  }
...
gfx::NativeView RenderFrameHostImpl::GetNativeView() {
  ApplicationWindowHostView* view = application_window_host_->GetWidget()->GetView();
  if (!view)
    return nullptr;
...
blink::mojom::PageVisibilityState RenderFrameHostImpl::GetVisibilityState() {
  ...
  // detached from the frame tree. This tries to find a ApplicationWindowHost
  ...
  while (frame) {
    if (frame->application_window_host_)
      break;
  ...
  blink::mojom::PageVisibilityState visibility_state =
      GetApplicationWindowHost()->is_hidden()
...
gfx::Point RenderFrameHostImpl::AccessibilityOriginInScreen(
  ...
  ApplicationWindowHostView* view = static_cast<ApplicationWindowHostView*>(
      application_window_host_->GetWidget()->GetView());
  if (view)
...
void RenderFrameHostImpl::SetRenderFrameCreated(bool created) {
  ...
  if (created && application_window_host_) {
    mojom::WidgetPtr widget;
    GetRemoteInterfaces()->GetInterface(&widget);
    application_window_host_->SetWidget(std::move(widget));
    ...
    application_window_host_->SetInputTargetClient(std::move(input_target_client));
    application_window_host_->InitForFrame();
  }
...
void RenderFrameHostImpl::OnDidAddMessageToConsole(
  ...
  if (::logging::GetMinLogLevel() <= resolved_level &&
      (is_web_ui ||
       !GetSiteInstance()->GetApplicationContents()->IsOffTheRecord())) {
...
void RenderFrameHostImpl::DidCommitProvisionalLoad(
  ...
  if (frame_tree_node_->IsMainFrame() && GetView()) {
    ApplicationWindowHost::From(GetView()->GetApplicationWindowHost())
...
ApplicationWindowHost* RenderFrameHostImpl::GetApplicationWindowHost() {
  RenderFrameHostImpl* frame = this;
  while (frame) {
    if (frame->application_window_host_)
      return frame->application_window_host_;
    frame = frame->GetParent();
  }
...
ApplicationWindowHostView* RenderFrameHostImpl::GetView() {
  return GetApplicationWindowHost()->GetView();
}
...
void RenderFrameHostImpl::SwapOut(
  ...
  if (frame_tree_node_->IsMainFrame())
    application_window_host_->SetIsActive(false);
}
...
bool RenderFrameHostImpl::IsWaitingForUnloadACK() const {
  return application_window_host_->is_waiting_for_close_ack_ ||
...
void RenderFrameHostImpl::OnSwappedOut() {
  ...
  if (frame_tree_node_->IsMainFrame() && !application_window_host_->is_active())
    application_window_host_->set_is_swapped_out(true);
...
void RenderFrameHostImpl::OnContextMenu(const ContextMenuParams& params) {
  ...
  // ApplicationWindowHosts, such as with out-of-process iframes.
  gfx::Point original_point(validated_params.x, validated_params.y);
  gfx::Point transformed_point =
      static_cast<ApplicationWindowHostView*>(GetView())
...
void RenderFrameHostImpl::OnForwardResourceTimingToParent(
  ...
  if (!proxy) {
    ...
    bool rvh_is_active = application_window_host()->is_active();
    bool rvh_is_live = application_window_host()->IsRenderViewLive();
...
ApplicationWindowHostView* RenderFrameHostImpl::GetViewForAccessibility() {
  return static_cast<ApplicationWindowHostView*>(
      frame_tree_node_->IsMainFrame()
          ? application_window_host_->GetWidget()->GetView()
          : frame_tree_node_->frame_tree()
                ->GetMainFrame()
                ->application_window_host_->GetWidget()
                ->GetView());
}
...
void RenderFrameHostImpl::OnEnterFullscreen(
  ...
  // frameRectsChanged.
  application_window_host_->GetWidget()->SynchronizeVisualProperties();
}
...
void RenderFrameHostImpl::OnSelectionChanged(const base::string16& text,
  ...
  GetApplicationWindowHost()->SelectionChanged(text, offset, range);
}
...
void RenderFrameHostImpl::OnShowPopup(
  ...
  RenderViewHostDelegateView* view =
      application_window_host_->delegate_->GetDelegateView();
  if (view) {
...
void RenderFrameHostImpl::CreateNewWindow(
  ...
  // passed.
  if (!application_window_host_->GetWebkitPreferences().supports_multiple_windows) {
  ...
  StoragePartition* storage_partition = ApplicationContents::GetStoragePartition(
      GetSiteInstance()->GetApplicationContents(), GetSiteInstance());
  ...
  if (!params->opener_suppressed && !no_javascript_access) {
    ...
    // TODO(avi): When RenderViewHostImpl has-a ApplicationWindowHost, this
  ...
  bool succeeded =
      ApplicationWindowHost::FromID(render_process_id, main_frame_widget_route_id) !=
      nullptr;
  if (!succeeded) {
  ...
  if (blob_url_token) {
    blob_url_loader_factory =
        ChromeBlobStorageContext::URLLoaderFactoryForToken(
            GetSiteInstance()->GetApplicationContents(), std::move(blob_url_token));
  }
...
void GetRestrictedCookieManager(
  ...
  ApplicationContents* application_contents =
      render_frame_host_impl->GetProcess()->GetApplicationContents();
  StoragePartition* storage_partition =
      ApplicationContents::GetDefaultStoragePartition(application_contents);
...
void RenderFrameHostImpl::RegisterMojoInterfaces() {
  ...
  PermissionManager* permission_manager =
      GetProcess()->GetApplicationContents()->GetPermissionManager();
  ...
  if (base::FeatureList::IsEnabled(network::features::kNetworkService)) {
    StoragePartitionImpl* storage_partition =
        static_cast<StoragePartitionImpl*>(ApplicationContents::GetStoragePartition(
            GetSiteInstance()->GetApplicationContents(), GetSiteInstance()));
    // TODO(https://crbug.com/813479): Investigate why we need to explicitly
    // specify task runner for HostThread::IO here.
...
void RenderFrameHostImpl::ResetWaitingState() {
  ...
  send_before_unload_start_time_ = base::TimeTicks();
  application_window_host_->is_waiting_for_close_ack_ = false;
...
bool RenderFrameHostImpl::CanCommitOrigin(
  ...
  if (origin.scheme() == url::kFileScheme) {
    WebPreferences prefs = application_window_host_->GetWebkitPreferences();
...
void RenderFrameHostImpl::DispatchBeforeUnload(bool for_navigation,
  ...
  if (is_waiting_for_beforeunload_ack_) {
    ...
    if (application_window_host_->GetDelegate()->IsJavaScriptDialogShowing()) {
...
void RenderFrameHostImpl::CommitNavigation(
  ...
  if (is_view_source && IsCurrent()) {
    DCHECK(!GetParent());
    application_window_host()->Send(new FrameMsg_EnableViewSourceMode(routing_id_));
  }
  ...
  if (base::FeatureList::IsEnabled(network::features::kNetworkService) &&
    ...
    if (common_params.url.SchemeIsFile()) {
      ...
      auto file_factory = std::make_unique<FileURLLoaderFactory>(
          GetProcess()->GetApplicationContents()->GetPath(),
...
bool RenderFrameHostImpl::IsFocused() {
  return GetApplicationWindowHost()->is_focused() &&
...
bool RenderFrameHostImpl::UpdatePendingWebUI(const GURL& dest_url,
  ...
  WebUI::TypeID new_web_ui_type =
      WebUIControllerFactoryRegistry::GetInstance()->GetWebUIType(
          GetSiteInstance()->GetApplicationContents(), dest_url);
  ...
  if (pending_web_ui_ && !application_window_host_->GetProcess()->IsForGuestsOnly()) {
  ...
  } else if (application_window_host_->is_active()) {
...
bool RenderFrameHostImpl::IsSameSiteInstance(
  ...
  // As a sanity check, make sure the frame belongs to the same ApplicationContents.
  CHECK_EQ(GetSiteInstance()->GetApplicationContents(),
           other_render_frame_host->GetSiteInstance()->GetApplicationContents());
...
BrowserAccessibilityManager*
    RenderFrameHostImpl::GetOrCreateAccessibilityManager() {
  ApplicationWindowHostView* view = GetViewForAccessibility();
  if (view &&
    ...
    browser_accessibility_manager_.reset(
        view->CreateAccessibilityManager(this, is_root_frame));
  }
...
bool RenderFrameHostImpl::IsRenderFrameLive() {
  ...
  DCHECK(!is_live || application_window_host_->IsRenderViewLive());
...
void RenderFrameHostImpl::FilesSelectedInChooser(
  ...
  storage::FileSystemContext* const file_system_context =
      ApplicationContents::GetStoragePartition(GetProcess()->GetApplicationContents(),
                                          GetSiteInstance())
...
void RenderFrameHostImpl::CreateNetworkServiceDefaultFactoryAndObserve(
  ...
  auto* context = GetSiteInstance()->GetApplicationContents();
  GetContentClient()->browser()->WillCreateURLLoaderFactory(
  ...
  StoragePartitionImpl* storage_partition = static_cast<StoragePartitionImpl*>(
      ApplicationContents::GetStoragePartition(context, GetSiteInstance()));
...
RenderFrameHostImpl::TakeNavigationHandleForSameDocumentCommit(
  ...
  // TODO(ahemery): Remove when the full mojo interface is in portal.
...
void RenderFrameHostImpl::BeforeUnloadTimeout() {
  if (application_window_host_->GetDelegate()->ShouldIgnoreUnresponsiveRenderer())
    return;
...
void RenderFrameHostImpl::SetLastCommittedSiteUrl(const GURL& url) {
  ...
  if (!last_committed_site_url_.is_empty()) {
    RenderProcessHostImpl::RemoveFrameWithSite(
        frame_tree_node_->navigator()->GetController()->GetApplicationContents(),
...
bool RenderFrameHostImpl::ValidateDidCommitParams(
  ...
  // banned URLs into the navigation controller in the first portal.
  //
...
bool RenderFrameHostImpl::DidCommitNavigationInternal(
  ...
  if (!navigation_request_) {
    ...
    // the full mojo interface is in portal.
    // (https://bugs.chromium.org/p/chromium/issues/detail?id=784904)
    if (!is_loading()) {
      bool was_loading = frame_tree_node()->frame_tree()->IsLoading();
      is_loading_ = true;
