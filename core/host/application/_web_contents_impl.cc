...
#include "core/host/browser_plugin/browser_plugin_embedder.h"
#include "core/host/browser_plugin/browser_plugin_guest.h"
#include "core/host/child_process_security_policy_impl.h"
#include "core/host/devtools/protocol/page_handler.h"
#include "core/host/devtools/render_frame_devtools_agent_host.h"
#include "core/host/dom_storage/dom_storage_context_wrapper.h"
#include "core/host/dom_storage/session_storage_namespace_impl.h"
#include "core/host/download/mhtml_generation_manager.h"
#include "core/host/download/save_container.h"
#include "core/host/find_request_manager.h"
...
#include "core/host/media/capture/web_contents_audio_muter.h"
#include "core/host/media/media_web_contents_observer.h"
...
#include "core/host/web_contents/web_contents_view_child_frame.h"
#include "core/host/web_contents/web_contents_view_guest.h"
#include "core/host/webui/generic_handler.h"
#include "core/host/webui/web_ui_controller_factory_registry.h"
#include "core/host/webui/web_ui_impl.h"
#include "core/common/browser_plugin/browser_plugin_constants.h"
#include "core/common/browser_plugin/browser_plugin_messages.h"
#include "core/common/drag_messages.h"
#include "core/common/frame_messages.h"
#include "core/common/input_messages.h"
#include "core/common/page_messages.h"
#include "core/common/page_state_serialization.h"
...
#include "core/host/content_browser_client.h"
...
#include "core/host/javascript_dialog_manager.h"
...
#include "core/host/application_window_host_iterator.h"
#include "core/host/restore_type.h"
#include "core/host/security_style_explanations.h"
#include "core/host/ssl_status.h"
#include "core/host/storage_partition.h"
#include "core/host/web_contents_binding_set.h"
#include "core/host/web_contents_delegate.h"
...
#include "core/host/media/android/media_web_contents_observer_android.h"
#include "core/host/web_contents/web_contents_android.h"
...
bool HasMatchingWidgetHost(FrameTree* tree, ApplicationWindowHost* host) {
  ...
  for (FrameTreeNode* node : tree->Nodes()) {
    if (node->current_frame_host()->GetApplicationWindowHost() == host)
      return true;
  }
  return false;
}
...
WebContents* WebContents::CreateWithSessionStorage(
  ...
  WebContentsImpl* new_contents = new WebContentsImpl(params.application_contents);
...
WebContentsImpl::WebContentsImpl(ApplicationContents* application_contents)
    : delegate_(nullptr),
      controller_(this, application_contents),
...
WebContentsImpl::~WebContentsImpl() {
  ...
  for (ApplicationWindowHost* widget : created_widgets_)
...
bool WebContentsImpl::OnMessageReceived(RenderViewHostImpl* application_window_host,
  ...
  bool handled = true;
  IPC_BEGIN_MESSAGE_MAP_WITH_PARAM(WebContentsImpl, message, application_window_host)
...
ApplicationContents* WebContentsImpl::GetApplicationContents() const {
  return controller_.GetApplicationContents();
}
...
ApplicationWindowHostView* WebContentsImpl::GetApplicationWindowHostView() const {
  return GetRenderManager()->GetApplicationWindowHostView();
}

ApplicationWindowHostView* WebContentsImpl::GetTopLevelApplicationWindowHostView() {
  if (GetOuterWebContents())
    return GetOuterWebContents()->GetTopLevelApplicationWindowHostView();
  return GetRenderManager()->GetApplicationWindowHostView();
}

ApplicationWindowHostView* WebContentsImpl::GetFullscreenApplicationWindowHostView()
    const {
  if (auto* widget_host = GetFullscreenApplicationWindowHost())
...
std::vector<WebContentsImpl*> WebContentsImpl::GetInnerWebContents() {
  if (browser_plugin_embedder_) {
    ...
    GetApplicationContents()->GetGuestManager()->ForEachGuest(
...
#if defined(OS_ANDROID)
std::set<ApplicationWindowHost*> WebContentsImpl::GetAllApplicationWindowHosts() {
  std::set<ApplicationWindowHost*> set;
  if (ShowingInterstitialPage()) {
    auto* host =
  ...
  for (RenderFrameHost* rfh : GetAllFrames()) {
    auto* host = static_cast<RenderFrameHostImpl*>(rfh)->GetApplicationWindowHost();
    DCHECK(host);
    set.insert(host);
  }
  return set;
...
void WebContentsImpl::SetImportance(ChildProcessImportance importance) {
  ...
  // the set returned by |GetAllApplicationWindowHosts()|.
  for (auto* host : GetAllApplicationWindowHosts())
...
void WebContentsImpl::AttachToOuterWebContentsFrame(
  ...
  // Render{View,Frame}Host must be initialized and the ApplicationWindowHostView
  ...
  GetMainFrame()->Init();
  if (!render_manager->GetApplicationWindowHostView())
    CreateApplicationWindowHostViewForRenderManager(GetRenderViewHost());
...
void WebContentsImpl::ReattachToOuterWebContentsFrame() {
  ...
  render_manager->SetRWHViewForInnerContents(
      render_manager->GetApplicationWindowHostView());
...
void WebContentsImpl::NotifyPreferencesChanged() {
  std::set<RenderViewHost*> application_window_host_set;
  for (FrameTreeNode* node : frame_tree_.Nodes()) {
    ApplicationWindowHost* application_window_host =
        node->current_frame_host()->GetApplicationWindowHost();
    if (!application_window_host)
      continue;
    RenderViewHost* application_window_host = RenderViewHost::From(application_window_host);
    if (!application_window_host)
      continue;
    application_window_host_set.insert(application_window_host);
  }
  for (RenderViewHost* application_window_host : application_window_host_set)
    application_window_host->OnWebkitPreferencesChanged();
}
...
WebContents* WebContentsImpl::Clone() {
  ...
  // before.
  CreateParams create_params(GetApplicationContents(), GetSiteInstance());
...
void WebContentsImpl::Init(const WebContents::CreateParams& params) {
  ...
  if (!site_instance)
    site_instance = SiteInstance::Create(params.application_contents);

  // A main RenderFrameHost always has a ApplicationWindowHost, since it is always a
  // local root by definition.
  // TODO(avi): Once RenderViewHostImpl has-a ApplicationWindowHost, it will no
  ...
  if (GuestMode::IsCrossProcessFrameGuest(this)) {
    view_.reset(new WebContentsViewChildFrame(
        this, delegate, &application_window_host_delegate_view_));
  } else {
...
std::set<ApplicationWindowHostView*>
WebContentsImpl::GetApplicationWindowHostViewsInTree() {
  std::set<ApplicationWindowHostView*> set;
  if (ShowingInterstitialPage()) {
    if (ApplicationWindowHostView* rwhv = GetApplicationWindowHostView())
      set.insert(rwhv);
  } else {
    for (RenderFrameHost* rfh : GetAllFrames()) {
      if (ApplicationWindowHostView* rwhv = static_cast<RenderFrameHostImpl*>(rfh)
                                           ->frame_tree_node()
                                           ->render_manager()
                                           ->GetApplicationWindowHostView()) {
        set.insert(rwhv);
      }
    }
  }
...
void WebContentsImpl::LostCapture(ApplicationWindowHost* application_window_host) {
  if (!RenderViewHostImpl::From(application_window_host))
    return;
...
void WebContentsImpl::RenderWidgetCreated(
    ApplicationWindowHost* application_window_host) {
  created_widgets_.insert(application_window_host);
}
...
void WebContentsImpl::RenderWidgetDeleted(
  ...
  // us here.
  created_widgets_.erase(application_window_host);
...
void WebContentsImpl::RenderWidgetGotFocus(
  ...
  if (delegate_ && application_window_host && delegate_->EmbedsFullscreenWidget() &&
      application_window_host->GetView() == GetFullscreenApplicationWindowHostView()) {
    NotifyWebContentsFocused(application_window_host);
  }
}
...
void WebContentsImpl::RenderWidgetLostFocus(
  ...
  if (delegate_ && application_window_host && delegate_->EmbedsFullscreenWidget() &&
      application_window_host->GetView() == GetFullscreenApplicationWindowHostView()) {
    NotifyWebContentsLostFocus(application_window_host);
  }
}
...
ApplicationWindowHostInputEventRouter* WebContentsImpl::GetInputEventRouter() {
  ...
  if (!rwh_input_event_router_.get() && !is_being_destroyed_)
    rwh_input_event_router_.reset(new ApplicationWindowHostInputEventRouter);
...
ApplicationWindowHost* WebContentsImpl::GetFocusedApplicationWindowHost(
  ...
  ApplicationWindowHostView* view = focused_frame->current_frame_host()->GetView();
  if (!view)
    return nullptr;

  return ApplicationWindowHost::From(view->GetApplicationWindowHost());
}

ApplicationWindowHost* WebContentsImpl::GetApplicationWindowHostWithPageFocus() {
  ...
  return focused_web_contents->GetMainFrame()->GetApplicationWindowHost();
}
...
void WebContentsImpl::EnterFullscreenMode(const GURL& origin) {
  ...
  ApplicationWindowHostView* const widget_view = GetFullscreenApplicationWindowHostView();
  if (widget_view) {
    ApplicationWindowHost::From(widget_view->GetApplicationWindowHost())
...
void WebContentsImpl::ExitFullscreenMode(bool will_cause_resize) {
  ...
  if (!will_cause_resize) {
    if (ApplicationWindowHostView* rwhv = GetApplicationWindowHostView()) {
        if (ApplicationWindowHost* application_window_host = rwhv->GetApplicationWindowHost())
...
void WebContentsImpl::RequestToLockMouse(
  ...
  for (WebContentsImpl* current = this; current;
    ...
    if (current->mouse_lock_widget_) {
      application_window_host->GotResponseToLockMouseRequest(false);
      return;
    }
  }
  ...
  for (FrameTreeNode* node : frame_tree_.Nodes()) {
    if (node->current_frame_host()->GetApplicationWindowHost() ==
  ...
  if (widget_in_frame_tree && delegate_) {
    for (WebContentsImpl* current = this; current;
         current = current->GetOuterWebContents()) {
      current->mouse_lock_widget_ = application_window_host;
    }
...
void WebContentsImpl::LostMouseLock(ApplicationWindowHost* application_window_host) {
  CHECK(mouse_lock_widget_);

  if (mouse_lock_widget_->delegate()->GetAsWebContents() != this)
    return mouse_lock_widget_->delegate()->LostMouseLock(application_window_host);
...
bool WebContentsImpl::HasMouseLock(ApplicationWindowHost* application_window_host) {
  ...
  // platform ApplicationWindowHostView needs to hold the mouse lock from the OS.
  return mouse_lock_widget_ == application_window_host &&
         GetTopLevelApplicationWindowHostView()->IsMouseLocked();
}

ApplicationWindowHost* WebContentsImpl::GetMouseLockWidget() {
...
bool WebContentsImpl::RequestKeyboardLock(
  ...
  if (application_window_host->delegate()->GetAsWebContents() != this) {
    NOTREACHED();
    return false;
  }
...
ApplicationWindowHost* WebContentsImpl::GetKeyboardLockWidget() {
...
void WebContentsImpl::CreateNewWindow(
  ...
  scoped_refptr<SiteInstance> site_instance =
      params.opener_suppressed && !is_guest
          ? SiteInstance::CreateForURL(GetApplicationContents(), params.target_url)
  ...
  const std::string& partition_id =
      GetContentClient()->browser()->
          GetStoragePartitionIdForSite(GetApplicationContents(),
                                       site_instance->GetSiteURL());
  StoragePartition* partition = ApplicationContents::GetStoragePartition(
      GetApplicationContents(), site_instance.get());
  ...
  CreateParams create_params(GetApplicationContents(), site_instance.get());
...
void WebContentsImpl::CreateNewWidget(int32_t render_process_id,
  ...
  ApplicationWindowHost* widget_host = new ApplicationWindowHost(
      this, process, route_id, std::move(widget), IsHidden());

  ApplicationWindowHostView* widget_view =
...
void WebContentsImpl::ShowCreatedWindow(int process_id,
  ...
  if (popup) {
    ...
    ApplicationWindowHost* rwh = popup->GetMainFrame()->GetApplicationWindowHost();
...
void WebContentsImpl::ShowCreatedWidget(int process_id,
                                        ...
                                        bool is_fullscreen,
                                        const gfx::Rect& initial_rect) {
  ApplicationWindowHostView* widget_host_view =
      static_cast<ApplicationWindowHostView*>(
  ...
  if (GetOuterWebContents()) {
    view = GetOuterWebContents()->GetApplicationWindowHostView();
  } else {
  ...
  ApplicationWindowHost* application_window_host_impl = widget_host_view->host();
  application_window_host_impl->Init();
  // Only allow privileged mouse lock for fullscreen render widget, which is
  // used to implement Pepper Flash fullscreen.
  application_window_host_impl->set_allow_privileged_mouse_lock(is_fullscreen);
}
...
ApplicationWindowHostView* WebContentsImpl::GetCreatedWidget(int process_id,
                                                        int route_id) {
...
RenderFrameHost* WebContentsImpl::GetGuestByInstanceID(
  ...
  BrowserPluginGuestManager* guest_manager =
      GetApplicationContents()->GetGuestManager();
  if (!guest_manager)
    return nullptr;
...
void WebContentsImpl::SendScreenRects() {
  ...
  ApplicationWindowHostView* rwhv =
      static_cast<ApplicationWindowHostView*>(GetApplicationWindowHostView());
  if (rwhv) {
...
bool WebContentsImpl::IsWidgetForMainFrame(
    ApplicationWindowHost* application_window_host) {
  return application_window_host == GetMainFrame()->GetApplicationWindowHost();
}

BrowserAccessibilityManager*
    WebContentsImpl::GetRootAccessibilityManager() {
...
BrowserAccessibilityManager*
    WebContentsImpl::GetOrCreateRootAccessibilityManager() {
  RenderFrameHostImpl* rfh = static_cast<RenderFrameHostImpl*>(
      ShowingInterstitialPage() ? GetInterstitialPage()->GetMainFrame()
                                : GetMainFrame());
  return rfh ? rfh->GetOrCreateAccessibilityManager() : nullptr;
}
...
void WebContentsImpl::ResizeDueToAutoResize(
  ...
  if (application_window_host != GetRenderViewHost()->GetWidget())
    return;

  auto_resize_size_ = new_size;

  // Out-of-process iframe visible viewport sizes usually come from the
  // top-level ApplicationWindowHostView, but when auto-resize is enabled on the
  // top frame then that size is used instead.
  for (FrameTreeNode* node : frame_tree_.Nodes()) {
    if (node->current_frame_host()->is_local_root()) {
      ApplicationWindowHost* host =
          node->current_frame_host()->GetApplicationWindowHost();
      if (host != application_window_host)
...
void WebContentsImpl::AttachInterstitialPage(
  ...
  if (node_.OuterContentsFrameTreeNode()) {
    if (GetRenderManager()->GetProxyToOuterDelegate()) {
      DCHECK(
          static_cast<ApplicationWindowHostView*>(interstitial_page->GetView())
  ...
  if (accessibility_mode_.has_mode(ui::AXMode::kNativeAPIs)) {
    ...
    if (rfh) {
      BrowserAccessibilityManager* accessibility_manager =
          rfh->GetOrCreateAccessibilityManager();
      if (accessibility_manager)
...
void WebContentsImpl::SetHistoryOffsetAndLengthForView(
    RenderViewHost* application_window_host,
    int history_offset,
    int history_length) {
  application_window_host->Send(new PageMsg_SetHistoryOffsetAndLength(
      application_window_host->GetRoutingID(), history_offset, history_length));
}
...
void WebContentsImpl::OnSavePage() {
  ...
  if (!IsSavable()) {
    download::RecordSaveContainerEvent(
        download::SAVE_CONTAINER_DOWNLOAD_ON_NON_HTML);
  ...
  // Create the save container and possibly prompt the user for the name to save
  // the page as. The user prompt is an asynchronous operation that runs on
  // another thread.
  save_container_ = new SaveContainer(this);
  save_container_->GetSaveInfo();
}
...
bool WebContentsImpl::SavePage(const base::FilePath& main_file,
  ...
  save_container_ = new SaveContainer(this, save_type, main_file, dir_path);
  return save_container_->Init(SaveContainerDownloadCreatedCallback());
}
...
void WebContentsImpl::SaveFrameWithHeaders(
  ...
  StoragePartition* storage_partition = ApplicationContents::GetStoragePartition(
      GetApplicationContents(), frame_host->GetSiteInstance());
  int64_t post_id = -1;
  if (is_main_frame) {
  ...
  ApplicationContents::GetDownloadManager(GetApplicationContents())
...
void WebContentsImpl::NotifyWebContentsFocused(
    ApplicationWindowHost* application_window_host) {
  for (auto& observer : observers_)
    observer.OnWebContentsFocused(application_window_host);
}

void WebContentsImpl::NotifyWebContentsLostFocus(
    ApplicationWindowHost* application_window_host) {
  for (auto& observer : observers_)
    observer.OnWebContentsLostFocus(application_window_host);
}

void WebContentsImpl::SystemDragEnded(ApplicationWindowHost* source_rwh) {
  if (source_rwh)
...
void WebContentsImpl::DidNavigateMainFramePostCommit(
  ...
  if (details.is_navigation_to_different_page()) {
    ...
    ApplicationWindowHostView* rwhvb =
        static_cast<ApplicationWindowHostView*>(GetApplicationWindowHostView());
    if (rwhvb)
...
void WebContentsImpl::ViewSource(RenderFrameHostImpl* frame) {
  ...
  WebContentsImpl* view_source_contents =
      static_cast<WebContentsImpl*>(Create(CreateParams(GetApplicationContents())));
...
// Notifies the ApplicationWindowHost instance about the fact that the page is
...
void WebContentsImpl::NotifyFrameSwapped(RenderFrameHost* old_host,
  ...
  if (importance_host) {
    static_cast<RenderFrameHostImpl*>(new_host)
        ->GetApplicationWindowHost()
        ->SetImportance(importance_host->GetApplicationWindowHost()->importance());
  }
#endif
  for (auto& observer : observers_)
...
void WebContentsImpl::ShowContextMenu(RenderFrameHost* render_frame_host,
  ...
  application_window_host_delegate_view_->ShowContextMenu(render_frame_host,
                                                   context_menu_params);
}
...
void WebContentsImpl::RunJavaScriptDialog(RenderFrameHost* render_frame_host,
  ...
  auto* application_window_host_impl =
      static_cast<RenderFrameHostImpl*>(render_frame_host)
          ->GetApplicationWindowHost();
  if (application_window_host_impl)
    application_window_host_impl->ForceFirstFrameAfterNavigationTimeout();
...
void WebContentsImpl::SetShowingContextMenu(bool showing) {
  ...
  if (auto* view = GetApplicationWindowHostView()) {
    ...
    static_cast<ApplicationWindowHostView*>(view)->SetShowingContextMenu(
        showing);
  }
}
...
void WebContentsImpl::RenderViewCreated(RenderViewHost* application_window_host) {
  if (delegate_)
    view_->SetOverscrollControllerEnabled(CanOverscrollContent());
  ...
  NotificationService::current()->Notify(
      ...
      Source<WebContents>(this),
      Details<RenderViewHost>(application_window_host));

  view_->RenderViewCreated(application_window_host);

  for (auto& observer : observers_)
    observer.RenderViewCreated(application_window_host);
  if (delegate_)
    RenderFrameDevToolsAgentHost::WebContentsCreated(this);
}
...
void WebContentsImpl::UpdateTargetURL(RenderViewHost* application_window_host,
                                      const GURL& url) {
  if (fullscreen_widget_routing_id_ != MSG_ROUTING_NONE) {
    ...
    if (fs && fs->GetApplicationWindowHost() != application_window_host->GetWidget())
      return;
  }
...
void WebContentsImpl::SetAsFocusedWebContentsIfNecessary() {
  ...
  if (old_contents)
    old_contents->GetMainFrame()->GetApplicationWindowHost()->SetPageFocus(false);
  ...
  if (ShowingInterstitialPage()) {
    ...
    GetMainFrame()->GetApplicationWindowHost()->SetPageFocus(true);
  }
}
...
void WebContentsImpl::FocusOwningWebContents(
  ...
  ApplicationWindowHost* focused_widget =
      GetFocusedApplicationWindowHost(application_window_host);

  if (focused_widget != application_window_host &&
      (!focused_widget ||
       focused_widget->delegate() != application_window_host->delegate())) {
    SetAsFocusedWebContentsIfNecessary();
  }
}
...
void WebContentsImpl::RendererUnresponsive(
    ApplicationWindowHost* application_window_host) {
  for (auto& observer : observers_)
    observer.OnRendererUnresponsive(application_window_host->GetProcess());

  if (ShouldIgnoreUnresponsiveRenderer())
    return;

  if (!application_window_host->renderer_initialized())
    return;

  if (delegate_)
    delegate_->RendererUnresponsive(this, application_window_host);
}

void WebContentsImpl::RendererResponsive(
    ApplicationWindowHost* application_window_host) {
  if (delegate_)
    delegate_->RendererResponsive(this, application_window_host);
}
...
void WebContentsImpl::RenderProcessGoneFromRenderManager(
  ...
  RenderViewTerminated(application_window_host, crashed_status_, crashed_error_code_);
}
...
void WebContentsImpl::CreateApplicationWindowHostViewForRenderManager(
    RenderViewHost* application_window_host) {
  ApplicationWindowHostView* rwh_view =
      view_->CreateViewForWidget(application_window_host->GetWidget(), false);
...
bool WebContentsImpl::CreateRenderViewForRenderManager(
  ...
  if (proxy_routing_id == MSG_ROUTING_NONE)
    CreateApplicationWindowHostViewForRenderManager(application_window_host);

  if (!static_cast<RenderViewHostImpl*>(application_window_host)
  ...
  SetHistoryOffsetAndLengthForView(application_window_host,
  ...
  ApplicationWindowHostView* rwh_view = application_window_host->GetWidget()->GetView();
  if (rwh_view) {
    if (ApplicationWindowHost* application_window_host = rwh_view->GetApplicationWindowHost())
      application_window_host->SynchronizeVisualProperties();
  }
#endif
...
bool WebContentsImpl::CreateRenderFrameForRenderManager(
  ...
  // TODO(nasko): When ApplicationWindowHost is owned by RenderFrameHost, the passed
  // RenderFrameHost will have to be associated with the appropriate
  // ApplicationWindowHostView or a new one should be created here.
...
ApplicationWindowHost* WebContentsImpl::GetFullscreenApplicationWindowHost() const {
...
bool WebContentsImpl::SetDeviceEmulationSize(const gfx::Size& new_size) {
  ...
  ApplicationWindowHostView* rwhv = GetMainFrame()->GetView();
