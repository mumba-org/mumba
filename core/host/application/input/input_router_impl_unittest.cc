...
#include "core/host/application/input/mock_input_router_client.h"
#include "core/common/content_constants_internal.h"
#include "core/common/edit_command.h"
...
#include "content/public/test/test_application_contents.h"
...
class MockInputRouterImplClient : public InputRouterImplClient {
 public:
  mojom::WindowInputHandler* GetWindowInputHandler() override {
  ...
  MockWindowInputHandler::MessageVector GetAndResetDispatchedMessages() {
  ...
  void Forwardcommon::GestureEventWithLatencyInfo(
      ...
      const ui::LatencyInfo& latency_info) override {
    input_router_client_.Forwardcommon::GestureEventWithLatencyInfo(gesture_event,
                                                            latency_info);
  }
  ...
  void set_filter_state(common::InputEventAckState filter_state) {
  ...
  MockWindowInputHandler widget_input_handler_;
};
...
class InputRouterImplTest : public testing::Test {
 ...
 protected:
  using DispatchedMessages = MockWindowInputHandler::MessageVector;
  // testing::Test
  void SetUp() override {
  ...
  void SimulateWheelEvent(float x,
                          ...
                          int modifiers,
                          bool precise) {
    input_router_->SendWheelEvent(common::MouseWheelEventWithLatencyInfo(
  ...
  void SimulateWheelEventWithPhase(float x,
    ...
    input_router_->SendWheelEvent(common::MouseWheelEventWithLatencyInfo(wheel_event));
  }
  ...
  void SimulateMouseEvent(WebInputEvent::Type type, int x, int y) {
    input_router_->SendMouseEvent(common::MouseEventWithLatencyInfo(
  ...
  void SimulateGestureEvent(WebGestureEvent gesture) {
    ...
    input_router_->SendGestureEvent(common::GestureEventWithLatencyInfo(gesture));
  }
  ...
  uint32_t SendTouchEvent() {
    ...
    input_router_->SendTouchEvent(common::TouchEventWithLatencyInfo(touch_event_));
...
// Tests ported from ApplicationWindowHostTest --------------------------------------
...
TEST_F(InputRouterImplTest,
  ...
  CancelTouchTimeout();
  dispatched_messages[0]->ToEvent()->CallCallback(
      common::InputEventAckSource::COMPOSITOR_THREAD, ui::LatencyInfo(),
