...
ApplicationWindowHostLatencyTracker::ApplicationWindowHostLatencyTracker(
    bool metric_sampling,
    ApplicationWindowHostDelegate* delegate)
    : LatencyTracker(),//metric_sampling, ukm::kInvalidSourceId),//, GenerateUkmSourceId()),
      last_event_id_(0),
      latency_component_id_(0),
      has_seen_first_gesture_scroll_update_(false),
      active_multi_finger_gesture_(false),
      touch_start_default_prevented_(false),
      application_window_host_delegate_(delegate) {}

ApplicationWindowHostLatencyTracker::~ApplicationWindowHostLatencyTracker() {}

void ApplicationWindowHostLatencyTracker::Initialize(int routing_id,
                                                int process_id) {
...
void ApplicationWindowHostLatencyTracker::ComputeInputLatencyHistograms(
  ...
  bool action_prevented = ack_result == common::INPUT_EVENT_ACK_STATE_CONSUMED;
...
void ApplicationWindowHostLatencyTracker::OnInputEvent(
  ...
  //if (!set_url_for_ukm_ && application_window_host_delegate_ &&
  //    ukm_source_id() != ukm::kInvalidSourceId) {
  //  application_window_host_delegate_->UpdateUrlForUkmSource(ukm::UkmRecorder::Get(),
  //                                                      ukm_source_id());
  ...
  // This is the only portal to add the BEGIN_RWH component. So this component
  ...
  if (event.GetType() == blink::WebInputEvent::kGestureScrollBegin) {
    ...
    base::TimeTicks original_component;
...
void ApplicationWindowHostLatencyTracker::OnInputEventAck(
    const blink::WebInputEvent& event,
    LatencyInfo* latency, common::InputEventAckState ack_result) {
  DCHECK(latency);
  ...
  if (!rendering_scheduled || latency->coalesced()) {
    latency->AddLatencyNumber(
        ui::INPUT_EVENT_LATENCY_TERMINATED_NO_SWAP_COMPONENT);
  }
