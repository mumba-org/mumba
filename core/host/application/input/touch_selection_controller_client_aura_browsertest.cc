...
#include "core/host/overscroll_configuration.h"
...
class TestTouchSelectionControllerClientAura
 ...
 public:
  explicit TestTouchSelectionControllerClientAura(
      ApplicationWindowHostViewAura* rwhva)
...
class TouchSelectionControllerClientAuraTest : public ContentBrowserTest {
 ...
 protected:
  ...
  ApplicationWindowHostViewAura* GetApplicationWindowHostViewAura() {
    return static_cast<ApplicationWindowHostViewAura*>(
        shell()->web_contents()->GetApplicationWindowHostView());
  }
  ...
  void InitSelectionController() {
    ApplicationWindowHostViewAura* rwhva = GetApplicationWindowHostViewAura();
...
class GestureLongPressWaiter : public ApplicationWindowHost::InputEventObserver {
 public:
  GestureLongPressWaiter(ApplicationWindowHost* rwh)
      : rwh_(static_cast<ApplicationWindowHost*>(rwh)->GetWeakPtr()),
  ...
  void OnInputEventAck(common::InputEventAckSource,
 ...
 private:
  base::WeakPtr<ApplicationWindowHost> rwh_;
  std::unique_ptr<base::RunLoop> run_loop_;
...
class TouchSelectionControllerClientAuraSiteIsolationTest
 ...
 public:
  ...
  void SelectWithLongPress(gfx::Point point,
                           ApplicationWindowHostView* expected_target) {
    // Get main frame view for event insertion.
    ApplicationWindowHostViewAura* main_view = GetApplicationWindowHostViewAura();
    ...
    GestureLongPressWaiter long_press_waiter(
        expected_target->GetApplicationWindowHost());
 ...
 private:
  void SendTouch(ApplicationWindowHostViewAura* view,
                 ui::EventType type,
                 gfx::Point point) {
  ...
  void SendGestureTap(ApplicationWindowHostViewAura* view, gfx::Point point) {
  ...
  void SendGestureLongPress(ApplicationWindowHostViewAura* view, gfx::Point point) {
    ui::GestureEventDetails long_press_details(ui::ET_GESTURE_LONG_PRESS);
...
class FrameStableObserver {
 public:
  FrameStableObserver(ApplicationWindowHostView* view, base::TimeDelta delta)
 ...
 private:
  ApplicationWindowHostView* view_;
...
IN_PROC_BROWSER_TEST_P(TouchSelectionControllerClientAuraSiteIsolationTest,
  ...
  ApplicationWindowHostViewAura* parent_view =
      static_cast<ApplicationWindowHostViewAura*>(
          root->current_frame_host()->GetApplicationWindowHost()->GetView());
  ...
  ApplicationWindowHostViewChildFrame* child_view =
      static_cast<ApplicationWindowHostViewChildFrame*>(
          child->current_frame_host()->GetApplicationWindowHost()->GetView());
...
IN_PROC_BROWSER_TEST_F(TouchSelectionControllerClientAuraScaleFactorTest,
  ...
  const ui::TouchSelectionController* controller =
      GetApplicationWindowHostViewAura()->selection_controller();
