...
#include "core/common/input/web_touch_event_traits.h"
...
class PassthroughTouchEventQueueTest : public testing::Test,
 ...
 public:
  ...
  void SendTouchEventImmediately(
      const common::TouchEventWithLatencyInfo& event) override {
  ...
  void OnTouchEventAck(const common::TouchEventWithLatencyInfo& event,
    ...
    if (followup_gesture_event_) {
      ...
      queue_->OnGestureScrollEvent(common::GestureEventWithLatencyInfo(
 ...
 protected:
  ...
  void SendTouchEvent(WebTouchEvent event) {
    ...
    queue_->QueueEvent(common::TouchEventWithLatencyInfo(event, ui::LatencyInfo()));
  }
  ...
  void SendGestureEvent(WebInputEvent::Type type) {
    ...
    queue_->OnGestureScrollEvent(
        common::GestureEventWithLatencyInfo(event, ui::LatencyInfo()));
  }

  void SendTouchEventAck(common::InputEventAckState ack_result) {
    DCHECK(!sent_events_ids_.empty());
    queue_->ProcessTouchAck(common::InputEventAckSource::COMPOSITOR_THREAD, ack_result,
                            ui::LatencyInfo(), sent_events_ids_.front());
    sent_events_ids_.pop_front();
  }

  void SendTouchEventAckLast(common::InputEventAckState ack_result) {
    DCHECK(!sent_events_ids_.empty());
    queue_->ProcessTouchAck(common::InputEventAckSource::COMPOSITOR_THREAD, ack_result,
  ...
  void SendTouchEventAckWithID(common::InputEventAckState ack_result,
                               int unique_event_id) {
    queue_->ProcessTouchAck(common::InputEventAckSource::COMPOSITOR_THREAD, ack_result,
  ...
  void SendGestureEventAck(WebInputEvent::Type type,
    ...
    common::GestureEventWithLatencyInfo event(type, blink::WebInputEvent::kNoModifiers,
  ...
  void SetSyncAckResult(common::InputEventAckState sync_ack_result) {
    sync_ack_result_.reset(new common::InputEventAckState(sync_ack_result));
  }
  ...
  common::InputEventAckState acked_event_state() const {
