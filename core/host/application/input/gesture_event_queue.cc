...
#include "core/host/application/input/touchscreen_tap_suppression_controller.h"
...
GestureEventQueue::GestureEventWithLatencyInfoAndAckState::GestureEventWithLatencyInfoAndAckState(
        const common::GestureEventWithLatencyInfo& event)
    : common::GestureEventWithLatencyInfo(event) {}
...
bool GestureEventQueue::OnScrollBegin(
  ...
  if (synthetic && have_unsent_events) {
    common::GestureEventWithLatencyInfo* last_event = &coalesced_gesture_events_.back();
...
void GestureEventQueue::ProcessGestureAck(common::InputEventAckSource ack_source,
  ...
  for (auto& outstanding_event : coalesced_gesture_events_) {
    if (outstanding_event.ack_state() != common::INPUT_EVENT_ACK_STATE_UNKNOWN)
      continue;
...
void GestureEventQueue::AckGestureEventToClient(
    const common::GestureEventWithLatencyInfo& event_with_latency,
...
void GestureEventQueue::LegacyProcessGestureAck(
  ...
  common::GestureEventWithLatencyInfo event_with_latency =
  ...
  const common::GestureEventWithLatencyInfo& first_gesture_event =
  ...
  common::GestureEventWithLatencyInfo second_gesture_event;
...
void GestureEventQueue::QueueScrollOrPinchAndForwardIfNecessary(
  ...
  if (!unsent_events_count) {
    ...
    if (coalesced_gesture_events_.size() == 1) {
      ...
      const common::GestureEventWithLatencyInfo& first_event =
          coalesced_gesture_events_.front();
  ...
  common::GestureEventWithLatencyInfo scroll_event;
  scroll_event.event = coalesced_events.first;
  scroll_event.latency = oldest_latency;

  common::GestureEventWithLatencyInfo pinch_event;
