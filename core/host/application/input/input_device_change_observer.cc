...
InputDeviceChangeObserver::InputDeviceChangeObserver(ApplicationWindowHost* rvh) {
  application_window_host_ = rvh;
#if defined(OS_WIN)
...
void InputDeviceChangeObserver::NotifyRenderViewHost() {
  common::WebPreferences prefs = application_window_host_->GetWebkitPreferences();
  ...
  if (input_device_changed) {
    ...
    application_window_host_->OnWebkitPreferencesChanged();
  }
}
