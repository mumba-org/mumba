...
// Dont know if we will use the touch emulator. But if we do
// we need to fix the images being used as cursors
...
gfx::SizeF TouchEmulator::InitCursorFromResource(
    common::WebCursor* cursor, float scale, int resource_id) {
  gfx::Image& cursor_image = ui::ResourceBundle::GetSharedInstance().GetNativeImageNamed(resource_id);
      //common::GetClient()->GetNativeImageNamed(resource_id);
  common::CursorInfo cursor_info;
...
bool TouchEmulator::HandleTouchEvent(const blink::WebTouchEvent& event) {
  ...
  bool is_sequence_start = common::WebTouchEventTraits::IsTouchSequenceStart(event);
...
bool TouchEmulator::HandleTouchEventAck(
    const blink::WebTouchEvent& event, common::InputEventAckState ack_result) {
  bool is_sequence_end = common::WebTouchEventTraits::IsTouchSequenceEnd(event);
  if (emulated_stream_active_sequence_count_) {
    if (is_sequence_end)
      emulated_stream_active_sequence_count_--;

    int taps_count_before = pending_taps_count_;
    const bool event_consumed = ack_result == common::INPUT_EVENT_ACK_STATE_CONSUMED;
    if (gesture_provider_) {
