...
SyntheticPointerAction::SyntheticPointerAction(
    ...
    : params_(params),
      gesture_source_type_(common::SyntheticGestureParams::DEFAULT_INPUT),
...
SyntheticGesture::Result SyntheticPointerAction::ForwardInputEvents(
  ...
  if (state_ == UNINITIALIZED) {
    ...
    if (gesture_source_type_ == common::SyntheticGestureParams::DEFAULT_INPUT)
  ...
  DCHECK_NE(gesture_source_type_, common::SyntheticGestureParams::DEFAULT_INPUT);
  if (gesture_source_type_ == common::SyntheticGestureParams::DEFAULT_INPUT)
...
SyntheticPointerAction::ForwardTouchOrMouseInputEvents(
  ...
  common::SyntheticPointerActionListParams::ParamList& param_list =
      params_.params[num_actions_dispatched_];
  for (const common::SyntheticPointerActionParams& param : param_list) {
    if (!synthetic_pointer_driver_->UserInputCheck(param))
      return INVALID;

    switch (param.pointer_action_type()) {
      case common::SyntheticPointerActionParams::PointerActionType::PRESS:
        synthetic_pointer_driver_->Press(param.position().x(),
                                         param.position().y(), param.index(),
                                         param.button());
        break;
      case common::SyntheticPointerActionParams::PointerActionType::MOVE:
        synthetic_pointer_driver_->Move(param.position().x(),
                                        param.position().y(), param.index());
        break;
      case common::SyntheticPointerActionParams::PointerActionType::RELEASE:
        synthetic_pointer_driver_->Release(param.index(), param.button());
        break;
      case common::SyntheticPointerActionParams::PointerActionType::IDLE:
        break;
      case common::SyntheticPointerActionParams::PointerActionType::NOT_INITIALIZED:
        return INVALID;
    }
  }
