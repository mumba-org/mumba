...
class GestureEventQueueTest : public testing::Test,
 ...
 public:
  ...
  void SendGestureEventImmediately(
      const common::GestureEventWithLatencyInfo& event) override {
    ++sent_gesture_event_count_;
    if (sync_ack_result_) {
      std::unique_ptr<common::InputEventAckState> ack_result =
  ...
  void SendMouseEventImmediately(
      const common::MouseEventWithLatencyInfo& event) override {}
 ...
 protected:
  ...
  void SimulateGestureEvent(const WebGestureEvent& gesture) {
    queue()->QueueEvent(common::GestureEventWithLatencyInfo(gesture));
  }
  ...
  void SendInputEventACK(WebInputEvent::Type type,
                         common::InputEventAckState ack) {
    queue()->ProcessGestureAck(common::InputEventAckSource::COMPOSITOR_THREAD, ack,
                               type, ui::LatencyInfo());
  }
  ...
  void set_synchronous_ack(common::InputEventAckState ack_result) {
    sync_ack_result_.reset(new common::InputEventAckState(ack_result));
  }
