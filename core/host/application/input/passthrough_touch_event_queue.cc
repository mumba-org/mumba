...
#include "core/host/application/input/touch_timeout_handler.h"
...
PassthroughTouchEventQueue::TouchEventWithLatencyInfoAndAckState::
    TouchEventWithLatencyInfoAndAckState(const common::TouchEventWithLatencyInfo& event)
    : common::TouchEventWithLatencyInfo(event),
      ack_state_(common::INPUT_EVENT_ACK_STATE_UNKNOWN) {}
...
void PassthroughTouchEventQueue::SendTouchCancelEventForTouchEvent(
    const common::TouchEventWithLatencyInfo& event_to_cancel) {
  common::TouchEventWithLatencyInfo event = event_to_cancel;
  common::WebTouchEventTraits::ResetTypeAndTouchStates(
...
void PassthroughTouchEventQueue::QueueEvent(
  ...
  if (filter_result != FORWARD_TO_RENDERER) {
    ...
    common::InputEventAckState ack_state =
    ...
    event_with_ack_state.set_ack_info(common::InputEventAckSource::BROWSER, ack_state);
    outstanding_touches_.insert(event_with_ack_state);
    AckCompletedEvents();
    return;
  }
  common::TouchEventWithLatencyInfo cloned_event(event);
...
void PassthroughTouchEventQueue::PrependTouchScrollNotification() {
  ...
  common::TouchEventWithLatencyInfo touch(WebInputEvent::kTouchScrollStarted,
...
void PassthroughTouchEventQueue::OnGestureEventAck(
  ...
  if (event.event.GetType() == blink::WebInputEvent::kGestureScrollUpdate &&
    ...
    send_touch_events_async_ = (ack_result == common::INPUT_EVENT_ACK_STATE_CONSUMED);
  }
}
...
void PassthroughTouchEventQueue::FlushQueue() {
  ...
  while (!outstanding_touches_.empty()) {
    ...
    if (event.ack_state() == common::INPUT_EVENT_ACK_STATE_UNKNOWN)
      event.set_ack_info(common::InputEventAckSource::BROWSER,
                         common::INPUT_EVENT_ACK_STATE_NO_CONSUMER_EXISTS);
...
void PassthroughTouchEventQueue::AckCompletedEvents() {
  ...
  while (!outstanding_touches_.empty()) {
    auto iter = outstanding_touches_.begin();
    if (iter->ack_state() == common::INPUT_EVENT_ACK_STATE_UNKNOWN)
      break;
    TouchEventWithLatencyInfoAndAckState event = *iter;
    outstanding_touches_.erase(iter);
    AckTouchEventToClient(event, event.ack_source(), event.ack_state());
...
void PassthroughTouchEventQueue::AckTouchEventToClient(
    const common::TouchEventWithLatencyInfo& acked_event,
    common::InputEventAckSource ack_source,
    common::InputEventAckState ack_result) {
...
void PassthroughTouchEventQueue::SendTouchEventImmediately(
    common::TouchEventWithLatencyInfo* touch,
    bool wait_for_ack) {
...
PassthroughTouchEventQueue::FilterBeforeForwarding(const WebTouchEvent& event) {
  ...
  if (common::WebTouchEventTraits::IsTouchSequenceStart(event)) {
...
void PassthroughTouchEventQueue::UpdateTouchConsumerStates(
  ...
  } else if (common::WebTouchEventTraits::IsTouchSequenceEnd(event)) {
    maybe_has_handler_for_current_sequence_ = false;
  }
}
...
const common::TouchEventWithLatencyInfo&
