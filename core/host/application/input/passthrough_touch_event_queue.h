...
#ifndef MUMBA_HOST_APPLICATION_INPUT_PASSTHROUGH_TOUCH_EVENT_QUEUE_H_
#define MUMBA_HOST_APPLICATION_INPUT_PASSTHROUGH_TOUCH_EVENT_QUEUE_H_
...
class CONTENT_EXPORT PassthroughTouchEventQueueClient {
 public:
  ...
  virtual void SendTouchEventImmediately(
      const common::TouchEventWithLatencyInfo& event) = 0;
...
class CONTENT_EXPORT PassthroughTouchEventQueue {
 public:
  ...
  void QueueEvent(const common::TouchEventWithLatencyInfo& event);

  void PrependTouchScrollNotification();

  void ProcessTouchAck(common::InputEventAckSource ack_source,
                       common::InputEventAckState ack_result,
                       const ui::LatencyInfo& latency_info,
                       const uint32_t unique_touch_event_id);
  void OnGestureScrollEvent(const common::GestureEventWithLatencyInfo& gesture_event);
 ...
 private:
  ...
  class TouchEventWithLatencyInfoAndAckState
      : public common::TouchEventWithLatencyInfo {
   public:
    TouchEventWithLatencyInfoAndAckState(const common::TouchEventWithLatencyInfo&);
   ...
   private:
    common::InputEventAckSource ack_source_;
  ...
  void AckTouchEventToClient(const common::TouchEventWithLatencyInfo& acked_event,
                             common::InputEventAckSource ack_source,
                             common::InputEventAckState ack_result);

  void SendTouchEventImmediately(common::TouchEventWithLatencyInfo* touch,
                                 bool wait_for_ack);

  void AckCompletedEvents();

  bool IsTimeoutRunningForTesting() const;
  const common::TouchEventWithLatencyInfo& GetLatestEventForTesting() const;
...
#endif  // MUMBA_HOST_APPLICATION_INPUT_PASSTHROUGH_TOUCH_EVENT_QUEUE_H_
