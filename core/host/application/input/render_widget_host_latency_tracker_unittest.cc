...
#include "core/common/input/synthetic_web_input_event_builders.h"
#include "core/host/application/native_web_keyboard_event.h"
#include "content/test/test_content_browser_client.h"
#include "content/test/test_application_window_host.h"
...
void AddFakeComponentsWithTimeStamp(
    const ApplicationWindowHostLatencyTracker& tracker,
...
void AddFakeComponents(const ApplicationWindowHostLatencyTracker& tracker,
                       ui::LatencyInfo* latency) {
  latency->AddLatencyNumberWithTimestamp(
...
class ApplicationWindowHostLatencyTrackerTestBrowserClient
    : public TestContentBrowserClient {
 public:
  ApplicationWindowHostLatencyTrackerTestBrowserClient() {}
  ~ApplicationWindowHostLatencyTrackerTestBrowserClient() override {}

  ukm::TestUkmRecorder* GetTestUkmRecorder() { return &test_ukm_recorder_; }

 private:
  ukm::TestAutoSetUkmRecorder test_ukm_recorder_;
  DISALLOW_COPY_AND_ASSIGN(ApplicationWindowHostLatencyTrackerTestBrowserClient);
};

class ApplicationWindowHostLatencyTrackerTest
    : public RenderViewHostImplTestHarness {
 public:
  ApplicationWindowHostLatencyTrackerTest() : old_browser_client_(nullptr) {
    ResetHistograms();
  }
  ...
  ApplicationWindowHostLatencyTracker* tracker() { return tracker_.get(); }
  ...
  void SetUp() override {
    ...
    tracker_ =
        std::make_unique<ApplicationWindowHostLatencyTracker>(false, contents());
 ...
 protected:
  DISALLOW_COPY_AND_ASSIGN(ApplicationWindowHostLatencyTrackerTest);
  const int kTestRoutingId = 3;
  const int kTestProcessId = 1;
  std::unique_ptr<base::HistogramTester> histogram_tester_;
  std::unique_ptr<ApplicationWindowHostLatencyTracker> tracker_;
  ApplicationWindowHostLatencyTrackerTestBrowserClient test_browser_client_;
  ContentBrowserClient* old_browser_client_;
};

TEST_F(ApplicationWindowHostLatencyTrackerTest, TestValidEventTiming) {
...
TEST_F(ApplicationWindowHostLatencyTrackerTest, TestWheelToFirstScrollHistograms) {
  const GURL url(kUrl);
...
TEST_F(ApplicationWindowHostLatencyTrackerTest, TestWheelToScrollHistograms) {
  const GURL url(kUrl);
  size_t total_ukm_entry_count = 0;
  contents()->NavigateAndCommit(url);
  for (bool rendering_on_main : {false, true}) {
...
TEST_F(ApplicationWindowHostLatencyTrackerTest, TestTouchToFirstScrollHistograms) {
  const GURL url(kUrl);
  contents()->NavigateAndCommit(url);
  size_t total_ukm_entry_count = 0;
  for (bool rendering_on_main : {false, true}) {
...
TEST_F(ApplicationWindowHostLatencyTrackerTest, TestTouchToScrollHistograms) {
  const GURL url(kUrl);
  contents()->NavigateAndCommit(url);
  size_t total_ukm_entry_count = 0;
  for (bool rendering_on_main : {false, true}) {
...
TEST_F(ApplicationWindowHostLatencyTrackerTest,
...
TEST_F(ApplicationWindowHostLatencyTrackerTest, ScrollLatency) {
...
TEST_F(ApplicationWindowHostLatencyTrackerTest, TouchBlockingAndQueueingTime) {
  ...
  for (common::InputEventAckState blocking :
...
TEST_F(ApplicationWindowHostLatencyTrackerTest, KeyBlockingAndQueueingTime) {
  // These numbers are sensitive to where the histogram buckets are.
...
TEST_F(ApplicationWindowHostLatencyTrackerTest, KeyEndToEndLatency) {
  // These numbers are sensitive to where the histogram buckets are.
...
TEST_F(ApplicationWindowHostLatencyTrackerTest,
  ...
  SyntheticWebTouchEvent event;
  common::InputEventAckState ack_state = INPUT_EVENT_ACK_STATE_NOT_CONSUMED;
...
// touch gesture.
TEST_F(ApplicationWindowHostLatencyTrackerTest, WheelDuringMultiFingerTouch) {
...
TEST_F(ApplicationWindowHostLatencyTrackerTest, ExpectedQueueingTimeAccuracy) {
  // These numbers are sensitive to where the histogram buckets are.
