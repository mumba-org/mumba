...
bool TouchTimeoutHandler::AckedTimeoutEventRequiresCancel(
    common::InputEventAckState ack_result) const {
  DCHECK(HasTimeoutEvent());
  if (ack_result != common::INPUT_EVENT_ACK_STATE_NO_CONSUMER_EXISTS)
    return true;
  return !common::WebTouchEventTraits::IsTouchSequenceStart(timeout_event_.event);
}
