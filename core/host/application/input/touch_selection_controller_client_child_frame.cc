...
gfx::Point TouchSelectionControllerClientChildFrame::ConvertFromRoot(
  ...
  ApplicationWindowHostView* root_view = rwhv_->GetRootApplicationWindowHostView();
  if (root_view) {
...
void TouchSelectionControllerClientChildFrame::MoveCaret(
  ...
  ApplicationWindowHostDelegate* host_delegate = rwhv_->host()->delegate();
  if (host_delegate)
...
void TouchSelectionControllerClientChildFrame::RunContextMenu() {
  ...
  ApplicationWindowHost* host = rwhv_->host();
  HostThread::PostTask(HostThread::IO, FROM_HERE,
    base::BindOnce(&common::mojom::ApplicationWindow::ShowContextMenu, 
      base::Unretained(host->GetApplicationWindowInterface()),
      common::mojom::MenuSourceType::TOUCH,
