...
common::InputEventAckState MockInputRouterClient::FilterInputEvent(
...
void MockInputRouterClient::Forwardcommon::GestureEventWithLatencyInfo(
    const blink::WebGestureEvent& gesture_event,
    const ui::LatencyInfo& latency_info) {
  if (input_router_)
    input_router_->SendGestureEvent(
        common::GestureEventWithLatencyInfo(gesture_event, latency_info));
}
...
void MockInputRouterClient::ForwardWheelEventWithLatencyInfo(
  ...
  if (input_router_) {
    input_router_->SendWheelEvent(
        common::MouseWheelEventWithLatencyInfo(wheel_event, latency_info));
  }
}
