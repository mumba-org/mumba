...
class URLDataManagerBackend : public base::SupportsUserData::Data {
 public:
  ...
  URLDataManagerBackend(Domain* domain);
  ...
  void UpdateAppDataSource(const std::string& source_name,
  ...
  void DataAvailable(RequestID request_id, scoped_refptr<base::RefCountedMemory> contents, bool should_complete);

  // Look up the data source for the request. Returns the source if it is found,
  // else NULL.
  URLDataSource* GetDataSourceFromURL(const GURL& url);

  bool HaveDataSourceForURL(const GURL& url) const;
  ...
  // its embedder).
  static std::vector<std::string> GetAppSchemes();

 private:
  friend class URLRequestHostJob;

  typedef std::map<std::string,
      scoped_refptr<URLDataSource> > DataSourceMap;
  typedef std::map<RequestID, URLRequestHostJob*> PendingRequestMap;

  // Called by the job when it's starting up.
  // Returns false if |url| is not a URL managed by this object.
  bool StartRequest(const net::URLRequest* request, URLRequestHostJob* job);
  ...
  void RemoveRequest(URLRequestHostJob* job);

  // Returns true if the job exists in |pending_requests_|. False otherwise.
  // Called by ~URLRequestHostJob to verify that |pending_requests_| is kept
  // up to date.
  bool HasPendingJob(URLRequestHostJob* job) const;
  ...
  // All pending URLRequestHostJobs, keyed by ID of the request.
  // URLRequestHostJob calls into this object when it's constructed and
