...
#include "core/host/context_factory.h"
...
void RecyclableCompositorMac::Recycle(
  ...
  HostThread::PostTask(
      HostThread::UI, FROM_HERE, base::Bind(&ReleaseSpareCompositors));
}
...
BrowserCompositorMac::BrowserCompositorMac(
  ...
  SetApplicationWindowHostIsHidden(application_window_host_is_hidden);
...
void BrowserCompositorMac::SetApplicationWindowHostIsHidden(bool hidden) {
  application_window_host_is_hidden_ = hidden;
  UpdateState();
}
...
void BrowserCompositorMac::UpdateState() {
  ...
  if (!application_window_host_is_hidden_) {
