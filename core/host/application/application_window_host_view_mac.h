...
#ifndef MUMBA_HOST_APPLICATION_RENDER_WIDGET_HOST_VIEW_MAC_H_
#define MUMBA_HOST_APPLICATION_RENDER_WIDGET_HOST_VIEW_MAC_H_
...
#include "core/host/application/browser_compositor_view_mac.h"
#include "core/host/application/input/mouse_wheel_phase_handler.h"
#include "core/host/application/application_window_host_ns_view_client.h"
...
class ApplicationWindowHostNSViewBridge;
class ApplicationWindowHostViewMac;
class WebContents;
class WebCursor;
}
...
@class ApplicationWindowHostViewCocoa;

namespace host {

///////////////////////////////////////////////////////////////////////////////
// ApplicationWindowHostViewMac
//
//  An object representing the "View" of a rendered web page. This object is
//  responsible for displaying the content of the web page, and integrating with
//  the Cocoa view system. It is the implementation of the ApplicationWindowHostView
//  that the cross-platform ApplicationWindowHost object uses
//  to display the data.
//
//  Comment excerpted from application_window_host.h:
//
//    "The lifetime of the ApplicationWindowHost* is tied to the render process.
//     If the render process dies, the ApplicationWindowHost* goes away and all
//     references to it must become NULL."
//
// ApplicationWindowHostView class hierarchy described in application_window_host_view.h.
class CONTENT_EXPORT ApplicationWindowHostViewMac
    : public ApplicationWindowHostView,
      public ApplicationWindowHostNSViewClient,
 ...
 public:
  ...
  ApplicationWindowHostViewMac(ApplicationWindowHost* widget, bool is_guest_view_hack);

  ApplicationWindowHostViewCocoa* cocoa_view() const;
  ...
  // Implementation of ApplicationWindowHostView.
  ...
  ApplicationWindowHost* GetActiveWidget();

  // Returns the composition range information for the active ApplicationWindowHost
  ...
  // information for this ApplicationWindowHostViewMac (which is serving as a
  ...
  // Returns the ApplicationWindowHostDelegate corresponding to the currently focused
  // ApplicationWindowHost. It is different from |application_window_host_->delegate()|
  // when there are focused inner WebContentses on the page. Also, this method
  // can return nullptr; for instance when |application_window_host_| becomes nullptr
  // in the destruction path of the WebContentsImpl.
  ApplicationWindowHostDelegate* GetFocusedApplicationWindowHostDelegate();

  // Returns the ApplicationWindowHost to which Ime messages from the NSView
  // should be targeted. This exists to preserve historical behavior, and may
  // not be the desired behavior.
  // https://crbug.com/831843
  ApplicationWindowHost* GetWidgetForIme();

  // When inside a block of handling a keyboard event, returns the
  // ApplicationWindowHost to which all keyboard and Ime messages from the NSView
  // should be fowarded. This exists to preserve historical behavior, and may
  // not be the desired behavior.
  // https://crbug.com/831843
  ApplicationWindowHost* GetWidgetForKeyboardEvent();

 protected:
  // This class is to be deleted through the Destroy method.
  ~ApplicationWindowHostViewMac() override;

 private:
  friend class ApplicationWindowHostViewMacTest;
  FRIEND_TEST_ALL_PREFIXES(ApplicationWindowHostViewMacTest, GetPageTextForSpeech);
  ...
  // Shuts down the application_window_host_.  This is a separate function so we can
  ...
  std::unique_ptr<ApplicationWindowHostNSViewBridge> ns_view_bridge_;
  ...
  // ApplicationWindowHostViewGuest.
  ...
  // Used to ensure that a consistent ApplicationWindowHost is targeted throughout
  ...
  base::WeakPtrFactory<ApplicationWindowHostViewMac> weak_factory_;

  DISALLOW_COPY_AND_ASSIGN(ApplicationWindowHostViewMac);
};

// ApplicationWindowHostViewCocoa is not exported outside of content. This helper
// method provides the tests with the class object so that they can override the
// methods according to the tests' requirements.
CONTENT_EXPORT Class GetApplicationWindowHostViewCocoaClassForTesting();

}  // namespace host

#endif  // MUMBA_HOST_APPLICATION_RENDER_WIDGET_HOST_VIEW_MAC_H_
