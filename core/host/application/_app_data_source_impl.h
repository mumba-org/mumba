...
class CONTENT_EXPORT AppDataSourceImpl : public URLDataSourceImpl,
                                         public RpcDataSource {
 public:
  // RpcDataSource implementation:
  ...
  void SetRequestFilter(
      const RpcDataSource::HandleRequestCallback& callback) override;
  ...
  bool IsAppDataSourceImpl() const override;

 protected:
  ~AppDataSourceImpl() override;
 ...
 private:
  ...
  friend class RpcDataSource;
  friend class AppDataSourceTest;

  FRIEND_TEST_ALL_PREFIXES(AppDataSourceTest, IsGzipped);

  explicit AppDataSourceImpl(const std::string& source_name);
  ...
  RpcDataSource::HandleRequestCallback filter_callback_;
  bool add_csp_;
  ...
  DISALLOW_COPY_AND_ASSIGN(AppDataSourceImpl);
};
