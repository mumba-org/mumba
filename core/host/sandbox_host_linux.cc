...
void SandboxHostLinux::Init() {
  ...
  const int host_socket = fds[1];
  // The SandboxIPC handler is not expected to write to |host_socket|.
  // Instead, it replies on a temporary socket provided by the caller.
  PCHECK(0 == shutdown(host_socket, SHUT_WR)) << "shutdown";
  ...
  ipc_handler_.reset(new SandboxIPCHandler(child_lifeline_fd, host_socket));
  ipc_thread_.reset(
