...
//#include "core/host/ssl/ssl_error_handler.h"
//#include "core/host/ssl/ssl_manager.h"
#include "core/host/websockets/websocket_handshake_request_info_impl.h"
...
class WebSocketManager::Delegate final : public network::WebSocket::Delegate {
 public:
  ...
  void OnSSLCertificateError(
      ...
      bool fatal) override {
    DLOG(ERROR) << "WebSocketManager::Delegate::OnSSLCertificateError";
...
class WebSocketManager::Handle : public base::SupportsUserData::Data,
 ...
 public:
  ...
  ~Handle() override {
    DCHECK(!manager_) << "Should have received ApplicationProcessHostDestroyed";
  }
  ...
  void ApplicationProcessHostDestroyed(ApplicationProcessHost* host) override {
    ...
    HostThread::DeleteSoon(HostThread::IO, FROM_HERE, manager_);
    manager_ = nullptr;
  }
...
void WebSocketManager::CreateWebSocket(
  ...
  ApplicationProcessHost* host = ApplicationProcessHost::FromID(process_id);
  DCHECK(host);
  ...
  if (!handle) {
    handle = new Handle(
        new WebSocketManager(process_id, host->GetUrlRequestContextGetter()));//, host->GetStoragePartition()));
  ...
  if (base::FeatureList::IsEnabled(network::features::kNetworkService)) {
    ...
    HostNetworkContext* network_context = domain->process()->GetNetworkContext();
    HostThread::PostTask(
      HostThread::IO, FROM_HERE,
      base::BindOnce(&WebSocketManager::CreateWebSocketOnIO,
  ...
  // Maintain a WebSocketManager per ApplicationProcessHost. While the instance of
...
WebSocketManager::WebSocketManager(int process_id,
  ...
  //if (storage_partition) {
    ...
    // only via WebSocketManager::Handle::ApplicationProcessHostDestroyed which
...
void WebSocketManager::CreateWebSocketOnIO(
  HostNetworkContext* network_context,
  int process_id,
  int frame_id,
  url::Origin origin,
  network::mojom::WebSocketRequest request) {
  network_context->CreateWebSocket(std::move(request), process_id, frame_id, origin);
}
