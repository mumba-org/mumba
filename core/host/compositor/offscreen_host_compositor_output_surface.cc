...
OffscreenHostCompositorOutputSurface::
    OffscreenHostCompositorOutputSurface(
...
void OffscreenHostCompositorOutputSurface::BindToClient(
...
void OffscreenHostCompositorOutputSurface::EnsureBackbuffer() {
...
void OffscreenHostCompositorOutputSurface::DiscardBackbuffer() {
  GLES2Interface* gl = context_provider_->ContextGL();
...
void OffscreenHostCompositorOutputSurface::SetDrawRectangle(
    const gfx::Rect& draw_rectangle) {
  NOTREACHED();
}

void OffscreenHostCompositorOutputSurface::Reshape(
    const gfx::Size& size,
    float scale_factor,
...
void OffscreenHostCompositorOutputSurface::BindFramebuffer() {
...
void OffscreenHostCompositorOutputSurface::SwapBuffers(
  ...
  context_provider_->ContextSupport()->SignalSyncToken(
      sync_token,
      base::BindOnce(
          &OffscreenHostCompositorOutputSurface::OnSwapBuffersComplete,
          weak_ptr_factory_.GetWeakPtr(), frame.latency_info, ++swap_id_));
}

bool OffscreenHostCompositorOutputSurface::IsDisplayedAsOverlayPlane()
    const {
  return false;
}

unsigned OffscreenHostCompositorOutputSurface::GetOverlayTextureId() const {
  return 0;
}

gfx::BufferFormat
OffscreenHostCompositorOutputSurface::GetOverlayBufferFormat() const {
  return gfx::BufferFormat::RGBX_8888;
}

bool OffscreenHostCompositorOutputSurface::SurfaceIsSuspendForRecycle()
    const {
  return false;
}

GLenum
OffscreenHostCompositorOutputSurface::GetFramebufferCopyTextureFormat() {
  return GLCopyTextureInternalFormat(kFboTextureFormat);
}

void OffscreenHostCompositorOutputSurface::OnReflectorChanged() {
  if (reflector_) {
...
void OffscreenHostCompositorOutputSurface::OnSwapBuffersComplete(
...
gpu::VulkanSurface*
OffscreenHostCompositorOutputSurface::GetVulkanSurface() {
  NOTIMPLEMENTED();
  return nullptr;
}
#endif
