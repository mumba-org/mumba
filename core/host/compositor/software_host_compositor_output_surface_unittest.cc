...
class SoftwareHostCompositorOutputSurfaceTest : public testing::Test {
 public:
  SoftwareHostCompositorOutputSurfaceTest()
      : begin_frame_source_(std::make_unique<viz::DelayBasedTimeSource>(
                                message_loop_.task_runner().get()),
                            viz::BeginFrameSource::kNotRestartableId) {}
  ~SoftwareHostCompositorOutputSurfaceTest() override = default;
  ...
  std::unique_ptr<content::HostCompositorOutputSurface> CreateSurface(
      std::unique_ptr<viz::SoftwareOutputDevice> device);

 protected:
  std::unique_ptr<content::HostCompositorOutputSurface> output_surface_;
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(SoftwareHostCompositorOutputSurfaceTest);
};

void SoftwareHostCompositorOutputSurfaceTest::SetUp() {
...
void SoftwareHostCompositorOutputSurfaceTest::TearDown() {
  output_surface_.reset();
  compositor_.reset();
  ui::TerminateContextFactoryForTests();
}

std::unique_ptr<content::HostCompositorOutputSurface>
SoftwareHostCompositorOutputSurfaceTest::CreateSurface(
    std::unique_ptr<viz::SoftwareOutputDevice> device) {
  return std::make_unique<content::SoftwareHostCompositorOutputSurface>(
      std::move(device),
      base::Bind(
          &SoftwareHostCompositorOutputSurfaceTest::UpdateVSyncParameters,
          base::Unretained(this)),
      base::ThreadTaskRunnerHandle::Get());
}

void SoftwareHostCompositorOutputSurfaceTest::UpdateVSyncParameters(
    base::TimeTicks timebase,
    base::TimeDelta interval) {
  update_vsync_parameters_call_count_++;
}

TEST_F(SoftwareHostCompositorOutputSurfaceTest, NoVSyncProvider) {
...
TEST_F(SoftwareHostCompositorOutputSurfaceTest, VSyncProviderUpdates) {
  cc::FakeOutputSurfaceClient output_surface_client;
  std::unique_ptr<viz::SoftwareOutputDevice> software_device(
