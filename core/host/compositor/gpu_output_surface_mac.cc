...
void GpuOutputSurfaceMac::SwapBuffers(viz::OutputSurfaceFrame frame) {
  GpuSurfacelessHostCompositorOutputSurface::SwapBuffers(std::move(frame));
...
void GpuOutputSurfaceMac::SetSurfaceSuspendedForRecycle(bool suspended) {
  if (suspended) {
    ...
    // the host. Make sure that these frames are not displayed by ignoring
    // them in GpuProcessHostUIShim, until the host issues a SwapBuffers for
    // the new content.
    ...
    // host will not flash stale content when adding the remote CALayer to
