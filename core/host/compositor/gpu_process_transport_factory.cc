...
#include "core/host/compositor/gpu_surfaceless_host_compositor_output_surface.h"
#include "core/host/compositor/in_process_display_client.h"
#include "core/host/compositor/offscreen_host_compositor_output_surface.h"
...
#include "core/host/compositor/gpu_output_surface_mac.h"
...
struct GpuProcessTransportFactory::PerCompositorData {
  ...
  HostCompositorOutputSurface* display_output_surface = nullptr;
...
GpuProcessTransportFactory::GpuProcessTransportFactory(
  ...
  cc::SetClientNameForMetrics("Host");
  ...
  if (command_line->HasSwitch(switches::kDisableGpuVsync)) {
    ...
    // See comments in gl_switches about this flag.  The host compositor
...
void GpuProcessTransportFactory::EstablishedGpuChannel(
  ...
  if (gpu_channel_host &&
    ...
    DLOG(INFO) << "GPU_FEATURE_TYPE_GPU_COMPOSITING != enabled => software compositing";
    use_gpu_compositing = false;
  }
  // Gpu compositing may have been disabled in the meantime.
  if (is_gpu_compositing_disabled_) {
    DLOG(INFO) << "is_gpu_compositing_disabled_ = true => software compositing";
    use_gpu_compositing = false;
  }
  ...
  if (!use_gpu_compositing || vulkan_context_provider) {
    ...
    // use gpu compositing.
    DLOG(INFO) << "gpu_channel_host is null => software compositing";
    use_gpu_compositing = false;
    shared_worker_context_provider_ = nullptr;
  } else {
    ...
    if (!shared_worker_context_provider_) {
      ...
      if (result != gpu::ContextResult::kSuccess) {
        ...
        if (result == gpu::ContextResult::kFatalFailure) {
          DLOG(INFO) << "gpu::ContextResult::kFatalFailure => software compositing";
          use_gpu_compositing = false;
        }
      }
    }

    // The |context_provider| is used for both the host compositor and the
  ...
  UMA_HISTOGRAM_BOOLEAN("Aura.CreatedGpuHostCompositor",
  ...
  HostCompositorOutputSurface::UpdateVSyncParametersCallback vsync_callback =
      base::Bind(&ui::Compositor::SetDisplayVSyncParameters, compositor);
  GpuVSyncControl* gpu_vsync_control = nullptr;

  std::unique_ptr<HostCompositorOutputSurface> display_output_surface;
#if BUILDFLAG(ENABLE_VULKAN)
  std::unique_ptr<VulkanHostCompositorOutputSurface> vulkan_surface;
  if (vulkan_context_provider) {
    vulkan_surface.reset(new VulkanHostCompositorOutputSurface(
  ...
  if (!display_output_surface) {
    if (!use_gpu_compositing) {
      if (!is_gpu_compositing_disabled_ &&
        ...
        DLOG(INFO) << "DisableGpuCompositing()";
        DisableGpuCompositing(compositor.get());
      }
      display_output_surface =
          std::make_unique<SoftwareHostCompositorOutputSurface>(
      ...
      if (data->surface_handle == gpu::kNullSurfaceHandle) {
        display_output_surface =
            std::make_unique<OffscreenHostCompositorOutputSurface>(
        ...
        auto gpu_output_surface =
            std::make_unique<GpuSurfacelessHostCompositorOutputSurface>(
...
#endif
        auto gpu_output_surface =
            std::make_unique<GpuHostCompositorOutputSurface>(
                context_provider, std::move(vsync_callback),
                std::move(validator));
        gpu_vsync_control = gpu_output_surface.get();
        display_output_surface = std::move(gpu_output_surface);
...
void GpuProcessTransportFactory::DisableGpuCompositing(
  ...
  DLOG(INFO) << "GpuProcessTransportFactory::DisableGpuCompositing";
...
GpuProcessTransportFactory::GetHostFrameSinkManager() {
  return HostMainLoop::GetInstance()->host_frame_sink_manager();
}
...
viz::FrameSinkManagerImpl* GpuProcessTransportFactory::GetFrameSinkManager() {
  return HostMainLoop::GetInstance()->GetFrameSinkManager();
}
...
GpuProcessTransportFactory::SharedMainThreadContextProvider() {
  ...
  if (!gpu_channel_host ||
      gpu_channel_host->gpu_feature_info()
              .status_values[gpu::GPU_FEATURE_TYPE_GPU_COMPOSITING] !=
          gpu::kGpuFeatureStatusEnabled) {
    DLOG(INFO) << "DisableGpuCompositing(): gpu_channel_host ? " << gpu_channel_host << " GPU_FEATURE_TYPE_GPU_COMPOSITING ? " << (gpu_channel_host->gpu_feature_info()
              .status_values[gpu::GPU_FEATURE_TYPE_GPU_COMPOSITING] ==
          gpu::kGpuFeatureStatusEnabled);
    DisableGpuCompositing(nullptr);
    if (gpu_channel_host)
...
GpuProcessTransportFactory::CreateContextCommon(
  ...
  // All host contexts get the same stream id and priority.
  int32_t stream_id = common::kGpuStreamIdDefault;
  gpu::SchedulingPriority stream_priority = common::kGpuStreamPriorityUI;

  // This is called from a few places to create different contexts:
  // - The shared main thread context (offscreen).
  // - The compositor context, which is used by the host compositor
