...
GpuSurfacelessHostCompositorOutputSurface::
    GpuSurfacelessHostCompositorOutputSurface(
    ...
    : GpuHostCompositorOutputSurface(std::move(context),
  ...
  // presenting the back buffer, enabling the host compositor to run ahead.
...
bool GpuSurfacelessHostCompositorOutputSurface::IsDisplayedAsOverlayPlane()
    const {
  return true;
}

unsigned GpuSurfacelessHostCompositorOutputSurface::GetOverlayTextureId()
    const {
  return buffer_queue_->GetCurrentTextureId();
}

gfx::BufferFormat
GpuSurfacelessHostCompositorOutputSurface::GetOverlayBufferFormat() const {
  DCHECK(buffer_queue_);
  return buffer_queue_->buffer_format();
}

void GpuSurfacelessHostCompositorOutputSurface::SwapBuffers(
  ...
  GpuHostCompositorOutputSurface::SwapBuffers(std::move(frame));
}

void GpuSurfacelessHostCompositorOutputSurface::BindFramebuffer() {
  DCHECK(buffer_queue_);
  buffer_queue_->BindFramebuffer();
}

GLenum GpuSurfacelessHostCompositorOutputSurface::
    GetFramebufferCopyTextureFormat() {
  return buffer_queue_->internal_format();
}

void GpuSurfacelessHostCompositorOutputSurface::Reshape(
    const gfx::Size& size,
    ...
    bool use_stencil) {
  reshape_size_ = size;
  GpuHostCompositorOutputSurface::Reshape(
...
void GpuSurfacelessHostCompositorOutputSurface::OnGpuSwapBuffersCompleted(
  ...
  GpuHostCompositorOutputSurface::OnGpuSwapBuffersCompleted(modified_params);
  if (force_swap)
