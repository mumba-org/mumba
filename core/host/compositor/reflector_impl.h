...
class OwnedMailbox;
class HostCompositorOutputSurface;
...
class CONTENT_EXPORT ReflectorImpl : public ui::Reflector {
 public:
  ...
  // Called in |HostCompositorOutputSurface::SwapBuffers| to copy
  // the full screen image to the |mailbox_| texture.
  void OnSourceSwapBuffers(const gfx::Size& surface_size);

  // Called in |HostCompositorOutputSurface::PostSubBuffer| copy
  ...
  void OnSourceSurfaceReady(HostCompositorOutputSurface* surface);
 ...
 private:
  ...
  bool flip_texture_;
  HostCompositorOutputSurface* output_surface_;
