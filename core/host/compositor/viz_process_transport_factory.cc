...
// The client id for the host process. It must not conflict with any
// child process client id.
constexpr uint32_t kHostClientId = 0u;
...
scoped_refptr<ui::ContextProviderCommandBuffer> CreateContextProviderImpl(
  ...
  return base::MakeRefCounted<ui::ContextProviderCommandBuffer>(
      ...
      common::kGpuStreamIdDefault, common::kGpuStreamPriorityUI, gpu::kNullSurfaceHandle,
...
VizProcessTransportFactory::VizProcessTransportFactory(
    ...
    : gpu_channel_establish_factory_(gpu_channel_establish_factory),
      ...
      frame_sink_id_allocator_(kHostClientId),
...
viz::FrameSinkManagerImpl* VizProcessTransportFactory::GetFrameSinkManager() {
  // When running with viz there is no FrameSinkManagerImpl in the host
  // process. FrameSinkManagerImpl runs in the GPU process instead. Anything in
  // the host process that relies FrameSinkManagerImpl or SurfaceManager
...
void VizProcessTransportFactory::OnEstablishedGpuChannel(
    ...
    scoped_refptr<gpu::GpuChannelHost> gpu_channel_host) {
  DLOG(INFO) << "VizProcessTransportFactory::OnEstablishedGpuChannel";
  ui::Compositor* compositor = compositor_weak_ptr.get();
  if (!compositor) {
    DLOG(INFO) << "fatal: no compositor set. cancelling the whole operatioon";
    return;
  }
...
//#endif
  ...
  // host compositor may request a new CompositorFrameSink on context loss,
  // which will destroy the existing CompositorFrameSink.
  GetHostFrameSinkManager()->CreateRootCompositorFrameSink(
      std::move(root_params));

  // Create LayerTreeFrameSink with the host end of CompositorFrameSink.
