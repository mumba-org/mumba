...
class HostMessageFilter::Internal : public IPC::MessageFilter {
 public:
  explicit Internal(HostMessageFilter* filter) : filter_(filter) {}
 ...
 private:
  ...
  bool OnMessageReceived(const IPC::Message& message) override {
    HostThread::ID thread = HostThread::IO;
    filter_->OverrideThreadForMessage(message, &thread);

    if (thread == HostThread::IO) {
  ...
  bool DispatchMessage(const IPC::Message& message) {
    ...
    DCHECK(HostThread::CurrentlyOn(HostThread::IO) || rv) <<
        "Must handle messages that were dispatched to another thread!";
    return rv;
  }

  scoped_refptr<HostMessageFilter> filter_;

  DISALLOW_COPY_AND_ASSIGN(Internal);
};

HostMessageFilter::HostMessageFilter(uint32_t message_class_to_filter)
    : internal_(nullptr),
      sender_(nullptr),
      message_classes_to_filter_(1, message_class_to_filter) {}

HostMessageFilter::HostMessageFilter(
...
void HostMessageFilter::AddAssociatedInterface(
...
base::ProcessHandle HostMessageFilter::PeerHandle() {
  return peer_process_.Handle();
}

void HostMessageFilter::OnDestruct() const {
  delete this;
}

bool HostMessageFilter::Send(IPC::Message* message) {
  if (message->is_sync()) {
    // We don't support sending synchronous messages from the host.  If we
    ...
    // host, since it might allow a corrupt/malicious renderer to hang us.
    NOTREACHED() << "Can't send sync message through HostMessageFilter!";
    return false;
  }
  ...
  if (!HostThread::CurrentlyOn(HostThread::IO)) {
    HostThread::PostTask(
        ...
        base::BindOnce(base::IgnoreResult(&HostMessageFilter::Send), this,
                       message));
    return true;
  }
...
base::TaskRunner* HostMessageFilter::OverrideTaskRunnerForMessage(
    const IPC::Message& message) {
  return nullptr;
}

void HostMessageFilter::ShutdownForBadMessage() {
  ...
  ChildProcessLauncher::TerminateProcess(
      peer_process_, common::RESULT_CODE_KILLED_BAD_MESSAGE);
 ...
 // HostChildProcessHostImpl::HistogramBadMessageTerminated(
 //     common::PROCESS_TYPE_RENDERER);
}

HostMessageFilter::~HostMessageFilter() {
}

IPC::MessageFilter* HostMessageFilter::GetFilter() {
...
void HostMessageFilter::RegisterAssociatedInterfaces(
