...
namespace {
class PamAuthorizer : public Authenticator {
 public:
  PamAuthorizer(std::unique_ptr<Authenticator> underlying);
...
PamAuthorizer::PamAuthorizer(
    std::unique_ptr<Authenticator> underlying)
    : underlying_(std::move(underlying)), local_login_status_(NOT_CHECKED) {}

PamAuthorizer::~PamAuthorizer() {}

Authenticator::State PamAuthorizer::state() const {
