...
#include "core/host/net/pseudotcp_channel_factory.h"
...
void PseudoTcpChannelFactory::OnDatagramChannelCreated(
    const std::string& name,
    const ChannelCreatedCallback& callback,
    std::unique_ptr<protocol::PeerDatagramSocket> datagram_socket) {
...
void PseudoTcpChannelFactory::OnPseudoTcpConnected(
  ...
  std::unique_ptr<protocol::PeerStreamSocket> socket(it->second);
  pending_sockets_.erase(it);
