...
#ifndef MUMBA_HOST_NET_PAIRING_REGISTRY_H_
#define MUMBA_HOST_NET_PAIRING_REGISTRY_H_
...
class PairingRegistry : public base::RefCountedThreadSafe<PairingRegistry> {
 ...
 private:
  ...
  void DoLoadAll(
      const PairingRegistry::GetAllPairingsCallback& callback);
  void DoDeleteAll(
      const PairingRegistry::DoneCallback& callback);
  void DoLoad(
      const std::string& client_id,
