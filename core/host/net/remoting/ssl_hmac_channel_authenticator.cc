...
#include "core/host/net/ssl_hmac_channel_authenticator.h"
...
#include "core/host/net/auth_util.h"
...
// Implements net::StreamSocket interface on top of PeerStreamSocket to be passed
// to net::SSLClientSocket and net::SSLServerSocket.
class NetStreamSocketAdapter : public net::StreamSocket {
 public:
  NetStreamSocketAdapter(std::unique_ptr<protocol::PeerStreamSocket> socket)
...
// Implements PeerStreamSocket interface on top of net::StreamSocket.
class PeerStreamSocketAdapter : public protocol::PeerStreamSocket {
 public:
  PeerStreamSocketAdapter(std::unique_ptr<net::StreamSocket> socket,
                         std::unique_ptr<net::SSLServerContext> server_context)
      : server_context_(std::move(server_context)),
        socket_(std::move(socket)) {}
  ~PeerStreamSocketAdapter() override = default;
...
void SslHmacChannelAuthenticator::CheckDone(bool* callback_called) {
  if (auth_write_buf_.get() == nullptr && auth_read_buf_.get() == nullptr) {
    ...
    std::move(done_callback_)
        .Run(net::OK, std::make_unique<PeerStreamSocketAdapter>(
