...
bool ParsePinHashFromConfig(const std::string& value,
  ...
  if (function_name == "plain") {
    *pin_hash_out = GetSharedSecretHash(host_id, *pin_hash_out);
    return true;
...
std::string MakeHostPinHash(const std::string& host_id,
  ...
  std::string hash = GetSharedSecretHash(host_id, pin);
...
bool VerifyHostPinHash(const std::string& hash,
  ...
  std::string hash_calculated = GetSharedSecretHash(host_id, pin);
