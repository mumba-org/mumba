...
#include "core/host/net/channel_multiplexer.h"
...
class PendingPacket {
 public:
  PendingPacket(std::unique_ptr<protocol::MultiplexPacket> packet)
 ...
 private:
  std::unique_ptr<protocol::MultiplexPacket> packet;
  size_t pos = 0U;
...
class ChannelMultiplexer::MuxChannel {
 public:
  ...
  std::unique_ptr<protocol::PeerStreamSocket> CreateSocket();
  void OnIncomingPacket(std::unique_ptr<protocol::MultiplexPacket> packet);
...
class ChannelMultiplexer::MuxSocket : public protocol::PeerStreamSocket {
 public:
...
std::unique_ptr<protocol::PeerStreamSocket>
...
int ChannelMultiplexer::MuxSocket::Write(
  ...
  std::unique_ptr<protocol::MultiplexPacket> packet(new protocol::MultiplexPacket());
  ...
  if (base_channel_.get()) {
    ...
    reader_.StartReading(base_channel_.get(),
                         ...
                         base::Bind(&ChannelMultiplexer::OnBaseChannelError,
                                    base::Unretained(this)));
    writer_.Start(base::Bind(&protocol::PeerStreamSocket::Write,
...
void ChannelMultiplexer::OnIncomingPacket(
  ...
  std::unique_ptr<protocol::MultiplexPacket> packet =
      protocol::ParseMessage<protocol::MultiplexPacket>(buffer.get());
  if (!packet)
    return;
