...
//#include "core/host/net/codec/audio_encoder.h"
//#include "core/host/net/codec/audio_encoder_opus.h"
//#include "core/host/net/codec/video_encoder.h"
//#include "core/host/net/audio_pump.h"
//#include "core/host/net/audio_source.h"
//#include "core/host/net/audio_writer.h"
...
//#include "core/host/net/host_video_dispatcher.h"
//#include "core/host/net/input_stub.h"
#include "core/host/net/transport_context.h"
//#include "core/host/net/video_frame_pump.h"
...
IcePeerConnection::IcePeerConnection(
...
void IcePeerConnection::SetEventHandler(
    PeerConnection::EventHandler* event_handler) {
  DCHECK(thread_checker_.CalledOnValidThread());
  event_handler_ = event_handler;
}

Session* IcePeerConnection::session() const {
  DCHECK(thread_checker_.CalledOnValidThread());
  return session_.get();
}

void IcePeerConnection::Disconnect(ErrorCode error) {
  DCHECK(thread_checker_.CalledOnValidThread());
...
// std::unique_ptr<VideoStream> IcePeerConnection::StartVideoStream(
...
// std::unique_ptr<AudioStream> IcePeerConnection::StartAudioStream(
...
ClientStub* IcePeerConnection::client_stub() {
  DCHECK(thread_checker_.CalledOnValidThread());
  return control_dispatcher_.get();
}

// void IcePeerConnection::set_clipboard_stub(
//     protocol::ClipboardStub* clipboard_stub) {
//   DCHECK(thread_checker_.CalledOnValidThread());
//   control_dispatcher_->set_clipboard_stub(clipboard_stub);
// }

void IcePeerConnection::set_host_stub(HostStub* host_stub) {
  DCHECK(thread_checker_.CalledOnValidThread());
  control_dispatcher_->set_host_stub(host_stub);
}

// void IcePeerConnection::set_input_stub(protocol::InputStub* input_stub) {
//   DCHECK(thread_checker_.CalledOnValidThread());
//   event_dispatcher_->set_input_stub(input_stub);
// }

void IcePeerConnection::OnSessionStateChange(Session::State state) {
  DCHECK(thread_checker_.CalledOnValidThread());
...
void IcePeerConnection::OnIceTransportRouteChange(
    const std::string& channel_name,
    const TransportRoute& route) {
  event_handler_->OnChannelChange(channel_name, route);
}

void IcePeerConnection::OnIceTransportError(ErrorCode error) {
  DCHECK(thread_checker_.CalledOnValidThread());
  Disconnect(error);
}

void IcePeerConnection::OnChannelInitialized(
...
void IcePeerConnection::OnChannelClosed(
    ChannelDispatcherBase* channel_dispatcher) {
  DCHECK(thread_checker_.CalledOnValidThread());
  Disconnect(OK);
}

void IcePeerConnection::NotifyIfChannelsReady() {
  DCHECK(thread_checker_.CalledOnValidThread());
...
void IcePeerConnection::CloseChannels() {
