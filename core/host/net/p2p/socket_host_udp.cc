...
#include "core/host/net/p2p/socket_host_udp.h"
...
P2PSocketHostUdp::P2PSocketHostUdp(
    ...
    : P2PSocketHost(delegate, std::move(message_sender), socket_id, P2PSocketHost::UDP),
...
P2PSocketHostUdp::P2PSocketHostUdp(Delegate* delegate,
                                   base::WeakPtr<IPC::Sender> message_sender,
                                   int socket_id,
                                   P2PMessageThrottler* throttler,
                                   net::NetLog* net_log)
    : P2PSocketHostUdp(delegate, 
...
bool P2PSocketHostUdp::Init(const common::P2PSocketOptions& options) {
  DCHECK_EQ(state_, STATE_UNINITIALIZED);
  DCHECK((options.port_range.min_port == 0 && options.port_range.max_port == 0) || options.port_range.min_port > 0);
  DCHECK_LE(options.port_range.min_port, options.port_range.max_port);

  int result = -1;
  if (options.port_range.min_port == 0) {
    result = socket_->Listen(options.local_address);
  } else if (options.local_address.port() == 0) {
    for (unsigned port = options.port_range.min_port; port <= options.port_range.max_port && result < 0; ++port) {
      result = socket_->Listen(net::IPEndPoint(options.local_address.address(), port));
      if (result < 0 && port != options.port_range.max_port)
        socket_ = socket_factory_.Run(net_log_);
    }
  } else if (options.local_address.port() >= options.port_range.min_port &&
             options.local_address.port() <= options.port_range.max_port) {
    result = socket_->Listen(options.local_address);
  }
  if (result < 0) {
    LOG(ERROR) << "bind() to " << options.local_address.address().ToString()
               << (options.port_range.min_port == 0
                       ? base::StringPrintf(":%d", options.local_address.port())
                       : base::StringPrintf(", port range [%d-%d]", options.port_range.min_port,
                                            options.port_range.max_port))
               << " failed: " << result;
    OnError();
    return false;
  }
  ...
  message_sender_->Send(new P2PMsg_OnSocketCreated(
      id_, address, options.remote_address.ip_address));
  ...
  if (!base::ContainsKey(connected_peers_, packet.to)) {
    ...
    if (throttler_->DropNextPacket(packet.size)) {
      ...
      message_sender_->Send(new P2PMsg_OnSendComplete(
          id_, common::P2PSendPacketMetrics(packet.id, packet.packet_options.packet_id,
                                    send_time)));
...
void P2PSocketHostUdp::HandleSendResult(uint64_t packet_id,
  ...
  message_sender_->Send(new P2PMsg_OnSendComplete(
      id_,
      common::P2PSendPacketMetrics(packet_id, transport_sequence_number, send_time)));
}
...
bool P2PSocketHostUdp::SetOption(common::P2PSocketOption option, int value) {
  if (state_ != STATE_OPEN) {
...
void P2PSocketHostUdp::ReceiveRpcMessage(int call_id, int method_type) {}
void P2PSocketHostUdp::SendRpcMessage(int call_id, int method_type, std::vector<char> data) {}
void P2PSocketHostUdp::SendRpcMessageNow(int call_id, int method_type, std::vector<char> data) {}
void P2PSocketHostUdp::SendRpcStatus(int call_id, int status_code) {}
