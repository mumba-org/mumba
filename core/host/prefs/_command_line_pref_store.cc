...
#include "core/host/prefs/command_line_pref_store.h"
...
#include "core/host/prefs/proxy_config_dictionary.h"
...
namespace {
 ...
 //      { switches::kStorageCacheDir, prefs::kStorageCacheDir },
 //};
 ...
 //      //{ switches::kStorageCacheSize, prefs::kStorageCacheSize },
 ...
 void CommandLinePrefStore::ApplySimpleSwitches() {
  ...
  for (size_t i = 0; i < arraysize(string_switch_map_); ++i) {
   if (command_line_->HasSwitch(string_switch_map_[i].switch_name)) {
    auto val = std::unique_ptr<base::StringValue>(new base::StringValue(command_line_->GetSwitchValueASCII(
  ...
  for (size_t i = 0; i < arraysize(boolean_switch_map_); ++i) {
   if (command_line_->HasSwitch(boolean_switch_map_[i].switch_name)) {
    auto val = std::unique_ptr<base::FundamentalValue>(new base::FundamentalValue(boolean_switch_map_[i].set_value));
