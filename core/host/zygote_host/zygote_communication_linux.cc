...
bool ZygoteCommunication::SendMessage(const base::Pickle& data,
  ...
  CHECK(data.size() <= common::kZygoteMaxMessageLength)
      << "Trying to send too-large message to zygote (sending " << data.size()
      << " bytes, max is " << common::kZygoteMaxMessageLength << ")";
...
pid_t ZygoteCommunication::ForkRequest(
  ...
  pickle.WriteInt(common::kZygoteCommandFork);
  ...
  pid_t pid;
  {
    ...
    {
      char buf[sizeof(common::kZygoteChildPingMessage) + 1];
      ...
      if (n != sizeof(common::kZygoteChildPingMessage) ||
          0 != memcmp(buf, common::kZygoteChildPingMessage,
                      sizeof(common::kZygoteChildPingMessage))) {
      ...
      pid_pickle.WriteInt(common::kZygoteCommandForkRealPID);
...
void ZygoteCommunication::EnsureProcessTerminated(pid_t process) {
  ...
  pickle.WriteInt(common::kZygoteCommandReap);
...
void ZygoteCommunication::Init(
  ...
  cmd_line.AppendSwitch(switches::kZygoteProcess);


  const base::CommandLine& host_command_line =
      *base::CommandLine::ForCurrentProcess();
  if (host_command_line.HasSwitch(switches::kZygoteCmdPrefix)) {
    cmd_line.PrependWrapper(
        host_command_line.GetSwitchValueNative(switches::kZygoteCmdPrefix));
  }
  ...
  base::Pickle pickle;
  pickle.WriteInt(common::kZygoteCommandGetSandboxStatus);
  if (!SendMessage(pickle, nullptr))
...
base::TerminationStatus ZygoteCommunication::GetTerminationStatus(
  ...
  DCHECK(init_);
  base::Pickle pickle;
  pickle.WriteInt(common::kZygoteCommandGetTerminationStatus);
