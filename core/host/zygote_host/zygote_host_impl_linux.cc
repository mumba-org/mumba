...
void ZygoteHostImpl::Init(const base::CommandLine& command_line) {
  ...
  // When running as root with the sandbox enabled, the host process
...
pid_t ZygoteHostImpl::LaunchZygote(
  ...
  options.fds_to_remap.emplace_back(fds[1], common::kZygoteSocketPairFd);
  ...
  if (use_namespace_sandbox_ || use_suid_sandbox_) {
    ...
    CHECK(ReceiveFixedMessage(fds[0], common::kZygoteBootMessage,
                              sizeof(common::kZygoteBootMessage), &boot_pid));
    ...
    pid_t real_pid;
    CHECK(ReceiveFixedMessage(fds[0], common::kZygoteHelloMessage,
                              sizeof(common::kZygoteHelloMessage), &real_pid));
    CHECK_GT(real_pid, 1);
