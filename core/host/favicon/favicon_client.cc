...
FaviconClient::FaviconClient(scoped_refptr<Workspace> workspace) {//: 
...
bool FaviconClient::IsNativeApplicationURL(const GURL& url) {
  return false;//url.SchemeIs(content::kChromeUIScheme) ||
         //url.SchemeIs(extensions::kExtensionScheme);
}

base::CancelableTaskTracker::TaskId
FaviconClient::GetFaviconForNativeApplicationURL(
    const GURL& url,
