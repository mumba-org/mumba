...
// send a message to the first chrome host process with the current
...
// chrome's host process, eg. "SingletonLock -> example.com-9156".  When the
...
//#include "mumba/grit/content_strings.h"
...
//#if defined(OS_LINUX)
//#include "chrome/host/ui/process_singleton_dialog_linux.h"
//#endif
...
// Timeout for the current host process to respond. 20 seconds should be
// enough.
const int kTimeoutInSeconds = 20;
// Number of retries to notify the host. 20 retries over 20 seconds = 1 try
// per second.
...
bool SymlinkPath(const base::FilePath& target, const base::FilePath& path) {
  //DLOG(INFO) << "SymlinkPath " << target << " -> " << path;
  if (!base::CreateSymbolicLink(target, path)) {
    DLOG(ERROR) << "Process singleton: failed to symlink " << target << " -> " << path;
...
bool IsChromeProcess(pid_t pid) {
  ...
  return (!other_chrome_path.empty() &&
          other_chrome_path.BaseName() ==
              base::FilePath(constants::kHostProcessExecutableName));
}
...
class ProcessSingleton::LinuxWatcher
 ...
 public:
  ...
  // opens a new host tab.  This runs on the UI thread.
...
void ProcessSingleton::LinuxWatcher::HandleMessage(
  ...
  if (parent_->notification_callback_.Run(base::CommandLine(argv),
    ...
    LOG(WARNING) << "Not handling interprocess notification as host"
                    " is shutting down";
...
void ProcessSingleton::LinuxWatcher::SocketReader::
  ...
  // Return to the UI thread to handle opening a new host tab.
...
ProcessSingleton::NotifyResult ProcessSingleton::NotifyOtherProcessWithTimeout(
  ...
  for (int retries = 0; retries <= retry_attempts; ++retries) {
    ...
    // chrome host.  If so, we loop and try again for |timeout|.

    std::string hostname;
    if (!ParseLockPath(lock_path_, &hostname, &pid)) {
      //DLOG(INFO) << "ParseLockPath() = false. lock_path_ = " << lock_path_ << " -> PROCESS_NONE";
      // No lockfile exists.
      return PROCESS_NONE;
    }
    ...
    if (hostname.empty()) {
      // Invalid lockfile.
      //DLOG(INFO) << "hostname.empty(). INVALID_LOCK_FILE lock_path_ = " << lock_path_ << " -> PROCESS_NONE";
    ...
    if (hostname != net::GetHostName() && !IsChromeProcess(pid)) {
      //DLOG(INFO) << "hostname != net::GetHostName() && !IsChromeProcess(pid) lock_path_ = " << lock_path_ << " -> PROCESS_NONE";
    ...
    if (!IsChromeProcess(pid)) {
      //(INFO) << "!IsChromeProcess(pid) lock_path_ = " << lock_path_ << " -> PROCESS_NONE";
    ...
    if (IsSameChromeInstance(pid)) {
      //DLOG(INFO) << "IsSameChromeInstance(pid) lock_path_ = " << lock_path_ << " -> PROCESS_NONE";
    ...
    if (retries == retry_attempts) {
      ...
      //DLOG(INFO) << "retries == retry_attempts lock_path_ = " << lock_path_ << " -> PROCESS_NONE";
      return PROCESS_NONE;
    }
...
ProcessSingleton::NotifyOtherProcessWithTimeoutOrCreate(
  ...
  if (Create()) {
    //DLOG(INFO) << "Create() = true -> PROCESS_NONE";
...
bool ProcessSingleton::Create() {
  ...
  if (!SymlinkPath(symlink_content, lock_path_)) {
    DLOG(ERROR) << "Process singleton: failed to symlink " << lock_path_;
  ...
  base::FilePath socket_target_path =
      socket_dir_.GetPath().Append(constants::kSingletonSocketFilename);
  SetupSocket(socket_target_path.value(), &sock, &addr);

  // Setup the socket symlink and the two cookies.
  base::FilePath cookie(GenerateCookie());
  base::FilePath remote_cookie_path =
      socket_dir_.GetPath().Append(constants::kSingletonCookieFilename);
