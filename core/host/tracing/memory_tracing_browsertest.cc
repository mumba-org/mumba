...
class MemoryTracingTest : public ContentHostTest {
 public:
 ...
 protected:
  ...
  void Navigate(Host* host) {
    NavigateToURL(host, GetTestUrl("", "title1.html"));
  }
...
#define MAYBE_HostInitiatedSingleDump DISABLED_HostInitiatedSingleDump
#else
#define MAYBE_HostInitiatedSingleDump HostInitiatedSingleDump
#endif  // defined(OS_ANDROID) && defined(ADDRESS_SANITIZER)

// Checks that a memory dump initiated from a the main host thread ends up in
// a single dump even in single process mode.
IN_PROC_BROWSER_TEST_F(SingleProcessMemoryTracingTest,
                       MAYBE_HostInitiatedSingleDump) {
  Navigate(host());
...
#define MAYBE_HostInitiatedDump DISABLED_HostInitiatedDump
#else
#define MAYBE_HostInitiatedDump HostInitiatedDump
#endif
// Checks that a memory dump initiated from a the main host thread ends up in
// a successful dump.
IN_PROC_BROWSER_TEST_F(MemoryTracingTest, MAYBE_HostInitiatedDump) {
  Navigate(host());
