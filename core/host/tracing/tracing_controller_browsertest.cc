...
class TracingTestHostClient : public TestContentHostClient {
 public:
...
class TracingControllerTest : public ContentHostTest {
 public:
  ...
  void TearDown() override { ContentHostTest::TearDown(); }

  void Navigate(Host* host) {
    NavigateToURL(host, GetTestUrl("", "title.html"));
  }
  ...
  void TestStartAndStopTracingStringWithFilter() {
    TracingTestHostClient client;
    ContentHostClient* old_client = SetHostClientForTesting(&client);
    Navigate(host());
    ...
    SetHostClientForTesting(old_client);
  }
