...
#include "core/host/tracing/grit/tracing_resources.h"
...
TracingUI::TracingUI(WebUI* web_ui)
    : WebUIController(web_ui),
      delegate_(GetContentClient()->host()->GetTracingDelegate()),
      weak_factory_(this) {
  ...
  WebUIDataSource::Add(host_context, source);
...
void TracingUI::DoUploadInternal(const std::string& file_contents,
  ...
  trace_uploader_ = delegate_->GetTraceUploader(
      HostContext::GetDefaultStoragePartition(
          web_ui()->GetWebContents()->GetHostContext())->
