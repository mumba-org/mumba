...
void EtwTracingAgent::StartTracing(const std::string& config,
  ...
  // Tracing agents, e.g. this, live as long as HostMainLoop lives and so
...
void EtwTracingAgent::StopAndFlush(tracing::mojom::RecorderPtr recorder) {
  ...
  // HostMainLoop lives and so using base::Unretained here is safe.
  thread_.task_runner()->PostTask(
      FROM_HERE,
...
void EtwTracingAgent::ProcessEvent(EVENT_TRACE* event) {
  ...
  // Ignore events that are received after the host is closed.
  if (instance)
