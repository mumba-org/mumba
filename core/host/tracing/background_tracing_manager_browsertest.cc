...
#include "core/host/tracing/background_tracing_rule.h"
...
class BackgroundTracingManagerHostTest : public ContentHostTest {
 public:
  BackgroundTracingManagerHostTest() {}

 private:
  DISALLOW_COPY_AND_ASSIGN(BackgroundTracingManagerHostTest);
};
...
class BackgroundTracingManagerUploadConfigWrapper {
 public:
  ...
  void Upload(const scoped_refptr<base::RefCountedString>& file_contents,
    ...
    HostThread::PostTask(HostThread::UI, FROM_HERE, callback_);
  }
...
IN_PROC_BROWSER_TEST_F(BackgroundTracingManagerHostTest,
...
// This tests that host metadata gets included in the trace.
IN_PROC_BROWSER_TEST_F(BackgroundTracingManagerHostTest,
...
// provide a argument-filtered trace and has no predicate in portal to do the
// filtering (in this case, only the host process gets it set), will crash
...
IN_PROC_BROWSER_TEST_F(BackgroundTracingManagerHostTest,
  ...
  NavigateToURL(host(), GetTestUrl("", "about:blank"));
