...
#include "chrome/host/process_singleton.h"
...
#include "chrome/host/host_integration.h"
#include "chrome/host/ui/simple_message_box.h"
#include "chrome/host/win/chrome_process_finder.h"
...
BOOL CALLBACK HostWindowEnumeration(HWND window, LPARAM param) {
...
ProcessSingleton::NotifyResult ProcessSingleton::NotifyOtherProcess() {
  ...
  switch (chrome::AttemptToNotifyRunningChrome(remote_window_, false)) {
    ...
    case chrome::NOTIFY_WINDOW_HUNG:
      // Fall through and potentially terminate the hung host.
      break;
  }
  ...
  ::EnumThreadWindows(thread_id,
                      &HostWindowEnumeration,
                      reinterpret_cast<LPARAM>(&visible_window));

  // If there is a visible host window, ask the user before killing it.
  ...
  // Time to take action. Kill the host process.
...
ProcessSingleton::NotifyOtherProcessOrCreate() {
  ...
  for (int i = 0; i < 2; ++i) {
    if (Create()) {
      ...
      return PROCESS_NONE;  // This is the single host process.
    }
    ...
    if (result == PROCESS_NOTIFIED || result == LOCK_ERROR) {
      ...
      // The single host process was notified, the user chose not to
      // terminate a hung host, or the lock file could not be created.
      // Nothing more to do.
      return result;
    }
