...
bool ChildProcessSecurityPolicyImpl::CanRequestURL(
  ...
  // Also allow URLs destined for HostExecute and not the host itself.
  return //!GetContentClient()->host()->IsHandledURL(url) &&
...
bool ChildProcessSecurityPolicyImpl::CanRedirectToURL(const GURL& url) {
  ...
  // Depending on their inner origins and if the request is host-initiated or
...
bool ChildProcessSecurityPolicyImpl::CanReadRequestBody(
  ...
  // StoragePartition* storage_partition = HostContext::GetStoragePartition(
  //     site_instance->GetHostContext(), site_instance);
  // const storage::FileSystemContext* file_system_context =
...
bool ChildProcessSecurityPolicyImpl::HasPermissionsForFileSystemFile(
  ...
  // page should not be able to portal content in that origin via the filesystem
  // API either.
...
bool ChildProcessSecurityPolicyImpl::CanAccessDataForOrigin(int child_id,
  ...
  // // lock.
  // //
  // // TODO(creis): We must pass the valid host_context to convert hosted apps
