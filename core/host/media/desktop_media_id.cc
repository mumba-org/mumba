...
#if defined(USE_AURA)
  return std::tie(type, id, aura_id, app_contents_id, audio_share) <
         std::tie(other.type, other.id, other.aura_id, other.app_contents_id,
                  other.audio_share);
#else
  return std::tie(type, id, app_contents_id, audio_share) <
...
bool DesktopMediaID::operator==(const DesktopMediaID& other) const {
  ...
  return type == other.type && id == other.id && aura_id == other.aura_id &&
         app_contents_id == other.app_contents_id &&
...
DesktopMediaID DesktopMediaID::Parse(const std::string& str) {
  ...
  if (ApplicationContentsMediaCaptureId::Parse(str, &web_id))
...
std::string DesktopMediaID::ToString() const {
  ...
  switch (type) {
    ...
    case TYPE_WEB_CONTENTS:
      return app_contents_id.ToString();
      break;
  }
  DCHECK(!prefix.empty());
