...
#if !defined(USE_AURA)
#include "core/host/android/content_view_core.h"
...
#if !defined(USE_AURA)
      if (ApplicationContentsDelegate* delegate = application_contents_->GetDelegate()) {
        should_block =
...
BrowserMediaPlayerManager::BrowserMediaPlayerManager(
    ...
    : render_frame_host_(render_frame_host),
      ...
      application_contents_(ApplicationContents::FromRenderFrameHost(render_frame_host)),
      weak_ptr_factory_(this) {
}

BrowserMediaPlayerManager::~BrowserMediaPlayerManager() {
  // During the tear down process, OnDestroyPlayer() may or may not be called
  // (e.g. the ApplicationContents may be destroyed before the render process). So
...
#else
  if (ApplicationContentsDelegate* delegate = application_contents_->GetDelegate())
    delegate->ExitFullscreenModeForTab(application_contents_);
...
bool BrowserMediaPlayerManager::RequestPlay(int player_id,
  ...
  return MediaApplicationContentsObserverAndroid::FromApplicationContents(application_contents_)
...
std::unique_ptr<MediaPlayerAndroid> BrowserMediaPlayerManager::SwapPlayer(
  ...
  for (auto it = players_.begin(); it != players_.end(); ++it) {
    if ((*it)->player_id() == player_id) {
      ...
      MediaApplicationContentsObserverAndroid::FromApplicationContents(application_contents_)
