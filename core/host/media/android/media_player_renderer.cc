...
#include "core/host/media/android/media_resource_getter_impl.h"
...
#include "core/common/content_client.h"
...
MediaPlayerRenderer::MediaPlayerRenderer(int process_id,
  ...
  ApplicationContents* application_contents_impl =
      static_cast<ApplicationContents*>(web_contents);
  application_contents_muted_ = application_contents_impl && application_contents_impl->IsAudioMuted();

  if (web_contents) {
    MediaPlayerRendererApplicationContentsObserver::CreateForApplicationContents(web_contents);
    application_contents_observer_ =
        MediaPlayerRendererApplicationContentsObserver::FromApplicationContents(web_contents);
    if (application_contents_observer_)
      application_contents_observer_->AddMediaPlayerRenderer(this);
  }
}
...
MediaPlayerRenderer::~MediaPlayerRenderer() {
  ...
  if (application_contents_observer_)
    application_contents_observer_->RemoveMediaPlayerRenderer(this);
}
...
void MediaPlayerRenderer::UpdateVolume() {
  float volume = application_contents_muted_ ? 0 : volume_;
  if (media_player_)
...
void MediaPlayerRenderer::OnUpdateAudioMutingState(bool muted) {
  application_contents_muted_ = muted;
  UpdateVolume();
}

void MediaPlayerRenderer::OnApplicationContentsDestroyed() {
  application_contents_observer_ = nullptr;
}
