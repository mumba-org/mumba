...
void BrowserSurfaceViewManager::OnCreateFullscreenSurface(
  ...
  ApplicationWindowHostViewBase * rwhvb =
      static_cast<ApplicationWindowHostViewBase*>(render_frame_host_->GetView());
  if (rwhvb->IsInVR()) {
  ...
  // ApplicationContents has it. Ignore this request and return a null surface id.
  ...
  ApplicationContents* web_contents =
      ApplicationContents::FromRenderFrameHost(render_frame_host_);
