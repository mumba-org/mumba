...
#include "core/host/media/session/audio_focus_delegate.h"
#include "core/host/media/session/media_session_controller.h"
...
#include "core/host/media/session/media_session_android.h"
...
using MapApplicationWindowHostToDepth = std::map<ApplicationWindowHost*, size_t>;

size_t ComputeWindowDepth(ApplicationWindowHost* awh,
                         MapApplicationWindowHostToDepth* map_awh_to_depth) {
  DCHECK(awh);
  size_t depth = 0;
  ApplicationWindowHost* current_window = awh;
  // while (current_window) {
  //   auto it = map_awh_to_depth->find(current_window);
  //   if (it != map_awh_to_depth->end()) {
  //     depth += it->second;
  //     break;
  //   }
  //   ++depth;
...
// static
MediaSession* MediaSession::Get(ApplicationContents* web_contents) {
  return MediaSessionImpl::Get(web_contents);
}

// static
MediaSessionImpl* MediaSessionImpl::Get(ApplicationContents* app_contents) {
  MediaSessionImpl* session = FromApplicationContents(app_contents);
  if (!session) {
    CreateForApplicationContents(app_contents);
    session = FromApplicationContents(app_contents);
    session->Initialize();
  }
  return session;
}
...
MediaSessionImpl::~MediaSessionImpl() {
  //DLOG(INFO) << "~MediaSessionImpl: " << this;
...
void MediaSessionImpl::ApplicationContentsDestroyed() {
  // This should only work for tests. In production, all the players should have
  // already been removed before ApplicationContents is destroyed.
  ...
  // ApplicationContents is destroyed. See https://crbug.com/651069
...
void MediaSessionImpl::ApplicationWindowDeleted(ApplicationWindowHost* awh) {
  if (services_.count(awh))
    OnServiceDestroyed(services_[awh]);
}
...
//   ApplicationWindowHost* awh = navigation_handle->GetApplicationWindowHost();
//   if (services_.count(awh))
//     services_[awh]->DidFinishNavigation();
// }
...
MediaSessionImpl::MediaSessionImpl(ApplicationContents* app_contents)
...
void MediaSessionImpl::OnServiceCreated(MediaSessionServiceImpl* service) {
  ApplicationWindowHost* awh = service->GetApplicationWindowHost();
  if (!awh)
    return;

  services_[awh] = service;
  UpdateRoutedService();
}

void MediaSessionImpl::OnServiceDestroyed(MediaSessionServiceImpl* service) {
  services_.erase(service->GetApplicationWindowHost());
...
void MediaSessionImpl::DidReceiveAction(
  ...
  if (blink::mojom::MediaSessionAction::PAUSE == action) {
    ApplicationWindowHost* awh_of_routed_service =
        routed_service_ ? routed_service_->GetApplicationWindowHost() : nullptr;
    for (const auto& player : normal_players_) {
      if (player.observer->application_window_host() != awh_of_routed_service)
...
bool MediaSessionImpl::IsServiceActiveForApplicationWindowHost(ApplicationWindowHost* awh) {
  return services_.find(awh) != services_.end();
}
...
MediaSessionServiceImpl* MediaSessionImpl::ComputeServiceForRouting() {
  ...
  std::set<ApplicationWindowHost*> windows;
  for (const auto& player : normal_players_) {
    ApplicationWindowHost* window = player.observer->application_window_host();
    if (window)
  ...
  ApplicationWindowHost* best_window = nullptr;
  size_t min_depth = std::numeric_limits<size_t>::max();
  std::map<ApplicationWindowHost*, size_t> map_awh_to_depth;

  for (ApplicationWindowHost* window : windows) {
    size_t depth = ComputeWindowDepth(window, &map_awh_to_depth);
    if (depth >= min_depth)
      continue;
    if (!IsServiceActiveForApplicationWindowHost(window))
      continue;
    best_window = window;
    min_depth = depth;
  }

  return best_window ? services_[best_window] : nullptr;
}
