...
#include "core/host/application/application_contents_android.h"
#include "core/host/application/application_contents_impl.h"
#include "core/common/android/media_metadata_android.h"
...
MediaSessionAndroid::MediaSessionAndroid(MediaSessionImpl* session)
  ...
  ApplicationContentsAndroid* contents_android = GetApplicationContentsAndroid();
  if (contents_android)
...
// static
ScopedJavaLocalRef<jobject> JNI_MediaSessionImpl_GetMediaSessionFromApplicationContents(
    JNIEnv* env,
    const JavaParamRef<jclass>& clazz,
    const JavaParamRef<jobject>& j_contents_android) {
  ApplicationContents* contents = ApplicationContents::FromJavaApplicationContents(j_contents_android);
  if (!contents)
...
ApplicationContentsAndroid* MediaSessionAndroid::GetApplicationContentsAndroid() {
  MediaSessionImpl* session = static_cast<MediaSessionImpl*>(media_session());
  if (!session)
    return nullptr;
  ApplicationContents* contents =
      static_cast<ApplicationContents*>(session->web_contents());
  if (!contents)
    return nullptr;
  return contents->GetApplicationContentsAndroid();
}
