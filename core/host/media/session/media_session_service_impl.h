...
#ifndef MUMBA_HOST_MEDIA_SESSION_MEDIA_SESSION_SERVICE_IMPL_H_
#define MUMBA_HOST_MEDIA_SESSION_MEDIA_SESSION_SERVICE_IMPL_H_
...
class CONTENT_EXPORT MediaSessionServiceImpl
 ...
 public:
  ...
  static void Create(ApplicationWindowHost* app_window_host,
  ...
  ApplicationWindowHost* GetApplicationWindowHost();

  blink::mojom::MediaSessionPlaybackState playback_state() const {
    return playback_state_;
  }
  const base::Optional<common::MediaMetadata>& metadata() const { return metadata_; }
  ...
  void SetMetadata(const base::Optional<common::MediaMetadata>& metadata) override;
 ...
 protected:
  explicit MediaSessionServiceImpl(ApplicationWindowHost* app_window_host);//RenderFrameHost* render_frame_host);
 ...
 private:
  ...
  const int app_window_process_id_;
  const int app_window_routing_id_;
  ...
  base::Optional<common::MediaMetadata> metadata_;
...
#endif  // MUMBA_HOST_MEDIA_SESSION_MEDIA_SESSION_SERVICE_IMPL_H_
