...
class MediaSessionControllerTest : public ApplicationWindowHostImplTestHarness {
 public:
  void SetUp() override {
    ApplicationWindowHostImplTestHarness::SetUp();
    id_ = ApplicationContentsObserver::MediaPlayerId(contents()->GetMainFrame(), 0);
  ...
  void TearDown() override {
    ...
    controller_.reset();
    ApplicationWindowHostImplTestHarness::TearDown();
  }
 ...
 protected:
  std::unique_ptr<MediaSessionController> CreateController() {
    return std::unique_ptr<MediaSessionController>(new MediaSessionController(
        id_, contents()->media_application_contents_observer()));
  }
  ...
  ApplicationContentsObserver::MediaPlayerId id_;
