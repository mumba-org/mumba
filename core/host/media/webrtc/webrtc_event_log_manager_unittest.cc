...
#include "core/host/media/webrtc/webrtc_event_log_manager.h"
...
#include "core/host/media/webrtc/webrtc_event_log_manager_remote.h"
...
using MockApplicationProcessHost = content::MockApplicationProcessHost;
using PeerConnectionKey = WebRtcEventLogPeerConnectionKey;
using ApplicationProcessHost = ApplicationProcessHost;
...
PeerConnectionKey GetPeerConnectionKey(const ApplicationProcessHost* rph, int lid) {
...
class WebRtcEventLogManagerTestBase : public ::testing::TestWithParam<bool> {
 public:
  ...
  void LoadProfiles() {
    ...
    rph_ = std::make_unique<MockApplicationProcessHost>(browser_context_);
  }
  ...
  void SuppressUploading() {
    ...
    upload_suppressing_rph_ = std::make_unique<MockApplicationProcessHost>(
  ...
  content::TestHostThreadBundle test_browser_thread_bundle_;
  ...
  // Default BrowserContext and ApplicationProcessHost, to be used by tests which
  ...
  std::unique_ptr<MockApplicationProcessHost> rph_;
  ...
  std::unique_ptr<MockApplicationProcessHost> upload_suppressing_rph_;
  ...
  //    a ApplicationProcessHost is deallocated (which can potentially trigger a
...
class WebRtcEventLogManagerTestCacheClearing
  ...
  void CreatePendingLogFilesForBrowserContext(BrowserContext* browser_context) {
    ...
    for (size_t i = 0; i < kMaxActiveRemoteBoundWebRtcEventLogs; ++i) {
      elements->rphs.push_back(
          std::make_unique<MockApplicationProcessHost>(browser_context));
 ...
 protected:
  struct BrowserContextAssociatedElements {
    std::vector<std::unique_ptr<MockApplicationProcessHost>> rphs;
...
TEST_F(WebRtcEventLogManagerTest, LocalLogMultipleActiveFiles) {
  ...
  std::list<MockApplicationProcessHost> rphs;
...
TEST_F(WebRtcEventLogManagerTest, RemoteLogFileCreatedInCorrectDirectory) {
  ...
  for (size_t i = 0; i < kLogsNum; ++i) {
    ...
    rphs.emplace_back(std::make_unique<MockApplicationProcessHost>(browser_context));
  }
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  std::unique_ptr<MockApplicationProcessHost> rphs[2] = {
      std::make_unique<MockApplicationProcessHost>(browser_context_),
      std::make_unique<MockApplicationProcessHost>(browser_context_),
  };
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  std::unique_ptr<MockApplicationProcessHost> rphs[2] = {
      std::make_unique<MockApplicationProcessHost>(browser_contexts[0]),
      std::make_unique<MockApplicationProcessHost>(browser_contexts[1])};
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  for (size_t i = 0; i < 2; ++i) {
    ...
    rphs[i] = std::make_unique<MockApplicationProcessHost>(browser_contexts[i]);
  }
...
TEST_F(WebRtcEventLogManagerTest, LocalLogsClosedWhenApplicationProcessHostExits) {
  const auto key = GetPeerConnectionKey(rph_.get(), kLid);
  ASSERT_TRUE(PeerConnectionAdded(key.render_process_id, key.lid));
  ASSERT_TRUE(EnableLocalLogging());

  // The expectation for OnLocalLogStopped() will be saturated by this
  // destruction of the ApplicationProcessHost, which triggers an implicit
  // removal of all PeerConnections associated with it.
  EXPECT_CALL(local_observer_, OnLocalLogStopped(key)).Times(1);
  rph_.reset();
}

TEST_F(WebRtcEventLogManagerTest, RemoteLogsClosedWhenApplicationProcessHostExits) {
  const auto key = GetPeerConnectionKey(rph_.get(), kLid);
  ASSERT_TRUE(PeerConnectionAdded(key.render_process_id, key.lid));
  ASSERT_TRUE(StartRemoteLogging(key.render_process_id, GetUniqueId(key)));
...
// Once a ApplicationProcessHost exits/crashes, its PeerConnections are removed,
// which means that they can no longer suppress an upload.
TEST_F(WebRtcEventLogManagerTest,
       ApplicationProcessHostExitCanRemoveUploadSuppression) {
  SuppressUploading();
  ...
  // PeerConnection is associated, that upload will take portal.
  base::RunLoop run_loop;
