...
class WebRtcApprtcBrowserTest : public WebRtcTestBase {
 ...
 protected:
  ...
  bool WaitForCallToComeUp(ApplicationContents* tab_contents) {
    return test::PollingWaitUntil(kIsApprtcCallUpJavascript, "true",
                                  tab_contents);
  }

  bool EvalInJavascriptFile(ApplicationContents* tab_contents,
...
IN_PROC_BROWSER_TEST_F(WebRtcApprtcBrowserTest, MANUAL_WorksOnApprtc) {
  ...
  ApplicationContents* left_tab =
      browser()->tab_strip_model()->GetActiveApplicationContents();
  PermissionRequestManager::FromApplicationContents(left_tab)
      ->set_auto_response_for_test(PermissionRequestManager::ACCEPT_ALL);
  InfoBarResponder left_infobar_responder(
      InfoBarService::FromApplicationContents(left_tab), InfoBarResponder::ACCEPT);
  ...
  chrome::AddTabAt(browser(), GURL(), -1, true);
  ApplicationContents* right_tab =
      browser()->tab_strip_model()->GetActiveApplicationContents();
  PermissionRequestManager::FromApplicationContents(right_tab)
      ->set_auto_response_for_test(PermissionRequestManager::ACCEPT_ALL);
  InfoBarResponder right_infobar_responder(
      InfoBarService::FromApplicationContents(right_tab), InfoBarResponder::ACCEPT);
  ui_test_utils::NavigateToURL(browser(), room_url);
  ...
  chrome::CloseApplicationContents(browser(), left_tab, false);
  chrome::CloseApplicationContents(browser(), right_tab, false);
}
