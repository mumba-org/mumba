...
#include "core/host/media/webrtc/tab_desktop_media_list.h"
...
#include "core/host/media/webrtc/desktop_media_list_observer.h"
 ...
 protected:
  ...
  void AddWebcontents(int favicon_greyscale) {
    ...
    std::unique_ptr<ApplicationContents> contents(
        ApplicationContentsTester::CreateTestApplicationContents(
    ...
    tab_strip_model->AppendApplicationContents(std::move(contents), false);
  }
  ...
  void TearDown() override {
    ...
    // owned ApplicationContents. Then |manually_added_web_contents_| won't be
    // necessary. https://crbug.com/832879.
    TabStripModel* tab_strip_model = browser_->tab_strip_model();
    for (ApplicationContents* contents : manually_added_web_contents_) {
      tab_strip_model->DetachApplicationContentsAt(
          tab_strip_model->GetIndexOfApplicationContents(contents));
    }
    manually_added_web_contents_.clear();
  ...
  std::vector<ApplicationContents*> manually_added_web_contents_;

  content::TestHostThreadBundle thread_bundle_;
...
TEST_F(TabDesktopMediaListTest, RemoveTab) {
  ...
  TabStripModel* tab_strip_model = browser_->tab_strip_model();
  ASSERT_TRUE(tab_strip_model);
  std::unique_ptr<ApplicationContents> released_web_contents =
      tab_strip_model->DetachApplicationContentsAt(kDefaultSourceCount - 1);
...
TEST_F(TabDesktopMediaListTest, MoveTab) {
  ...
  ApplicationContents* contents0 = tab_strip_model->GetApplicationContentsAt(0);
  ASSERT_TRUE(contents0);
  base::TimeTicks t0 = contents0->GetLastActiveTime();
  ApplicationContents* contents1 = tab_strip_model->GetApplicationContentsAt(1);
...
TEST_F(TabDesktopMediaListTest, UpdateTitle) {
  ...
  ApplicationContents* contents =
      tab_strip_model->GetApplicationContentsAt(kDefaultSourceCount - 1);
  ASSERT_TRUE(contents);
