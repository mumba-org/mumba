...
class PermissionRequestObserver : public PermissionRequestManager::Observer {
 public:
  explicit PermissionRequestObserver(ApplicationContents* web_contents)
      : request_manager_(
            PermissionRequestManager::FromApplicationContents(web_contents)),
...
bool WebRtcTestBase::GetUserMediaWithSpecificConstraintsAndAccept(
  ...
  std::string result;
  PermissionRequestManager::FromApplicationContents(tab_contents)
...
void WebRtcTestBase::GetUserMediaAndDeny(ApplicationContents* tab_contents) {
...
void WebRtcTestBase::GetUserMedia(ApplicationContents* tab_contents,
                                  const std::string& constraints) const {
...
ApplicationContents* WebRtcTestBase::OpenPageAndGetUserMediaInNewTab(
...
WebRtcTestBase::OpenPageAndGetUserMediaInNewTabWithConstraints(
  ...
  // okay).
  PermissionRequestManager::FromApplicationContents(new_tab)
      ->set_auto_response_for_test(PermissionRequestManager::ACCEPT_ALL);
...
ApplicationContents* WebRtcTestBase::OpenTestPageAndGetUserMediaInNewTab(
    const std::string& test_page) const {
  return OpenPageAndGetUserMediaInNewTab(
      embedded_test_server()->GetURL(test_page));
}

ApplicationContents* WebRtcTestBase::OpenTestPageInNewTab(
    const std::string& test_page) const {
  chrome::AddTabAt(browser(), GURL(), -1, true);
...
void WebRtcTestBase::CreateDataChannel(ApplicationContents* tab,
                                       const std::string& label) {
  EXPECT_EQ("ok-created",
            ExecuteJavascript("createDataChannel('" + label + "')", tab));
}

void WebRtcTestBase::NegotiateCall(ApplicationContents* from_tab,
                                   ApplicationContents* to_tab) const {
...
void WebRtcTestBase::HangUp(ApplicationContents* from_tab) const {
...
void WebRtcTestBase::OpenDatabase(ApplicationContents* tab) const {
  EXPECT_EQ("ok-database-opened", ExecuteJavascript("openDatabase()", tab));
}

void WebRtcTestBase::CloseDatabase(ApplicationContents* tab) const {
  EXPECT_EQ("ok-database-closed", ExecuteJavascript("closeDatabase()", tab));
}

void WebRtcTestBase::DeleteDatabase(ApplicationContents* tab) const {
  EXPECT_EQ("ok-database-deleted", ExecuteJavascript("deleteDatabase()", tab));
}

void WebRtcTestBase::GenerateAndCloneCertificate(
    ApplicationContents* tab, const std::string& keygen_algorithm) const {
  std::string javascript = base::StringPrintf(
...
WebRtcTestBase::GetStatsReportDictionary(ApplicationContents* tab) const {
...
void WebRtcTestBase::SetDefaultVideoCodec(ApplicationContents* tab,
...
void WebRtcTestBase::EnableOpusDtx(ApplicationContents* tab) const {
  EXPECT_EQ("ok-forced", ExecuteJavascript("forceOpusDtx()", tab));
}

void WebRtcTestBase::CreateAndAddStreams(ApplicationContents* tab,
                                         size_t count) const {
  EXPECT_EQ(
...
void WebRtcTestBase::RemoveTrack(ApplicationContents* tab,
...
bool WebRtcTestBase::HasStreamWithTrack(ApplicationContents* tab,
...
bool WebRtcTestBase::HasSenderWithTrack(ApplicationContents* tab,
                                        std::string track_id) const {
  std::string javascript =
...
bool WebRtcTestBase::HasReceiverWithTrack(ApplicationContents* tab,
                                          std::string track_id) const {
  std::string javascript =
...
void WebRtcTestBase::CollectGarbage(ApplicationContents* tab) const {
  EXPECT_EQ("ok-gc", ExecuteJavascript("collectGarbage()", tab));
}

std::string WebRtcTestBase::GetDesktopMediaStream(ApplicationContents* tab) {
