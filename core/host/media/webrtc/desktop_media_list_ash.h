...
class DesktopMediaListAsh : public DesktopMediaListBase {
 public:
  explicit DesktopMediaListAsh(DesktopMediaID::Type type);
 ...
 private:
  ...
  void EnumerateSources(
      std::vector<DesktopMediaListAsh::SourceDescription>* windows);
  void CaptureThumbnail(DesktopMediaID id, aura::Window* window);
  void OnThumbnailCaptured(DesktopMediaID id, gfx::Image image);
