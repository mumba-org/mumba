...
class DesktopMediaPicker {
 public:
  typedef base::Callback<void(DesktopMediaID)> DoneCallback;
  struct Params {
    Params();
    ~Params();

    // ApplicationContents this picker is relative to, can be null.
    ApplicationContents* app_contents = nullptr;
