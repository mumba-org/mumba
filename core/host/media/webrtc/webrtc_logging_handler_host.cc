...
#include "core/host/media/webrtc/webrtc_logging_handler_host.h"
...
#include "core/host/media/webrtc/webrtc_log_uploader.h"
#include "core/host/media/webrtc/webrtc_rtp_dump_handler.h"
...
// Key used to attach the handler to the ApplicationProcessHost.
...
void WebRtcLoggingHandlerHost::StartRtpDump(
    RtpDumpType type,
    const GenericDoneCallback& callback,
    const ApplicationProcessHost::WebRtcStopRtpDumpCallback&
        stop_callback) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
...
base::FilePath WebRtcLoggingHandlerHost::GetLogDirectoryAndEnsureExists() {
  ...
  // Since we can be alive after the ApplicationProcessHost and the BrowserContext
