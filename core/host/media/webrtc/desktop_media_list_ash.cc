...
#include "core/host/media/webrtc/desktop_media_list_ash.h"

#include "ash/public/cpp/domain_window_ids.h"
#include "ash/shell.h"
...
DesktopMediaListAsh::DesktopMediaListAsh(DesktopMediaID::Type type)
    : DesktopMediaListBase(base::TimeDelta::FromMilliseconds(
          kDefaultDesktopMediaListUpdatePeriod)),
      weak_factory_(this) {
  DCHECK(type == DesktopMediaID::TYPE_SCREEN ||
         type == DesktopMediaID::TYPE_WINDOW);
  type_ = type;
}
...
void DesktopMediaListAsh::EnumerateWindowsForRoot(
  ...
  aura::Window* container = ash::Domain::GetContainer(root_window, container_id);
  if (!container)
    return;
  for (aura::Window::Windows::const_iterator it = container->children().begin();
    ...
    if (!(*it)->IsVisible() || !(*it)->CanFocus())
      continue;
    DesktopMediaID id = DesktopMediaID::RegisterAuraWindow(
        DesktopMediaID::TYPE_WINDOW, *it);
...
void DesktopMediaListAsh::EnumerateSources(
  ...
  aura::Window::Windows root_windows = ash::Domain::GetAllRootWindows();

  for (size_t i = 0; i < root_windows.size(); ++i) {
    if (type_ == DesktopMediaID::TYPE_SCREEN) {
      SourceDescription screen_source(
          DesktopMediaID::RegisterAuraWindow(
              DesktopMediaID::TYPE_SCREEN, root_windows[i]),
          root_windows[i]->GetTitle());

      if (root_windows[i] == ash::Domain::GetPrimaryRootWindow())
      ...
      EnumerateWindowsForRoot(
          sources, root_windows[i], ash::kDomainWindowId_DefaultContainer);
      EnumerateWindowsForRoot(
          sources, root_windows[i], ash::kDomainWindowId_AlwaysOnTopContainer);
    }
  }
}

void DesktopMediaListAsh::CaptureThumbnail(DesktopMediaID id,
                                           aura::Window* window) {
...
void DesktopMediaListAsh::OnThumbnailCaptured(DesktopMediaID id,
                                              gfx::Image image) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
