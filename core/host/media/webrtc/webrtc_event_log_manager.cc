...
class PeerConnectionTrackerProxyImpl
 ...
 private:
  static void SetWebRtcEventLoggingStateInternal(
    ...
    ApplicationProcessHost* host = ApplicationProcessHost::FromID(key.render_process_id);
    if (!host) {
...
const BrowserContext* GetBrowserContext(int render_process_id) {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  ApplicationProcessHost* const host = ApplicationProcessHost::FromID(render_process_id);
...
WebRtcEventLogManager::~WebRtcEventLogManager() {
  ...
  for (ApplicationProcessHost* host : observed_render_process_hosts_) {
...
void WebRtcEventLogManager::PeerConnectionAdded(
  ...
  ApplicationProcessHost* rph = ApplicationProcessHost::FromID(render_process_id);
  if (!rph) {
...
void WebRtcEventLogManager::PeerConnectionRemoved(
  ...
  if (browser_context_id == kNullBrowserContextId) {
    ...
    // RenderProcessExited() / ApplicationProcessHostDestroyed.
    MaybeReply(std::move(reply), false);
    return;
  }
...
void WebRtcEventLogManager::RenderProcessExited(
  ...
  DCHECK_CURRENTLY_ON(HostThread::UI);
  ApplicationProcessHostExitedDestroyed(host);
}

void WebRtcEventLogManager::ApplicationProcessHostDestroyed(
    ApplicationProcessHost* host) {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  ApplicationProcessHostExitedDestroyed(host);
}

void WebRtcEventLogManager::ApplicationProcessHostExitedDestroyed(
    ApplicationProcessHost* host) {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  DCHECK(host);
...
void WebRtcEventLogManager::EnableForBrowserContextInternal(
  ...
  if (reply) {
    HostThread::PostTask(HostThread::UI, FROM_HERE, std::move(reply));
  }
}
...
void WebRtcEventLogManager::RenderProcessExitedInternal(int render_process_id) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  local_logs_manager_.ApplicationProcessHostExitedDestroyed(render_process_id);
  if (remote_logs_manager_) {
    remote_logs_manager_->ApplicationProcessHostExitedDestroyed(render_process_id);
  }
}
