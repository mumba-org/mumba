...
#include "core/host/media/mojo_audio_logging_adapter.h"
...
//   content::ApplicationProcessHost* render_process_host =
//       content::ApplicationProcessHost::FromID(render_process_id);
//   if (!render_process_host) {
//     // This could happen in tests.
//     LOG(ERROR) << "Cannot get ApplicationProcessHost";
//     return false;
//   }
...
class AudioLogImpl : public media::mojom::AudioLog {
 ...
 private:
  // If possible, i.e. a ApplicationContents exists for the given RenderFrameHostID,
  // tells an existing AudioLogEntry the ApplicationContents title for easier
  // differentiation on the UI. Note that the log entry must be created (by
  // calling OnCreated() before calling this method.
  void SetApplicationContentsTitle();
  ...
  static void SendApplicationContentsTitleHelper(
...
void AudioLogImpl::SetApplicationContentsTitle() {
  if (render_process_id_ < 0 || render_frame_id_ < 0)
    return;
  std::unique_ptr<base::DictionaryValue> dict(new base::DictionaryValue());
  StoreComponentMetadata(dict.get());
  SendApplicationContentsTitleHelper(FormatCacheKey(), std::move(dict),
...
// static
void AudioLogImpl::SendApplicationContentsTitleHelper(
    const std::string& cache_key,
    std::unique_ptr<base::DictionaryValue> dict,
    int render_process_id,
    int render_frame_id) {
  ...
  if (!HostThread::CurrentlyOn(HostThread::UI)) {
    HostThread::PostTask(
        ...
        base::BindOnce(&SendApplicationContentsTitleHelper, cache_key, std::move(dict),
                       render_process_id, render_frame_id));
    return;
  }

  const ApplicationContents* web_contents = ApplicationContents::FromApplicationWindowHost(
  ...
  dict->SetString("application_contents_title", web_contents->GetTitle());
...
void MediaInternals::MediaInternalsUMAHandler::SavePlayerState(
  ...
  if (it == player_info_map.end()) {
    ...
    std::tie(it, success) = player_info_map.emplace(
        std::make_pair(event.id, PipelineInfo(false)));//IsIncognito(render_process_id))));
    if (!success) {
...
void MediaInternals::Observe(int type,
  ...
  DCHECK_CURRENTLY_ON(HostThread::UI);
  ApplicationProcessHost* process = Source<ApplicationProcessHost>(source).ptr();
