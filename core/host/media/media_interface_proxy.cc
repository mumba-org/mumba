...
#include "core/host/provision_fetcher_impl.h"
...
#include "base/guid.h"
#include "core/host/media/cdm_storage_impl.h"
...
// TODO(xhwang): Move this to a common portal.
...
void MediaInterfaceProxy::CreateRenderer(
  ...
  if (type == media::mojom::HostedRendererType::kFlinging) {
    std::unique_ptr<FlingingRenderer> renderer =
        FlingingRenderer::Create(application_window_host_, type_specific_id);
...
MediaInterfaceProxy::GetFrameServices(const std::string& cdm_guid,
  ...
  if (!cdm_file_system_id.empty()) {
    provider->registry()->AddInterface(base::BindRepeating(
        &CdmStorageImpl::Create, application_window_host_, cdm_file_system_id));
  }
  ...
  common::GetClient()->host()->ExposeInterfacesToMediaService(
      provider->registry(), application_window_host_);
...
media::mojom::CdmFactory* MediaInterfaceProxy::GetCdmFactory(
  ...
  std::unique_ptr<common::CdmInfo> cdm_info =
...
void MediaInterfaceProxy::CreateMediaPlayerRenderer(
  ...
  auto renderer = std::make_unique<MediaPlayerRenderer>(
      ...
      application_window_host_->delegate()->GetAsApplicationContents());
