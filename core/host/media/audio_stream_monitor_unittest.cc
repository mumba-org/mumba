...
#include "core/host/application_contents_delegate.h"
...
class MockApplicationContentsDelegate : public ApplicationContentsDelegate {
 public:
  MOCK_METHOD2(NavigationStateChanged,
               void(ApplicationContents* source, InvalidateTypes changed_flags));
};

}  // namespace

class AudioStreamMonitorTest : public ApplicationWindowHostTestHarness {
 public:
  ...
  void SetUp() override {
    ApplicationWindowHostTestHarness::SetUp();

    ApplicationContents* web_contents = reinterpret_cast<ApplicationContents*>(
        ApplicationWindowHostTestHarness::web_contents());
    web_contents->SetDelegate(&mock_application_contents_delegate_);
  ...
  void ExpectRecentlyAudibleChangeNotification(bool new_recently_audible) {
    EXPECT_CALL(
        mock_application_contents_delegate_,
        NavigationStateChanged(ApplicationWindowHostTestHarness::web_contents(),
                               INVALIDATE_TYPE_TAB))
  ...
  ApplicationContents* web_contents() { return monitor_->application_contents_; }
 ...
 private:
  ...
  MockApplicationContentsDelegate mock_application_contents_delegate_;
...
TEST_F(AudioStreamMonitorTest, IndicatorIsOnUntilHoldingPeriodHasPassed) {
  ...
  // Expect ApplicationContents will get one call form AudioStreamMonitor to toggle the
