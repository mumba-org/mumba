...
#define DCHECK_ON_DEVICE_THREAD() DCHECK_CURRENTLY_ON(HostThread::IO)
#define DCHECK_NOT_ON_DEVICE_THREAD() DCHECK_CURRENTLY_ON(HostThread::UI)

// Convenience macro to block the test procedure and run all pending UI tasks.
#define RUN_UI_TASKS() RunAllPendingInMessageLoop(HostThread::UI)

// Convenience macro to post a task to run on the device thread.
#define POST_DEVICE_TASK(closure) \
  HostThread::PostTask(HostThread::IO, FROM_HERE, closure)

// Convenience macro to block the test procedure until all pending tasks have
// run on the device thread.
#define WAIT_FOR_DEVICE_TASKS() RunAllPendingInMessageLoop(HostThread::IO)
...
// Helper to return the capture parameters containerd in a VideoCaptureParams.
...
class FrameSinkVideoCaptureDeviceTest : public testing::Test {
 public:
  FrameSinkVideoCaptureDeviceTest()
      : browser_threads_(TestHostThreadBundle::REAL_IO_THREAD) {}
 ...
 private:
  ...
  TestHostThreadBundle browser_threads_;
