...
// by the entire RenderFrame tree associated with a ApplicationContents instance.  The
...
// interruption.  In other words, ApplicationContentsAudioInputStream provides tab-level
// audio mirroring.

#ifndef MUMBA_HOST_MEDIA_CAPTURE_WEB_CONTENTS_AUDIO_INPUT_STREAM_H_
#define MUMBA_HOST_MEDIA_CAPTURE_WEB_CONTENTS_AUDIO_INPUT_STREAM_H_
...
class ApplicationContentsTracker;

class CONTENT_EXPORT ApplicationContentsAudioInputStream
 ...
 public:
  ...
  // ApplicationContentsCaptureUtil::ExtractTabCaptureTarget().  The caller must
  ...
  static ApplicationContentsAudioInputStream* Create(
 ...
 private:
  friend class ApplicationContentsAudioInputStreamTest;
  ...
  // AudioMirroringManager on the IO thread, the ApplicationContentsTracker on the UI
  // thread, and the VirtualAudioOuputStreams on the audio thread.
  class Impl;

  ApplicationContentsAudioInputStream(int render_process_id,
  ...
  ~ApplicationContentsAudioInputStream() override;

  scoped_refptr<Impl> impl_;

  DISALLOW_COPY_AND_ASSIGN(ApplicationContentsAudioInputStream);
};

}  // namespace host

#endif  // MUMBA_HOST_MEDIA_CAPTURE_WEB_CONTENTS_AUDIO_INPUT_STREAM_H_
