...
// that ensures the object is destroyed on the UI HostThread.
template <typename T>
std::unique_ptr<T, HostThread::DeleteOnUIThread> RescopeToUIThread(
    std::unique_ptr<T>&& ptr) {
  return std::unique_ptr<T, HostThread::DeleteOnUIThread>(ptr.release());
}
...
void FrameSinkVideoCaptureDevice::AllocateAndStartWithReceiver(
  ...
  HostThread::PostTaskAndReplyWithResult(
      HostThread::UI, FROM_HERE, base::BindOnce(capturer_creator_),
