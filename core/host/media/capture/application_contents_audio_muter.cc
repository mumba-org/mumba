...
// identifies the audio streams rendered by a ApplicationContents and provides
// AudioDiscarders to AudioMirroringManager.
class ApplicationContentsAudioMuter::MuteDestination
    : public base::RefCountedThreadSafe<MuteDestination>,
      public AudioMirroringManager::MirroringDestination {
 public:
  explicit MuteDestination(ApplicationContents* web_contents)
      : application_contents_(web_contents) {}
 ...
 private:
  ...
  void QueryForMatchesOnUIThread(const std::set<SourceFrameRef>& candidates,
    ...
    // ApplicationContents being muted.
    for (std::set<SourceFrameRef>::const_iterator i = candidates.begin();
      ...
      if (contents_containing_frame == application_contents_)
        matches.insert(*i);
    }
...
ApplicationContentsAudioMuter::ApplicationContentsAudioMuter(ApplicationContents* web_contents)
    : destination_(new MuteDestination(web_contents)), is_muting_(false) {
  DCHECK_CURRENTLY_ON(HostThread::UI);
}

ApplicationContentsAudioMuter::~ApplicationContentsAudioMuter() {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  StopMuting();
}

void ApplicationContentsAudioMuter::StartMuting() {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  if (is_muting_)
    return;
  is_muting_ = true;
...
void ApplicationContentsAudioMuter::StopMuting() {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  if (!is_muting_)
    return;
  is_muting_ = false;
