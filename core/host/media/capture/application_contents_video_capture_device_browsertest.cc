  ...
  class FakeVideoFrameReceiver : public media::VideoFrameReceiver {
   ...
   private:
    ...
    void OnFrameReadyInBuffer(
      ...
      // This destruction observer will notify the ApplicationContentsVideoCaptureDevice
...
class ApplicationContentsVideoCaptureDeviceBrowserTest : public ContentBrowserTest {
 public:
  FakeVideoCaptureStack* capture_stack() { return &capture_stack_; }
  ApplicationContentsVideoCaptureDevice* device() const { return device_.get(); }
  ...
  // Returns the size of the ApplicationContents top-level frame view.
  ...
  // which is based on the size of the Domain window.
  ...
  void AllocateAndStartAndWaitForFirstFrame() {
    ...
    capture_stack()->SetFrameCallback(
        base::BindRepeating(&ApplicationContentsVideoCaptureDeviceBrowserTest::OnFrame,
                            base::Unretained(this)));

    auto* const main_frame = shell()->web_contents()->GetMainFrame();
    device_ = std::make_unique<ApplicationContentsVideoCaptureDevice>(
        main_frame->GetProcess()->GetID(), main_frame->GetRoutingID());
    device_->AllocateAndStartWithReceiver(
        SnapshotCaptureParams(), capture_stack()->CreateFrameReceiver());
    RunAllPendingInMessageLoop(HostThread::UI);
  ...
  void WaitForFrameWithColor(SkColor color) {
    ...
    while (!testing::Test::HasFailure()) {
      ...
      HostThread::PostDelayedTask(HostThread::UI, FROM_HERE,
 ...
 protected:
  ...
  void SetUpOnMainThread() override {
    ...
    embedded_test_server()->RegisterRequestHandler(base::BindRepeating(
        &ApplicationContentsVideoCaptureDeviceBrowserTest::HandleRequest,
        base::Unretained(this)));
 ...
 private:
  ...
  std::unique_ptr<ApplicationContentsVideoCaptureDevice> device_;
...
// static
constexpr char ApplicationContentsVideoCaptureDeviceBrowserTest::kInnerFrameHostname[];
// static
constexpr char ApplicationContentsVideoCaptureDeviceBrowserTest::kInnerFramePath[];
// static
constexpr char ApplicationContentsVideoCaptureDeviceBrowserTest::kOuterFrameHostname[];
// static
constexpr char ApplicationContentsVideoCaptureDeviceBrowserTest::kOuterFramePath[];
// static
constexpr char ApplicationContentsVideoCaptureDeviceBrowserTest::kSingleFrameHostname[];
// static
constexpr char ApplicationContentsVideoCaptureDeviceBrowserTest::kSingleFramePath[];

// Tests that the device refuses to start if the ApplicationContents target was
// destroyed before the device could start.
IN_PROC_BROWSER_TEST_F(ApplicationContentsVideoCaptureDeviceBrowserTest,
                       ErrorsOutIfApplicationContentsHasGoneBeforeDeviceStart) {
...
// errors-out because the ApplicationContents is destroyed before the device is stopped.
IN_PROC_BROWSER_TEST_F(ApplicationContentsVideoCaptureDeviceBrowserTest,
                       ErrorsOutWhenApplicationContentsIsDestroyed) {
  NavigateToInitialDocument();
  ...
  // Delete the ApplicationContents instance and the Domain, and allow the the "target
...
class ApplicationContentsVideoCaptureDeviceBrowserTestP
    : public ApplicationContentsVideoCaptureDeviceBrowserTest,
...
INSTANTIATE_TEST_CASE_P(
    ,
    ApplicationContentsVideoCaptureDeviceBrowserTestP,
    testing::Combine(
...
// compositing, whether the ApplicationContents is visible/hidden or occluded/unoccluded
// and whether the main document contains a cross-site iframe.
IN_PROC_BROWSER_TEST_P(ApplicationContentsVideoCaptureDeviceBrowserTestP,
  ...
  for (int visilibilty_case = 0; visilibilty_case < 3; ++visilibilty_case) {
    switch (visilibilty_case) {
      case 0:
        VLOG(1) << "Visibility case: ApplicationContents is showing.";
        ...
        break;
      case 1:
        VLOG(1) << "Visibility case: ApplicationContents is hidden.";
        ...
        break;
      case 2:
        VLOG(1) << "Visibility case: ApplicationContents is showing, but occluded.";
        shell()->web_contents()->WasShown();
