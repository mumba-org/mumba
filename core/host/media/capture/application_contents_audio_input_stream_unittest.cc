...
class MockApplicationContentsTracker : public ApplicationContentsTracker {
 public:
  MockApplicationContentsTracker() : ApplicationContentsTracker(false) {}
 ...
 private:
  virtual ~MockApplicationContentsTracker() {}

  DISALLOW_COPY_AND_ASSIGN(MockApplicationContentsTracker);
};
...
class ApplicationContentsAudioInputStreamTest : public testing::TestWithParam<bool> {
 public:
  ApplicationContentsAudioInputStreamTest()
      : thread_bundle_(new TestHostThreadBundle(
            TestHostThreadBundle::REAL_IO_THREAD)),
        audio_thread_("Audio thread"),
        mock_mirroring_manager_(new MockAudioMirroringManager()),
        mock_tracker_(new MockApplicationContentsTracker()),
        mock_vais_(nullptr),
        wcais_(nullptr),
  ...
  ~ApplicationContentsAudioInputStreamTest() override {
    audio_thread_.Stop();
  ...
  // If this value is true, we are testing a ApplicationContentsAudioInputStream
  // instance, which requests duplicate audio.
  // Otherwise, we are testing a ApplicationContentsAudioInputStream instance, which
  ...
  void Open() {
    ...
    wcais_ = new ApplicationContentsAudioInputStream(
  ...
  void Close() {
    // ApplicationContentsAudioInputStream self-destructs on Close().  Its internal
 ...
 private:
  ...
  std::unique_ptr<TestHostThreadBundle> thread_bundle_;
  base::Thread audio_thread_;

  std::unique_ptr<MockAudioMirroringManager> mock_mirroring_manager_;
  scoped_refptr<MockApplicationContentsTracker> mock_tracker_;

  MockVirtualAudioInputStream* mock_vais_;  // Owned by wcais_.
  ApplicationContentsAudioInputStream* wcais_;  // Self-destructs on Close().

  // Mock consumer of audio data.
  MockAudioInputCallback mock_input_callback_;

  // Provided by ApplicationContentsAudioInputStream to the mock ApplicationContentsTracker.
  // This callback is saved here, and test code will invoke it to simulate
  // target change events.
  ApplicationContentsTracker::ChangeCallback change_callback_;

  // Provided by ApplicationContentsAudioInputStream to the mock AudioMirroringManager.
  ...
  // Streams provided by calls to ApplicationContentsAudioInputStream::AddInput().  Each
  ...
  DISALLOW_COPY_AND_ASSIGN(ApplicationContentsAudioInputStreamTest);
};

#define RUN_ON_AUDIO_THREAD(method) \
  RunOnAudioThread(base::Bind(&ApplicationContentsAudioInputStreamTest::method,  \
                              base::Unretained(this)))

TEST_P(ApplicationContentsAudioInputStreamTest, OpenedButNeverStarted) {
  RUN_ON_AUDIO_THREAD(Open);
  RUN_ON_AUDIO_THREAD(Close);
}

TEST_P(ApplicationContentsAudioInputStreamTest, MirroringNothing) {
  RUN_ON_AUDIO_THREAD(Open);
...
TEST_P(ApplicationContentsAudioInputStreamTest, MirroringOutputOutlivesSession) {
  RUN_ON_AUDIO_THREAD(Open);
  RUN_ON_AUDIO_THREAD(Start);
...
TEST_P(ApplicationContentsAudioInputStreamTest, MirroringOutputWithinSession) {
  RUN_ON_AUDIO_THREAD(Open);
  RUN_ON_AUDIO_THREAD(Start);
  RUN_ON_AUDIO_THREAD(AddAnotherInput);
  WaitForData();
...
TEST_P(ApplicationContentsAudioInputStreamTest, MirroringNothingWithTargetChange) {
  RUN_ON_AUDIO_THREAD(Open);
  RUN_ON_AUDIO_THREAD(Start);
...
TEST_P(ApplicationContentsAudioInputStreamTest, MirroringOneStreamAfterTargetChange) {
  RUN_ON_AUDIO_THREAD(Open);
  RUN_ON_AUDIO_THREAD(Start);
  RUN_ON_AUDIO_THREAD(ChangeMirroringTarget);
  RUN_ON_AUDIO_THREAD(AddAnotherInput);
...
TEST_P(ApplicationContentsAudioInputStreamTest, MirroringOneStreamWithTargetChange) {
  RUN_ON_AUDIO_THREAD(Open);
  RUN_ON_AUDIO_THREAD(Start);
  RUN_ON_AUDIO_THREAD(AddAnotherInput);
  WaitForData();
...
TEST_P(ApplicationContentsAudioInputStreamTest, MirroringLostTarget) {
  RUN_ON_AUDIO_THREAD(Open);
  RUN_ON_AUDIO_THREAD(Start);
  RUN_ON_AUDIO_THREAD(AddAnotherInput);
  WaitForData();
...
TEST_P(ApplicationContentsAudioInputStreamTest, MirroringMultipleStreamsAndTargets) {
  RUN_ON_AUDIO_THREAD(Open);
  RUN_ON_AUDIO_THREAD(Start);
  RUN_ON_AUDIO_THREAD(AddAnotherInput);
  WaitForData();
...
INSTANTIATE_TEST_CASE_P(, ApplicationContentsAudioInputStreamTest, ::testing::Bool());
