...
#include "core/host/webrtc/webrtc_webcam_browsertest.h"
#include "core/host/browser_child_process_host.h"
...
IN_PROC_BROWSER_TEST_F(WebRtcVideoCaptureBrowserTest,
  ...
  ASSERT_TRUE(ExecuteScriptAndExtractString(
      host(), kStartVideoCaptureAndVerifySize, &result));
  ASSERT_EQ("OK", result);
  ...
  ASSERT_TRUE(ExecuteScriptAndExtractString(host(), kWaitForVideoToTurnBlack,
                                            &result));
  ASSERT_EQ("OK", result);
  ASSERT_TRUE(ExecuteScriptAndExtractString(
      host(), kVerifyHasReceivedTrackEndedEvent, &result));
  ASSERT_EQ("OK", result);
