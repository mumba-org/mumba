...
#include "core/host/webrtc/webrtc_content_browsertest_base.h"
...
IN_PROC_BROWSER_TEST_F(WebRtcAudioDebugRecordingsBrowserTest,
  ...
  GURL url(embedded_test_server()->GetURL("/media/peerconnection-call.html"));
  NavigateToURL(host(), url);
  NavigateToURL(host2, url);
  ExecuteJavascriptAndWaitForOk("call({video: true, audio: true});");
  std::string result;
  EXPECT_TRUE(ExecuteScriptAndExtractString(
      host2, "call({video: true, audio: true});", &result));
  ASSERT_STREQ("OK", result.c_str());

  ExecuteJavascriptAndWaitForOk("hangup();");
  EXPECT_TRUE(ExecuteScriptAndExtractString(host2, "hangup();", &result));
  ASSERT_STREQ("OK", result.c_str());
