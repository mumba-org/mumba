...
#include "content/host/browser/host.h"
...
class MAYBE_WebRtcInternalsBrowserTest: public ContentBrowserTest {
 ...
 protected:
  bool ExecuteJavascript(const string& javascript) {
    return ExecuteScript(host(), javascript);
  }

  void ExpectTitle(const std::string& expected_title) const {
    base::string16 expected_title16(base::ASCIIToUTF16(expected_title));
    TitleWatcher title_watcher(host()->web_contents(), expected_title16);
  ...
  void VerifyUserMediaRequest(
    ...
    ASSERT_TRUE(
        ExecuteScriptAndExtractString(host(),
    ...
    ASSERT_TRUE(
        ExecuteScriptAndExtractBool(host(),
                                    "window.domAutomationController.send("
  ...
  void VerifyPeerConnectionEntry(const PeerConnectionEntry& pc) {
    ...
    for (size_t i = 0; i < pc.events_.size(); ++i) {
      ...
      ASSERT_TRUE(ExecuteScriptAndExtractString(host(), ss.str(), &result));
  ...
  void VerifyGraphDataPoint(const string& pc_id, const string& graph_id,
    ...
    string actual_value;
    ASSERT_TRUE(
        ExecuteScriptAndExtractString(host(), ss.str(), &actual_value));
  ...
  string GetSsrcInfo(const string& ssrc_id) {
    ...
    EXPECT_TRUE(ExecuteScriptAndExtractString(
        ...
        "window.domAutomationController.send(JSON.stringify("
           "ssrcInfoManager.streamInfoDomain_['" + ssrc_id + "']))",
        &result));
    return result;
  }

  int GetSsrcInfoBlockCount(Host* host) {
    int count = 0;
...
IN_PROC_BROWSER_TEST_F(MAYBE_WebRtcInternalsBrowserTest, UpdatePeerConnection) {
  ...
  EXPECT_GT(GetSsrcInfoBlockCount(host()), 0);
}
...
IN_PROC_BROWSER_TEST_F(MAYBE_WebRtcInternalsBrowserTest,
  ...
  Host* host2 = CreateBrowser();
  NavigateToURL(host2, url2);
  ...
  // Wait until the stats table shells are created.
  count = 0;
  ...
  ASSERT_TRUE(ExecuteScriptAndExtractBool(
      host2,
      "var tableDomains = $('peer-connections-list')"
      "    .getElementsByClassName('stats-table-shell');"
      "var result = true;"
      "for (var i = 0; i < tableDomains.length && result; ++i) {"
      "  var tables = tableDomains[i].getElementsByTagName('table');"
      "  for (var j = 0; j < tables.length && result; ++j) {"
      "    result = (tables[j].rows.length > 1);"
      "  }"
      "  if (!result) {"
      "    console.log(tableDomains[i].innerHTML);"
      "  }"
      "}"
  ...
  count = GetSsrcInfoBlockCount(host2);
  EXPECT_GT(count, 0);
}
