...
#include "core/host/service_manager/common_host_interfaces.h"
...
#include "core/host/discardable_shared_memory_manager.h"
...
void StartServiceInUtilityProcess(
  ...
  common::BindInterface(process_host, mojo::MakeRequest(&service_factory));
...
class BuiltinManifestProvider : public catalog::ManifestProvider {
 public:
  ...
  void AddServiceManifest(base::StringPiece name, int resource_id) {
    ...
    std::unique_ptr<base::Value> overlay_value =  std::unique_ptr<base::Value>();
    //    common::GetClient()->host()->GetServiceManifestOverlay(name);
...
void GetGeolocationRequestContextFromContentClient(
    base::OnceCallback<void(scoped_refptr<net::URLRequestContextGetter>)>
        callback) {
  //GetContentClient()->host()->GetGeolocationRequestContext(
  //    std::move(callback));
}
...
void RegisterUIServiceInProcessIfNecessary(
    common::ServiceManagerConnection* connection) {
  // Some tests don't create HostMainLoop.
  if (!HostMainLoop::GetInstance())
    return;
  ...
  info.factory = base::Bind(
      ...
      HostMainLoop::GetInstance()->image_cursors_set()->GetWeakPtr(),
...
OutOfProcessServiceInfo::OutOfProcessServiceInfo() =
    default;
 ...
 private:
  ...
  void StartOnIOThread(
    ...
    service_manager_->RegisterService(
        service_manager::Identity(common::mojom::kPackagedServicesServiceName,
  ...
  static void OnInstanceQuit(const service_manager::Identity& id) {
    //if (GetContentClient()->host()->ShouldTerminateOnServiceQuit(id)) {
      // Don't LOG(FATAL) because we don't want a host crash report.
    //  LOG(ERROR) << "Terminating because service '" << id.name()
   //              << "' quit unexpectedly.";
      // Skip shutdown to reduce the risk that other code in the host will
...
ServiceManagerContext::ServiceManagerContext() {
  ...
  if (service_manager::ServiceManagerIsRemote()) {
    ...
    static const struct ManifestInfo {
      ...
      int resource_id;
        ...
        {common::mojom::kHostServiceName, IDR_MOJO_HOST_MANIFEST},
        {common::mojom::kGpuServiceName, IDR_MOJO_GPU_MANIFEST},
        {common::mojom::kDomainServiceName, IDR_MOJO_DOMAIN_MANIFEST},
        {common::mojom::kApplicationServiceName, IDR_MOJO_APPLICATION_MANIFEST},
        {common::mojom::kPackagedServicesServiceName,
         IDR_MOJO_PACKAGED_SERVICES_MANIFEST},
       // {mojom::kPluginServiceName, IDR_MOJO_CONTENT_PLUGIN_MANIFEST},
      //  {mojom::kRendererServiceName, IDR_MOJO_CONTENT_RENDERER_MANIFEST},
        {common::mojom::kUtilityServiceName, IDR_MOJO_UTILITY_MANIFEST},
        {catalog::mojom::kServiceName, IDR_MOJO_CATALOG_MANIFEST},
        // automation
        // {"automation.Accessibility", IDR_AUTOMATION_ACCESSIBILITY_MANIFEST},
        // {"automation.AnimationInterface", IDR_AUTOMATION_ANIMATION_MANIFEST},
        // {"automation.ApplicationCache", IDR_AUTOMATION_APPLICATION_CACHE_MANIFEST},
        // {"automation.CacheStorage", IDR_AUTOMATION_CACHE_STORAGE_MANIFEST},
        // {"automation.CSS", IDR_AUTOMATION_CSS_MANIFEST},
        // {"automation.DatabaseInterface", IDR_AUTOMATION_DATABASE_MANIFEST},
        // {"automation.DeviceOrientationInterface", IDR_AUTOMATION_DEVICE_ORIENTATION_MANIFEST},
        // {"automation.DOM", IDR_AUTOMATION_DOM_MANIFEST},
        // {"automation.DOMSnapshot", IDR_AUTOMATION_DOM_SNAPSHOT_MANIFEST},
        // {"automation.DOMStorage", IDR_AUTOMATION_DOM_STORAGE_MANIFEST},
        // {"automation.Emulation", IDR_AUTOMATION_EMULATION_MANIFEST},
        // {"automation.Headless", IDR_AUTOMATION_HEADLESS_MANIFEST},
        // {"automation.Host", IDR_AUTOMATION_HOST_MANIFEST},
        // {"automation.IndexedDB", IDR_AUTOMATION_INDEXED_DB_MANIFEST},
        // {"automation.Input", IDR_AUTOMATION_INPUT_MANIFEST},
        // {"automation.IO", IDR_AUTOMATION_IO_MANIFEST},
        // {"automation.LayerTree", IDR_AUTOMATION_LAYER_TREE_MANIFEST},
        // {"automation.Network", IDR_AUTOMATION_NETWORK_MANIFEST},
        // {"automation.Overlay", IDR_AUTOMATION_OVERLAY_MANIFEST},
        // {"automation.Page", IDR_AUTOMATION_PAGE_MANIFEST},
        // {"automation.ServiceWorker", IDR_AUTOMATION_SERVICE_WORKER_MANIFEST},
        // {"automation.Storage", IDR_AUTOMATION_STORAGE_MANIFEST},
        // {"automation.SystemInfo", IDR_AUTOMATION_SYSTEM_INFO_MANIFEST},
        // {"automation.Target", IDR_AUTOMATION_TARGET_MANIFEST},
        // {"automation.Tethering", IDR_AUTOMATION_TETHERING_MANIFEST},
    };
    ...
    //      GetContentClient()->host()->GetExtraServiceManifests()) {
  ...
  packaged_services_connection_ = common::ServiceManagerConnection::Create(
      std::move(packaged_services_request),
      HostThread::GetTaskRunnerForThread(HostThread::IO));

  service_manager::mojom::ServicePtr root_host_service;
  common::ServiceManagerConnection::SetForProcess(common::ServiceManagerConnection::Create(
      mojo::MakeRequest(&root_host_service),
      HostThread::GetTaskRunnerForThread(HostThread::IO)));
  auto* host_connection = common::ServiceManagerConnection::GetForProcess();

  service_manager::mojom::PIDReceiverPtr pid_receiver;
  packaged_services_connection_->GetConnector()->StartService(
      service_manager::Identity(common::mojom::kHostServiceName,
                                service_manager::mojom::kRootUserID),
      std::move(root_host_service), mojo::MakeRequest(&pid_receiver));
  ...
  // See the comments on wake_lock_context_host.h, content_host_client.h and
  ...
  device_info.factory = base::Bind(
      ...
      std::string(),//GetContentClient()->host()->GetGeolocationApiKey(),
      base::Bind(&WakeLockContextHost::GetNativeViewForContext),
      base::Bind(&OverrideSystemLocationProvider),
  ...
  //ContentHostClient::StaticServiceMap services;
  //GetContentClient()->host()->RegisterInProcessServices(&services);
  ...
  g_io_thread_connector.Get() = host_connection->GetConnector()->Clone();

  //HostClient::OutOfProcessServiceMap 
   std::map<std::string, OutOfProcessServiceInfo> out_of_process_services;
 // common::GetClient()->host()->RegisterOutOfProcessServices(
 //     &out_of_process_services);

  out_of_process_services[data_decoder::mojom::kServiceName] =
      OutOfProcessServiceInfo(base::ASCIIToUTF16("Data Decoder Service"));

  bool network_service_enabled =
      base::FeatureList::IsEnabled(network::features::kNetworkService);
  bool network_service_in_process = true;
  ...
  if (network_service_enabled) {
    if (network_service_in_process) {
      ...
      network_service_info.task_runner =
          HostThread::GetTaskRunnerForThread(HostThread::IO);
      packaged_services_connection_->AddEmbeddedService(
          common::mojom::kNetworkServiceName, network_service_info);
    } else {
      out_of_process_services[common::mojom::kNetworkServiceName] =
          OutOfProcessServiceInfo(base::ASCIIToUTF16("Network Service"));
    }
  } else {
  ...
  if (features::IsVideoCaptureServiceEnabledForOutOfProcess()) {
    out_of_process_services[video_capture::mojom::kServiceName] =
        OutOfProcessServiceInfo(base::ASCIIToUTF16("Video Capture Service"));
  }

#if BUILDFLAG(ENABLE_MOJO_MEDIA_IN_UTILITY_PROCESS)
  out_of_process_services[media::mojom::kMediaServiceName] =
      OutOfProcessServiceInfo(base::ASCIIToUTF16("Media Service"));
#endif

#if BUILDFLAG(ENABLE_LIBRARY_CDMS)
  out_of_process_services[media::mojom::kCdmServiceName] =
      OutOfProcessServiceInfo(base::ASCIIToUTF16("Content Decryption Module Service"));
#endif

  if (ShouldEnableVizService()) {
    out_of_process_services[viz::mojom::kVizServiceName] =
        OutOfProcessServiceInfo(base::ASCIIToUTF16("Visuals Service"));
  }
  ...
  RegisterCommonHostInterfaces(host_connection);
  host_connection->Start();
  ...
  if (network_service_enabled && !network_service_in_process) {
    ...
    host_connection->GetConnector()->StartService(
        common::mojom::kNetworkServiceName);
  }
}

ServiceManagerContext::~ServiceManagerContext() {
  // NOTE: The in-process ServiceManager MUST be destroyed before the host
  // process-wide ServiceManagerConnection. Otherwise it's possible for the
  // ServiceManager to receive connection requests for service:content_host
  // which it may attempt to service by launching a new instance of the host.
  if (in_process_context_)
    in_process_context_->ShutDown();
  if (common::ServiceManagerConnection::GetForProcess())
    common::ServiceManagerConnection::DestroyForProcess();
