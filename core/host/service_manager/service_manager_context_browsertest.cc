...
#include "content/host/host/host_content_host_client.h"
...
using ServiceManagerContextHostTest = ContentHostTest;

// "MANUAL" tests only run when kRunManualTestsFlag is set.
IN_PROC_BROWSER_TEST_F(ServiceManagerContextHostTest,
                       MANUAL_TerminateOnServiceQuit) {
  HostContentHostClient::Get()
  ...
  base::RunLoop loop;
  // Terminate the service. Host should exit in response with an error.
  echo_ptr->Quit();
  loop.Run();
}
...
TEST(ServiceManagerContextTest, MAYBE_TerminateOnServiceQuit) {
  ...
  new_test.AppendSwitchASCII(
      ...
      "ServiceManagerContextHostTest.MANUAL_TerminateOnServiceQuit");
