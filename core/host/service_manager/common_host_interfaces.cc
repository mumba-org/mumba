...
#if defined(OS_WIN)
//#include "core/host/application/dwrite_font_proxy_message_filter_win.h"
#include "core/shared/common/font_cache_dispatcher_win.h"
#elif defined(OS_MACOSX)
#include "core/shared/common/font_loader_dispatcher_mac.h"
#endif
...
class ConnectionFilterImpl : public common::ConnectionFilter {
 public:
  ConnectionFilterImpl() {
#if defined(OS_WIN)
    registry_.AddInterface(base::BindRepeating(&common::FontCacheDispatcher::Create));
   // registry_.AddInterface(
   //     base::BindRepeating(&DWriteFontProxyImpl::Create),
   //     base::CreateSequencedTaskRunnerWithTraits(
   //         {base::TaskPriority::USER_BLOCKING, base::MayBlock()}));
#elif defined(OS_MACOSX)
    registry_.AddInterface(base::BindRepeating(&common::FontLoaderDispatcher::Create));
#endif
    if (!features::IsMusEnabled()) {
      ...
      auto* host_main_loop = HostMainLoop::GetInstance();
      if (host_main_loop) {
        auto* manager = host_main_loop->discardable_shared_memory_manager();
        if (manager) {
