...
class CONTENT_EXPORT AppCacheUpdateJob
 ...
 private:
  ...
  void HandleCacheFailure(const common::AppCacheErrorDetails& details,
                          ResultType result,
  ...
  void NotifySingleHost(AppCacheHost* host, common::AppCacheEventID event_id);
  void NotifyAllAssociatedHosts(common::AppCacheEventID event_id);
  void NotifyAllProgress(const GURL& url);
  void NotifyAllFinalProgress();
  void NotifyAllError(const common::AppCacheErrorDetails& detals);
  ...
  void CancelAllMasterEntryFetches(const common::AppCacheErrorDetails& details);
