...
#include "core/host/appcache/appcache_database.h"
#include "core/host/appcache/appcache_entry.h"
#include "core/host/appcache/appcache_group.h"
#include "core/host/appcache/appcache_histograms.h"
#include "core/host/appcache/appcache_quota_client.h"
 ...
 public:
  explicit InitTask(AppCacheStorageImpl* storage)
    ...
    if (!storage->is_incognito_) {
      db_file_path_ =
          storage->cache_directory_.Append(common::kAppCacheDatabaseName);
...
void AppCacheStorageImpl::GetAllInfoTask::Run() {
  ...
  for (const url::Origin& origin : origins) {
    common::AppCacheInfoVector& infos = info_collection_->infos_by_origin[origin];
    std::vector<AppCacheDatabase::GroupRecord> groups;
...
class NetworkNamespaceHelper {
 public:
  ...
  bool IsInNetworkNamespace(const GURL& url, int64_t cache_id) {
    std::pair<WhiteListMap::iterator, bool> result = namespaces_map_.insert(
        WhiteListMap::value_type(cache_id, common::AppCacheNamespaceVector()));
    if (result.second)
 ...
 private:
  void GetOnlineWhiteListForCache(int64_t cache_id,
                                  common::AppCacheNamespaceVector* namespaces) {
    ...
    for (const auto& record : records) {
      namespaces->push_back(common::AppCacheNamespace(common::APPCACHE_NETWORK_NAMESPACE,
  ...
  // Key is cache id
  using WhiteListMap = std::map<int64_t, common::AppCacheNamespaceVector>;
 ...
 public:
  FindMainResponseTask(AppCacheStorageImpl* storage,
      ...
      : DatabaseTask(storage), url_(url),
        ...
        cache_id_(common::kAppCacheNoCacheId), group_id_(0) {
    if (groups_in_use) {
...
void AppCacheStorageImpl::FindMainResponseTask::Run() {
  ...
  int64_t preferred_cache_id = common::kAppCacheNoCacheId;
  ...
  if (FindExactMatch(preferred_cache_id) ||
    ...
    // We found something.
    DCHECK(cache_id_ != common::kAppCacheNoCacheId && !manifest_url_.is_empty() &&
           group_id_ != 0);
    return;
  }
...
FindMainResponseTask::FindFirstValidNamespace(
  ...
  for (auto* namespace_record : namespaces) {
    ...
    if (database_->FindEntry(namespace_record->cache_id,
      ...
      if (namespace_record->namespace_.type == common::APPCACHE_FALLBACK_NAMESPACE)
...
void AppCacheStorageImpl::Initialize(
  ...
  if (!is_incognito_)
    db_file_path = cache_directory_.Append(common::kAppCacheDatabaseName);
...
void AppCacheStorageImpl::DeliverShortCircuitedFindMainResponse(
  ...
  if (delegate_ref->delegate) {
    ...
    CallOnMainResponseFound(
        ...
        GURL(), AppCacheEntry(),
        cache.get() ? cache->cache_id() : common::kAppCacheNoCacheId,
        group.get() ? group->group_id() : common::kAppCacheNoCacheId,
