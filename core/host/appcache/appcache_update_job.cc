...
#include "core/host/appcache/appcache_histograms.h"
#include "core/host/appcache/appcache_update_request_base.h"
#include "core/host/appcache/appcache_update_url_fetcher.h"
...
bool IsEvictableError(AppCacheUpdateJob::ResultType result,
  ...
  switch (result) {
    ...
    case AppCacheUpdateJob::MANIFEST_ERROR:
      return details.reason == common::AppCacheErrorReason::APPCACHE_SIGNATURE_ERROR;
...
class HostNotifier {
 public:
  ...
  using NotifyHostMap = std::map<common::AppCacheFrontend*, HostIds>;
  ...
  void SendNotifications(common::AppCacheEventID event_id) {
    for (auto& pair : hosts_to_notify_) {
      common::AppCacheFrontend* frontend = pair.first;
  ...
  void SendErrorNotifications(const common::AppCacheErrorDetails& details) {
  ...
  void SendLogMessage(const std::string& message) {
    for (const auto& pair : hosts_to_notify_) {
      ...
      for (const auto& id : pair.second)
        frontend->OnLogMessage(id, common::APPCACHE_LOG_WARNING, message);
    }
  }
...
void AppCacheUpdateJob::StartUpdate(AppCacheHost* host,
  ...
  if (update_status == AppCacheGroup::CHECKING ||
    ...
    if (host) {
      NotifySingleHost(host, common::AppCacheEventID::APPCACHE_CHECKING_EVENT);
      if (update_status == AppCacheGroup::DOWNLOADING)
        NotifySingleHost(host, common::AppCacheEventID::APPCACHE_DOWNLOADING_EVENT);
  ...
  if (group_->HasCache()) {
    ...
    NotifyAllAssociatedHosts(common::AppCacheEventID::APPCACHE_CHECKING_EVENT);
  } else {
...
void AppCacheUpdateJob::HandleCacheFailure(
    const common::AppCacheErrorDetails& error_details,
    ResultType result,
...
void AppCacheUpdateJob::HandleManifestFetchCompleted(URLFetcher* fetcher,
  ...
  if (is_valid_response_code) {
    ...
    HandleCacheFailure(
        common::AppCacheErrorDetails(
            message, common::AppCacheErrorReason::APPCACHE_MANIFEST_ERROR,
...
void AppCacheUpdateJob::OnGroupMadeObsolete(AppCacheGroup* group,
  ...
  CancelAllMasterEntryFetches(
      common::AppCacheErrorDetails("The cache has been made obsolete, "
                           "the manifest file returned 404 or 410",
                           common::AppCacheErrorReason::APPCACHE_MANIFEST_ERROR, GURL(),
                           response_code, false /*is_cross_origin*/));
  if (success) {
    DCHECK(group->is_obsolete());
    NotifyAllAssociatedHosts(common::AppCacheEventID::APPCACHE_OBSOLETE_EVENT);
    internal_state_ = COMPLETED;
    MaybeCompleteUpdate();
  } else {
    // Treat failure to mark group obsolete as a cache failure.
    HandleCacheFailure(
        common::AppCacheErrorDetails("Failed to mark the cache as obsolete",
...
void AppCacheUpdateJob::ContinueHandleManifestFetchCompleted(bool changed) {
  ...
  if (!ParseManifest(manifest_url_, manifest_data_.data(),
    ...
    HandleCacheFailure(
        common::AppCacheErrorDetails(message,
  ...
  NotifyAllAssociatedHosts(common::AppCacheEventID::APPCACHE_DOWNLOADING_EVENT);
  FetchUrls();
  FetchMasterEntries();
      ...
      if (response_code == 304 && fetcher->existing_entry().has_response_id()) {
        ...
        switch (result) {
          case DISKCACHE_ERROR:
            HandleCacheFailure(
                common::AppCacheErrorDetails(
                    message, common::AppCacheErrorReason::APPCACHE_UNKNOWN_ERROR,
          ...
          case NETWORK_ERROR:
            HandleCacheFailure(
                common::AppCacheErrorDetails(
                    message, common::AppCacheErrorReason::APPCACHE_RESOURCE_ERROR, url,
                    0, is_cross_origin),
                result, url);
            break;
          default:
...
void AppCacheUpdateJob::HandleMasterEntryFetchCompleted(URLFetcher* fetcher,
  ...
  if (response_code / 100 == 2) {
    ...
    host_notifier.SendErrorNotifications(common::AppCacheErrorDetails(
        message, common::AppCacheErrorReason::APPCACHE_MANIFEST_ERROR,
...
void AppCacheUpdateJob::HandleManifestRefetchCompleted(URLFetcher* fetcher,
  ...
  if (response_code == 304 || manifest_data_ == fetcher->manifest_data()) {
    ...
    if (response_code == 200) {
      HandleCacheFailure(
          common::AppCacheErrorDetails("Manifest changed during update",
      ...
      HandleCacheFailure(
          common::AppCacheErrorDetails(
              message, common::AppCacheErrorReason::APPCACHE_MANIFEST_ERROR, GURL(),
              response_code, false /*is_cross_origin*/),
          fetcher->result(), GURL());
    }
  }
...
void AppCacheUpdateJob::OnManifestInfoWriteComplete(int result) {
  if (result > 0) {
    ...
    HandleCacheFailure(
        common::AppCacheErrorDetails("Failed to write the manifest headers to storage",
                             common::AppCacheErrorReason::APPCACHE_UNKNOWN_ERROR,
                             GURL(), 0, false /*is_cross_origin*/),
...
void AppCacheUpdateJob::OnManifestDataWriteComplete(int result) {
  if (result > 0) {
    ...
    HandleCacheFailure(
        common::AppCacheErrorDetails("Failed to write the manifest data to storage",
                             common::AppCacheErrorReason::APPCACHE_UNKNOWN_ERROR,
                             GURL(), 0, false /*is_cross_origin*/),
        DISKCACHE_ERROR, GURL());
  }
...
void AppCacheUpdateJob::OnGroupAndNewestCacheStored(AppCacheGroup* group,
  ...
  common::AppCacheErrorReason reason = common::AppCacheErrorReason::APPCACHE_UNKNOWN_ERROR;
  std::string message("Failed to commit new cache to storage");
  if (would_exceed_quota) {
    message.append(", would exceed quota");
    result = QUOTA_ERROR;
    reason = common::AppCacheErrorReason::APPCACHE_QUOTA_ERROR;
  }
  HandleCacheFailure(
      common::AppCacheErrorDetails(message, reason, GURL(), 0,
...
void AppCacheUpdateJob::NotifyAllAssociatedHosts(common::AppCacheEventID event_id) {
  HostNotifier host_notifier;
...
void AppCacheUpdateJob::NotifyAllError(const common::AppCacheErrorDetails& details) {
  HostNotifier host_notifier;
  AddAllAssociatedHostsToNotifier(&host_notifier);
...
void AppCacheUpdateJob::CheckIfManifestChanged() {
  ...
  if (!entry) {
    ...
    if (service_->storage() == storage_) {
      ...
      HandleCacheFailure(
          common::AppCacheErrorDetails("Manifest entry not found in existing cache",
                               common::AppCacheErrorReason::APPCACHE_UNKNOWN_ERROR,
                               GURL(), 0, false /*is_cross_origin*/),
          DB_ERROR, GURL());
...
void AppCacheUpdateJob::MaybeCompleteUpdate() {
  ...
  switch (internal_state_) {
    case NO_UPDATE:
      ...
      group_->SetUpdateAppCacheStatus(AppCacheGroup::IDLE);
      // 6.9.4 steps 7.3-7.7.
      NotifyAllAssociatedHosts(common::AppCacheEventID::APPCACHE_NO_UPDATE_EVENT);
    ...
    case REFETCH_MANIFEST:
      ...
      group_->SetUpdateAppCacheStatus(AppCacheGroup::IDLE);
      if (update_type_ == CACHE_ATTEMPT)
        NotifyAllAssociatedHosts(common::AppCacheEventID::APPCACHE_CACHED_EVENT);
      else
        NotifyAllAssociatedHosts(common::AppCacheEventID::APPCACHE_UPDATE_READY_EVENT);
      DiscardDuplicateResponses();
      internal_state_ = COMPLETED;
