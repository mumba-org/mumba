...
#include "core/host/appcache/appcache_internals_ui.h"
...
std::unique_ptr<base::DictionaryValue> GetDictionaryValueForAppCacheInfo(
    const content::common::AppCacheInfo& appcache_info) {
  std::unique_ptr<base::DictionaryValue> dict_value(
      new base::DictionaryValue());
...
std::unique_ptr<base::ListValue> GetListValueForAppCacheInfoVector(
  ...
  for (const common::AppCacheInfo& info : appcache_info_vector)
