...
#include "core/host/appcache/appcache_dispatcher_host.h"
...
void AppCacheDispatcherHost::SelectCache(int32_t host_id,
  ...
  if (appcache_service_) {
    ...
    frontend_proxy_.OnCacheSelected(host_id, std::move(common::AppCacheInfo()));
  }
}
...
void AppCacheDispatcherHost::GetResourceList(int32_t host_id,
  ...
  std::vector<common::AppCacheResourceInfo> params;
  std::vector<common::mojom::AppCacheResourceInfoPtr> out;
  if (appcache_service_) {
...
void AppCacheDispatcherHost::GetStatus(int32_t host_id,
  ...
  std::move(callback).Run(common::AppCacheStatus::APPCACHE_STATUS_UNCACHED);
}
