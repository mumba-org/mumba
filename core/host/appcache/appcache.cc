...
GURL AppCache::GetNamespaceEntryUrl(const common::AppCacheNamespaceVector& namespaces,
...
bool SortNamespacesByLength(
    const common::AppCacheNamespace& lhs, const common::AppCacheNamespace& rhs) {
...
void AppCache::InitializeWithDatabaseRecords(
  ...
  for (size_t i = 0; i < whitelists.size(); ++i) {
    ...
    online_whitelist_namespaces_.push_back(
        common::AppCacheNamespace(common::APPCACHE_NETWORK_NAMESPACE,
...
bool AppCache::FindResponseForRequest(const GURL& url,
  ...
  *found_network_namespace = IsInNetworkNamespace(url_no_ref);
  ...
  const common::AppCacheNamespace* intercept_namespace =
  ...
  const common::AppCacheNamespace* fallback_namespace =
...
void AppCache::ToResourceInfoVector(common::AppCacheResourceInfoVector* infos) const {
  DCHECK(infos && infos->empty());
  for (const auto& pair : entries_) {
    infos->push_back(common::AppCacheResourceInfo());
    common::AppCacheResourceInfo& info = infos->back();
    info.url = pair.first;
...
// static
const common::AppCacheNamespace* AppCache::FindNamespace(
