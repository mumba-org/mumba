...
#include "core/host/appcache/appcache_url_loader_job.h"
#include "core/host/appcache/appcache_url_loader_request.h"
...
AppCacheRequestHandler::AppCacheRequestHandler(
    ...
    : host_(host),
      ...
      old_process_id_(0),
      old_host_id_(common::kAppCacheNoHostId),
      cache_id_(common::kAppCacheNoCacheId),
...
AppCacheJob* AppCacheRequestHandler::MaybeLoadResource(
  ...
  found_cache_id_ = common::kAppCacheNoCacheId;
...
void AppCacheRequestHandler::DeliverErrorResponse() {
  ...
  DCHECK_EQ(common::kAppCacheNoCacheId, cache_id_);
...
std::unique_ptr<AppCacheJob> AppCacheRequestHandler::MaybeLoadMainResource(
  ...
  const AppCacheHost* spawning_host =
      (resource_type_ == common::RESOURCE_TYPE_SHARED_WORKER) ?
...
void AppCacheRequestHandler::OnMainResponseFound(
  ...
  if (was_blocked_by_policy) {
    if (IsResourceTypeFrame(resource_type_)) {
      ...
      DCHECK_EQ(resource_type_, common::RESOURCE_TYPE_SHARED_WORKER);
  ...
  if (IsMainResourceType(resource_type_) && cache_id != common::kAppCacheNoCacheId) {
...
void AppCacheRequestHandler::RunLoaderCallbackForMainResource(
    ...
    common::SingleRequestURLLoaderFactory::RequestHandler handler) {
