...
void AppCacheDatabase::ReadNamespaceRecords(
  ...
  while (statement->Step()) {
    common::AppCacheNamespaceType type = static_cast<common::AppCacheNamespaceType>(
        statement->ColumnInt(2));
    NamespaceRecordVector* records =
        (type == common::APPCACHE_FALLBACK_NAMESPACE) ? fallbacks : intercepts;
...
void AppCacheDatabase::ReadNamespaceRecord(
  ...
  record->namespace_.type =
      static_cast<common::AppCacheNamespaceType>(statement->ColumnInt(2));
  ...
  DCHECK(record->namespace_.type == common::APPCACHE_FALLBACK_NAMESPACE ||
         record->namespace_.type == common::APPCACHE_INTERCEPT_NAMESPACE);
...
bool AppCacheDatabase::UpgradeSchema() {
  ...
  if (meta_table_->GetVersionNumber() == 3) {
    ...
    DCHECK_EQ(0, static_cast<int>(common::APPCACHE_FALLBACK_NAMESPACE));
    if (!db_->Execute(
            "INSERT INTO Namespaces"
