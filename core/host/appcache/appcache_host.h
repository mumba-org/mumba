...
#include "core/host/appcache/appcache_storage.h"
...
using GetStatusCallback = base::OnceCallback<void(common::AppCacheStatus)>;
...
class CONTENT_EXPORT AppCacheHost
 ...
 public:
  ...
  AppCacheHost(int host_id, common::AppCacheFrontend* frontend,
  ...
  void GetResourceList(std::vector<common::AppCacheResourceInfo>* resource_infos);
  ...
  common::AppCacheFrontend* frontend() const { return frontend_; }

  // PlzNavigate:
  // The AppCacheHost instance is created with a dummy common::AppCacheFrontend
  // pointer when the navigation starts. We need to switch it to the
  // actual frontend when the navigation commits.
  void set_frontend(common::AppCacheFrontend* frontend) { frontend_ = frontend; }
  ...
  bool is_selection_pending() const {
    return pending_selected_cache_id_ != common::kAppCacheNoCacheId ||
  ...
  void CompleteTransfer(int host_id, common::AppCacheFrontend* frontend);
 ...
 private:
  ...
  common::AppCacheStatus GetStatus();
  ...
  bool is_for_dedicated_worker() const {
    return parent_host_id_ != common::kAppCacheNoHostId;
  }
  ...
  common::AppCacheFrontend* frontend_;
