...
#include "core/host/appcache/appcache_subresource_url_factory.h"
...
void FillCacheInfo(const AppCache* cache,
                   ...
                   common::AppCacheStatus status, common::AppCacheInfo* info) {
...
AppCacheHost::AppCacheHost(int host_id,
                           common::AppCacheFrontend* frontend,
                           AppCacheServiceImpl* service)
    : host_id_(host_id),
      spawning_host_id_(common::kAppCacheNoHostId),
      spawning_process_id_(0),
      parent_host_id_(common::kAppCacheNoHostId),
      parent_process_id_(0),
      pending_main_resource_cache_id_(common::kAppCacheNoCacheId),
      pending_selected_cache_id_(common::kAppCacheNoCacheId),
...
bool AppCacheHost::SelectCache(const GURL& document_url,
  ...
  if (cache_document_was_loaded_from != common::kAppCacheNoCacheId) {
  ...
  if (!manifest_url.is_empty() &&
    ...
    if (policy &&
      ...
      frontend_->OnErrorEventRaised(
          host_ids, common::AppCacheErrorDetails(
                        "Cache creation was blocked by the content policy",
                        common::AppCacheErrorReason::APPCACHE_POLICY_ERROR, GURL(), 0,
...
bool AppCacheHost::SelectCacheForSharedWorker(int64_t appcache_id) {
  ...
  was_select_cache_called_ = true;
  if (appcache_id != common::kAppCacheNoCacheId) {
...
bool AppCacheHost::MarkAsForeignEntry(const GURL& document_url,
  ...
  SelectCache(document_url, common::kAppCacheNoCacheId, GURL());
  return true;
}
...
common::AppCacheStatus AppCacheHost::GetStatus() {
  // 6.9.8 Application cache API
  AppCache* cache = associated_cache();
  if (!cache)
    return common::AppCacheStatus::APPCACHE_STATUS_UNCACHED;

  // A cache without an owning group represents the cache being constructed
  // during the application cache update process.
  if (!cache->owning_group())
    return common::AppCacheStatus::APPCACHE_STATUS_DOWNLOADING;

  if (cache->owning_group()->is_obsolete())
    return common::AppCacheStatus::APPCACHE_STATUS_OBSOLETE;
  if (cache->owning_group()->update_status() == AppCacheGroup::CHECKING)
    return common::AppCacheStatus::APPCACHE_STATUS_CHECKING;
  if (cache->owning_group()->update_status() == AppCacheGroup::DOWNLOADING)
    return common::AppCacheStatus::APPCACHE_STATUS_DOWNLOADING;
  if (swappable_cache_.get())
    return common::AppCacheStatus::APPCACHE_STATUS_UPDATE_READY;
  return common::AppCacheStatus::APPCACHE_STATUS_IDLE;
}
...
void AppCacheHost::FinishCacheSelection(
  ...
  if (cache) {
    ...
    frontend_->OnLogMessage(
        host_id_, common::APPCACHE_LOG_INFO,
        base::StringPrintf(
...
void AppCacheHost::PrepareForTransfer() {
  ...
  DCHECK(!group_being_updated_.get());
  host_id_ = common::kAppCacheNoHostId;
  frontend_ = nullptr;
}

void AppCacheHost::CompleteTransfer(int host_id, common::AppCacheFrontend* frontend) {
  host_id_ = host_id;
  frontend_ = frontend;
}
