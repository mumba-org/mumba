...
const char ServiceStopHandler::kFullname[] = "/mumba.Mumba/ServiceStop";

ServiceStopHandler::ServiceStopHandler(): 
  fullname_(ServiceStopHandler::kFullname) {
  Init();
}

ServiceStopHandler::~ServiceStopHandler() {}

base::StringPiece ServiceStopHandler::ns() const {
  auto offset = service_name_.find_first_of(".");
  return service_name_.substr(offset);
}

void ServiceStopHandler::HandleCall(std::vector<char> data, base::Callback<void(int)> cb) {
  std::string service_name(data.data(), data.size());
  HostThread::PostTask(HostThread::UI,
    FROM_HERE,
    base::Bind(&ServiceStopHandler::ProcessServiceStopOnUI, 
      base::Unretained(this),
      service_name,
      base::Passed(std::move(cb))));
}

void ServiceStopHandler::ProcessServiceStopOnUI(const std::string& service_name, base::Callback<void(int)> cb) {
  DLOG(INFO) << "stopping app service '" << service_name << "'";
  scoped_refptr<Workspace> workspace = Workspace::GetCurrent();
  workspace->ShutdownDomain(service_name, base::Bind(&ServiceStopHandler::OnServiceStop, base::Unretained(this), base::Passed(std::move(cb))));
}

void ServiceStopHandler::OnServiceStop(base::Callback<void(int)> cb, int code) {
  scoped_refptr<Workspace> workspace = Workspace::GetCurrent();
  ...
  if (!cb.is_null()) {
    HostThread::PostTask(HostThread::IO, FROM_HERE, base::BindOnce(cb, code));
  }
}

void ServiceStopHandler::Init() {
