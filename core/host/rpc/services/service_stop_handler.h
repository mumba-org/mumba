...
#ifndef MUMBA_HOST_RPC_SERVICE_STOP_HANDLER_H_
#define MUMBA_HOST_RPC_SERVICE_STOP_HANDLER_H_
...
class ServiceStopHandler : public MumbaServicesUnaryCallHandler {
public:
  
  static const char kFullname[];

  ServiceStopHandler();
  ~ServiceStopHandler() override;
...
private:
  ...
  void ProcessServiceStopOnUI(const std::string& service_name, base::Callback<void(int)> cb);
  void OnServiceStop(base::Callback<void(int)> cb, int code);
