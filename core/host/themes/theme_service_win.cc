...
bool ThemeServiceWin::ShouldUseNativeFrame() const {
  const bool use_native_frame_if_enabled = !HasCustomImage(IDR_THEME_FRAME);
...
SkColor ThemeServiceWin::GetDefaultColor(int id) const {
  if (DwmColorsAllowed()) {
  ...
  return ThemeService::GetDefaultColor(id);
}
