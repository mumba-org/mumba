...
//#include "core/host/themes/theme_syncable_service.h"
#include "core/host/ui/dock_finder.h"
//#include "core/common/buildflags.h"
//#include "core/common/chrome_constants.h"
...
//const char kThemePackFilename[] = "cached_theme.pak";
...
// void WritePackToDiskCallback(DockThemePack* pack,
//                              const base::FilePath& directory) {
//   base::FilePath path = directory.Append(kThemePackFilename);
...
// ThemeService::DockThemeProvider -----------------------------------------

ThemeService::DockThemeProvider::DockThemeProvider(
    const ThemeService& theme_service)
    : theme_service_(theme_service){}

ThemeService::DockThemeProvider::~DockThemeProvider() {}

gfx::ImageSkia* ThemeService::DockThemeProvider::GetImageSkiaNamed(
    int id) const {
  return theme_service_.GetImageSkiaNamed(id);
}

SkColor ThemeService::DockThemeProvider::GetColor(int id) const {
  return theme_service_.GetColor(id);
}

color_utils::HSL ThemeService::DockThemeProvider::GetTint(int id) const {
  return theme_service_.GetTint(id);
}

int ThemeService::DockThemeProvider::GetDisplayProperty(int id) const {
  return theme_service_.GetDisplayProperty(id);
}

bool ThemeService::DockThemeProvider::ShouldUseNativeFrame() const {
  return false;//theme_service_.ShouldUseNativeFrame();
}

bool ThemeService::DockThemeProvider::HasCustomImage(int id) const {
  return theme_service_.HasCustomImage(id);
}

base::RefCountedMemory* ThemeService::DockThemeProvider::GetRawData(
    int id,
...
void ThemeService::Init(scoped_refptr<Workspace> workspace) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
std::string ThemeService::GetThemeID() const {
  return ThemeService::kDefaultThemeID;//profile_->GetPrefs()->GetString(prefs::kCurrentThemeID);
}
...
// static
const ui::ThemeProvider& ThemeService::GetThemeProviderForWorkspace(scoped_refptr<Workspace> workspace) {
  ThemeService* service = ThemeServiceFactory::GetForWorkspace(workspace);
  return service->original_theme_provider_;
}
...
SkColor ThemeService::GetDefaultColor(int id) const {
  ...
  switch (id) {
    ...
    case ThemeProperties::COLOR_TOOLBAR_BUTTON_ICON_INACTIVE:
      ...
      return SkColorSetA(
          GetColor(ThemeProperties::COLOR_TOOLBAR_BUTTON_ICON),
          is_touch ? 0x6E : 0x33);
    ...
    case ThemeProperties::COLOR_TOOLBAR_TOP_SEPARATOR_INACTIVE: {
      ...
      const SkColor frame_color = GetColor(frame_id);
    ...
    case ThemeProperties::COLOR_TAB_THROBBER_WAITING: {
      ...
      color_utils::HSL hsl = GetTint(ThemeProperties::TINT_BUTTONS);
...
void ThemeService::NotifyThemeChanged() {
  ...
  service->Notify(NOTIFICATION_DOCK_THEME_CHANGED,
...
scoped_refptr<Workspace> ThemeService::workspace() const { 
  return workspace_; 
}
...
bool ThemeService::HasCustomImage(int id) const {
  return DockThemePack::IsPersistentImageID(id) && theme_supplier_ &&
  ...
  switch (id) {
    ...
    case ThemeProperties::NTP_LOGO_ALTERNATE: {
      ...
      return IsColorGrayscale(
          GetColor(ThemeProperties::COLOR_NTP_BACKGROUND)) ? 0 : 1;
    }
