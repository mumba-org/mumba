...
#include "core/host/themes/dock_theme_pack.h"
...
// static
// void DockThemePack::BuildFromExtension(
//     const extensions::Extension* extension,
//     scoped_refptr<DockThemePack> pack) {
//   DCHECK(extension);
...
//   // image if a caller of DockThemePack::GetImageNamed() requests an
//   // ImageSkiaRep for a scale factor not specified by the theme author.
//   // Callers of DockThemePack::GetImageNamed() to be able to retrieve
...
//   // The DockThemePack is now in a consistent state.
//   pack->is_valid_ = true;
// }

// static
scoped_refptr<DockThemePack> DockThemePack::BuildFromDataPack(
  ...
  scoped_refptr<DockThemePack> pack(new DockThemePack);
  ...
  if (!pack->data_pack_->GetStringPiece(kHeaderID, &pointer))
    return NULL;
  pack->header_ = reinterpret_cast<DockThemePackHeader*>(const_cast<char*>(
      pointer.data()));
  ...
  std::string theme_id(reinterpret_cast<char*>(pack->header_->theme_id), kIdSize);
  std::string truncated_id = expected_id.substr(0, kIdSize);
...
// static
bool DockThemePack::IsPersistentImageID(int id) {
  for (size_t i = 0; i < kPersistingImagesLength; ++i)
...
DockThemePack::DockThemePack() : CustomThemeSupplier(NATIVE_X11) {//EXTENSION) {
...
bool DockThemePack::WriteToDisk(const base::FilePath& path) const {
  // Add resources for each of the property arrays.
  RawDataForWriting resources;
  resources[kHeaderID] = base::StringPiece(
      reinterpret_cast<const char*>(header_), sizeof(DockThemePackHeader));
...
bool DockThemePack::GetTint(int id, color_utils::HSL* hsl) const {
  if (tints_) {
...
bool DockThemePack::GetColor(int id, SkColor* color) const {
  if (colors_) {
...
bool DockThemePack::GetDisplayProperty(int id, int* result) const {
...
gfx::Image DockThemePack::GetImageNamed(int idr_id) {
...
base::RefCountedMemory* DockThemePack::GetRawData(
    int idr_id,
...
bool DockThemePack::HasCustomImage(int idr_id) const {
  int prs_id = GetPersistentIDByIDR(idr_id);
  if (prs_id == -1)
    return false;
...
void DockThemePack::BuildHeader(const std::string& id) {
  header_ = new DockThemePackHeader;
...
void DockThemePack::BuildTintsFromJSON(
...
void DockThemePack::BuildColorsFromJSON(
...
void DockThemePack::ReadColorsFromJSON(
    const base::DictionaryValue* colors_value,
    std::map<int, SkColor>* temp_colors) {
...
void DockThemePack::GenerateMissingColors(
  ...
  if (it != colors->end()) {
    ...
    frame =
        ThemeProperties::GetDefaultColor(ThemeProperties::COLOR_FRAME);
  }
...
void DockThemePack::BuildDisplayPropertiesFromJSON(
...
void DockThemePack::ParseImageNamesFromJSON(
...
void DockThemePack::AddFileAtScaleToMap(const std::string& image_name,
...
void DockThemePack::BuildSourceImagesArray(const FilePathMap& file_paths) {
  std::vector<int> ids;
...
bool DockThemePack::LoadRawBitmapsTo(
...
void DockThemePack::CreateImages(ImageCache* images) const {
  CropImages(images);
  CreateFrameImages(images);
  CreateTabBackgroundImages(images);
}

void DockThemePack::CropImages(ImageCache* images) const {
...
void DockThemePack::CreateFrameImages(ImageCache* images) const {
...
void DockThemePack::CreateTabBackgroundImages(ImageCache* images) const {
...
void DockThemePack::RepackImages(const ImageCache& images,
...
void DockThemePack::MergeImageCaches(
...
void DockThemePack::AddRawImagesTo(const RawImages& images,
...
color_utils::HSL DockThemePack::GetTintInternal(int id) const {
  color_utils::HSL hsl;
  if (GetTint(id, &hsl))
    return hsl;
  ...
  return ThemeProperties::GetDefaultTint(original_id);
}

int DockThemePack::GetRawIDByPersistentID(
    int prs_id,
    ui::ScaleFactor scale_factor) const {
  if (prs_id < 0)
    return -1;
...
bool DockThemePack::GetScaleFactorFromManifestKey(
...
void DockThemePack::GenerateRawImageForAllSupportedScales(int prs_id) {
