...
// because once we've constructed a DockThemePack through the
// BuildFromExtension() interface, we WriteToDisk() on a thread other than the
// UI thread that consumes a DockThemePack. There is no locking; thread
// safety between the writing thread and the UI thread is ensured by having the
// data be immutable.
//
// DockThemePacks are always deleted on a sequence with I/O allowed because
...
class DockThemePack : public CustomThemeSupplier {
 public:
  ...
  static scoped_refptr<DockThemePack> BuildFromDataPack(
 ...
 private:
  friend class DockThemePackTest;
  ...
  ~DockThemePack() override;

  // Builds a header ready to write to disk.
  void BuildHeader(const std::string& id);
...
#pragma pack(push,1)
  ...
  // following three structs, DockThemePack will either allocate an array or
...
#pragma pack(pop)
  ...
  DISALLOW_COPY_AND_ASSIGN(DockThemePack);
};
