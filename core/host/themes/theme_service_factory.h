...
class ThemeServiceFactory {//: public BrowserContextKeyedServiceFactory {
 public:
  ...
  static ThemeService* GetForWorkspace(scoped_refptr<Workspace> workspace);
 ...
 private:
  ...
  ThemeService* GetServiceForWorkspace(scoped_refptr<Workspace> workspace) const;
  std::unique_ptr<ThemeService> BuildServiceForWorkspace(scoped_refptr<Workspace> workspace) const;
