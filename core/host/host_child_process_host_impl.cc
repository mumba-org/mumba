...
// shared/host cannot depend on host/
...
#include "core/host/host_child_process_host_delegate.h"
...
static base::LazyInstance<
    HostChildProcessHostImpl::HostChildProcessList>::DestructorAtExit
    g_child_process_list = LAZY_INSTANCE_INITIALIZER;

base::LazyInstance<base::ObserverList<HostChildProcessObserver>>::
    DestructorAtExit g_host_child_process_observers =
        LAZY_INSTANCE_INITIALIZER;

void NotifyProcessLaunchedAndConnected(const ChildProcessData& data) {
  for (auto& observer : g_host_child_process_observers.Get())
    observer.HostChildProcessLaunchedAndConnected(data);
}

void NotifyProcessHostConnected(const ChildProcessData& data) {
  for (auto& observer : g_host_child_process_observers.Get())
    observer.HostChildProcessHostConnected(data);
}

void NotifyProcessHostDisconnected(const ChildProcessData& data) {
  for (auto& observer : g_host_child_process_observers.Get())
    observer.HostChildProcessHostDisconnected(data);
}

void NotifyProcessCrashed(const ChildProcessData& data,
                          const ChildProcessTerminationInfo& info) {
  for (auto& observer : g_host_child_process_observers.Get())
    observer.HostChildProcessCrashed(data, info);
}

void NotifyProcessKilled(const ChildProcessData& data,
                         const ChildProcessTerminationInfo& info) {
  for (auto& observer : g_host_child_process_observers.Get())
    observer.HostChildProcessKilled(data, info);
}

}  // namespace

HostChildProcessHost* HostChildProcessHost::Create(
    common::ProcessType process_type,
    HostChildProcessHostDelegate* delegate) {
...
HostChildProcessHost* HostChildProcessHost::Create(
  ...
  return new HostChildProcessHostImpl(process_type, delegate, service_name);
}

HostChildProcessHost* HostChildProcessHost::FromID(int child_process_id) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  HostChildProcessHostImpl::HostChildProcessList* process_list =
      g_child_process_list.Pointer();
  for (HostChildProcessHostImpl* host : *process_list) {
...
#if defined(OS_MACOSX)
base::PortProvider* HostChildProcessHost::GetPortProvider() {
  return MachBroker::GetInstance();
}
#endif

// static
HostChildProcessHostImpl::HostChildProcessList*
    HostChildProcessHostImpl::GetIterator() {
  return g_child_process_list.Pointer();
}

// static
void HostChildProcessHostImpl::AddObserver(
    HostChildProcessObserver* observer) {
  DCHECK_CURRENTLY_ON(HostThread::UI);
  g_host_child_process_observers.Get().AddObserver(observer);
}

// static
void HostChildProcessHostImpl::RemoveObserver(
    HostChildProcessObserver* observer) {
  // TODO(phajdan.jr): Check thread after fixing http://crbug.com/167126.
  g_host_child_process_observers.Get().RemoveObserver(observer);
}

HostChildProcessHostImpl::HostChildProcessHostImpl(
    common::ProcessType process_type,
    HostChildProcessHostDelegate* delegate,
    const std::string& service_name)
    : data_(process_type),
      ...
      weak_factory_(this) {
  data_.id = common::ChildProcessHostImpl::GenerateChildProcessUniqueId();

  child_process_host_.reset(common::ChildProcessHost::Create(this));
  AddFilter(new TraceMessageFilter(data_.id));

  g_child_process_list.Get().push_back(this);
//  GetClient()->host()->HostChildProcessHostCreated(this);
  ...
  if (!service_name.empty()) {
    ...
    child_connection_.reset(
        new common::ChildConnection(child_identity, broker_client_invitation_.get(),
...
// static
void HostChildProcessHostImpl::TerminateAll() {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  // Make a copy since the HostChildProcessHost dtor mutates the original
  // list.
  HostChildProcessList copy = g_child_process_list.Get();
  for (HostChildProcessList::iterator it = copy.begin();
       it != copy.end(); ++it) {
...
// static
void HostChildProcessHostImpl::CopyFeatureAndFieldTrialFlags(
...
// static
void HostChildProcessHostImpl::CopyTraceStartupFlags(
    base::CommandLine* cmd_line) {
...
void HostChildProcessHostImpl::Launch(
    std::unique_ptr<common::SandboxedProcessLauncherDelegate> delegate,
    std::unique_ptr<base::CommandLine> cmd_line,
    bool terminate_on_shutdown) {
  DCHECK_CURRENTLY_ON(HostThread::IO);

  //GetClient()->host()->AppendExtraCommandLineSwitches(cmd_line.get(),
   //                                                             data_.id);
  ...
  cmd_line->CopySwitchesFrom(host_command_line, kForwardSwitches,
  ...
  child_process_.reset(new ChildProcessLauncher(
      ...
      base::Bind(&HostChildProcessHostImpl::OnMojoError,
...
const ChildProcessData& HostChildProcessHostImpl::GetData() const {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  return data_;
}

common::ChildProcessHost* HostChildProcessHostImpl::GetHost() const {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  return child_process_host_.get();
}

const base::Process& HostChildProcessHostImpl::GetProcess() const {
  DCHECK_CURRENTLY_ON(HostThread::IO);
...
HostChildProcessHostImpl::TakeMetricsAllocator() {
  return std::move(metrics_allocator_);
}

void HostChildProcessHostImpl::SetName(const base::string16& name) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  data_.name = name;
}

void HostChildProcessHostImpl::SetHandle(base::ProcessHandle handle) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  data_.handle = handle;
}

::service_manager::mojom::ServiceRequest
HostChildProcessHostImpl::TakeInProcessServiceRequest() {
  DCHECK(broker_client_invitation_);
...
void HostChildProcessHostImpl::ForceShutdown() {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  g_child_process_list.Get().remove(this);
  child_process_host_->ForceShutdown();
}

void HostChildProcessHostImpl::AddFilter(HostMessageFilter* filter) {
  child_process_host_->AddFilter(filter->GetFilter());
}

void HostChildProcessHostImpl::BindInterface(
...
void HostChildProcessHostImpl::HistogramBadMessageTerminated(
    common::ProcessType process_type) {
  UMA_HISTOGRAM_ENUMERATION("ChildProcess.BadMessgeTerminated", process_type,
                            common::PROCESS_TYPE_MAX);
}

ChildProcessTerminationInfo HostChildProcessHostImpl::GetTerminationInfo(
    bool known_dead) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  if (!child_process_) {
...
bool HostChildProcessHostImpl::OnMessageReceived(
    const IPC::Message& message) {
  return delegate_->OnMessageReceived(message);
}

void HostChildProcessHostImpl::OnChannelConnected(int32_t peer_pid) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
...
void HostChildProcessHostImpl::OnChannelError() {
  delegate_->OnChannelError();
}

void HostChildProcessHostImpl::OnBadMessageReceived(
    const IPC::Message& message) {
...
void HostChildProcessHostImpl::TerminateOnBadMessageReceived(
    const std::string& error) {
  HistogramBadMessageTerminated(static_cast<common::ProcessType>(data_.process_type));
  ...
  child_process_->Terminate(common::RESULT_CODE_KILLED_BAD_MESSAGE);
}

void HostChildProcessHostImpl::OnChannelInitialized(IPC::Channel* channel) {
  channel_ = channel;
}

void HostChildProcessHostImpl::OnChildDisconnected() {
  DCHECK_CURRENTLY_ON(HostThread::IO);
#if defined(OS_WIN)
    ...
    switch (info.status) {
      ...
      case base::TERMINATION_STATUS_ABNORMAL_TERMINATION: {
        ...
        UMA_HISTOGRAM_ENUMERATION("ChildProcess.Crashed2",
                                  static_cast<common::ProcessType>(data_.process_type),
                                  common::PROCESS_TYPE_MAX);
        break;
      }
...
bool HostChildProcessHostImpl::Send(IPC::Message* message) {
  return child_process_host_->Send(message);
}

void HostChildProcessHostImpl::CreateMetricsAllocator() {
  // Create a persistent memory segment for subprocess histograms only if
  // they're active in the host.
  ...
  switch (data_.process_type) {
    ...
    default:
      ...
      if (process_type >= common::PROCESS_TYPE_END)
        process_type += 1000 - common::PROCESS_TYPE_END;
...
void HostChildProcessHostImpl::ShareMetricsAllocatorToProcess() {
  if (metrics_allocator_) {
    HistogramController::GetInstance()->SetHistogramMemory<common::ChildProcessHost>(
        GetHost(),
...
void HostChildProcessHostImpl::OnProcessLaunchFailed(int error_code) {
  delegate_->OnProcessLaunchFailed(error_code);
  notify_child_disconnected_ = false;
  delete delegate_;  // Will delete us
}

void HostChildProcessHostImpl::OnProcessLaunched() {
  DCHECK_CURRENTLY_ON(HostThread::IO);
...
bool HostChildProcessHostImpl::IsProcessLaunched() const {
  DCHECK_CURRENTLY_ON(HostThread::IO);
...
// static
void HostChildProcessHostImpl::OnMojoError(
    base::WeakPtr<HostChildProcessHostImpl> process,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    const std::string& error) {
  if (!task_runner->BelongsToCurrentThread()) {
    task_runner->PostTask(
        FROM_HERE, base::BindOnce(&HostChildProcessHostImpl::OnMojoError,
  ...
  HistogramBadMessageTerminated(
      static_cast<common::ProcessType>(process->data_.process_type));
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kDisableKillAfterBadIPC)) {
    return;
  }
  ...
  base::debug::DumpWithoutCrashing();
  process->child_process_->Terminate(common::RESULT_CODE_KILLED_BAD_MESSAGE);
}

#if defined(OS_WIN)

void HostChildProcessHostImpl::OnObjectSignaled(HANDLE object) {
  OnChildDisconnected();
}
