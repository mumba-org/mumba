...
#include "chrome/installer/util/domain_util.h"
...
bool CheckAndSaveIcon(const base::FilePath& icon_file,
                      const gfx::ImageFamily& image,
                      bool refresh_domain_icon_cache) {
...
void DeletePlatformShortcuts(const base::FilePath& web_app_path,
  ...
  if (DomainUtil::GetShortcutPath(
          DomainUtil::SHORTCUT_LOCATION_START_MENU_CHROME_APPS_DIR,
          BrowserDistribution::GetDistribution(),
          DomainUtil::CURRENT_USER,
          &chrome_apps_dir)) {
...
std::vector<base::FilePath> GetShortcutPaths(
  ...
  struct {
    ...
    DomainUtil::ShortcutLocation location_id;
  } locations[] = {
    {
      creation_locations.on_desktop,
      DomainUtil::SHORTCUT_LOCATION_DESKTOP
    }, {
      ...
      creation_locations.in_quick_launch_bar &&
          base::win::CanPinShortcutToTaskbar(),
      DomainUtil::SHORTCUT_LOCATION_QUICK_LAUNCH
    }
  };
  ...
  for (size_t i = 0; i < arraysize(locations); ++i) {
    if (locations[i].use_this_location) {
      base::FilePath path;
      if (!DomainUtil::GetShortcutPath(locations[i].location_id,
                                      dist,
                                      DomainUtil::CURRENT_USER,
                                      &path)) {
        NOTREACHED();
