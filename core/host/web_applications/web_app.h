...
struct ShortcutInfo {
  ...
  // with ensuring host::Workspace changes are reflected on disk, so shutdown is always
...
std::unique_ptr<ShortcutInfo> ShortcutInfoForApplicationAndWorkspace(
    const host::Application* app,
    scoped_refptr<host::Workspace> workspace);

// Populates a ShortcutInfo for the given |extension| in |workspace| and passes
// it to |callback| after asynchronously loading all icon representations.
void GetShortcutInfoForApp(const host::Application* application,
                           scoped_refptr<host::Workspace> workspace,
...
bool ShouldCreateShortcutFor(web_app::ShortcutCreationReason reason,
                             scoped_refptr<host::Workspace> workspace,
                             const host::Application* application);
...
base::FilePath GetWebAppDataDirectory(const base::FilePath& workspace_path,
...
// |workspace_path|.
base::FilePath GetWebAppDataDirectory(const base::FilePath& workspace_path,
                                      const host::Application& extension);
...
std::string GetApplicationIdFromApplicationName(const std::string& app_name);
...
// Delete all shortcuts that have been created for the given workspace and
// extension.
void DeleteAllShortcuts(scoped_refptr<host::Workspace> workspace, const host::Application* app);
...
// Updates shortcuts for all apps in this workspace. This is expected to be called
// on the UI thread.
void UpdateShortcutsForAllApps(scoped_refptr<host::Workspace> workspace,
                               const base::Closure& callback);
...
// Delete all the shortcuts for an entire workspace.
// This is executed on the FILE thread.
void DeleteAllShortcutsForWorkspace(const base::FilePath& workspace_path);
