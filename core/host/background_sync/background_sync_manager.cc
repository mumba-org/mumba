...
#include "core/host/background_sync/background_sync_metrics.h"
#include "core/host/background_sync/background_sync_network_observer.h"
#include "core/host/background_sync/background_sync_registration_options.h"
...
Domain* GetDomainOnUIThread(
  ...
  return service_worker_context->domain();// storage_partition_impl->browser_context();
}
...
BackgroundSyncController* GetBackgroundSyncControllerOnUIThread(
  ...
  // if (!browser_context)
  //   return nullptr;
  Domain* domain = GetDomainOnUIThread(std::move(service_worker_context));
  if (!domain)
    return nullptr;

  return domain->GetBackgroundSyncController();
}
...
void BackgroundSyncManager::EmulateDispatchSyncEvent(
    ...
    ServiceWorkerVersion::StatusCallback callback) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  common::ServiceWorkerStatusCode code = CanEmulateSyncEvent(active_version);
  if (code != common::SERVICE_WORKER_OK) {
...
void BackgroundSyncManager::DisableAndClearDidGetRegistrations(
  ...
  if (status != common::SERVICE_WORKER_OK || user_data.empty()) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE,
                                                  std::move(callback));
    return;
  }
...
void BackgroundSyncManager::EventCompleteImpl(
  ...
  if (sw_registration) {
    HasMainFrameProviderHost(
        ...
        base::BindOnce(&BackgroundSyncMetrics::RecordEventResult,
                       status_code == common::SERVICE_WORKER_OK));
  }
  ...
  } else if (status_code != common::SERVICE_WORKER_OK &&
...
void BackgroundSyncManager::EventCompleteDidStore(
  ...
  if (status_code == common::SERVICE_WORKER_ERROR_NOT_FOUND) {
  ...
  if (status_code != common::SERVICE_WORKER_OK) {
    LOG(ERROR) << "BackgroundSync failed to store registration due to backend "
                  "failure.";
    DisableAndClearManager(std::move(callback));
    return;
...
common::ServiceWorkerStatusCode BackgroundSyncManager::CanEmulateSyncEvent(
    scoped_refptr<ServiceWorkerVersion> active_version) {
  if (!active_version)
