...
#include "core/host/host_child_process_host.h"
...
class HostChildProcessHostIterator;
class HostChildProcessObserver;
...
class CONTENT_EXPORT HostChildProcessHostImpl
    : public HostChildProcessHost,
 ...
 public:
  HostChildProcessHostImpl(common::ProcessType process_type,
                            HostChildProcessHostDelegate* delegate,
                            const std::string& service_name);
  ~HostChildProcessHostImpl() override;

  // Terminates all child processes and deletes each HostChildProcessHost
  // instance.
  ...
  // HostChildProcessHost implementation:
  bool Send(IPC::Message* message) override;
  void Launch(std::unique_ptr<common::SandboxedProcessLauncherDelegate> delegate,
              std::unique_ptr<base::CommandLine> cmd_line,
              bool terminate_on_shutdown) override;
  const ChildProcessData& GetData() const override;
  common::ChildProcessHost* GetHost() const override;
  ...
  static void HistogramBadMessageTerminated(common::ProcessType process_type);

  HostChildProcessHostDelegate* delegate() const { return delegate_; }

  common::ChildConnection* child_connection() const {
  ...
  typedef std::list<HostChildProcessHostImpl*> HostChildProcessList;
 private:
  friend class HostChildProcessHostIterator;
  friend class HostChildProcessObserver;

  static HostChildProcessList* GetIterator();

  static void AddObserver(HostChildProcessObserver* observer);
  static void RemoveObserver(HostChildProcessObserver* observer);
  ...
  HostChildProcessHostDelegate* delegate_;
  std::unique_ptr<common::ChildProcessHost> child_process_host_;
  ...
  base::WeakPtrFactory<HostChildProcessHostImpl> weak_factory_;
};
