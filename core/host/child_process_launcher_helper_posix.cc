...
#include "core/host/posix_file_descriptor_info_impl.h"
...
std::map<std::string, std::string>& GetServiceNameByProcessTypeMap() {
  static auto* service_name_resolver = new std::map<std::string, std::string>(
      {// The service names are defined in the JSON manifests, so we don't have
       ...
       {switches::kGpuProcess, "gpu"},
       {switches::kDomainProcess, "domain"},
       //{switches::kPpapiPluginProcess, "content_plugin"},
       {switches::kApplicationProcess, "application"},
       {switches::kUtilityProcess, "utility"}});
...
//   if (command_line.HasSwitchValueASCII(switches::kGpuProcess)) {
//     static breakpad::CrashHandlerHostLinux* crash_handler = NULL;
//     if (!crash_handler)
//       crash_handler = CreateCrashHandlerHost("gpu");
//     return crash_handler->GetDeathSignalSocket();
//   } else if (command_line.HasSwitchValueASCII(switches::kUtilityProcess)) {
//     static breakpad::CrashHandlerHostLinux* crash_handler = nullptr;
//     if (!crash_handler)
//       crash_handler = CreateCrashHandlerHost("utility");
//     return crash_handler->GetDeathSignalSocket();
//   } else if (command_line.HasSwitchValueASCII(switches::kDomainProcess)) {
//     static breakpad::CrashHandlerHostLinux* crash_handler = nullptr;
//     if (!crash_handler)
//       crash_handler = CreateCrashHandlerHost("shell");
//     return crash_handler->GetDeathSignalSocket();
//   }
...
std::unique_ptr<PosixFileDescriptorInfo> CreateDefaultPosixFilesToMap(
    int child_process_id,
    const mojo::PlatformHandle& mojo_client_handle,
  ...
  // GetContentClient()->host()->GetAdditionalMappedFilesForChildProcess(
