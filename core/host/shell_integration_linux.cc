...
//#include "core/grit/chrome_unscaled_resources.h"
...
// https://wiki.gnome.org/Projects/GnomeDomain/ApplicationBased
...
std::string GetDesktopName(base::Environment* env) {
  ...
  // non-official, containerd versions using the built-in value.
  std::string name;
...
web_app::ShortcutLocations GetExistingShortcutLocations(
  ...
  return GetExistingShortcutLocations(env, workspace_path, extension_id,
                                      desktop_path);
}
...
base::FilePath GetWebShortcutFilename(const GURL& url) {
  ...
  std::string filename =
      std::string(constants::kHostProcessExecutableName) + "-" + url.spec();
...
base::FilePath GetExtensionShortcutFilename(const base::FilePath& workspace_path,
                                            const std::string& extension_id) {
  DCHECK(!extension_id.empty());

  // Use a prefix, because xdg-desktop-menu requires it.
  std::string filename(constants::kHostProcessExecutableName);
  filename.append("-")
      .append(extension_id)
      .append("-")
      .append(workspace_path.BaseName().value());
  base::i18n::ReplaceIllegalCharactersInPath(&filename, '_');
...
std::vector<base::FilePath> GetExistingProfileShortcutFilenames(
  ...
  // Use a prefix, because xdg-desktop-menu requires it.
  std::string prefix(constants::kHostProcessExecutableName);
  prefix.append("-");
  std::string suffix("-");
  suffix.append(workspace_path.BaseName().value());
...
std::string GetDesktopFileContents(
  ...
  base::CommandLine cmd_line = shell_integration::CommandLineArgsForLauncher(
      url, extension_id, workspace_path);
...
bool CreateDesktopShortcut(
  ...
  if (!shortcut_info.extension_id.empty()) {
    shortcut_filename = GetExtensionShortcutFilename(
        shortcut_info.workspace_path, shortcut_info.extension_id);
...
void DeleteDesktopShortcuts(const base::FilePath& workspace_path,
                            const std::string& extension_id) {
  base::AssertBlockingAllowed();
...
void DeleteAllDesktopShortcuts(const base::FilePath& workspace_path) {
  base::AssertBlockingAllowed();
  ...
  if (PathService::Get(base::DIR_USER_DESKTOP, &desktop_path)) {
    std::vector<base::FilePath> shortcut_filenames_desktop =
        GetExistingProfileShortcutFilenames(workspace_path, desktop_path);
  ...
  std::vector<base::FilePath> shortcut_filenames_app_menu =
      GetExistingProfileShortcutFilenames(workspace_path, applications_menu);
