...
#include "core/host/font_utils_linux.h"
...
SandboxIPCHandler::SandboxIPCHandler(int lifeline_fd, int host_socket)
    : lifeline_fd_(lifeline_fd), host_socket_(host_socket) {}

void SandboxIPCHandler::Run() {
  struct pollfd pfds[2];
  pfds[0].fd = lifeline_fd_;
  pfds[0].events = POLLIN;
  pfds[1].fd = host_socket_;
  ...
  for (;;) {
    ...
    // The host process will close the other end of this pipe on shutdown,
    // so we should exit.
    if (pfds[0].revents) {
      break;
    }
    ...
    if (pfds[1].revents & POLLIN) {
      HandleRequestFromChild(host_socket_);
    }
  }
...
void SandboxIPCHandler::HandleRequestFromChild(int fd) {
  ...
  char buf[common::FontConfigIPC::kMaxFontFamilyLength + 128];
  ...
  if (kind == common::FontConfigIPC::METHOD_MATCH) {
    HandleFontMatchRequest(fd, iter, fds);
  } else if (kind == common::FontConfigIPC::METHOD_OPEN) {
...
SandboxIPCHandler::~SandboxIPCHandler() {
  if (IGNORE_EINTR(close(lifeline_fd_)) < 0)
    PLOG(ERROR) << "close";
  if (IGNORE_EINTR(close(host_socket_)) < 0)
    PLOG(ERROR) << "close";
}
