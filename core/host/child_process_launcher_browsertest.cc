...
class ChildProcessLauncherHostTest : public ContentHostTest {};

IN_PROC_BROWSER_TEST_F(ChildProcessLauncherHostTest, ChildSpawnFail) {
  ...
  NavigationEntry* last_entry =
      host()->web_contents()->GetController().GetLastCommittedEntry();
  ...
  window->LoadURL(url);
  nav_observer2.Wait();
  last_entry = host()->web_contents()->GetController().GetLastCommittedEntry();
  // Make sure that we navigated to the proper URL.
  CHECK(last_entry && last_entry->GetPageType() == PAGE_TYPE_NORMAL);
  CHECK(host()->web_contents()->GetLastCommittedURL() == url);
