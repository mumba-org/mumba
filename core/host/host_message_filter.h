...
namespace host {
//class HostAssociatedInterfaceTest;
struct HostMessageFilterTraits;
class DomainProcessHost;
class ApplicationProcessHost;

// Base class for message filters in the host process.  You can receive and
// send messages on any thread.
class CONTENT_EXPORT HostMessageFilter
    : public base::RefCountedThreadSafe<
          HostMessageFilter, HostMessageFilterTraits>,
      public IPC::Sender {
 public:
  explicit HostMessageFilter(uint32_t message_class_to_filter);
  HostMessageFilter(const uint32_t* message_classes_to_filter,
  ...
  // messages (since we don't want to block the host on any other process).
  ...
  // host thread, change |thread| to the id of the target thread
 ...
 private:
  friend class base::RefCountedThreadSafe<HostMessageFilter,
                                          HostMessageFilterTraits>;

  class Internal;
  friend class HostAssociatedInterfaceTest;
  friend class HostChildProcessHostImpl;
...
struct HostMessageFilterTraits {
  static void Destruct(const HostMessageFilter* filter) {
    filter->OnDestruct();
  }
};
