...
#include "core/host/background_fetch/background_fetch_job_controller.h"
...
#include "core/host/background_fetch/background_fetch_registration_notifier.h"
#include "core/host/background_fetch/background_fetch_scheduler.h"
...
BackgroundFetchContext::BackgroundFetchContext(
    ...
    : domain_(domain),
      data_manager_(domain, service_worker_context),
      event_dispatcher_(service_worker_context),
      registration_notifier_(
          std::make_unique<BackgroundFetchRegistrationNotifier>()),
      delegate_proxy_(domain->GetBackgroundFetchDelegate()),//browser_context_->GetBackgroundFetchDelegate()),
...
void BackgroundFetchContext::DidGetRegistration(
    ...
    std::unique_ptr<common::BackgroundFetchRegistration> registration) {
  DCHECK_CURRENTLY_ON(HostThread::IO);
  ...
  registration->downloaded += job_controllers_[registration->unique_id]->GetInProgressDownloadedBytes();
