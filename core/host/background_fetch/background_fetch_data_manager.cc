...
#include "core/host/background_fetch/background_fetch_constants.h"
#include "core/host/background_fetch/background_fetch_cross_origin_filter.h"
#include "core/host/background_fetch/background_fetch_request_info.h"
#include "core/host/background_fetch/storage/cleanup_task.h"
...
#include "core/host/background_fetch/storage/delete_registration_task.h"
#include "core/host/background_fetch/storage/get_developer_ids_task.h"
#include "core/host/background_fetch/storage/get_metadata_task.h"
#include "core/host/background_fetch/storage/mark_registration_for_deletion_task.h"
#include "core/host/background_fetch/storage/update_registration_ui_task.h"
...
//   auto registration = std::make_unique<common::BackgroundFetchRegistration>();
...
class BackgroundFetchDataManager::RegistrationData {
 public:
  RegistrationData(const BackgroundFetchRegistrationId& registration_id,
    ...
    for (const common::ServiceWorkerFetchRequest& fetch_request : requests) {
  ...
  const common::BackgroundFetchOptions& options() const { return options_; }
...
BackgroundFetchDataManager::BackgroundFetchDataManager(
  ...
  blob_storage_context_ =
      base::WrapRefCounted(ChromeBlobStorageContext::GetFor(domain));//browser_context));
  DCHECK(blob_storage_context_);

  HostThread::PostAfterStartupTask(
      FROM_HERE, HostThread::GetTaskRunnerForThread(HostThread::IO),
...
void BackgroundFetchDataManager::GetSettledFetchesForRegistration(
  ...
  std::vector<common::BackgroundFetchSettledFetch> settled_fetches;
  settled_fetches.reserve(requests.size());

  std::vector<std::unique_ptr<storage::BlobDataHandle>> blob_data_handles;

  for (const auto& request : requests) {
    common::BackgroundFetchSettledFetch settled_fetch;
