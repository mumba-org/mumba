...
#include "core/host/background_fetch/background_fetch_delegate_proxy.h"
...
class BackgroundFetchDelegateProxy::Core
 ...
 public:
  ...
  ~Core() override { DCHECK_CURRENTLY_ON(HostThread::UI); }
  ...
  void StartRequest(const std::string& job_unique_id,
    ...
    const common::ServiceWorkerFetchRequest& fetch_request = request->fetch_request();
