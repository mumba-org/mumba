...
// HistogramController is used on the host process to collect histogram data.
// Only the host UI thread is allowed to interact with the
...
class HistogramController {
 ...
 private:
  ...
  using ChildHistogramFetcherMap =
      std::map<T*, common::mojom::ChildHistogramFetcherPtr>;
  ...
  template <class T>
  common::mojom::ChildHistogramFetcher* GetChildHistogramFetcherInterface(
      T* host);
  ...
  ChildHistogramFetcherMap<common::ChildProcessHost> child_histogram_fetchers_;
