...
#include "core/host/android/application_window_host_connector_hosttest.h"

#include "core/host/frame_host/interstitial_page_impl.h"
#include "core/host/interstitial_page_delegate.h"
#include "content/public/test/host_test_utils.h"
#include "content/public/test/content_host_test_utils.h"
#include "content/test/content_host_test_utils_internal.h"
...
IN_PROC_BROWSER_TEST_F(RenderWidgetHostConnectorTest,
  ...
  EXPECT_TRUE(NavigateToURL(host(), main_url));
  ...
  Host* new_host = OpenPopup(iframe, GURL(url::kAboutBlankURL), "");
  EXPECT_TRUE(new_host);

  auto* rwhva_popup = static_cast<RenderWidgetHostViewAndroid*>(
      new_host->web_contents()->GetRenderWidgetHostView());
...
IN_PROC_BROWSER_TEST_F(RenderWidgetHostConnectorTest,
  ...
  EXPECT_TRUE(NavigateToURL(host(), http_url));
  RenderWidgetHostViewAndroid* old_rwhva = application_window_host_view_android();
  RenderWidgetHostConnector* connector = application_window_host_connector();
  ...
  EXPECT_TRUE(NavigateToURL(host(), https_url));
  RenderWidgetHostViewAndroid* new_rwhva = application_window_host_view_android();
...
IN_PROC_BROWSER_TEST_F(RenderWidgetHostConnectorTest,
  ...
  EXPECT_TRUE(
      NavigateToURL(host(), embedded_test_server()->GetURL("/title1.html")));

  RenderWidgetHostViewAndroid* main_rwhva = application_window_host_view_android();
  RenderWidgetHostConnector* connector = application_window_host_connector();
