...
#include "core/host/android/synchronous_compositor_client.h"
...
void SynchronousCompositorHost::SetSoftwareDrawSharedMemoryIfNeeded(
  ...
  {
    TRACE_EVENT1("host", "AllocateSharedMemory", "buffer_size", buffer_size);
...
void SynchronousCompositorHost::LayerTreeFrameSinkCreated() {
  ...
  // New LayerTreeFrameSink is not aware of state from Host side. So need to
  // re-send all host side state here.
  mojom::SynchronousCompositor* compositor = GetSynchronousCompositor();
  DCHECK(compositor);
  compositor->SetMemoryPolicy(bytes_limit_);
