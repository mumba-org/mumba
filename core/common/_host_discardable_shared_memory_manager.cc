...
#include "core/common/host_discardable_shared_memory_manager.h"
...
#include "core/common/discardable_shared_memory_heap.h"
#include "core/shared/common/child_process_host.h"
...
class DiscardableMemoryImpl : public base::DiscardableMemory {
 public:
  DiscardableMemoryImpl(std::unique_ptr<base::DiscardableSharedMemory> shared_memory,
  ...
  for (const auto& process_entry : processes_) {
    ...
    for (const auto& segment_entry : process_segments) {
      ...
      const uint64 child_tracing_process_id =
          ChildProcessHost::ChildProcessUniqueIdToTracingProcessId(
              child_process_id);
