...
class DiscardableSharedMemoryHeap {
 public:
  class Span : public base::LinkNode<Span> {
   public:
    ~Span();
  ...
  std::unique_ptr<Span> Grow(std::unique_ptr<base::DiscardableSharedMemory> shared_memory,
                        size_t size,
                        int32_t id,
