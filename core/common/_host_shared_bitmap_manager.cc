...
#include "core/common/host_shared_bitmap_manager.h"
...
std::unique_ptr<cc::SharedBitmap> HostSharedBitmapManager::AllocateSharedBitmap(
  ...
  data->pixels = std::unique_ptr<uint8[]>(new uint8[bitmap_size]);
...
std::unique_ptr<cc::SharedBitmap> HostSharedBitmapManager::GetSharedBitmapFromId(
    const gfx::Size& size,
    const cc::SharedBitmapId& id) {
  base::AutoLock lock(lock_);
