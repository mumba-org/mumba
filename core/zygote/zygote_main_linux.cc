...
using common::kZygoteBootMessage;
...
// are known to be culprits, e.g. lttng.
  ...
  // MUMBA: we are disabling this here, because
  // we are enabling the namespace sandbox
  // only in the container process
  ...
  //   EnterNamespaceSandbox(linux_sandbox, std::move(post_fork_parent_callback), std::move(chroot_dir));
  // } else {
...
bool ZygoteMain(
  ...
  DLOG(INFO) << "ZygoteMain";
  sandbox::SetAmZygoteOrRenderer(true, common::GetSandboxFD());
  ...
  for (const auto& fork_delegate : fork_delegates) {
    fork_delegate->Init(common::GetSandboxFD(), using_layer1_sandbox);
  }
  ...
  Zygote zygote(sandbox_flags, std::move(fork_delegates),
                base::GlobalDescriptors::Descriptor(
                    static_cast<uint32_t>(kSandboxIPCChannel), common::GetSandboxFD()));
