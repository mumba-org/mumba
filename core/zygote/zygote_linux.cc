...
#include "core/zygote/zygote_linux.h"
...
#include "core/common/zygote_commands_linux.h"
...
#include "core/common/send_zygote_child_ping_linux.h"
...
using common::kZygoteHelloMessage;
using common::kZygoteSocketPairFd;
using common::kZygoteMaxMessageLength;
using common::SendZygoteChildPing;
using common::kZygoteCommandForkRealPID;
using common::kZygoteCommandReap;
using common::kZygoteCommandFork;
using common::kZygoteCommandGetTerminationStatus;
using common::kZygoteCommandGetSandboxStatus;
...
void KillAndReap(pid_t pid, common::ZygoteForkDelegate* helper) {
  if (helper) {
...
Zygote::Zygote(int sandbox_flags,
               std::vector<std::unique_ptr<common::ZygoteForkDelegate>> helpers,
  ...
  if (iter.ReadInt(&kind)) {
    switch (kind) {
      case kZygoteCommandFork: {
        DLOG(INFO) << "ZygoteCommandFork: our pid: " << getpid();
        // This function call can return multiple times, once per fork().
...
void Zygote::HandleGetTerminationStatus(int fd, base::PickleIterator iter) {
  ...
  if (!got_termination_status) {
    ...
    exit_code = common::RESULT_CODE_NORMAL_EXIT;
  }
...
int Zygote::ForkWithRealPid(const std::string& process_type,
  ...
  common::ZygoteForkDelegate* helper = nullptr;
  ...
  if (helper) {
    ...
    if (sandbox_flags_ & service_manager::SandboxLinux::kPIDNS &&
      ...
      DLOG(INFO) << "drop caps: container process? " << (process_type == "container");
      bool drop_capabilities_in_child = process_type != "container";
      pid = sandbox::NamespaceSandbox::ForkInNewPidNamespace(
        /*drop_capabilities_in_child=*/drop_capabilities_in_child);
    } else {
...
bool Zygote::HandleForkRequest(int fd,
  ...
  DLOG(INFO) << "Zygote::HandleForkRequest: child pid: " << child_pid;
