...
#include "core/shared/common/resource_load_info.mojom.h"
#include "core/shared/common/inter_process_time_ticks_converter.h"
//#include "core/shared/common/navigation_params.h"
#include "core/shared/common/throttling_url_loader.h"
//#include "core/shared/common/browser_side_navigation_policy.h"
#include "core/shared/common/resource_load_info.mojom.h"
#include "core/shared/common/resource_type.h"
#include "core/domain/fixed_received_data.h"
#include "core/domain/request_peer.h"
#include "core/domain/resource_dispatcher_delegate.h"
#include "core/domain/navigation_response_override_parameters.h"
//#include "core/domain/request_extra_data.h"
#include "core/domain/sync_load_context.h"
#include "core/domain/sync_load_response.h"
#include "core/domain/url_loader_client_impl.h"
//#include "core/domain/application_frame_impl.h"
...
void RemoteToLocalTimeTicks(
    const common::InterProcessTimeTicksConverter& converter,
    base::TimeTicks* time) {
  common::RemoteTimeTicks remote_time = common::RemoteTimeTicks::FromTimeTicks(*time);
...
void CheckSchemeForReferrerPolicy(const network::ResourceRequest& request) {
  if ((request.referrer_policy == common::Referrer::GetDefaultReferrerPolicy() ||
...
void NotifySubresourceStarted(
  ...
  // TODO: we need to reimplement this using our own native structures
...
void NotifyResourceLoadComplete(
    scoped_refptr<base::SingleThreadTaskRunner> thread_task_runner,
    int render_frame_id,
    common::mojom::ResourceLoadInfoPtr resource_load_info) {
  if (!thread_task_runner)
    return;
...
void ResourceDispatcher::OnReceivedResponse(
  ...
  //DLOG(INFO) << "ResourceDispatcher::OnReceivedResponse";
  ...
  if (!IsResourceTypeFrame(request_info->resource_type)) {
    // FIXME: The DomainMainThread::current() doesnt work on worker threads
    //        as the DomainMainThread::current() singleton is allocated on TLS
    //        so its only reachable whe we are running on the main thread.

    NotifySubresourceStarted(//DomainMainThread::current()->main_thread_runner(),
                             DomainMainThread::DeprecatedGetMainTaskRunner(),
...
void ResourceDispatcher::OnReceivedRedirect(
  ...
  if (request_info->peer->OnReceivedRedirect(redirect_info,
    ...
    common::mojom::RedirectInfoPtr net_redirect_info = common::mojom::RedirectInfo::New();
    net_redirect_info->url = redirect_info.new_url;
    net_redirect_info->network_info = common::mojom::CommonNetworkInfo::New();
...
void ResourceDispatcher::OnRequestComplete(
  ...
  //(INFO) << "ResourceDispatcher::OnRequestComplete";
  ...
  auto resource_load_info = common::mojom::ResourceLoadInfo::New();
  ...
  resource_load_info->network_info = common::mojom::CommonNetworkInfo::New();
  ...
  NotifyResourceLoadComplete(DomainMainThread::DeprecatedGetMainTaskRunner(),
                             request_info->render_frame_id,
                             std::move(resource_load_info));
...
void ResourceDispatcher::Cancel(
    int request_id,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner) {
  //DLOG(INFO) << "ResourceDispatcher::Cancel";    
  PendingRequestMap::iterator it = pending_requests_.find(request_id);
  if (it == pending_requests_.end()) {
...
void ResourceDispatcher::StartSync(
    ...
    std::vector<std::unique_ptr<common::URLLoaderThrottle>> throttles,
    double timeout,
...
int ResourceDispatcher::StartAsync(
  ...
  pending_requests_[request_id] = std::make_unique<PendingRequestInfo>(
      std::move(peer), static_cast<common::ResourceType>(request->resource_type),
  ...
  if (base::FeatureList::IsEnabled(network::features::kNetworkService) &&
      request->fetch_request_context_type != common::REQUEST_CONTEXT_TYPE_FETCH) {
  ...
  std::unique_ptr<common::ThrottlingURLLoader> url_loader =
      common::ThrottlingURLLoader::CreateLoaderAndStart(
...
void ResourceDispatcher::ToResourceResponseInfo(
  ...
  *renderer_info = browser_info;
  ...
  common::InterProcessTimeTicksConverter converter(
      common::LocalTimeTicks::FromTimeTicks(request_info.local_request_start),
      common::LocalTimeTicks::FromTimeTicks(request_info.local_response_start),
      common::RemoteTimeTicks::FromTimeTicks(browser_info.request_start),
      common::RemoteTimeTicks::FromTimeTicks(browser_info.response_start));
