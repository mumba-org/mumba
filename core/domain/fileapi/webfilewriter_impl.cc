...
FileSystemDispatcher* GetFileSystemDispatcher() {
  return DomainMainThread::current()
 ...
 public:
  ...
  void Truncate(const GURL& path,
    ...
    if (!GetFileSystemDispatcher())
      return;
    DomainMainThread::current()->file_system_dispatcher()->Truncate(
  ...
  void Write(const GURL& path,
    ...
    if (!GetFileSystemDispatcher())
      return;
    DomainMainThread::current()->file_system_dispatcher()->Write(
  ...
  void Cancel(const StatusCallback& status_callback) {
    ...
    if (!GetFileSystemDispatcher())
      return;
    DomainMainThread::current()->file_system_dispatcher()->Cancel(
        request_id_,
        base::Bind(&WriterBridge::DidFinish, this));
  }
