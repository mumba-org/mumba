...
#include "core/domain/mojo/blink_interface_provider_impl.h"
#include "core/domain/web_blob_registry_impl.h"
...
#include "core/domain/worker_thread_registry.h"
#include "core/domain/cache_storage/webserviceworkercachestorage_impl.h"
#include "core/domain/webgraphicscontext3d_provider_impl.h"
...
BlinkPlatformImpl::BlinkPlatformImpl(
  ...
  //    DomainMainThread::current()->notification_dispatcher();
...
scoped_refptr<ChildURLLoaderFactoryBundle> BlinkPlatformImpl::CreateDefaultURLLoaderFactoryBundle() {
...
scoped_refptr<cc::TextureLayer> BlinkPlatformImpl::GetExternalTextureLayerForCanvas(cc::TextureLayerClient* client) {
  return nullptr;
}

void BlinkPlatformImpl::SetExternalTextureLayerForCanvas(const blink::WebString& target, scoped_refptr<cc::TextureLayer>) {

}

void BlinkPlatformImpl::OnExternalTextureLayerForCanvasInjected(cc::TextureLayerClient* client) {
...
void BlinkPlatformImpl::RecordAction(const blink::UserMetricsAction& name) {
  if (common::ChildThread* child_thread = common::ChildThread::Get())
  ...
  for (size_t i = 0; i < arraysize(kDataResources); ++i) {
    if (!strcmp(name, kDataResources[i].name)) {
      ...
      if (!resource.empty() && kDataResources[i].is_gzipped) {
        ...
        std::string resource_str = resource.as_string();
        //DLOG(INFO) << "GetDataResource: name: " << name << " size: " << resource_str.size();
        CHECK(compression::GzipUncompress(resource_str, &uncompressed));
...
bool BlinkPlatformImpl::AllowScriptExtensionForServiceWorker(
  ...
  return false;//common::GetClient()->AllowScriptExtensionForServiceWorker(scriptUrl);
}
...
BlinkPlatformImpl::CreateNetworkURLLoaderFactory() {
  ...
  if (base::FeatureList::IsEnabled(network::features::kNetworkService)) {
    ...
    thread_->channel()->GetRemoteAssociatedInterface(&factory_ptr);
    url_loader_factory = std::move(factory_ptr);
  }
...
BlinkPlatformImpl::CreateDataConsumerHandle(mojo::ScopedDataPipeConsumerHandle handle) {
...
viz::FrameSinkId BlinkPlatformImpl::GenerateFrameSinkId() {
  viz::FrameSinkId frame_sink_id(thread_->domain_process_id(),
