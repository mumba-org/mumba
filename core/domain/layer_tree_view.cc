...
class ReportTimeSwapPromise : public cc::SwapPromise {
 public:
  ReportTimeSwapPromise(base::Callback<void (blink::WebLayerTreeView::SwapResult, double)> callback,
 ...
 private:
  ...
  base::Callback<void (blink::WebLayerTreeView::SwapResult, double)> callback_;
...
ReportTimeSwapPromise::DidNotSwapAction ReportTimeSwapPromise::DidNotSwap(
  ...
  task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(callback_), result,
                                                   base::saturated_cast<double>(base::TimeTicks::Now().ToInternalValue())));
  return ReportTimeSwapPromise::DidNotSwapAction::KEEP_ACTIVE;
}
...
cc::LayerSelection ConvertFromWebSelection(const blink::WebSelection& web_selection) {
...
bool LayerTreeView::IsForSubframe() {
  return false;
}
...
void LayerTreeView::SetRootLayer(const blink::WebLayer& layer) {
  layer_tree_host_->SetRootLayer(const_cast<blink::WebLayer &>(layer).CcLayer());
}
...
void LayerTreeView::StartPageScaleAnimation(const blink::WebPoint& target_offset,
                                            bool use_anchor,
                                            float new_page_scale,
                                            double duration_sec) {
  base::TimeDelta duration = base::TimeDelta::FromMicroseconds(
      duration_sec * base::Time::kMicrosecondsPerSecond);
  layer_tree_host_->StartPageScaleAnimation(gfx::Vector2d(target_offset.x, target_offset.y), use_anchor,
...
void LayerTreeView::RegisterViewportLayers(const ViewportLayers& layers) {
  ...
  viewport_layers.overscroll_elasticity = const_cast<blink::WebLayer *>(layers.overscroll_elasticity)->CcLayer();
  viewport_layers.page_scale = const_cast<blink::WebLayer *>(layers.page_scale)->CcLayer();
  viewport_layers.inner_viewport_container = const_cast<blink::WebLayer *>(layers.inner_viewport_container)->CcLayer();
  viewport_layers.outer_viewport_container = const_cast<blink::WebLayer *>(layers.outer_viewport_container)->CcLayer();
  viewport_layers.inner_viewport_scroll = const_cast<blink::WebLayer *>(layers.inner_viewport_scroll)->CcLayer();
  viewport_layers.outer_viewport_scroll = const_cast<blink::WebLayer *>(layers.outer_viewport_scroll)->CcLayer();
...
blink::WebEventListenerProperties LayerTreeView::EventListenerProperties(
  ...
  return static_cast<blink::WebEventListenerProperties>(layer_tree_host_->event_listener_properties(static_cast<cc::EventListenerClass>(event_class)));
}
...
void LayerTreeView::AddPresentationCallback(
  ...
  std::vector<ReportTimeCallback> callbacks;
