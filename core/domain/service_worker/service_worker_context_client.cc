...
#include "core/shared/common/push_event_payload.h"
#include "core/shared/common/referrer.h"
#include "core/shared/common/weak_wrapper_shared_url_loader_factory.h"
//#include "core/domain/application_client.h"
#include "core/domain/url_loader_throttle_provider.h"
//#include "core/domain/document_state.h"
#include "core/domain/worker_thread.h"
#include "core/domain/child_url_loader_factory_bundle.h"
#include "core/domain/request_extra_data.h"
#include "core/domain/web_data_consumer_handle_impl.h"
#include "core/domain/web_url_loader_impl.h"
#include "core/domain/web_url_request_util.h"
#include "core/domain/notifications/notification_data_conversions.h"
#include "core/domain/domain_main_thread.h"
#include "core/domain/url_loader_throttle_provider_impl.h"
#include "core/domain/websocket_handshake_throttle_provider_impl.h"
...
#include "core/domain/service_worker/service_worker_fetch_context_impl.h"
#include "core/domain/service_worker/service_worker_network_provider.h"
#include "core/domain/service_worker/service_worker_provider_context.h"
#include "core/domain/service_worker/service_worker_timeout_timer.h"
#include "core/domain/service_worker/service_worker_type_converters.h"
#include "core/domain/service_worker/service_worker_type_util.h"
#include "core/domain/service_worker/web_service_worker_impl.h"
#include "core/domain/service_worker/web_service_worker_provider_impl.h"
#include "core/domain/service_worker/web_service_worker_registration_impl.h"
...
class WebServiceWorkerNetworkProviderImpl
 ...
 public:
  ...
  std::unique_ptr<blink::WebURLLoader> CreateURLLoader(
    ...
    DomainMainThread* domain_thread = DomainMainThread::current();
    if (domain_thread && provider_->script_loader_factory() &&
        common::ServiceWorkerUtils::IsServicificationEnabled() &&
        IsScriptRequest(request)) {
      // TODO(crbug.com/796425): Temporarily wrap the raw
      // common::mojom::URLLoaderFactory pointer into SharedURLLoaderFactory.
      return std::make_unique<WebURLLoaderImpl>(
          domain_thread->resource_dispatcher(), std::move(task_runner),
...
void ToWebServiceWorkerRequest(const network::ResourceRequest& request,
  ...
  web_request->SetIsMainResourceLoad(common::ServiceWorkerUtils::IsMainResourceType(
      static_cast<common::ResourceType>(request.resource_type)));
  web_request->SetCredentialsMode(request.fetch_credentials_mode);
  web_request->SetCacheMode(
      common::ServiceWorkerFetchRequest::GetCacheModeFromLoadFlags(request.load_flags));
  web_request->SetRedirectMode(request.fetch_redirect_mode);
  web_request->SetRequestContext(GetBlinkRequestContext(
      static_cast<common::RequestContextType>(request.fetch_request_context_type)));
...
void ToWebServiceWorkerRequest(const common::ServiceWorkerFetchRequest& request,
                               blink::WebServiceWorkerRequest* web_request) {
  DCHECK(web_request);
...
void ToWebServiceWorkerResponse(const common::ServiceWorkerResponse& response,
...
struct ServiceWorkerContextClient::WorkerContextData {
  ...
  mojo::Binding<common::mojom::ServiceWorkerEventDispatcher> event_dispatcher_binding;
  ...
  std::map<int, common::mojom::ServiceWorkerFetchResponseCallbackPtr>
...
ServiceWorkerContextClient::ServiceWorkerContextClient(
    ...
    common::ServiceWorkerContextInstance* instance,
    ...
    : embedded_worker_id_(embedded_worker_id),
      ...
      instance_(instance) {
  instance_host_ =
      common::mojom::ThreadSafeEmbeddedWorkerInstanceHostAssociatedPtr::Create(
...
void ServiceWorkerContextClient::DidInitializeWorkerContext(
  ...
  // NOTE: Chrome doesnt really use this, except to warn extensions
  // about it. Maybe in our case we can ping the service process
...
void ServiceWorkerContextClient::RespondToFetchEventWithNoResponse(
  ...
  const common::mojom::ServiceWorkerFetchResponseCallbackPtr& response_callback =
...
void ServiceWorkerContextClient::RespondToFetchEvent(
  ...
  //common::mojom::ServiceWorkerResponsePtr response = GetServiceWorkerResponseFromWebResponse(web_response);
  const common::mojom::ServiceWorkerFetchResponseCallbackPtr& response_callback =
      context_->fetch_response_callbacks[fetch_event_id];
...
ServiceWorkerContextClient::CreateServiceWorkerFetchContext() {
  ...
  scoped_refptr<ChildURLLoaderFactoryBundle> url_loader_factory_bundle =
    thread->blink_platform()->CreateDefaultURLLoaderFactoryBundle();
  ...
  return std::make_unique<ServiceWorkerFetchContextImpl>(
      ...
      std::make_unique<URLLoaderThrottleProviderImpl>(URLLoaderThrottleProviderType::kWorker),
      std::make_unique<WebSocketHandshakeThrottleProviderImpl>());
}
...
void ServiceWorkerContextClient::SendWorkerStarted() {
  DCHECK(worker_task_runner_->RunsTasksInCurrentSequence());
  common::mojom::EmbeddedWorkerStartTimingPtr timing =
      common::mojom::EmbeddedWorkerStartTiming::New();
...
void ServiceWorkerContextClient::DispatchBackgroundFetchFailEvent(
    const std::string& developer_id,
    const std::vector<common::BackgroundFetchSettledFetch>& fetches,
...
void ServiceWorkerContextClient::DispatchPushEvent(
    const common::PushEventPayload& payload,
  ...
  int request_id = context_->timeout_timer->StartEventWithCustomTimeout(
      ...
      base::TimeDelta::FromSeconds(common::mojom::kPushEventTimeoutSeconds));
...
void ServiceWorkerContextClient::Ping(PingCallback callback) {
  // unlike a normal service worker, this is a real service process
  // serving applications and the concept of idle here is not the same
  // as the worker thread might be used in other ways.. mostly to 
  // comunicate with the application, but it will not be possible
  // to really know if the thread is waiting for something to be delivered
  // to the application. So in practive the worker thread is always running
  // except when the application is gone
  context_->timeout_timer->ResetIdleTimerDelay();
