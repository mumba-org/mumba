...
ServiceWorkerNetworkProvider::CreateForNavigation(
    int route_id,
    const common::RequestNavigationParams& request_params,
  ...
  bool should_create_provider = true;
  int provider_id = common::kInvalidServiceWorkerProviderId;
  ...
  DCHECK(common::ServiceWorkerUtils::IsBrowserAssignedProviderId(provider_id) ||
         provider_id == common::kInvalidServiceWorkerProviderId);
  if (provider_id == common::kInvalidServiceWorkerProviderId)
...
ServiceWorkerNetworkProvider::CreateForSharedWorker(
    common::mojom::ServiceWorkerProviderInfoForSharedWorkerPtr info,
...
ServiceWorkerNetworkProvider::CreateForController(
    common::mojom::ServiceWorkerProviderInfoForStartWorkerPtr info) {
...
int ServiceWorkerNetworkProvider::provider_id() const {
  if (!context())
    return common::kInvalidServiceWorkerProviderId;
...
ServiceWorkerNetworkProvider::ServiceWorkerNetworkProvider(
    ...
    common::mojom::ControllerServiceWorkerInfoPtr controller_info,
    scoped_refptr<network::SharedURLLoaderFactory> default_loader_factory) {
  DCHECK_NE(provider_id, common::kInvalidServiceWorkerProviderId);
  DCHECK(provider_type == blink::mojom::ServiceWorkerProviderType::kForWindow ||
         provider_type ==
             blink::mojom::ServiceWorkerProviderType::kForSharedWorker);

  common::ServiceWorkerProviderHostInfo host_info(provider_id, route_id, provider_type,
  ...
  if (DomainMainThread::current()) {
    ...
    context_ = base::MakeRefCounted<ServiceWorkerProviderContext>(
        ...
        std::move(default_loader_factory));
    //ChildThread::current()->channel()->GetRemoteAssociatedInterface(
    DomainMainThread::current()->channel()->GetRemoteAssociatedInterface(
        &dispatcher_host_);
