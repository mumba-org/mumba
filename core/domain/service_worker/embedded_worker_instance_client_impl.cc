...
#include "core/domain/service_worker/web_service_worker_installed_scripts_manager_impl.h"
...
void EmbeddedWorkerInstanceClientImpl::WorkerContextDestroyed() {
  ...
  base::ScopedAllowBlockingForTesting scoped_allow_block;
  DCHECK(wrapper_);
...
void EmbeddedWorkerInstanceClientImpl::StartWorker(
  ...
  //DCHECK(common::ChildThreadImpl::current());
  DCHECK(!wrapper_);
  TRACE_EVENT0("ServiceWorker",
  ...
  common::ServiceWorkerContextInstance* instance = domain_thread->GetServiceWorkerContextInstance();
 
  if (!worker_native_client_factory_)
    worker_native_client_factory_ = domain_thread->GetWorkerNativeClientFactory();
...
void EmbeddedWorkerInstanceClientImpl::StopWorker() {
  ...
  //worker_native_client_->OnWorkerTerminate();
  //worker_native_client_.reset();
...
EmbeddedWorkerInstanceClientImpl::StartWorkerContext(
    ...
    service_manager::mojom::InterfaceProviderPtr interface_provider) {
  DCHECK(DomainMainThread::DeprecatedGetMainTaskRunner()->RunsTasksInCurrentSequence());
  //blink::InitializePlatformLanguage();
  // FIXME: FrameFetchContext::RecordLoadingActivity calls V8DOMActivityLogger
  //        which requires a handle scope
  v8::Isolate* isolate = v8::Isolate::GetCurrent();
  //DCHECK(isolate);
  v8::HandleScope v8_handles(isolate);
  ...
  start_data.script_type = static_cast<blink::mojom::ScriptType>(params->script_type);
  start_data.user_agent =
      blink::WebString::FromUTF8(common::GetClient()->GetUserAgent());
  ...
  if (start_data.script_type == blink::mojom::ScriptType::kNative) {
    DCHECK(worker_native_client_factory_.get());
    worker_native_client_ = worker_native_client_factory_->CreateWorkerNativeClient();
    start_data.worker_native_client = worker_native_client_.get();
  }
