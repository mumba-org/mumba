...
network::mojom::URLLoaderFactory* ChildURLLoaderFactoryBundle::GetFactoryForURL(
  ...
  if (it != factories_.end()) {
    //DLOG(INFO) << "found a factory for " << url.scheme() << " scheme on factories list";
    return it->second.get();
  }

  // FIXME: this is suppose to be a temporary hack
  // returning the rpc url loader for customized
  // application schemes
  // NOTE: this will get fixed once the PlaceRegistry
  // register each application url, sending the custom factory
  ...
  if (rpc_it != factories_.end() && !is_default) {
    //DLOG(INFO) << "returnin rpc for " << url.scheme();
    return rpc_it->second.get();
  }

  if (default_factory_) {
    //DLOG(INFO) << "using default_factory for " << url.scheme() << " scheme";
...
void ChildURLLoaderFactoryBundle::CreateLoaderAndStart(
  ...
  //DLOG(INFO) << "ChildURLLoaderFactoryBundle::CreateLoaderAndStart";
  ...
  //DLOG(INFO) << "calling network::mojom::URLLoaderFactory->CreateLoaderAndStart() for url " << request.url;
