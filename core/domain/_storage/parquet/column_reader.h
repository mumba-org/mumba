...
#include "core/shared/domain/storage/parquet/column_page.h"
...
class ColumnReader {
 public:
  ...
  ParquetType::type type() const { return descr_->physical_type(); }
...
static inline void DefinitionLevelsToBitmap(
  ...
  internal::BitmapWriter valid_bits_writer(valid_bits, valid_bits_offset,
                                                    num_def_levels);
    ...
    if (def_levels[i] == max_definition_level) {
      ...
      if (def_levels[i] < max_definition_level) {
        ...
        *null_count += 1;
        ...
        LOG(ERROR) << "definition level exceeds maximum";
      }
    }
...
inline int64_t TypedColumnReader<DType>::ReadBatch(int64_t batch_size,
  ...
  if (descr_->max_repetition_level() > 0 && rep_levels) {
    ...
    if (def_levels && num_def_levels != num_rep_levels) {
      ...
      LOG(ERROR) << "Number of decoded rep / def levels did not match";
    }
  }
  ...
  if (descr_->max_definition_level() > 0) {
    ...
    if (!has_spaced_values) {
      ...
      for (int64_t i = 0; i < total_values; i++) {
        BitUtil::SetBit(valid_bits, valid_bits_offset + i);
      }
...
typedef TypedColumnReader<ParquetBooleanType> BoolReader;
typedef TypedColumnReader<ParquetInt32Type> Int32Reader;
typedef TypedColumnReader<ParquetInt64Type> Int64Reader;
typedef TypedColumnReader<ParquetInt96Type> Int96Reader;
typedef TypedColumnReader<ParquetFloatType> FloatReader;
typedef TypedColumnReader<ParquetDoubleType> DoubleReader;
typedef TypedColumnReader<ParquetByteArrayType> ByteArrayReader;
typedef TypedColumnReader<ParquetFLBAType> FixedLenByteArrayReader;

extern template class TypedColumnReader<ParquetBooleanType>;
extern template class TypedColumnReader<ParquetInt32Type>;
extern template class TypedColumnReader<ParquetInt64Type>;
extern template class TypedColumnReader<ParquetInt96Type>;
extern template class TypedColumnReader<ParquetFloatType>;
extern template class TypedColumnReader<ParquetDoubleType>;
extern template class TypedColumnReader<ParquetByteArrayType>;
extern template class TypedColumnReader<ParquetFLBAType>;
