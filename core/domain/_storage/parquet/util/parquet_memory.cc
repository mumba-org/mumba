...
#include "data/util/logging.h"

//using arrowMemoryPool;
...
io::FileInterface* ArrowInputFile::file_interface() { return file_.get(); }
...
ArrowOutputStream::ArrowOutputStream(
    const std::shared_ptr<io::OutputStream> file)
    : file_(file) {}

io::FileInterface* ArrowOutputStream::file_interface() { return file_.get(); }
...
InMemoryInputStream::InMemoryInputStream(RandomAccessSource* source, int64_t start,
  ...
  if (buffer_->size() < num_bytes) {
    ...
    LOG(ERROR) << "Unable to read column chunk data";
  }
  len_ = buffer_->size();
}
  ...
  *num_bytes = std::min(num_to_peek, stream_end_ - stream_offset_);
  ...
  if (*num_bytes > (buffer_size_ - buffer_offset_)) {
    ...
    if (bytes_read < *num_bytes) {
      ...
      LOG(ERROR) << "Failed reading column data from source";
    }
    buffer_offset_ = 0;
  }
