...
#include "data/io/interfaces.h"
#include "data/io/memory.h"
#include "data/memory_pool.h"
#include "data/status.h"
#include "data/util/compression.h"
...
static inline std::unique_ptr<Codec> GetCodecFromArrow(Compression::type codec) {
  std::unique_ptr<Codec> result;
  switch (codec) {
    case Compression::UNCOMPRESSED:
      break;
    case Compression::SNAPPY:
      PARQUET_THROW_NOT_OK(Codec::Create(Compression::SNAPPY, &result));
      break;
    case Compression::GZIP:
      PARQUET_THROW_NOT_OK(Codec::Create(Compression::GZIP, &result));
      break;
    //case Compression::LZO:
    //  PARQUET_THROW_NOT_OK(Codec::Create(Compression::LZO, &result));
    //  break;
    case Compression::BROTLI:
      PARQUET_THROW_NOT_OK(Codec::Create(Compression::BROTLI, &result));
      break;
    //case Compression::LZ4:
    //  PARQUET_THROW_NOT_OK(Codec::Create(Compression::LZ4, &result));
    //  break;
    case Compression::ZSTD:
      PARQUET_THROW_NOT_OK(Codec::Create(Compression::ZSTD, &result));
      break;
    default:
      break;
  }
...
//using ResizableBuffer = ResizableBuffer;
//using PoolBuffer = ::PoolBuffer;

template <class T>
class Vector {
 public:
  explicit Vector(int64_t size, MemoryPool* pool);
...
class ChunkedAllocator {
 public:
  explicit ChunkedAllocator(MemoryPool* pool = default_memory_pool());
...
// File input and output interfaces that translate arrowStatus to exceptions
...
/// resources
class RandomAccessSource : virtual public FileInterface {
 public:
...
class OutputStream : virtual public FileInterface {
 public:
...
class ArrowFileMethods : virtual public FileInterface {
 public:
 ...
 protected:
  virtual io::FileInterface* file_interface() = 0;
};

/// This interface depends on the threadsafety of the underlying Arrow file interface
class ArrowInputFile : public ArrowFileMethods, public RandomAccessSource {
 public:
  ...
  std::shared_ptr<io::ReadableFileInterface> file() const { return file_; }
 ...
 private:
  io::FileInterface* file_interface() override;
  std::shared_ptr<io::ReadableFileInterface> file_;
};

class ArrowOutputStream : public ArrowFileMethods, public OutputStream {
 public:
  explicit ArrowOutputStream(const std::shared_ptr<io::OutputStream> file);

  // Copy bytes into the output stream
  void Write(const uint8_t* data, int64_t length) override;

  std::shared_ptr<io::OutputStream> file() { return file_; }
 ...
 private:
  io::FileInterface* file_interface() override;
  std::shared_ptr<io::OutputStream> file_;
};
...
std::shared_ptr<PoolBuffer> PARQUET_EXPORT AllocateBuffer(MemoryPool* pool,
                                                          int64_t size = 0);

std::unique_ptr<PoolBuffer> PARQUET_EXPORT AllocateUniqueBuffer(MemoryPool* pool,
                                                                int64_t size = 0);
