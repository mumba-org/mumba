...
#include "core/shared/domain/storage/parquet/schema-internal.h"
...
PrimitiveNode::PrimitiveNode(const std::string& name, Repetition::type repetition,
                             ParquetType::type type, LogicalType::type logical_type, int length,
  ...
  switch (logical_type) {
    ...
    case LogicalType::DECIMAL:
      if ((type != ParquetType::INT32) && (type != ParquetType::INT64) && (type != ParquetType::BYTE_ARRAY) &&
    ...
    case LogicalType::INTERVAL:
      if ((type != ParquetType::FIXED_LEN_BYTE_ARRAY) || (length != 12)) {
  ...
  if (type == ParquetType::FIXED_LEN_BYTE_ARRAY) {
    if (length <= 0) {
...
bool PrimitiveNode::EqualsInternal(const PrimitiveNode* other) const {
  ...
  if (physical_type_ == ParquetType::FIXED_LEN_BYTE_ARRAY) {
