...
#include "core/shared/domain/storage/parquet/encoding-internal.h"
#include "core/shared/domain/storage/parquet/types.h"
#include "core/shared/domain/storage/parquet/properties.h"
//#include "core/shared/domain/storage/parquet/thrift.h"
...
int LevelDecoder::SetData(Encoding::type encoding, int16_t max_level,
  ...
  switch (encoding) {
    ...
    LOG(ERROR) << "Unknown encoding type for levels.";
  }
  return -1;
}
...
void TypedColumnReader<DType>::ConfigureDictionary(const DictionaryPage* page) {
  ...
  if (it != decoders_.end()) {
    ...
    LOG(ERROR) << "Column cannot have more than one dictionary.";
  }
  ...
  if (page->encoding() == Encoding::PLAIN_DICTIONARY ||
    ...
    LOG(ERROR) << "only plain dictionary encoding has been implemented";
  }
    ...
    if (current_page_->type() == PageType::DICTIONARY_PAGE) {
      ...
      if (it != decoders_.end()) {
        ...
        switch (encoding) {
          ...
          LOG(ERROR) << "Dictionary page must be before data page.";
...
template class PARQUET_TEMPLATE_EXPORT TypedColumnReader<ParquetBooleanType>;
template class PARQUET_TEMPLATE_EXPORT TypedColumnReader<ParquetInt32Type>;
template class PARQUET_TEMPLATE_EXPORT TypedColumnReader<ParquetInt64Type>;
template class PARQUET_TEMPLATE_EXPORT TypedColumnReader<ParquetInt96Type>;
template class PARQUET_TEMPLATE_EXPORT TypedColumnReader<ParquetFloatType>;
template class PARQUET_TEMPLATE_EXPORT TypedColumnReader<ParquetDoubleType>;
template class PARQUET_TEMPLATE_EXPORT TypedColumnReader<ParquetByteArrayType>;
template class PARQUET_TEMPLATE_EXPORT TypedColumnReader<ParquetFLBAType>;
