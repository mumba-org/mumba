...
#include "core/shared/domain/storage/parquet/parquet_version.h"
...
class ReaderProperties {
 public:
  explicit ReaderProperties(MemoryPool* pool = default_memory_pool())
      : pool_(pool) {
...
class WriterProperties {
   ...
   public:
    ...
    Builder* encoding(Encoding::type encoding_type) {
      if (encoding_type == Encoding::PLAIN_DICTIONARY ||
        ...
        LOG(ERROR) << "Can't use dictionary encoding as fallback encoding";
        return nullptr;
      }
 ...
 private:
  explicit WriterProperties(
      MemoryPool* pool, int64_t dictionary_pagesize_limit,
