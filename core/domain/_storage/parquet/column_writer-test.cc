...
#include "core/shared/domain/storage/parquet/column_writer.h"
#include "core/shared/domain/storage/parquet/parquet_types.h"
#include "core/shared/domain/storage/parquet/test-specialization.h"
#include "core/shared/domain/storage/parquet/test-util.h"
...
TYPED_TEST(TestPrimitiveWriter, OptionalSpaced) {
  ...
  this->GenerateData(SMALL_SIZE);
  std::vector<int16_t> definition_levels(SMALL_SIZE, 1);
  std::vector<uint8_t> valid_bits(BitUtil::BytesForBits(SMALL_SIZE), 255);

  definition_levels[SMALL_SIZE - 1] = 0;
  BitUtil::ClearBit(valid_bits.data(), SMALL_SIZE - 1);
  definition_levels[1] = 0;
  BitUtil::ClearBit(valid_bits.data(), 1);
