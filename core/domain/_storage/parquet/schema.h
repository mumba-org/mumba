...
class PrimitiveNode : public Node {
 public:
  ...
  ParquetType::type physical_type() const { return physical_type_; }
 ...
 private:
  PrimitiveNode(const std::string& name, Repetition::type repetition, ParquetType::type type,
                LogicalType::type logical_type = LogicalType::NONE, int length = -1,
                int precision = -1, int scale = -1, int id = -1);

  ParquetType::type physical_type_;
...
#define PRIMITIVE_FACTORY(FuncName, TYPE)                                              \
  static inline NodePtr FuncName(const std::string& name,                              \
    ...
    return PrimitiveNode::Make(name, repetition, ParquetType::TYPE);                          \
  }
...
class ColumnDescriptor {
 public:
  ...
  ParquetType::type physical_type() const { return primitive_node_->physical_type(); }
...
// Domain for the converted Parquet schema with a computed information from
