...
#include "data/util/bit-stream-utils.h"
#include "data/util/bit-util.h"
#include "data/util/cpu-info.h"
#include "data/util/hash-util.h"
#include "data/util/macros.h"
#include "data/util/rle-encoding.h"

#include "core/shared/domain/storage/parquet/encoding.h"
...
#include "core/shared/domain/storage/parquet/util/parquet_memory.h"

namespace domain {

//namespace BitUtil = ::BitUtil;
//using HashUtil = ::HashUtil;
...
class PlainDecoder : public Decoder<DType> {
 public:
  ...
  explicit PlainDecoder(const ColumnDescriptor* descr)
    ...
    if (descr_ && descr_->physical_type() == ParquetType::FIXED_LEN_BYTE_ARRAY) {
...
template <>
class PlainDecoder<ParquetBooleanType> : public Decoder<ParquetBooleanType> {
 public:
  explicit PlainDecoder(const ColumnDescriptor* descr)
      : Decoder<ParquetBooleanType>(descr, Encoding::PLAIN) {}
...
template <>
class PlainEncoder<ParquetBooleanType> : public Encoder<ParquetBooleanType> {
 public:
  explicit PlainEncoder(const ColumnDescriptor* descr,
                        MemoryPool* pool = default_memory_pool())
      : Encoder<ParquetBooleanType>(descr, Encoding::PLAIN, pool),
...
template <>
inline void PlainEncoder<ParquetByteArrayType>::Put(const ByteArray* src, int num_values) {
  for (int i = 0; i < num_values; ++i) {
...
template <>
inline void PlainEncoder<ParquetFLBAType>::Put(const FixedLenByteArray* src, int num_values) {
  for (int i = 0; i < num_values; ++i) {
    // Write the result to the output stream
...
template <>
inline void DictionaryDecoder<ParquetBooleanType>::SetDict(Decoder<ParquetBooleanType>* dictionary) {
  //ParquetException::NYI("Dictionary encoding is not implemented for boolean values");
  LOG(ERROR) << "Dictionary encoding is not implemented for boolean values";
}

template <>
inline void DictionaryDecoder<ParquetByteArrayType>::SetDict(
    Decoder<ParquetByteArrayType>* dictionary) {
  int num_dictionary_values = dictionary->values_left();
  dictionary_.Resize(num_dictionary_values);
  dictionary->Decode(&dictionary_[0], num_dictionary_values);
...
template <>
inline void DictionaryDecoder<ParquetFLBAType>::SetDict(Decoder<ParquetFLBAType>* dictionary) {
  int num_dictionary_values = dictionary->values_left();
  dictionary_.Resize(num_dictionary_values);
  dictionary->Decode(&dictionary_[0], num_dictionary_values);
...
class DictEncoder : public Encoder<DType> {
 public:
  ...
  void PutSpaced(const T* src, int num_values, const uint8_t* valid_bits,
                 int64_t valid_bits_offset) override {
    internal::BitmapReader valid_bits_reader(valid_bits, valid_bits_offset,
                                                      num_values);
...
template <>
inline int DictEncoder<ParquetByteArrayType>::Hash(const ByteArray& value) const {
  if (value.len > 0) {
...
template <>
inline int DictEncoder<ParquetFLBAType>::Hash(const FixedLenByteArray& value) const {
  if (type_length_ > 0) {
    DCHECK_NE(nullptr, value.ptr) << "Value ptr cannot be NULL";
  }
...
template <>
inline bool DictEncoder<ParquetFLBAType>::SlotDifferent(const FixedLenByteArray& v,
                                                 hash_slot_t slot) {
...
template <>
inline void DictEncoder<ParquetByteArrayType>::AddDictKey(const ByteArray& v) {
...
template <>
inline void DictEncoder<ParquetFLBAType>::AddDictKey(const FixedLenByteArray& v) {
...
template <>
inline void DictEncoder<ParquetBooleanType>::WriteDict(uint8_t* buffer) {
  // For primitive types, only a memcpy
  // memcpy(buffer, uniques_.data(), sizeof(typename DType::c_type) * uniques_.size());
...
template <>
inline void DictEncoder<ParquetByteArrayType>::WriteDict(uint8_t* buffer) {
...
template <>
inline void DictEncoder<ParquetFLBAType>::WriteDict(uint8_t* buffer) {
...
class DeltaBitPackDecoder : public Decoder<DType> {
 public:
  ...
  explicit DeltaBitPackDecoder(const ColumnDescriptor* descr,
    ...
    if (DType::type_num != ParquetType::INT32 && DType::type_num != ParquetType::INT64) {
      //throw ParquetException("Delta bit pack encoding should only be for integer data.");
      LOG(ERROR) << "Delta bit pack encoding should only be for integer data.";
    }
  }
...
class DeltaLengthByteArrayDecoder : public Decoder<ParquetByteArrayType> {
 public:
  explicit DeltaLengthByteArrayDecoder(
      const ColumnDescriptor* descr,
      MemoryPool* pool = default_memory_pool())
      : Decoder<ParquetByteArrayType>(descr, Encoding::DELTA_LENGTH_BYTE_ARRAY),
 ...
 private:
  using Decoder<ParquetByteArrayType>::num_values_;
  DeltaBitPackDecoder<ParquetInt32Type> len_decoder_;
  const uint8_t* data_;
  int len_;
};
...
class DeltaByteArrayDecoder : public Decoder<ParquetByteArrayType> {
 public:
  explicit DeltaByteArrayDecoder(
      const ColumnDescriptor* descr,
      MemoryPool* pool = default_memory_pool())
      : Decoder<ParquetByteArrayType>(descr, Encoding::DELTA_BYTE_ARRAY),
 ...
 private:
  ...
  DeltaBitPackDecoder<ParquetInt32Type> prefix_len_decoder_;
