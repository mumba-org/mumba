...
#include "core/shared/domain/storage/parquet/file_reader.h"
...
std::unique_ptr<ParquetFileReader> ParquetFileReader::OpenFile(
    ...
    const std::shared_ptr<FileMetaData>& metadata) {
  std::shared_ptr<io::ReadableFileInterface> source;
  if (memory_map) {
    std::shared_ptr<io::ReadableFile> handle;
    PARQUET_THROW_NOT_OK(
        io::ReadableFile::Open(path, props.memory_pool(), &handle));
    source = handle;
  } else {
    std::shared_ptr<io::MemoryMappedFile> handle;
    PARQUET_THROW_NOT_OK(
        io::MemoryMappedFile::Open(path, io::FileMode::READ, &handle));
    source = handle;
  }
