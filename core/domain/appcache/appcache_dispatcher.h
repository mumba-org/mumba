...
#ifndef CORE_DOMAIN_APPCACHE_APPCACHE_DISPATCHER_H_
#define CORE_DOMAIN_APPCACHE_APPCACHE_DISPATCHER_H_
...
#include "core/domain/appcache/appcache_backend_proxy.h"
...
class AppCacheDispatcher : public common::mojom::AppCacheFrontend {
 public:
  explicit AppCacheDispatcher(common::AppCacheFrontend* frontend);
  ~AppCacheDispatcher() override;

  void Bind(common::mojom::AppCacheFrontendRequest request);
 ...
 private:
  ...
  void CacheSelected(int32_t host_id, common::mojom::AppCacheInfoPtr info) override;
  ...
  void ErrorEventRaised(const std::vector<int32_t>& host_ids,
                        common::mojom::AppCacheErrorDetailsPtr details) override;
  ...
  std::unique_ptr<common::AppCacheFrontend> frontend_;
  mojo::Binding<common::mojom::AppCacheFrontend> binding_;
};

}  // namespace content

#endif  // CORE_DOMAIN_APPCACHE_APPCACHE_DISPATCHER_H_
