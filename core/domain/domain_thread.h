...
class CONTENT_EXPORT DomainThread {
 public:
  ...
  // Note: see related ContentDomainClient::PostAfterStartupTask.
  ...
  // Sets the delegate for DomainThread::IO.
  //
  ...
  // DomainThreadImpl(DomainThread::IO) is created and unregistered after
  // it was destroyed and its underlying thread shutdown.
  static void SetIOThreadDelegate(DomainThreadDelegate* delegate);
  ...
  // bind to SequencedTaskRunner instead of specific DomainThreads.
...
#endif  // MUMBA_DOMAIN_DOMAIN_THREAD_H_
