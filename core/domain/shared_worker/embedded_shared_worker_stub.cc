...
#include "core/domain/shared_worker/embedded_shared_worker_stub.h"
...
#include "core/domain/service_worker/worker_fetch_context_impl.h"
 ...
 public:
  SharedWorkerWebApplicationCacheHostImpl(
      ...
      : WebApplicationCacheHostImpl(
            client,
            DomainMainThread::current()->appcache_dispatcher()->backend_proxy(),
            kAppCacheNoHostId) {}
...
class WebServiceWorkerNetworkProviderForSharedWorker
 ...
 public:
  ...
  std::unique_ptr<blink::WebURLLoader> CreateURLLoader(
    ...
    DomainMainThread* render_thread = DomainMainThread::current();
    // DomainMainThread is nullptr in some tests.
    if (!render_thread) {
      return nullptr;
    }
    ...
    // TODO(crbug.com/796425): Temporarily wrap the raw common::mojom::URLLoaderFactory
    // pointer into SharedURLLoaderFactory.
    return std::make_unique<WebURLLoaderImpl>(
        DomainMainThread::current()->resource_dispatcher(),
...
EmbeddedSharedWorkerStub::EmbeddedSharedWorkerStub(
    ...
    : binding_(this, std::move(request)),
      ...
      name_(info->name),
      url_(info->url) {
  DomainProcess::current()->AddRefProcess();
...
EmbeddedSharedWorkerStub::~EmbeddedSharedWorkerStub() {
  ...
  DomainProcess::current()->ReleaseProcess();
}
...
EmbeddedSharedWorkerStub::CreateServiceWorkerNetworkProvider() {
  ...
  if (DomainMainThread* render_thread = DomainMainThread::current()) {
  ...
  return std::make_unique<WebServiceWorkerNetworkProviderForSharedWorker>(
      std::move(provider), common::IsOriginSecure(url_));
}
...
void EmbeddedSharedWorkerStub::WaitForServiceWorkerControllerInfo(
  ...
  //DLOG(INFO) << "EmbeddedSharedWorkerStub::WaitForServiceWorkerControllerInfo";
...
EmbeddedSharedWorkerStub::CreateWorkerFetchContext(
  ...
  ServiceWorkerProviderContext* context =
      static_cast<WebServiceWorkerNetworkProviderForSharedWorker*>(
          ...
          ->provider()
          ->context();
  common::mojom::ServiceWorkerWorkerClientRequest request =
  ...
  auto worker_fetch_context = std::make_unique<WorkerFetchContextImpl>(
      ...
      // common::GetClient()->renderer()->CreateURLLoaderThrottleProvider(
      //     URLLoaderThrottleProviderType::kWorker),
      // common::GetClient()
      //     ->renderer()
      //     ->CreateWebSocketHandshakeThrottleProvider(),
      DomainMainThread::current()->thread_safe_sender());//,
      //DomainMainThread::current()->window_dispatcher());
  ...
  worker_fetch_context->set_is_secure_context(common::IsOriginSecure(url_));
...
void EmbeddedSharedWorkerStub::Connect(int connection_request_id,
  ...
  if (running_) {
    ...
    // common::mojom::SharedWorker::Connect() for one of the documents can come in
