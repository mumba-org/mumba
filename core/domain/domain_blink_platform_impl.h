// Copyright 2015 The Chromium Authors. All rights reserved.
// Copyright 2021 Mumba. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef CORE_DOMAIN_DOMAIN_BLINK_PLATFORM_IMPL_H_
#define CORE_DOMAIN_DOMAIN_BLINK_PLATFORM_IMPL_H_

#include <memory>

#include "base/macros.h"
#include "core/domain/blink_platform_impl.h"

namespace blink {
namespace scheduler {
class WebThreadBase;
}
}

namespace domain {

class DomainBlinkPlatformImpl : public BlinkPlatformImpl {
 public:
  DomainBlinkPlatformImpl();
  ~DomainBlinkPlatformImpl() override;

  // BlinkPlatformImpl implementation.
  blink::WebThread* CurrentThread() override;

 private:
  std::unique_ptr<blink::scheduler::WebThreadBase> main_thread_;

  DISALLOW_COPY_AND_ASSIGN(DomainBlinkPlatformImpl);
};

}  // namespace domain

#endif  // CORE_DOMAIN_DOMAIN_BLINK_PLATFORM_IMPL_H_
