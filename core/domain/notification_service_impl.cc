...
#include "core/domain/notification_observer.h"
#include "core/domain/notification_types.h"

namespace domain {

namespace {

base::LazyInstance<base::ThreadLocalPointer<NotificationServiceImpl>>::DestructorAtExit
...
void NotificationServiceImpl::RemoveObserver(NotificationObserver* observer,
  ...
  // future.
  // NOTE: when this check shows crashes, use HostThread::DeleteOnIOThread or
...
void NotificationServiceImpl::Notify(int type,
  ...
  if (HasKey(observers_[NOTIFICATION_ALL], AllSources()) &&
      source != AllSources()) {
    for (NotificationObserver& obs : *observers_[NOTIFICATION_ALL][AllSources().map_key()]) {
      obs.Observe(type, source, details);
    }
  }

  // Notify observers of all types and the given source
  if (HasKey(observers_[NOTIFICATION_ALL], source)) {
    for (NotificationObserver& obs : *observers_[NOTIFICATION_ALL][source.map_key()]) {
      obs.Observe(type, source, details);
    }
  }
  ...
  if (HasKey(observers_[type], AllSources()) &&
      source != AllSources()) {
    for (NotificationObserver& obs : *observers_[type][AllSources().map_key()]) {
      obs.Observe(type, source, details);
    }
  }
  ...
  if (HasKey(observers_[type], source)) {
    for (NotificationObserver& obs : *observers_[type][source.map_key()]) {
      obs.Observe(type, source, details);
    }                  
  }
}
