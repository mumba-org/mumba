...
// This disconnects from the window server, and then indicates that Mumba
...
// running a shell needs to also be reflected in chrome_main.cc for
...
bool DomainMainPlatformDelegate::EnableSandbox() {
  ...
  // The sandbox is now engaged. Make sure that the shell has not connected
  // itself to Cocoa.
  CHECK(NSApp == nil);
