...
#include "core/utility/utility_thread.h"
...
class ContentCdmServiceClient final : public media::CdmService::Client {
 public:
  ...
  void AddCdmHostFilePaths(
    ...
    common::GetClient()->AddContentDecryptionModules(nullptr,
                                             cdm_host_file_paths);
  }
#endif  // BUILDFLAG(ENABLE_CDM_HOST_VERIFICATION)
};
...
void UtilityServiceFactory::RegisterServices(ServiceMap* services) {
  common::GetClient()->utility()->RegisterServices(services);
  ...
  if (base::FeatureList::IsEnabled(network::features::kNetworkService)) {
    common::GetClient()->utility()->RegisterNetworkBinders(
    ...
    network_info.task_runner = common::ChildProcess::current()->io_task_runner();
    services->insert(
        std::make_pair(common::mojom::kNetworkServiceName, network_info));
  }
}
