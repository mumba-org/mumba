...
#include "core/gpu/gpu_sandbox_hook_linux.h"
...
int GpuMain(const common::MainParams& parameters) {
  ...
  base::trace_event::TraceLog::GetInstance()->SetProcessSortIndex(
      common::kTraceEventGpuProcessSortIndex);
  ...
  if (command_line.HasSwitch(switches::kHeadless)) {
    ...
    if (!gfx::GetXDisplay()) {
      return common::RESULT_CODE_GPU_DEAD_ON_ARRIVAL;
    }
...
#error "Unsupported Linux platform."
  ...
  common::GetClient()->SetGpuInfo(gpu_init->gpu_info());
  ...
  auto* client = common::GetClient()->gpu();
  if (client) {
  ...
  if (command_line.HasSwitch(switches::kEnableOOPRasterization)) {
    ...
    SkFontConfigInterface::SetGlobal(new common::FontConfigIPC(common::GetSandboxFD()))
        ->unref();
#endif
  }
  ...
  return dead_on_arrival ? common::RESULT_CODE_GPU_DEAD_ON_ARRIVAL : 0;
}
...
bool StartSandboxLinux(gpu::GpuWatchdogThread* watchdog_thread,
  ...
  sandbox_options.accelerated_video_decode_enabled = true;
      //!gpu_prefs.disable_accelerated_video_decode;
  sandbox_options.accelerated_video_encode_enabled = true;
      //!gpu_prefs.disable_accelerated_video_encode;

  //DLOG(INFO) << "gpu::StartSandboxLinux: InitializeSandbox()";
