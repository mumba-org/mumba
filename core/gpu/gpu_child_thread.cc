...
#include "core/gpu/gpu_child_thread.h"
...
#include "core/gpu/gpu_service_factory.h"
...
common::ChildThreadImpl::Options GetOptions() {
  common::ChildThreadImpl::Options::Builder builder;
...
// Release() is called.
class QueueingConnectionFilter : public common::ConnectionFilter {
 public:
...
viz::VizMainImpl::ExternalDependencies CreateVizMainDependencies(
  ...
  if (common::GetClient()->gpu())
    deps.sync_point_manager = common::GetClient()->gpu()->GetSyncPointManager();
  auto* process = common::ChildProcess::current();
...
GpuChildThread::GpuChildThread(const common::InProcessChildThreadParams& params,
...
GpuChildThread::GpuChildThread(const common::ChildThreadImpl::Options& options,
                               std::unique_ptr<gpu::GpuInit> gpu_init)
    : common::ChildThreadImpl(options),
      viz_main_(this,
...
void GpuChildThread::Init(const base::Time& process_start_time) {
  ...
  if (!in_process_gpu()) {
    media::SetMediaDrmBridgeClient(
        common::GetClient()->GetMediaDrmBridgeClient());
  }
#endif
  ...
  if (common::GetClient()->gpu())  // nullptr in tests.
    common::GetClient()->gpu()->InitializeRegistry(registry.get());
...
void GpuChildThread::OnGpuServiceConnection(viz::GpuServiceImpl* gpu_service) {
  ...
  if (common::GetClient()->gpu()) {  // NULL in tests.
    common::GetClient()->gpu()->GpuServiceInitialized(
        gpu_service->gpu_preferences());
  }
...
void GpuChildThread::PostCompositorThreadCreated(
  ...
  auto* gpu_client = common::GetClient()->gpu();
  if (gpu_client)
...
std::unique_ptr<media::AndroidOverlay> GpuChildThread::CreateAndroidOverlay(
  ...
  if (main_task_runner->RunsTasksInCurrentSequence()) {
    ChildThread::Get()->GetConnector()->BindInterface(
        common::mojom::kShellServiceName, &overlay_provider);
  } else {
    // Create a connector on this sequence and bind it on the main thread.
    service_manager::mojom::ConnectorRequest request;
    auto connector = service_manager::Connector::Create(&request);
    connector->BindInterface(common::mojom::kShellServiceName,
                             &overlay_provider);
