...
class CONTENT_EXPORT ServiceWorkerUtils {
 public:
  ...
  CONTENT_EXPORT static bool ContainsDisallowedCharacter(const GURL& scope,
                                          const GURL& script_url,
                                          std::string* error_message);
  ...
  CONTENT_EXPORT static std::string ErrorTypeToString(
      blink::mojom::ServiceWorkerErrorType error);

  CONTENT_EXPORT static std::string ClientTypeToString(
  ...
  // overflow.
  CONTENT_EXPORT static bool ExtractSinglePartHttpRange(const net::HttpRequestHeaders& headers,
                                         bool* has_range_out,
                                         uint64_t* offset_out,
                                         uint64_t* size_out);

  CONTENT_EXPORT static bool ShouldBypassCacheDueToUpdateViaCache(
      bool is_main_script,
