...
class CONTENT_EXPORT ServiceWorkerLoaderHelpers {
 public:
  // Populates |out_head->headers| with the given |status_code|, |status_text|,
  // and |headers|.
  CONTENT_EXPORT static void SaveResponseHeaders(const int status_code,
                                  const std::string& status_text,
                                  const ServiceWorkerHeaderMap& headers,
                                  network::ResourceResponseHead* out_head);
  // Populates |out_head| (except for headers) with given |response|.
  CONTENT_EXPORT static void SaveResponseInfo(const ServiceWorkerResponse& response,
                               network::ResourceResponseHead* out_head);

  // Returns a redirect info if |response_head| is an redirect response.
  // Otherwise returns base::nullopt.
  CONTENT_EXPORT static base::Optional<net::RedirectInfo> ComputeRedirectInfo(
  ...
  CONTENT_EXPORT static int ReadBlobResponseBody(
  ...
  CONTENT_EXPORT static scoped_refptr<network::ResourceRequestBody> CloneResourceRequestBody(
