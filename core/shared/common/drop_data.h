...
#ifndef CORE_COMMON_DROP_DATA_H_
#define CORE_COMMON_DROP_DATA_H_
...
#if defined (OS_WIN)
enum class DropDataMetadataKind {
#else
enum CONTENT_EXPORT class DropDataMetadataKind {
#endif
  STRING = 0,
  FILENAME,
  FILESYSTEMFILE,
...
struct CONTENT_EXPORT DropDataMetadata {
  DropDataMetadata();
  static DropDataMetadata CreateForMimeType(const DropDataMetadataKind& kind,
                                    const base::string16& mime_type);
  static DropDataMetadata CreateForFilePath(const base::FilePath& filename);
  static DropDataMetadata CreateForFileSystemUrl(const GURL& file_system_url);
  DropDataMetadata(const DropDataMetadata& other);
  ~DropDataMetadata();

  DropDataMetadataKind kind;
...
#endif  // CORE_COMMON_DROP_DATA_H_
