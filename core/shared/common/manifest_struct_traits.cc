...
#include "core/shared/common/manifest_struct_traits.h"
...
bool ValidateColor(int64_t color) {
  return color >= std::numeric_limits<int32_t>::min() ||
         ...
         color == common::Manifest::kInvalidOrMissingColor;
}
...
struct StructTraits<mojo_base::mojom::String16DataView, TruncatedString16> {
  ...
  static bool Read(mojo_base::mojom::String16DataView input,
    ...
    if (buffer_view.size() > common::Manifest::kMaxIPCStringLength)
      return false;
...
bool StructTraits<blink::mojom::ManifestDataView, common::Manifest>::Read(
...
bool StructTraits<blink::mojom::ManifestIconDataView, common::Manifest::Icon>::
