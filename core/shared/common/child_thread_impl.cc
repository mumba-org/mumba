...
class ChannelBootstrapFilter : public ConnectionFilter {
 ...
 private:
  ...
  void OnBindInterface(const service_manager::BindSourceInfo& source_info,
    ...
    if (source_info.identity.name() != mojom::kHostServiceName)
      return;
...
ChildThreadImpl::Options::Builder&
ChildThreadImpl::Options::Builder::ManualInit(bool manual_init) {
  options_.manual_init = manual_init;
  return *this;
}
...
ChildThreadImpl::ChildThreadImpl()
    : options_(Options::Builder().Build()),
...
void ChildThreadImpl::OnFieldTrialGroupFinalized(
  ...
  GetConnector()->BindInterface(mojom::kHostServiceName,
...
void ChildThreadImpl::Init() {
  ...
  if (!IsInHostProcess()) {
    ...
    service_request_pipe =
        options_.broker_client_invitation->ExtractInProcessMessagePipe(
            options_.in_process_service_request_token);
  }
  ...
  if (command_line.HasSwitch(switches::kGpuProcess) || 
    ...
    command_line.HasSwitch(switches::kDomainProcess) ||
  ...
  // Add filters passed here via options_.
  for (auto* startup_filter : options_.startup_filters) {
  ...
  if (options_.auto_start_service_manager_connection &&
...
void ChildThreadImpl::StartServiceManagerConnection() {
  ...
  GetClient()->OnServiceManagerConnected(
...
bool ChildThreadImpl::IsInHostProcess() const {
