...
template <>
struct CONTENT_EXPORT ParamTraits<common::DragEventSourceInfo> {
  typedef common::DragEventSourceInfo param_type;
...
template <>
struct CONTENT_EXPORT ParamTraits<common::TextInputState> {
  typedef common::TextInputState param_type;
  static void Write(base::Pickle* m, const param_type& p);
  static bool Read(const base::Pickle* m,
                   base::PickleIterator* iter,
                   param_type* r);
...
template <>
struct CONTENT_EXPORT ParamTraits<blink::WebDragOperation> {
  typedef blink::WebDragOperation param_type;
  static void Write(base::Pickle* m, const param_type& p) {
    m->WriteUInt32(p);
  }
  static bool Read(const base::Pickle* m,
    ...
    *r = static_cast<blink::WebDragOperation>(value);
    
    return true;
  }
  static void Log(const param_type& p, std::string* l) {
    l->append("<WebDragOperation");
  }
};

template <>
struct CONTENT_EXPORT ParamTraits<common::WebCursor> {
...
// template <>
// struct CONTENT_EXPORT ParamTraits<common::FrameMsg_ViewChanged_Params> {
//   using param_type = common::FrameMsg_ViewChanged_Params;
//   static void Write(base::Pickle* m, const param_type& p);
//   static bool Read(const base::Pickle* m,
//                    base::PickleIterator* iter,
//                    param_type* p);
