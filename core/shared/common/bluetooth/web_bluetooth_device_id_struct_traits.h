...
#include "core/shared/common/bluetooth/web_bluetooth_device_id.h"
...
struct StructTraits<blink::mojom::WebBluetoothDeviceIdDataView,
                    common::WebBluetoothDeviceId> {
  static const std::string& device_id(
      const common::WebBluetoothDeviceId& device_id) {
    return device_id.str();
  }

  static bool Read(blink::mojom::WebBluetoothDeviceIdDataView input,
                   common::WebBluetoothDeviceId* output) {
    std::string result;

    if (!input.ReadDeviceId(&result))
      return false;
    if (!common::WebBluetoothDeviceId::IsValid(result))
      return false;

    *output = common::WebBluetoothDeviceId(std::move(result));
    return true;
  }
};
