...
CloneableMessage CloneableMessage::ShallowClone() const {
  ...
  for (const auto& blob : blobs) {
    ...
    blink::mojom::BlobPtr blob_proxy(
        blink::mojom::BlobPtrInfo(std::move(handle), blob->blob.version()));
    blink::mojom::BlobPtrInfo blob_clone_info;
    blob_proxy->Clone(MakeRequest(&blob_clone_info));
    clone.blobs.push_back(
        blink::mojom::SerializedBlob::New(blob->uuid, blob->content_type, blob->size,
