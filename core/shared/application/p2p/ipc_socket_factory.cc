...
#include "core/shared/application/p2p/host_address_request.h"
...
bool IsTcpClientSocket(common::P2PSocketType type) {
  return (type == common::P2P_SOCKET_STUN_TCP_CLIENT) ||
         (type == common::P2P_SOCKET_TCP_CLIENT) ||
         (type == common::P2P_SOCKET_STUN_SSLTCP_CLIENT) ||
         (type == common::P2P_SOCKET_SSLTCP_CLIENT) ||
         (type == common::P2P_SOCKET_TLS_CLIENT) ||
         (type == common::P2P_SOCKET_STUN_TLS_CLIENT);
}

bool JingleSocketOptionToP2PSocketOption(rtc::Socket::Option option,
                                         common::P2PSocketOption* ipc_option) {
  switch (option) {
    case rtc::Socket::OPT_RCVBUF:
      *ipc_option = common::P2P_SOCKET_OPT_RCVBUF;
      break;
    case rtc::Socket::OPT_SNDBUF:
      *ipc_option = common::P2P_SOCKET_OPT_SNDBUF;
      break;
    case rtc::Socket::OPT_DSCP:
      *ipc_option = common::P2P_SOCKET_OPT_DSCP;
      break;
...
class IpcPacketSocket : public rtc::AsyncPacketSocket,
 ...
 public:
  ...
  bool Init(common::P2PSocketType type,
 ...
 private:
  ...
  int DoSetOption(common::P2PSocketOption option, int value);
  ...
  int error_;
  int options_[common::P2P_SOCKET_OPT_MAX];
...
IpcPacketSocket::IpcPacketSocket()
  ...
  std::fill_n(options_, static_cast<int> (common::P2P_SOCKET_OPT_MAX),
...
bool IpcPacketSocket::Init(common::P2PSocketType type,
                           P2PSocketClientImpl* client,
                           const rtc::SocketAddress& local_address,
                           uint16_t min_port,
                           uint16_t max_port,
  ...
  common::P2PHostAndIPEndPoint remote_info(remote_address.hostname(), remote_endpoint);
...
int IpcPacketSocket::GetOption(rtc::Socket::Option option, int* value) {
  common::P2PSocketOption p2p_socket_option = common::P2P_SOCKET_OPT_MAX;
...
int IpcPacketSocket::DoSetOption(common::P2PSocketOption option, int value) {
  DCHECK(thread_checker_.CalledOnValidThread());
...
void IpcPacketSocket::OnOpen(const net::IPEndPoint& local_address,
  ...
  for (int i = 0; i < common::P2P_SOCKET_OPT_MAX; ++i) {
    if (options_[i] != kDefaultNonSetOptionValue)
      DoSetOption(static_cast<common::P2PSocketOption> (i), options_[i]);
  }
...
void IpcPacketSocket::OnSendComplete(const common::P2PSendPacketMetrics& send_metrics) {
  DCHECK(thread_checker_.CalledOnValidThread());
...
rtc::AsyncPacketSocket* IpcPacketSocketFactory::CreateUdpSocket(
  ...
  std::unique_ptr<IpcPacketSocket> socket(new IpcPacketSocket());
  if (!socket->Init(common::P2P_SOCKET_UDP, socket_client, local_address, min_port,
...
rtc::AsyncPacketSocket* IpcPacketSocketFactory::CreateServerTcpSocket(
  ...
  common::P2PSocketType type = (opts & rtc::PacketSocketFactory::OPT_STUN) ?
      common::P2P_SOCKET_STUN_TCP_SERVER : common::P2P_SOCKET_TCP_SERVER;
  P2PSocketClientImpl* socket_client =
      new P2PSocketClientImpl(socket_dispatcher_, traffic_annotation_);
  std::unique_ptr<IpcPacketSocket> socket(new IpcPacketSocket());
...
rtc::AsyncPacketSocket* IpcPacketSocketFactory::CreateClientTcpSocket(
  ...
  if (opts & rtc::PacketSocketFactory::OPT_SSLTCP) {
    type = (opts & rtc::PacketSocketFactory::OPT_STUN) ?
        common::P2P_SOCKET_STUN_SSLTCP_CLIENT : common::P2P_SOCKET_SSLTCP_CLIENT;
  } else if (opts & rtc::PacketSocketFactory::OPT_TLS) {
    type = (opts & rtc::PacketSocketFactory::OPT_STUN) ?
        common::P2P_SOCKET_STUN_TLS_CLIENT : common::P2P_SOCKET_TLS_CLIENT;
  } else {
    type = (opts & rtc::PacketSocketFactory::OPT_STUN) ?
        common::P2P_SOCKET_STUN_TCP_CLIENT : common::P2P_SOCKET_TCP_CLIENT;
  }
  P2PSocketClientImpl* socket_client =
      new P2PSocketClientImpl(socket_dispatcher_, traffic_annotation_);
  std::unique_ptr<IpcPacketSocket> socket(new IpcPacketSocket());
