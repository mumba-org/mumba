 ...
 public:
  SharedWorkerWebApplicationCacheHostImpl(
      ...
      : WebApplicationCacheHostImpl(
            client,
            ApplicationThread::current()->appcache_dispatcher()->backend_proxy(),
            kAppCacheNoHostId) {}
...
class WebServiceWorkerNetworkProviderForSharedWorker
 ...
 public:
  ...
  std::unique_ptr<blink::WebURLLoader> CreateURLLoader(
    ...
    // ApplicationThread is nullptr in some tests.
    if (!render_thread) {
      return nullptr;
    }
    ...
    const blink::WebURL& request_web_url = request.Url();
    GURL request_url(request_web_url.GetString().Utf8(), request_web_url.GetParsed(), request_web_url.IsValid());
    ...
    return std::make_unique<ApplicationURLLoader>(
        ApplicationThread::current()->resource_dispatcher(),
...
EmbeddedSharedWorkerStub::EmbeddedSharedWorkerStub(
    ...
    : binding_(this, std::move(request)),
      ...
      name_(info->name),
      url_(info->url) {
  ApplicationProcess::current()->AddRefProcess();
  ...
  impl_->StartWorkerContext(
      blink::KURL(String::FromUTF8(url_.possibly_invalid_spec().data())), blink::WebString::FromUTF8(name_),
...
EmbeddedSharedWorkerStub::~EmbeddedSharedWorkerStub() {
  ...
  ApplicationProcess::current()->ReleaseProcess();
}
...
EmbeddedSharedWorkerStub::CreateServiceWorkerNetworkProvider() {
  ...
  if (ApplicationThread* render_thread = ApplicationThread::current()) {
...
EmbeddedSharedWorkerStub::CreateWorkerFetchContext(
  ...
  auto worker_fetch_context = std::make_unique<WorkerFetchContextImpl>(
      ...
      ApplicationThread::current()->thread_safe_sender(),
      ApplicationThread::current()->window_dispatcher());
  ...
  worker_fetch_context->set_site_for_cookies(blink::KURL(String::FromUTF8(url_.possibly_invalid_spec().data())));
