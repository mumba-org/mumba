...
#include "core/shared/application/fileapi/webfilesystem_impl.h"
...
#include "core/shared/application/file_info_util.h"
#include "core/shared/application/fileapi/file_system_dispatcher.h"
#include "core/shared/application/fileapi/webfilewriter_impl.h"
...
void DidReceiveSnapshotFile(int request_id) {
  if (ApplicationThread::current())
    ApplicationThread::current()->Send(
...
void CallDispatcherOnMainThread(
  ...
  if (!ApplicationThread::current() ||
      !ApplicationThread::current()->file_system_dispatcher())
    return;

  DCHECK(!waitable_results);
  DispatchToMethod(ApplicationThread::current()->file_system_dispatcher(),
                   method, params);
}
...
void DidOpenFileSystem(const std::string& name,
                       const GURL& root,
                       WebFileSystemCallbacks* callbacks) {
  callbacks->DidOpenFileSystem(blink::WebString::FromUTF8(name), blink::KURL(root.possibly_invalid_spec().data()));
}
...
void DidResolveURL(const std::string& name,
                   ...
                   bool is_directory,
                   WebFileSystemCallbacks* callbacks) {
  callbacks->DidResolveURL(blink::WebString::FromUTF8(name), 
                           blink::KURL(root_url.possibly_invalid_spec().data()),
...
void WebFileSystemImpl::OpenFileSystem(const blink::WebURL& storage_partition,
  ...
  CallDispatcherOnMainThread(
      ...
      std::make_tuple(
          GURL(storage_partition.GetString().Utf8().data(), storage_partition.GetParsed(), storage_partition.IsValid()), 
...
void WebFileSystemImpl::ResolveURL(const blink::WebURL& filesystem_url,
  ...
  CallDispatcherOnMainThread(
      ...
      std::make_tuple(
          GURL(filesystem_url.GetString().Utf8().data(), filesystem_url.GetParsed(), filesystem_url.IsValid()),
...
void WebFileSystemImpl::Move(const blink::WebURL& src_path,
  ...
  CallDispatcherOnMainThread(
      ...
      std::make_tuple(
          GURL(src_path.GetString().Utf8().data(), src_path.GetParsed(), src_path.IsValid()), 
          GURL(dest_path.GetString().Utf8().data(), dest_path.GetParsed(), dest_path.IsValid()),
          base::Bind(&StatusCallbackAdapter,
                     base::ThreadTaskRunnerHandle::Get(), callbacks_id,
                     base::RetainedRef(waitable_results))),
      waitable_results.get());
...
void WebFileSystemImpl::Remove(const blink::WebURL& path,
  ...
  CallDispatcherOnMainThread(
      ...
      std::make_tuple(
          GURL(path.GetString().Utf8().data(), path.GetParsed(), path.IsValid()), 
          false /* recursive */,
          base::Bind(&StatusCallbackAdapter,
                     base::ThreadTaskRunnerHandle::Get(), callbacks_id,
                     base::RetainedRef(waitable_results))),
...
void WebFileSystemImpl::CreateFileWriter(const WebURL& path,
                                         blink::WebFileWriterClient* client,
                                         WebFileSystemCallbacks callbacks) {
  GURL path_url(path.GetString().Utf8().data(), path.GetParsed(), path.IsValid());
  int callbacks_id = RegisterCallbacks(callbacks);
  scoped_refptr<WaitableCallbackResults> waitable_results =
      MaybeCreateWaitableResults(callbacks, callbacks_id);
  CallDispatcherOnMainThread(
      ...
      std::make_tuple(
          ...
          base::Bind(&CreateFileWriterCallbackAdapter,
                     base::ThreadTaskRunnerHandle::Get(), callbacks_id,
                     base::RetainedRef(waitable_results),
                     main_thread_task_runner_, path_url, client),
          base::Bind(&StatusCallbackAdapter,
                     base::ThreadTaskRunnerHandle::Get(), callbacks_id,
                     base::RetainedRef(waitable_results))),
      waitable_results.get());
