...
common::mojom::MediaStreamDispatcherHostPtr
...
void MockMojoMediaStreamDispatcherHost::GenerateStream(
  ...
  if (controls.audio.requested) {
    common::MediaStreamDevice audio_device;
  ...
  if (controls.video.requested) {
    common::MediaStreamDevice video_device;
...
void MockMojoMediaStreamDispatcherHost::StopStreamDevice(
    ...
    int32_t session_id) {
  for (const common::MediaStreamDevice& device : audio_devices_) {
    if (device.id == device_id && device.session_id == session_id) {
      ++stop_audio_device_counter_;
      return;
    }
  }
  for (const common::MediaStreamDevice& device : video_devices_) {
    if (device.id == device_id && device.session_id == session_id) {
