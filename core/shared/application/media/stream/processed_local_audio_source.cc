...
bool ProcessedLocalAudioSource::EnsureSourceIsStarted() {
  ...
  common::MediaStreamDevice modified_device(device());
...
void ProcessedLocalAudioSource::OnCaptureStarted() {
  started_callback_.Run(this, common::MEDIA_DEVICE_OK, "");
}
...
int ProcessedLocalAudioSource::GetBufferSize(int sample_rate) const {
  ...
  // If the buffer size is missing from the common::MediaStreamDevice, provide 10ms as a
  // fall-back.
  //
