...
#include "core/shared/application/media/stream/media_stream_constraints_util_sets.h"
 ...
 public:
  ...
  AudioCaptureSettings SelectBestSettings(
    ...
    bool disable_local_echo_default =
        media_stream_source != common::kMediaStreamSourceDesktop;
...
SelectSettingsAudioCapture(MediaStreamAudioSource* source,
  ...
  if (source->device().type != common::MEDIA_DEVICE_AUDIO_CAPTURE &&
      source->device().type != common::MEDIA_TAB_AUDIO_CAPTURE &&
      source->device().type != common::MEDIA_DESKTOP_AUDIO_CAPTURE) {
    return AudioCaptureSettings();
  }
  ...
  if (source->device().type == common::MEDIA_TAB_AUDIO_CAPTURE &&
      !media_stream_source.empty() &&
      media_stream_source != common::kMediaStreamSourceTab) {
    return AudioCaptureSettings(
        constraints.Basic().media_stream_source.GetName());
  }
  if (source->device().type == common::MEDIA_DESKTOP_AUDIO_CAPTURE &&
      !media_stream_source.empty() &&
      media_stream_source != common::kMediaStreamSourceSystem &&
      media_stream_source != common::kMediaStreamSourceDesktop) {
    return AudioCaptureSettings(
        constraints.Basic().media_stream_source.GetName());
  }
