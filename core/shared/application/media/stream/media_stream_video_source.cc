...
MediaStreamVideoSource::MediaStreamVideoSource()
    : state_(NEW),
      track_adapter_(
          new VideoTrackAdapter(ApplicationProcess::current()->io_task_runner())),
      weak_factory_(this) {}
...
void MediaStreamVideoSource::OnStartDone(common::MediaStreamRequestResult result) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void MediaStreamVideoSource::FinalizeAddPendingTracks() {
  ...
  for (const auto& track_info : pending_track_descriptors) {
    common::MediaStreamRequestResult result = common::MEDIA_DEVICE_OK;
    if (state_ != STARTED)
      result = common::MEDIA_DEVICE_TRACK_START_FAILURE_VIDEO;
