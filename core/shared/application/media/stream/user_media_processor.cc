...
#include "core/shared/application/media/stream/user_media_processor.h"
...
#include "core/shared/application/media/stream/local_media_stream_audio_source.h"
#include "core/shared/application/media/stream/media_stream_audio_processor.h"
#include "core/shared/application/media/stream/media_stream_audio_source.h"
#include "core/shared/application/media/stream/media_stream_constraints_util.h"
#include "core/shared/application/media/stream/media_stream_constraints_util_audio.h"
#include "core/shared/application/media/stream/media_stream_constraints_util_video_content.h"
#include "core/shared/application/media/stream/media_stream_constraints_util_video_device.h"
#include "core/shared/application/media/stream/media_stream_device_observer.h"
#include "core/shared/application/media/stream/media_stream_video_capturer_source.h"
#include "core/shared/application/media/stream/media_stream_video_track.h"
#include "core/shared/application/media/stream/processed_local_audio_source.h"
#include "core/shared/application/media/stream/user_media_client_impl.h"
#include "core/shared/application/media/webrtc/peer_connection_dependency_factory.h"
#include "core/shared/application/media/webrtc/webrtc_uma_histograms.h"
#include "core/shared/application/media/webrtc_logging.h"
//#include "core/shared/application/render_frame_impl.h"
//#include "core/shared/application/render_widget.h"
...
void InitializeTrackControls(const blink::WebMediaConstraints& constraints,
                             common::TrackControls* track_controls) {
...
bool IsSameDevice(const common::MediaStreamDevice& device,
                  const common::MediaStreamDevice& other_device) {
...
bool IsSameSource(const blink::WebMediaStreamSource& source,
  ...
  const common::MediaStreamDevice& device = source_extra_data->device();

  MediaStreamSource* const other_source_extra_data =
      static_cast<MediaStreamSource*>(other_source.GetExtraData());
  const common::MediaStreamDevice& other_device = other_source_extra_data->device();

  return IsSameDevice(device, other_device);
}

bool IsValidAudioContentSource(const std::string& source) {
  return source == common::kMediaStreamSourceTab ||
         source == common::kMediaStreamSourceDesktop ||
         source == common::kMediaStreamSourceSystem;
}

bool IsValidVideoContentSource(const std::string& source) {
  return source == common::kMediaStreamSourceTab ||
         source == common::kMediaStreamSourceDesktop ||
         source == common::kMediaStreamSourceScreen;
}
...
class UserMediaProcessor::RequestInfo
 ...
 public:
  ...
  void SetDevices(common::MediaStreamDevices audio_devices,
                  common::MediaStreamDevices video_devices) {
  ...
  const common::MediaStreamDevices& audio_devices() const { return audio_devices_; }
  const common::MediaStreamDevices& video_devices() const { return video_devices_; }
  ...
  common::StreamControls* stream_controls() { return &stream_controls_; }
 ...
 private:
  ...
  common::StreamControls stream_controls_;
  const url::Origin security_origin_;
  ResourcesReady ready_callback_;
  common::MediaStreamRequestResult request_result_ = common::MEDIA_DEVICE_OK;
...
void UserMediaProcessor::RequestInfo::StartAudioTrack(
  ...
  if (!is_pending) {
    OnTrackStarted(
        native_source,
        connected ? common::MEDIA_DEVICE_OK : common::MEDIA_DEVICE_TRACK_START_FAILURE_AUDIO,
        "");
  }
}
...
void UserMediaProcessor::SetupAudioInput() {
  ...
  if (IsDeviceSource(audio_controls.stream_source)) {
    ...
    if (!IsValidAudioContentSource(audio_controls.stream_source)) {
      ...
      common::MediaStreamRequestResult result = common::MEDIA_DEVICE_CONSTRAINT_NOT_SATISFIED;
...
void UserMediaProcessor::SelectAudioDeviceSettings(
  ...
  for (const auto& device : audio_input_capabilities) {
    ...
    if (it != local_sources_.end()) {
      ...
      if (source->device().type == common::MEDIA_DEVICE_AUDIO_CAPTURE)
        audio_source = static_cast<MediaStreamAudioSource*>(source);
    }
    if (audio_source) {
      capabilities.emplace_back(audio_source);
    } else {
      capabilities.emplace_back(device->device_id,
                                device->parameters);
    }
  }
  ...
  if (!settings.HasValue()) {
    ...
    common::MediaStreamRequestResult result =
        failed_constraint_name.IsEmpty()
            ? common::MEDIA_DEVICE_NO_HARDWARE
            : common::MEDIA_DEVICE_CONSTRAINT_NOT_SATISFIED;
    GetUserMediaRequestFailed(result, failed_constraint_name);
    return;
  }
...
void UserMediaProcessor::SelectVideoContentSettings() {
  ...
  if (!settings.HasValue()) {
    ...
    GetUserMediaRequestFailed(common::MEDIA_DEVICE_CONSTRAINT_NOT_SATISFIED,
...
gfx::Size UserMediaProcessor::GetScreenSize() {
  ...
  if (dispatcher_) {  // Can be null in tests.
    blink::WebScreenInfo info =
        dispatcher_->GetScreenInfo();
...
void UserMediaProcessor::OnDeviceStopped(const common::MediaStreamDevice& device) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
MediaStreamAudioSource* UserMediaProcessor::CreateAudioSource(
  ...
  common::StreamControls* stream_controls = current_request_info_->stream_controls();
  ...
  if (IsScreenCaptureMediaType(device.type) ||
    ...
    return new LocalMediaStreamAudioSource(
        routing_id_, device, stream_controls->hotword_enabled,
...
MediaStreamVideoSource* UserMediaProcessor::CreateVideoSource(
  ...
  return new MediaStreamVideoCapturerSource(
      routing_id_, stop_callback, device,
...
void UserMediaProcessor::CreateAudioTracks(
  ...
  common::MediaStreamDevices overridden_audio_devices = devices;
...
void UserMediaProcessor::DelayedGetUserMediaRequestSucceeded(
  ...
  LogUserMediaRequestResult(common::MEDIA_DEVICE_OK);
...
const blink::WebMediaStreamSource* UserMediaProcessor::FindLocalSource(
    ...
    const common::MediaStreamDevice& device) const {
  for (const auto& local_source : sources) {
    MediaStreamSource* const source =
        static_cast<MediaStreamSource*>(local_source.GetExtraData());
    const common::MediaStreamDevice& active_device = source->device();
  ...
  for (LocalStreamSources::iterator device_it = pending_local_sources_.begin();
    ...
    if (IsSameSource(*device_it, source)) {
      ...
      NotifyCurrentRequestInfoOfAudioSourceStarted(
          source_extra_data,
          is_audio_source ? common::MEDIA_DEVICE_TRACK_START_FAILURE_AUDIO
                          : common::MEDIA_DEVICE_TRACK_START_FAILURE_VIDEO,
...
const common::mojom::MediaStreamDispatcherHostPtr&
UserMediaProcessor::GetMediaStreamDispatcherHost() {
  if (!dispatcher_host_) {
    dispatcher_->main_thread()->GetRemoteInterfaces()->GetInterface(
