...
namespace {
void ResetCallback(std::unique_ptr<common::VideoCaptureDeliverFrameCB> callback) {
...
class MediaStreamVideoTrack::FrameDeliverer
 ...
 public:
  ...
  void AddCallback(VideoSinkId id, const common::VideoCaptureDeliverFrameCB& callback);
 ...
 private:
  ...
  // Render Thread.
  base::ThreadChecker main_application_thread_checker_;
  const scoped_refptr<base::SingleThreadTaskRunner> io_task_runner_;

  bool enabled_;
  scoped_refptr<media::VideoFrame> black_frame_;

  typedef std::pair<VideoSinkId, common::VideoCaptureDeliverFrameCB>
      VideoIdCallbackPair;
...
void MediaStreamVideoTrack::FrameDeliverer::AddCallback(
    VideoSinkId id,
    const common::VideoCaptureDeliverFrameCB& callback) {
  DCHECK(main_application_thread_checker_.CalledOnValidThread());
...
void MediaStreamVideoTrack::FrameDeliverer::RemoveCallbackOnIO(
  ...
  for (; it != callbacks_.end(); ++it) {
    if (it->first == id) {
      ...
      callback.reset(new common::VideoCaptureDeliverFrameCB(it->second));
      callbacks_.erase(it);
...
void MediaStreamVideoTrack::GetSettings(
  ...
  const base::Optional<common::CameraCalibration> calibration =
