...
//#include "core/shared/application/dispatcher_impl.h"
...
UserMediaClientImpl::UserMediaClientImpl(
    ...
    : ApplicationWindowDispatcherObserver(dispatcher),
...
void UserMediaClientImpl::RequestUserMedia(
  ...
  //        dispatcher()->GetWebFrame() ==
  //            static_cast<blink::WebFrame*>(
  //                web_request.OwnerDocument().GetFrame()));

  if (ApplicationThread::current()) {
    ApplicationThread::current()->peer_connection_tracker()->TrackGetUserMedia(
        web_request);
  }
...
void UserMediaClientImpl::CurrentRequestCompleted() {
  ...
  if (!pending_request_infos_.empty()) {
    static_cast<blink::WebLocalFrame *>(GetWebFrame(routing_id_))
...
UserMediaClientImpl::GetMediaDevicesDispatcher() {
  if (!media_devices_dispatcher_) {
    application_thread()->GetRemoteInterfaces()->GetInterface(
