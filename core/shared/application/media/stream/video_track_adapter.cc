...
#include "core/shared/application/media/stream/video_track_adapter.h"
...
void ResetCallbackOnMainApplicationThread(
    std::unique_ptr<common::VideoCaptureDeliverFrameCB> callback) {
...
class VideoTrackAdapter::VideoFrameResolutionAdapter
 ...
 public:
  ...
  void AddCallback(const MediaStreamVideoTrack* track,
                   const common::VideoCaptureDeliverFrameCB& callback);
  ...
  std::unique_ptr<common::VideoCaptureDeliverFrameCB> RemoveAndGetCallback(
 ...
 private:
  ...
  typedef std::pair<const MediaStreamVideoTrack*, common::VideoCaptureDeliverFrameCB>
      VideoIdCallbackPair;
...
void VideoTrackAdapter::VideoFrameResolutionAdapter::RemoveAndReleaseCallback(
  ...
  for (; it != callbacks_.end(); ++it) {
    if (it->first == track) {
      ...
      std::unique_ptr<common::VideoCaptureDeliverFrameCB> callback(
          new common::VideoCaptureDeliverFrameCB(it->second));
      callbacks_.erase(it);
      renderer_task_runner_->PostTask(
          FROM_HERE, base::BindOnce(&ResetCallbackOnMainApplicationThread,
                                    std::move(callback)));
...
std::unique_ptr<common::VideoCaptureDeliverFrameCB>
VideoTrackAdapter::VideoFrameResolutionAdapter::RemoveAndGetCallback(
    const MediaStreamVideoTrack* track) {
  DCHECK(io_thread_checker_.CalledOnValidThread());
  std::unique_ptr<common::VideoCaptureDeliverFrameCB> ret;
  for (auto it = callbacks_.begin(); it != callbacks_.end(); ++it) {
    if (it->first == track) {
      // Make sure the VideoCaptureDeliverFrameCB is released on the main
      // render thread since it was added on the main render thread in
      // VideoTrackAdapter::AddTrack.
      ret = std::make_unique<common::VideoCaptureDeliverFrameCB>(it->second);
      callbacks_.erase(it);
      break;
    }
  }
...
void VideoTrackAdapter::AddTrack(const MediaStreamVideoTrack* track,
                                 common::VideoCaptureDeliverFrameCB frame_callback,
                                 const VideoTrackAdapterSettings& settings) {
...
void VideoTrackAdapter::ReconfigureTrackOnIO(
  ...
  std::unique_ptr<common::VideoCaptureDeliverFrameCB> track_frame_callback;
  // Remove the track.
  for (auto it = adapters_.begin(); it != adapters_.end(); ++it) {
