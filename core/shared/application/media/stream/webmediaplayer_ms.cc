...
#include "core/shared/application/media_stream_audio_renderer.h"
#include "core/shared/application/media_stream_renderer_factory.h"
#include "core/shared/application/media_stream_video_renderer.h"
...
#include "core/shared/application/media/stream/webmediaplayer_ms_compositor.h"
...
void WebMediaPlayerMS::Load(
  ...
  GURL url;
  if (source.IsURL()) {
    blink::WebURL source_url = source.GetAsURL();
    url = GURL(source_url.GetString().Utf8().data(), source_url.GetParsed(), source_url.IsValid());
  }
...
void WebMediaPlayerMS::ReloadVideo() {
  ...
  if (!paused_)
    delegate_->DidPlayerSizeChange(delegate_id_, gfx::Size(NaturalSize().width, NaturalSize().height));
}
...
blink::WebSize WebMediaPlayerMS::NaturalSize() const {
  ...
  return blink::WebSize(compositor_->GetCurrentSize().width(), compositor_->GetCurrentSize().height());
}
...
void WebMediaPlayerMS::Paint(cc::PaintCanvas* canvas,
  ...
  if (frame && frame->HasTextures()) {
    auto* provider =
        ApplicationThread::current()->SharedMainThreadContextProvider().get();
    // GPU Process crashed.
    if (!provider)
      return;
