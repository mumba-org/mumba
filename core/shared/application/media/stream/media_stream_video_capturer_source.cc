...
class LocalVideoCapturerSource final : public media::VideoCapturerSource {
 ...
 private:
  void OnStateUpdate(common::VideoCaptureState state);
...
LocalVideoCapturerSource::LocalVideoCapturerSource(int session_id)
    : session_id_(session_id),
      manager_(ApplicationThread::current()->video_capture_impl_manager()),
...
void LocalVideoCapturerSource::StartCapture(
    ...
    const common::VideoCaptureDeliverFrameCB& new_frame_callback,
...
void LocalVideoCapturerSource::OnStateUpdate(common::VideoCaptureState state) {
  DCHECK(thread_checker_.CalledOnValidThread());
...
void MediaStreamVideoCapturerSource::OnCapturingLinkSecured(bool is_secure) {
  ...
  GetMediaStreamDispatcherHost(thread)//render_frame)
      ->SetCapturingLinkSecured(device().session_id, device().type, is_secure);
}

void MediaStreamVideoCapturerSource::StartSourceImpl(
    const common::VideoCaptureDeliverFrameCB& frame_callback) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  state_ = STARTING;
...
void MediaStreamVideoCapturerSource::StopSourceForRestartImpl() {
  ...
  if (device().type == common::MEDIA_NO_SERVICE)
  ...
  switch (state_) {
    case STARTING:
      if (is_running) {
        ...
        OnStartDone(common::MEDIA_DEVICE_OK);
      } else {
        state_ = STOPPED;
        OnStartDone(common::MEDIA_DEVICE_TRACK_START_FAILURE_VIDEO);
      }
      break;
    case STARTED:
      if (!is_running) {
