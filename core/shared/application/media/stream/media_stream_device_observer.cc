...
#include "core/shared/application/media/stream/media_stream_dispatcher_eventhandler.h"
...
bool RemoveStreamDeviceFromArray(const common::MediaStreamDevice& device,
                                 common::MediaStreamDevices* devices) {
  for (common::MediaStreamDevices::iterator device_it = devices->begin();
...
struct MediaStreamDeviceObserver::Stream {
  ...
  common::MediaStreamDevices audio_devices;
  common::MediaStreamDevices video_devices;
};

MediaStreamDeviceObserver::MediaStreamDeviceObserver(ApplicationWindowDispatcher* dispatcher)
    : ApplicationWindowDispatcherObserver(dispatcher), binding_(this) {
...
common::MediaStreamDevices MediaStreamDeviceObserver::GetNonScreenCaptureDevices() {
  common::MediaStreamDevices video_devices;
...
void MediaStreamDeviceObserver::RemoveStreamDevice(
  ...
  while (stream_it != label_stream_map_.end()) {
    common::MediaStreamDevices& audio_devices = stream_it->second.audio_devices;
    common::MediaStreamDevices& video_devices = stream_it->second.video_devices;
