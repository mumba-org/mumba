...
VideoCaptureImplManager::~VideoCaptureImplManager() {
  ...
  for (auto& entry : devices_) {
    ApplicationProcess::current()->io_task_runner()->DeleteSoon(FROM_HERE,
                                                          entry.impl.release());
  }
  devices_.clear();
}
...
base::Closure VideoCaptureImplManager::StartCapture(
  ...
  ApplicationProcess::current()->io_task_runner()->PostTask(
...
void VideoCaptureImplManager::SuspendDevices(
  ...
  for (const common::MediaStreamDevice& device : video_devices) {
