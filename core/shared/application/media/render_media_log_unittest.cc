...
class RenderMediaLogTest : public testing::Test {
 public:
  ...
  int message_count() { return application_thread_.sink().message_count(); }

  std::vector<media::MediaLogEvent> GetMediaLogEvents() {
    const IPC::Message* msg = application_thread_.sink().GetFirstMessageMatching(
 ...
 private:
  ...
  MockApplicationThread application_thread_;
