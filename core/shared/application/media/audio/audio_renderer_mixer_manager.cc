...
#include "core/shared/application/media/audio/audio_renderer_sink_cache.h"
...
media::OutputDeviceInfo AudioRendererMixerManager::GetOutputDeviceInfo(
    ...
    int session_id,
    const std::string& device_id) {
  //DLOG(INFO) << "AudioRendererMixerManager::GetOutputDeviceInfo: device_id = " << device_id;
  // FIXME: the input device_id is bogus.. so fixing this to 'default'
  std::string local_device_id(device_id);
  //DLOG(INFO) << "AudioRendererMixerManager::GetOutputDeviceInfo: device_id = " << device_id << " passing '" << local_device_id << "'";
