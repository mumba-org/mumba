...
#include "core/shared/application/media/audio/audio_renderer_sink_cache_impl.h"
...
// FIX: without a way to bind into the lifecicle of a frame
//      the cache will be 'broken' as it will never GC

// class AudioRendererSinkCacheImpl::FrameObserver : public RenderFrameObserver {
//  public:
//   explicit FrameObserver(application::RenderFrame* render_frame)
//       : RenderFrameObserver(render_frame) {}
//   ~FrameObserver() override{};

//  private:
//   // application::RenderFrameObserver implementation:
...
std::unique_ptr<AudioRendererSinkCache> AudioRendererSinkCache::Create() {
  //DLOG(INFO) << "AudioRendererSinkCache::Create";
...
scoped_refptr<media::AudioRendererSink> AudioRendererSinkCacheImpl::GetSink(
  ...
  CacheEntry cache_entry = {source_render_frame_id, device_id,
                            create_sink_cb_.Run(source_render_frame_id,
                                                0 /* session_id */, std::move(device_id)),
                            true /* used */};
