...
void AudioOutputIPCFactory::RegisterRemoteFactory(
  ...
  common::mojom::RendererAudioOutputStreamFactoryPtr factory_ptr;
...
void AudioOutputIPCFactory::RegisterRemoteFactory(
    int frame_id,
    common::mojom::RendererAudioOutputStreamFactoryPtrInfo factory_ptr) {
    //service_manager::InterfaceProvider* interface_provider) {
  //common::mojom::RendererAudioOutputStreamFactoryPtr factory_ptr;
  //interface_provider->GetInterface(&factory_ptr);
  // PassInterface unbinds the message pipe from the current thread. This
  ...
  io_task_runner_->PostTask(
      ...
      base::BindOnce(&AudioOutputIPCFactory::RegisterRemoteFactoryOnIOThread,
                     base::Unretained(this), frame_id,
                     std::move(factory_ptr)));//factory_ptr.PassInterface()));
  //RegisterRemoteFactoryOnIOThread(frame_id, std::move(factory_ptr));
}
...
void AudioOutputIPCFactory::RegisterRemoteFactoryOnIOThread(
    int frame_id,
    common::mojom::RendererAudioOutputStreamFactoryPtrInfo factory_ptr_info) {
  DCHECK(io_task_runner_->BelongsToCurrentThread());
