...
scoped_refptr<media::AudioOutputDevice> NewOutputDevice(
  ...
  auto device = base::MakeRefCounted<media::AudioOutputDevice>(
      ...
      AudioOutputIPCFactory::get()->io_task_runner(), session_id, std::move(device_id),
      // Set authorization request timeout at 80% of renderer hung timeout,
      // but no more than kMaxAuthorizationTimeout.
      base::TimeDelta::FromMilliseconds(
          std::min(common::kHungRendererDelayMs * 8 / 10, kMaxAuthorizationTimeoutMs)));
  //DLOG(INFO) << "NewOutputDevice: device->RequestDeviceAuthorization(). device_id [" << device_id.size() << "] = " << device_id;
...
scoped_refptr<media::SwitchableAudioRendererSink> NewMixableSink(
  ...
  DCHECK(application_thread) << "ApplicationThread is not instantiated, or "
                        << "GetOutputDeviceInfo() is called on a wrong thread ";
  return scoped_refptr<media::AudioRendererMixerInput>(
      application_thread->GetAudioRendererMixerManager()->CreateInput(
...
AudioDeviceFactory::NewAudioRendererMixerSink(int render_frame_id,
                                              int session_id,
                                              std::string device_id) {
  return NewFinalAudioRendererSink(render_frame_id, session_id, std::move(device_id));
}
  ...
  if (factory_) {
    scoped_refptr<media::AudioRendererSink> device =
        factory_->CreateAudioRendererSink(source_type, render_frame_id,
                                          session_id, std::move(device_id));
    if (device)
      return device;
  }

  if (IsMixable(source_type))
    return NewMixableSink(source_type, render_frame_id, session_id, std::move(device_id));
...
AudioDeviceFactory::NewSwitchableAudioRendererSink(
    ...
    int session_id,
    std::string device_id) {
  //DLOG(INFO) << "AudioDeviceFactory::NewSwitchableAudioRendererSink: " << device_id;
  if (factory_) {
    scoped_refptr<media::SwitchableAudioRendererSink> sink =
        factory_->CreateSwitchableAudioRendererSink(
            source_type, render_frame_id, session_id, std::move(device_id));
    if (sink)
      return sink;
  }
  ...
  NOTIMPLEMENTED();
  return scoped_refptr<media::SwitchableAudioRendererSink>();
}
...
AudioDeviceFactory::NewAudioCapturerSource(int render_frame_id,
  ...
  return base::MakeRefCounted<media::AudioInputDevice>(
      AudioInputIPCFactory::get()->CreateAudioInputIPC(render_frame_id, session_id));
}
...
media::OutputDeviceInfo AudioDeviceFactory::GetOutputDeviceInfo(
  ...
  DCHECK(application_thread) << "ApplicationThread is not instantiated, or "
                        << "GetOutputDeviceInfo() is called on a wrong thread ";
  return application_thread->GetAudioRendererMixerManager()->GetOutputDeviceInfo(
      render_frame_id, session_id, device_id);
}
...
AudioDeviceFactory::NewFinalAudioRendererSink(int render_frame_id,
                                              int session_id,
                                              std::string device_id) {
  //DLOG(INFO) << "AudioDeviceFactory::NewFinalAudioRendererSink: device_id [" << device_id.size() <<"] = " << device_id;
  if (factory_) {
  ...
  return NewOutputDevice(render_frame_id, session_id, std::move(device_id));
}
