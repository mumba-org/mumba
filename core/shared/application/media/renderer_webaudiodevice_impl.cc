...
#include "core/shared/application/media/renderer_webaudiodevice_impl.h"
...
int FrameIdFromCurrentContext() {
  ...
  return MSG_ROUTING_NONE;//render_frame ? render_frame->GetRoutingID() : MSG_ROUTING_NONE;
}
...
void RendererWebAudioDeviceImpl::Start() {
  //DLOG(INFO) << "RendererWebAudioDeviceImpl::Start";
...
RendererWebAudioDeviceImpl::GetMediaTaskRunner() {
  if (!media_task_runner_) {
    media_task_runner_ =
        ApplicationThread::current()->GetMediaThreadTaskRunner();
  }
