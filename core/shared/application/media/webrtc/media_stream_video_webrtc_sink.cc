...
#include "core/shared/application/media/webrtc/media_stream_video_webrtc_sink.h"
...
#include "core/shared/application/media_stream_utils.h"
...
rtc::Optional<bool> ToAbslOptionalBool(const base::Optional<bool>& value) {
...
class MediaStreamVideoWebRtcSink::WebRtcVideoSourceAdapter
 ...
 private:
  ...
  // |application_thread_checker_| is bound to the main render thread.
...
void MediaStreamVideoWebRtcSink::RequestRefreshFrame() {
  DCHECK(thread_checker_.CalledOnValidThread());
  application::RequestRefreshFrameFromVideoTrack(connected_track());
}
