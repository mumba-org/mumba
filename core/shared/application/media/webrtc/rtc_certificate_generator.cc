...
#include "core/shared/application/media/webrtc/rtc_certificate_generator.h"
...
void GenerateCertificateWithOptionalExpiration(
  ...
  PeerConnectionDependencyFactory* pc_dependency_factory =
      ApplicationThread::current()->GetPeerConnectionDependencyFactory();
...
void RTCCertificateGenerator::GenerateCertificate(
    ...
    std::unique_ptr<blink::WebRTCCertificateCallback> observer,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner) {
  GenerateCertificateWithOptionalExpiration(key_params, rtc::Optional<uint64_t>(),
