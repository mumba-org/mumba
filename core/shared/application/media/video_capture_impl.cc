...
#include "core/shared/application/media/video_capture_impl.h"
...
struct VideoCaptureImpl::ClientInfo {
  ...
  common::VideoCaptureStateUpdateCB state_update_cb;

  common::VideoCaptureDeliverFrameCB deliver_frame_cb;
};
...
VideoCaptureImpl::VideoCaptureImpl(media::VideoCaptureSessionId session_id)
    : device_id_(session_id),
      ...
      state_(common::VIDEO_CAPTURE_STATE_STOPPED),
      weak_factory_(this) {
  io_thread_checker_.DetachFromThread();

  if (ApplicationThread::current()) {  // This will be null in unit tests.
...
VideoCaptureImpl::~VideoCaptureImpl() {
  ...
  if ((state_ == common::VIDEO_CAPTURE_STATE_STARTING ||
       state_ == common::VIDEO_CAPTURE_STATE_STARTED) &&
      GetVideoCaptureHost())
...
void VideoCaptureImpl::StartCapture(
    ...
    const common::VideoCaptureStateUpdateCB& state_update_cb,
    const common::VideoCaptureDeliverFrameCB& deliver_frame_cb) {
  ...
  switch (state_) {
    case common::VIDEO_CAPTURE_STATE_STARTING:
    case common::VIDEO_CAPTURE_STATE_STARTED:
      ...
      return;
    case common::VIDEO_CAPTURE_STATE_STOPPING:
      clients_pending_on_restart_[client_id] = client_info;
      DVLOG(1) << __func__ << " Got new resolution while stopping: "
               << params.requested_format.frame_size.ToString();
      return;
    case common::VIDEO_CAPTURE_STATE_STOPPED:
    case common::VIDEO_CAPTURE_STATE_ENDED:
      clients_[client_id] = client_info;
      params_ = params;
      ...
      return;
    case common::VIDEO_CAPTURE_STATE_ERROR:
      state_update_cb.Run(common::VIDEO_CAPTURE_STATE_ERROR);
      return;
    case common::VIDEO_CAPTURE_STATE_PAUSED:
    case common::VIDEO_CAPTURE_STATE_RESUMED:
...
void VideoCaptureImpl::GetDeviceSupportedFormats(
    const common::VideoCaptureDeviceFormatsCB& callback) {
  DCHECK(io_thread_checker_.CalledOnValidThread());
  ...
  switch (state) {
    case media::mojom::VideoCaptureState::STARTED:
      ...
      for (const auto& client : clients_)
        client.second.state_update_cb.Run(common::VIDEO_CAPTURE_STATE_STARTED);
    ...
    case media::mojom::VideoCaptureState::PAUSED:
      for (const auto& client : clients_)
        client.second.state_update_cb.Run(common::VIDEO_CAPTURE_STATE_PAUSED);
      break;
    case media::mojom::VideoCaptureState::RESUMED:
      for (const auto& client : clients_)
        client.second.state_update_cb.Run(common::VIDEO_CAPTURE_STATE_RESUMED);
      break;
    case media::mojom::VideoCaptureState::FAILED:
      for (const auto& client : clients_)
        client.second.state_update_cb.Run(common::VIDEO_CAPTURE_STATE_ERROR);
      clients_.clear();
      state_ = common::VIDEO_CAPTURE_STATE_ERROR;
      break;
    case media::mojom::VideoCaptureState::ENDED:
      // We'll only notify the client that the stream has stopped.
      for (const auto& client : clients_)
        client.second.state_update_cb.Run(common::VIDEO_CAPTURE_STATE_STOPPED);
      clients_.clear();
      state_ = common::VIDEO_CAPTURE_STATE_ENDED;
      break;
  }
}
...
void VideoCaptureImpl::OnBufferReady(int32_t buffer_id,
  ...
  bool consume_buffer = state_ == common::VIDEO_CAPTURE_STATE_STARTED;
...
void VideoCaptureImpl::StopDevice() {
  ...
  if (state_ != common::VIDEO_CAPTURE_STATE_STARTING &&
      state_ != common::VIDEO_CAPTURE_STATE_STARTED)
    return;
  state_ = common::VIDEO_CAPTURE_STATE_STOPPING;
  GetVideoCaptureHost()->Stop(device_id_);
...
void VideoCaptureImpl::RestartCapture() {
  DCHECK(io_thread_checker_.CalledOnValidThread());
  DCHECK_EQ(state_, common::VIDEO_CAPTURE_STATE_STOPPED);
...
void VideoCaptureImpl::StartCaptureInternal() {
  DCHECK(io_thread_checker_.CalledOnValidThread());
  state_ = common::VIDEO_CAPTURE_STATE_STARTING;
...
bool VideoCaptureImpl::RemoveClient(int client_id, ClientInfoMap* clients) {
  ...
  it->second.state_update_cb.Run(common::VIDEO_CAPTURE_STATE_STOPPED);
  clients->erase(it);
  return true;
}
