...
#include "core/shared/application/media/audio/audio_device_factory.h"
#include "core/shared/application/media/render_media_log.h"
#include "core/shared/application/media/application_webmediaplayer_delegate.h"
#include "core/shared/application/media/stream/media_stream_renderer_factory_impl.h"
#include "core/shared/application/media/stream/webmediaplayer_ms.h"
#include "core/shared/application/media/web_media_element_source_utils.h"
#include "core/shared/application/media/media_permission_dispatcher.h"
////#include "core/shared/application/render_frame_impl.h"
#include "core/shared/application/application_thread.h"
//#include "core/shared/application/render_view_impl.h"
...
#include "core/shared/application/media/android/media_player_renderer_client_factory.h"
#include "core/shared/application/media/android/renderer_media_player_manager.h"
#include "core/shared/application/media/android/stream_texture_wrapper_impl.h"
...
#include "core/shared/application/media/media_interface_factory.h"
//#endif
...
void ObtainAndSetContextProvider(
    base::OnceCallback<void(viz::ContextProvider*)> set_context_provider_callback,
    media::GpuVideoAcceleratorFactories* gpu_info) {
  //if (gpu_info.first) {
    //scoped_refptr<ui::ContextProviderCommandBuffer> context_provider =
    //    gpu_info.first->GetMediaContextProvider();
    viz::ContextProvider* context_provider = gpu_info->GetMediaContextProvider();
    std::move(set_context_provider_callback)
        .Run(context_provider); //std::move(context_provider));
  //} else {
  //  std::move(set_context_provider_callback).Run(nullptr);
  //}
}
...
void PostMediaContextProviderToCallback(
  ...
  base::PostTaskAndReplyWithResult(
      main_task_runner.get(), FROM_HERE, base::BindOnce([]() {
        return application::ApplicationThread::current()->GetGpuFactories();
      }),
...
bool MediaFactory::VideoSurfaceLayerEnabled() {
  ...
  ApplicationThread* application_thread =
      ApplicationThread::current();
  if (application_thread && application_thread->layout_test_mode()) {// &&
      //!application_thread->LayoutTestModeUsesDisplayCompositorPixelDump()) {
    return false;
  }
...
void MediaFactory::SetupMojo() {
  ...
  remote_interfaces_ = application_thread_->GetRemoteInterfaces();//render_frame_->GetRemoteInterfaces();
  DCHECK(remote_interfaces_);
}
...
blink::WebMediaPlayer* MediaFactory::CreateMediaPlayer(
  ...
  ApplicationThread* application_thread = ApplicationThread::current();
  ...
  scoped_refptr<media::SwitchableAudioRendererSink> audio_renderer_sink =
      AudioDeviceFactory::NewSwitchableAudioRendererSink(
          ...
          application_thread_->routing_id(), 
          0, 
          sink_id.IsEmpty() ? "default" : sink_id.Utf8());

  //DLOG(INFO) << "MediaFactory::CreateMediaPlayer: audio_renderer_sink = " << audio_renderer_sink.get();

  common::WebPreferences webkit_preferences = common::WebPreferences();
      //application_thread_->GetWebkitPreferences();
  ...
  std::unique_ptr<media::MediaLog> media_log(new RenderMediaLog(
      ...
      web_frame_->GetTaskRunner(blink::TaskType::kInternalMedia)));
  ...
  if (use_surface_layer_for_video) {
    ...
    video_frame_compositor_task_runner =
        application_thread->GetMediaThreadTaskRunner();
        
    submitter = blink::WebVideoFrameSubmitter::Create(
        base::BindRepeating(
            &PostMediaContextProviderToCallback,
            ApplicationThread::current()->GetCompositorMainThreadTaskRunner()),
        application_thread_->GetGpuMemoryBufferManager(),
        settings);
  } else {
    video_frame_compositor_task_runner =
        application_thread->compositor_task_runner()
            ? application_thread->compositor_task_runner()
            : web_frame_->GetTaskRunner(//render_frame_->GetTaskRunner(
...
/*WebMediaPlayerParams(
  ...
  std::unique_ptr<media::WebMediaPlayerParams> params(
      new media::WebMediaPlayerParams(
          std::move(media_log),
          base::BindRepeating(&MediaFactory::DeferMediaLoad,
                              base::Unretained(this),
                              web_frame_),
          std::move(audio_renderer_sink), 
          media_task_runner,
          application_thread->GetWorkerTaskRunner(),
          application_thread->compositor_task_runner(),
          video_frame_compositor_task_runner,
          base::Bind(&v8::Isolate::AdjustAmountOfExternalAllocatedMemory,
                     base::Unretained(blink::MainThreadIsolate())),
          initial_cdm,
          media_surface_manager_,
          request_routing_token_cb_, 
          media_observer,
          ...
          base::Bind(&blink::WebSurfaceLayerBridge::Create,
                      layer_tree_view),
          ApplicationThread::current()->SharedMainThreadContextProvider(),
          use_surface_layer_for_video,
          base::BindRepeating(
              &media::ApplicationWebMediaPlayerDelegate::OnPictureInPictureSurfaceIdUpdated,
              base::Unretained(media_player_delegate_), 
              // fix: delegate_id
              -1),
          base::BindRepeating(&media::ApplicationWebMediaPlayerDelegate::OnExitPictureInPicture,
                              base::Unretained(media_player_delegate_), 
                              // fix: delegate_id
                              -1)));
  ...
  //DLOG(INFO) << "MediaFactory: creating media::WebMediaPlayerImpl ..";
...
media::MediaPermission* MediaFactory::GetMediaPermission() {
  if (!media_permission_dispatcher_) {
    media_permission_dispatcher_.reset(new MediaPermissionDispatcher(
        base::Bind(
            &ApplicationThread::GetInterface<blink::mojom::PermissionService>,
            base::Unretained(application_thread_)),
        base::Bind(&ApplicationThread::IsEncryptedMediaEnabled,
                   base::Unretained(application_thread_))));
  }
  ...
  factory_selector->AddFactory(
      ...
      std::make_unique<MediaPlayerRendererClientFactory>(
          ...
          base::Bind(&StreamTextureWrapperImpl::Create,
                     application_thread->EnableStreamTextureCopy(),
                     application_thread->GetStreamTexureFactory(),
  ...
  if (use_mojo_renderer_factory) {
    factory_selector->AddFactory(
        ...
        std::make_unique<media::MojoRendererFactory>(
            ...
            base::Bind(&ApplicationThread::GetGpuFactories,
                       base::Unretained(application_thread)),
            GetMediaInterfaceFactory()));
...
blink::WebMediaPlayer* MediaFactory::CreateWebMediaPlayerForMediaStream(
  ...
  ApplicationThread* const application_thread = ApplicationThread::current();

  scoped_refptr<base::SingleThreadTaskRunner> compositor_task_runner =
      application_thread->compositor_task_runner();
  if (!compositor_task_runner.get())
    compositor_task_runner =
        web_frame_->GetTaskRunner(blink::TaskType::kInternalMediaRealTime);
        //render_frame_->GetTaskRunner(blink::TaskType::kInternalMediaRealTime);

  //DLOG(INFO) << "MediaFactory: creating a STREAMING WebMediaPlayer ..";
  return new WebMediaPlayerMS(
      ...
      std::make_unique<RenderMediaLog>(
          ...
          web_frame_->GetTaskRunner(blink::TaskType::kInternalMedia)),
          //render_frame_->GetTaskRunner(blink::TaskType::kInternalMedia)),
      CreateMediaStreamRendererFactory(), application_thread->GetIOTaskRunner(),
      compositor_task_runner, application_thread->GetMediaThreadTaskRunner(),
      application_thread->GetWorkerTaskRunner(), application_thread->GetGpuFactories(),
      sink_id);
}
...
MediaFactory::GetWebMediaPlayerDelegate() {
  if (!media_player_delegate_) {
    media_player_delegate_ =
        new media::ApplicationWebMediaPlayerDelegate(
          application_thread_,
          web_frame_,
          delegate_state_,
          callbacks_);
...
void MediaFactory::DeferMediaLoad(blink::WebLocalFrame* frame,
