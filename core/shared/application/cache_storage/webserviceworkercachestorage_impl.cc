...
common::ServiceWorkerFetchRequest FetchRequestFromWebRequest(
    const WebServiceWorkerRequest& web_request) {
  common::ServiceWorkerHeaderMap headers;
  GetServiceWorkerHeaderMapFromWebRequest(web_request, &headers);

  return common::ServiceWorkerFetchRequest(
      GURL(web_request.Url().GetString().Utf8().data(),
           web_request.Url().GetParsed(), 
           web_request.Url().IsValid()), 
      web_request.Method().Ascii(), 
      headers,
      common::Referrer(
        GURL(web_request.ReferrerUrl().GetString().Utf8().data(), 
             web_request.ReferrerUrl().GetParsed(), 
             web_request.ReferrerUrl().IsValid()), 
...
void PopulateWebRequestFromFetchRequest(
  ...
  web_request->SetURL(KURL(request.url.possibly_invalid_spec().data()));
...
common::CacheStorageCacheQueryParams QueryParamsFromWebQueryParams(
    const blink::WebServiceWorkerCache::QueryParams& web_query_params) {
  common::CacheStorageCacheQueryParams query_params;
...
common::CacheStorageCacheOperationType CacheOperationTypeFromWebCacheOperationType(
    blink::WebServiceWorkerCache::OperationType operation_type) {
  switch (operation_type) {
    case blink::WebServiceWorkerCache::kOperationTypePut:
      return common::CACHE_STORAGE_CACHE_OPERATION_TYPE_PUT;
    case blink::WebServiceWorkerCache::kOperationTypeDelete:
      return common::CACHE_STORAGE_CACHE_OPERATION_TYPE_DELETE;
    default:
      return common::CACHE_STORAGE_CACHE_OPERATION_TYPE_UNDEFINED;
  }
}

common::CacheStorageBatchOperation BatchOperationFromWebBatchOperation(
...
class WebServiceWorkerCacheStorageImpl::CacheRef
 ...
 public:
  ...
  void DispatchKeys(
      ...
      const WebServiceWorkerRequest& request,
      const blink::WebServiceWorkerCache::QueryParams& query_params) {
    //DLOG(INFO) << "WebServiceWorkerCacheStorageImpl::CacheRef::DispatchKeys";
...
class WebServiceWorkerCacheStorageImpl::WebCache
 ...
 public:
  ...
  void DispatchKeys(std::unique_ptr<CacheWithRequestsCallbacks> callbacks,
                    const WebServiceWorkerRequest& request,
                    const QueryParams& query_params) override {
    //DLOG(INFO) << "WebServiceWorkerCacheStorageImpl::WebCache::DispatchKeys";
...
WebServiceWorkerCacheStorageImpl::WebServiceWorkerCacheStorageImpl(IPC::SyncChannel* channel)
    //service_manager::InterfaceProvider* provider)
    : weak_factory_(this) {
  //DLOG(INFO) << "WebServiceWorkerCacheStorageImpl()";
  // Sets up the Mojo InterfacePtr to send IPCs to browser process.
  //provider->GetInterface(mojo::MakeRequest(&cache_storage_ptr_));
  channel->GetRemoteAssociatedInterface(&cache_storage_ptr_);
}
...
void WebServiceWorkerCacheStorageImpl::DispatchKeys(
  ...
  //DLOG(INFO) << "WebServiceWorkerCacheStorageImpl::DispatchKeys";
...
void WebServiceWorkerCacheStorageImpl::KeysCallback(
  ...
  //DLOG(INFO) << "WebServiceWorkerCacheStorageImpl::KeysCallback: returned keys: " << keys.size();
...
WebServiceWorkerCacheStorageImpl::GetCacheStorage() {
  //DLOG(INFO) << "WebServiceWorkerCacheStorageImpl::GetCacheStorage";
...
void WebServiceWorkerCacheStorageImpl::PopulateWebResponseFromResponse(
    ...
    blink::WebServiceWorkerResponse* web_response) {
  web_response->SetURLList(ToKURLVector(response.url_list));
...
WebServiceWorkerCacheStorageImpl::WebResponsesFromResponses(
    const std::vector<common::ServiceWorkerResponse>& responses) {
