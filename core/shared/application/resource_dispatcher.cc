...
#include "core/shared/application/sync_load_context.h"
#include "core/shared/application/sync_load_response.h"
#include "core/shared/application/url_loader_client_impl.h"
...
void ResourceDispatcher::OnReceivedResponse(
  ...
  if (!IsResourceTypeFrame(request_info->resource_type)) {
    // FIXME: The ApplicationThread::current() doesnt work on worker threads
    //        as the ApplicationThread::current() singleton is allocated on TLS
    //        so its only reachable whe we are running on the main thread.

    NotifySubresourceStarted(//ApplicationThread::current()->main_thread_runner(),
                             ApplicationThread::DeprecatedGetMainTaskRunner(),
...
void ResourceDispatcher::OnRequestComplete(
  ...
  NotifyResourceLoadComplete(ApplicationThread::DeprecatedGetMainTaskRunner(),
                             request_info->render_frame_id,
                             std::move(resource_load_info));
