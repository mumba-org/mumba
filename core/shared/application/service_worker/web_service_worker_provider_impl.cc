...
void WebServiceWorkerProviderImpl::RegisterServiceWorker(
  ...
  DCHECK(callbacks);
  GURL pattern(web_pattern.GetString().Utf8().data(), web_pattern.GetParsed(), web_pattern.IsValid());
  GURL script_url(web_script_url.GetString().Utf8().data(), web_script_url.GetParsed(), web_script_url.IsValid());
  if (pattern.possibly_invalid_spec().size() > url::kMaxURLChars ||
      script_url.possibly_invalid_spec().size() > url::kMaxURLChars) {
    std::string error_message(common::kServiceWorkerRegisterErrorPrefix);
...
void WebServiceWorkerProviderImpl::GetRegistration(
  ...
  DCHECK(callbacks);
  GURL document_url(web_document_url.GetString().Utf8().data(), web_document_url.GetParsed(), web_document_url.IsValid());
  if (document_url.possibly_invalid_spec().size() > url::kMaxURLChars) {
    std::string error_message(common::kServiceWorkerGetRegistrationErrorPrefix);
...
void WebServiceWorkerProviderImpl::GetRegistrations(
  ...
  DCHECK(callbacks);
  if (!context_->container_host()) {
    std::string error_message(common::kServiceWorkerGetRegistrationsErrorPrefix);
    error_message += kLostConnectionErrorMessage;
    callbacks->OnError(blink::WebServiceWorkerError(
        blink::mojom::ServiceWorkerErrorType::kAbort,
        blink::WebString::FromASCII(error_message)));
...
bool WebServiceWorkerProviderImpl::ValidateScopeAndScriptURL(
  ...
  std::string error;
  GURL url(script_url.GetString().Utf8().data(), script_url.GetParsed(), script_url.IsValid());
  GURL scope_url(scope.GetString().Utf8().data(), scope.GetParsed(), scope.IsValid());
  bool has_error = common::ServiceWorkerUtils::ContainsDisallowedCharacter(
      scope_url, url, &error);
  if (has_error)
...
void WebServiceWorkerProviderImpl::OnRegistered(
  ...
  TRACE_EVENT_ASYNC_END2(
      "ServiceWorker", "WebServiceWorkerProviderImpl::RegisterServiceWorker",
      this, "Error", common::ServiceWorkerUtils::ErrorTypeToString(error), "Message",
...
void WebServiceWorkerProviderImpl::OnDidGetRegistration(
  ...
  TRACE_EVENT_ASYNC_END2("ServiceWorker",
                         ...
                         "Error", common::ServiceWorkerUtils::ErrorTypeToString(error),
