...
#include "core/shared/application/service_worker/controller_service_worker_impl.h"
...
#include "core/shared/application/service_worker/service_worker_provider_context.h"
#include "core/shared/application/service_worker/service_worker_timeout_timer.h"
#include "core/shared/application/service_worker/service_worker_type_converters.h"
#include "core/shared/application/service_worker/service_worker_type_util.h"
#include "core/shared/application/service_worker/web_service_worker_impl.h"
#include "core/shared/application/service_worker/web_service_worker_provider_impl.h"
#include "core/shared/application/service_worker/web_service_worker_registration_impl.h"
...
class WebServiceWorkerNetworkProviderImpl
 ...
 public:
  ...
  std::unique_ptr<blink::WebURLLoader> CreateURLLoader(
    ...
    // ApplicationThread* render_thread = ApplicationThread::current();
    ...
    int resp_handler_count = window->CountResponseHandler();//callbacks_.CountResponseHandler(state_);
    for (int i = 0; i < resp_handler_count; i++) {
      CResponseHandler handler;
      void* handler_state = window->GetResponseHandlerAt(//callbacks_.GetResponseHandlerAt(
      i,
      &handler);
      if (handler_state) {
...
blink::WebServiceWorkerClientInfo ToWebServiceWorkerClientInfo(
  ...
  web_client_info.url = blink::KURL(client_info->url.possibly_invalid_spec().data());
...
void ToWebServiceWorkerRequest(const network::ResourceRequest& request,
  ...
  DCHECK(web_request);
  web_request->SetURL(blink::KURL(request.url.possibly_invalid_spec().data()));
...
void ToWebServiceWorkerResponse(const common::ServiceWorkerResponse& response,
  ...
  for (const GURL& url : response.url_list)
    url_list.push_back(blink::KURL(url.possibly_invalid_spec().data()));
...
class ServiceWorkerContextClient::NavigationPreloadRequest final
 ...
 public:
  ...
  void OnReceiveResponse(
    ...
    ApplicationURLLoader::PopulateURLResponse(
      blink::KURL(url_.possibly_invalid_spec().data()), 
      response_head, 
      response_.get(),
...
void ServiceWorkerContextClient::OpenNewTab(
  ...
  context_->service_worker_host->OpenNewTab(
      GURL(url.GetString().Utf8().data(), url.GetParsed(), url.IsValid()), 
...
void ServiceWorkerContextClient::ClearCachedMetadata(const blink::WebURL& url) {
  DCHECK(context_->service_worker_host);
  context_->service_worker_host->ClearCachedMetadata(GURL(url.GetString().Utf8().data(), url.GetParsed(), url.IsValid()));
}
...
ServiceWorkerContextClient::CreateServiceWorkerFetchContext() {
  ...
  scoped_refptr<ChildURLLoaderFactoryBundle> url_loader_factory_bundle = thread->window_dispatcher()->loader_factories();
...
void ServiceWorkerContextClient::Ping(PingCallback callback) {
  //DLOG(INFO) << "ServiceWorkerContextClient::Ping";
