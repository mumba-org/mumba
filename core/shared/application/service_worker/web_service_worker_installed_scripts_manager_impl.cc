 ...
 public:
  ...
  void OnReadable(MojoResult) {
    ...
    if (bytes_read > 0) {
      ...
      chunks_.emplace_back(char_buffer, bytes_read);
    }
...
bool WebServiceWorkerInstalledScriptsManagerImpl::IsScriptInstalled(
  ...
  return base::ContainsKey(installed_urls_, gscript_url);
}
...
WebServiceWorkerInstalledScriptsManagerImpl::GetRawScriptData(
  ...
  ThreadSafeScriptContainer::ScriptStatus status =
      script_container_->GetStatusOnWorkerThread(gscript_url);
  ...
  if (status == ThreadSafeScriptContainer::ScriptStatus::kTaken) {
    script_container_->ResetOnWorkerThread(gscript_url);
    (*manager_host_)->RequestInstalledScript(gscript_url);
    status = script_container_->GetStatusOnWorkerThread(gscript_url);
  }
  ...
  if (status == ThreadSafeScriptContainer::ScriptStatus::kPending) {
    ...
    const bool success = script_container_->WaitOnWorkerThread(gscript_url);
  ...
  return script_container_->TakeOnWorkerThread(gscript_url);
}
