...
scoped_refptr<WebServiceWorkerImpl> ServiceWorkerDispatcher::GetServiceWorker(int handle_id) {
  WorkerObjectMap::iterator found = service_workers_.find(handle_id);
  if (found != service_workers_.end()) {
    return found->second;
  }
  return nullptr;
