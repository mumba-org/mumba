...
blink::WebURL WebServiceWorkerRegistrationImpl::Scope() const {
  return blink::KURL(String::FromUTF8(info_->options->scope.possibly_invalid_spec().data()));
}
...
blink::mojom::ScriptType WebServiceWorkerRegistrationImpl::ScriptType() const {
  return info_->options->type;
}
...
void WebServiceWorkerRegistrationImpl::SetVersionAttributes(
  ...
  ServiceWorkerDispatcher* dispatcher =
      ServiceWorkerDispatcher::GetThreadSpecificInstance();
  DCHECK(dispatcher);
  common::ChangedVersionAttributesMask mask(changed_mask);
