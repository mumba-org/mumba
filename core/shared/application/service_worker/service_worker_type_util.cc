...
class HeaderVisitor : public blink::WebHTTPHeaderVisitor {
 public:
  explicit HeaderVisitor(common::ServiceWorkerHeaderMap* headers) : headers_(headers) {}
  ...
  void VisitHeader(const blink::WebString& name,
    ...
    headers_->insert(common::ServiceWorkerHeaderMap::value_type(
        header_name, header_value));
  }

 private:
  common::ServiceWorkerHeaderMap* const headers_;
};
...
std::unique_ptr<common::ServiceWorkerHeaderMap> GetHeaderMap(
    const blink::WebServiceWorkerResponse& web_response) {
  std::unique_ptr<common::ServiceWorkerHeaderMap> result =
      std::make_unique<common::ServiceWorkerHeaderMap>();
  web_response.VisitHTTPHeaderFields(MakeHeaderVisitor(result.get()).get());
  return result;
}

std::unique_ptr<common::ServiceWorkerHeaderList> GetHeaderList(
    const blink::WebVector<blink::WebString>& web_headers) {
  std::unique_ptr<common::ServiceWorkerHeaderList> result =
      std::make_unique<common::ServiceWorkerHeaderList>(web_headers.size());
...
std::unique_ptr<std::vector<GURL>> GetURLList(
  ...
  std::transform(web_url_list.begin(), web_url_list.end(), result->begin(),
                 [](const blink::WebURL& url) { return GURL(url.GetString().Utf8().data(), url.GetParsed(), url.IsValid()); });
  return result;
}
...
common::ServiceWorkerResponse GetServiceWorkerResponseFromWebResponse(
  ...
  common::ServiceWorkerResponse response = common::ServiceWorkerResponse(
