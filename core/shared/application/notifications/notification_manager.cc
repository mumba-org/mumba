...
#include "core/shared/application/notifications/notification_manager.h"
...
common::NotificationResources ToNotificationResources(
    std::unique_ptr<blink::WebNotificationResources> web_resources) {
  common::NotificationResources resources;
...
void NotificationManager::GetNotifications(
  ...
  const blink::WebURL& scope_url = service_worker_registration_impl->Scope();
  GURL origin = GURL(scope_url.GetString().Utf8(), scope_url.GetParsed(), scope_url.IsValid()).GetOrigin();
  int64_t service_worker_registration_id =
