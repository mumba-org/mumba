...
class PngDecoderState {
 public:
  ...
  PngDecoderState(PNGCodec::ColorFormat ofmt, unsigned char** o)
      : output_format(ofmt),
        output_channels(0),
        bitmap(NULL),
        is_opaque(true),
...
void DecodeInfoCallback(png_struct* png_ptr, png_info* info_ptr) {
  ...
  if (state->bitmap) {
    ...
    size_t sz = state->width * state->output_channels * state->height;
    *state->output = reinterpret_cast<unsigned char *>(malloc(sz));
    state->output_size = sz;
  }
}
...
bool PNGCodec::Decode(const unsigned char* input, size_t input_size,
                      ColorFormat format, unsigned char** output,
...
static bool EncodeSkPixmap(const SkPixmap& src,
  ...
  bool result = SkPngEncoder::Encode(&dst, src, options);
  sk_sp<SkData> data = dst.detachAsData();
  *output = result ? reinterpret_cast<unsigned char *>(data->writable_data()) : nullptr;
  return result;
}
...
// static 
SkiaImage* PNGCodec::DecodeAsImage(const unsigned char* input, size_t input_size) {
  //DLOG(INFO) << "PNGCodec::DecodeAsImage";
  SkCodec::Result result;
  std::unique_ptr<SkStream> src = std::make_unique<SkMemoryStream>(input, input_size);
  std::unique_ptr<SkCodec> codec = SkCodec::MakeFromStream(std::move(src), &result);
  if (result != SkCodec::kSuccess) {
    return nullptr;
  }
  size_t size = codec->getInfo().bytesPerPixel() * codec->getInfo().width();
  void* out = malloc(size);
  //DLOG(INFO) << "PNGCodec::DecodeAsImage: input size = " << input_size << " rowBytes = " << size;
  SkCodec::Result read = codec->getPixels(codec->getInfo(), out, size);
  if (read != SkCodec::kSuccess) {
    return nullptr;
  }
  sk_sp<SkData> data = SkData::MakeFromMalloc(out, size);
  //sk_sp<SkData> data = SkData::MakeUninitialized(size);
  return new SkiaImage(SkImage::MakeRasterData(codec->getInfo(), data, codec->getInfo().minRowBytes()));
}