...
cgrpc_metadata_array *cgrpc_metadata_array_copy(cgrpc_metadata_array *src) {
  ...
  if (src->count > 0) {
    ...
    dst->metadata = (grpc_metadata *) gpr_malloc(dst->capacity * sizeof(grpc_metadata));
    dst->count = src->count;
...
void cgrpc_metadata_array_append_metadata(cgrpc_metadata_array *metadata, const char *key, const char *value) {
  if (metadata->count >= metadata->capacity) {
    ...
    if (metadata->metadata != NULL) {
      metadata->metadata = (grpc_metadata *) gpr_realloc(metadata->metadata, new_capacity * sizeof(grpc_metadata));
    } else {
      metadata->metadata = (grpc_metadata *) gpr_malloc(new_capacity * sizeof(grpc_metadata));
    }
