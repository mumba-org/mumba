...
cgrpc_server *cgrpc_server_create(const char *address, 
  ...
  server->port = grpc_server_add_insecure_http2_port(server->server, address, state, read_cb);
  return server;
}
...
cgrpc_server *cgrpc_server_create_secure(const char *address,
  ...
  // prepare the server to listen
  server->port = grpc_server_add_secure_http2_port(server->server, address, credentials, state, read_cb);
  return server;
}
